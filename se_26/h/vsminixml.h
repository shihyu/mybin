 ...
 * @categories XMLCFG_Functions
 */
int VSAPI vsXMLCFGAddChildText(int iHandle,int iIndex,const char *pText,int TextLen=-1);
 ...
 */
int VSAPI vsXMLCFGFindProperty(int handle,int node,VSPSZ pszName,int caseSensitivePropertyNames=-1);
 ...
 * 
 * @return Returns {@code true} if the node exists and it's name matches {@code name}
 *
 * @categories XMLCFG_Functions
 */
...
/**
 * Add a single XML tag with attributes and potentially embedded text to the 
 * given file.  If the file does not exist, it will create one with just the 
 * single record in it. 
 *  
 * If you need to add a more complex set of records, you can create 
 * pszText as a string by opening a virtual XML document as a string, then 
 * adding to it and saving it to a complete string. 
 * 
 * @param pszFilename            file name to create or append to
 * @param iEncoding              file encoding to use if creating
 * @param iIndentAmount          amount to indent for each level.  -1 to use tabs
 * @param pszDocTypeInfo         document type definition information (optional)
 * @param pszTopLevelTagName     top-level tag name for XML file
 * @param pszRecordTagName       name for record tag to add
 * @param numAttributes          number of attributes to add
 * @param pszAttributeNames      list of attribute names
 * @param pszAttributeValues     paralell list of attribute values
 * @param pszText                (optional) text to add within record
 * 
 ...
 */
extern VSDLLEXPORT
int VSAPI vsXMLCFGAppendRecordToFile(const char *pszFilename, 
                                     const int iEncoding,
                                     const int iIndentAmount,
                                     const char *pszDocTypeInfo,
                                     const char *pszTopLevelTagName, 
                                     const char *pszRecordTagName,
                                     const int numAttributes,
                                     const char *pszAttributeNames[],
                                     const char *pszAttributeValues[],
