 ...
 * filename.  This corresponds to the {@code p_LangId} property of 
 * {@code pszfilename}, not necessarily the literal file extension.
 *  
 * @param pszfilename   name of file to set language type for 
 * @param pszLangId     {@code p_LangId} property for {@code pszfilename} 
 *  
 * @return 0 on success, &lt;0 on error.
 * 
 * @categories Tagging_Functions
...
/**
 * Retrieve the name of the first file included by {@code pszFileName}.
 *
 ...
 * @param include_max    size of buffer for {@code pszIncludeName}
 *
...
/**
 * Retrieve the name of a file that includes {@code pszFileName}.
 *
 ...
 * @param included_max  size of buffer for {@code pszIncluded}
 *
 * @return 0 on success, &lt;0 on error.
 *
 * @categories Tagging_Functions
 ...
 * @param pattern_char_bitset64 case-sensitive bitset of which ascii chars are used in {@code tag_name_pattern}
 * @param pattern_char_bitset32 case-insensitive bitset of which ascii chars are used in {@code tag_name_pattern}
 * 
 ...
 *  
 * @param identifierPrefix     identifier prefix to attempt to correct
 * @param symbolName           symbol name to check (must be non-null) 
 * @param exactNameMatch       do we expect the prefix to match the entire symbol? 
 ...
 *  
 * This function also supports using {@literal 's'} for search options to indicate 
 ...
 * matching {@code pszSearch}, normalize the class name if necessary
 * (find the class/package scope that this class belongs to).
 * If {@code (normalize==1)} attempt to normalize the class name, finding a
 ...
 * derives from.  This will create an entry for {@code class_name}
 ...
 * @param line_number     (optional) line number where reference to class name in {@code file_name}
 *
 * @return 0 on success, &lt;0 on error.
 *
 * @categories Tagging_Functions
...
vsTagNormalizeClasses(const char *class_parents,
                      ...
                      VSHREFVAR template_args = VSHVAR_NULL,
                      VSHREFVAR visited = VSHVAR_NULL, int depth = 0,
                      int line_number = 1);
 ...
 */
EXTERN_C int VSAPI
vsTagListSymbolsInClass(int treewid, int tree_index, 
                        VSHREFVAR tag_files,
                        const char *prefix, 
 ...
 * @param arg_max       number of byte allocated to {@code argument}
 ...
 * <p>
 * If {@code lang} is not specified, the current object must be an 
 * editor control. 
 *
 ...
 * @param aliased_to_len  number of bytes allocated to {@code aliased_to} 
 * @param visited         (optional, reference) hash table of past tagging results 
 * @param depth           (optional) search depth, to cap recursion 
 *
