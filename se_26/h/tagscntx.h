...
/**
 * Codehelp (Context Tagging) option flags.
 */
enum VSCodeHelpFlags : VSUINT64 {
    // no flags
    VSCODEHELPFLAG_NULL = 0,

    // shen on, typing open paren, or other funciton call operator charcter
    // will trigger auto function argument help
    VSCODEHELPFLAG_AUTO_FUNCTION_HELP                  =        0x1,
    // When on, typing . or -> or other member help operators will trigger
    // auto-list members 
 ...
 * @param inner_name_on_right (reference) right-most {@code inner} class name (class name only)
 * @param outer_class_on_left (reference) remaining left-hand {@code outer} class name (class_name - inner_name)
 * @param outer_first_only    (optional, default 0) make outer name first identifier only
 */
EXTERN_C
void VSAPI tag_split_class_name(VSPSZ class_name, 
                                VSHREFVAR inner_name_on_right,
                                VSHREFVAR outer_class_on_left 
                                /*, int outer_first_only=0*/);
...
/**
 * Determine if {@code parent_class} is a parent of {@code child_class}, that is, does
 * {@code child_class} derive either directly or transitively from {@code parent_class}?
 * <p> 
 * For synchronization, threads should perform a tag_lock_context(false) 
 * prior to invoking this function.
 *
 ...
 *
 * @return 
 * Returns {@literal 1} if {@code child_class} derives from {@code parent_class}, 
 * otherwise {@literal 0}.
 */
EXTERN_C
...
/**
 * Lookup {@code symbol} and see if it could be a typedef symbol.  If so,
 * return 1, otherwise return 0.
 * <p>
 * This function ignores class scope.  Simply put, if {@code symbol} is a typedef,
 * anywhere, this function may return true.  Thus, it's should really be used
 * only as an arbiter prior to attempting to match {@code symbol} in context as a
 ...
 * @param depth            depth of recursive search
 *
 * @return 1 if {@code symbol} could be a typedef, 0 otherwise, &lt;0 on error.
 */
EXTERN_C
...
/**
 * Look up {@code symbol} and see if it is a simple preprocessing symbol.
 ...
 * @param line_no          if found in current context, define must be before {@code line}
...
/**
 * Look up {@code symbol} and see if it is a class name.
 * The current object must be an editor control or current buffer.
 * <p> 
 * For synchronization, threads should perform a tag_lock_context(false) 
 * prior to invoking this function.
 *
 * @param symbol           current symbol to look for
 * @param outer_class      class that {@code symbol} must be in
...
/**
 * Look up {@code symbol} and see if it is a template class.
...
/**
 * Look up {@code symbol} and see if it is a package, namespace, module or unit.
 * <p> 
 * For synchronization, threads should perform a tag_lock_context(false) 
 * prior to invoking this function.
 *
 ...
 * @param depth            (optional) depth of recursive search
 *
 * @return &gt;0 if {@code symbol} or prefix of matches package, otherwise returns 0.
 ...
 */
extern SEStringRet VSAPI
tag_get_package_from_class_name(SEStringConst class_name, 
                                VSHREFVAR tag_files,
                                bool case_sensitive,
                                VSHREFVAR visited, int depth=0);
 ...
 * @param depth           (optional) depth of recursive search
 *
 * @return 1 if the definition of the given class {@code search_class} is found,
 ...
 * @param find_first    {@code true} to find first, {@code false} to find next
 */
EXTERN_C
...
tag_list_duplicate_tags(VSHREFVAR taglist, VSHREFVAR filelist,
                        ...
                        /*, int context_flags=SE_TAG_CONTEXT_NULL*/);
