...
#define VSP_CASE_SENSITIVE      96   /* Bit. */
...
#define VSP_ADJUST_COLOR_CODING_START  337   // bool
#define VSP_PERSISTENT_WID             338   // int
#define VSP_WINDOW_COLOR_PROFILE       339   // string
#define VSP_BUFFER_COLOR_PROFILE       340   // string
#define VSP_MINIMAP_COLOR_PROFILE      341   // string
...
#define VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME 0x400 /* Delete temp file probably created by "e +t" */
...
/* 
When on, prefix area is displayed for read-only files.
If non-zero and buffer is read only, VSLCBUFFLAG_READWRITE is
added to VSP_LCBUFFLAGS when buffer is created.
*/
#define VSOPTION_LCREADONLY            43  
...
/**
 * Show symbol coloring, references, and other stream marker overlays 
 * in the minimap.
 */
#define VSOPTION_MINIMAP_SHOW_SYMBOL_COLORING 130

/**
 * Options controlling how mouse scroll events are treated in the 
 * document tabs.  Enable/disable vertical scrolling, horizontal 
 * scrolling, and whether or not scrolling changes tabs. 
 */
#define VSOPTION_DOCUMENT_TABS_SCROLL_FLAGS 131
   #define VSDOCUMENT_TABS_SWITCH_ON_VERT_SCROLL 0x01
   #define VSDOCUMENT_TABS_SWITCH_ON_HORZ_SCROLL 0x02

/**
 * non-zero if -fn invocation option was specified.
 */
#define VSOPTION_OPEN_AS_FILE_NOT_WORKSPACE 132
...
/**
 * Places {@code Len} bytes of data starting from the byte offset specified
 * by {@code SeekPos} into {@code pszBuf}.  The resulting string is always
 * null terminated.  The buffer {@code pszBuf} must be large enough to
 * contain {@code Len}+1 bytes of data.
 *
 ...
 * {@code pszBuf} must be large enough to
 * contain {@code Len}+1 bytes of data.
 *
 * @param appendNull	If false, do not append a null to
 * {@code pszBuf}. Buffer will be entirely filled with data read
 * from
 * {@code wid}.
 *
 * @see vsInsertLine
...
/**
 * Places the current line into {@code pszBuf}.  The resulting string is
 * always null terminated.  No more than {@code BufLen} bytes are
 * written.  Use the {@link vsQLineLength} function to determine the
...
/**
 * This function is identical to the {@link vsGetLine} function, except
...
/**
 * This function is identical to the {@link vsInsertLine} function, except
...
/**
 * Sets the current line of the current buffer to {@code pBuf}.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 ...
 *
 * @param BufLen	Number of characters in {@code pBuf}. If
 * {@code BufLen} is -1, {@code pBuf} must be
 * null terminated.
 *
 * @see vsGetLine
 * @see vsInsertLine
...
/**
 * This function is identical to the {@link vsReplaceLine} function,
...
/**
 * {@code psz} is displayed on the message line.
 *
 * @see vsMessage
 *
 * @categories Miscellaneous_Functions
...
/**
 * Places cursor at first line and first column of buffer.  {@code wid}
...
/**
 * Places text cursor at end of last line of buffer.  {@code wid} identifies
...
/**
 * Moves cursor down the number of lines specified by {@code Noflines}.
 ...
 * {@code Noflines}, including wrapped long lnes (each wrapped
...
/**
 * Moves cursor up the number of lines specified by {@code Noflines}.  If
 * the destination line is not in view, it is center scrolled or smooth
 * scrolled into view.  {@code wid} identifies the object to be operated on.
 ...
 * non-zero value for {@code IncludeNLChars} if you want to include the
 ...
 * {@code IncludeNLChars} if you want to
 ...
 * characters.  Specify a non-zero value for {@code IncludeNLChars} if
 ...
 * as {@link vsSelectLine} and {@link vsCopyToCursor}.  Specify a
 * non-zero value for {@code AllocBookmark} to allocate a book mark.
 * Bookmarks can not be deleted.
 *
 * <p>IMPORTANT: The active selection or selection showing may not be
 * freed by the {@link vsFreeSelection} function.  Use the
 * {@link vsShowSelection} function to make another mark active before
 ...
 * {@code mark_id}.  <b>_free_selection</b> will not free the active
 * selection (the one that is seen on screen).  {@code mark_id} is a handle
 * to a selection returned by {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.  A {@code mark_id} of -1 is not
 * allowed.  The default selection or selection show
 * ({@link vsShowSelection}) can be freed.
 *
...
/**
 * Clears selection specified.  {@code mark_id} is a handle to a selection
 * returned by {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.  A {@code mark_id} of -1 identifies the
 * active selection.
 *
 * @return Returns 0 if {@code mark_id} is a valid selection handle.  Otherwise
 ...
 * processing lines of text.  The first {@link vsSelectLine} becomes the
 * pivot point.  Subsequent {@link vsSelectLine} calls will extend the
 ...
 * current object.
 *
 * @param markid	{@code mark_id} is a selection handle
 * allocated by {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.  A
 * {@code mark_id} of -1 specifies the active
 ...
 * boundaries.  The first {@link vsSelectChar} becomes the pivot point.
 * Subsequent {@link vsSelectChar} calls will extend the selection
 ...
 * columns of text.  The first {@link vsSelectBlock} becomes the pivot
 * point.  Subsequent {@link vsSelectBlock} calls will extend the
 * selection between the pivot point and the cursor.
 *
 * @return Returns 0 if successful.  Possible returns are
 * TEXT_NOT_SELECT_RC.  and
...
/**
 * Copies the selection specified by {@code mark_id} to cursor.  Character
 ...
 *
 * @param markid	{@code mark_id} is a selection handle
 * allocated by the {@link vsAllocSelection} or
 * {@link vsDuplicateSelection} function.  A
 * {@code mark_id} of -1 specifies the active
 * selection or selection showing and is always
 * allocated.
 *
...
/**
 * Runs the command specified.  {@code pszCommand} can be any
 ...
 * the command retrieve buffer.  {@code wid} identifies the object to be
...
/**
 * Retrieve actual color profile name being used by an editor control window.
 * 
 * @param wid            Editor control window id.
 * @param pszProfileName (output) Actual color profile name used to draw text.
 * @param nProfileName   Size of color profile name buffer.
 * 
 * @return 0 on success.
 */
EXTERN_C
int vsGetWindowColorProfile(int wid, char* pszProfileName, int nProfileName);
...
/**
 * When the {@link VSP_TRUNCATELENGTH} is non-zero, replace
 ...
 * {@link vsSearchQNofSkipped} and {@link vsSearchQSkipped} to
 ...
 * Skipped lines data returned by the {@link vsSearchQNofSkipped} and
 * {@link vsSearchQSkipped} functions.
 *
 ...
 * When the {@link VSP_TRUNCATELENGTH} is non-zero, replace
 * operations are skipped if the resulting line becomes longer than the
 * truncation length.  The {@link vsSearchInitSkipped} function is used
 * in conjunction with {@link vsSearchQNofSkipped} and
 * {@link vsSearchQSkipped} to return information about what was
 ...
 * {@link vsSearchQNofSkipped} and {@link vsSearchQSkipped}
 * functions.
 *
...
/**
 * @return Returns 0 if there are no lines longer than the {@code AllowedLineLen}
 ...
 * variable is not found, {@code DefaultValue} is returned.
 *
...
/**
 * Restores inactive bookmarks for the file specified by {@code wid}.
 ...
 *
 * @param free_markid	When non-zero, {@link vsFreeSelection} is
 ...
 * if {@code markid} is an active bookmark.
 *
 ...
 * Specify 0 if {@code markid} is an active
 * bookmark.
 * 
 * @param  persistent_wid Optional window id so the bookmark can
 *                        be restored to the same window if
 *                        possible.
 *
 * @see vsBookmarkAdd
 * @see vsBookmarkRemove
 ...
 * {@code vsbmflags}	One or more of these flags must be in the
 * bookmark found.
 *
 * @see vsBookmarkAdd
 * @see vsBookmarkRemove
...
/**
 * Copies corrensponding message into {@code pszMessage}.  Set to
 * "Message not available" for a non-existant error code.
 *
 * @return Returns {@code pszMessage}.
 *
...
void VSAPI vsMessageFindExe(char *pszExe,  // MAXFILENAME
                            const char *arg0);
/**
 * Copies current contents of message line into {@code pszMessage}.
 *
 * @return Returns {@code pszMessage}.
 *
 * @param pszMessage	Output buffer for message.  This can be 0.
 ...
 * you use {@link vsFindIndex} to lookup your
 ...
 *
 * @return Returns {@code pszDocumentName}.
 *
 * @param buf_id	          Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID), 
 *                            {@link vsBufEdit}, or vsBufMatch.
 * @param documentName	      Set to document name (VSP_DOCUMENTNAME). 
 *                            If {@code defaultToBufferName} is non-zero
 *                            and the document name is "" then this
 *                            parameter is set to the buffer name (VSP_BUFNAME).
 * @param MaxDocumentName	  Number of characters allocated to {@code pszDocumentName}.
 * @param pMaxDocumentNameLen If this is not 0, this is set to the number of
 *                            characters needed to be allocated.
 * @param defaultToBufferName Indicates that the {@code pszDocumentName} parameter
 ...
 *
 * @param buf_id	    Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID),
 *                      {@link vsBufEdit}, or {@link vsBufMatch}.
 * @param pszBufName	Name of buffer corresponding to {@code buf_id} specified.
 ...
 * @param buf_id	    Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID),
 *                      {@link vsBufEdit}, or {@link vsBufMatch}.
 * @param bufferName	(output) Name of buffer corresponding to {@code buf_id} specified.
 *
 * @categories Buffer_Functions
 */
 ...
 *         buffer id specified.  Returns {@code pszLanguageId}.
 *
 * @attention
 * This function is thread-safe.
 *
 * @param buf_id	    Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID),
 *                      {@link vsBufEdit}, or {@link vsBufMatch}.
 * @param pszLanguageId	Name of language setting corresponding to {@code buf_id} specified.
 ...
 * @param buf_id	    Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID),
 *                      {@link vsBufEdit}, or {@link vsBufMatch}.
 * @param languageId	Name of language setting corresponding to {@code buf_id} specified.
 *
 * @categories Buffer_Functions
 */
 ...
 *         buffer id specified.  Returns {@code pszModeName}.
 *
 * @attention
 * This function is thread-safe.
 *
 * @param buf_id	       Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID),
 *                         {@link vsBufEdit}, or {@link vsBufMatch}.
 * @param pszModeName      Name of language mode corresponding to {@code buf_id} specified.
 *                         This can be NULL, in which case, only the length of
 ...
 * @param buf_id	       Buffer id returned by {@link vsPropGetI}({@code wid},VSP_BUF_ID),
 *                         {@link vsBufEdit}, or {@link vsBufMatch}.
 * @param modeName         (output) Name of language mode corresponding to {@code buf_id} specified.
 *
 * @categories Buffer_Functions
 */
 ...
 * dialog box.  Call the {@link vsPrintSetConfig} function to set print
...
#define VSLANGPROPNAME_COLOR_PROFILE "color_profile"
#define VSLANGPROPNAME_ALT_COLOR_PROFILE "alt_color_profile"
#define VSLANGPROPNAME_ALT_COLOR_FILESPEC "alt_color_filespec"
 ...
 *                      ("Tools", "Options...", "File Extension Manager")
 *
 * @param pszInheritsFrom  {@code pszExtension} is set
 ...
 *                   ("Tools", "Options", "Language Setup...").
 *
 * @param pszParentLangId  {@code pszLangId} is set to
 *                         inherit language specific callbacks
 *                         from the language specified by
 *                         {@code pszParentLangId}.
 ...
 *                      {@link vsGetDefaultLanguageOptions}
 ...
 * Note that {@code pszLangId} must be a real language ID, not a
 ...
 * given language?
 * <p>
 * If {@code pszLangId} is not specified, the current object
 * must be an editor control.
 *
 * @attention
 * This function is thread-safe provided
 * {@code pszLangId} is not NULL or the empty string.
 *
 * @param parent        language ID to compare to
 * @param pszLangId     current language ID
 *                      (default={@link p_LangId})
 *
 * @return {@code true} if the language matches, {@code false} otherwise.
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Tagging_Functions
 * @since 13.0
 ...
 * {@link vsMenuLoad},
 * {@link vsMenuFind}, or
 * {@link vsMenuGetState}.  This handle may
 * also be a handle to menu resource returned
 * by {@link vsFindIndex} or
 * {@link vsNameMatch}.
 *
 ...
 * {@code by_category} parameter.
 *
 * @param pmenu_handle	Handle to loaded menu which contains
 * menu item found.
 *
 * @param pposition	Position of menu item found within
 * {@code pmenu_handle}.  0 is the position of
 * the first menu item.  Use the vsMenuQInfo
 * function to determine the number of items in
 * a loaded menu.
 *
 * @param by_category	May be {@literal 'M'} or  {@literal 'C'}.  If the {@literal 'M'} option is given,
 * {@code pszCommand} is the command of the
 * menu item to be found.  If the {@literal 'C'} option is
 * given,  {@code pszCommand} is the category
 * of the menu item to be found.  Defaults to
 * {@literal 'C'} if not specified.
 *
 * @example
 * <pre>
 * void VSAPI dllproc ()
 ...
 * {@code by_category} parameter.
 *
 * @param by_category	May be {@literal 'M'}, {@literal 'C'}, {@literal 'P'}.  If the {@literal 'M'} option is
 * given, {@code pszFindCommand} is the
 * command of the menu item to be found. If
 * the {@literal 'C'} option is given,
 * {@code pszFindCommand} is the category of
 * the menu item to be found.  If the {@literal 'P'} option
 * is given, {@code pszFindCommand} is an
 * integer (you must cast
 * {@code pszFindCommand} to int) position
 * (0..vsMenuQInfo({@code menu_handle})-1)
 * of the menu item with in
 * {@code menu_handle} in ASCIIZ format.
 * Defaults to {@literal 'C'}.
 *
 ...
 * {@code pszItemText}.  We recommend
 * VSMAXMENUSTRING.
 *
 * @param pszCommand	For a sub-menu, {@code command} is set to a
 ...
 * {@code pszHelpCommand}.  We recommend
 * VSMAXMENUSTRING.
 *
 ...
 * {@code pszHelpMessage}. We recommend
 * VSMAXMENUSTRING.
 *
 ...
 * vsMenuFind, or vsMenuGetState.
 *
 * @param position	Position within menu {@code menu_handle}.
 ...
 * {@link vsMenuSetState} function for more
 ...
 * resource as a pop-up menu with the {@link vsMenuShow} function
 * requires that you specify {@literal 'P'} as the second argument to this function.
 * {@code menu_index} is a handle to a menu resource returned by
 * {@link vsFindIndex} or {@link vsNameMatch}.
 *
 ...
 * {@link vsNameMatch}.
 *
 * @param menu_option	Specify {@literal 'P'} for a popup-menu or {@literal 'M'} for
 * menu bar.
 *
 * @example
 * <pre>
 ...
 *
 * @param by_category	May be {@literal 'M'}, {@literal 'C'}, {@literal 'P'}.  If the {@literal 'M'} option is
 * given, {@code psaFindCommand} is the
 * command of the menu item to be found. If
 * the {@literal 'C'} option is given,
 * {@code pszFindCommand} is the category of
 * the menu item to be found.  If the {@literal 'P'} option
 ...
 * </i>to int) position
 * (0..{@link vsMenuQInfo}(<i>menu_handle
 ...
 * 	{@code x} coordinate represents left most
 * corner of menu.</dd>
 * <dt>VSVPM_CENTERALIGN</dt><dd>
 * 	{@code x} coordinate represents horizontal
 * center of menu</dd>
 * <dt>VSVPM_RIGHTALIGN</dt><dd>
 * 	{@code x} coordinate represents right side of
 * menu.</dd>
 * </dl>
 *
 ...
 * it can be restored later by {@link vsRestorePos2}.  It is safe to make
 * edits in between calls to vsSavePos2 and vsRestorePos2.</p>
 *
 * <p>IMPORTANT:  If you do not call {@link vsRestorePos2}, you need to
 * call {@link vsFreeSelection} to free the selection allocated.</p>
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 ...
 * current object.
 *
 * @param mark_id	mark id allocated by {@link vsSavePos2}.
 *
 ...
 * {@code pbuf} must have been initialized by {@link vsSavePos}.   NO
 * EDITS should have been made to this buffer inbetween the vsSavePos
 * and vsRestorePos calls.  Use {@link vsSavePos2} and
 * {@link vsRestorePos2} to save and restore where edits are made.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 * @param pbuf	Structure initialized by {@link vsSavePos}.
 *
 * @categories CursorMovement_Functions
 *
 */
...
/**
 * <p>Saves the editor cursor position and scroll position information so that
 * it can be restored later by {@link vsRestorePos}. NO EDITS should be
 * made to this buffer inbetween the vsSavePos and vsRestorePos calls.
 * Use {@link vsSavePos2} and {@link vsRestorePos2} to save and
 ...
 * {@link vsSearchPromptReplace}, {@link vsCommandReplace}, or
 ...
 * /{@code FindString}/{@code ReplaceString}/{@code Options}") except
 * that {@link vsRefresh} is not called, a new undo step is never started,
 ...
 * {@link vsSearch} for list of most options.
 * The following addition options may be
 * specified:
 *
 * <dl>
 * <dt><i>*</i></dt><dd>Make changes without prompting.</dd>
 * <dt>{@code P}</dt><dd>Wrap to beginning/end when string not
 * found.</dd>
 * </dl>
 *
 * @appliesTo Edit_Window, Editor_Control
 ...
 * /{@code FindString}/{@code Options}") except that {@link vsRefresh} is
 ...
 * {@link vsRefresh} is not called, a new undo step is never started, and
 * code is not generated by macro recording.</p>
 *
 * <p>The {@link VSP_MODIFY} property is turned off if the output
 ...
 *
 * <dt>+{@code ddd}</dt><dd>Save file without line end characters and pad or
 * truncate lines so that each line is {@code ddd}
 ...
 *
 * <dt>+F{@code ddd}</dt><dd>Save file using ASCII character {@code ddd} as the
 ...
 *
 * <dl>
 * <dt>{@literal 'C'}</dt><dd>Returns the index of the last command or
 * function executed by a key,
 * {@link vsCallKey}, or {@link vsExecute}.<br><br>
 *
 * Note that this is different than the Slick-C&reg;
 * <b>last_index</b> function which updates
 * this value when <b>call_index</b> is
 * called.  Call {@link vsSetLastIndex} to
 * explicitly update this value.</dd>
 *
 * <dt>{@literal 'L'}</dt><dd>Returns the index of the last command or
 * function executed by a key or the
 * {@link vsCallKey} function.</dd>
 *
 * <dt>{@literal 'K'}</dt><dd>Returns the index of the last event table used
 * to determine a key binding.</dd>
 *
 * <dt>{@literal 'W'}</dt><dd>Returns the command line/menu wait value.
 * This flag is used by a command to determine
 * whether it was invoke by {@link vsExecute}
 * (command line or menu) or a key press.
 * This flag is set to non-zero when
 * {@link vsExecute} is called and 0 when a
 * key is pressed or {@link vsCallKey} is
 * called.</dd>
 *
 * @see vsQLastIndex
 * @see vsQPrevIndex
...
/**
 * @return Sets a last index corresponding to {@code option} given.
 *
 * @param option	Indicates last index information to Set.
 *
 * <dl>
 * <dt>{@literal 'C'}</dt><dd>Set the index of the last command or
 * function executed by a key,
 * {@link vsCallKey}, or {@link vsExecute}.</dd>
 *
 * <dt>{@literal 'L'}</dt><dd>Set the index of the last command or
 * function executed by a key or the
 * {@link vsCallKey} function.</dd>
 *
 * <dt>{@literal 'K'}</dt><dd>Set the index of the last event table used to
 * determine a key binding.</dd>
 *
 * <dt>{@literal 'W'}</dt><dd>Set the command line/menu wait value.
 * This flag is used by a command to determine
 * whether it was invoke by {@link vsExecute}
 * (command line or menu) or a key press.
 * This flag is set to non-zero when
...
/**
 * @return Returns a last index corresponding to {@code option} given.
 *
 * @param option	Indicates last index information to return.
 *
 * <dl>
 * <dt>{@literal 'C'}</dt><dd>Returns the index of the previous command
 * or function executed by a key,
 * {@link vsCallKey}, or {@link vsExecute}.
 ...
 * this value when <b>call_index</b> is
 * called.  Call {@link vsSetPrevIndex} to
 * explicitly update this value.</dd>
 *
 * <dt>{@literal 'L'}</dt><dd>Returns the index of the previous command
 ...
 *
 * <dl>
 * <dt>{@literal 'C'}</dt><dd>Set the index of the previous command or
 * function executed by a key,
 * {@link vsCallKey}, or {@link vsExecute}.</dd>
 *
 * <dt>{@literal 'L'}</dt><dd>Set the index of the previous command or
 * function executed by a key or the
 * {@link vsCallKey} function.</dd>
 * </dl>
 *
 ...
 * current object.
 *
 * @param option	{@literal 'D'} specifies to return the position of mouse
 * in pixels relative to the screen.  '\0' specifies
 * to return the position of the mouse in pixels
 * relative to the window specified.  {@literal 'M'}
 * specifies to return the position in the scale
 * mode (VSP_SCALEMODE) of {@code wid}
...
/**
 * @return Returns {@code true} if the position of the mouse for the last event
...
/**
 * @return If successful, returns 0 and places current word in {@code pszWord}.
 ...
 * {@code pMaxWordLen}	If this is not 0, this is set to the number of
 ...
 * {@code pszWord}.  Otherwise a non-zero value is returned.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 * @param {@code x}, {@code y}	Pixel coordinates under which to check for
 * word or selection.
 *
 * @param XYAreInScreenCoordinates
 * 	If this is not zero, ({@code x},{@code y}) are in
 * screen coordinates, otherwise
 * ({@code x},{@code y}) are relative to the editor
 * control specified by {@code wid}.
 *
 * @param MaybeReturnSelectedText
 * 	If non-zero and a single line of selected text
 * exists under ({@code x},{@code y}), the
 * selected text is returned.  Otherwise, the
 * word under ({@code x},{@code y}) is returned.
 *
 ...
 * This function is only valid when {@code wid} is an editor control.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 ...
 *
 * @param XYAreInScreenCoordinates
 * 	If this is not zero, ({@code x},{@code y}) are in screen coordinates,
 *    otherwise ({@code x},{@code y}) are relative to the editor control
 *    specified by {@code wid}.
 *
 ...
 * visible window created.  Use the {@link vsActivateWindow} and/or the
 * VSP_WINDOWID property to switch windows.  The call
 * {@link vsPropGetI}(0,VSP_WINDOWID) returns the current window id.</p>
 *
 * @param pTempWindowId	Set to newly created window id.
 * {@code pOrigWindowId}	Set to current window id.  There is always a
 * valid current window id.
 *
 * @example
 * <pre>
 ...
 *
 * @param doDeleteBuffer	When {@code doDeleteBuffer} is 0, the
 ...
 * {@link vsActiveWindow} and/or the VSP_WINDOWID property to switch
 * windows.  The call {@link vsPropGetI}(0,VSP_WINDOWID) returns the
 * current window id.</p>
 *
 * @return If successful, returns 0 and the temp window/buffer is active.
 * Otherwise a negative error code is returned.
 ...
 * {@code pszFilename} from disk.</dd>
 *
 * <dt>"+bi <i>buf_id></i>"</dt><dd>Ignore {@code pszFilename} and
 * create new window of buffer corresponding to
 * {@code buf_id}.</dd>
 *
 * <dt>"+d"</dt><dd>Always read {@code pszFilename} from disk
 * even if this file is already loaded in the
 * editor.</dd>
 *
 * <dt>"+b"</dt><dd>Only succeed if a buffer already exists of the
 * name {@code pszFilename}.</dd>
 * </dl>
 *
...
/**
 * Creates a modal dialog box and displays {@code pszMessage} and
 * various buttons.  {@code pszMessage} may contain carriage return
 * characters to break a string message into multiple lines.  By default,
 * {@code pszMessage} is broken into multiple lines on word boundaries
 * if necessary.
 *
...
/**
 * @return Returns {@code true} if the given file has support
...
/**
 * @return Returns {@code true} if the given file has support for a tagging
...
/**
 * Converts {@code pszSrcFilename} to a path specification which is
 * relative to the directory specified by {@code pszToDir}.
 *
 * @return Returns {@code pszDestFilename}.
 *
 * @param pszDestFilename	Result of convert {@code pszSrcFilename} to
...
/**
 * Converts {@code pszSrcFilename} to a fully qualified path.
 *
 * @return Returns {@code pszDestFilename}.
 *
 * @param pszDestFilename	Result of convert {@code pszSrcFilename} to
 ...
 * {@code pszToDir}.  This may already be fully
 * qualified.
 *
 * @param pszToDir	Directory with or without trailing backslash
 * (UNIX: forward slash).  If 0 or "" is given,
...
/**
 * @return Returns {@code pszResult}.  To set the emulation, call
 * {@link vsExecute} with the command "emulate {@code emu_mode}",
 * where {@code emu_mode} is one of the following value, for example,
 ...
 * {@code wid} the output editor control for recorded source.
 *
...
/**
 * Use this function and the {@link vsMacroQRecordSource} function to
 * determine whether your macro command needs to generate source
 * code.  If you have written a command, you should use
 * {@link vsMacroQRecordSource} to check whether you need to
 * generate source.  If your command executes another command with
 * {@link vsExecute} or {@link vsCallIndex} you may want to use
 * {@link vsMacroSetRecordSource} to turn on or off macro recording
 ...
 * value returned by {@link vsMacroQRunning}.
 *
...
/**
 * Use {@link vsMacroQRecordSource} and the
 * {@link vsMacroQRecording} function to determine whether your
 * macro command needs to generate source code.  If you have written a
 * command, you should use {@link vsMacroQRecordSource} to check
 * whether you need to generate source.  If your command executes
 * another command with {@link vsExecute} or {@link vsCallIndex}
 * you may want to use {@link vsMacroSetRecordSource} to turn on or
...
/**
 * Determines what {@link vsMacroQRecordSource} will return to a
 * command called by {@link vsExecute} or {@link vsCallIndex}.  By
 * default,  will Use {@link vsMacroQRecordSource} and the
 * {@link vsMacroQRecording} function to determine whether your
 * macro command needs to generate source code.  If you have written a
 * command, you should use {@link vsMacroQRecordSource} to check
 * whether you need to generate source. If your command executes
 ...
 * {@link vsExecute} or {@link vsCallIndex}
 ...
 * which calls the {@link vsQIdle}() function.
 *
 * @example (Windows)
 * <pre>
 * static HHOOK ghook;
...
/**
 * @return Returns string corresponding to {@code option} specified.
 *
 ...
 *
 * @param TextLen	Number of characters in {@code pText}.  -1
 * means {@code pText} is null terminated and
...
/**
 * Gets column width setting for column {@code i}. Column text is
...
/**
 * Sets column width of column {@code i}.   Column text is separated with
 * a tab character.
 *
 * @return Returns 0 if successful.
 *
...
#define VSTREENODE_STRIKE_THRU       0x800   // node caption is strikethru
...
#define TREE_SEARCH_CONTAINS  0x20
 ...
 *        <dt>VSTREE_OVERLAY_BITMAP1</dt><dd>Treat {@code iOverlayBMIndex} as an overlay index.
 *            Otherwise, {@code iOverlayBMIndex} is ignored.</dd>
 *        </dl>
 *
...
int VSAPI vsTreeAddItem(int iWID,
                        ...
                        int iShowChildren,
                        int iNodeFlags=0,
                        VSHVAR hvarUserInfo=VSHVAR_NULL);
void VSAPI vsTreeSortCaption(int wid, int iHandle, const slickedit::SEString &primaryOptions, const slickedit::SEString &secondaryOptions="", const slickedit::SEString &exInfo="");
/**
 ...
 */
int VSAPI vsTreeGetPrevIndex(int wid, int iHandle, int iSearchHidden VSDEFAULT(0), int iSearchCollapsed VSDEFAULT(0));
 ...
 * @param iWID Window ID of tree.  Specify 0 if it is the
 *            current window
 * @param iIndex Tree node index to find out the hidden state of
 * 
 * @return int non-zero if hidden
 */
EXTERN_C
int VSAPI vsTreeNodeIsHidden(int iWID, int iIndex, int iNewValue = -1);

EXTERN_C
void VSAPI vsTreeSetAllNodesVisible(int iWID, int iSetNodeVisible = -1, int iParentIndex = -1);
/**
 * @return 
...
/**
 * @return If {@code undo_option} is {@literal 'U'}, NOTHING_TO_UNDO_RC if there is
 * no undo information.  If {@code undo_option} is {@literal 'R'},
 ...
 * current object.
 *
 * @param undo_option	Specify {@literal 'U'} to test for undo information.
 * Specify {@literal 'R'} to test for redo information.
 *
 * @appliesTo Edit_Window, Editor_Control
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods
 ...
 * {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.  A
 * {@code mark_id} of -1 identifies the active
 * selection.
 *
...
/**
 * This function retrieves the selected text.  {@code pszBuf} is set to ""
 ...
 * current object.
 *
 * @param mark_id	{@code H}andle to a selection returned by
 * {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.  A
 * {@code mark_id} of -1 identifies the active
 * selection.
 ...
 *
 * @return Returns 0 if {@code wid} and {@code markid} are valid.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 ...
 * {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.  -1 specifies
 * current selection.
 *
 ...
 * this column (
 * {@link vsQSelectType}({@code markid},{@literal 'I'}) ).
 *
 ...
 * {@code pszLoadOptions} is "+bi" or "+b".
 *
 ...
 * open to file.</dd>
 *
 * <dt>"+E:{@code ddd}"</dt><dd>Expand tabs to spaces.  This expands tabs in
 * increments of {@code ddd}.</dd>
 *
 * <dt>"+E"</dt><dd>Expand tabs to spaces. If your tabs settings
 * for the file being loaded are of the form
 * "+<{@code increment}>" (like +4), then  tabs
 * are expanded in increments of
 * {@code increment}.  Otherwise tabs are
 * expanded in increments of 8.  To set your
 * tabs in a form "+<{@code increment}>", bring
 ...
 * {@code pszTempOption}.  See
 * {@link vsLoadFiles} function for a list of
...
/**
 * @return Returns line number ({@link VSP_LINE}) corresponding to
 * {@code RealLineNumber}.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 * @param RealLineNumber	Real line number ({@link VSP_RLINE}).
 *
...
/**
 * @return Returns line number ({@link VSP_LINE}) corresponding 
 *         to offset
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
...
/**
 * @return Returns real line number ({@link VSP_RLINE}) corresponding to
 * {@code LineNumber}.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 * @param LineNumber	Old line number ({@link VSP_LINE}).
 *
 * @see vsQLineNumberFromReal
 * @see vsQLineNumberFromOld
 * @see vsQRealLineNumberFromLine
...
/**
 * @return Returns old line number ({@link VSP_OLDLINENUMBER})
 * corresponding to {@code LineNumber}.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 * @param LineNumber	Line number ({@link VSP_LINE}).
 *
 * @see vsQLineNumberFromReal
 * @see vsQLineNumberFromOld
 * @see vsQRealLineNumberFromLine
 ...
 * VSREFRESH_LINE).  If {@code linenum} is
 ...
 * {@link vsBufEdit} does not call this callback.
 *
 ...
 * <p>This callback returns the {@code index} to bitmap for the line number
 * specified.  {@code index} must be an index into the names table of a
 * registered bitmap.  Use {@link vsUpdatePicture} to register a bitmap.
 ...
 * {@code OldLineNum}	Old line number
 * (VSP_OLDLINENUMBER).
 *
 *
 * @categories Editor_Control_Callback_Functions
 ...
 * {@link vsTagSetTagFiles} function is called.
 *
 ...
 * {@code pszTagFileName} is "", the value of
 ...
 * function.
 *
 * @param x	Recommended {@code x} screen coordinate
 * to display menu.
 *
 * @param y	Recommended {@code y} screen coordinate
 * to display menu.
 *
 * @categories Editor_Control_Callback_Functions
 *
...
/**
 * <pre>void (VSAPI * vspfnChangeColorProfile)(int wid, VSPSZ pszProfileName);</pre>
 * <p>
 * The COLOR_PROFILE_CHANGE callback function is used to notify of a 
 * color profile change in an editor control window.
 * <p>
 * Actual color profile name being used by an editor control window.
 * <p>
 * Note:<br>
 * This callback is not called the first time an editor control
 * is created. Use vsGetWindowColorProfile after calling
 * vsCreateEditorCtl to retrieve the color profile name when an
 * editor control window is created.
 *
 * @param wid            Editor control window id.
 * @param pszProfileName Actual color profile used to draw text.
 *
 * @categories Editor_Control_Callback_Functions
 */
VSCALLBACK_WIN_COLOR_PROFILE_CHANGE=55,
 ...
 *
 * <ul>
 * <li>{@link VSCALLBACK_WIN_UPDATE_EDITOR_STATUS}</li>
 * <li>{@link VSCALLBACK_WIN_GOTFOCUS}</li>
 * <li>{@link VSCALLBACK_WIN_LOSTFOCUS}</li>
 * <li>{@link VSCALLBACK_WIN_CMDLINE_FUNCTIONS}</li>
 * <li>{@link VSCALLBACK_WIN_MESSAGE}</li>
 * <li>{@link VSCALLBACK_WIN_GET_MESSAGE}</li>
 * <li>{@link VSCALLBACK_WIN_FONT_CHANGE}</li>
 * <li>{@link VSCALLBACK_APP_QUERY_END_SESSION}</li>
 * <li>{@link VSCALLBACK_APP_MENU_ADD_FILE_HISTORY}</li>
 * <li>{@link VSCALLBACK_APP_MENU_ADD_WORKSPACE_HISTORY}</li>
 * <li>{@link VSCALLBACK_APP_POPUP_MENU}</li>
 * <li>{@link VSCALLBACK_APP_POPUP_MENU2}</li>
 * <li>{@link VSCALLBACK_APP_ADD_BUFFER}</li>
 * <li>{@link VSCALLBACK_APP_RENAME_BUFFER}</li>
 * <li>{@link VSCALLBACK_BUF_QDEBUGBITMAP}</li>
 * <li>{@link VSCALLBACK_BUF_SET_LINE_COLOR}</li>
 * <li>{@link VSCALLBACK_APP_READ_ONLY_ERROR}</li>
 * <li>{@link VSCALLBACK_APP_KEY_OR_MOUSE_EVENT_READ}</li>
 * <li>{@link VSCALLBACK_APP_BPMQ_DEFAULT_BITMAP}</li>
 * <li>{@link VSCALLBACK_APP_MESSAGEBOX}</li>
 * <li>{@link VSCALLBACK_APP_DISPATCH_MESSAGE}</li>
 * <li>{@link VSCALLBACK_APP_MODAL_WAIT}</li>
 * <li>{@link VSCALLBACK_APP_TAG_FILE_REFRESH}</li>
 * <li>{@link VSCALLBACK_APP_TAG_FILE_ADD_REMOVE}</li>
 * <li>{@link VSCALLBACK_APP_TAG_FILE_MODIFIED}</li>
 * <li><B>VSCALLBACK_APP_MFFIND_ACTIVATE_TOOLBAR_
 * AND_SEARCH_TAB</B></li>
 * <li>{@link VSCALLBACK_APP_MFFIND_IS_TOOLBAR_VISIBLE}</li>
 * <li>{@link VSCALLBACK_APP_MFFIND_QEDITOR_CTL_WID}</li>
 * <li>{@link VSCALLBACK_APP_MFFIND_CLOSE_TOOLBAR}</li>
 * <li>{@link VSCALLBACK_APP_DELETE_WINDOW}</li>
 * <li>{@link VSCALLBACK_APP_REFRESH_BREAKPOINTS}</li>
 * <li>{@link VSCALLBACK_APP_BREAKPOINT_LINE_DELETED}</li>
 * </ul>
 *
 ...
 * {@link vsBufEdit} or {@link vsCommandName} is called.</p>
 *
 * <p>This callback is typically used to set the extension specific
 * {@link VSCALLBACK_BUF_SET_LINE_COLOR} callback.</p>
 *
 * @param wid	Window id of editor control.
 *
 ...
 *
 * <ul>
 * <li>{@link VSCALLBACK_AR_BUF_LINES_INSERTED}</li>
 * <li>{@link VSCALLBACK_AR_BUF_LINES_DELETED}</li>
 * <li>{@link VSCALLBACK_AR_BUF_DELETE_TEXT}</li>
 * <li>{@link VSCALLBACK_AR_BUF_INSERT_TEXT}</li>
 * <li>{@link VSCALLBACK_AR_BUF_REPLACE_TEXT}</li>
 * <li>{@link VSCALLBACK_AR_BUF_LINES_INSERTED2}</li>
 * <li>{@link VSCALLBACK_AR_BUF_LINES_DELETED2}</li>
 * <li>{@link VSCALLBACK_AR_BUF_TEXT_CHANGE}</li>
 * <li>{@link VSCALLBACK_AR_BUF_DRAW_LINE}</li>
 * <li>{@link VSCALLBACK_AR_APP_DELETE_BUFFER}</li>
 * <li>{@link VSCALLBACK_AR_APP_SELECT_MODE}</li>
 * <li>{@link VSCALLBACK_AR_APP_ADD_BUFFER}</li>
 * <li>{@link VSCALLBACK_AR_APP_RENAME_BUFFER}</li>
 * <li>{@link VSCALLBACK_AR_APP_RENAME_DOCUMENT}</li>
 * </ul>
 *
 * @param pfn	Pointer to callback function.  Make sure you
 * remove your callback function with
 * {@link vsCallbackRemove} if you unload a
 ...
 * {@link vsPropGetI}({@code wid},VSP_BUF_
 * ID),  {@link vsBufEdit}, or
 * {@link vsBufMatch}.
 *
 ...
 * buffer and not by users writing macros.
 *
 * @return Returns pointer to OEM buffer data set by the {@link vsBufSetData}
 * function.
 *
 * @param buf_id	Buffer id returned by
 * {@link vsPropGetI}({@code wid},VSP_BUF_
 ...
 * Use the {@link vsSetDefaultColor} function to set the color attributes.
 * Color indexes are passed to the {@link vsSetTextColor} function to
...
/**
 * Frees a color index allocated by {@link vsAllocColor}.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 ...
 * allocated by {@link vsAllocColor}.  0
 ...
 * conjuction with {@link vsIsWindowValid} to traverse all 
 * SlickEdit objects. 
 *
 * @example
 * <pre>
 ...
 *
 * <p>After calling this function, you need to call {@link vsRefresh} with
 * the {@literal 'A'} option so that the scroll bars are updated.</p>
 *
 * @param wid	Window id of object.  0 specifies the current
 * object.
 *
 ...
 *
 * <dl>
 * <dt>0 or {@literal 'C'}</dt><dd>Change position and size of window for its
 * current state.</dd>
 *
 * <dt>{@literal 'N'}</dt><dd>Change normalized position and size of
 * window.</dd>
 * </dl>
 *
 * @appliesTo All_Window_Objects
...
#define VSCFG_LIVE_ERRORS_ERROR          107
#define VSCFG_LIVE_ERRORS_WARNING        108

// colors 109-127 reserved for future use
#define VSCFG_UNUSED                     109

// "named" symbol coloring colors
#define VSCFG_SYMBOL_COLOR_SYMBOL_NOT_FOUND                 128
#define VSCFG_SYMBOL_COLOR_LOCAL_VARIABLE                   129
#define VSCFG_SYMBOL_COLOR_STATIC_LOCAL_VARIABLE            130
#define VSCFG_SYMBOL_COLOR_PARAMETER                        131
#define VSCFG_SYMBOL_COLOR_TEMPLATE_PARAMETER               132
#define VSCFG_SYMBOL_COLOR_PUBLIC_MEMBER_VARIABLE           133
#define VSCFG_SYMBOL_COLOR_PACKAGE_MEMBER_VARIABLE          134
#define VSCFG_SYMBOL_COLOR_PROTECTED_MEMBER_VARIABLE        135
#define VSCFG_SYMBOL_COLOR_PRIVATE_MEMBER_VARIABLE          136
#define VSCFG_SYMBOL_COLOR_PUBLIC_STATIC_MEMBER_VARIABLE    137
#define VSCFG_SYMBOL_COLOR_PACKAGE_STATIC_MEMBER_VARIABLE   138
#define VSCFG_SYMBOL_COLOR_PROTECTED_STATIC_MEMBER_VARIABLE 139
#define VSCFG_SYMBOL_COLOR_PRIVATE_STATIC_MEMBER_VARIABLE   140
#define VSCFG_SYMBOL_COLOR_GLOBAL_VARIABLE                  141
#define VSCFG_SYMBOL_COLOR_STATIC_GLOBAL_VARIABLE           142
#define VSCFG_SYMBOL_COLOR_GLOBAL_FUNCTION                  143
#define VSCFG_SYMBOL_COLOR_STATIC_GLOBAL_FUNCTION           144
#define VSCFG_SYMBOL_COLOR_CLASS_CONSTRUCTOR                145
#define VSCFG_SYMBOL_COLOR_CLASS_DESTRUCTOR                 146
#define VSCFG_SYMBOL_COLOR_PUBLIC_MEMBER_FUNCTION           147
#define VSCFG_SYMBOL_COLOR_PACKAGE_MEMBER_FUNCTION          148
#define VSCFG_SYMBOL_COLOR_PROTECTED_MEMBER_FUNCTION        149
#define VSCFG_SYMBOL_COLOR_PRIVATE_MEMBER_FUNCTION          150
#define VSCFG_SYMBOL_COLOR_PUBLIC_STATIC_MEMBER_FUNCTION    151
#define VSCFG_SYMBOL_COLOR_PACKAGE_STATIC_MEMBER_FUNCTION   152
#define VSCFG_SYMBOL_COLOR_PROTECTED_STATIC_MEMBER_FUNCTION 153
#define VSCFG_SYMBOL_COLOR_PRIVATE_STATIC_MEMBER_FUNCTION   154
#define VSCFG_SYMBOL_COLOR_NESTED_FUNCTION                  155
#define VSCFG_SYMBOL_COLOR_PUBLIC_CLASS_PROPERTY            156
#define VSCFG_SYMBOL_COLOR_PACKAGE_CLASS_PROPERTY           157
#define VSCFG_SYMBOL_COLOR_PROTECTED_CLASS_PROPERTY         158
#define VSCFG_SYMBOL_COLOR_PRIVATE_CLASS_PROPERTY           159
#define VSCFG_SYMBOL_COLOR_PUBLIC_STATIC_CLASS_PROPERTY     160
#define VSCFG_SYMBOL_COLOR_PACKAGE_STATIC_CLASS_PROPERTY    161
#define VSCFG_SYMBOL_COLOR_PROTECTED_STATIC_CLASS_PROPERTY  162
#define VSCFG_SYMBOL_COLOR_PRIVATE_STATIC_CLASS_PROPERTY    163
#define VSCFG_SYMBOL_COLOR_CLASS                            164
#define VSCFG_SYMBOL_COLOR_TEMPLATE_CLASS                   165
#define VSCFG_SYMBOL_COLOR_ABSTRACT_CLASS                   166
#define VSCFG_SYMBOL_COLOR_INTERFACE_CLASS                  167
#define VSCFG_SYMBOL_COLOR_STRUCT                           168
#define VSCFG_SYMBOL_COLOR_UNION_OR_VARIANT_TYPE            169
#define VSCFG_SYMBOL_COLOR_TYPE_DEFINITION_OR_ALIAS         170
#define VSCFG_SYMBOL_COLOR_PREPROCESSOR_MACRO               171
#define VSCFG_SYMBOL_COLOR_PACKAGE_OR_NAMESPACE             172
#define VSCFG_SYMBOL_COLOR_SYMBOLIC_CONSTANT                173
#define VSCFG_SYMBOL_COLOR_ENUMERATED_TYPE                  174
#define VSCFG_SYMBOL_COLOR_ENUMERATION_CONSTANT             175
#define VSCFG_SYMBOL_COLOR_STATEMENT_LABEL                  176
#define VSCFG_SYMBOL_COLOR_DATABASE_TABLE                   177
#define VSCFG_SYMBOL_COLOR_DATABASE_TABLE_COLUMN            178
#define VSCFG_SYMBOL_COLOR_DATABASE_TABLE_INDEX             179
#define VSCFG_SYMBOL_COLOR_DATABASE_TABLE_VIEW              180
#define VSCFG_SYMBOL_COLOR_MODULE                           181
#define VSCFG_SYMBOL_COLOR_CONCEPT                          182
#define VSCFG_SYMBOL_COLOR_RULE                             183
#define VSCFG_SYMBOL_COLOR_ANNOTATION_TYPE                  184
#define VSCFG_SYMBOL_COLOR_ANNOTATION                       185
#define VSCFG_SYMBOL_COLOR_CONTROL                          186
#define VSCFG_SYMBOL_COLOR_MENU                             187
#define VSCFG_SYMBOL_COLOR_FORM                             188
#define VSCFG_SYMBOL_COLOR_EVENT_TABLE                      189
#define VSCFG_SYMBOL_COLOR_PROTECTED_SCOPE                  190
#define VSCFG_SYMBOL_COLOR_PRIVATE_SCOPE                    191

// 64-color abstract symbol coloring palette
#define VSCFG_SYMBOL_COLOR_PALETTE_00                       192
#define VSCFG_SYMBOL_COLOR_PALETTE_01                       193
#define VSCFG_SYMBOL_COLOR_PALETTE_02                       194
#define VSCFG_SYMBOL_COLOR_PALETTE_03                       195
#define VSCFG_SYMBOL_COLOR_PALETTE_04                       196
#define VSCFG_SYMBOL_COLOR_PALETTE_05                       197
#define VSCFG_SYMBOL_COLOR_PALETTE_06                       198
#define VSCFG_SYMBOL_COLOR_PALETTE_07                       199
#define VSCFG_SYMBOL_COLOR_PALETTE_08                       200
#define VSCFG_SYMBOL_COLOR_PALETTE_09                       201
#define VSCFG_SYMBOL_COLOR_PALETTE_10                       202
#define VSCFG_SYMBOL_COLOR_PALETTE_11                       203
#define VSCFG_SYMBOL_COLOR_PALETTE_12                       204
#define VSCFG_SYMBOL_COLOR_PALETTE_13                       205
#define VSCFG_SYMBOL_COLOR_PALETTE_14                       206
#define VSCFG_SYMBOL_COLOR_PALETTE_15                       207
#define VSCFG_SYMBOL_COLOR_PALETTE_16                       208
#define VSCFG_SYMBOL_COLOR_PALETTE_17                       209
#define VSCFG_SYMBOL_COLOR_PALETTE_18                       210
#define VSCFG_SYMBOL_COLOR_PALETTE_19                       211
#define VSCFG_SYMBOL_COLOR_PALETTE_20                       212
#define VSCFG_SYMBOL_COLOR_PALETTE_21                       213
#define VSCFG_SYMBOL_COLOR_PALETTE_22                       214
#define VSCFG_SYMBOL_COLOR_PALETTE_23                       215
#define VSCFG_SYMBOL_COLOR_PALETTE_24                       216
#define VSCFG_SYMBOL_COLOR_PALETTE_25                       217
#define VSCFG_SYMBOL_COLOR_PALETTE_26                       218
#define VSCFG_SYMBOL_COLOR_PALETTE_27                       219
#define VSCFG_SYMBOL_COLOR_PALETTE_28                       220
#define VSCFG_SYMBOL_COLOR_PALETTE_29                       221
#define VSCFG_SYMBOL_COLOR_PALETTE_30                       222
#define VSCFG_SYMBOL_COLOR_PALETTE_31                       223
#define VSCFG_SYMBOL_COLOR_PALETTE_32                       224
#define VSCFG_SYMBOL_COLOR_PALETTE_33                       225
#define VSCFG_SYMBOL_COLOR_PALETTE_34                       226
#define VSCFG_SYMBOL_COLOR_PALETTE_35                       227
#define VSCFG_SYMBOL_COLOR_PALETTE_36                       228
#define VSCFG_SYMBOL_COLOR_PALETTE_37                       229
#define VSCFG_SYMBOL_COLOR_PALETTE_38                       230
#define VSCFG_SYMBOL_COLOR_PALETTE_39                       231
#define VSCFG_SYMBOL_COLOR_PALETTE_40                       232
#define VSCFG_SYMBOL_COLOR_PALETTE_41                       233
#define VSCFG_SYMBOL_COLOR_PALETTE_42                       234
#define VSCFG_SYMBOL_COLOR_PALETTE_43                       235
#define VSCFG_SYMBOL_COLOR_PALETTE_44                       236
#define VSCFG_SYMBOL_COLOR_PALETTE_45                       237
#define VSCFG_SYMBOL_COLOR_PALETTE_46                       238
#define VSCFG_SYMBOL_COLOR_PALETTE_47                       239
#define VSCFG_SYMBOL_COLOR_PALETTE_48                       240
#define VSCFG_SYMBOL_COLOR_PALETTE_49                       241
#define VSCFG_SYMBOL_COLOR_PALETTE_50                       242
#define VSCFG_SYMBOL_COLOR_PALETTE_51                       243
#define VSCFG_SYMBOL_COLOR_PALETTE_52                       244
#define VSCFG_SYMBOL_COLOR_PALETTE_53                       245
#define VSCFG_SYMBOL_COLOR_PALETTE_54                       246
#define VSCFG_SYMBOL_COLOR_PALETTE_55                       247
#define VSCFG_SYMBOL_COLOR_PALETTE_56                       248
#define VSCFG_SYMBOL_COLOR_PALETTE_57                       249
#define VSCFG_SYMBOL_COLOR_PALETTE_58                       250
#define VSCFG_SYMBOL_COLOR_PALETTE_59                       251
#define VSCFG_SYMBOL_COLOR_PALETTE_60                       252
#define VSCFG_SYMBOL_COLOR_PALETTE_61                       253
#define VSCFG_SYMBOL_COLOR_PALETTE_62                       254
#define VSCFG_SYMBOL_COLOR_PALETTE_63                       255

#define VSCFG_FIRST_SYMBOL_COLOR                            128
#define VSCFG_LAST_SYMBOL_COLOR                             255
#define VSCFG_FIRST_COLOR                                     1
#define VSCFG_LAST_COLOR                                    255
 ...
 * index allocated by {@link vsAllocColor}.
 *
 * @param FGColor	New rgb foreground color.
 * @param SetFGColor	Indicates whether to use {@code FGColor} as
 * new foreground color.
 *
 * @param BGColor	New rgb background color.
 * @param SetBGColor	Indicates whether to use {@code BGColor} as
 * new foreground color.
 *
 * @param FontFlag	Set to one of the VSFONTFLAG_?? constants or 0.
 * @param SetFontFlag	Indicates whether to use {@code FontFlag} as
 * new foreground color.
 *
 * @param cfgParentColor   One of the VSCFG_??? constants or a color
 *                         index allocated by {@link vsAllocColor}
 * @param SetParentColor   Indicates whether to use {@code cfgParentColor}
 ...
 *
 */
int VSAPI vsSetDefaultProfileColor(const slickedit::SEString &profileName,
                                   const int cfgfield,
                                   const int FGColor=0,        const bool SetFGColor=false,
                                   const int BGColor=0,        const bool SetBGColor=false,
                                   const int FontFlag=0,       const bool SetFontFlag=false,
                                   const int cfgParentColor=0, const bool SetParentColor=false);
...
/**
 * Set color profile specified 
 *  
 * @param pszProfileName
 * @param optionLevel
 */
int VSAPI vsSetColorsFromProfile(VSPSZ pszProfileName, int optionLevel=0);
 ...
 *                         which {@code cfgfield} inherits properties from.
...
/**
 * Gets rgb color values (always in Windows format) and font style flag
 * for the <i><b>cfgfield</b></i> specified in the given profile.
 *
 * @return Returns 0 if successful. 
 *         Returns STRING_NOT_FOUND_RC if there is no such color profile. 
 *
 * @param profileName      Color profile name
 * @param cfgfield         One of the VSCFG_??? constants or a color
 *                         index allocated by {@link vsAllocColor}.
 ...
 * @see vsSetDefaultColor
 * @see vsGetDefaultColor
 * @see vsSetDefaultProfileColor 
 *
 * @appliesTo Edit_Window, Editor_Control
 *
 * @categories Edit_Window_Functions, Editor_Control_Functions
 */
int VSAPI vsGetDefaultProfileColor(const slickedit::SEString &profileName,
                                   int cfgfield,
                                   int *pFGColor,int *pBGColor,
                                   int *pFontFlag VSDEFAULT(nullptr),
                                   int *pcfgParentColor VSDEFAULT(nullptr));
 ...
 * event is dispatched or {@link vsExecute} is
 ...
 * dispatched or {@link vsExecute} is called.
 * You can not set this option.</dd>
 *
 ...
 *
 * <dl>
 * <dt>{@literal 'I'}</dt><dd>Caption is for an iconized window.  No path
 * is placed in the caption.</dd>
 * <dt>{@literal 'N'}</dt><dd>Caption is for a normalized window.</dd>
 * <dt>{@literal 'M'}</dt><dd>Caption is for a maximized window.</dd>
 * </dl>
 *
...
// IF GUI header files have been included
    ...
    * {@link vsUnregisterWindow} before you destroy your window.
    *
    ...
    * the same as {@code hwnd}.  For UNIX,
    * specify the same value as {@code hwnd} if
 ...
 * SlickEdit will set focus to the {@code hwnd} window when there are no
 ...
 * disable the {@code hwndframe} window when a modal dialog box is
 * displayed.
 *
 ...
 * the same as {@code hwnd}.
 *
 * @param hwnd	System window handle MDI frame client
 * window.  The {@code hwndmdiclient}
...
#define  VSBDS_FIXED_SINGLE  1
    ...
    * callback, you need to call {@link vsRefresh} with the {@literal 'A'} option so
    * that the scroll bars are updated.</p>
    *
    * <p>You do not have to call {@link vsDestroyEditorCtl} on Windows
    * because SlickEdit gets the Windows WM_DESTROY message
    * and can cleanup their.  However, under X windows you must call
    * {@link vsDestroyEditorCtl} just before destroying the parent window
    ...
    *
    * @param {@code x}, {@code y}, {@code width}, {@code height}	Size in pixels to
    ...
    * {@link vsWinQData} function to get your
    * window specific user data.  Use the
    * {@link vsWinSetData} function to change
    * this value later.
    *
 ...
 * on it. {@code dropFiles} contains the list of filenames. 
 ...
 * @categories File_Functions
 */
void VSAPI vsDropFiles(const slickedit::SEArray<slickedit::SEString> &dropFiles, int atWid=0,int mouseWid=0);
 ...
 * specified by {@code wid}. If editor control is already
 ...
 * @return	Returns an instance handle (window id) to the object, 
 *  
 * {@code object_name}.  Returns 0 if an instance is not found.  
 * Specify the {@literal 'N'} option if you only want to find a non-edited 
 * instance of an object. Specify the {@literal 'E'} option if you only 
...
/**
 * Float/dock MDI child window {@code wid}.
 * 
 * @param wid 
 */
...
/**
 * Return 1 if MDI child window {@code wid} is a 
 ...
 * {@code pszServerName}.  We recommend
 ...
 *  
 * @param wid     Window id
 * @param state   {@literal 'N'},{@literal 'F'},{@literal 'M'}
 */
 ...
 * {@code pfnMDIChildNewHorizontalTabGroup}. 
 */
...
*/
   ...
   */
   ...
   /**
    * Register window {@code wid} as a tool-window. Return 0
    * on success, < 0 on error. 
    *
    * <p>
    *
    * Set {@code flags} to one or more of vsToolWindowFlag. 
    *
    * <p>
    *
    * Set {@code preferredArea} to one of vsDockAreaPos. In 
    ...
    * with {@code pfnMDIToolWindowRegister}, into the Slick-C 
    * array of {@code wids}. Set {@code mdi_wid} to a 
   ...
   /**
    * Add tool-window {@code wid}. Set {@code mdi_wid} to 
    ...
    * <p>
    *
    * Set {@code dFlags} to one or more of 
   ...
   /**
    * Remove tool-window specified by {@code wid}. Tool-window
    * is NOT destroyed. 
    *
    * @param wid
    */
    ...
    * specified by {@code wid}. If tool-window is already part 
    * of a tile, then it is removed and inserted into new tile. Set 
    * {@code mdi_wid} to add new tile to specific mdi-window; 
    * set to 0 for current mdi-window. Return 0 on success, < 0 on 
    * error. 
    *
    * <p>
    *
    * If {@code vertical} is true, then a vertical tile is
   ...
   /**
    * Set tool-window specified by {@code wid} active. If 
   ...
   /**
    * Restore tool-window {@code wid} to 
    * {@code position}. 
    *  
    * @param wid 
    * @param position 
   ...
   /**
    * Return true if tool-window {@code wid} is a floating 
   ...
   /**
    * Finds tool-window adjacent to tool-window {@code wid}. 
    *  
    * @param wid            Tool-window window id (form wid)
    ...
    * {@code mdi_wid}. Set <code>mdi_wid=0</code> to specify 
   ...
   /**
    * Return the restore {@code info} for the tool-window form
    * name {@code form_name} at given {@code position}. 
   ...
   /**
    * Return the {@code vsDockAreaPos} position of tool-window
    * {@code wid} relative to the MDI area. If tool-window 
   ...
   /**
    * Test tool-window {@code wid} flag {@code f}. If 
   ...
   /**
    * (Un)set tool-window {@code wid} flag {@code f}. If 
   ...
   /**
    * Auto-hide tool-window {@code wid} to 
   ...
   /**
    * Raise auto-hide tool-window {@code wid}. 
    * 
    * @param wid 
    */
   ...
   /**
    * Lower auto-hide tool-window {@code wid}. 
    * 
    * @param wid 
    */
   ...
   /**
    * Return true if tool-window {@code wid} is an auto-hide 
    * window. Note that if you want to know whether an auto-hide 
    * tool-window is visible (raised) or hidden (lowered), then use
    * {@code pfnMDIToolWindowIsAutoRaised}. 
    * {@code to_mdi_wid} specifies which mdi window to test. 
   ...
   /**
    * Return true if tool-window {@code wid} is a raised (i.e.
    * visible) auto-hide window. {@code to_mdi_wid} specifies 
   ...
   /**
    * Restore auto-hide tool-window {@code wid} to last known 
    ...
    * {@code mdi_wid} in {@code area}. Results are stored 
    * in {@code info}. 
    *
   ...
   /**
    * Clear dock-channel in {@code area} from MDI window 
    * {@code mdi_wid}. This will remove all tabs and hide the 
    * dock-channel. 
    *
    * @param mdi_wid  Window id of MDI frame window.
...
#define VSMDITILE_TILE       0x8
   ...
   */
    ...
    * control specified by {@code wid}. If editor control is 
    * already part of a tabgroup, then it is removed and inserted 
    * into new tabgroup. 
    *
    * <p>
    *
    * If {@code vertical} is true, then a vertical tabgroup is
    ...
    * {@code mdi_wid} and return as encoded string. The format 
    * of this string is user-implementation specific. Specify which 
    * areas to restore by setting {@code areas} to one or more
    ...
    * <p>
    *
    * Restore windows from Slick-C array {@code wids} into 
    * specified mdi window {@code mdi_wid} using layout 
    * {@code state} returned from 
    * {@code pfnMDIWindowSaveLayout}. The format of this 
    * string is user-implementation specific. Specify which areas 
    * to restore by setting {@code areas} to one or more 
    * vsWindowLayoutArea flags. Set optional {@code flags} to 
    ...
    * <p>
    *
    * Restore MDI {@code state} returned from
    * {@code pfnMDISaveState}. The format of this string
    * is user-implementation specific. Specify which areas to
    * restore by setting {@code areas} to one or more 
    * vsWindowLayoutArea flags. Set optional {@code flags} to 
    * one or more vsRestoreStateFlag. 
    * 
    *
    ...
    *    <dl compact>
    *    <dt><b>{@literal 'B'}</b> <dd>Basic group info
    *    <dt><b>{@literal 'A'}</b> <dd>All info. Fill in tab array
    *    </dl>
    */
   ...
   /**
    * Return true if MDI child window {@code wid} is a 
    ...
    * {@code name}. User properies are saved and restored by 
    * the MDI window {@code mdi_wid}. Set a user property 
...
/************************Miscellaneous option callbacks*******************************************/
   /*
       ...
       editor instance.
   */
   int (VSAPI *pfnUseCurrentInstance)(int new_option, const char *pszCmdline,const slickedit::SEString &instance_id);

   void (VSAPI *pfnTouchInstance)();

   slickedit::SEString (VSAPI *pfnInstanceId)();

   void (VSAPI *pfnInstanceGetList)(slickedit::SEArray<slickedit::SEString> &instance_list);
 ...
 * "{@link vsCreateEditorCtl}".  Make sure you call
 * {@link vsPrepareForTerminate} and {@link vsTerminate} to properly
 ...
 * {@link vsPrepareForTerminate} before calling before
 * {@link vsTerminate}.  SlickEdit destroys its MDI Frame
...
/**
 * Frees allocated resources.  {@link vsPrepareForTerminate} must be
 * callled before calling this function.  No other vs??? calls should be
 * made after calling this function.  You may not call {@link vsInit} after
 * calling vsTerminate.
 *
 * @categories Miscellaneous_Functions
 *
 ...
 * match or tagged expression found by the last {@link vsSearch} or
 * {@link vsRepeatSearch}.
 *
 * @param TaggedExpression	Tagged expression number. Specify
 * a {@code MatchGroup} of -1 to get the length
 ...
 * found by {@link vsSearch} or {@link vsRepeatSearch} and the cursor
 ...
 * last {@link vsSearch} or {@link vsRepeatSearch}.
 *
 ...
 * last regex search.
 *  
 * @param minLength    (default 1) minimum match group length to count 
 *  
 * @see vsSearch
 * @see vsRepeatSearch
 * @see vsSearchReplace
 ...
 */
EXTERN_C
int VSAPI vsMatchGroupNames(int min_length, slickedit::SEString &match_names);
 ...
 *                  current object.
 * @param markid
 *                  {@code mark_id} is a handle to a selection returned by
 *                  {@link vsAllocSelection} or {@link vsDuplicateSelection}.  A
 *                  {@code mark_id} of -1 {@code mark_id} parameter identifies the active
 *                  selection.
 ...
 * -1, then {@code pBuf} must point to a null terminated string.  This
 * function is slow because it performs word wrap.  Use
 * {@link vsInsertLine} or {@link vsInsertText} function for better
 * performance.
 *
 * @param wid	Window id of object.  0 specifies the current
 * object.
 ...
 * characters.
 *
 * @param BufLen	Number of characters in {@code pBuf}.  -1
 * specifies that {@code pBuf} is null terminate
 ...
 * of the functions {@link vsSearch} or {@link vsRepeatSearch}.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
...
/**
 * Replaces the match found by the last {@link vsSearch} or
 * {@link vsRepeatSearch} with the replace string specified and
 ...
 * {@code pReplaceString}.  -1 means
 * {@code pReplaceString} is null terminated.
 *
...
/**
 * Saves search information set by one of the functions {@link vsSearch}
 * or {@link vsRepeatSearch}.  Use {@link vsRestoreSearch} to restore
...
/**
 * Restores search information saved by the {@link vsSaveSearch}
 * function.
 *
 * @see vsMatchLength
 * @see vsMatchStart
 ...
 * {@code wid}, the resulting lines are inserted after the end of the
 ...
 * @param markid	{@code mark_id} is a selection handle
 * allocated by the {@link vsAllocSelection} or
 * {@link vsDuplicateSelection} function.
 ...
 * TOO_MANY_SELECTIONS_RC.  {@code mark_id} is a handle to a
 * selection returned by one of the built-ins {@link vsAllocSelection} or
 * {@link vsDuplicateSelection}.
 *
 * <p>IMPORTANT:  If -1 is specified for the {@code mark_id} parameter, a
 ...
 *
 * @param mark_id	Selection id to duplicated.   If -1 is specified
 * for the {@code mark_id} parameter, a handle
 ...
 * buffer.  Unlike the the {@link vsExpandTabs} function, this function
 * gets the source text from the current line.</p>
 *
 * <p>The {@code StartCol} and {@code ColWidth} specification correspond to
 * the input {@code string} as if tab characters were expanded according to
 ...
 *
 * @param wid	{@code wid} identifies the object to be operated on.  0
 ...
 * {@code pszDest} not including ASCII 0.
 *
 * @param StartCol	Input. Start (imaginary) column for sub-
 * string.
 *
 * @param ColWidth	Input.  Number of (imaginary) columns to get
 * from {@code pSource}.   -1 specifies the rest or the
 * string.  -2 specifies to use the
 * {@link vsTruncQLineLength} instead of the real line
 * length.  This has the same effect as specifying -1 if the
 * {@link VSP_TRUNCATELENGTH} property is 0.
 *
 * @param Option	Input.  Specify {@literal 'E'} if you want all tabs
 * expanded.  Otherwise specify {@literal 'S'} to return tabs
 * unexpanded.
 *
 * @see vsExpandTabs
 * @see vsTextColC
 ...
 * depending on {@code option}.  See table
 * below.
 *
 ...
 * handle.
 *
 * @return Returns 0 if the {@code markid} specified is valid.  Otherwise a
 * negative error code is returned.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 ...
 *
 * <p>If {@code option}=={@literal 'L'} or option=={@literal 'T'}</p>
 * <ul>
 * <li>VSSELECT_LINE</li>
 * <li>VSSELECT_CHAR</li>
 ...
 * </ul>
 *
 * <p>If {@code option}=={@literal 'S'}</p>
 * <ul>
 * <li>VSSELECT_CURSOREXTENDS</li>
 * </ul>
 *
 * <p>If {@code option}=={@literal 'U'}</p>
 * <ul>
 * <li>VSSELECT_PERSISTENT</li>
 * </ul>
 *
 * @param option	{@literal 'L'}, {@literal 'T'}, {@literal 'S'}, or {@literal 'U'}.
 * 	For {@literal 'L'} or {@literal 'T'} the select type is set to the
 * {@code type} given.  When the {@literal 'L'} option,
 ...
 * new line selection.
 *
 * <p>For {@literal 'S'}, the selection style is set to extend as
 * the cursor moves or not to extend as the
 * cursor moves.</p>
 *
 * <p>For {@literal 'U'}, the selection style is set to be
...
/**
 * @return If a value of {@literal 'T'} is specified for the second parameter, one of the
 * selection types VSSELECT_BLOCK, VSSELECT_CHAR, or
 * VSSELECT_LINE is returned. 0 is returned if the selection specified
 * has not been set.
 *
 * <p>When a value of {@literal 'S'} is specified for the second parameter, the select
 ...
 *
 * <p>If a value of {@literal 'I'} is specified for the second parameter, 1 is returned if
 * the selection specified is an inclusive selection.  Otherwise 0 is
 * returned.</p>
 *
 * <p>When a value of {@literal 'U'} is specified for the second parameter, a
...
/**
 * Places {@code Len} bytes of data starting from the byte offset specified
 * by {@code RealSeekPos} into {@code pszBuf}.  The resulting string is
 * always null terminated.  The buffer {@code pszBuf} must be large
 * enough to contain {@code Len}+1 bytes of data.   Lines with the VSLF_
...
/**
 * Searches for the {@code OldLineNumber} specified and places the
 ...
 * VSLF_ NOSAVE flag).  Use the {@link vsSetAllOldLineNumbers}
 ...
 * Use the {@link vsGoToOldLineNumber} method to go to an old line
 ...
 * @param wid Window id of editor control.  0 specifies the current object.
 *
 * @param Flags {@code Flags} is one or more of the following constants ORed together:
 *
 * <dl>
 * <dt>
 ...
 *
 * @param Mask The {@code Mask} indicates which bits will be set
 * according to {@code Flags}.  If {@code Mask} is 0, {@code Mask}
 * is set to the value of {@code Flags}.
 *
 * @see vsQLineFlags
 * @see vsSetLineFlags
 * @see vsResetModifiedLineFlags
 ...
 * to end of file.  When {@code DelLen} is
 * greater than the number of characters left in
 * the file, the text up to the end of file is
 * deleted.
 *
 * @param option	Maybe 0 or {@literal 'C'}.  Specify {@literal 'C'} to indicate that
 * {@code DelLen} is a positive imaginary count
 * (as if tabs were expanded) of characters to
 * delete in the current line.
 *
 * <p>If {@code option} is zero, number of characters specified by
 * {@code DelLen} starting from the cursor location are deleted.  Each end
 * of line character is treated as 1 character (as is).</p>
 *
 * <p>If {@code option} is {@literal 'C'}, the number of columns (as if tabs were
 * expanded) specified by {@code DelLen} are deleted from the current
...
/**
 * <p>Insert {@code string} at cursor position.  You may use this function to
 * insert multiple lines of text.  {@code NLCh1} and {@code NLCh2} define
 * a 1 or two character end of line sequence.  You must specify
 * {@code NLCh1}=={@code NLCh2} if you want a 1 character end of line
 * sequence.  The end of line sequence is used to parse the source string
 * {@code pBuf} into multiple lines.  The {@code NLCh1} and
 * {@code NLCh2} are ignored if the binary option is non-zero.</p>
 *
 * <p>When you use this function on a record file (<b>p_buf_width</b>!=0)
 * and the {@code binary} argument is non-zero, all data is inserted into
 ...
 * {@code pBuf} is a new line, you can end up with a blank line that has
 * no new line characters in it.  You can test for this condition by calling
 * {@link vsQLineLength}(0,1) which will return 0.  Use
 * {@link vsDeleteLine}(0) to delete the line. </p>
 *
 * @return Returns 0 if successful.
 *
 * @param wid	Window id of editor control.  0 specifies the
 ...
 * {@code binary} argument if you want to allow
...
/**
 * This function is identical to the {@link vsInsertText} function, except
 * that the input string is in the same format as the internal buffer data
 * which can be SBCS/DBCS or UTF-8.  See "<b>Unicode and
 * SBCS/DBCS C API Programming</b>".
 *
 ...
 * {@link vsQPoint} function may be used to get the text line position
 ...
 * built-in function {@link vsQPoint}.
 *
 ...
 * current object.
 *
 * @param pPoint	If {@code option}=={@literal 'P'},<i> </i>ouput for seek
 ...
 *
 * @param option {@literal 'P'} or {@literal 'L'}.
 *
 * <dl>
 * <dt>{@literal 'P'}</dt><dd>Sets {@code pPoint} to the seek position to the beginning
 * of the current line.  Sets {@code pDownCount} to the number of lines to
 * cursor down to get to the current line when starting from the seek
 * position to the beginning of line.  It is unusual for
 * {@code pDownCount} to be non-zero.  This can only occur for record
 ...
 * characters.</dd>
 *
 * <dt>{@literal 'L'}</dt><dd>Sets {@code pPoint} to current line number.  If the current
 * line number is not known, {@code pPoint} is set to -1.</dd>
 * </dl>
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods
 *
 ...
 * the file has non-savable lines.  Use {@link vsGoToROffset},
 * {@link vsQROffset}, <b>p_RLine</b>, and <b>p_RNoflines</b> for
...
/**
 * Searches for {@code pSearchString} specified.  If pReplaceString is
 * non-zero, and search and replace without prompting is performed.
 * Press and hold Ctrl+Alt+Shift to terminate a long search.
 * {@code pszOptions} is one or more of the following option letters:
 *
 ...
 * {@code pSearchString}.  You can specify -1 if
 * {@code pSearchString} is null terminated.
 *
 ...
 * <dt>X{@code CCLetters}</dt><dd>Requires the first character of search string
 * NOT be one of the color coding elements specified.
 * For example, "XCS" requires that the first character
 * not be in a comment or string. {@code CCLetters} is
 ...
 * </dd>
 *
 * <dt>C{@code CCLetters}</dt><dd>Requires the first character of search string to
 * be one of the color coding elements specified. See
 * {@code CCLetters} above.</dd>
 * </dl>
 *
 * @param pReplaceString	String to replace search string with.  Specify
 * 0 to just perform a search.  If
 * {@code pReplaceString} is not NULL, a
 * search and replace without prompting is
 * performed.
 *
 * @param ReplaceStringLen	Number of characters in
 * {@code pReplaceString}.  You can specify -1
 * if {@code pReplaceString} is null terminated.
 *
...
/**
 * Repeats a search initiated by {@link vsSearch}.  This will not repeat a
 ...
 * specified by {@code StartCol}.  If {@code StartCol} is not given,
 ...
 * {@link vsSetLineFlags} function for information on meaning of flags
 * returned.
 *
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 ...
 * with the {@literal 'A'} option, all editor controls and other SlickEdit
 ...
 * are done call this function with the {@literal 'A'} option to update the window.</p>
 *
 * @param wid	Window id of object.  0 specifies the current
 * object.
 *
 * @param option	{@literal 'A'} or {@literal 'W'}. The {@literal 'A'} option refreshes all editor
 * controls, all editor control scroll bars, and
 * calls the VSINIT.pfnRefresh callback
 * function if one exists.  While it may sound
 * like the {@literal 'A'} option is inefficient, it is actually
 * highly optimized.  SlickEdit only
 * updates editor controls which have data that
 * has been modified.  Use the {@literal 'W'} option to
 * flush paint messages for the object specified.
 * The {@literal 'W'} option will not update the scroll bars
 ...
 * {@code pSource}.  You can specify -1 if
 ...
 * get from {@code pSource}.   -1 specifies the
 * rest or the string.
 *
 * @param Option	Input.  Specify {@literal 'E'} if you want all tabs
 * expanded.  Otherwise specify {@literal 'S'} to return
 * tabs unexpanded.
 *
 * <p>The {@code StartCol} and {@code ColWidth} specification correspond to
 * the input {@code string} as if tab characters were expanded according to
 ...
 * {@link vsCommandSave} function instead of this function.
 *
 ...
 * {@code pszRev1Filename}, and 0 for
 * {@code iRev1WindowId}.  Specify -1 to have
 * this parameter ignored.
 *
 * @param iRev1WindowId	Window Index for the revision 1 buffer.  If you
 * use this, be sure to specify "" for
 * {@code pszRev1Filename}, and -1 for
 * {@code iBuf1Index}.  Specify 0 to have this
 * parameter ignored.
 *
 ...
 * you use this, be sure to specify "" for
 * {@code pszRev2Filename}, and 0 for
 * {@code iRev2WindowId}.  Specify -1 to have
 * this parameter ignored.
 *
 * @param iRev2WindowId	Window Index for the revision 2 buffer.  If you
 * use this, be sure to specify "" for
 * {@code pszRev2Filename}, and -1 for
 * {@code iBuf2Index}.  Specify 0 to have this
 * parameter ignored.
 *
 ...
 * use this, be sure to specify "" for
 * {@code pszOutputFilename}, and 0 for
 * {@code iOutputWindowId}.  If you specify
 * {@code iOutputBufferId} or
 * {@code iOutputWindowId}, the output will be
 ...
 * this, be sure to specify "" for
 * {@code pszOutputFilename}, and -1 for
 * {@code iOutputBufferId}.  If you specify
 * {@code iOutputBufferId} or
 * {@code iOutputWindowId}, the output will be
 * left open after merge terminates (if there are
 * conflicts) so that you can deal with the
 ...
 * {@code pszBaseFilename} is a buffer name and not a relative
 * filename which needs the path to be fully resolved.</dd>
 *
 * <dt>VSMERGE_OPT_REV1_BUFFER</dt><dd>
 * {@code pszRev1Filename} is a buffer name and not a relative
 * filename which needs the path to be fully resolved.</dd>
 *
 * <dt>VSMERGE_OPT_REV2_BUFFER</dt><dd>
 * {@code pszRev2Filename} is a buffer name and not a relative
 * filename which needs the path to be fully resolved.</dd>
 *
 ...
 * refreshed when {@link vsRefresh} is called
 ...
 * {@code filename} is absolute) {@code filename} specified.  This function
 ...
 *         {@code CheckRange} is non-zero) specified.
 */
 ...
 *
 * @param isRealLineNum  Indicates whether {@code RealLineNum} is
 ...
 * </dl>
 *
 * @param type  Type allocated by {@link vsMarkerTypeAlloc}().
 *
 ...
 * @param isRealOffset
 *                   When non-zero, indicates that {@code StartOffset} does NOT include lines with the NOSAVE VSLF_NOSAVE flag set.
 ...
 * counting no save lines).
 *
 * @param isRealLineNum	Indicates whether {@code RealLineNum} is a
...
/*
   ...
   {@code NofLines} property of the line marker.
*/
...
/**
 * Mask for both "VSMARKERTYPEFLAG_DRAW_LINE_*" flags.
 */
#define VSMARKERTYPEFLAG_DRAW_LINE_MASK     (VSMARKERTYPEFLAG_DRAW_LINE_LEFT | VSMARKERTYPEFLAG_DRAW_LINE_RIGHT)
...
/**
 * Mask for both "VSMARKERTYPEFLAG_DRAW_SQUIGGLY_*" flags.
 */
#define VSMARKERTYPEFLAG_DRAW_SQUIGGLY_MASK     (VSMARKERTYPEFLAG_DRAW_SQUIGGLY_LEFT | VSMARKERTYPEFLAG_DRAW_SQUIGGLY_RIGHT)
...
/**
 * Mask for all "VSMARKERTYPEFLAG_DRAW_TRIANGLE_*" flags.
 */
#define VSMARKERTYPEFLAG_DRAW_TRIANGLE_MASK     ( VSMARKERTYPEFLAG_DRAW_TRIANGLE_TOP_LEFT     | \
                                                  VSMARKERTYPEFLAG_DRAW_TRIANGLE_TOP_RIGHT    | \
                                                  VSMARKERTYPEFLAG_DRAW_TRIANGLE_BOTTOM_RIGHT | \
                                                  VSMARKERTYPEFLAG_DRAW_TRIANGLE_BOTTOM_LEFT  )
...
/**
 * Indicates that this stream marker is for symbol coloring. 
 * This makes it easy to determine if a particular stream marker should 
 * be displayed in the minimap or not. 
 */
#define VSMARKERTYPEFLAG_SYMBOL_COLORING            0x800000

/**
 * Indicates that this stream marker should be displayed as bold text. 
 * This combines with the other VSMARKERTYPEFLAG_FONT_* flags, as 
 * well as the font flags specified for the stream marker color index. 
 */
#define VSMARKERTYPEFLAG_FONT_BOLD                  0x1000000
/**
 * Indicates that this stream marker should be displayed as italic text.
 * This combines with the other VSMARKERTYPEFLAG_FONT_* flags, as 
 * well as the font flags specified for the stream marker color index. 
 */
#define VSMARKERTYPEFLAG_FONT_ITALIC                0x2000000
/**
 * Indicates that this stream marker should be displayed as underlined text.
 * This combines with the other VSMARKERTYPEFLAG_FONT_* flags, as 
 * well as the font flags specified for the stream marker color index. 
 */
#define VSMARKERTYPEFLAG_FONT_UNDERLINE             0x4000000

/**
 * Indicates that this stream marker should be displayed with the 
 * foregournd and backgorund colors reversed.
 */
#define VSMARKERTYPEFLAG_REVERSE_COLOR              0x8000000

/**
 * Indicates that this stream marker should be displayed as strike-through text.
 * This combines with the other VSMARKERTYPEFLAG_FONT_* flags, as 
 * well as the font flags specified for the stream marker color index. 
 */
#define VSMARKERTYPEFLAG_FONT_STRIKETHRU            0x10000000

///**
// * When on, if this marker type has a color index set, use that color 
// * for the scroll bar marker instead of the marker type color.
// */
//#define VSMARKERTYPEFLAG_USE_STREAM_MARKER_COLOR    0x20000000
 ...
 *
 * @param wid        Window id of editor control.  0 specifies the current object.
 * @param LineNum    Line number returned by vsPropGetI64(wid,VSP_LINE)
 ...
 *         {@code CheckRange} is non-zero) specified.
 *         -1 is appended to the end of the list to indicate the end of the list.
 *         {@code nullptr0 is returned if no matches are found.
 *         Call {@link vsFree} to free the list.
 *  
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
 ...
 * 
 * @param wid           Window id of editor control.  0 specifies the current object.
 * @param markerTypeId  (optional) marker type ID to restrict search to
 * @param BMIndex       (optional) look for line markers with this bitmap ID
 * @param userData64    (optional) look for line markers with this integer user data 
 * @param pNofItems     May be null.  Set to number of items in array returned.
 * 
 * @return Returns list of line markers indexes which match the marker type, 
 *         bitmap index, and/or user data specified.
 *         -1 is appended to the end of the list to indicate the end of the list.
 *         {@code nullptr0 is returned if no matches are found.
 *         Call {@link vsFree} to free the list.
 */
int *VSAPI vsLineMarkerAllocFindListWithType(int wid, int markerTypeId=0, int BMIndex=0, VSINT64 userData64=0, int *pNofItems=nullptr);
 ...
 * @param type
 * @param internalStartBlock        (optional) Editor block+1 or 0 to indicate null.
 * @param internalStartBlockPos_l   (optional) This is offset by one for backward compatibility.
 *
 ...
 * @param internalStartBlockPos_l   (optional) This is offset by one for backward compatibility.
 * 
 * @return Returns list of stream markers indexes that match the type, bitmap, and user data specified.
 *         -1 is appended to the end of the list to indicate the end of the list.  0 is returned if no matches are found.
 *         Call vsFree to free the list.
 */
int *VSAPI vsStreamMarkerAllocFindListWithType(int wid, int *pNofItems, int markerTypeId=0, int BMIndex=0, VSINT64 userData64=0, class VSHBLOCKTAB_LIST *internalStartBlock=0,seSeekPos internalStartBlockPos_l=(seSeekPos)0);
...
/**
 * Removes all line markers that have the specified {@code BMIndex}.
 *
 * @param BMIndex	Index of bitmap in names table.
 *
 * @appliesTo Edit_Window, Editor_Control
...
/**
 * Removes all line markers of that have the specified {@code iUserData} 
 * and the given {@code type}. 
 *
 * @param type       Type of marker.
 * @param iUserData  64-bit user data value to look for
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
void VSAPI vsLineMarkerRemoveAllUserDataInt64(int type, VSINT64 iUserData);
void VSAPI scLineMarkerRemoveAllUserDataInt64(int type, VSINT64Param iUserData);
 ...
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerRemoveBMIndex(int wid,int BMIndex);
/**
 * Removes all stream markers for the specified editor control that have the
 * specified {@code iUserData} and the given {@code type}.
 *
 * @param wid	Window id of editor control.  0 specifies the current object.
 * @param type	Type of marker.
 * @param iUserData  64-bit user data value to look for
 ...
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerRemoveUserDataInt64(int wid, int type, VSINT64 iUserData);
void VSAPI scStreamMarkerRemoveUserDataInt64(int wid, int type, VSINT64Param iUserData);
/**
 * Removes all stream markers that have the specified {@code BMIndex}.
 *
 * @param BMIndex	Index of bitmap in names table.
 *
 * @appliesTo Edit_Window, Editor_Control
...
/**
 * Removes all stream markers that have the specified {@code iUserData} 
 * and the given {@code type}. 
 *
 * @param type	Type of marker.
 * @param iUserData  64-bit user data value to look for
 ...
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerRemoveAllUserDataInt64(int type, VSINT64 iUserData);
void VSAPI scStreamMarkerRemoveAllUserDataInt64(int type, VSINT64Param iUserData);
 ...
 * @categories Marker_Functions
 */
void VSAPI vsLineMarkerRemoveUserDataInt64(int wid, int type, VSINT64 iUserData);
void VSAPI scLineMarkerRemoveUserDataInt64(int wid, int type, VSINT64Param iUserData);
/**
 * Removes all line markers for the specified editor control that have the
 * specified type.
 *
...
/** 
 * Stores a pointer to user-defined data with the given stream marker.
 *
 * @param StreamMarkerIndex   Index of stream marker returned by a vsStreamMarkerAdd function. 
 * @param pUserData           pointer to user data to associate with marker 
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
...
/** 
 * Stores the window ID of the editor scroll bar associated with thsi marker.
 *
 * @param StreamMarkerIndex   Index of stream marker returned by a vsStreamMarkerAdd function. 
 * @param scrollbar_wid       window ID of editor scroll bar
 *  
 * @note 
 * This is an internal function. 
 */
int VSAPI vsStreamMarkerSetScrollBarWID(int StreamMarkerIndex, int scrollbar_wid);

/** 
 * Set the text length of the given stream marker. 
 *
 * @param StreamMarkerIndex   Index of stream marker returned by a vsStreamMarkerAdd function.
...
/** 
 * Set the start offset for a stream marker. 
 *  
 * @param StreamMarkerIndex   Index of stream marker returned by a vsStreamMarkerAdd function.
 * @param StartOffset         Replace StartOffset of this stream marker with the StartOffset
 * 
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions 
 */
...
/** 
 * Set the marker type for this stream marker. 
 *
 * @param StreamMarkerIndex      Index of stream marker returned by a vsStreamMarkerAdd function.
...
/** 
 * Set additional style flags for this stream marker. 
 * These flags will be combined with the flags specified for the marker type.
 *
 * @param StreamMarkerIndex      Index of stream marker returned by a vsStreamMarkerAdd function.
 * @param MarkerTypeFlags        bitset VSMARKTYPEFLAG_* style flags
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
EXTERN_C
void VSAPI vsStreamMarkerSetStyleFlags(int StreamMarkerIndex, int MarkerTypeFlags);

/** 
 * Set the text message associated with the given stream marker.
 *
 * @param StreamMarkerIndex      Index of stream marker returned by a vsStreamMarkerAdd function.
...
/** 
 * Set the user data as a 64-bit integer associated with the given stream marker.
 *
 * @param StreamMarkerIndex Index of stream marker returned by a vsStreamMarkerAdd function.
 * @param iUserData         64-bit integer user data value
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerSetUserDataInt64(int StreamMarkerIndex, VSINT64 iUserData);
void VSAPI scStreamMarkerSetUserDataInt64(int StreamMarkerIndex, VSINT64Param iUserData);
/** 
 * @return 
 * Returns the user data associated with the given stream marker.
 *
 * @param StreamMarkerIndex Index of stream marker returned by a vsStreamMarkerAdd function.
 *
 * @appliesTo Edit_Window, Editor_Control
...
/** 
 * @return 
 * Returns the window ID of the associated editor scroll bar for the given stream marker.
 *
 * @param StreamMarkerIndex Index of stream marker returned by a vsStreamMarkerAdd function.
 *  
 * @note 
 * This is an internal function. 
 */
int VSAPI vsStreamMarkerGetScrollBarWID(int StreamMarkerIndex);
...
/** 
 * @return 
 * Returns the color index for the text in the stream marker specified.
 *
 * @param StreamMarkerIndex   Index of stream marker returned by a vsStreamMarkerAdd function.
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
VSCOLORINDEX VSAPI vsStreamMarkerGetTextColor(int StreamMarkerIndex);

/** 
 * @return 
 * Return additional style flags for this stream marker. 
 * (a bitset VSMARKTYPEFLAG_* style flags)
 *
 * @param StreamMarkerIndex      Index of stream marker returned by a vsStreamMarkerAdd function.
 *
 * @appliesTo Edit_Window, Editor_Control
 ...
 */
EXTERN_C
int VSAPI vsStreamMarkerGetStyleFlags(int StreamMarkerIndex);
 ...
 * 
 * @param wid Window ID of editor control 
 * @param iType Type returned by {@link vsScrollMarkupAllocType} 
...
/**
 * Update scroll markup information for the given child window
 *
 * @appliesTo Edit_Window, Editor_Control
 * @categories Marker_Functions
 */
void VSAPI vsScrollMarkupUpdateModel(int wid);
 ...
 *
 * <dl>
 * <dt>{@literal 'T'}</dt><dd>Return time in the format {@code hh}:{@code mmcc}
 * where {@code hh} is an hour between 1 and 12, mm
 * is the minutes between 0 and 59, and {@code cc} is
 * the "am" or "pm".</dd>
 *
 * <dt>{@literal 'B'}</dt><dd>Return binary time in milliseconds.  This options is
 * used for comparing dates with the :< and :>
 * operators.</dd>
 *
 * <dt>{@literal 'M'}</dt><dd>24-hour (Military) time in the format
 * {@code hh}:{@code mm}:{@code ss} where {@code hh}
 * is an hour between 0 and 23, {@code mm} is the
 * minutes between 0 and 59, and {@code ss} is the
 * seconds between 0 and 59.</dd>
 *
 * <dt>{@literal 'L'}</dt><dd>Returns time in the current local format.  Under
 ...
 * where {@code YYYY} is the year, {@code MM} is the month,
 * {@code DD} is the day, {@code hh} is an hour between 0 and 23,
 * {@code mm} is the minutes between 0 and 59, {@code ss} is the
 * seconds between 0 and 59, and {@code fff} is the fractional
...
VSDLLEXPORT slickedit::SEString VSAPI vsInstanceId();
VSDLLEXPORT void VSAPI vsInstanceGetList(slickedit::SEArray<slickedit::SEString> &instance_listinstance_list);
 ...
 *         corresponding to {@code mode_name}. 
 *  
 ...
 * 
 * @return The language ID for the language 
 *         corresponding to {@code lexer_name}. 
 *  
...
VSPSZ VSAPI vsFilename2LangId(VSPSZ filename, int flags);
...
EXTERN_C int VSAPI vsIsBinaryModeFile(VSPSZ filename, int flags = 0);
