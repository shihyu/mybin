 ...
 * NOTE:  typically the {@code frame_id} is ignored,
 ...
 * This function returns {@literal '0'} if the event was handled and disposed of.
 * Otherwise, it will return {@literal '1'}, indicating that you need to
 ...
 *
 * @return {@code true} if the disassembly is to be displayed, false if not, <0 on error.
 * 
 */
 ...
 *
 * @return {@code true} if the disassembly is up to date.
 *         Returns <0 on error.
 * 
 ...
 *                        string, assume {@literal 'localhost'}.  In the future, it may also attempt
 ...
 * @param frame_id    index of stack frame
 *
 * @return Return the number of field in {@code this} for the current stack frame.
 * 
 */
 ...
 *
 * @param class_id   index of class in class table
 * @param parent_id  index of parent class (usually {@literal '1'})
 * @param class_name (referene) name of parent class
 *
 * @return 0 on success, <0 on error.
 * 
...
/**
 * @return {@code true} if the current thread is up to date.
 *         Returns <0 on error.
 * <p>
...
/**
 * @return {@code true} if the threads are up to date.
 *         Returns <0 on error.
 * <p>
 * {@link dbg_clear_threads()} will mark the threads as stale.
 * 
 ...
 *
 * @param i          index of source directory to get, first index is {@literal '1'}
...
/**
 * Save the fact that {@code file_name} maps to {@code full_path}.
 *
 * @param file_name       source file name (with extension)
 ...
 *
 * @param i          index of runtime expr to get, first index is {@literal {@literal '1'}}
...
/**
 * Get the details about a member of {@code this} object.
 *
 * @param thread_id   index of thread
 * @param frame_id    index of stack frame
...
EXTERN_C int VSAPI dbg_get_member(int thread_id,int frame_id,VSPSZ member_path,
                                  ...
                                  VSHREFVAR raw_value);
/**
 * Set the value of a member of {@code this} object.
 *
 * @param thread_id   index of thread
 * @param frame_id    index of stack frame
 * @param member_path path to member in list of member variables
 ...
 * If {@code file_name} is NULL, clear the disassembly for all files.
 * 
 * @param file_name     source file to remove disassembly for
 *
 * @return 0 on success, <0 on error.
 ...
 * @param reply            command reply data
 * @param errmsg           output skimmed from stderr
 * @param parse_reply      If {@code false}, {@code reply} is a string,
 ...
 * Set the instruction pointer for the top frame in the given thread
 * to correspond to the given file/line/address.
 * NOTE:  the {@code frame_id} is ignored, only the top frame can be modified.
 * 
 * @param thread_id     Thread ID for instruction pointer to change
 * @param frame_id      Frame ID to modify instruction pointer for
 * @param class_name    name of class breakpoint is restricted to
...
EXTERN_C int VSAPI dbg_windbg_write_dumpfile(VSPSZ filename,
                                             bool full_with_heap=false, 
                                             bool small_without_heap=false);
