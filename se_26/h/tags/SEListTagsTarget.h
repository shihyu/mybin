...
public:
   ...
   /**
    * @return 
    * Return {@code true} if the modify flags indicate that this buffer was modified. 
    */
   ...
   /** 
    * @return 
    * Return {@code true} if the file information for this file matches the file
   ...
   /** 
    * @return 
    * Return {@code true} if the date information for this file matches the date 
    * information for the given object.
    * 
    * @param rhs     object to compare to
    */
   ...
   /**
    * Return {@code true} if the tagging target is to insert tags into a tag database.
    */
   bool isTargetDatabase() const;
   void setTargetDatabase();

   /**
    * Return {@code true} if the tagging target is to update the tags found in 
    * the current file. 
    */
   ...
   /**
    * Return {@code true} if the tagging target is to insert tags in a match set 
   ...
   /**
    * Return {@code true} if the tagging target is to update the current list of 
    * local variables. 
    */
   ...
   /**
    * Return {@code true} if the tagging job is currently running. 
    */
   ...
   void setTargetIdle();
   /**
    * Return {@literal '1'} if the tagging job is completely finished 
   ...
   /**
    * Return {@code true} if the parsing is finished.
    */
   bool isParsingFinished() const;
   /**
    * Return {@code true} if the tagging job is completely finished. 
    */
   ...
   /**
    * Return {@code true} if the tagging remove job is completely finished. 
    */
   ...
   /**
    * Return {@code true} if statement tagging is supported for this language.
    */
   bool isStatementTaggingSupported() const;
   /**
    * Return {@code true} if local variable tagging is supported for this language.
    */
   bool isLocalTaggingSupported() const;
   /**
    * Return {@code true} if the token list is supported for this language.
    */
   bool isTokenListSupported() const;

   /**
    * If we are tagging the current context, return {@code true} if statement-level 
    * tagging is enabled and we should insert tags for control statements. 
    */
   bool getStatementTaggingMode() const;

   /**
    * If we are inserting tags into a tag database, return {@code true} if we need 
   ...
   /** 
    * If we are operating on a tag database, return {@code true} if a request 
   ...
   /**
    * Return {@code true} if listing local variables and we should skip locals which 
   ...
   /**
    * Return {@code true} if listing local variables and we need to start right 
   ...
   /** 
    * Return {@code true} if we are reading input data from a string rather than 
    * a file or an editor buffer. 
    */
   bool getReadFromStringMode() const;
   /** 
    * Return {@code true} if we are reading input data from an editor control
    */
   bool getReadFromEditorMode() const;
   /** 
    * Return {@code true} if we are reading input data from a file.
    */
   bool getReadFromFileMode() const;

   /**
    * Return {@code true} if the tagging information is being updated asynchronously. 
    */
...
public:
    ...
    * for the first tag.  Does nothing if {@code tagIndex} is out of range. 
    * Remove all tags from tagIndex to the end of the array if {@code removeToEnd} 
    * is true. 
    */
   ...
   /** 
    * @return 
    * Return {@code true} if the the given offset within the current file is 
   ...
   /**
    * Find the symbol in this context which is closest to the current line 
    * in the current file, either looking above or below. 
    * 
    * @param fileName              full path to current file
    * @param currentLineNumber     current real line number
    * @param findTagAfter          if {@code true} look for a symbol that starts after the current line, 
    *                              otherwise look for a symbol that starts before the current line. 
    * @param filterFlags           tag type filter flags
    * @param pFilterByTypes        extended tag filter options
    * @param allowStatements       allow a statement to be the current item? 
    * @param allowOutlineOnly      allow symbols that are flagged as outline only
    * 
    * @return 
    * Returns the tag index of the symbol which the location corresponds to. 
    *  
    * @see findCurrentContextId() 
    */
   const unsigned int findNearestContextId(const SEString &fileName, 
                                           const unsigned int currentLineNumber,
                                           const bool findTagAfter=false,
   ...
   /**
    * Returns info for the embedded section indexed by {@code index}
    *  
   ...
   /**
    * Return {@code true} if this tagging target should stop parsing now 
   ...
   /**
    * Return {@code true} if tagging logging is turned on.
    */
   bool isTaggingLogging() const;

   /**
    * Returns {@code true} if the tagging should allow dupliate global 
   ...
   /**
    * Returns {@code true} if the tagging should allow dupliate local 
    ...
    * A lock on a non-const instance will guarantee uniqueness.
    * @return 
    * Return {@code true} if the lock was successfully obtained, false otherwise. 
    */
   bool tryWriteLock();
   /**
    * Try to obtain a lock for thread synchronization. 
