 ...
 * <p> 
 * The only field that is hidden is {@code mTokenType}, because it is mirrored 
...
public:
   ...
   /**
    * @return Returns {@code true} if this token matches the given token. 
   ...
   /**
    * @return Returns {@code true} if this token does not match the given token. 
    * @param src  token object to compare to 
    */
   bool operator != (const SETokenStruct &src) const;
   bool operator != (const SETokenReadInterface &src) const;
