...
public:
   ...
   void clear();
   /**
    * @return Return {@code true} if this object is in it's initial state.
    */
   ...
   /** 
    * @return 
    * Return {@code true} if the context ID of the outer container for this item 
    * has been determined.
    */
   const bool isOuterContextKnown() const;
   /**
    * @return 
    * Returns {@code true} if this symbol can serve as a container for other symbols. 
    * Returns {@code false} if we have not determinded if this symbol can be a container. 
    */
   const bool isContainer() const;
   /**
    * @return 
    * Returns {@code true} if this symbol can serve as a container for other symbols. 
    * Returns {@code true} if we have not determinded if this symbol can be a container. 
    * Returns {@code false} if this symbol is definately not a container. 
    */
   const bool isContainerMaybe() const;
   /**
    * @return 
    * Return {@code true} if we have determined if this symbol can be a container. 
    */
    ...
    * @param tokenID      unique identifier for this token in {@code pTokenList} 
    */
   ...
   /**
    * @return {@code true} if the tag spans the given offset 
    *  
   ...
   /**
    * @return {@code true} if the tag spans the given line number 
    *  
    ...
    * The result is returned as a string class instance.
    *
    ...
    * @param include_args     if {@code false}, does not include function signature
    * @param include_tab      append class name after signature if {@code true},
    *                         prepend class name with :: if {@code false}
    * @param prettyPrintArgs  if {@code false}, insert argument list as is, 
    *                         otherwise attempt to add space to make it more readable 
    *  
    * @return caption as a string instance.
    */
   SEString makeCaption(const bool include_class=false,
                        const bool include_args=false,
                        const bool include_tab=false,
                        const bool prettyPrintArgs=true) const;
   ...
   int insertInTree(int treeWid, 
                    ...
                    VSHVAR user_info=VSHVAR_NULL) const;
   ...
   int insertInTreeOutline(int treeId, 
                           ...
                           VSHVAR userInfo=VSHVAR_NULL ) const;

   /** 
    * @return 
    * Returns {@code true} if the given file/line/seekpos is still within the context 
    * of this symbol.  Returns {@code false} otherwise.
    * 
   ...
   /** 
    * @return 
    * Return {@code true} if the tag filters match for the this symbol.
    * 
   ...
   /**
    * @return 
    * Return {@code true} if the given tag name pattern matches this symbol name
   ...
   /** 
    * @return 
    * Return {@code true} if the given identifier matches this symbol name after 
...
private:
   ...
   SESharedPointer<struct SEPrivateTagInformation> mpTagInformation;
...
extern unsigned cmHashKey(const slickedit::SETagInformation &cm);
