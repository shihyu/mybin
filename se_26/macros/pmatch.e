...
int _find_matching_paren(int pmatch_max_diff_ksize=MAXINT,bool quiet=false,int pmatch_max_level=MAXINT,bool alwaysUpdate=true,bool matchAllParens=false, int initialNestLevel=0, int depth=1)
{
   ...
   if (p % 2) {   /* begin paren? */
      ...
      if ( ! p ) {
         ...
         save_search(auto searchstring,auto options,auto word_re,auto s4,auto s5);
...
//#define PMATCH_CHARS 'a-zA-Z0-9_$#'
         ...
         word=get_match_text();
         restore_search(searchstring,options,word_re,s4,s5);
...
struct BlockMatchingColors {
   int mBlockMatchingEmbedded;
   int mBlockMatchingCurrentLine;
   int mBlockMatchingCurrentLineEmbedded;
};

static int gBlockMatchingPicType=-1;
static int gBlockMatchingNumPics=0;
static BlockMatchingColors gBlockMatchingColorsPerScheme:[];
...
definit()
{
   ...
   gBlockMatchingColorsPerScheme = null;
}
...
void _UpdateShowMatchingParen(bool alwaysUpdate=false)
{
   ...
   */
   ...
   colorIds := gBlockMatchingColorsPerScheme:[p_WindowColorProfile];
   if (colorIds == null) {
      BlockMatchingColors newColorIds;
      newColorIds.mBlockMatchingEmbedded            = -1;
      newColorIds.mBlockMatchingCurrentLine         = -1;
      newColorIds.mBlockMatchingCurrentLineEmbedded = -1;
      gBlockMatchingColorsPerScheme:[p_WindowColorProfile] = newColorIds;
      colorIds = gBlockMatchingColorsPerScheme:[p_WindowColorProfile];
   }
   if (colorIds.mBlockMatchingEmbedded <= 0 || 
       _default_color(colorIds.mBlockMatchingEmbedded) != _default_color(-CFG_BLOCK_MATCHING)) {
      typeless fg, bg, ff;
      ...
      if (colorIds.mBlockMatchingEmbedded<=0) {
         colorIds.mBlockMatchingEmbedded = _AllocColor();
      }
      typeless old_fg, old_bg, old_ff;
      parse _default_color(colorIds.mBlockMatchingEmbedded) with old_fg old_bg old_ff;
      if (old_fg!=fg || old_bg!=bg || old_ff!=ff) {
         _default_color(colorIds.mBlockMatchingEmbedded,fg,bg,ff);
      }
      gBlockMatchingColorsPerScheme:[p_WindowColorProfile] = colorIds;
   }

   // workaround in order to support block matching color with current line color
   if (p_color_flags & CLINE_COLOR_FLAG) {
      if (colorIds.mBlockMatchingCurrentLine <= 0) {
         colorIds.mBlockMatchingCurrentLine = _AllocColor();
      }
      typeless fg, bg, ff;
      parse _default_color(CFG_BLOCK_MATCHING) with fg bg ff;
      if (ff & F_INHERIT_BG_COLOR) {
      ...
      typeless old_fg, old_bg, old_ff;
      parse _default_color(colorIds.mBlockMatchingCurrentLine) with old_fg old_bg old_ff;

      if (old_fg!=fg || old_bg!=bg || old_ff!=ff) {
         _default_color(colorIds.mBlockMatchingCurrentLine,fg,bg,ff);
      }
      
      if (colorIds.mBlockMatchingCurrentLineEmbedded <= 0) {
         colorIds.mBlockMatchingCurrentLineEmbedded = _AllocColor();
      }
      parse _default_color(-CFG_BLOCK_MATCHING) with fg bg ff;
      if (ff & F_INHERIT_BG_COLOR) {
         parse _default_color(-CFG_CLINE) with . bg .;
      ...
      parse _default_color(colorIds.mBlockMatchingCurrentLineEmbedded) with old_fg old_bg old_ff;
      if (old_fg!=fg || old_bg!=bg || old_ff!=ff) {
         _default_color(colorIds.mBlockMatchingCurrentLineEmbedded,fg,bg,ff);
      }
      gBlockMatchingColorsPerScheme:[p_WindowColorProfile] = colorIds;
   }
      ...
      if (def_highlight_paren_under_cursor || 
         ...
         if (p_EmbeddedLexerName != '') {
            if ((p_color_flags & CLINE_COLOR_FLAG) && point()==orig_line) {
               _StreamMarkerSetTextColor(marker, colorIds.mBlockMatchingCurrentLineEmbedded);
            } else {
               _StreamMarkerSetTextColor(marker, colorIds.mBlockMatchingEmbedded);
            }
         } else {
            if ((p_color_flags & CLINE_COLOR_FLAG) && point()==orig_line) {
               _StreamMarkerSetTextColor(marker, colorIds.mBlockMatchingCurrentLine);
            } else {
...
/**
 * Free allocated colors if the given color scheme is modified.
 * 
 * @param colorProfileName 
 */
void _color_profile_modified_pmatch(_str colorProfileName="")
{
   colorIds := gBlockMatchingColorsPerScheme:[colorProfileName] = null;
   if (colorIds == null) return;
   if (colorIds.mBlockMatchingCurrentLine > CFG_LAST_DEFAULT_COLOR) {
      _FreeColor(colorIds.mBlockMatchingCurrentLine, colorProfileName);
   }
   if (colorIds.mBlockMatchingCurrentLineEmbedded > CFG_LAST_DEFAULT_COLOR) {
      _FreeColor(colorIds.mBlockMatchingCurrentLineEmbedded, colorProfileName);
   }
   if (colorIds.mBlockMatchingEmbedded > CFG_LAST_DEFAULT_COLOR) {
      _FreeColor(colorIds.mBlockMatchingEmbedded, colorProfileName);
   }
   gBlockMatchingColorsPerScheme:[colorProfileName] = null;
}
