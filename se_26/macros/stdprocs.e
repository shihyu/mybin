 ...
 *         {@code pos} or {@code lastpos} function.
 * @param n tagged expression name (maybe be number).
 *        If left to default (empty string), returns the entire string match.
 *
 * @see pos
 ...
 * @param start_close_seperator  seperator used for start and close of expression
 * @param firstexp               (reference) set to first item in list (parenexp gets all args)
 *
 * @return true on success, false if it fails to find paren match
 */
bool match_generic(_str &prefixexp, _str &parenexp, int &num_args, _str start_close_seperator, _str &firstexp="")
{
   //say("match_generic()");
   ...
   for (p=1;;p++) {
      ...
      if (ch:==start_char) {
         ...
         //say("found close char, nesting="nesting);
         nesting--;
         if (!nesting) {
            if (num_args==1) {
               firstexp = substr(prefixexp, 1, p-1);
            }
            p--;
            break;
         }
      } else if (ch:==seperator) {
         //say("found seperator, nesting="nesting);
         if (nesting==1) {
            if (num_args==1) {
               firstexp = substr(prefixexp, 1, p-1);
               //say("match_generic H"__LINE__": first="firstexp" p="p);
            }
            num_args++;
         }
      } else {
         np := pos(":q|'\\*?'|:v|?", prefixexp, p, 'r');
         //say("match_generic H"__LINE__": OTHER, prefixexp="prefixexp" p="p" np="np);
         if (!np) {
            num_args=0;
            prefixexp = substr(prefixexp, p);
            //say("match_generic H"__LINE__": NO MATCH, prefixexp="prefixexp);
            return false;
         }
         p = np + pos('') - 1;
      }
   ...
   if (p<=0) {
      ...
      //say("match_generic H"__LINE__": parenexp="parenexp);
   }
   prefixexp = substr(prefixexp, p+2);
   //say("match_generic H"__LINE__": OUT prefixexp="prefixexp);
   return true;
}
/**
...
/**
 * Extract a bracketed expression from the beginning of the
 * given prefix expression and find the number of arguments in
 ...
 * 
 * @param word          current word under cursor
 * @param word_list     array or hash table of words handled by syntax expansion
 * @param notused       This parameter is no longer used.
 * @param aliasfilename (reference) set to name of alias file
 * @param findAliases   Look for aliases and set aliasfilename
 * @param noPrompt      Do not prompt. Return semicolon delimited list of matches 
 * @param fuzzyMatch    Allow matches which are slight misspellings (fuzzy) 
 *
 ...
 * @categories Miscellaneous_Functions
 */
_str min_abbrev2(_str word, 
                 typeless &word_list, 
                 _str notused,
                 _str &aliasfilename, 
                 bool findAliases=true,
                 bool noPrompt=false,
...
*/
   ...
   if (doSyntaxExpansion) {
      ...
      for (i:=0; true; ++i) {
         ...
         } else if (fuzzyMatch && tag_matches_prefix_with_corrections(cword, keyword, true, LangCaseSensitive)) {
            hitlist[count++]=keyword;
         }
      }
   }
   ...
   if (findAliases && aliasfilename!='') {
      ...
      if (!status) {
         ...
         foreach (auto curword in list) {
            ...
            } else if (!haveExactWordMatch && fuzzyMatch && 
                       tag_matches_prefix_with_corrections(cword, curword, true, LangCaseSensitive)) {
               add_to_array(hitlist,curword);
               alias_hitlist[alias_count++]=curword;
            }
         }
 ...
 *                   <ul>
 *                   <li>If {@code index} is less than 0, the existing
 *                       picture will be replaced if it already exists.
 *                       If it does not exist, the new picture is
 *                       inserted.</li>
 *                   <li>If {@code index} is 0, a new picture is inserted.</li>
 *                   <li>If {@code index} is greater than 0, it is assumed
 ...
 *                   Pass {@code true} to suppresss this message.
 *  
...
void _append_retrieve(int ctl_wid,_str value,_str ctl_name='')
{
   ...
   if (!ctl_wid) {
      ...
      case OI_TEXT_BOX:
         ...
         break;
      case OI_FRAME:
         if (ctl_wid.p_checkable) {
            command= CTLSEPARATOR:+'fr 'ctl_name':'value;
         }
         break;
      case OI_CHECK_BOX:
...
static int _save_fr2(int wid,_str form_name)
{
   ...
   case OI_FRAME:
      if (wid.p_checkable) {
         gtempdata :+= CTLSEPARATOR:+ 'fr 'ctl_name':'wid.p_value;
      }
      break;
   case OI_CHECK_BOX:
...
bool _custom_tab_action() {
   index := _FindLanguageCallbackIndex("_%s_custom_tab_action", p_LangId);
...
static void smarttab_for_one_cursor(bool &can_align_cursors)
{
   ...
   if (_on_line0() || def_keys == 'ispf-keys') {
      call_root_key(TAB);
      can_align_cursors = false;
   } else if (_custom_tab_action()) {
      can_align_cursors = false;
   } else if (!_IsSmartTabEnabled()) {
      call_root_key(TAB);
   } else if (_argument=='') {
      //_undo('S');
   }
...
static void smarttab_for_each_cursor()
{
   can_align_cursors := true;
   bool lines_with_cursors:[];
   right_most_column := 0;
   num_cursors := 0;

   // do not align cursors in hex mode
   if (p_hex_mode==HM_HEX_ON) {
      can_align_cursors = false;
   }

   // check if smart tab is enabled enough to align cursors
   smartpaste_index := 0;
   ...
   // option to align cursors not on?
   if (!se.lang.api.LanguageSettings.getTabAlignsCursors(p_LangId)) {
      can_align_cursors = false;
   }

   // if smarttab is being repeated, then do not align cursors
   if (_argument != "") {
      can_align_cursors = false;
   }

   // loop through each active cursor
   ...
   for (ff:=true; ; ff=false) {
      ...
      // check if we already saw a cursor on this line
      if (lines_with_cursors._indexin(p_line)) {
         can_align_cursors = false;
      } else {
         lines_with_cursors:[p_line] = true;
      }

      // if the cursor is in the prefix area, we can't align columns
      if (p_LCHasCursor) {
         can_align_cursors = false;
      }
      // if any line is within leading whitespace, do not align columns,
      // we are in smarttab re-indent's line's territory
      if (_in_leading_indent()) {
         can_align_cursors = false;
      }

      // do smarttab for this cursor
      smarttab_for_one_cursor(can_align_cursors);

      // check if we are at a more-righter column
      num_cursors++;
      if (p_col > right_most_column) {
         right_most_column = p_col;
      }
   ...
   if (can_align_cursors && right_most_column > 0 && num_cursors > 1) {
      // add an undo step before aligning cursors
      _undo('S');

      // loop again through each active cursor to skip blanks
      save_search(auto s1, auto s2, auto s3, auto s4, auto s5);
      for (ff=true; ; ff=false) {
         if (multicursor) {
            if (!_MultiCursorNext(ff)) {
      ...
      // loop again through each active cursor to align cursors
      for (ff=true; ; ff=false) {
         if (multicursor) {
            if (!_MultiCursorNext(ff)) {
               break;
         ...
         // move by tab stops until columns are aligned
         while (p_col < right_most_column) {
            smarttab_for_one_cursor(can_align_cursors);
         }
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void c_tab,gnu_ctab() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_TEXT_BOX|VSARG2_MARK|VSARG2_LINEHEX|VSARG2_ONLY_BIND_MODALLY)
{
   if (command_state()) {
      call_root_key(TAB);
      return;
...
/**
 * If the cursor is on or before the first non blank character
 * of the current line and the proper C statement indent column is after the current column,
 * then the current statement is reindented and the cursor is placed on the first non blank
 * character.  Otherwise the root key table binding for the TAB key is invoked.  This command
 * should only be bound to a mode key table. 
 ...
 * @see gnu_ctab
 */
_command void smarttab() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_TEXT_BOX|VSARG2_MARK|VSARG2_LINEHEX|VSARG2_ONLY_BIND_MODALLY)
{
   if (command_state()) {
      call_root_key(TAB);
      return;
 ...
 * {@code f} is contained in the {@code flags} bitset. 
 * 
 * @param flags 
 * @param f 
 * 
