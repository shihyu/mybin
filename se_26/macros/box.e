...
int _OnUpdate_box(CMDUI& cmdui, int target_wid, _str command) {
   if (!target_wid || !target_wid._isEditorCtl(false)) {
      return MF_GRAYED;
   }
   status:=target_wid._EmbeddedStart(auto orig_values);
   lang:=target_wid.p_LangId;
   if ( status==1 ) target_wid._EmbeddedEnd(orig_values);
...
_command int box() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK|VSARG2_REQUIRES_AB_SELECTION)
{
   ...
   if ( firstline_is_top ) {
      ...
      if ( s.m_thside!='' ) {
         if (length(s.m_thside) == 1) {
            topside=substr('',1,topsidelen,s.m_thside);
         } else {
            topside="";
            while (length(topside) < topsidelen) {
               topside :+= s.m_thside;
            }
            width += (length(topside) - topsidelen);
            topsidelen = length(topside);
         }
      }
      get_line_raw(line);
      if ( comment_col ) {
   ...
   if ( lastline_is_bottom ) {
      ...
      if ( s.m_bhside!='' ) {
         if (length(s.m_bhside) == 1) {
            bottomside=substr('',1,bottomsidelen,s.m_bhside);
         } else {
            bottomside="";
            while (length(bottomside) < bottomsidelen) {
               bottomside :+= s.m_bhside;
            }
            bottomsidelen = length(bottomside);
         }
      }                                                      
      if ( comment_col ) {
         // It's a line selection, so there is nothing before 'start_col'
...
static bool can_erase_selected_box(BlockCommentSettings s)
{
   save_pos(auto p);
   ...
   if ( down() ) {
      ...
      single_line= (_end_select_compare()>0 || noflines==1);
      up();
   }
   restore_pos(p2);
   ...
   // if we make it here, the comment is valid
   restore_pos(p);
   return true;
}
...
_command int box_erase() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   ...
   if (!can_erase_selected_box(s)) {
      _message_box('Incorrect comment chars on line 'p_line);
      return(1);
   }
...
bool extractBoxedLine(_str &line, _str lsc, _str mc = '', _str rsc = '', _str regex = '')
{
   ...
   comment_pos := pos('S0');
   comment_len := pos('0');
   before := expand_tabs(line, 1, comment_pos-1, 'S');
   middle := "";
   after  := "";

   // figure out length of middle segment
   if (mc == '') {
      midlen := comment_len - length(lsc);
      if (rsc != '') {
         midlen -= length(rsc);
      }
      middle = expand_tabs(line, comment_pos + length(lsc), midlen);
   } else {
      // since there is a middle comment string, we know that the
      // middle is made up of comment characters, and therefore not
      // important to us
      middle = '';
   }

   after = expand_tabs(line, comment_pos+comment_len, -1, 'S');
...
_command int comment_erase() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   ...
   for ( ;; ) {
      ...
      ss :=_escape_re_chars(s.m_comment_left):+'(?@|$)';
      if ( s.m_comment_right!="" ) {
      ...
      if ( s.m_comment_col == 0 && stype == 'LINE' ) {
         ...
         // we have a comment col or a block comment in this case
   ...
   // make this is a funky box comment we can erase?
   if (status) {
      if (p_LastModified != orig_LastModified) {
         _undo();
      }
      restore_pos(p);
      if (can_erase_selected_box(s)) {
