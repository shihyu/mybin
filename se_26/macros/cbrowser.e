...
static struct CB_TAG_CATEGORY gh_cb_categories:[] = {
   ...
   "Libraries" => {
      {SE_TAG_TYPE_LIBRARY},
      120, SE_TAG_FLAG_NULL, false, true, false, false, true, true
   },
   CB_packages => {
      {SE_TAG_TYPE_PACKAGE,SE_TAG_TYPE_NAMESPACE},
   ...
   "GUI Event Tables" => {
      {SE_TAG_TYPE_EVENTTAB,SE_TAG_TYPE_EVENT},
...
//
void ctl_class_tree_view.rbutton_up,context()
{
...
// tag file is in the list twice, it will only return the first occurence.
// p_window_id must be the symbol browser tree control.
//
...
static void _ClassBrowserHighlightCallback(_str cmdline)
{
   ...
   // get the details about the current selection
   int currIndex = index;
   if (currIndex <= 0 || !f.ctl_class_tree_view._TreeIndexIsValid(index)) {
      return;
   }
...
int push_pos_in_file(_str file_name, int line_no, int col_no)
{
   ...
   if (mark >= 0) {
      ...
      _mdi.p_child.push_bookmark(mark,persistent_wid: persistent_wid);
   }
   return 0;
}
...
int push_tag_in_file(_str proc_name, _str file_name, _str class_name, _str type_name, int line_no)
{
   ...
   // push the bookmark
   _mdi.p_child.push_destination();
   return(_mdi.p_child.push_bookmark(mark,persistent_wid: persistent_wid));
}
...
void cb_refresh_property_view_for_one_window(struct VS_TAG_BROWSE_INFO cm,int form_wid=-1)
{
   ...
   _nocheck _control ctl_factory_check_box;
   ...
   _nocheck _control ctl_async_check_box;
   _nocheck _control ctl_late_check_box;
   ...
   if (cm.class_name != "" && cm.member_name != "" && cm.type_name :!= "enumc" && cm.type_name :!= "typedef" && cm.type_name :!= "database" && cm.type_name :!="table" && cm.type_name :!= "column" && cm.type_name :!= "view" && cm.type_name :!= "index") {
      ...
      f.ctl_async_check_box.p_enabled        = true;
   } else {
      ...
      f.ctl_export_check_box.p_enabled       = true;
      f.ctl_linkage_check_box.p_enabled      = true;
      f.ctl_async_check_box.p_enabled        = false;
   }
   ...
   case "constr":
      ...
      f.ctl_export_check_box.p_enabled       = true;
      f.ctl_linkage_check_box.p_enabled      = true;
      if (_LanguageInheritsFrom("dart", cm.language)) {
         f.ctl_async_check_box.p_enabled     = true;
         f.ctl_factory_check_box.p_enabled   = true;
      } else if (_LanguageInheritsFrom("cs", cm.language)) {
         f.ctl_async_check_box.p_enabled     = true;
         f.ctl_factory_check_box.p_enabled   = false;
      } else {
         f.ctl_async_check_box.p_enabled     = false;
   ...
   case "prop":
      ...
      f.ctl_late_check_box.p_enabled      = false;
      if (_LanguageInheritsFrom("dart", cm.language)) {
         f.ctl_late_check_box.p_enabled   = true;
      }
      break;
   case "class":
   case "struct":
   ...
   f.ctl_async_check_box.p_value        = (cm.flags & SE_TAG_FLAG_ASYNCHRONOUS)? 1:0;
   f.ctl_factory_check_box.p_value      = (cm.flags & SE_TAG_FLAG_FACTORY)?      1:0;
   f.ctl_late_check_box.p_value         = (cm.flags & SE_TAG_FLAG_LATE)?         1:0;
...
void _ada_disable_props()
{
   ...
   ctl_async_check_box.p_enabled        = false;
   ctl_factory_check_box.p_enabled      = false;
   ctl_late_check_box.p_enabled         = false;
}
...
void _cs_disable_props()
{
   ...
   ctl_constinit_check_box.p_enabled    = false;
   ctl_consteval_check_box.p_enabled    = false;
   ctl_async_check_box.p_enabled        = true;
   ctl_factory_check_box.p_enabled      = false;
   ctl_late_check_box.p_enabled         = false;
}
...
void _tbsymbol_props_form.on_resize(bool forcedResize=false)
{
   ...
   _control ctl_async_check_box;
   _control ctl_factory_check_box;
   _control ctl_late_check_box;
   ...
   check_box_order :+= ctl_factory_check_box;
   ...
   check_box_order :+= ctl_async_check_box;
   ...
   check_box_order :+= ctl_late_check_box;

   // lay out the check boxes in the maximum number of columns that fit.
   tab_index := ctl_static_check_box.p_tab_index;
   ...
   for (i:=0; i<check_box_order._length(); i++) {
      ...
      check_box_order[i].p_tab_index = tab_index++;
   }
...
/**
 * Return true if tool-window with name {@code form_name} 
 * is in the same tabgroup as tool-window {@code wid}. If 
