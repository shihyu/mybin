...
{
   format_list:=_file_types_get_filter_by_name(def_file_types, 'Google Go Files');
   wildcards:=_file_types_get_wildcards_by_name(def_file_types, 'Google Go Files');
   if( format_list == "" ) {
      // Fall back
      format_list = "Google Go Files (*.go)";
      wildcards = "*.go";
   }
...
// Returns the array of search paths for Go module imports.  On most systems, 
// this would be a single path, but it can be a colon/semicolon separated list in the 
// environment.
static void getGOPATH(_str (&paths)[])
{
   paths._makeempty();
   gp := get_env('GOPATH');
   if (gp == '') {
      if (_isWindows()) {
         paths :+= get_env('USERPROFILE')'\go';
      } else {
         paths :+= get_env('HOME')'/go';
      }
   } else {
      while (gp != '') {
         parse gp with auto path (PATHSEP) gp;
         paths :+= path;
      }
   }
}

// Finds the go.mod file in the same directory as or underneath the 
// open project file.  Returns empty string if there's no project open, 
// or it can't be found.  
static _str find_current_project_modfile()
{
   if (_project_name == '') {
      return '';
   }
   ...
   // Check project files.  We limit it to cached project files so we don't
   // do searches on filesystems of unknown size/status.
   ff := true;
   m := '';
   do {
      m = project_file_match('go.mod', ff);
      ff=false;
      if (m != '' && file_exists(m)) {
         break;
      }
...
// Extracts the full_name all of the deps where .selected == true and
// puts them into dest.
static void extract_dep_paths(SelectableDep (&deps)[], _str (&dest)[])
{
   dest._makeempty();
   foreach (auto d in deps) {
      if (d.selected) {
         dest :+= d.full_name;
      }
   }
...
// For go modules, adds any external dependencies so they'll be tagged.
// This can be a long running command if the 'go' command has to connect to
// remote repositories to supply the information.  But as long as the project
// has been built since the last change to go.mod, it should be close to 
// instantaneous.
_command void googlego_add_external_dependencies(_str quiet = '') name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   if (_project_name == '') {
      _message_box('Need a open Go project.');
      return;
   }

   // Ask 'go' tool for module list.
   goexe := guessGoExePath()'go'EXTENSION_EXE;
   proghand := progress_show("Checking external module dependencies.", 20);
   rc := exec_command_to_temp_view(goexe' list -m all', auto tempView, auto origView, proghand, 20, true);
   progress_close(proghand);
   if (rc < 0) {
      _message_box('Could not get module list from go: 'get_message(rc));
      return;
   } else if (rc > 0) {
      p_window_id = origView;
      _delete_temp_view(tempView);
      _message_box('Project is not a go module.');
      return;
   }

   // Paths for replacements can be specified from the location of the go.mod, 
   // so locate the go.mod for the current project.
   proj_dir := _strip_filename(_project_name, 'N');
   _maybe_append_filesep(proj_dir);
   aproj_dir := absolute(proj_dir);
   gomod := find_current_project_modfile();
   gomod_dir := '';
   if (gomod != '') {
      gomod_dir = _strip_filename(gomod, 'N');
      _maybe_append_filesep(gomod_dir);
   }

   //ENHANCEMENT collect vendor directories, and search them first.  If an import is in a 
   // vendor directory, don't add it as an external dependency.

   _str modroots[];

   // If there's a GOPATH, we need to search each one for the actual module locations.
   getGOPATH(modroots);
   for (i := 0; i < modroots._length(); i++) {
      modroots[i] = modroots[i]:+FILESEP:+'pkg':+FILESEP:+'mod':+FILESEP;
   }

   // Parse out module and version information, and then find it in one of the
   // GOPATH locations.
   SelectableDep files[];  files._makeempty();
   top();
   do {
      // ie: github.com/spf13/cast v1.2.0
      //     github.com/spf13/cobra v0.0.3
      // Replacements where we need to use the RHS name and version/path:
      //     go.etcd.io/bbolt v1.3.0 => go.etcd.io/bbolt v1.3.0   
      //     k8s.io/helm v2.11.0+incompatible => ./bonk/helm
      rc = search('^([^ ]+) (.+)$', 'L@>');
      if (rc == 0) {
         modname := get_text(match_length('1'), match_length('S1'));
         ver := get_text(match_length('2'), match_length('S2'));
         if (pos('=>', ver) > 0) {
            // A module replacement of some sort.
            parse ver with auto lhs ' => ' auto repmod ' ' auto repver;
            //say('replacement 'repmod' 'repver);
            fst := substr(repmod, 1, 1);
            if (fst == "." || fst == FILESEP) {
               // A path.  We only need to add this if it looks like it is outside
               // of the workspace path.
               areplacementmod := absolute(repmod, gomod_dir, true);
               //say('VS 'areplacementmod' proj 'aproj_dir);
               if (beginsWith(areplacementmod, aproj_dir)) {
                  // Already covered by tagging, skip it.
                  //say('   skipping, in proj dir');
                  continue;
               } else {
                  SelectableDep d;
                  d.full_name = areplacementmod;
                  d.dep_name = modname;
                  d.ver = '(LOCAL PATH)';
                  d.desc = areplacementmod;
                  files :+= d;
                  continue;
               }
            } else {
               // A simple module version replacement.
               //say('   simple');
               modname = repmod;
               ver = repver;
            }
         ...
         relpath := modname:+'@':+ver;
         foreach (auto modroot in modroots) {
            p := modroot:+relpath;
            if (file_exists(p)) {
               SelectableDep d;
               d.full_name = p;
   ...
   if (files._length() > 0) {
      if (quiet != '') {
         ...
         // Present a dialog.
         rc = user_select_external_dependencies(files);
         if (rc) {
            return;
         }
      }
      ...
      _str paths[];
      extract_dep_paths(files, paths);
      remove_existing_jar_dependencies(0);
      add_dependency_directories(paths, '*.go');
      message('Selected 'paths._length()'/'files._length()' external modules for tagging.');
      tbrefresh_project_by_name(extern_dependency_project_name());
   } else {
      message('No external dependencies found.');
   }
}

// For a project that doesn't have a target for updating the external go 
// modules, add it.
#define UPDATE_MENU_NAME "Update Go Module Dependencies"
static void maybe_add_update_menu_entry(int handle)
{
   ...
   foreach (auto cfg in cfgs) {
      ...
      ment := _xmlcfg_find_simple(handle, "Target[strieq(@Name,'":+UPDATE_MENU_NAME:+"')]", menuroot);
      if (ment < 0) {
         changed = true;
         ment = _xmlcfg_add(handle, menuroot, 'Target', VSXMLCFG_NODE_ELEMENT_START, VSXMLCFG_ADD_AS_FIRST_CHILD);
         _xmlcfg_add_attribute(handle, ment, 'Name', UPDATE_MENU_NAME);
         _xmlcfg_add_attribute(handle, ment, 'MenuCaption', UPDATE_MENU_NAME);
         ...
         _xmlcfg_add_attribute(handle, ex, 'CmdLine', 'googlego-add-external-dependencies');
...
void _prjopen_googlego(bool singleFileProject)
{
   ...
   if (h) {
      if (_ProjectGet_TemplateName(h) == 'Google Go') {
