...
static void _init_options(_str search_options)
{
   ...
   VSWID_HIDDEN.search('',search_options);
   save_search(auto a1, auto search_flags, auto a3,auto a4,auto a5);
   ...
   if (search_flags & (VSSEARCHFLAG_RE|VSSEARCHFLAG_PERLRE|VSSEARCHFLAG_VIMRE|VSSEARCHFLAG_WILDCARDRE)) {
      _findre.p_value = 1;
      if (search_flags & VSSEARCHFLAG_PERLRE) {
...
_findstring.on_create(_str init_options='',_str wid=0)
{
   ...
   if (def_keys == "brief-keys" && !pos('-',init_options)) {
      // Search in forward direction
      strappend(init_options,'+');
   }
   if (_retrieve_prev_form() || def_find_init_defaults) {
      _init_options(_search_default_options():+init_options);
   }
   if (wid && !wid._isEditorCtl(false)) {
      _findre_type.p_visible=_findre.p_visible=_findword.p_visible=_findback.p_visible=false;
      _findhidden.p_visible=_findcursorend.p_visible=_findmark.p_visible=_findwrap.p_visible=false;
      _findcase.p_y=_findback.p_y;
      height := _dy2ly(p_active_form.p_xyscale_mode, p_active_form.p_client_height);
      cancel_y_extent := _findcancel.p_y_extent;
      diff:= height- (cancel_y_extent + _findok.p_y);
   ...
   if (!def_find_init_defaults) {
      _init_options(old_search_options:+init_options);
   }

   if (old_search_misc_options != '') {
      _init_misc_search_opts(old_search_misc_options,init_options);
   }
   ...
   if (init_str != '') {
      if ((def_mfsearch_init_flags & MFSEARCH_INIT_AUTO_ESCAPE_REGEX) &&
          _findre.p_value) {
         options := 'R';
         if (_findre_type.p_text == RE_TYPE_PERL_STRING) {
             options = 'L';
         } else if (_findre_type.p_text == RE_TYPE_VIM_STRING) {
            options = '~';
         } else if (_findre_type.p_text == RE_TYPE_WILDCARD_STRING) {
            options = '&';
         }
...
static void _init_misc_search_opts(_str options,_str init_options)
{
   _str opt, rest;
   parse options with opt ";" rest;

   _findwrap.p_value = 0;
   direction_specified:=pos('[+\-]',init_options,1,'r');
 ...
 * @param search_string  String to search for
 * @param search_options  Search options. See {@link find} 
 *          command for search options. The backward search direction option('-')
 *          is not supported. You can use '*' to delete without prompting.
 * 
 ...
 * contain "xyz" using the default search case.</DD> 
 * <DT><b>delete-search `xyz`*</b></DT><dd>Remove lines which 
 * contain "xyz" using the default search case without prompting.</DD> 
 ...
 * @categories File_Functions, Edit_Window_Methods, Editor_Control_Methods, Search_Functions
 */
_command int delete_search(_str search_string='', _str search_options=null, bool copyDeletedLinesToClipboard=false) name_info(','VSARG2_MARK|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   if (search_options==null) {
      parse search_string with  1 auto delim +1  search_string (delim) search_options;
   ...
   int temp_wid=0;
   if (copyDeletedLinesToClipboard) {
      utf8:=p_UTF8;
   ...
   for (;;) {
      ...
      if (doDelete) {
         ...
         if (copyDeletedLinesToClipboard) {
            temp_wid.bottom();
            temp_wid._move_to_cursor(match_id);
         } else {
   ...
   if (copyDeletedLinesToClipboard) {
      ...
      temp_wid.select_all_line();
      temp_wid.copy_to_clipboard();
...
/**
 * Deletes lines containing a match and copies them to the 
 * clipboard. 
 *  
 * <p>Performs a prompted search unless the '*' option is 
 * spcecified. 
 ...
 * @example Command Line Examples
 * <DL style="margin-left:20pt;">
 * <DT><b>cut-search /xyz/</b></DT><dd>Remove lines which 
 * contain "xyz" using the default search case..</DD> 
 * <DT><b>cut-search $/$</b></DT><DD>Remove lines which contain
 * the character slash. Any non-blank delimiter can be 
 * used.</DD> 
 * <DT><b>cut-search /i/w</b></DT><DD>Remove lines which 
 * contain a word match of i.</DD> 
 * <DT><b>cut-search /xyz/e</b></DT><DD>Remove lines which 
 * contain "xyx" in exact case.</DD> 
 * </DL>
 *  
 * @see find_next
 ...
 * @categories File_Functions, Edit_Window_Methods, Editor_Control_Methods, Search_Functions
 */
_command int cut_search(_str search_string='', _str search_options=null, bool copyDeletedLinesToClipboard=false) name_info(','VSARG2_MARK|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   return delete_search(search_string,search_options,true);
}
/**
...
{   
   ...
   return(_mffind(search_string, search_options, 
                  ...
                  MFFIND_GLOBAL|MFFIND_QUIET|MFFIND_INTERNAL_EXCLUDE_BINARY_FILES, 
                  true, false, 
                  ALLFILES_RE, '', true, 0));
}
/**
...
#endif
      //if (wildcards=='') {
         _file_types_add_wildcards_to_combo();
