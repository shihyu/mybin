...
void _ctlclose.on_create(se.vc.IVersionedFile *pVersionedFile=null,_str version="",
                         ...
                         bool saveLocalFileRestore=true)
{
   _SetDialogInfoHt("inOnCreate",1);
   _SetDialogInfoHt("inOnResize",0);
   _SetDialogInfoHt("saveLocalFileRestore",saveLocalFileRestore);
   if ( !_haveProDiff() ) {
   ...
   gVersionedFiles:[p_active_form.p_window_id] = *pVersionedFile;
   pVersionedFile = &gVersionedFiles:[p_active_form.p_window_id];
   _DiffSetFileLabelsMissing();

//   _SetDialogInfoHt("pVersionedFile",pVersionedFile,_ctlfile1);
   localFilename := pVersionedFile->localFilename();

   parse buf_match(localFilename,1,'xv') with auto localFileBufID .;
   _nocheck _control _ctlfile1;
   _SetDialogInfoHt("localFileBufID", localFileBufID, _ctlfile1);
   ...
   if ( lowcase(typeCaption)=="backup history" ) {
      p_active_form.p_caption = BACKUP_HISTORY_DIALOG_CAPTION_PREFIX:+pVersionedFile->localFilename(version);
   } else {
      p_active_form.p_caption = HISTORY_DIFF_DIALOG_CAPTION_PREFIX:+pVersionedFile->typeCaption()' - 'pVersionedFile->localFilename(version);
   }
...
se.vc.IVersionedFile gVersionedFiles:[];
...
{
   se.vc.IVersionedFile *pVersionedFile = getPointerToVersionedFile();
...
void _history_diff_form.on_load()
{
   ...
   if ( *pVersionedFile instanceof se.vc.BackupHistoryVersionedFile ) {
      ...
      retrieveValueGrabBarVert = _moncfg_retrieve_value( "_history_diff_backup_history_form._grabbar_vert" );
   } else {
      retrieveValueGrabBarHorz = _moncfg_retrieve_value( "_history_diff_form._grabbar_horz" );
      retrieveValueGrabBarVert = _moncfg_retrieve_value( "_history_diff_form._grabbar_vert" );
   }

   if (retrieveValueGrabBarHorz != null && isinteger(retrieveValueGrabBarHorz)) _grabbar_horz.p_y = retrieveValueGrabBarHorz;
   if (retrieveValueGrabBarVert != null && isinteger(retrieveValueGrabBarVert)) _grabbar_vert.p_x = retrieveValueGrabBarVert;
}

se.vc.IVersionedFile *getPointerToVersionedFile()
{
   if ( !gVersionedFiles._indexin(p_active_form.p_window_id) ) {
      return null;
   }
   return &gVersionedFiles:[p_active_form.p_window_id];
}

void returnFocusToEditor()
{
   wid := _GetDialogInfoHt("lastActiveEditWindowID");
   if ( wid != null && _iswindow_valid(wid) ) {
      wid._set_focus();
   }
}
...
void _ctlclose.on_destroy()
{
   ...
   localFileBufID := _GetDialogInfoHt("localFileBufID", _ctlfile1);
   DIFF_SHOWING_INFO misc = _DiffGetDiffShowingInfo();
   ...
   if ( *pVersionedFile instanceof se.vc.BackupHistoryVersionedFile ) {
      ...
      _moncfg_append_retrieve(_grabbar_vert, _grabbar_vert.p_x, "_history_diff_backup_history_form._grabbar_vert" );
      if ( _GetDialogInfoHt("saveLocalFileRestore")==true )  {
      ...
      _moncfg_append_retrieve(_grabbar_vert, _grabbar_vert.p_x, "_history_diff_form._grabbar_vert" );
   ...
   _safe_hidden_window();
   origBufID := p_buf_id;
   if ( misc!=null && misc.buffer1.WholeFileBufId!=-1 ) {
      _free_selection(misc.buffer1.MarkId);

      // Load buffer whether it was in memory or not
      status := load_files('+bi 'misc.buffer1.WholeFileBufId);
      if ( !status ) {
         // If buffer was in memory, delete it.  Otherwise reset read only
         // and isdiffed
         if ( misc.buffer1.SymbolViewId != null && misc.buffer1.SymbolViewId!=0 ) {
            // This file was not in memory, close it.
            _delete_buffer();
         } else {
            p_readonly_mode = misc.buffer1.origReadOnly;

            // there are increments. Be sure this buffer is not actually diffed
            // let the increments go for the other buffers, this is a special
            // case where we know we are done diffing this buffer
            _SetBufferInfoHt(DIFFEDIT_CONST_BUFFER_IS_DIFFED:+localFileBufID, null);
         }
         load_files('+bi 'origBufID);
      }
   } else {
      status := load_files('+bi 'localFileBufID);
      if ( !status ) {
         _SetBufferInfoHt(DIFFEDIT_CONST_BUFFER_IS_DIFFED:+localFileBufID, null);
      }
      load_files('+bi 'origBufID);
...
static void resizeDialog(typeless formWID="")
{
   ...
   ctlframe1.p_width = clientWidth - (4*bufferX);
   widthDiv3 := ctlframe1.p_width intdiv 3;
   ctlframe1.p_y_extent = _grabbar_horz.p_y ;

   ctlVersionList.p_width = _grabbar_vert.p_x-bufferX;
   //_grabbar_vert.p_x = ctlVersionList.p_x_extent+bufferX;
   ctlminihtml1.p_height = ctlVersionList.p_height = ctlframe1.p_height-(2*bufferY);

   ctlminihtml1.p_x = _grabbar_vert.p_x_extent;

   ctlminihtml1.p_width = ctlframe1.p_width - (ctlminihtml1.p_x+bufferX);
   ...
   _grabbar_vert.p_y = bufferY;
   _grabbar_vert.p_height = ctlframe1.p_height - (2*bufferY);

   vscroll1.p_x=_ctlfile1.p_x_extent;
   _ctlfile2.p_x = vscroll1.p_x_extent;

   editorHeight := clientHeight-(ctlframe1.p_height+_ctlcopy_left.p_height+ctltype_combo.p_height+ctlcontextCombo1.p_height+hscroll1.p_height+_grabbar_horz.p_height+_ctlfile1line.p_height+(11*bufferY));
   ...
   _ctlfile1line.p_y = hscroll1.p_y_extent + bufferY;

   ctlcomment.p_y = ctlopen.p_y = ctlsave_as.p_y = _ctlclose.p_y = ctlNextDiff.p_y = ctlPrevDiff.p_y = ctltypetoggle.p_y = ctltypetoggle.p_next.p_y = _ctlfind.p_y = _ctlfile1line.p_y_extent+bufferY;
...
{
   origWID := p_window_id;
   p_window_id = p_active_form/*.ctlframe1*/;
   clientHeight := _dy2ly(SM_TWIP,p_client_height);
   clientWidth := _dx2lx(SM_TWIP,p_client_width);
   p_window_id = origWID;
...
void _history_diff_form.on_got_focus()
{
   _ctlfile1.hex_off();
   _ctlfile2.hex_off();
...
static int loadTreeVersionForStaticFile(bool doFirstDifference)
{
   ...
   if ( curVersionWID==null ) {
      ...
      _DiffAddToDeleteList(curVersionWID.p_buf_id,false,"",&delItemList);

      misc.buffer1.origReadOnly = p_readonly_mode;
      typeless t=2;
      ...
      _DiffAddToDeleteList(origCurVersionWID.p_buf_id,false,"",&delItemList);
      _delete_temp_view(origCurVersionWID, false);
      _DiffSetDeleteList(delItemList,origFID._ctlfile1);

      misc.buffer1.WholeFileBufId = origBufID;
      misc.buffer1.MarkId = markid;
#if 0 //16:18pm 7/6/2021
      {
         p_window_id = HIDDEN_WINDOW_ID;
         _safe_hidden_window();
         origBufID = p_buf_id;
         status = load_files('+bi 'misc.buffer1.WholeFileBufId);
         if (status) {
            say('loadTreeVersionForStaticFile !!!! misc.buffer1.WholeFileBufId='misc.buffer1.WholeFileBufId' already invalid status='status);
         }
         p_window_id = origWID;
      }
#endif
...
//   say('loadTreeVersionForStaticFile diff 15 time='(int)t15-(int)t11);
   ...
   status = _ctlfile2.load_files('+bi 'curPrevVersionWID.p_buf_id);
...
_command void history_diff_machine(_str filename="",_str version="") name_info(FILE_ARG','VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveBackupHistory()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_OR_STANDARD_EDITION_1ARG, "Backup History");
      return;
   ...
   show('-xy -modal _history_diff_form',&backupHistoryFile, version);
}

_command void history_diff_machine_file(_str filename="") name_info(FILE_ARG','VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveBackupHistory()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_OR_STANDARD_EDITION_1ARG, "Backup History");
      return;
   ...
   show('-xy _history_diff_form',&backupHistoryFile,"",localDiff,true);
}

void _HistoryDiffBackupHistoryFile(_str filename,_str version,bool useStaticFile=true,bool saveLocalFileRestore=true)
{
   se.vc.BackupHistoryVersionedFile backupHistoryFile(filename);

   show('-modal -xy  _history_diff_form',&backupHistoryFile,version,useStaticFile,false,saveLocalFileRestore);
}
