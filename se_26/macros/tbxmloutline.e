...
bool isOutlineViewActive(int child_wid= -1, bool ForceUpdate=false)
{
   if (def_outline_view_enabled == true) {
      if (ForceUpdate) return true;
      return isCurrentBufferSupportedForOutlineView(child_wid, ForceUpdate);
   }
   return false;
}
 ...
 * not. 
 */
bool isCurrentBufferSupportedForOutlineView(int child_wid=-1, bool ForceUpdate=false)
{
   if (child_wid<=0) {
      if (child_wid==0) {
         return false;
   ...
   langID := child_wid.p_LangId;
   supported := isLanguageSupportedForOutlineView(langID);
   if (!supported) {
      return false;
   }
   if (ForceUpdate) {
   ...
   if (!child_wid._CheckUpdateContextSizeLimits(MODIFYFLAG_CONTEXT_UPDATED, quiet:true)) {
      return false;
   }
   return true;
}
