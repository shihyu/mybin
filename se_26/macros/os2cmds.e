...
_str _VslickErrorInfo(bool InsertEcho=true)
{
   ...
   if (!def_show_build_vslickerrorpath && InsertEcho) {
      if (_isUnix()) {
         return(_chr(2):+Echo:+'-e "\002"':+'VSLICKERRORPATH="'getcwd():+VslickIncludePath'"');
      } else {
         return(_chr(2):+Echo:+_chr(2):+'VSLICKERRORPATH="'getcwd():+VslickIncludePath'"');
      }
   } else {
...
_command void toggle_process_tab_output() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   if (p_buf_name=='.process') {
      // toggle the option (for the Build tool window)
      def_process_tab_output = !def_process_tab_output;
      _config_modify_flags(CFGMODIFY_DEFVAR);

      if (!def_process_tab_output && !p_mdi_child) {
         //
         // turning the option OFF from the tool window
         //
         // INVOKED FROM:  tool window
         // PRECONDITION:  send build output to tool window
         // POSTCONDITION: send build output to editor
         // ACTION:        need to activate editor window
         //
         start_process_window();
         if (p_LangId=='process') {
            bottom();_end_line();
         }
      } else if (!def_process_tab_output && p_mdi_child) {
         //
         // turning the option OFF from an editor window
         //
         // INVOKED FROM:  editor window
         // PRECONDITION:  send build output to tool window
         // POSTCONDITION: send build output to editor
         // ACTION:        do nothing, editor already active
         //
      } else if (def_process_tab_output && !p_mdi_child) {
         //
         // turning the option ON from the tool window
         //
         // INVOKED FROM:  tool window
         // PRECONDITION:  send build output to editor
         // POSTCONDITION: send build output to tool window
         // ACTION:        if the editor build window is not active,
         //                make it active, and leave option alone
         //                otherwise, do nothing
         //
         child_wid := _MDIGetActiveMDIChild();
         if (child_wid && child_wid.p_LangId != 'process') {
            // editor window is active, so just make it current
         ...
         ///
         // turning the option ON from an editor window (activate the tool window)
         // 
         // INVOKED FROM:  editor window
         // PRECONDITION:  send build output to editor
         // POSTCONDITION: send build output to tool window
         // ACTION:        close the editor window, and active the build tool window
         //
         if (p_LangId=='process') quit();
         start_process_tab();
      }
      ...
      if (_process_is_terminal_idname(idname)) {
         // toggle the option (for the Terminal tool window)
         def_terminal_tab_output = !def_terminal_tab_output;
         _config_modify_flags(CFGMODIFY_DEFVAR);

         if (!def_terminal_tab_output && !p_mdi_child) {
            // turning the option OFF from the tool window
            start_process_window(false,true,false,true,idname);
            if (p_LangId=='process') {
               bottom();_end_line();
            }
         } else if (def_terminal_tab_output && !p_mdi_child) {
            // turning the option ON from the tool window
            child_wid := _MDIGetActiveMDIChild();
            if (child_wid && child_wid.p_DocumentName != _terminal_idname_to_tab_name(idname)) {
               // editor window is active, so just make it current
            ...
            // turning the option ON from an editor window (activate the tool window)
            if (p_DocumentName == _terminal_idname_to_tab_name(idname)) quit();
            start_process_tab(false,true,false,true,idname);
         }

      } else if (_process_is_interactive_idname(idname)) {
         // toggle the option (for the Interactive tool window)
         def_interactive_tab_output = !def_interactive_tab_output;
         _config_modify_flags(CFGMODIFY_DEFVAR);

         if (!def_interactive_tab_output && !p_mdi_child) {
            // turning the option OFF from the tool window
            start_process_window(false,true,false,true,idname);
            if (p_LangId=='process') {
               bottom();_end_line();
            }
         } else if (def_interactive_tab_output && !p_mdi_child) {
            // turning the option ON from the tool window
            child_wid := _MDIGetActiveMDIChild();
            if (child_wid && child_wid.p_DocumentName != _interactive_idname_to_tab_name(idname)) {
               // editor window is active, so just make it current
            ...
            // turning the option ON from an editor window (activate the tool window)
            if (p_DocumentName == _interactive_idname_to_tab_name(idname)) quit();
            start_process_tab(false,true,false,true,idname);
         }
      }
   }
...
int _OnUpdate_toggle_process_tab_output(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   if (!target_wid || !target_wid._isEditorCtl(false) || target_wid.p_buf_name=='.process') {
      checked := (def_process_tab_output? 0:MF_CHECKED);
      return (checked|MF_ENABLED);
   } 
   idname := target_wid._ConcurProcessName();
   if (_process_is_terminal_idname(idname)) {
      checked := (def_terminal_tab_output? 0:MF_CHECKED);
      return (checked|MF_ENABLED);
   }
   if (_process_is_interactive_idname(idname)) {
      checked := (def_interactive_tab_output? 0:MF_CHECKED);
      return (checked|MF_ENABLED);
   }
   // not a build, terminal, or interactive window
   return MF_GRAYED;
}
