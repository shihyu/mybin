...
int _dmDoLetter(_str letter)
{
   ...
   case OI_LABEL:
      if (wid.p_eventtab) {
         if (eventtab_index(wid.p_eventtab,wid.p_eventtab,event2index(LBUTTON_UP)) > 0) {
            p_window_id = wid;
            wid.call_event(wid,LBUTTON_UP);
            return(0);
         } else if (eventtab_index(wid.p_eventtab,wid.p_eventtab,event2index(LBUTTON_DOWN)) > 0) {
            p_window_id = wid;
            wid.call_event(wid,LBUTTON_DOWN);
            return(0);
         }
      }
      break;
