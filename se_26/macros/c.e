...
void _c_endbrace(bool inhibit_beautify=false) {
   ...
   // if a close brace is typed within an if or try statement, then the only
   // thing they could be doing is trying to add an else or else-if
   if (c_is_extension_of_statement('if|try|catch')) {
      c_expand_space();
   }
}
                     ...
                     bool optSemicolon_searches_for_newline=true)
{
   ...
   for (;;) {
      ...
      */ 
      if (in_powershell && optSemicolon && _powershell_treat_this_newline_like_a_semicolon()) {
         status=0;
      } else {
...
static int find_block_col()
{
   ...
   */
   ...
   if (cfg==CFG_KEYWORD && pos(' 'word' ',' with for foreach foreach_reverse if elsif elseif switch while lock using trap catch ')) {
      ...
      return(p_col);
      //return(p_col-length(word)+1);
   } else if (_LanguageInheritsFrom('c') && word == 'requires') {
      // 'requires' is only treated as a keyword in C++ concept declarations.
      _first_non_blank();
      return(p_col);
...
static SYNTAX_EXPANSION_INFO julia_space_words:[]= {
   'break'            => { "break" },
   'catch'            => { "catch" },
   'continue'         => { "continue" },
   'elseif'           => { "elseif" },
...
static SYNTAX_EXPANSION_INFO dart_space_words:[] = {
   'main'             => { "void main() { ... }" },
   'print '           => { "print(\"\");" },
   'assert'           => { "assert(...);" },
   'var'              => { "var" },
...
{
   if (p_LangId=='rs'  || p_LangId=='powershell' || p_LangId=='julia' || p_LangId=='googlego') return false;
   save_pos(auto p);
                                         ...
                                         int* start_col = null)
{
   ...
   if (ch != ')') {
      ...
      curline := p_line;
      save_pos(auto p2);
      save_selection(auto orig_mark);
      _deselect();_select_line();
      s:=_nrseek()-1000000;  // Limit search to 1 megabyte
      if (s<0) s=0;
      goto_point(s);
      _select_line();
      restore_pos(p2);
...
// If the cursor is between the ) and possibly the '{' of
// a requires() clause in a C++20 concept declaration, returns
// the column position of the requires keyword, and true.
// Returns false otherwise.
static bool after_requires_clause(int& requires_col)
{
   rv := false;
   save_pos(auto curpos);
   _clex_skip_blanks('-');
   if (get_text_safe() == ')') {
      if (_find_matching_paren(MAXINT, true) == 0) {
         prev_char();
         rcol := 0;
         word := cur_identifier(rcol);
         if (word == 'requires') {
            rv = true;
            requires_col = rcol;
         }
...
bool _c_expand_enter()
{
   ...
   // Handle requires here, because it's really just a piece of a declaration,
   // not a statement that we'd want c_begin_stmt_col to stop on.
   if (_LanguageInheritsFrom('c') && after_requires_clause(auto requires_col) && 
   ...
   if (_LanguageInheritsFrom('groovy') || _LanguageInheritsFrom('scala') || _LanguageInheritsFrom('r') ||
       _LanguageInheritsFrom('kotlin') || _LanguageInheritsFrom('julia') || 
       _LanguageInheritsFrom('googlego') || _LanguageInheritsFrom('swift')
       ) {
      idx := find_index('calc_nextline_indent_from_tags', PROC_TYPE);
      if (idx > 0) {
         if (_LanguageInheritsFrom('julia')) {
            save_pos(auto julia_pos);
            julia_rest:='';
            julia_end_col:=0;
            julia_orig_col:=p_col;
            _begin_line();
            julia_status:=search('^[ \t]@\c{end|elseif|else|finally|catch}([ \t]|$)|','hr@');
            end_text:=get_match_text(0);
            julia_new_col:=p_col;
            if (end_text!='' && !julia_status && julia_orig_col>_text_colc(0,'L')) {
               julia_rest= _expand_tabsc(p_col,-1,'S');
               julia_end_col= _julia_get_end_col();
            }
            restore_pos(julia_pos);
            if (julia_end_col && julia_new_col!=julia_end_col) {
               if (!_macro('R')) _undo('s');
   ...
   //say("first_word="first_word", splits="line_splits", orig_line="cur_line", LineEndsWithBrace="LineEndsWithBrace);
   ...
   // C++ concept requires clause.
   if ((p_LangId == 'c' || p_LangId == 'm') && first_word == 'requires') {
      if (LineEndsWithBrace) {
         indent_on_enter(0, fw_col+p_SyntaxIndent);
      } else {
         indent_on_enter(0, fw_col);
      }
      return false;
   }
...
static void dangling_clause_continue(_str word) {
   for (;;) {
      /*
         ...
         Might need this fix for other languages with "if" expressions.
      */
      bool word_was_if= (word=='if' && _LanguageInheritsFrom('powershell'));
      save_pos(auto p);
      if (p_col==1) {
         up();_end_line();
      } else {
      ...
      if (_clex_find(0,'g') != CFG_KEYWORD) {
         ...
         if (word_was_if) {
            ...
            */
            ...
            begin_stat_col := c_begin_stat_col(true /* RestorePos */,
                                  false /* Don't skip first begin statement marker */,
                                  true /* Don't return first non-blank */,
                                  false  /* Return 0 if no code before cursor. */,
                                  false, /* haven't already recursed */
                                  true  /* return -1 if no text after cursor*/
                                  );
            if (begin_stat_col<=0) {
               if (doRight) {
                  right();
...
int _c_get_syntax_completions(var words)
{
   ...
   if (_LanguageInheritsFrom('phpscript')) {
      ...
      space_words = rust_space_words;
   } else if (_LanguageInheritsFrom('julia')) {
      space_words = julia_space_words;
   } else if (_LanguageInheritsFrom('dart')) {
      space_words = dart_space_words;
   } else {
...
// is the code above the current line (which contains 'else', "catch" or "finally")
// a partial if {  } or try { } statement
bool c_is_extension_of_statement(_str stmt_keywords, 
                                 bool must_be_within_stmt=true, 
                                 bool must_be_followed_by_extension=false)
{
   ...
   // keep track of the open brace position
   brace_col := 1;
   ...
   do {
      ...
      if (get_text() != '}') {
         left();
         else_or_catch_or_finally := cur_identifier(auto start_col);
         if (else_or_catch_or_finally != "") {
            p_col = start_col;
            if (_QROffset() <= 0) break;
      ...
      // skip backwards from the open brace
      if (_QROffset() <= 0) break;
      _GoToROffset(_QROffset()-1);
      status = _clex_skip_blanks('h-');

      // found 'try' keyword?
      if (pos('|':+cur_identifier(auto start_col):+'|',  '|':+stmt_keywords:+'|') > 0) {
         found_stmt = true;
         break;
      }

      // skip parens on 'catch' or 'if' statement
      if (get_text() != ')') {
         break;
      }
      status = find_matching_paren(true);
      ...
      if (pos('|':+cur_identifier(start_col):+'|',  '|':+stmt_keywords:+'|') > 0) {
         found_stmt = true;
      }

   } while (false);

   // insert an open brace to match looking for the end statement brace
   // remove it if we do not find it on the same column
   if (found_stmt && must_be_within_stmt) {
      restore_pos(brace_pos);
      insert_line('{');
      status := find_matching_paren(true);
      if (status || p_col != brace_col) {
         found_stmt = false;
      } else if (must_be_followed_by_extension) {
         _GoToROffset(_QROffset()+1);
         status = _clex_skip_blanks('h');
         if (pos('|':+cur_identifier(auto start_col):+'|',  '|else|elseif|catch|finally|') <= 0) {
            found_stmt = false;
         }
      }
      restore_pos(brace_pos);
...
static int c_expand_space()
{
   ...
   if (semicolon_case) {
      ...
      word=min_abbrev2(nobrace_word,dart_space_words,"",
                       aliasfilename,!open_paren_case,open_paren_case);
      is_dart = true;
      ...
      word=min_abbrev2(nobrace_word,julia_space_words,"",
                       aliasfilename,!open_paren_case,open_paren_case);
      is_julia=true;
   } else {
   ...
   if ( word=='' && doSyntaxExpansion) {
      ...
      if (is_julia) {
         if ((first_word:=='global' || first_word=='local') && second_word=='function' && rest=='') {
            word=first_word' 'second_word;
         } else if (first_word:=='mutable' && second_word=='struct' && rest=='') {
            word=first_word' 'second_word;
         }
      } else if (!def_always_prompt_for_else_if && first_word=='}' && second_word!='' && rest=='' && second_word=='else') {
         ...
         //We need a modeless dialog so user can keep typing.
         return(1);
      } else if (first_word=='}' && second_word=='' && c_is_extension_of_statement('if', must_be_within_stmt:false)) {
         brace_before='} ';
         ...
         word='}finally';
         else_special_case=true;
      } else if (is_dart && first_word=='}' && second_word!='' && rest=='' && second_word:==substr('on',1,length(second_word))) {
         word='} on  catch';
         if_special_case=true;
      } else if (is_dart && second_word=='' && length(first_word)>1 && first_word:==substr('}on',1,length(first_word))) {
         word='}on  catch';
         if_special_case=true;
      ...
      } else if ((is_cpp || _LanguageInheritsFrom('m')) && first_word == 'requires') {
         // Requires clause in c++20 concept.
         word = "requires";
      } else {
         return(1);
      }
   ...
   if (pick_else_or_else_if) {
      if (!def_always_prompt_for_else_if && c_is_extension_of_statement('if', must_be_within_stmt:true, must_be_followed_by_extension:true)) {
         if (is_php || _LanguageInheritsFrom('rul')) {
            word="elseif";
         } else {
            word="else if";
         }
      } else if (is_php || _LanguageInheritsFrom('rul')) {
   ...
   if ( word=='main' ) {
      if (_LanguageInheritsFrom('java') || p_LangId == 'groovy') {
         ...
         // If there is no class in this file
         replace_line(indent_string(width)'void main(List<String> arguments) {');
         insert_line("");
         insert_line(indent_string(width)'}');
         up();p_col=width+((indent_fl)?syntax_indent:0)+1;
      ...
      // If there is no class in this file
      if (semicolon_case) {
         replace_line(indent_string(width)'void ();');
         semicolon_case=false;
      } else {
         replace_line(indent_string(width)'void () {');
         insert_line("");
         p_col=width+((indent_fl)?syntax_indent:0)+1;
         add_hotspot();
         insert_line(indent_string(width)'}');
         up();
         up();
      ...
      p_col=width+4+2;
      add_hotspot();
   } else if ( is_julia && (word=='else' || word=='finally' || word=='return' || word=='break' || word=='continue')) {
      ...
      _end_line();
   } else if ( is_julia && (word=='elseif' || word=='catch' || word=='finally')) {
      if (nobrace_word==word) {
         if (semicolon_case) {
            replace_line(line';');
         } else {
      ...
      _end_line();
               ...
               (word=='for' || word=='function' || word=='macro' ||
                word=='global function' || word=='local function' || 
                word=='if' || word=='let' || word=='begin' || 
                word=='module' || word=='baremodule' || 
                word=='struct'|| word=='mutable struct' || word=='while' || word=='try')) {
      julia_space_after:=' ';
      if (word=='begin' || word=='try') {
         julia_space_after='';
      }
      replace_line(line:+julia_space_after);
      if (word=='try') {
         insert_line(indent_string(width+p_SyntaxIndent));
         insert_line(indent_string(width):+'catch ');surround_end_line:=p_line;
         insert_line(indent_string(width):+'end');
         set_surround_mode_end_line(surround_end_line,2);
         up(2);_end_line();
      } else {
         insert_line(indent_string(width):+'end');
   ...
   } else if ( word=='if' || word=='elseif' || word=='else if' || word=='catch' || if_special_case || (is_dart && word == "on  catch")) {
      ...
      save_pos(auto after_on);
      if (is_dart && endsWith(word, "on  catch")) {
         p_col = length(line)-5;
         add_hotspot();
         save_pos(after_on);
      }
      stmt_keyword := (endsWith(word, "catch"))? 'try' : 'if';
      expansion_end = _c_maybe_insert_braces(noSpaceBeforeParen, 
                                             insertBraceImmediately && !c_is_extension_of_statement(stmt_keyword),
                                             width, word,
                                             c_else_followed_by_brace_else(word),
                                             openparen!='');
      if (openparen!='') maybe_autobracket_parens();
      add_hotspot();
      // go back to the first hotspot (for Dart)
      if (is_dart && endsWith(word, "on  catch")) {
         restore_pos(after_on);
      }
   } else if ( word=='else' || word=='finally') {
      typeless p;
      typeless s1,s2,s3,s4;
      ...
      stmt_keyword := (word=='finally' || word == '}finally' || word == '} finally')? 'try' : 'if';
      replace_line(line:+e1);
      expansion_end = _c_maybe_insert_braces(noSpaceBeforeParen, !c_is_extension_of_statement(stmt_keyword), width, word);
      _end_line();
      ...
      */
      ...
      expansion_end = _c_maybe_insert_braces(noSpaceBeforeParen, insertBraceImmediately,width,word);
      add_hotspot();
   } else if ((is_idl || is_php || is_dlang || is_dart) && pos(' 'word' ',' exception interface struct class module union ' )) {
      replace_line(line' 'e1);
      expansion_end = _c_maybe_insert_braces(noSpaceBeforeParen, insertBraceImmediately,width,word);
      if (maybespace:==' ') {
         left();
   ...
   } else if ((is_dart) && (word=='mixin' || word=='abstract class')) {
      replace_line(line' 'e1);
      expansion_end = _c_maybe_insert_braces(noSpaceBeforeParen, insertBraceImmediately,width,word);
      if (maybespace:==' ') {
         left();
      ...
      replace_line(line'  union'maybespace'('parenspace:+parenspace')'e1);
      expansion_end = _c_maybe_insert_braces(noSpaceBeforeParen, insertBraceImmediately,width,word);
      left();
      maybe_autobracket_parens();
      ...
      if (!style3) {
         ...
         if (!cuddleElse) {
            ...
            if (is_dart) {
               insert_line(indent_string(width)'on  catch':+maybespace'('parenspace:+parenspace')'e1);
               p_col = width+4;
               add_hotspot();
            } else {
            ...
            if (is_dart) {
               insert_line(indent_string(width)'}'bracespace'on  catch':+maybespace'('parenspace:+parenspace')'e1);
               p_col = width+1+length(bracespace)+4;
               add_hotspot();
            } else {
      ...
      left();
      left();
   } else if ( word=='assert' || word=="static_assert" ) {
      replace_line(indent_string(width):+word:+'();');
      _end_line();
      left();
      left();
   } else if ( word=='import' && _LanguageInheritsFrom('dart')) {
      replace_line(indent_string(width)"import '';");
      _end_line();
      left();
      left();
...
int _dart_delete_char(_str force_wrap='')
{
   return _c_delete_char(force_wrap);
}
int _dart_rubout_char(_str force_wrap='')
{
   return _c_rubout_char(force_wrap);
}
...
bool supports_advanced_bracket_cfg(_str langId) 
{
   ...
   // use c_begin()
   return new_beautifier_supported_language(langId) || langId=='e' || langId=='as' || langId=='r' || _LanguageInheritsFrom('kotlin', langId) || langId=='kotlins' || langId=='clojure' || langId=='yaml' || _LanguageInheritsFrom('terraform',langId) || _LanguageInheritsFrom('julia',langId);
}
...
// Returns true if the cursor is in the body of a "requires"
// clause in a C++ concept declaration.  
static bool in_requires_body()
{
   return _c_last_enclosing_control_stmt('requires', auto found) > 0;
}
...
static int c_expand_begin()
{
   ...
   if (line == '' && in_requires_body()) {
           // C++ concepts: In a requires body, braces surround expressions, and should
           // not introduce newlines.
        forceSamelineAutobrace=true;
  }

   requires_col := -1;
   if (_LanguageInheritsFrom('c') || _LanguageInheritsFrom('m')) {
      after_requires_clause(requires_col);
   }
   ...
   */
   ...
   if (!forceSamelineAutobrace && p_LangId == 'c' && line != '') {
      startLine := p_line;
      left(); left();
   ...
   } else if (requires_col != -1) {
      begin_brace_col = col = requires_col;
   } else if ( line!='{' ) {
