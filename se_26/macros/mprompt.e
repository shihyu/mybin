   ...
   if( arg(FIRST_DATA_ARG_INDEX) != "" ) {
      ...
      for( ;; ) {
         ...
         for( ;; ) {
            ...
            } else if( strieq(option,'BNF') ) {
               browse_button_eventtab=defeventtab _textbox_browse_file.ctlbrowse;
               have_browse_button=true;
               if (completion == "") completion = FILENEW_ARG;
            ...
            if (TextBoxWidth=='') {
               ...
               for (i=1;;++i) {
                  ...
                  if( wid.p_object != OI_CHECK_BOX ) {
                     ...
                     wid.p_width=max(BOX_DEFAULT_WIDTH,(newwid.p_x_extent)-wid.p_x) - browse_button_width;
                     if (browse_button_width > 0) {
                        wid.p_next.p_x=wid.p_x_extent + AFTER_LABEL_PAD;
                     }
                  }
               }
            }
...
_ok.lbutton_up()
{
   ...
   for (;;) {
      ...
      if( wid.p_object!=OI_CHECK_BOX ) {
         ...
         case 'BNF':
            file_name = _maybe_unquote_filename(wid.p_text);
            dir_name  = _strip_filename(file_name,'N');
            if (!file_exists(file_name) && !file_exists(dir_name)) {
               _message_box("Directory not found: "dir_name);
               if (length(wid.p_text) == 0) {
                  len =2;
               } else {
