...
static void _init_markers()
{
   if (gsymbol_marker <= 0) {
      ...
      _MarkerTypeSetPriority(gsymbol_marker, VSMARKER_TYPE_PRIORITY_TAG_HIGHLIGHT);
   }
...
void _UpdateContextHighlights(bool AlwaysUpdate = false, bool ForceUpdate = false)
{
   ...
   if (!_mdi.p_child._CheckUpdateContextSizeLimits(0, quiet:true)) {
      return;
   }
