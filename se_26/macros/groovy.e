...
static _str func_keyword_at_seek(long startSeek,_str default_word='') {
   save_pos(auto p);
   goto_point(startSeek);
   _str word=default_word;
   if (_clex_find(0,'g')==CFG_KEYWORD) {
      word=cur_identifier(auto start_col);
   } else if (get_text()=='[') {
      word='[';
   }
   if (word=='global' || word=='local' || word=='macro') {
      word='function';
   }
   restore_pos(p);
   return word;
                                     ...
                                     long rightOff)
{
   ...
   if (tagType == 'func' && startSeek == 0 && _LanguageInheritsFrom('groovy')) {
      ...
      answer = 1;
   } else if ((tagType=='func' || tagType=='subfunc') && _LanguageInheritsFrom('julia') && func_keyword_at_seek(startSeek,'function'):!='function') {
      answer = 1;
   } else {
...
static bool indent_on_statement_split(long startSeek,long cursorOff,long endSeek) {
   if (cursorOff>=endSeek) {
      /* Statement could end with binary operator. 
         Need to be more conservative here since it's way more
         annoying to be incorrect and indent.

         Maybe look for specific binary operators like '+' (not '++').
      */
      return false;
   }
   // Splitting before end of statement with Enter. Lean toward indenting.
   save_pos(auto p);
   ...
   if (p_col>1) {
      ...
      if (match_length()) {
         ...
         result= ch!=')' && ch!=']' && ch!=';' && !pos('['id_chars']',ch,1,'r');
      }
   }
   restore_pos(p);
...
static void fix_endseek(_str tagType,typeless startSeek,typeless &endSeek) {
   if (tagType=='struct') {
      return;
   }
   save_pos(auto p);
...
static void googlego_adjust_call_endseek(typeless startSeek,typeless &endSeek) {
   save_pos(auto p);
   goto_point(startSeek);
   search('(','@');
   if (point('s')<endSeek) {
      ...
      if (!status) {
         endSeek=(typeless)point('s')+1;
      }
   }
   restore_pos(p);
}
static void swift_fix_case_statement_start_and_seek(typeless &startSeek,typeless &endSeek,typeless nameSeek) {
   save_pos(auto p);
   goto_point(nameSeek);
   if (_clex_find(0,'G')==CFG_KEYWORD) {
      word:=cur_identifier(auto start_col);
...
static void googlego_fix_import_startseek(typeless &startSeek) {
   save_pos(auto p);
   goto_point(startSeek);
   _clex_skip_blanks('h');
   startSeek=point('s');
...
int calc_nextline_indent_from_tags()
{
   ...
   innerCtx := ctx;
   innerType := '';
   in_googlego:=_LanguageInheritsFrom('googlego');
   in_swift:=_LanguageInheritsFrom('swift');
   ...
   while (ctx > 0) {
      ...
      tag_get_detail2(VS_TAGDETAIL_statement_flags, ctx, auto flags);
      if (in_googlego) {
         fix_endseek(tagType,startSeek,endSeek);
      }
      ...
      if (_LanguageInheritsFrom('julia')) {
         // For Julia, sometimes have to add function scope for blocks which create scopes
         /*
             let # this let generates func for outer scope block because it is NOT inside a function.
             end
         */
         if (tagType=='func' || tagType=='subfunc') {
            first_word:=func_keyword_at_seek(startSeek);
            //say('first_word='first_word);
            if (first_word=='try') {
               tagType='statement';
               //say('H1');
            } else if (first_word:!='function' && first_word:!='') {
               tagType='statement';
               //say('H2');
            } else if (first_word:=='[') {
               tagType='clause';
               //say('H3');
            } else if (flags & SE_TAG_FLAG_ANONYMOUS) {
               // This could be a lambda function that looks like one of these:
               //     param -> result
               //     (p1,p2) -> result
               // Don't worry for now. This code seems to work.
               tagType='call';
               //say('H4');
            }
         } else if (tagType=='define') {
            tagType='func';
         }
      }
      ...
      case 'if':
         ...
         if (_LanguageInheritsFrom('julia') || in_googlego || in_swift) {
            if (in_swift) {
               /* Parser generates garbage for the start seek position for a case statment
               */
               tag_get_detail2(VS_TAGDETAIL_statement_name_seekpos, ctx, auto nameSeek);
               swift_fix_case_statement_start_and_seek(startSeek,endSeek,nameSeek);
            }
            // The else clause, has no parens to deal with.
            answer = indent_in_block_introduction(startSeek, endSeek, cursorOff,leftChar);
         } else if (intro == 'if' || intro == 'switch'|| (intro=='when' && _LanguageInheritsFrom('kotlin') && kotlin_when_has_parens(ctx))) {
      ...
      case 'switch':
         if (in_googlego || in_swift) {
            // Google go or Swift switch statement does not need parens
            _GoToROffset(startSeek);
            // The else clause, has no parens to deal with.
            answer = indent_in_block_introduction(startSeek, endSeek, cursorOff,leftChar);
            if (answer == 0) {
      ...
      case 'constr':
         // Control statements with an expression in (), and indent after the end.
         if ((in_googlego || in_swift) && tagType=='loop') {
            // Google go or Swift for statement does not need parens
            _GoToROffset(startSeek);
            // The else clause, has no parens to deal with.
            answer = indent_in_block_introduction(startSeek, endSeek, cursorOff,leftChar);
            if (answer == 0) {
            ...
            break;
         } else if (tagType == 'func' && isClosure(startSeek)) {
      ...
      case 'call':
         if (in_googlego) {
            /* Google go parser has a bug and seem to pickup extra statement 
               garbage after the call if call not terminated with semicolon.
            ...
            */
            googlego_adjust_call_endseek(startSeek,endSeek);
         }
      ...
      case 'const':
         ...
         if (cursorOff >= endSeek || cursorOff<=startSeek
             || !indent_on_statement_split(startSeek,cursorOff,endSeek)) {
            ctx = outer;
         } else {
            ctx = -1;                   
         }
      ...
      case 'import':
      case 'package':
         if (in_googlego && tagType=='import') {
            googlego_fix_import_startseek(startSeek);
            answer = plain_statement_indent(startSeek, endSeek, cursorOff,leftChar);
            if (cursorOff >= endSeek || cursorOff<=startSeek
                || !indent_on_statement_split(startSeek,cursorOff,endSeek)) {
               ctx = outer;
...
bool _googlego_is_smarttab_supported() {
   return true;
}
int googlego_smartpaste(bool char_cbtype,int first_col,int Noflines,bool allow_col_1=false)
{
   int comment_col=0;
   // Find first non-blank line which could be a comment.
   first_line := "";
   ...
   if (j<=Noflines) {
      ...
      if (!status) {
         ...
         should_allow_col1 := allow_col_1 || pasting_open_block/* || _LanguageInheritsFrom('googlego')*/;
         indent_case:=beaut_case_indent();
         cword := _c_get_wordplus();
         //int syntax_indent=p_SyntaxIndent;
         if (!status && (cword=='case' || cword=='default:')) {
            int enter_col=0;
...
bool _swift_is_smarttab_supported() {
   return true;
}
int swift_smartpaste(bool char_cbtype,int first_col,int Noflines,bool allow_col_1=false) {
   return googlego_smartpaste(char_cbtype,first_col,Noflines,allow_col_1);
}
