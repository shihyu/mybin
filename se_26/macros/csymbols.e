                         ...
                         VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   if (_chdebug) {
      isay(depth,"_c_parse_return_type(current_scope="current_scope);
   ...
   if (_c_is_type_inferred(return_type, auto add_const=false)) {
      ...
      if (_LanguageInheritsFrom("dart")) {
         ch1 := _first_char(return_type);
         chN := _last_char(return_type);
         if (ch1=="{" && chN=='}') {
            rt.return_type = "dart/core/Set";
            return_type = substr(return_type,2);
            match_generic(return_type, auto bracketexp, num_args, "{},", auto firstexp);
            tag_return_type_init(auto item_rt);
            item_status := _c_get_type_of_expression(errorArgs, tag_files, 
                                                     "", "", "", 
                                                     VSCODEHELP_PREFIX_NULL, 
                                                     expr:firstexp, 
                                                     item_rt, visited, depth+1);
            if (!item_status) {
               rt.istemplate=true;
               rt.template_args:['E'] = firstexp;
               rt.template_names[0]='E';
               rt.template_types:['E']=item_rt;
            }
         }
         return 0;
      }
   ...
   while (return_type != "") {
      ...
      case "!(":
         ...
         if (status >= 0 && rt.return_type :!= search_class_name) {
            ...
            status = _c_substitute_template_args(orig_search_class, 
                                                 ...
                                                 rt.isvariadic,
                                                 template_class_name, 
                                                 template_file:    orig_rt.filename,
                                                 template_line_no: orig_rt.line_number,
                                                 tag_files, 
                                                 visited, depth+1);
            if (_chdebug) {
      ...
      case "CFString": if (ch:=="CFString" && _LanguageInheritsFrom("m"))  {rt.return_type="NSString";break;}
                       if (ch:=="CFString" && _LanguageInheritsFrom("c"))  {rt.return_type="NSString";break;}
      ...
      case "__pointer_traits_element_type":
         if (_LanguageInheritsFrom("c") && 
             _c_is_stl_class(search_class_name) &&
             _first_char(return_type) == '<' && 
             pos(' 'ch' ', " rebind_alloc __pointer_traits_element_type ")) {
            rebind_template_return_type := substr(return_type,2);
            _str rebind_template_parms[];
            if (match_templates(rebind_template_return_type, rebind_template_parms) && rebind_template_parms._length() > 0 && substr(rebind_template_return_type,1,6) :== "::type") {
               ...
               // And then use the result
               if (!status) {
                  if (ch :== "__pointer_traits_element_type" && value_rt.pointer_count > 0) {
      ...
      default:
         ...
         if (!isjava && ch!="interface" &&
            ...
            case "consteval":
               ...
               continue;
            default:
               if (rt.return_type == "" &&  _c_is_builtin_type(ch)) {
                  rt.return_type = ch;
                  rt.return_flags |= VSCODEHELP_RETURN_TYPE_BUILTIN;
               }
               continue;
...
/**
 * @return 
 * Return {@code true} if {@code this} is to be treated as a reference type 
 * in the given language. 
 *  
 * @param langId     language to check
 */
bool _c_this_is_reference(_str langId="")
{
   ...
   switch (langId) {
   case "":               return false;
   case "groovy":         return true;
   case "js":             return true;
   case "as":             return true;
   case "e":              return true;
   case "d":              return true;
   case "dart":           return true;
   case "lua":            return true;
   case "vera":           return true;
   case "systemverilog":  return true;
   default:
                                 ...
                                 int depth=0)
{
   ...
   if (symbol :== "this" && !(cur_tag_flags & SE_TAG_FLAG_STATIC) && cur_class_name!="") {
      if (search_class_name :== "" && context_id > 0) {
         ...
         rt.pointer_count = (isjava || _c_this_is_reference())? 0 : 1;
   ...
   for (;;) {
      for (i=1; i<=num_matches; i++) {
         ...
         if (tag_tree_type_is_package(cm.type_name) && (cm.return_type=="") &&
            ...
            if (i==1 && !isjava && _LanguageInheritsFrom("c") && rt.return_type=="" && !rt.istemplate) {
               if (_c_is_stl_class(cm.member_name:+VS_TAGSEPARATOR_package)) {
                  if (cm.class_name=="" && cm.member_name == symbol) {
                     if ( _chdebug ) {
                        isay(depth, "_c_get_type_of_matches: STL package: "symbol);
                     }
            ...
            if (i==1 && !isjava && _LanguageInheritsFrom("c") && rt.return_type=="boost" && !rt.istemplate) {
               if (cm.member_name=="container" || cm.member_name=="detail") {
                  if (cm.class_name=="boost" && cm.member_name == symbol) {
                     if ( _chdebug ) {
                        isay(depth, "_c_get_type_of_matches: BOOST package: "symbol);
                     }
                     rt.return_type=symbol;
                     rt.pointer_count=0;
                     rt.istemplate=false;
            ...
            if (i==1 && _LanguageInheritsFrom("julia") && rt.return_type=="" && !rt.istemplate) {
               if (symbol == "Main" && cm.class_name=="" && cm.member_name == symbol) {
                  if ( _chdebug ) {
                     isay(depth, "_c_get_type_of_matches: Julia Main package: "symbol);
                  }
                  rt.return_type=symbol;
                  rt.pointer_count=0;
                  rt.istemplate=false;
            ...
            if (i==1 && _LanguageInheritsFrom("dart") && rt.return_type=="" && !rt.istemplate) {
               if (symbol == "dart" && cm.class_name=="" && cm.member_name == symbol) {
                  if ( _chdebug ) {
                     isay(depth, "_c_get_type_of_matches: Dart package: "symbol);
                  }
                  rt.return_type=symbol;
                  rt.pointer_count=0;
                  rt.istemplate=false;
            ...
            if (i==1 && isjava && _LanguageInheritsFrom("java") && rt.return_type=="" && !rt.istemplate) {
               if (_java_is_jdk_package(cm.member_name:+VS_TAGSEPARATOR_package)) {
                  if (cm.class_name=="" && cm.member_name == symbol) {
                     if ( _chdebug ) {
                        isay(depth, "_c_get_type_of_matches: Java JDK package: "symbol);
                     }
                     rt.return_type=symbol;
                     rt.pointer_count=0;
                     rt.istemplate=false;
            ...
            if (i==1 && _LanguageInheritsFrom("cs") && rt.return_type=="" && !rt.istemplate) {
               if (symbol == "System" && cm.class_name=="" && cm.member_name == symbol) {
                  if ( _chdebug ) {
                     isay(depth, "_c_get_type_of_matches: C# package: "symbol);
                  }
                  rt.return_type=symbol;
                  rt.pointer_count=0;
                  rt.istemplate=false;
                               ...
                               _str symbol="", _str search_class_name="")
{
   ...
   case "[":
      if (_LanguageInheritsFrom("m") && previous_id == "" && rt.return_type == "") {
         match_generic(prefixexp, auto bracketexp, num_args, "[],", auto firstexp);
         status = _c_get_type_of_expression(errorArgs, tag_files, 
                                            "", "", "", 
      ...
      if (_LanguageInheritsFrom("dart") && previous_id == "" && rt.return_type == "") {
         rt.return_type = "dart/core/List";
         match_generic(prefixexp, auto bracketexp, num_args, "[],", auto firstexp);
         parse bracketexp with bracketexp ',';
         tag_return_type_init(auto item_rt);
         item_status := _c_get_type_of_expression(errorArgs, tag_files, 
                                                  "", "", "", 
                                                  VSCODEHELP_PREFIX_NULL, 
   ...
   case "this":
      ...
      previous_id = "";
      if (!isjava && !_c_this_is_reference()) {
         rt.pointer_count = 1;
      }
      if (_LanguageInheritsFrom("cs")) {
         rt.return_flags |= VSCODEHELP_RETURN_TYPE_NON_STATIC_ONLY;
   ...
   case "const":
      ...
      if (_LanguageInheritsFrom("dart") && ch=="const") {
         rt.return_flags |= VSCODEHELP_RETURN_TYPE_CONST_ONLY;
         previous_id = "";
         if (_chdebug) {
            isay(depth,"_c_get_type_of_part: found Dart const expression: "ch);
         }
         return 0;
      }
                                    ...
                                    _str symbol="", _str search_class_name="")
{
   ...
   while (prefixexp != "") {
      ...
      id_arglist := "";
      if (_LanguageInheritsFrom("c") && 
          isid_valid(ch) && ch != "NULL" && ch != "nullptr" &&
