                     ...
                     _str alternate_ext1='',_str alternate_ext2='')
{
   ...
   info.options = DIFF_NO_BUFFER_SETUP|DIFF_ALWAYS_STORE_RESULTS|def_diff_flags;
   info.numDiffOutputs = 0;
   info.isSourceDiff = false;
   info.loadOptions = def_load_options;
   info.gaugeWID = 0;
   info.maxFastFileSize = 0;
   info.lineRange1 = 1;
   info.lineRange2 = 1;

   info.smartDiffLimit = 0;
   ...
   for (i=0;i<DifferentList._length();++i) {
      ...
      if (exists1>-1 && exists2>-1 ) {
         ...
         DIFF_INFO diffinfo;
         diffinfo.viewID1 = viewid1;
         diffinfo.viewID2 = viewid2;
         diffinfo.options = DIFF_OUTPUT_BOOLEAN|DIFF_NO_BUFFER_SETUP|DIFF_DONT_STORE_RESULTS;
         diffinfo.numDiffOutputs = 0;
         diffinfo.isSourceDiff = false;
         diffinfo.loadOptions = def_load_options;
         diffinfo.gaugeWID = 0;
         diffinfo.maxFastFileSize = 800;
         diffinfo.lineRange1 = r1;
         diffinfo.lineRange2 = r2;
         diffinfo.smartDiffLimit = 0;
   ...
   for (i=0;i<OneList._length();++i) {
      int bm1 = _pic_symbol;
      int bm2 = _pic_symbol;
      INTARRAY newOverlays1;
      INTARRAY newOverlays2;
      addedModifyOverlay := false;
      _str cur=OneList[i];
      _str type=OneTable:[cur];
      parse type with type (_chr(1)) .;
      if (type=='+') {
         newOverlays1 :+= _pic_symbolm_overlay;
         newOverlays2 :+= _pic_symbolp_overlay;
         state=DIFF_TAG_STATUS_PATH2;
      } else if (type=='-') {
         newOverlays1 :+= _pic_symbolp_overlay;
         newOverlays2 :+= _pic_symbolm_overlay;
         state=DIFF_TAG_STATUS_PATH1;
      } else if (type:=="\t") {
         state=DIFF_TAG_STATUS_MATCH;
      } else if (type=='Different') {
         newOverlays1 :+= _pic_symbold_overlay;
         newOverlays2 :+= _pic_symbold_overlay;
         addedModifyOverlay = true;
         state=DIFF_TAG_STATUS_DIFFERENT;
      } else if (type=='Moved') {
         newOverlays1 :+= _pic_symbolmoved_overlay;
         newOverlays2 :+= _pic_symbolmoved_overlay;
      ...
      if (!pAlternateInfo) {
         if (ParentIndex1>-1) {
            ...
            if ( newindex>0 ) {
               wid1._TreeGetInfo(newindex,auto ShowChildren,auto NonCurrentBMIndex,auto CurrentBMIndex,auto moreFlags,auto lineNumber,-1,auto overlays1);
               wid1._TreeMergeOverlays(overlays1,newOverlays1);
               wid1._TreeSetInfo(newindex,ShowChildren,NonCurrentBMIndex,CurrentBMIndex,moreflags,1,-1,overlays1);
            }
         }
         if (ParentIndex2>-1) {
            ...
            if ( newindex2>0 ) {
               wid2._TreeGetInfo(newindex2,auto ShowChildren,auto NonCurrentBMIndex,auto CurrentBMIndex,auto moreFlags,auto lineNumber,-1,auto overlays2);
               wid2._TreeMergeOverlays(overlays2,newOverlays2);
               wid2._TreeSetInfo(newindex2,ShowChildren,NonCurrentBMIndex,CurrentBMIndex,moreflags,1,-1,overlays2);
            }
         }
      }else{
                                     ...
                                     _str ver1,_str ver2)
{
   ...
   _safe_hidden_window();
   DIFF_INFO diffinfo;
   diffinfo.viewID1 = WID1;
   diffinfo.viewID2 = WID2;
   diffinfo.options = DIFF_NO_BUFFER_SETUP|def_diff_flags;
   diffinfo.numDiffOutputs = 0;
   diffinfo.isSourceDiff = false;
   diffinfo.loadOptions = def_load_options;
   diffinfo.gaugeWID = 0;
   diffinfo.maxFastFileSize = 0;
   diffinfo.lineRange1 = 1;
   diffinfo.lineRange2 = 1;
   diffinfo.smartDiffLimit = 0;
   diffinfo.imaginaryText = null;
   diffinfo.tokenExclusionMappings=null;
   ...
   for (i=0;i<DifferentList._length();++i) {
      ...
      if (exists1>-1 && exists2>-1 ) {
         ...
         diffinfo.viewID1 = WID1;
         diffinfo.viewID2 = WID2;
         diffinfo.options = DIFF_OUTPUT_BOOLEAN|DIFF_NO_BUFFER_SETUP|def_diff_flags;
         diffinfo.numDiffOutputs = 0;
         diffinfo.isSourceDiff = false;
         diffinfo.loadOptions = def_load_options;
         diffinfo.gaugeWID = 0;
         ...
         DiffFreeAllColorInfo(diffinfo.viewID1.p_buf_id);
         DiffFreeAllColorInfo(diffinfo.viewID2.p_buf_id);
         if (!status) {
            OneTable:[DifferentList[i]]='Moved';
         }
      }
   ...
   for (i=0;i<OneList._length();++i) {
      ...
      parse type with type (_chr(1)) .;
      if (type=='+') {
         overlays1 :+= _pic_symbolp_overlay;
         overlays2 :+= _pic_symbolm_overlay;
         bmname = "<img src=\"vslick://_f_symbol_add.svg@"imageSize"\">";
         state=DIFF_TAG_STATUS_PATH2;
      }else if (type=='-') {
         overlays1 :+= _pic_symbolm_overlay;
         overlays2 :+= _pic_symbolp_overlay;
         bmname = "<img src=\"vslick://_f_symbol_delete.svg@"imageSize"\">";
         state=DIFF_TAG_STATUS_PATH1;
      }else if (type=="\t") {
         continue;
      }else if (type=='Different') {
         overlays1 :+= _pic_symbold_overlay;
         overlays2 :+= _pic_symbold_overlay;
         bmname = "<img src=\"vslick://_f_symbol_modified.svg@"imageSize"\">";
         state=DIFF_TAG_STATUS_DIFFERENT;
      }else if (type=='Moved') {
         overlays1 :+= _pic_symbolmoved_overlay;
         overlays2 :+= _pic_symbolmoved_overlay;
                             ...
                             DIFF_TAG_LIST_INFO *pListInfo,int OpenClose=0)
{
   ...
   if (overlays._length()==0) {
      // No overalys, symbols match
      ch='M';
   }else if ( _MFDiffHaveThisOverlay(overlays,_pic_symbolp_overlay) ) {
      ch='-';
   }else if ( _MFDiffHaveThisOverlay(overlays,_pic_symbolm_overlay) ) {
      ch='+';
   }else if ( _MFDiffHaveThisOverlay(overlays,_pic_symbolm_overlay) ) {
      ch='D';
   }else if ( _MFDiffHaveThisOverlay(overlays,_pic_symbolmoved) ) {
      ch='Moved';
   }
   typeless type='';
   orig_view_id := p_window_id;
