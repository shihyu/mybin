 ...
 *
 */
_command int man(_str param='',bool quiet=false) name_info(','VSARG2_REQUIRES_MDI)
{
   _macro_delete_line();
   _str section_list='';
   if ( param=='' ) {
      ...
      // SlickEdit options must be first
      param=strip(param);
      for (;;) {
         if (substr(param,1,3)!='-se') {
            break;
         }
         option:=parse_file(param);
         if (option=='-ses') {
            section_list=parse_file(param);
         } else if (option=='-seq') {
            quiet=true;
         } else {
            _message_box('invalid SlickEdit man option 'option);
            return 1;
         }
      }
      if (section_list!='') {
         for (;;) {
            ...
            parse section_list with auto section ',' section_list;
            if (isinteger(section)) {
               //man %(section)
               set_env('section',section);
               _str command=_parse_project_command(param,'','','');
               set_env('section',null);
               status:=man(command,quiet);
               if (!status) {
                  return 0;
               }
            }
...
_command void select_mode(_str mode_name='') name_info(MODENAME_ARG','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   if (!_IsTempBufName()) {
      ...
      } else if (ext == '' && p_buf_name != '' && !_IsTempBufName()) {
...
/**
 * Options controlling how the {@code format_columns}
