...
static _str gDLLDependantSlickCModules[] = {
   ...
   // highlighting
   "se/color/ColorIdAllocator",
   "se/color/HighlightOptions",
   "se/color/HighlightWordInfo",
   "se/color/HighlightProfile",
   "se/color/HighlightAnalyzer",
   "se/color/HighlightTextChangeListener",
   "tbhighlight",
 ...
 */
static void makeNloadArray(_str path, _str (&moduleList)[], bool doLoad=true, bool doShell=false, bool async=true)
{
   ...
   loop {
      ...
      while (compile_i < moduleList._length() && compile_i-load_i < MAX_VSTW_PROCESSES) {
         ...
         make_status := _make(_maybe_quote_filename(module_path), async);
         process_make_rc(make_status, module_path, quiet: async);
...
defmain()
{
  ...
  makeNloadArray(path, gIntrinsicSlickCModules, async:false);
  ...
  makeNloadArray(path, gDefaultSlickCModules, doLoad:false, doShell:true);
  ...
  makeNloadArray(path, gCompileOnlySlickCModules, doLoad:false);
