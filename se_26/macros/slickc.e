...
/**
 * If the current workspace is "slickedit-*.vpw", and the workspace directory 
 * contains files named macros/slick.sh and macros/builtins.e, then build the 
 * Slick-C tag file using the macros under the workspace rather than the 
 * ones in the editor installation directory. 
 * 
 * @default false
 * @categories Configuration_Variables
 */
bool def_use_workspace_slickc_macros = false;
...
static int slick_expand_space()
{
   ...
   if ( word=='if' || word=='else if' || if_special_case) {
      ...
      maybe_insert_braces(noSpaceBefore, 
                          insertBraceImmediately && !c_is_extension_of_statement('if'), 
                          width, word,
      ...
      maybe_insert_braces(noSpaceBefore,!c_is_extension_of_statement('if'),width,word);
      _end_line();
   } else if ( word=='for' ) {
      replace_line(line:+maybespace'('parenspace:+parenspace')'e1);
      maybe_insert_braces(noSpaceBefore, insertBraceImmediately,width,word);
...
int _e_MaybeBuildTagFile(int &tfindex, bool withRefs=false, bool useThread=false, bool forceRebuild=false)
{
   ...
   ext := 'e';
   tagfilename := "";
   if (ext_MaybeRecycleTagFile(tfindex,tagfilename,ext,TAG_BASENAME,allowGlobalTagFile:false) && !forceRebuild) {
   ...
   // do not start another tag file build if one is already running
   if (useThread && !tag_check_async_tag_file_build(tagfilename, auto isRunning, auto percentProgress)) {
 ...
 * <p>
 * Prior to loading the module, it will invoke the {@code _on_unload_XXX}
 ...
 *
 */
_command int load(_str arg1='',bool doClearErrorMessage=true) name_info(SLICKC_FILE_ARG',')    /* make and load module */
{
   saved_buffer := false;
   default_buf_name := "";
   module := "";
   ...
   if (status > 0) {
      // List is in application order, so prepend them to search the last applied
      // first.
      incpath := origInclude;
      foreach (auto spec in fix_list) {
         parse spec with auto path "\t" auto revision;
         if (incpath == '') {
            incpath = path;
         } else {
            incpath = path:+PATHSEP:+incpath;
         }
      }
      set_env("VSLICKINCLUDE", incpath);
   }
   ...
   if (_macroCompilePermitted(module)) {
      ...
      if (doClearErrorMessage) {
         clearSlickCErrorMessages(); 
      }
      _allow_clearSlickCErrorMessages=doClearErrorMessage;
      _make(module);
      _allow_clearSlickCErrorMessages=true;
   } else {
      rc=0;
   }
   typeless make_rc=rc;
...
static _str list_macfiles_delete_callback(_str item)
{
 ...
 * {@code _on_unload_module_XXX} callbacks with a single argument
...
/**
 * Determine if the given Slick-C module is already loaded.
 * 
 * @param filename  Path to Slick-C module or dynamic library
 * @param extension file extension to look for (.ex or .dll)
 * 
 * @return 
 * Returns 0 if the given module is not loaded. 
 * Returns names table index of module if it is loaded. 
 *  
 * @see load() 
 * @see dload() 
 * @see unload() 
...
/**
 * Delete the given Slick-C module from the {@link def_macfiles} configuration 
 * variable.  This matches modules based on file name only, so it will delete 
 * an existing item with the same file name in a different directory path.
 * 
 * @param filename  Path to Slick-C module or dynamic library
 * @param extension file extension to look for (.ex or .dll)
 * @param doTagging if {@code true}, remove the module from the Slick-C tag file
 * 
 * @return 
 * Returns {@code true} if the given module is found and removed from 
 * {@link def_macfiles}, otherwise, returns {@code false} to indicate that 
 * the module was not there in the first place. 
 *  
 * @see load() 
 * @see dload() 
 * @see unload() 
 ...
 * @categories Miscellaneous_Functions
 */
bool _macfile_delete(_str filename,_str extension, bool doTagging=true)
{
   filename=strip(filename,'B','"');
   // Don't try to support VSLICKEXT environment variable
   if (!_file_eq(_macro_ext,'.e')) return false;
   module := _strip_filename(filename,(extension=='')? 'P':'PE');
...
/**
 * Add the given Slick-C module to the {@link def_macfiles} configuration variable, 
 * provided this is not an already-loaded system module. 
 * 
 * @param filename       Path to Slick-C module or dynamic library
 * @param extension      file extension to look for (.ex or .dll)
 * @param alreadyPresent non-zero if a module with the same name is already loaded
 * @param supportMacro   is this a support macro that should not be tagged?
 *  
 * @see load() 
 * @see dload() 
 * @see unload() 
...
void _macfile_add(_str filename,_str extension,int alreadyPresent=0,bool supportMacro=false)
{
   ...
   deleted_module_from_macfiles := _macfile_delete(filename,_macro_ext'x',false);
   ...
   if (substr(filename,1,2)==".." && pos('..',filename,4)) {
      filename = relative(origFilename, _ConfigPath(), false);
   }
   ...
   // if this module was already loaded, but not in def_macfiles,
   // then assume this was a system module, not a user-loaded module
   if (alreadyPresent && !deleted_module_from_macfiles) return;

   module := _strip_filename(filename,(extension=='')? '':'E');
   module :+= extension;
   list := def_macfiles;
   for (;;) {
      word := parse_file(list,false);
      if (word=='') {
         break;
      }
