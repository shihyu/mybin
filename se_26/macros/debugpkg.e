...
int _OnUpdate_debug_edit_breakpoint(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   if (!target_wid || !target_wid._isEditorCtl()) {
      // If it's a tree view, it's the context menu for the breakpoints tool window.
      return(target_wid.p_object == OI_TREE_VIEW) ? MF_ENABLED :  MF_GRAYED;
   }
   if (!target_wid._isdebugging_supported(strict:false)) {
      return(MF_GRAYED);
   }
...
static _str EvaluateMouseExpression(MouseOverExpression &e, _str &number_msg)
{
   ...
   if (_chdebug) {
      _dump_var(e, "EvaluateMouseExpression H"__LINE__": e");
   }
   // color coding check
   is_number := false;
                          ...
                          VSCodeHelpFlags extra_codehelp_flags=VSCODEHELPFLAG_NULL)
{
   ...
   mo_info := _haveContextTagging() && (codehelp_flags & VSCODEHELPFLAG_MOUSE_OVER_INFO) != 0;
   if (!debug_active() && !mo_info && !def_number_base_popups && !useCursorXY) {
   ...
   if (select_active2() && _in_selection() && !(isEclipsePlugin() && _eclipse_debug_active())) {
      ...
      if (lookForSymbolMatches) {
         ...
         if (status >= 0 && tag_get_num_of_matches() > 0) {
            ...
            if (codehelp_flags & VSCODEHELPFLAG_GO_TO_DEF_CASE_SENSITIVE) {
            ...
            for (i:=1; i<=n; i++) {
               ...
               add_comment_info := "";
               if (codehelp_flags & VSCODEHELPFLAG_DISPLAY_MEMBER_COMMENTS) {
               ...
               if (_CheckTimeout()) break;
               tag_get_match_info(i, cm);
               if (codehelp_flags & VSCODEHELPFLAG_DISPLAY_RETURN_TYPE) {
