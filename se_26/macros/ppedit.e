                                     ...
                                     bool hideSyscpp=false)
{
   ...
   if (isUserCPP) {
      ...
      for (;;) {
         // read one macro definition and insert into tree
         if (cm_load_macro(macro_name, macro_value, pic, langId)) {
            if (!gusercpp_hashtab._indexin(macro_name)) {
               macro_key := cm_make_macro_key(macro_name, justName);
...
int cm_save_macro(_str macro_name, _str macro_value, int pic, _str langId)
{
   ...
   if (pic==gi_pic_define) {
      ...
      curr_line = pound"undef "macro_name;
   }
   insert_line(curr_line);
   insert_line('');
   return 0;
...
// Write a #delete to remove the previous definition of the given macro
int cm_delete_macro(_str macro_name, _str langId, bool add_blank_line_after=true)
{
   pound := cm_get_pound_sign(langId);
   insert_line(pound"delete "macro_name);
...
static int cm_save_macro_definitions(int t, _str macrofilename, _str langId)
{
   ...
   while (index > 0) {
      ...
      if (gusercpp_hashtab._indexin(justName)) {
         hasSysCppMacro := gsyscpp_hashtab._indexin(macro_key);
         isSysCppMacro  := hasSysCppMacro;
         if (isSysCppMacro) {
   ...
   foreach (justName => index in gusercpp_hashtab) {
      if (!index) {
         cm_delete_macro(justName, langId);
      }
   }
...
void ctl_ok_btn.on_create(_str langId="", _str userFileName="")
{
   ...
   if (filename!="") {
      cm_load_macro_definitions(ctl_macros_tree, filename, langId, isUserCPP:false, hideSyscpp);
   }
   ...
   if (userFileName != "") {
      ...
      cm_load_macro_definitions(ctl_macros_tree, filename, langId, isUserCPP:true);
   }
   if (filename=="") {
      filename = _lang_user_ppedit_path(langId, false);
   }
...
void ctl_delete_btn.lbutton_up()
{
   ...
   macro_key  := cm_make_macro_key(macro_name, auto justName);
   if (gsyscpp_hashtab._indexin(macro_key)) {
      if (gsyscpp_hashtab:[macro_key] == currIndex) {
         gusercpp_hashtab:[justName] = 0;
      } else {
         yesno := _message_box("Macro " :+ macro_name :+ " was pre-defined but is modified locally.\n\nDo you want to revert to the pre-defined macro?",'',MB_YESNO|MB_ICONQUESTION);
         if (yesno == IDYES) {
