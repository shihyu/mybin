...
struct DIFF_BUFFER_INFO {
   ...
   // doNotCopyBuffer is used for instances where the buffer is read only or 
   // we have a copy of the original open already and do not want to make 
   // another one (usually svc update)
   bool doNotCopyBuffer;
};

struct DIFF_SHOWING_INFO {
   int DiffParentWID;
   int IntraLineIsOff;
   ...
   DIFF_BUFFER_INFO buffer1;
   DIFF_BUFFER_INFO buffer2;
};
...
struct DIFF_SETUP_FILE_DATA {
   ...
   // For SVC update where the user can switch back and forth between difs, 
   // We don't want to copy that differ every time.  The file is read only
   // and there is no point in copying it after we initially load it.
   bool doNotCopyBuffer;
};
...
const DIFFEDIT_CONST_MISC_INFO=              "diffConstMiscInfo:";
...
struct DIFF_INFO {
     ...
     *  
     * This option is ignored when {@code iIsSourceDiff} is false.
     */
...
const BACKUP_HISTORY_DIALOG_CAPTION_PREFIX = "Backup History - ";
const HISTORY_DIFF_DIALOG_CAPTION_PREFIX   = "History Diff - ";

const BACKUP_HISTORY_NAME = "Backup History";
const HISTORY_DIFF_NAME   = "History Diff";

int def_diff_force_bin_compare_limit = 4096;
