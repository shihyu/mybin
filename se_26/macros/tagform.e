...
void ctlautotag_btn.lbutton_up()
{
   ...
   if (status!=COMMAND_CANCELLED_RC) {
      ...
      //ctl_tag_files._TreeDelete(TREE_ROOT_INDEX,'c');
...
static _str TAGFORM_SKIP_ON_CHANGE(...) {
   if (arg()) ctl_tag_files.p_user=arg(1);
   return ctl_tag_files.p_user;
}
...
/**
 * If set to {@code true}, attempt to find the common base directory for the 
 * source files in the tag file and only show the relative paths in the list 
 * to conserve horizontal space.  When {@code false}, the full path of all 
 * source files is always shown, which may mean that the file names are 
 * scrolled horizontally off of the right edge of the list. 
 */
static const TAGFORM_COMMON_DIR_ENABLED=false;
/**
 * Minimum common directory length required for it to be worthwhile to 
 * trim the common directory off of the files in the source file list. 
 * Only matters if {@link TAGFORM_COMMON_DIR_ENABLED} is {@code true}.
 */
static const TAGFORM_COMMON_DIR_MIN_LENGTH=20;
...
//////////////////////////////////////////////////////////////////////////////
// calculate common direcotry root
void get_common_directory_root(_str &common_dir, _str filename)
{
   if (!TAGFORM_COMMON_DIR_ENABLED) {
      common_dir = "";
      return;
   }
   if (common_dir == null) {
      common_dir = _strip_filename(filename, 'N');
      return;
   }
   while (length(common_dir) > TAGFORM_COMMON_DIR_MIN_LENGTH) {
      filename_prefix := substr(filename, 1, length(common_dir));
      if (file_eq(common_dir, filename_prefix)) {
         return;
      }
      orig_common_dir := common_dir;
      common_dir = strip(common_dir, 'T', FILESEP);
      common_dir = strip(common_dir, 'T', FILESEP2);
      common_dir = _strip_filename(common_dir, 'N');
      if (common_dir == orig_common_dir) {
         common_dir = "";
         return;
      }
   }
...
//
static int LoadFileNameList(_str tag_filename, bool quiet=false, bool findCommonDir=TAGFORM_COMMON_DIR_ENABLED)
{
   ...
   if (p_name == "ctl_file_list") {
      ...
      lastCommonDir   = ctl_dir_label.p_user;
   } else {
      quiet = true;
   }
   ...
   if (!quiet) {
      ...
      if (num_files > TAGFORM_PROGRESS_THRESHOLD) {
         ...
         ctl_files_gauge.p_x_extent = ctl_file_list.p_x_extent - 60;
   ...
   // get the common base path for source files in the database
   common_dir := lastCommonDir;

   // If this is a project or workspace tag file, restrict path to workspace path
   if (_workspace_filename != "") {
      if (isWorkspaceTagFileName(tag_filename) || isProjectTagFileName(tag_filename)) {
         get_common_directory_root(common_dir, _workspace_filename);
      }
   }
   ...
   count := 0;
   filename := "";
   if (TAGFORM_COMMON_DIR_ENABLED && findCommonDir) {
      status=tag_find_file(filename);
      while (!status) {
         get_common_directory_root(common_dir, filename);
         ++count;
         if (!quiet && ctl_files_gauge.p_visible && (count % increment) == 0) {
            ctl_files_gauge.p_value = (count intdiv 2);
   ...
   while (!status) {
      ...
      if (TAGFORM_COMMON_DIR_ENABLED && findCommonDir && 
          length(common_dir) > TAGFORM_COMMON_DIR_MIN_LENGTH && 
          file_eq(common_dir, substr(filename, 1, length(common_dir)))) {
         filename_suffix = substr(filename, length(common_dir)+1);
      }
      switch (p_object) {
      case OI_COMBO_BOX:
         _lbadd_item(filename_suffix);
         break;
      case OI_TREE_VIEW:
         _TreeAddItem(TREE_ROOT_INDEX, filename_suffix, TREE_ADD_AS_CHILD,
                      _pic_file, _pic_file, TREE_NODE_LEAF);
         break;
      case OI_EDITOR:
      case OI_FORM:
         insert_line(filename_suffix);
         break;
      case OI_LIST_BOX:
         _lbadd_item_index(count, filename_suffix, 0);
      ...
      if (!quiet && ctl_files_gauge.p_visible && (count % increment) == 0) {
         if (TAGFORM_COMMON_DIR_ENABLED && findCommonDir) {
            ctl_files_gauge.p_value = (common_count intdiv 2) + (count intdiv 2);
         } else {
   ...
   // maybe set up label with common directory
   if (!quiet && p_object!=OI_EDITOR && _find_control("ctl_dir_label") > 0) {
      if (TAGFORM_COMMON_DIR_ENABLED && findCommonDir && common_dir != null && length(common_dir) > TAGFORM_COMMON_DIR_MIN_LENGTH) {
         ctl_dir_label.p_caption = "Dir: ":+common_dir;
         ctl_dir_label.p_user = common_dir;
         ctl_dir_label.p_visible = true;
      } else {
         ctl_dir_label.p_caption = "";
         ctl_dir_label.p_user = "";
         ctl_dir_label.p_visible = false;
      }
   }
   ...
   if (!quiet) {
      ...
      ctl_files_label.p_caption = get_message(VSRC_CFG_TAG_FILES_COUNT, numFiles, tagFileSize);
...
static void AddLanguageTagFiles(int index,_str TagFileList,_str AllTagFileList,bool useThread=false)
{
   ...
   for (;;) {
      ...
      if (!AddedList._indexin(_file_case(CurTagFilename))) {
         ...
         index=ctl_tag_files._TreeAddItem(index,            // Relative Index
                                          allcaption,       // Caption
                                          flags,            // Flags
         ...
         flags=0;
         ctl_tag_files._TreeSetInfo(_TreeGetParentIndex(index), TREE_NODE_EXPANDED);
         ctl_tag_files._TreeSetCheckable(index, 1, 0, addingActiveTagFiles? TCB_CHECKED:TCB_UNCHECKED);
         AddedList:[_file_case(CurTagFilename)] = index;
      } else if (addingActiveTagFiles) {
         index = AddedList:[_file_case(CurTagFilename)];
         ctl_tag_files._TreeSetCheckState(index, TCB_CHECKED);
      }
   }
}
...
ctldone.on_create(_str forLangId="", _str tagfilename="")
{
   ...
   if (CurModeName != "") {
      ...
      if (_istagging_supported(langId)) {
         ...
         langSectionIndex := ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, languageTagFilesCaption);
         if (langSectionIndex <= 0) {
            compilerTagFilesCaption := '"':+CurModeName:+'" ':+COMPILER_CONFIG_FOLDER_NAME;
            langCompilerIndex := ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, compilerTagFilesCaption);
            if (langCompilerIndex > 0) {
               langSectionIndex = ctl_tag_files._TreeAddItem(langCompilerIndex,     // Relative Index
               ...
               langSectionIndex = ctl_tag_files._TreeAddItem(TREE_ROOT_INDEX,       // Relative Index
                                                             '"'CurModeName'" ':+LANGUAGE_CONFIG_FOLDER_NAME,//Caption
                                                             TREE_ADD_AS_CHILD,     // Flags
                                                             _pic_fldclos,          // Collapsed Bitmap Index
                                                             _pic_fldopen,          // Expanded Bitmap Index
         ...
         ctl_tag_files._TreeSetCurIndex(langSectionIndex);
         CurModeName='';
      }
   }
   ...
   if (CurModeName != "" && !isWorkspaceFile) {
      ...
      if (langSectionIndex > 0) {
         ctl_tag_files._TreeSetCurIndex(langSectionIndex);
         ctl_tag_files._TreeScroll(ctl_tag_files._TreeGetLineNumber(langSectionIndex));
         if (ctl_tag_files._TreeGetNumChildren(langSectionIndex) == 1) {
            ctl_tag_files._TreeDown();
         }
      }
   }
   ...
   if (tagfilename != "") {
      tagfileindex := ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, tagfilename, 'PT':+(_files_case_sensitive()? '':'I'));
      if (tagfileindex > 0) {
         ctl_tag_files._TreeSetCurIndex(tagfileindex);
      }
   }

   SetTagFiles();
   index:=ctl_tag_files._TreeCurIndex();
   if (index>=0) {
      ctl_tag_files.call_event(CHANGE_SELECTED,index,ctl_tag_files,ON_CHANGE,'W');
   }else{
      ...
      ctl_tag_files.call_event(CHANGE_SELECTED,TREE_ROOT_INDEX,ctl_tag_files,ON_CHANGE,'W');
   }
}
...
static int UpdateCompilerTagsFiles(_str (&compiler_names)[], _str langId)
{
   ...
   compilerTagFilesIndex := ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, compilerTagFilesCaption);
   if (compilerTagFilesIndex < 0) {
      if (compiler_names._length() <= 0) {
         return -1;
      }
      compilerTagFilesIndex = ctl_tag_files._TreeAddItem(TREE_ROOT_INDEX,
   ...
   ctl_tag_files._TreeBeginUpdate(compilerTagFilesIndex);
   for (i:=0; i<compiler_names._length(); ++i) {
      ...
      index := ctl_tag_files._TreeAddItem(compilerTagFilesIndex, 
                                          caption, 
                                          TREE_ADD_AS_CHILD, 
                                          bmp_index, bmp_index, 
                                          TREE_NODE_LEAF, 
                                          0, compilerTagFile);
      ctl_tag_files._TreeSetCheckable(index, 
                                      1, 0, 
                                      (compiler_names[i] == activeConfigName)? TCB_CHECKED:TCB_UNCHECKED);
   }
   ctl_tag_files._TreeEndUpdate(compilerTagFilesIndex);
...
static int UpdateAutoUpdateTagFiles()
{
   ...
   if (_workspace_filename != "") {
      autoUpdatedTagFilesIndex = ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, AUTO_UPDATED_FOLDER_NAME);
      if (autoUpdatedTagFilesIndex < 0) {
         autoUpdatedTagFilesIndex = ctl_tag_files._TreeAddItem(TREE_ROOT_INDEX,
      ...
      ctl_tag_files._TreeBeginUpdate(autoUpdatedTagFilesIndex);
      for (i:=0; i<autoUpdatedTagFiles._length() && i<localTagFiles._length(); i++) {
         ...
         // Store absolute local filename in user info for the node
         ctl_tag_files._TreeAddItem(autoUpdatedTagFilesIndex,
                                    caption,
                                    TREE_ADD_AS_CHILD,
                                    bmp_index, bmp_index,
                                    TREE_NODE_LEAF, 0,
      ...
      ctl_tag_files._TreeEndUpdate(autoUpdatedTagFilesIndex);
   } else {
      autoUpdatedTagFilesIndex = ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, AUTO_UPDATED_FOLDER_NAME);
      if (autoUpdatedTagFilesIndex > 0) {
         ctl_tag_files._TreeDelete(autoUpdatedTagFilesIndex, 'C');
      }
   }
...
static int UpdateWorkspaceTagFiles()
{
   ...
   if (_workspace_filename != "") {
      workspaceTagFilesIndex = ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, WORKSPACE_FOLDER_NAME);
      if (workspaceTagFilesIndex < 0) {
         workspaceTagFilesIndex = ctl_tag_files._TreeAddItem(TREE_ROOT_INDEX,
      ...
      ctl_tag_files._TreeBeginUpdate(workspaceTagFilesIndex);
      ...
      if (isEclipsePlugin()) {
         ...
         while (curProjTagfile != "") {
            add_project_tag_file(ctl_tag_files.p_window_id, workspaceTagFilesIndex, curProjTagfile);
            curProjTagfile = next_tag_file2(wspaceTagfiles, false);
         }

      } else if (_workspace_filename != "") {
         wkspaceTagFilename := workspace_tags_filename_only();
         add_project_tag_file(ctl_tag_files.p_window_id, workspaceTagFilesIndex, wkspaceTagFilename);
      ...
      /*
      ReferencesFilesIndex=ctl_tag_files._TreeAddItem(TREE_ROOT_INDEX,    // Relative Index
      ...
      */
      ...
      ctl_tag_files._TreeEndUpdate(workspaceTagFilesIndex);
   } else {
      workspaceTagFilesIndex = ctl_tag_files._TreeSearch(TREE_ROOT_INDEX, WORKSPACE_FOLDER_NAME);
      if (workspaceTagFilesIndex > 0) {
         ctl_tag_files._TreeDelete(workspaceTagFilesIndex, 'C');
      }
   }
...
static _str GetRealTagFilenameFromTree(int index)
{
   ...
   if(_workspace_filename != "" && ctl_tag_files._TreeGetParentIndex(index) == autoUpdateFilesIndex) {
...
//
void ctl_tag_files.on_change(int reason,int index)
{
   ...
   case CHANGE_SELECTED:
      if (index < 0) return;
      depth := ctl_tag_files._TreeGetDepth(index);
      if(depth==TAGFORM_FILE_DEPTH) {
         // handle auto updated tag files
         if(ctl_tag_files._TreeGetParentIndex(index) == autoUpdateFilesIndex) {
            ...
            mou_hour_glass(true);
            ctl_file_list.LoadFileNameList(GetRealTagFilenameFromTree(index));
            mou_hour_glass(false);
         
         } else if (ctl_tag_files._TreeGetParentIndex(index) == ProjectTagfilesIndex) {
            ctlremove_files_btn.p_enabled=false;
            ctlfiles_btn.p_enabled=false;
            ctltree_btn.p_enabled=false;
            ...
            mou_hour_glass(true);
            parentIndex = ctl_tag_files._TreeGetParentIndex(index);
            TagFilename:=GetRealTagFilenameFromTree(index);
            ctl_file_list.LoadFileNameList(TagFilename);
            mou_hour_glass(false);
         ...
         ctl_file_list._TreeDelete(TREE_ROOT_INDEX, 'C');
   ...
   case CHANGE_CHECK_TOGGLED:
      ...
      if (ctl_tag_files._TreeGetCheckState(index) == TCB_CHECKED) {
...
void ctl_file_list.on_change(int reason,int index)
{
   ...
   case CHANGE_LEAF_ENTER:
      ...
      if (filename != "") {
         common_dir := ctl_dir_label.p_user;
         if (common_dir != null && common_dir != "") {
            filename = absolute(filename, common_dir);
         }
...
{
   index := ctl_tag_files._TreeCurIndex();
   int depth=ctl_tag_files._TreeGetDepth(index);
...
void ctlfiles_btn.lbutton_up()
{
   ...
   //TagFilename=ctl_tag_files._TreeGetCaption(ctl_tag_files._TreeCurIndex());
   _str TagFilename=ctl_tag_files.GetRealTagFilenameFromTree(ctl_tag_files._TreeCurIndex());
   ...
   result=_OpenDialog("-modal",
                      ...
                      _file_types_append_filters(EXTRA_FILE_FILTERS,def_file_types),
...
_command int project_add_files_prompt_project(typeless files = "", _str project = '') name_info(',')
{
   ...
   if (project != '') {
      ...
      for (i:=0;;++i) {
         ...
         if (filename != '') {
            /* NOTE: We don't check for wildcards here because this command/function
               is only used for the Document context menu and for drag drop. In both
               these cases, we are not adding template files or new files.
            */
...
bool _VPJFileMatchesWildcard(int handle,_str abs_filename) {
   temp_handle:= -1;
   int wildcardNodes[];
   ...
   if(!_GetAssociatedProjectInfo(ProjectName,auto AssociatedFile,auto AssociatedFileType)) {
      assoc_handle := _xmlcfg_open(AssociatedFile, auto xcstatus, VSXMLCFG_OPEN_REFCOUNT|VSXMLCFG_OPEN_ADD_PCDATA);
      if (assoc_handle > 0) {
         is_wildcard_project:=_IsVisualStudioWildcardProject(assoc_handle);
         if (is_wildcard_project) {
            //ProjectName:=_strip_filename(VisualStudioProjectName,'E'):+'.vpj';
            ...
            _MSBuildGetCustomView(AssociatedFileType,assoc_handle, temp_handle, FilesNode, _workspace_filename, ProjectName);
            _VPJGetWildcards(temp_handle,wildcardNodes);
            handle=temp_handle;
         }
         _xmlcfg_close(assoc_handle); 
      }
   } else {
      _VPJGetWildcards(handle,wildcardNodes);
   }

   rel_filename:=relative(abs_filename,_strip_filename(ProjectName,'N'));
   matched_wildcard:=false;
   if (wildcardNodes._length()) {
      for (i:=0;i<wildcardNodes._length();++i) {
         int node=wildcardNodes[i];
         excludes:=_xmlcfg_get_attribute(handle,node,'Excludes');
         bool recurse=_xmlcfg_get_attribute(handle,node,'Recurse','0')!='0';
         wildcard:=_xmlcfg_get_attribute(handle,node,'N');
         if (recurse) {
            if (_FileRegexMatchPath(wildcard,rel_filename,true)) {
               if(excludes=='' || !_FileRegexMatchExcludePath(excludes,rel_filename)) {
                  matched_wildcard=true;
                  break;
               }
            }
         } else {
            if (_FileRegexMatchName(wildcard,rel_filename)) {
               if(excludes=='' || !_FileRegexMatchExcludeName(excludes,rel_filename)) {
                  matched_wildcard=true;
                  break;
               }
            }
...
bool _ProjectFileMatchesWildcards(int handle,int (&wildcardNodes)[],_str rel_filename) {
   matched_wildcard:=false;
   if (wildcardNodes._length()) {
      for (i:=0;i<wildcardNodes._length();++i) {
         int node=wildcardNodes[i];
                                     ...
                                     bool FileExistsOnDisk=true)
{
   ...
   if (_IsWorkspaceAssociated(_workspace_filename) &&
      ...
      bool only_added_files_matching_wildcards=false;
      if(_file_eq(_get_extension(GetProjectDisplayName(project_filename), true), JBUILDER_PROJECT_EXT)) {
         ...
         status=_AddFileToVCPPMakefile(filelist,project_filename,!FileExistsOnDisk,only_added_files_matching_wildcards);
      }
      _clearProjectFileListCache(_workspace_filename,project_filename);
      if (!only_added_files_matching_wildcards) {
   ...
   */
   ...
   for (;;) {
      ...
      if (filename=='') break;
      _str abs_filename=absolute(filename);
      filename=relative(abs_filename,project_path);
      int Node=_ProjectGet_FileNode(handle,filename);
      if (Node<0) {
         bool matched_wildcard=_ProjectFileMatchesWildcards(handle,wildcardNodes,filename);
         if (!matched_wildcard) {
            NewFilesList[NewFilesList._length()]=filename;
         } else if (!file_exists(abs_filename)) {
            typeless buf_id;
            parse buf_match(abs_filename,1,'vx') with buf_id .;
            if (buf_id=='') {
               _file_touch(abs_filename);
            } else {
               status2:=_open_temp_view(abs_filename,auto temp_wid2,auto orig_wid2,"+bi "buf_id);
               if (!status2) {
                  // Would be better if we could call a high level save. Caller really 
                  // should save the project files first.
                  _save_file();
                  _delete_temp_view(temp_wid2);
                  p_window_id=orig_wid2;
               }
            }
         }
      }
   ...
   if (NewFilesList._length()) {
      ...
      call_list('_prjupdate_',project_filename);
      toolbarUpdateFilterList(project_filename);
      // regenerate makefile
      ...
      // Added a wildcard, this will be updated later
      //toolbarUpdateFilterList(project_filename);
...
int ctlrebuild_tag_file_btn.lbutton_up()
{
   ...
   index := ctl_tag_files._TreeCurIndex();
   if (ctl_tag_files._TreeGetDepth(index)!=TAGFORM_FILE_DEPTH) return(0);
   orig_view_id:=0;
   get_window_id(orig_view_id);

   // get tag file name
   TagFilename := ctl_tag_files.GetRealTagFilenameFromTree(index);
   ...
   FolderIndex := ctl_tag_files._TreeGetParentIndex(index);
   mode_name   := "";
   parse ctl_tag_files._TreeGetCaption(FolderIndex) with '"'mode_name'"';
   ...
   if (_iswindow_valid(orig_view_id)) {
      activate_window(orig_view_id);
      if (ctl_tag_files._TreeIndexIsValid(index)) {
         if (bmp_index != 0) {
            ctl_tag_files._TreeSetInfo(index,TREE_NODE_LEAF,bmp_index,bmp_index);
            ctl_tag_files._TreeRefresh();
         }
         ctl_tag_files.call_event(CHANGE_SELECTED,index,ctl_tag_files,ON_CHANGE,'W');
      }
   }
...
int ctlretag_files_btn.lbutton_up()
{
   ...
   index := ctl_tag_files._TreeCurIndex();
   if (ctl_tag_files._TreeGetDepth(index)!=TAGFORM_FILE_DEPTH) return(0);
   _str TagFilename=ctl_tag_files.GetRealTagFilenameFromTree(index);
   ...
   mou_hour_glass(true);
   common_dir    := ctl_dir_label.p_user;
   file_list_wid := ctl_file_list.p_window_id;
   list_view_id := 0;
   orig_view_id := _create_temp_view(list_view_id);
   if (orig_view_id == '') {
      return (COMMAND_CANCELLED_RC);
   ...
   file_list_wid._TreeGetSelectionIndices(auto indexes);
   foreach (auto i in indexes) {
      filename := file_list_wid._TreeGetCaption(i);
      if (common_dir != null && common_dir != "") {
         filename = absolute(filename, common_dir);
      }
      list_view_id.insert_line(strip(filename));
   }
...
bool _GetWildcardsForTagFile(bool getFromTree,_str &wildcards,_str &mode_name="")
{
   if (getFromTree) {
      ...
      index := ctl_tag_files._TreeCurIndex();
      if (ctl_tag_files._TreeGetDepth(index)==TAGFORM_FILE_DEPTH) {
         index = ctl_tag_files._TreeGetParentIndex(index);
      }
      typeless ProjectTagfilesIndex, autoUpdateFilesIndex;
      typeless cppCompilerTagFilesIndex, javaCompilerTagFilesIndex, extFilesIndexList;
      parse TAG_FOLDER_INDEXES() with ProjectTagfilesIndex autoUpdateFilesIndex cppCompilerTagFilesIndex javaCompilerTagFilesIndex extFilesIndexList;
      ...
      while (extFilesIndexList != '') {
         ...
         if (temp_index!='' && index==temp_index) {
            ...
            parse ctl_tag_files._TreeGetCaption(temp_index) with '"'mode_name'"';
...
{
   result:=_file_types_get_wildcards_by_wildcards(def_file_types,wildcards,true);
   if (result=='') {
      return wildcards;
   }
   return result;
...
void ctltree_btn.lbutton_up()
{
   // Check if tree is selected, and chdir to directory containing tag file
   index := ctl_tag_files._TreeCurIndex();
   if (ctl_tag_files._TreeGetDepth(index)!=TAGFORM_FILE_DEPTH) return;
   _str TagFilename=ctl_tag_files.GetRealTagFilenameFromTree(index);
   ...
   def_file_types=_file_types_append_filters(EXTRA_FILE_FILTERS,def_file_types);
...
{
   _ul2_image_sizebar_handler(2*ctldone.p_width, ctl_file_list.p_x_extent-2*ctldone.p_width);
}
...
_tag_form.on_resize()
{
   ...
   ctl_tag_files.p_height = form_height - 2*ctl_tag_files.p_y - 2*ctldone.p_height; 
   ctlup_btn.resizeToolButton(ctl_tag_files.p_height intdiv 5 - PADDING_BETWEEN_CONTROL_BUTTONS);
   _divider.p_width = ctlup_btn.p_width;
   alignUpDownListButtons(ctl_tag_files.p_window_id, 
   ...
   alignUpDownListButtons(ctl_file_list.p_window_id, 
                          form_width - 60,
   ...
   ctl_file_list.p_x=_divider.p_x_extent+75;
   delta_x := (ctl_file_list.p_x - orig_list_x);
   ctl_files_label.p_x += delta_x;
   ctl_files_gauge.p_x += delta_x;
   ctl_files_gauge.p_x_extent = ctl_file_list.p_x_extent - ctl_tag_files.p_x; 
   ctl_dir_label.p_x += delta_x;
   ctl_dir_label.p_width = (ctlfiles_btn.p_x_extent - ctl_file_list.p_x);

   // adjust the tree height
   done_button_height := orig_ctldone_visible? ctldone.p_height+120 : 0;
   tree_height := form_height - ctl_tag_files.p_y - done_button_height - 120;
   ctl_tag_files.p_height = tree_height;
   ctl_file_list.p_height = tree_height;

   // adjust the divider position
   _divider.p_y = ctlautotag_btn.p_y_extent + 15;
   _divider.p_height = ctl_tag_files.p_y_extent - _divider.p_y;
   _divider.p_width  = ctlup_btn.p_width;

   // calculate the location for the "Done" button
   ctldone.p_x = ctl_tagfiles_label.p_x;
   ctldone.p_y = ctl_tag_files.p_y_extent + 120;
   ctl_dir_label.p_y = ctldone.p_y;
...
{
   index := ctl_tag_files._TreeGetFirstChildIndex(ParentIndex);
   str := "";
   for (;;) {
      if (index<0) break;
      if (!lookForCheck || ctl_tag_files._TreeGetCheckState(index) == TCB_CHECKED) {
         filename := ctl_tag_files._TreeGetCaption(index);
      ...
      index=ctl_tag_files._TreeGetNextSiblingIndex(index);
   }
   return(str);
}
...
static void SetLanguageTagFiles(int index)
{
   ...
   mode_name := "";
   parse ctl_tag_files._TreeGetCaption(index) with '"'mode_name'"';
   lang := _Modename2LangId(mode_name);
...
static void SetAutoUpdateTagFiles(int index)
{
   ...
   childIndex := ctl_tag_files._TreeGetFirstChildIndex(index);
   for(;;) {
      if(childIndex < 0) break;
      // get local and remote filenames
      _str local = ctl_tag_files._TreeGetUserInfo(childIndex);
      remote := ctl_tag_files._TreeGetCaption(childIndex);
      ...
      // move next child
      childIndex = ctl_tag_files._TreeGetNextSiblingIndex(childIndex);
   }
...
void ctlremove_tag_file_btn.lbutton_up()
{
   ...
   ctl_tag_files._TreeDelete(index);
   ...
   if (ctl_tag_files._TreeGetNumChildren(FolderIndex) == 0) {
      ctl_tag_files._TreeSetCurIndex(FolderIndex);
   }
...
void ctlremove_files_btn.lbutton_up()
{
   ...
   depth := ctl_tag_files._TreeGetDepth(index);
   if (depth!=TAGFORM_FILE_DEPTH) return;
   TagFilename := ctl_tag_files.GetRealTagFilenameFromTree(ctl_tag_files._TreeCurIndex());
   if (ctl_file_list._TreeGetNumSelectedItems() <= 0) return;

   // we can't remove files from a .bsc file
   if (lowcase(_get_extension(TagFilename,true)) :== BSC_FILE_EXT) {
      _beep();
      return;
   }
   ...
   new_limit  := ctl_file_list._TreeGetNumChildren(TREE_ROOT_INDEX)+5;
   if (new_limit<orig_limit) new_limit=orig_limit;
   _default_option(VSOPTION_WARNING_ARRAY_SIZE,new_limit);

   // get the list of files selected in the tree control
   _str LBFiles[];
   common_dir := ctl_dir_label.p_user;
   ctl_file_list._TreeGetSelectionIndices(auto indexes);
   foreach (auto i in indexes) {
      filename := ctl_file_list._TreeGetCaption(i);
      if (common_dir != null && common_dir != "") {
         filename = absolute(filename, common_dir);
      }
      LBFiles :+= filename;
...
static int ItemInTree(_str Item,int ParentIndex,_str Options='')
{
   ...
   for (;;) {
      if (index<0) break;
      CurItem := ctl_tag_files._TreeGetCaption(index);
      rest := "";
...
void ctlnew_tag_file_btn.lbutton_up()
{
   ...
   categoryLetter := 'E';
   depth := -1;
   origindex := ctl_tag_files._TreeCurIndex();
   if (origindex >= 0) {
      // use the depth to figure out whether this is a tag file or category
      depth = ctl_tag_files._TreeGetDepth(origindex);
      if (depth == TAGFORM_FILE_DEPTH) {
         // file, get the parent category
         FolderIndex = ctl_tag_files._TreeGetParentIndex(origindex);
      }else{
   ...
   default:   // Extension specific tag files
      ...
      while (true) {
         ...
         // see if this matches
         parse ctl_tag_files._TreeGetCaption(temp_index) with '"' auto caption_mode_name '"';
   ...
   if (index < 0) {
      ...
      if (addNewExtensionCategory != '') {
         FilesIndex := ctl_tag_files._TreeAddItem(TREE_ROOT_INDEX,     // Relative Index
      ...
      index=ctl_tag_files._TreeAddItem(origindex,                      // Relative Index
                                       allcaption,                     // Caption
                                       flags,                          // Flags
                                       bmp_index,                      // Collapsed Bitmap Index
                                       bmp_index,                      // Expanded Bitmap Index
                                       TREE_NODE_LEAF);                // Initial State
      parentindex := ctl_tag_files._TreeGetParentIndex(index);
      ctl_tag_files._TreeSetInfo(parentindex,TREE_NODE_EXPANDED);
      if (FolderIndex != ProjectTagfilesIndex && FolderIndex != autoUpdateFilesIndex) {
         ctl_tag_files._TreeSetCheckable(index, 1, 0, TCB_CHECKED);
      }

      // if this is an auto-updated tagfile, store the absolute local
      // path in the user info for the tree node
      if (FolderIndex>0 && FolderIndex == autoUpdateFilesIndex) {
         ctl_tag_files._TreeSetUserInfo(index, localAutoUpdatedCopy);
      }
   ...
   // select the tag file
   if (index > 0) {
      ctl_tag_files._TreeSetCurIndex(index);
   }
...
static void UpdateTagFilesForLanguages(_str forLangId="")
{
   ...
   foreach (auto langId => auto langTagFileList in langTagFilesTable) {
      ...
      langSectionIndex  := ctl_tag_files._TreeSearch(TREE_ROOT_INDEX,'"'mode_name'" ':+LANGUAGE_CONFIG_FOLDER_NAME);
      langCompilerIndex := ctl_tag_files._TreeSearch(TREE_ROOT_INDEX,'"'mode_name'" ':+COMPILER_CONFIG_FOLDER_NAME);
      ...
      if (langSectionIndex > 0) {
         ...
         ctl_tag_files._TreeBeginUpdate(langSectionIndex);
         ctl_tag_files.AddLanguageTagFiles(langSectionIndex,_replace_envvars(langTagFileList),_replace_envvars(langTagFileListAll),useThread);
         ctl_tag_files._TreeEndUpdate(langSectionIndex);
      } else {
...
static void UpdateTagFilesForm(_str forLangId="")
{
   ...
   if (formwid) {
      ...
      wid := p_window_id;
      p_window_id=formwid.ctl_tag_files;
      ...
      if (index > TREE_ROOT_INDEX && _TreeIndexIsValid(index)) {
         call_event(CHANGE_SELECTED,index,p_window_id,ON_CHANGE,'W');
         formwid.ctl_file_list.refresh('W');
         formwid.ctl_file_list.p_redraw=true;
      }
...
//
ctl_tag_files.rbutton_up,context()
{
   ctl_tag_files.call_event(ctl_tag_files,LBUTTON_DOWN);
...
//
void ctl_file_list.rbutton_up,context()
{
   ...
   menu_index  := find_index("_tag_list_menu",oi2type(OI_MENU));
   menu_handle := p_active_form._menu_load(menu_index,'P');

   tf_index := ctl_tag_files._TreeCurIndex();
   if (ctl_tag_files._TreeGetDepth(tf_index) != TAGFORM_FILE_DEPTH) {
      _menu_set_state(menu_handle,"treeitem",MF_GRAYED,'C');
   }
   if (ctl_file_list._TreeGetNumChildren(TREE_ROOT_INDEX) <= 0) {
      _menu_set_state(menu_handle,"listitem",MF_GRAYED,'C');
      _menu_set_state(menu_handle,"delitem",MF_GRAYED,'C');
   }
   TagFilename := ctl_tag_files.GetRealTagFilenameFromTree(tf_index);
   if (isWorkspaceTagFileName(TagFilename)) {
      _menu_set_state(menu_handle,"treeitem",MF_GRAYED,'C');
      _menu_set_state(menu_handle,"delitem",MF_GRAYED,'C');
...
{
   TAGFORM_SKIP_ON_CHANGE(1);
   status := ctl_tag_files._TreeMoveUp(FileIndex);
   TAGFORM_SKIP_ON_CHANGE(0);
   return (status == 0);
}
...
{
   TAGFORM_SKIP_ON_CHANGE(1);
   status := ctl_tag_files._TreeMoveDown(FileIndex);
   TAGFORM_SKIP_ON_CHANGE(0);
   return (status == 0);
}
