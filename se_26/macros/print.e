...
void ctlPreview.lbutton_up()
{
   ...
   color_profile_name := "";
   if (ctl_color_profile_name.p_enabled) {
      color_profile_name = ctl_color_profile_name._lbget_text();
      color_profile_name = se.color.ColorScheme.removeProfilePrefix(color_profile_name);
   }
   printOptions.color_profile_name = color_profile_name;
...
static void doQtPreviewDialog(int editorctl_wid=0, VSPRINTOPTIONS PrintOptions=null, bool printSelection=false)  {
   ...
   print_options := PrintOptions.print_options;
   ...
   _QtPrintPreviewDialog(
          ...
          PrintOptions.color_profile_name
          );
...
static void enableOrDisableColorProfile()
{
   enableColorProfile := true;
   if (!_print_color.p_enabled) {
      enableColorProfile = false;
   } else if (!ctlColorCoding.p_value && !_print_color.p_value && !ctlBgColor.p_value) {
      enableColorProfile = false;
   }

   if (ctl_color_profile_name.p_enabled != enableColorProfile) {
      ctl_color_profile_name.p_enabled = enableColorProfile;
      ctl_color_profile_label.p_enabled = enableColorProfile;
      ctl_color_profile_picker.p_enabled = enableColorProfile;
   }
}
...
void _ok.on_create()
{
   ...
   // set up default color profile name
   default_profile_name := def_color_scheme;
   if (wid && wid._isEditorCtl()) {
      default_profile_name = wid.p_WindowColorProfile;
   }
   ctl_color_profile_name.langUpdateColorProfileList(default_profile_name, includeAutomatic:false);
   ctl_color_profile_name._cbset_text("");
   if (ctl_color_profile_name._lbfind_item("Light: Default")) {
      ctl_color_profile_name._lbadd_item("Default", 60, _pic_lbvs);
   }
   if (ctl_color_profile_name._lbfind_item("Light: Grayscale")) {
      ctl_color_profile_name._lbadd_item("Grayscale", 60, _pic_lbvs);
   }
   ctl_color_profile_name._cbset_text("");
   ...
   color_profile_name := "";
   if (gvsPrintOptions._length() >= 9 && gvsPrintOptions.color_profile_name != null) {
      color_profile_name = gvsPrintOptions.color_profile_name;
      if (color_profile_name != "") {
         if (color_profile_name != "Default" && color_profile_name != "Grayscale") {
            color_profile_name = se.color.ColorScheme.addProfilePrefix(color_profile_name);
         }
         if (ctl_color_profile_name._lbfind_item(color_profile_name)) {
            ctl_color_profile_name._cbset_text(color_profile_name);
         }
      }
   }
   ...
   initPrintSchemes();
   wid=_form_parent();
   if (wid.p_HasBuffer && !(wid.p_window_flags & HIDE_WINDOW_OVERLAP) && wid.p_lexer_name=='') {
      ctlColorCoding.p_enabled=_print_color.p_enabled=ctlBgColor.p_enabled=false;
      //ctlColorCoding.p_value=_print_color.p_value=0;
      enableOrDisableColorProfile();
   }
...
static void _print_form_initial_alignment()
{
   ...
   sizeBrowseButtonToTextBox(ctl_color_profile_name.p_window_id, ctl_color_profile_picker.p_window_id);
...
void _ok.lbutton_up(bool doClose=true)
{
   ...
   gvsPrintOptions.color_profile_name=color_profile_name;
...
void ctl_color_profile_name.on_got_focus() 
{
   if (p_style != PSCBO_NOEDIT) {
      p_style = PSCBO_NOEDIT;
   }
}
void ctl_color_profile_name.on_lost_focus() 
{
...
void ctl_color_profile_name.on_drop_down(int reason) 
{
...
void ctl_color_profile_name.on_change(int reason, int index = 0)
{
   switch (reason) {
   case CHANGE_OTHER:
   case CHANGE_CLINE:
...
void ctl_color_profile_name.DEL()
{
   _cbset_text("");
}

void ctl_color_profile_picker.lbutton_up() 
{
   langId := "c";
   wid := _form_parent();
   if (wid && wid._isEditorCtl()) {
 ...
 * @see gui_print
 * 
 * @categories Buffer_Functions, Selection_Functions, Miscellaneous_Functions
 */
   ...
   if (result==1) {
      if (_param9!='') {
         ...
         while (!down()) {
            ...
            p_window_id=filelist_view_id;
            _macro('m',_macro('s'));
            _macro_call('print',_param1,_param2,_param3,_param4,_param5,_param6,_param7,_param8,_param11);
         }
         _delete_temp_view(filelist_view_id);
         p_window_id=orig_view_id;
      }else{
 ...
 * @categories Buffer_Functions, Selection_Functions, Miscellaneous_Functions
 */ 
_command print(_str font="",
               _str lheader="", _str lfooter="",
               _str options="",
               _str cheader="", _str cfooter="",
               _str rheader="", _str rfooter="",
               _str color_profile_name=""
              ) name_info(','VSARG2_READ_ONLY|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_NOEXIT_SCROLL)
{
   typeless status=0;
...
_command print_selection( _str print_font="",
                          _str print_header="",
                          _str print_footer="",
                          _str print_options="",
                          _str print_cheader="",
                          ...
                          _str print_rfooter="",
                          _str color_profile_name=""
                        ) name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_MARK)
{

   useDefaults := (print_font == "" && 
                   print_header == "" && 
                   print_footer == "" &&
                   print_options == "" && 
                   print_cheader == "" && 
...
void _PrintSetConfig(VSPRINTCONFIG &result)
{
   ...
   color_profile_name := result.ColorProfileName;
   if (color_profile_name == null) {
      color_profile_name = "";
   }
   gvsPrintOptions.color_profile_name=result.ColorProfileName;
...
void _PrintGetConfig(VSPRINTCONFIG &result)
{
   ...
   result.ColorProfileName = "";
   if (gvsPrintOptions._length() >= 9 && gvsPrintOptions.color_profile_name != null) {
      result.ColorProfileName = gvsPrintOptions.color_profile_name;
   }
}
