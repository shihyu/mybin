...
static const SLICKC_ERROR_MESSAGE_SOURCE=      "Slick-C Compiler (st)";
static const SLICKC_ERROR_MESSAGE_TYPE=        "Slick-C Compile Error";
...
_command vstw(_str name="",...) name_info(','VSARG2_REQUIRES_MDI_EDITORCTL|VSARG2_REQUIRES_PRO_EDITION)
{
  ...
  if (!_in_firstinit && !_BuildingStateFile()) {
...
static void updateSlickCErrorMessagesInErrorFile()
{
   if (!_haveProMacros()) {
      return;
   }
   if (_BuildingStateFile()) {
      return;
   }
   mc_index := find_index("get_messageCollection", PROC_TYPE);
   if (!index_callable(mc_index)) {
      return;
   }

   se.messages.MessageCollection* pCollection = get_messageCollection();
   if (!pCollection) return;
   pCollection->removeMessages(SLICKC_ERROR_MESSAGE_SOURCE);

   // create a message type
   se.messages.Message newMsgType;
   newMsgType.m_creator = SLICKC_ERROR_MESSAGE_SOURCE;
   newMsgType.m_type    = SLICKC_ERROR_MESSAGE_TYPE;
   newMsgType.m_sourceFile = "";
   newMsgType.m_date = "";
   newMsgType.m_autoClear = se.messages.Message.MSGCLEAR_ALWAYS;
   pCollection->newMessageType(newMsgType);
   pCollection->startBatch();

   num_messages := 0;
   loop {
      get_line(auto error_line);
      filename := parse_file(error_line);
      typeless line="";
      typeless col="";
      parse error_line with line col':' auto msg;

      se.messages.Message newMsg;
      newMsg.m_creator = SLICKC_ERROR_MESSAGE_SOURCE;
      newMsg.m_type    = SLICKC_ERROR_MESSAGE_TYPE;
      newMsg.m_sourceFile = filename;
      newMsg.m_date = "";
      newMsg.m_lineNumber = line;
      newMsg.m_colNumber = col;
      newMsg.m_description = msg;

      num_messages++;
      pCollection->addSingleMessage(newMsg);
      if (down()) break;
   }

   pCollection->endBatch();
   pCollection->notifyObservers();
   if (num_messages > 1) {
      // activate_messages();
      activate_tool_window('_tbmessages_browser_form', set_focus:false, '_message_tree', true);
...
void clearSlickCErrorMessages()
{
   ...
   if (index_callable(mc_index)) {
      se.messages.MessageCollection* pCollection = get_messageCollection();
      if (!pCollection) return;
      if (_allow_clearSlickCErrorMessages) {
         pCollection->removeMessages(SLICKC_ERROR_MESSAGE_SOURCE);
      }
   }
}
...
_str st_position_on_error(_str errfile="")
{
  ...
  if ( p_Noflines>0 ) {
    get_line(last_line);
    if (!_in_firstinit && !_BuildingStateFile()) {
       updateSlickCErrorMessagesInErrorFile(); 
    }
    _delete_buffer();
...
void _UpdateSlickCStack(int ignoreNStackItems=0,int errorCode=0,_str DumpFileName="",int timer_handle=-1)
{
   ...
   wid.edit1._lbclear();
   if (DumpFileName!='') {
      if (index_callable(find_index('_is_main_timer',PROC_TYPE|COMMAND_TYPE)) && 
          _is_main_timer(timer_handle)) {
         wid.edit1.insert_line(' Main timer stopped due to this Slick-C stack');
      }
...
_command void slickc_stack_goto() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_MDI|VSARG2_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   line := "";
   get_line(line);
   if (beginsWith(line, "stack[")) {
      parse line with "stack[" auto date_time ']' line;
   }
...
bool _on_slickc_error(int errorCode,_str filename,int timer_handle)
{
   _UpdateSlickCStack(1,errorCode,filename,timer_handle);
   //_StackDump(0,1,1);
