...
void _on_apptheme_changed(_str theme)
{
   if ( theme !=  g_toolbar_pic_theme ) {
      ...
      // change all editor window colors if they are using Automatic profile
      update_editor_colors  := (def_color_scheme == CONFIG_AUTOMATIC);
      update_minimap_colors := (def_minimap_color_scheme == CONFIG_AUTOMATIC);
      if (update_editor_colors || update_minimap_colors) {
         _change_all_window_colors(langId: null, CONFIG_AUTOMATIC, update_editor_colors, update_minimap_colors);
      }
   }
}
