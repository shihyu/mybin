...
static DefaultColorsConfig COLOR_PROFILES(...) {
   if (arg()) _ctl_color_scheme_list.p_user=arg(1);
   return _ctl_color_scheme_list.p_user;
}
static _str ORIG_COLOR_PROFILE(...) {
   if (arg()) ctl_code_sample.p_user=arg(1);
   return ctl_code_sample.p_user;
}
static _str ORIG_MINIMAP_PROFILE(...) {
   if (arg()) _ctl_minimap_label.p_user=arg(1);
   return _ctl_minimap_label.p_user;
}
...
static const MINIMAP_USE_SAME_PROFILE="<Same as Editor>";

void _quick_start_colors_form_save_settings()
{
   ORIG_COLOR_PROFILE(ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text()));
   ORIG_MINIMAP_PROFILE(ColorScheme.removeProfilePrefix(_ctl_minimap_scheme_list._lbget_text()));
}
...
bool _quick_start_colors_form_is_modified()
{
   ...
   // did the selected color profile change?
   return (ORIG_COLOR_PROFILE() != ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text()) ||
           ORIG_MINIMAP_PROFILE() != ColorScheme.removeProfilePrefix(_ctl_minimap_scheme_list._lbget_text())); 
}

bool _quick_start_colors_form_validate()
{
   if ( (ORIG_COLOR_PROFILE() == ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text())) && 
...
{
   // grab the color profile name and apply the color settings as the default
   profileName := ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text());
   applyColorProfile(profileName);

   // grab the color profile name and apply the color settings as the default
   minimapProfileName := ColorScheme.removeProfilePrefix(_ctl_minimap_scheme_list._lbget_text());
   if (minimapProfileName == MINIMAP_USE_SAME_PROFILE) minimapProfileName = "";
   applyMinimapColorProfile(profileName, minimapProfileName);
...
void _quick_start_colors_form_cancel()
{
   ...
   // reset stuff to the original profiles
   applyColorProfile(ORIG_COLOR_PROFILE(), true);
}

static void _quick_start_colors_form_restore()
{
   ...
   if (purpose == OP_QUICK_START) {
      currentProfile := ColorScheme.getDefaultProfile();
      if (currentProfile != ORIG_COLOR_PROFILE() && currentProfile != ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text())) {
         // they've gone and changed things!
         _ctl_color_scheme_list._lbfind_and_select_item(ColorScheme.addProfilePrefix(currentProfile));
         ORIG_COLOR_PROFILE(currentProfile);
      }

      minimapProfile := ColorScheme.getMinimapProfile();
      if (minimapProfile != ORIG_MINIMAP_PROFILE() && minimapProfile != ColorScheme.removeProfilePrefix(_ctl_minimap_scheme_list._lbget_text())) {
         // they've gone and changed things!
         if (minimapProfile == "") minimapProfile = MINIMAP_USE_SAME_PROFILE;
         _ctl_minimap_scheme_list._lbfind_and_select_item(ColorScheme.addProfilePrefix(minimapProfile));
         ORIG_MINIMAP_PROFILE(minimapProfile);
      }
   }
}
...
_ctl_color_scheme_list.on_create()
{
   ...
   app_theme := _app_theme_auto();
   if (!_isMac() && !_isWindows()) {
   ...
   if (_ctl_minimap_scheme_list.p_visible) {
   ...
   // force the color profile and minimap profiles to be set correctly
   call_event(CHANGE_OTHER, _ctl_color_scheme_list.p_window_id, ON_CHANGE, "W");
   call_event(CHANGE_OTHER, _ctl_minimap_scheme_list.p_window_id, ON_CHANGE, "W");
...
void fillInColorProfiles()
{
   ...
   _ctl_color_scheme_list._lbaddColorProfileNames(dcc);

   // load the options for the minimap profiles, including "" for the default to
   // use the same profile name as the main editor control
   _ctl_minimap_scheme_list._lbaddColorProfileNames(dcc);
   _ctl_minimap_scheme_list._lbadd_item(MINIMAP_USE_SAME_PROFILE,60,_pic_lbvs);
   _ctl_minimap_scheme_list._lbsort('i');

   currentProfile := ColorScheme.getDefaultProfile();
   if (_ctl_color_scheme_list._lbfind_and_select_item(ColorScheme.addProfilePrefix(currentProfile))) {
      _ctl_color_scheme_list._lbfind_and_select_item(ColorScheme.addProfilePrefix('Default'));
   }

   minimapProfile := ColorScheme.getMinimapProfile(allowEmpty:true);
   if (minimapProfile == "") {
      minimapProfile = MINIMAP_USE_SAME_PROFILE;
   } else {
      minimapProfile = ColorScheme.addProfilePrefix(minimapProfile);
   }
   if (_ctl_minimap_scheme_list._lbfind_and_select_item(minimapProfile)) {
      _ctl_minimap_scheme_list._lbfind_and_select_item(ColorScheme.addProfilePrefix('Default'));
   }
...
void _ctl_customize_colors_link.lbutton_up()
{
   ...
   _quick_start_colors_form_restore();
}
...
void _ctl_color_scheme_list.on_change(int reason)
{
   ...
   profileName := ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text());
   ctl_code_sample.p_WindowColorProfile = profileName;

   // update the minimap profile if it is the same as editor profile
   minimapProfileName := ColorScheme.removeProfilePrefix(_ctl_minimap_scheme_list._lbget_text());
   if (minimapProfileName == MINIMAP_USE_SAME_PROFILE) {
      minimapProfileName = profileName;
      minimap_wid := ctl_code_sample.p_minimap_wid;
      if (minimap_wid && _iswindow_valid(minimap_wid)) {
         minimap_wid.p_WindowColorProfile = profileName;
      }
   }
}

void _ctl_minimap_scheme_list.on_change(int reason)
{
   // grab the profile name
   profileName := ColorScheme.removeProfilePrefix(_ctl_minimap_scheme_list._lbget_text());
   if (profileName == MINIMAP_USE_SAME_PROFILE) {
      profileName = ColorScheme.removeProfilePrefix(_ctl_color_scheme_list._lbget_text());
   }
...
static void applyColorProfile(_str profileName, bool applySymbolColors = true)
{
   DefaultColorsConfig dcc;
   dcc = COLOR_PROFILES();
   if (dcc == null) return;
   scm := dcc.loadProfile(profileName);

   scm->applyColorProfile(profileName);

   if (applySymbolColors && scm->m_symbolColoringProfile != null) {
      if (scm->m_symbolColoringProfile != "" && scm->m_symbolColoringProfile != CONFIG_AUTOMATIC) {
         scm->applySymbolColorProfile();
      } else {
         localProfile := *scm;
         localProfile.m_symbolColoringProfile = "";
         localProfile.applySymbolColorProfile();
      }
   }
}

static void applyMinimapColorProfile(_str profileName, _str minimapProfileName)
{
   DefaultColorsConfig dcc = COLOR_PROFILES();
   if (dcc == null) return;
   realProfileName := minimapProfileName;
   followEditorProfile := false;
   if (minimapProfileName == "") {
      realProfileName = profileName;
      followEditorProfile = true;
   }
   scm := dcc.loadProfile(realProfileName);
   scm->applyMinimapColorProfile(realProfileName, followEditorProfile);
}
...
void _quick_start_setup_project_form.esc() 
{
...
void ctl_workspace_open.lbutton_down()
{
   if (ctl_workspace_open.p_user == "open") {
      _param1 = ctl_do_not_show.p_value;
      p_active_form._delete_window(p_name);
      return;
   }

   // otherwise, this is quick-start
   nextEnabled := prevEnabled := false;
   wid := p_active_form;
   wid.disableEnableNextPreviousOptionsButtons(true, nextEnabled, prevEnabled);
 
   orig_def_workspace_smart_open := def_workspace_smart_open;
   def_workspace_smart_open = false;
   parse p_name with "ctl_" auto command_name;
   execute(command_name);
   def_workspace_smart_open = orig_def_workspace_smart_open;
...
void _ctl_start_project_wizard.on_create(_str mode="quickstart")
{
   ctl_workspace_open.p_user = mode;
   if (mode == "open") {
      ctlframe_create.p_visible = false;
      ctlframe_create.p_enabled = false;
      ctlframe_samples.p_visible = false;
      ctlframe_samples.p_enabled = false;

      ctl_close.p_y -= ctlframe_create.p_height;
      ctl_help.p_y   -= ctlframe_create.p_height;
      ctl_do_not_show.p_y -= ctlframe_create.p_height;
      p_active_form.p_height -= ctlframe_create.p_height;

   } else {

      ctl_info_html.p_visible = false;
      ctlframe_open.p_y = ctl_info_html.p_y;
      ctlframe_create.p_y -= ctl_info_html.p_height;
      ctlframe_samples.p_y -= ctl_info_html.p_height;

      ctl_close.p_visible = false;
      ctl_help.p_visible = false;
      ctl_do_not_show.p_visible = false;
   }
...
void ctl_workspace_open_sample.lbutton_up()
{
   nextEnabled := prevEnabled := false;
   wid := p_active_form;
   wid.disableEnableNextPreviousOptionsButtons(true, nextEnabled, prevEnabled);

   workspace_open_sample();  
