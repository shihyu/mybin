...
int _new_rust_proj(bool inDialog,_str packageName,_str Filename,_str Path, bool add_to_workspace, bool ExecutableName,_str Dependency,bool ShowProperties=false,bool runInitMacros=true)
{
   ...
   // This ensures we know where cargo it, prompting if necessary. It only sets 
   // the environment for // the build shell, not the process shell, so we can't
   // depend on that.
   _cargo_set_environment2();
   filename := def_cargo_exe_path;
   if (filename == '') {
      filename = path_search('cargo', 'PATH', 'P');
   }
   ...
   status=shell('"'filename'" new 'Filename' --bin','q');
   if (!isdirectory(ProjectPath)) {
