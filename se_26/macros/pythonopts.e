...
_command int python_debug(_str cmdArgs = '') name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   debug_node := _ProjectGet_TargetNode(handle,'debug',config);
   if (dbgWorkingDir == '') {
      ...
      if (debug_node >= 0) {
         dbgWorkingDir = _getRunFromDirectory(handle, debug_node,_isEditorCtl(false)?p_buf_name:'');
      }
   }
   ...
   cd('+p 'dbgWorkingDir);
   if (debug_node > 0) {
      setEnvironmentFromProjectTargetNode(handle, debug_node, true);
   }
...
{
   format_list:=_file_types_get_filter_by_name(def_file_types, 'Python Files');
   wildCards:=_file_types_get_wildcards_by_name(def_file_types, 'Python Files');
   if( format_list == "" ) {
      // Fall back
