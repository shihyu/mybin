...
int svc_fill_in_branch_list(IVersionControl *pInterface,_str path,_str *pPullRepositoryName,_str &curBranch,int xbufferWID=0,int &biggestWidth=0)
{
   origWID := p_window_id;
   ...
   _lbclear();
   curBranch = "";
   status := pInterface->getBranchNames(auto branchNames,curBranch, path,true, *pPullRepositoryName,ctlshow_all_branches.p_value?SVC_BRANCH_ALL:0);
   if ( status ) {
      return status;
   }
...
void ctlok.lbutton_up()
{
   ...
   *pRemoteName = strip(remoteName);
   _save_form_response();
   p_active_form._ShowWindow(SW_HIDE);
   p_active_form.refresh('w');
   p_active_form._delete_window(0);

   // forces window to go away so we can see authentication window
   if (_isMac()) process_events(auto cancel);
}
