...
int find_complete_list_completions(var words, int max_matches, _str prefixexp="", bool forceUpdate=false)
{
   ...
   for (i=0; i<=back_matches._length()-1; i++) {
      ...
      comments := line:+"<hr>":+AutoCompleteParagraphTag();
      if (lineno > 0) {
         comments :+= "Word found on line ":+lineno:+" in current file";
      } else {
         comments :+= "Word found in current file";
      }

      // Add the result to auto complete
      tag_auto_complete_add_result(AUTO_COMPLETE_WORD_COMPLETION_PRIORITY,
