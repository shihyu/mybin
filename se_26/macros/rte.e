...
// The marker types and colors we own and maintain.  The RTE system pulls
// these when needed.  We don't push the values so we avoid problems with
// trying to make calls to a unloaded DLL during auto-restore or postinstall.
int grte_warning_marker_type;
int grte_warning_color;
int grte_error_marker_type;
int grte_error_color;
...
definit() {
   ...
   grte_warning_marker_type = _MarkerTypeAlloc();
   grte_error_marker_type = _MarkerTypeAlloc();
   _MarkerTypeSetFlags(grte_warning_marker_type, VSMARKERTYPEFLAG_DRAW_SQUIGGLY);
   _MarkerTypeSetFlags(grte_error_marker_type, VSMARKERTYPEFLAG_DRAW_SQUIGGLY);
   _MarkerTypeSetPriority(grte_warning_marker_type, VSMARKER_TYPE_PRIORITY_REAL_TIME_ERRORS);
   _MarkerTypeSetPriority(grte_error_marker_type,   VSMARKER_TYPE_PRIORITY_REAL_TIME_ERRORS);
   grte_warning_color = 0xe1e100;
   grte_error_color = 0xff0000;
}
...
//   rteSetJDKPath(def_jdk_install_dir);
   ...
   status := rteUpdateEditor(editorctl_wid.p_window_id, 
                             editorctl_wid.p_buf_name, 
                             editorctl_wid.p_buf_id, 
                             editorctl_wid.p_LangId);
   if (status >= 0) {
      //_mfXMLOutputIsActive=false;
      //_mfMessageListIsActive=true;
   }
...
static void rte_update_buffer_marker_colors(int wid, int marker_type, int color)
{
   int list[];
   wid._StreamMarkerFindList(list, wid, 0, wid.p_buf_size, VSNULLSEEK, marker_type);
   foreach (auto sm in list) {
      wid._StreamMarkerSetStyleColor(sm, color);
   }
}

static void rte_update_marker_colors(int marker_type, int color)
{
   get_window_id(auto orig_view_id);
   activate_window(VSWID_HIDDEN);
   _safe_hidden_window();
   orig_buf_id := p_buf_id;
   first_buf := _mdi.p_child.p_buf_id;
   p_buf_id = first_buf;
   for (;;) {
      rte_update_buffer_marker_colors(p_window_id, marker_type, color);
      _next_buffer('hnr');
      if (p_buf_id == first_buf) {
         break;
      }
...
void rte_update_warning_colors(int newcolor)
{
   rte_update_marker_colors(grte_warning_marker_type, newcolor);
}

void rte_update_error_colors(int newcolor)
{
   rte_update_marker_colors(grte_error_marker_type, newcolor);
}

void reset_buffer_color_profile_rte()
{
   ecol := _default_color(CFG_LIVE_ERRORS_ERROR);
   parse ecol with auto fgcolor ' '  auto rest;

   if (isinteger(fgcolor)) {
      grte_error_color = (int)fgcolor;
      rte_update_buffer_marker_colors(p_window_id, grte_error_marker_type, 
                                      grte_error_color);
   }

   wcol := _default_color(CFG_LIVE_ERRORS_WARNING);
   parse wcol with fgcolor ' '  rest;

   if (isinteger(fgcolor)) {
      grte_warning_color = (int)fgcolor;
      rte_update_buffer_marker_colors(p_window_id, grte_warning_marker_type, 
                                      grte_warning_color);
   }
}
