...
int _OnUpdate_vdlgedit_copy_control_name(CMDUI &cmdui,int target_wid,_str command)
{
   if (_deNofselected!=1 || !_display_wid || !_haveProMacros()) {
...
_command void vdlgedit_copy_control_name(_str option='') name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveProMacros()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Slick-C Form editing");
      return;
   ...
   if (!p_active_form.p_edit) return;
   if (_deNofselected==1 && _display_wid) {
      _copy_text_to_clipboard(_display_wid.p_name);
   }
}
...
static int xr_round(int x, int r)
{
...
static _str _decreate_control(int Nofselected)
{
   ...
   _lxy2lxy(p_scale_mode,SM_TWIP,orig_x,orig_y);
   orig_x=xr_round(orig_x,_grid_width());
   orig_y=xr_round(orig_y,_grid_height());
   ...
   for (;;) {
      ...
      case MOUSE_MOVE:
         ...
         new_x=xr_round(new_x,_grid_width());
         new_y=xr_round(new_y,_grid_height());
                            ...
                            int width=0, int height=0)
{
   ...
   if (width>0 && height>0) {
      ...
      x=xr_round(x,_grid_width());
      y=xr_round(y,_grid_height());
