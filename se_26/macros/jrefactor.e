         ...
         if (def_use_workspace_slickc_macros && _workspace_filename != "") {
            if (beginsWith(_strip_filename(_workspace_filename, 'P'), "slickedit-")) {
               ...
               if (file_exists(workspace_slick_sh:+"slick.sh") && 
                  ...
                  slickc_filename = workspace_slick_sh;
                  using_workspace_slickc = true;
               }
            }
         }

         if (using_workspace_slickc) {
            opts.m_project_include_path = slickc_filename;
            opts.m_system_include_path = get_env("VSLICKMACROS");;
         } else {
 ...
 * 
 * @param this_file_package_name    current package we are working on 
 * @param this_file_name            originating file name, where import is to be added.
                       ...
                       int depth=0)
{
   ...
   if (_LanguageInheritsFrom("e") || _LangaugeIsLikeCPP()) {
      ...
      if (length(import_file) == orig_import_len) {
         if (file_eq(_strip_filename(this_file_name,'ne'), _strip_filename(import_file,'ne'))) {
            import_file = _strip_filename(import_file,'p');
         }
      }
   }
...
void _jrefactor_organize_imports_form.on_create()
{
   ...
   horz_margin := ctl_auto_import.p_x;
   x1 := ctl_label_explicit_export_limit.p_x_extent + horz_margin;
   x2 := ctl_add_lines_label.p_x_extent + horz_margin;
   ctl_import_limit.p_x = max(x1,x2);
   ctl_add_lines.p_x    = max(x1,x2);
}
