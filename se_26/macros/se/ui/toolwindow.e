...
static ToolWindowInfo init_toolwindowtab:[] = {
   ...
   '_tbhighlight_form'           => {TWF_SUPPORTS_MULTIPLE, DOCKAREAPOS_RIGHT, 0},
};
...
/**
 * Return true if {@code wid} is a visible tool-window. 
 * 
 * @param wid 
 * 
 * @return bool
 ...
 * {@code form_name}, or 0 if not visible. Set 
 * {@code to_mdi_wid} if you only want the visible wid on a 
 * specific mdi window. 
 * 
 * @param form_name 
 * @param to_mdi_wid 
...
/**
 * Return true if {@code wid} is a docked tool-window or 
 * the child of a docked tool-window. Use 
 * {@code p_DockingArea} if you need to know which side the 
 * window is docked. 
 *
 * <p>
 *
 ...
 * {@code form_name}, or 0 if no docked window exists. Set 
 * {@code to_mdi_wid} if you only want the docked wid on a 
 * specific mdi window. 
 *
 * <p>
 *
...
/**
 * Return true if {@code wid} is an undocked tool-window or
 ...
 * {@code form_name}, or 0 if no undocked window exists. 
 * Set {@code to_mdi_wid} if you only want the undocked wid 
...
/** 
 * @return 
 * Return true if {@code wid} is a dockable tool-window 
...
/**
 * Return true if {@code wid} is a solo tool-window. 
 *
 * <p>
 *
...
/**
 * Return solo tool-window wid for form {@code form_name}, 
...
/**
 * Return true if {@code wid} is current Tab in its 
 ...
 * raised auto-hide window) for form {@code form_name}, or 
 * 0 if not current. Set {@code to_mdi_wid} if you only 
...
/**
 *  Test whether window {@code wid} is, or is a child of, a
 *  current tool-window. 
 * 
 * @param wid 
 * 
 ...
 * {@code form_name} and return window id. 
 *
 * <p>
 *
 * Set mdi-window id {@code mdi_wid} to find window on a 
 * specific mdi-window (defaults to 0 which finds on current 
 * mdi-window). Make sure to also include the 
 * {@code TWFF_CURRENT_MDI} flag in {@code flags}. 
 *
 * <p>
 *
 * Set {@code flags} to one or more of 
 * {@code ToolWindowFindFormFlag} to narrow search 
...
int tw_find_form(_str form_name, int mdi_wid=0, int flags=-1)
{
   ...
   if ( testFlag(flags, TWFF_FOCUS_FORM) ) {
      ...
      if ( wid > 0 && wid.p_active_form.p_name :== form_name && !wid.p_active_form.p_edit) {
         if ( wid.p_active_form._onCreateAlreadyCalled() ) {
            return wid.p_active_form;
         }
      }
   }

   // Case: Current Editor window
   // db - if current window is editor, find containing MDI
   if ( testFlag(flags, TWFF_EDITOR_MDI) ) {
      if (_isEditorCtl()) {
         editor_mdi_wid := _MDIFromChild(p_window_id);
         if ( editor_mdi_wid > 0 ) {
            wid := _MDIFindFormObject(editor_mdi_wid, form_name, 'n');
   ...
   if ( testFlag(flags, TWFF_CURRENT_MDI) ) {
      ...
      if ( mdi_wid > 0 ) {
         wid := _MDIFindFormObject(mdi_wid, form_name, 'n');
         if ( wid > 0 && wid._onCreateAlreadyCalled() ) {
            return wid;
         }
      }
   ...
   // No tool window found
   return 0;
}

/**
 * Return DockAreaPos of tool-window instance of form 
 * {@code form_name}, or 0 (DOCKAREAPOS_NONE) if 
 * tool-window is not docked. Set {@code to_mdi_wid} if you
 * only want the tool-window pos on a specific mdi window. Note 
 * that if {@code to_mdi_wid} is not specified, then the 
...
/**
 * Show tool-window by {@code form_name} and return window
...
/**
 * Hide tool-window by {@code form_name}. If the 
 * tool-window is part of a tabgroup, then the entire tabgroup 
 * is hidden. If <code>hide_others=true</code>, then all 
 * tool-windows in group except {@code form_name} are 
...
/**
 * Toggle a single tool-window by {@code form_name} on/off.
...
void tw_all_floating_set_visible(bool visible)
{
   ...
   if( window_revealed && focus_wid != 0 ) {
      focus_wid._set_focus();
   }
}
...
/**
 * Center the floating mdi-window parent of {@code wid} to
 * specified window {@code to_wid}. Set
...
/**
 * Show tool-window with name {@code form_name}. If 
 * tool-window is already showing, then it is set active. See 
 * {@code cmdline} for more options. 
 * 
...
_command int show_tool_window(_str cmdline='') name_info(FORM_ARG',')
{
            ...
            if (!_no_child_windows()) {
               ...
               if (_iswindow_valid(editorctl_wid) && editorctl_wid._isEditorCtl()) {
                  if (editorctl_wid.p_window_state == 'M') {
                     ...
                     mbrc := _message_box("The File Tabs tool window is deprecated.\nYou have Document Tabs enabled.\n\nShow File Tabs anyway?",
                                          "SlickEdit", 
                                          MB_YESNOCANCEL|MB_ICONQUESTION);
                     if (mbrc != IDYES) {
                        return COMMAND_CANCELLED_RC;
   ...
   if ( new_instance ) {
      ...
      if (tw_wid > 0) {
         ...
         tw_mdi_wid := _MDIFromChild(tw_wid);
         tw_add(tw_mdi_wid, wid, 0);
...
int _OnUpdate_show_tool_window(CMDUI& cmdui, int target_wid, _str command)
{
   ...
   // check if this window allows duplicates
   dup_supported := false;
   ToolWindowInfo* pTWInfo = tw_find_info(form_name);
   if (pTWInfo) {
      dup_supported = 0 != (pTWInfo->flags & TWF_SUPPORTS_MULTIPLE);
   }

   // check if form is already visible, auto-hidden, or undocked.
   mdi_current_wid := _MDICurrent();
   wid := tw_is_visible(form_name, mdi_current_wid);
   if ( !wid ) {
      wid = tw_is_auto_form(form_name, mdi_current_wid);
   }
   if ( !wid ) {
      wid = tw_is_undocked(form_name);
   }

   if (!wid && !dup_supported) {
      any_wid := tw_is_visible(form_name, 0);
      if ( !any_wid ) {
         any_wid = tw_is_auto_form(form_name, 0);
      }
      if (any_wid) {
...
/**
 * Activate the tool-window with name {@code form_name} and
 ...
 * {@code focus_control_name} if not ''. 
 *
 * <p>
 *
...
/**
 * Insert tool-window list into menu {@code menu_handle}.
 * 
 * @param menu_handle
...
/**
 * Delete the tool-window with name {@code form_name}. Do 
