...
class TextChangeNotify {
   ...
   void onTextChange(long startOffset, long endOffset)
   {
      id := m_list._length() - 1;
      while (id >= 0) {
         ITextChangeListener *pListener = m_list[id];
         if (pListener && (*pListener instanceof ITextChangeListener)) {
            pListener->onTextChange(startOffset, endOffset);
         } else {
            m_list._deleteel(id);
         }
         --id;
   ...
   {
      bufID := p_buf_id;
      TextChangeNotify *pNotifier = s_buffers._indexin(bufID);
      if (!pNotifier || !(*pNotifier instanceof TextChangeNotify)) {
         TextChangeNotify ev;
         s_buffers:[bufID] = ev;
         pNotifier = s_buffers._indexin(bufID);
      }
      pNotifier->add(ui);
   }
   ...
   {
      if (bufID < 0 && _isEditorCtl()) {
         bufID = p_buf_id;
      }
      if (bufID < 0) {
         foreach (bufID => auto notifierObject in s_buffers) {
            if (!notifierObject._isempty()) {
               notifierObject.remove(ui);
            }
         }
      } else {
         TextChangeNotify *pNotifier = s_buffers._indexin(bufID);
   ...
   private static void onTextChangeCallback(long startOffset, long endOffset)
   {
      ...
      TextChangeNotify *pNotifier = s_buffers._indexin(p_buf_id);
      if (!pNotifier || !(*pNotifier instanceof TextChangeNotify)) {
         return;
      }
      pNotifier->onTextChange(startOffset, endOffset);
   }
