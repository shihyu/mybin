...
enum_flags ToolWindowFindFormFlag {
   TWFF_ACTIVE_FORM = 0x01,   // Find active form (p_active_form)
   TWFF_FOCUS_FORM  = 0x02,   // Find form with focus (_get_focus().p_active_form)
   TWFF_CURRENT_MDI = 0x04,   // Find on current mdi-window (_MDICurrent(), or mdi_wid passed in to tw_find_form())
   TWFF_UNDOCKED    = 0x08,   // Find undocked windows (undocked windows are floating and not attached to an mdi-area
   TWFF_ANY         = 0x10,   // The "hail mary" flag finds non-edited form any where
   TWFF_EDITOR_MDI  = 0x20,   // Find on current mdi-window for the current editor window (_MDIFromChild(p_window_id))
};

/**
 * Register window {@code wid} as a tool-window. Must call 
 * this function before adding a tool-window to a layout. 
 *
 * <p>
 *
 * Set {@code flags} to one or more of ToolWindowFlag. 
 *
 * <p>
 *
 * Set {@code preferredArea} to one of DockArea.
 * 
 * @param wid 
 * @param flags
 * @param preferredArea
...
/**
 * Add tool-window {@code wid} to current tabgroup. Set 
 * {@code mdi_wid} to add to specific mdi-window; set to 0
 ...
 * {@code wid}. If tool-window is already part of a 
 * tile/tabgroup, then it is removed and inserted into 
 * new tile. Set {@code mdi_wid} to add new tile to 
...
/**
 * Create a new vertical tile from tool-window {@code wid}. 
 * If tool-window is already part of a tile/tabgroup, then it is 
 * removed and inserted into new tile. Set {@code mdi_wid} 
...
/**
 * Set tool-window {@code wid} active. If tool-window is
...
/**
 * Restore tool-window {@code wid} to last known 
 * {@code position}. 
 *  
 * @param wid 
 * @param position 
 ...
 * {@code tw_register}, into the array of 
 * {@code wids}. Set {@code mdi_wid} to a specific 
 ...
 * <code>tw_is_auto_raised()</code>. {@code to_mdi_wid} specifies 
