...
class OptionsImportTree : OptionsTree {
   ...
   public bool init(int treeID = 0, int frameID = 0, int helpID = 0, _str file = "")
   {
      ...
      if (OptionsTree.init(treeID, frameID, helpID, export_file)) {
         ...
         if (!checkExportFileVersion(auto version)) {
            close();
            return false;
         }
         if (def_config_imports_from!='' && def_config_imports_ver==_version()) {
            if (!pos(' 'version' ',' 'def_config_imports_from' ')) {
               strappend(def_config_imports_from,' 'version);
            }
         } else {
            def_config_imports_from=version;
            def_config_imports_ver=_version();
         }
#if 0
    ...
    */
   private bool checkExportFileVersion(_str &version)
   {
