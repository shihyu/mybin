...
//#define MAX_SEARCH_ADD_COUNT2 100
   ...
   private bool isNodeCurrentlyVisible(int index, bool checkParents = false)
   {
      ...
      if (m_included:[index] == null) {
         ...
         if (isATemplate(index)) {
            ...
            if (tempD != "") {
               //say("isNodeCurrentlyVisible H"__LINE__": DialogTemplate="tempD);
            ...
            if (tempD != "") {
               //say("isNodeCurrentlyVisible H"__LINE__": PropertyTemplate="tempD);
   ...
   private void readExportGroupRanges(int groupIndex, ExportImportGroup &group)
   {
      ...
      while (rangeIndex > 0) {
         ...
         includeNodes := _xmlcfg_get_attribute(m_xmlExportHandle, rangeIndex, "Include");

         allPaths:[startPath] = startPath;
         if (endPath != "") {
             allPaths:[endPath] = endPath;
             allRanges :+= startPath","endPath","includeNodes;
         } else {
             allRanges :+= startPath",,"includeNodes;
         }
      ...
      foreach (range in allRanges) {
          _str startPath, endPath, includeNodes;
          parse range with startPath","endPath","includeNodes;
          ...
          if (pathTable._indexin(startPath)) {
              ...
              if (endPath != "") {
                 ...
                 addGroupItem(startIndex, group, includeNodes);
              }
          }
      }
    ...
    *                      status)
    */
   private void addGroupItem(int index,
                             ...
                             _str includeNodes="")
   {
      ...
      if (includeNodes != "") {
         // if they specified nodes names to include, then include only those
         bool includeHash:[];
         name_list := includeNodes;
         while (name_list != "") {
            parse name_list with auto name ';' name_list;
            includeHash:[name] = true;
         }
         startIndex := getFirstChild(index);
         childIndex := startIndex;
         while (childIndex > 0) {
            // check if the child node matches the includes
            nodeName := getCaption(childIndex);
            if (includeHash._indexin(nodeName)) {
               // mark this node
               group.addGroupItem(defaultItem, childIndex, false);
            } else {
               addGroupItem(childIndex, group, includeNodes);
            }
            // next please
            childIndex = getNextSibling(childIndex);
            if (childIndex == startIndex) break;
         }

      } else {
         // just include this one node
         group.addGroupItem(defaultItem, index, false);
      }
   }
