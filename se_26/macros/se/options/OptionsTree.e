...
class OptionsTree {
   ...
   protected void handleChangeEvents(int changeEvents, _str (&commentLangs):[])
   {
      ...
      if (changeEvents & OCEF_RESET_LANGUAGE_FONTS_AND_COLORS) {
         _reset_language_fonts_and_colors();
      }

      // remove already used flags
      changeEvents &= ~(OCEF_MENU_BIND | OCEF_REINIT_SOCKET | OCEF_LOAD_USER_LEXER_FILE | OCEF_RESET_LANGUAGE_FONTS_AND_COLORS);
   ...
   /** 
    * @return 
    * Returns {@code true} if the given node or it's parent node is already 
    * selected in the options tree.  Otherwise, the current active top-level 
    * tree node is collapsed, so that the given node can be expanded.
    * 
    * @param xmlIndex   index of node that we want
    */
   public bool nodeHierarchyAlreadyActive(int xmlIndex)
   {
      ...
      if (treeIndex > 0) {
         parentIndex = m_treeHandle._TreeGetParentIndex(treeIndex);
      }

      currentIndex := m_currentTreeIndex;
      topIndex := currentIndex;
      while (currentIndex > 0) {
         // already on this langauge node 
         if (currentIndex == treeIndex || currentIndex == parentIndex) {
            return true;
         }
         topIndex = currentIndex;
         currentIndex = m_treeHandle._TreeGetParentIndex(currentIndex);
      }

      if (topIndex > 0) {
         m_treeHandle._TreeSetInfo(topIndex, 0);
      }
      return false;
   }
   ...
   public bool showLanguageNode(_str modeName, _str options = "", _str arguments = "", int errorBehavior = ISEB_RETURN)
   {
      ...
      if (index > 0) {
         ...
         // these are the same, so don't change!
         if (nodeHierarchyAlreadyActive(index)) {
            return true;
         }
