...
class MessageBrowser : se.lineinfo.LineInfoBrowser {
   ...
   public bool goToMessageCodeLocation(int index)
   {
      ...
      if (index >= m_messages._length()) return false;
      if (m_messages[index] == null) return false;
...
_command bool show_next_message() name_info(','VSARG2_READ_ONLY)
{
   form_wid := _tbGetActiveMessageListForm();
   if ((form_wid > 0) && _iswindow_valid(form_wid)) {
      if (form_wid.moveToNextMessage()) {
         form_wid.goToMessage();
         return true;
      }
   }
...
_command bool show_prev_message() name_info(','VSARG2_READ_ONLY)
{
   form_wid := _tbGetActiveMessageListForm();
   if ((form_wid > 0) && _iswindow_valid(form_wid)) {
      if (form_wid.moveToPrevMessage()) {
         form_wid.goToMessage();
         return true;
      }
   }
...
void _message_tree.'C-S-UP'()
{
   prev_error();
}

void _message_tree.'C-S-DOWN'()
{
   next_error();
}

void _message_tree.rbutton_up,context()
{
...
static bool moveToNextMessage()
{
   return (_message_tree._TreeDown() >= 0);
}

static bool moveToPrevMessage()
{
   return (_message_tree._TreeUp() >= 0);
}
