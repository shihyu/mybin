...
int parse_line(_str &filename, int &linenum, int &col)
{
   ...
   if (strieq(line,'file') || strieq(line,'directory')) {
      filename = rest;
      linenum = -1;
      restore_pos(p);
      return (0);
...
int _mffindGoTo(_str filename,int linenum,int pcol,bool maybePushBM=false, bool openNewWindow=false)
{
   ...
   if (last_char(filename)==FILESEP) {
      // Can't use OS open dialog for container files like .jar, .zip, etc.
      if(_FileQType(filename)==VSFILETYPE_NORMAL_FILE) {
         return gui_open('',filename);
      }
      smart_open(initial_directory: filename);
      return 0;
   }
