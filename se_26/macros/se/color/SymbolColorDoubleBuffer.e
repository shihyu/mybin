...
class SymbolColorDoubleBuffer {
    ...
    */
   public int createStreamMarker(int index, int markerType, int markerFlags=0)
   {
      if (index < m_symbolList._length()) {
         DoubleBufferInfo *psym = &m_symbolList[index];
         if (psym != null) {
            description := psym->m_description;
            if (description != null && psym->m_declaration) {
            ...
            if (streamMarkerId >= 0) {
               ...
               if (markerFlags != 0) {
                  _StreamMarkerSetStyleFlags(streamMarkerId, markerFlags);
               }
   ...
   /**
    * Sets whether this symbol is a function (inheriting CFG_FUNCTION).
    *
    * @param index
    * @param decl
    */
   public void setSymbolFunction(int index, bool decl)
   {
      if (index < m_symbolList._length()) {
         m_symbolList[index].m_function = decl;
      }
   }

   /** 
    * @return 
    * Returns whether this symbol is a function.
    *
    * @param index
    */
   public bool isSymbolFunction(int index)
   {
      if (index < m_symbolList._length()) {
         return m_symbolList[index].m_function;
      }
      return false;
   }
