...
class SymbolColorRuleBase : IColorCollection, sc.lang.IEquals {
    ...
    */
   _str m_compatibleColorProfiles[];
   ...
   SymbolColorRuleBase(_str name = "") {
      m_name = name;
      m_compatibleColorProfiles = null;
      m_ruleList = null;
   }
   ...
   /**
    * Add a color profile name as a compatible color scheme for this 
    ...
    */
   void addCompatibleColorProfile(_str name) {
      if (m_compatibleColorProfiles!=null && isCompatibleWithColorProfile(name)) return;
      m_compatibleColorProfiles :+= name;
   }
   /**
    * Replace the entire array of compatible color schemes. 
    * Use 'null' to remove all compatible color profile names. 
    *  
    * @param names   array of color profile names
    */
   void setCompatibleColorProfiles(_str names[]) {
      m_compatibleColorProfiles = names;
   }
    ...
    * @param name    color profile name 
    */
   void removeCompatibleColorProfile(_str name) {
      numSchemes := m_compatibleColorProfiles._length();
      for (i := numSchemes-1; i>=0; --i) {
         if (m_compatibleColorProfiles[i] == name) {
            m_compatibleColorProfiles._deleteel(i);
         }
      }
   }
    ...
    * @param name    color profile name 
    */
   bool isCompatibleWithColorProfile(_str name) {
      if (m_compatibleColorProfiles==null) return true;
      if (name == "") return true;
      foreach (auto profileName in m_compatibleColorProfiles) {
         if (profileName == SYMBOL_COLOR_COMPATIBLE_DARK && ColorScheme.isDarkColorProfile(name)) {
            return true;
         } else if (profileName == SYMBOL_COLOR_COMPATIBLE_LIGHT && !ColorScheme.isDarkColorProfile(name)) {
            return true;
    ...
    */
   _str getCompatibleColorProfiles() {
      if (m_compatibleColorProfiles == null) return "";
      return join(m_compatibleColorProfiles, ", ");
   }
    ...
    */
   _str initDefaultRuleBase(_str colorProfileName="") {
      window_color_profile_name := ColorScheme.realProfileName(colorProfileName);
      symbol_color_profile_name := "";
      if (ColorScheme.hasSymbolColoringRulesDefined(window_color_profile_name)) {
         symbol_color_profile_name = "All symbols";
      } else if (ColorScheme.isDarkColorProfile(window_color_profile_name)) {
   ...
   bool equals(sc.lang.IEquals &rhs) {
      ...
      if (pRHSRuleBase->m_compatibleColorProfiles != m_compatibleColorProfiles) {
         return false;
      }
      return true;
   }
    ...
    *  
    * @param symbol_color_profile_name   (optional) symbol coloring profile 
    * @param window_color_profile_name   (optoinal) current buffer color profile
    */
   static _str getDefaultSymbolColorProfile(_str symbol_color_profile_name="", _str window_color_profile_name="") {
      ...
      if (window_color_profile_name == "") window_color_profile_name = def_color_scheme;
      window_color_profile_name = ColorScheme.realProfileName(window_color_profile_name);
      listProfiles(auto profileNames, window_color_profile_name);
      ...
      profile_name = _plugin_get_property(VSCFGPACKAGE_COLOR_PROFILES,window_color_profile_name,'associated_symbol_profile');
      if (profile_name != "" &&
          _plugin_has_profile(VSCFGPACKAGE_SYMBOLCOLORING_PROFILES,profile_name) &&
          hash_profile._indexin(lowcase(profile_name))) {
         compatibleProfile = profile_name;
      }
      ...
      if (compatibleProfile != "") {
         foreach (profile_name in gSCProfileList) {
            ...
            if (pos(window_color_profile_name, profile_name, 1, 'i')) {
               compatibleProfile = profile_name;
               break;
            }
         }
      ...
      if (compatibleProfile == "" && profileNames._length() > 0) {
         compatibleProfile = profileNames[0];
      }

      // special case for light background or dark background
      if (compatibleProfile == se.color.SYMBOL_COLOR_COMPATIBLE_DARK) {
         if (ColorScheme.hasSymbolColoringRulesDefined(window_color_profile_name)) {
            symbol_color_profile_name = "All symbols";
         }
         compatibleProfile = "All symbols - Dark background";
      } else if (compatibleProfile == se.color.SYMBOL_COLOR_COMPATIBLE_LIGHT) {
         if (ColorScheme.hasSymbolColoringRulesDefined(window_color_profile_name)) {
            symbol_color_profile_name = "All symbols";
         }
         compatibleProfile = "All symbols - Light background";
      }
      ...
      if (_plugin_has_profile(VSCFGPACKAGE_SYMBOLCOLORING_PROFILES,compatibleProfile) && 
          hash_profile._indexin(lowcase(compatibleProfile))) {
   ...
   public static void listProfiles(_str (&profileNames)[],_str colorProfileName='') { 
      ...
      foreach (auto name in allProfileNames) {
         ...
         if (profileCompatibleWith(name,colorProfileName)) {
            profileNames :+= name;
         }
      }
   ...
   static _str tagFlagsToString(SETagFlags tag_flags) {
      ...
      if (tag_flags & SE_TAG_FLAG_IN_CONTEXT   ) s :+= "in_context|";
      if (tag_flags & SE_TAG_FLAG_NESTED_LOCAL ) s :+= "nested|";
      if (tag_flags & SE_TAG_FLAG_ASYNCHRONOUS ) s :+= "async|";
      if (tag_flags & SE_TAG_FLAG_FACTORY      ) s :+= "factory|";
      if (tag_flags & SE_TAG_FLAG_LATE         ) s :+= "late|";
      _maybe_strip(s, '|');
      return s;
   }
   /** 
   ...
   static SETagFlags parseTagFlags(_str s) {
      ...
      foreach (auto flag in flag_names) {
         ...
         case "in_context":   tag_flags |= SE_TAG_FLAG_IN_CONTEXT; break;      
         case "nested":       tag_flags |= SE_TAG_FLAG_NESTED_LOCAL; break;      
         case "async":        tag_flags |= SE_TAG_FLAG_ASYNCHRONOUS; break;      
         case "factory":      tag_flags |= SE_TAG_FLAG_FACTORY; break;      
         case "late":         tag_flags |= SE_TAG_FLAG_LATE; break;    
         }
      }
      return tag_flags;
   }
   ...
   static _str colorIndexToString(_str color) {
      if (isinteger(color)) {
         cfg_name := ColorScheme.getColorConstantName((int)color);
         if (cfg_name != null && cfg_name != "") {
            return "*" :+ cfg_name :+ "*";
         }
      }
      return color;   
   ...
   static _str parseColorName(_str s) {
      ...
      if (_first_char(s) == '*') {
         parse s with '*' auto cfg_name '*';
         cfg := ColorScheme.getCFGConstantByName(cfg_name);
         if (cfg > 0) {
            return cfg;
         }
      }
   ...
   private SymbolColorRule  loadOneRule(int handle, int property_node,_str ruleName,int (&hash_position):[]) {
      ...
      fg := se.color.ColorInfo.parseColorValue(_xmlcfg_get_attribute(handle, attrs_node, "fg"));
      if (!isValid) fg=0x000000;
      color.m_foreground = fg;
      bg := se.color.ColorInfo.parseColorValue(_xmlcfg_get_attribute(handle, attrs_node, "bg"));
   ...
   int loadProfile(_str profileName,int optionLevel=0,int (&hash_position):[]=null) {
      ...
      m_compatibleColorProfiles._makeempty();
      ...
      //compatibleProfiles := _xmlcfg_get_attribute(xmlcfgHandle,  schemeNode, "compatibleWith");
      //if (compatibleProfiles == null) compatibleProfiles="";
      //setCompatibleColorProfiles(split2array(compatibleProfiles, ";"));
      ...
      while (property_node>=0) {
         ...
         if (substr(ruleName,1,2)=='__') {
            if (ruleName=='__compatible_with') {
               compatibleProfiles := _xmlcfg_get_attribute(handle,  property_node, VSXMLCFG_PROPERTY_VALUE);
               if (compatibleProfiles == null) compatibleProfiles="";
               setCompatibleColorProfiles(split2array(compatibleProfiles, ";"));
            }
         } else {
   ...
   private static void addRuleToXml(int handle,int profile_node,se.color.SymbolColorRule &rule,int &last_position,int (&hash_position):[]) {
      ...
      if (rule.m_colorInfo.m_foreground >= 0) {
         _xmlcfg_set_attribute(handle,attrs_node,'fg',ColorInfo.formatHTMLHexColor(rule.m_colorInfo.m_foreground));
      }
      if (rule.m_colorInfo.m_background >= 0) {
         _xmlcfg_set_attribute(handle,attrs_node,'bg',ColorInfo.formatHTMLHexColor(rule.m_colorInfo.m_background));
      }
      _xmlcfg_set_attribute(handle,attrs_node,'font_flags',ColorInfo.fontFlagsToString(rule.m_colorInfo.m_fontFlags));
   ...
   void saveProfile() {
      ...
      _xmlcfg_add_property(handle, profile_node, "__compatible_with", join(m_compatibleColorProfiles, ";"));
