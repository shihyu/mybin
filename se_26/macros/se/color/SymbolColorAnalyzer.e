...
class SymbolColorAnalyzer : sc.lang.IAssignTo {
   ...
   /**
    * Stream marker flags used for highlighting function calls
    */
   private int m_streamMarkerFunFlags;
   /**
    * Set to VSMARKERTYPEFLAG_FONT_BOLD if the options to color definitions 
    * as bold is enabled.
    */
   private int m_streamMarkerDefFlags;
   ...
   /**
    * Indicates if highlighting of library symbols is enabled or disabled 
    * for this file. 
    */
   private bool m_isOverrideEnabled;
   ...
   SymbolColorAnalyzer(SymbolColorRuleBase ruleBase=null) {
      ...
      m_streamMarkerFunFlags = -1;
      m_streamMarkerDefFlags = -1;
      m_numIterations = 0;
      m_ruleIndex = null;
      m_isEnabled = true;
      m_isErrorsEnabled = false;
      m_isOverrideEnabled = true;
      m_doubleBuffer = null;
      m_symbolTable  = null;
      if (ruleBase != null) {
   ...
   /**
    * @return
    * Return 'true' if overriding of library symbols is enabled for this file.
    */
   bool isOverrideColoringEnabled() {
      return m_isOverrideEnabled;
   }

   /**
    * Turn overriding of library symbols on or off for the current file.
    */
   void enableOverrideColoring(bool onOff) {
      m_isOverrideEnabled = onOff;
   }
    ...
    */
   static void initAllSymbolAnalyzers(bool resetViewOptions=false) {
      if (_no_child_windows()) {
         return;
      }

      _str symbolColoringForProfile:[];
      SymbolColorRuleBase ruleBases:[];
      ...
      for (;;) {
         ...
         // determine the color scheme for this buffer
         colorProfileName := p_BufferColorProfile;
         if (colorProfileName == "") colorProfileName = def_color_scheme;
         colorProfileName = ColorScheme.realProfileName(colorProfileName);

         // determine the symbol coloring rule base for this buffer
         symbolColoringScheme := def_symbol_color_profile;
         if (analyzer != null && (symbolColoringScheme == "" || symbolColoringScheme == CONFIG_AUTOMATIC)) {
            // now determine the symbol coloring scheme for the color scheme
            if (symbolColoringForProfile._indexin(colorProfileName)) {
               symbolColoringScheme = symbolColoringForProfile:[colorProfileName];
            } else {
               symbolColoringScheme = SymbolColorRuleBase.getDefaultSymbolColorProfile(symbolColoringScheme, colorProfileName);
               symbolColoringForProfile:[colorProfileName] = symbolColoringScheme;
            }
         }

         // now find the rule base for this buffer, 
         // either already loaded, or load a new one.
         SymbolColorRuleBase *pRuleBase = null;
         if (analyzer != null) {
            if (ruleBases._indexin(symbolColoringScheme)) {
               pRuleBase = ruleBases._indexin(symbolColoringScheme);
            } else if (symbolColoringScheme == "") {
               SymbolColorRuleBase default_rb;
               ruleBases:[symbolColoringScheme] = default_rb;
               pRuleBase = ruleBases._indexin(symbolColoringScheme);
               pRuleBase->initDefaultRuleBase(colorProfileName);
            } else {
               SymbolColorRuleBase rb;
               ruleBases:[symbolColoringScheme] = rb;
               pRuleBase = ruleBases._indexin(symbolColoringScheme);
               pRuleBase->loadProfile(symbolColoringScheme);
            }
         }

         // initialize the analyzer with this rule base
         if (analyzer != null && pRuleBase != null) {
            ...
            if (resetViewOptions) {
               ...
               analyzer->enableOverrideColoring(_QSymbolColoringOverrides(true));
            }
         }
         _next_buffer('hr');
   ...
   /**
    * Get the stream marker type.  Allocate one if we don't already have one. 
    * Also sets up marker flags for definitions and functions. 
    */
   int getStreamMarkerType() {
      // allocate the stream marker type if it is not already allocated
      if (m_streamMarkerType <= 0) {
         m_streamMarkerType = _MarkerTypeAlloc();
         _MarkerTypeSetPriority(m_streamMarkerType, VSMARKER_TYPE_PRIORITY_SYMBOL_COLORING);
         _MarkerTypeSetFlags(m_streamMarkerType, VSMARKERTYPEFLAG_SYMBOL_COLORING);
      }
      // calculate additional font flags for symbol definitions
      if (m_streamMarkerDefFlags < 0) {
         m_streamMarkerDefFlags = 0;
         if (_GetSymbolColoringOptions() & SYMBOL_COLOR_BOLD_DEFINITIONS) {
            m_streamMarkerDefFlags = VSMARKERTYPEFLAG_FONT_BOLD;
         }
      }
      // calculate additional font flags for function names
      if (m_streamMarkerFunFlags < 0) {
         // get the current buffer color scheme
         colorProfileName := null;
         if (_isEditorCtl()) {
            colorProfileName = ColorScheme.realProfileName(p_BufferColorProfile);
         }
         // get the font flags for CFG_FUNCTION
         ColorInfo cfgFunction;
         cfgFunction.getColor(CFG_FUNCTION, colorProfileName);
         cfgFontFlags := cfgFunction.getFontFlags(cc:null, colorProfileName);
         // now calculate the marker font flags
         m_streamMarkerFunFlags = 0;
         if (cfgFontFlags & F_BOLD) {
            m_streamMarkerFunFlags |= VSMARKERTYPEFLAG_FONT_BOLD;
         }
         if (cfgFontFlags & F_ITALIC) {
            m_streamMarkerFunFlags |= VSMARKERTYPEFLAG_FONT_ITALIC;
         }
         if (cfgFontFlags & F_UNDERLINE) {
            m_streamMarkerFunFlags |= VSMARKERTYPEFLAG_FONT_UNDERLINE;
         }
      }
      // that's all folks
   ...
   /**
    * Get the list of symbol coloring markers at the given position. 
    * This will try all three marker types. 
    * 
    * @param markerIdList  (output) array of stream mark IDs
    * @param editor_wid    editor control window ID
    * @param offset        offset to find stream markers at
    */
   void getSymbolColorMarkers(int (&markerIdList)[], int editor_wid, long offset) {
      _StreamMarkerFindList(markerIdList, editor_wid, offset, 1, VSNULLSEEK, getStreamMarkerType());
   }
   ...
   SymbolColorRule *getSymbolColorUnderCursor() 
   {
      ...
      getSymbolColorMarkers(auto markerIdList, p_window_id, _QROffset());
   ...
   /** 
    * @return 
    * Return {@code true} if this line contains preprocessing, 
                                  ...
                                  bool &doOffScreen)
   {
      ...
      // make sure colored lines object is initialized
      if (m_coloredLines == null) {
         LineNumberRanges emptySet;
         m_coloredLines = emptySet;
      }

      // calculate the range of lines to color
      return m_coloredLines.determineLineRangeToColor(p_window_id, 
                                                      startLine, endLine, 
                                                      doRefresh, doMinimap, doOffScreen, 
                                                      def_symbol_color_chunk_size, 
                                                      def_symbol_color_off_page_lines);
   }
   ...
   int colorLines(int startLine=0, int endLine=0)
   {
      ...
      while (!status) {
         ...
         if (stage < 0) stage = DBS_GET_SYMBOLS;
         switch (stage) {
   ...
   private int getSymbolsStep(int startLine=0, int endLine=0, long seekPos = -1)
   {
      ...
      // If the language has a list-locals callback, allow it to override
      // the coloring of Library Symbols and User-defined keywords
      libraryExclusions := '234';
      langListLocalsIndex := _FindLanguageCallbackIndex("%s_list_locals");
      if (langListLocalsIndex > 0 && _QSymbolColoringOverrides()) {
         libraryExclusions = '3';
      }
      ...
      scope := 0;
      search_re   := id_re:+"|\\{|\\}";
      search_opts := "@rmXknscp"libraryExclusions"lvax";
      status := search(search_re, search_opts);
      timeout := false;
      while (!status) {
   ...
   int matchColorsStep(int arrayIndex = -1)
   {
      ...
      for (j := arrayIndex; j < numSymbols; j++) {
         ...
         // was this symbol colored as a function originally?
         if (_clex_find(0, 'g') == CFG_FUNCTION) {
            m_doubleBuffer.setSymbolFunction(j, true);
         }
         ...
         if (psym != null) {
            ...
            cfg := _clex_find(0, 'g');
            if (cfg!=CFG_KEYWORD && cfg!=CFG_PUNCTUATION && cfg!=CFG_OPERATOR) {
               ...
               if (status < 0 && status!=VSCODEHELPRC_BUILTIN_TYPE) {
                  ...
                  if ( useSymbolNotFoundColoring ) {
                     // do not color library symbols, punctuation, operators, or user-defined symbols as symbol-not found.
                     if (cfg != CFG_LIBRARY_SYMBOL && cfg != CFG_OPERATOR && cfg != CFG_PUNCTUATION && cfg != CFG_USER_DEFINED) {
                        psym = &errorSym;
   ...
   int colorLinesStep(int arrayIndex = -1)
   {
      ...
      // get the font flags for CFG_FUNCTION
      buffercolorProfileName := p_BufferColorProfile;
      minimapcolorProfileName := p_MinimapColorProfile;
      ColorInfo cfgFunction;
      cfgFunction.getColor(CFG_FUNCTION, buffercolorProfileName);
      functionFontFlags := (cfgFunction.getFontFlags(cc:null, buffercolorProfileName) & (F_BOLD|F_ITALIC|F_UNDERLINE));
      ...
      for (j := arrayIndex; j < numSymbols; j++) {
         ...
         if (ruleInfoP != null/* && psym != null*/) {
            ...
            int colorId = CFG_WINDOW_TEXT;
            if (ruleInfoP->m_colorInfo.inheritsEverything()) {
               colorId = ruleInfoP->m_colorInfo.getParentColorIndex();
            } else {
               colorId = m_ruleIndex->allocateColorId(*ruleInfoP, buffercolorProfileName);
            }
   ...
   int createMarkersStep(bool reset=false)
   {
      ...
      for (j := 0; j <= numSymbols; j++) {
         ...
         markerFlags := 0;
         if (m_doubleBuffer.isSymbolDeclaration(j)) {
            markerFlags = m_streamMarkerDefFlags;
         } else if (m_doubleBuffer.isSymbolFunction(j)) {
            markerFlags = m_streamMarkerFunFlags;
         }
         m_doubleBuffer.createStreamMarker(j, markerType, markerFlags);
      }
   ...
   void copy(sc.lang.IAssignTo &dest) {
      if (dest instanceof se.color.SymbolColorAnalyzer) {
         ...
         pdest->m_streamMarkerDefFlags = this.m_streamMarkerDefFlags;
         pdest->m_streamMarkerFunFlags = this.m_streamMarkerFunFlags;
         ...
         pdest->m_isOverrideEnabled = this.m_isOverrideEnabled;
...
_metadata enum_flags SYMBOL_COLOR_OPTIONS {
   ...
   SYMBOL_COLOR_NO_OVERRIDE_LIBRARY_SYMBOLS,
};
 ...
 * @categories Configuration_Variables
 */
int def_symbol_color_windows = VT_CURRENT_WINDOW;
...
bool _QSymbolColoringSupported(_str lang=null)
{
   ...
   if (lang == null && _isEditorCtl(/*allowHiddenWindow*/true)) {
      lang = p_LangId;
   }
   if (lang == null) {
      return false;
...
void _MaybeResetSymbolColoring()
{
   ...
   if (analyzer == null) {
      ...
      tmpAnalyzer.enableOverrideColoring(_QSymbolColoringOverrides(true));
...
static VisibleTabsCache gSymbolColoringVisibleTabs;

void _switchbuf_SymbolColoring(_str oldbuffname, _str flag)
{
   _ResetVisibleFileTabs(gSymbolColoringVisibleTabs);
}
...
{
   _UpdateSymbolColoringForAll(force, 
                               (def_symbol_color_windows == VT_CURRENT_WINDOW),
                               (def_symbol_color_windows != VT_ALL_WINDOWS));
}

static void _UpdateSymbolColoringForAll(bool force=false, bool doCurrentWindowOnly=false, bool doVisibleOnly=false)
{
   ...
   // update symbol coloring for all visible tabs
   _DoCallbackForAllVisibleTabs(gSymbolColoringVisibleTabs, 
                                _UpdateSymbolColoringForWindow, 
                                force, doCurrentWindowOnly, doVisibleOnly,
                                def_symbol_color_delay, 
                                def_symbol_color_timeout);
}
...
int _UpdateSymbolColoringForWindow(bool force=false, bool doMinimalWork=false, bool okToRefresh=false, int tab_index=0)
{
   ...
   if (!_QSymbolColoringSupported() || !_QSymbolColoringEnabled()) {
      ...
      se.color.SymbolColorAnalyzer *analyzer = _GetBufferInfoHtPtr(SM_BUFFER_INFO_KEY);
      if (analyzer != null) analyzer->resetSymbolColoring();
      _RemoveVisibleFileTab(gSymbolColoringVisibleTabs, p_window_id, tab_index);
      return 0;
   }
   ...
   if (!force && !_CheckUpdateContextSizeLimits(VS_UPDATEFLAG_tokens, quiet:true)) {
      if (_chdebug) {
...
int _OnUpdate_symbol_coloring_set_scheme(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   parse command with . auto profileName;
   ...
   if (analyzer != null) {
      ...
      if (ruleBase != null && profileName == ruleBase->m_name) {
         isActiveScheme = true;
      } else {
         checked = 0;
      }
   } else {
      if (def_symbol_color_profile == profileName) {
         isActiveScheme = true;
      }
   }
...
{
   profileName := _GetSymbolColoringSchemeName();

   // look up all the available symbol coloring scheme names
   //_str profileNames[];
   //_str compatibleWith[];
   //SymbolColorConfig.getSchemeNamesOnly(profileNames, compatibleWith, def_color_scheme);
   _str profileNames[];
   se.color.SymbolColorRuleBase scc;
   scc.listProfiles(profileNames, ColorScheme.getDefaultProfile());
   ...
   i = i + (doNext? 1:-1);
   if (i >= profileNames._length()) i=0;
   if (i < 0) i = profileNames._length()-1;

   if (profileNames[i] == profileName) {
      message("There is only one compatible scheme.");
      return;
   }
   message("Switching to '"profileNames[i]"' symbol coloring scheme");
   _SetSymbolColoringSchemeName(profileNames[i], true);
}
...
_command void _SetSymbolColoringSchemeName(_str name="", bool doUpdate=false) name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_NOEXIT_SCROLL)
{
   ...
   if ( !_isEditorCtl(/*allowHiddenWindow*/true)) {
   ...
   automaticProfileName := name;
   if (name=="" || name == CONFIG_AUTOMATIC) {
      // determine the color scheme for this buffer
      colorProfileName := target_wid.p_BufferColorProfile;
      if (colorProfileName == "") colorProfileName = def_color_scheme;
      colorProfileName = ColorScheme.realProfileName(colorProfileName);

      // get the default symbol coloring scheme for this window
      automaticProfileName = SymbolColorRuleBase.getDefaultSymbolColorProfile("", colorProfileName);
      if (automaticProfileName=="") return;
   }
   if (!_plugin_has_profile(VSCFGPACKAGE_SYMBOLCOLORING_PROFILES, automaticProfileName)) {
      return;
   }
   se.color.SymbolColorRuleBase rb;
   rb.loadProfile(name) ;
...
/**
 * @return 
 * Return whether symbol coloring error coloring is for the current file.
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods 
 */
...
/**
 * Enable or disable the symbol coloring show errors command for the current document. 
 * 
 * @param cmdui 
 * @param target_wid 
 * @param command 
...
/**
 * Modify the symbol coloring error coloring for the current file. 
 *  
 * @param onoff      Indicates if we are turning symbol coloring errors on or off 
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods 
 */
 ...
 *  
 * @param onoff      Indicates if we are turning symbol coloring errors on or off 
 * @param doUpdate   Update current document? 
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods 
 */
...
/**
 * @return 
 * Return whether symbol coloring overrides Library Symbols for the current file.
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods 
 */
bool _QSymbolColoringOverrides(bool ignorePerFileOption=false)
{
   // get the target editor control, usually _mdi.p_child
   target_wid := p_window_id;
   if ( !_isEditorCtl(/*allowHiddenWindow*/true)) {
   ...
   se.color.SymbolColorAnalyzer *analyzer = target_wid._GetBufferInfoHtPtr(SM_BUFFER_INFO_KEY);
   if (analyzer != null && !ignorePerFileOption) {
      return analyzer->isOverrideColoringEnabled();
   }

   // no rule base set for this file 
   return (target_wid._GetSymbolColoringOptions() & SYMBOL_COLOR_NO_OVERRIDE_LIBRARY_SYMBOLS) == 0;
}

/**
 * Enable or disable the symbol coloring override library symbols 
 * command for the current document. 
 * 
 * @param cmdui 
 * @param target_wid 
 * @param command 
 ...
 * @return int 
 */
int _OnUpdate_symbol_coloring_overrides_toggle(CMDUI &cmdui,int target_wid,_str command)
{
   // no context tagging?
   if (!_haveContextTagging()) {
      return(MF_GRAYED|MF_REQUIRES_PRO);
   ...
   grayed  := target_wid._QSymbolColoringEnabled()? MF_ENABLED:MF_GRAYED;
   checked := target_wid._QSymbolColoringOverrides()?  MF_CHECKED:MF_UNCHECKED;
   return MF_ENABLED|grayed|checked;
}

/**
 * Modify the symbol coloring library symbol coloring override for the current file. 
 * 
 * @param onoff      Indicates if we are turning symbol coloring overrides on or off 
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods 
 */
_command void symbol_coloring_overrides_toggle(_str onoff="") name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_NOEXIT_SCROLL|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveContextTagging()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Symbol coloring");
      return;
   ...
   if (onoff == "") {
      onoff = !_QSymbolColoringOverrides();
   }
   _SetSymbolColoringOverrides(onoff,true);
}
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods 
 */
void _SetSymbolColoringOverrides(_str onoff="", bool doUpdate=false)
{
   // get the target editor control, usually _mdi.p_child
   target_wid := p_window_id;
   if ( !_isEditorCtl(/*allowHiddenWindow*/true)) {
   ...
   if (analyzer == null) {
      se.color.SymbolColorAnalyzer tmpAnalyzer;
      tmpAnalyzer.enableOverrideColoring(target_wid._QSymbolColoringOverrides(true));
      target_wid._SetBufferInfoHt(SM_BUFFER_INFO_KEY, tmpAnalyzer);
      analyzer = _GetBufferInfoHtPtr(SM_BUFFER_INFO_KEY);
      if (analyzer == null) {
         return;
   ...
   // update the symbol coloring
   analyzer->enableOverrideColoring(onoff != 0);
   if (onoff != 0) {
      analyzer->enableSymbolColoring(true);
   }
   if (doUpdate) {
...
_str _GetSymbolColoringSchemeName()
{
   ...
   colorProfileName := def_color_scheme;
   if (target_wid._isEditorCtl()) {
      ...
      if (analyzer != null) {
         ...
         if (rb) {
            automaticProfileName := se.color.SymbolColorRuleBase.getDefaultSymbolColorProfile(CONFIG_AUTOMATIC, colorProfileName);
            if (rb->m_name != automaticProfileName) {
               return rb->m_name;
            }
...
void _init_menu_symbol_coloring(int menu_handle, int no_child_windows)
{
   ...
   if (_menu_find(subMenuHandle, "config Symbol Coloring", itemMenuHandle, itemPosition, "M")) {
      status = _menu_insert(subMenuHandle, insertedPosition, MF_ENABLED, "Customize...", 
                            "config Symbol Coloring", "", 
                            "help Symbol Coloring dialog", 
                            "Configure symbol coloring rules");
   }

   // add the link to the language specific Symbol Coloring options
   insertedPosition++;
   if (_menu_find(subMenuHandle, "setupext -appearance", itemMenuHandle, itemPosition, "M")) {
      status = _menu_insert(subMenuHandle, insertedPosition, MF_ENABLED, "Language &Appearance Options...", 
                            "setupext -appearance", "", 
                            "help Appearance Options (Language-Specific)", 
                            "Configure symbol coloring and appearance options for current languagerules");
   }
   ...
   if (_menu_find(subMenuHandle, "symbol_coloring_toggle", itemMenuHandle, itemPosition, "M")) {
      status = _menu_insert(subMenuHandle, insertedPosition, MF_ENABLED, 
   ...
   // add the link to turn on/off Symbol Coloring errors for the current file
   insertedPosition++;
   ...
   // add the link to turn on/off Symbol Coloring override library symbols for the current file
   insertedPosition++;
   if (_menu_find(subMenuHandle, "symbol_coloring_overrides_toggle", itemMenuHandle, itemPosition, "M")) {
      status = _menu_insert(subMenuHandle, insertedPosition, MF_ENABLED, 
                            "Override Library Symbols", 
                            "symbol_coloring_overrides_toggle", "", 
                            "help Symbol Coloring dialog", 
                            "Enable/disable symbol coloring for symbols colored as Library Symbols or User Defined Keywords");
   }

   // add a separate between the options and the symbol coloring profile names
   insertedPosition++;
   dashPosition := insertedPosition;
   if (_menu_find(subMenuHandle, "-", itemMenuHandle, itemPosition, "C")) {
      status = _menu_insert(subMenuHandle, insertedPosition, MF_ENABLED, "-", "", "-"); 
   }

   // get the current symbol coloring scheme name
   currentProfileName := def_symbol_color_profile;
   target_wid := p_window_id;
   ...
   if (target_wid._isEditorCtl()) {
      ...
      if (analyzer != null) {
         se.color.SymbolColorRuleBase *ruleBase = analyzer->getRuleBase();
         if (ruleBase != null) {
            currentProfileName = ruleBase->m_name;
         }
      }
   }
   ...
   // look up all the available symbol coloring scheme names
   _str profileNames[];
   SymbolColorRuleBase.listProfiles(profileNames, colorProfileName);
   ...
   foreach (name in profileNames) {
      ...
      if (name == CONFIG_AUTOMATIC) {
         automaticProfileName := se.color.SymbolColorRuleBase.getDefaultSymbolColorProfile(CONFIG_AUTOMATIC, colorProfileName);
         displayName = name :+ " (" :+ automaticProfileName :+ ")";
      }
      foundProfiles:[name] = true;
      if (_menu_find(subMenuHandle, name, itemMenuHandle, itemPosition, "C")) {
         status = _menu_insert(subMenuHandle, ++insertedPosition,
   ...
   for (itemPosition = _menu_info(subMenuHandle)-1; itemPosition > dashPosition; itemPosition--) {
      _menu_get_state(subMenuHandle, itemPosition, mf_flags, "p", caption,
                      auto dummyHandle = 0, categories, helpCommand, helpMessage);
      if (!foundProfiles._indexin(categories)) {
...
void _UpdatePositionalKeywordColoring(bool force=false)
{
   ...
   if (keyword_type == null) {
      ...
      _MarkerTypeSetPriority(keyword_type, VSMARKER_TYPE_PRIORITY_KEYWORDS);
