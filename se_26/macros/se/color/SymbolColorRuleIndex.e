...
class SymbolColorRuleIndex {
    ...
    */
   int allocateColorId(SymbolColorRule &rule, _str colorProfileName=null) {
      colorNameKey := rule.m_ruleName;
      if (m_colorIdHash != null && m_colorIdHash._indexin(colorNameKey)) {
         return m_colorIdHash:[colorNameKey];
      }
      ...
      colorId := tmpColorInfo.allocateColorId(&m_scheme, colorProfileName);
      m_colorIdHash:[colorNameKey] = colorId;
      return colorId;  
   }
    ...
    */
   int allocateStyledColorId(SymbolColorRule &rule, int fontFlag, _str colorProfileName=null, int cfgParent=0) {
      if (m_scheme==null) return CFG_WINDOW_TEXT;
      if (rule.m_colorInfo == null) return CFG_WINDOW_TEXT;

      origFontFlags := rule.m_colorInfo.getFontFlags(&m_scheme, colorProfileName);
      if ((origFontFlags & fontFlag) == fontFlag) {
         return allocateColorId(rule, colorProfileName);
      }
      colorNameKey := rule.m_ruleName"("fontFlag")";
      if (m_colorIdHash != null && m_colorIdHash._indexin(colorNameKey)) {
         return m_colorIdHash:[colorNameKey];
      }
      ...
      if (cfgParent && tmpColorInfo.m_parentName == null || tmpColorInfo.m_parentName == "" || tmpColorInfo.m_parentName == 0) {
         tmpColorInfo.m_parentName = cfgParent;
         tmpColorInfo.m_fontFlags |= F_INHERIT_STYLE;
      }
      colorId := tmpColorInfo.allocateColorId(&m_scheme, colorProfileName);
      m_colorIdHash:[colorNameKey] = colorId;
      return colorId;  
