...
#import "se/vc/VersionControlUtil.e"
#import "cvsutil.e"
#import "diff.e"
#import "dir.e"
#import "main.e"
...
class CVS : IVersionControl {
   ...
   private int cvsGetEntriesFileInfo(_str filename,_str &version,_str &timestamp='',_str &options='',_str &tagname='');
   ...
   int diffLocalFile(_str localFilename,_str version="",int options=0,bool modal=false) {
      ...
      if ( version!="" ) {
         ...
         cvsGetEntriesFileInfo(localFilename, auto curVerison="", auto timeStamp, auto entriesInfoOptions, auto tagname);
         version = curVerison;
      }
      origDir := getcwd();
      ...
      cmdLine := modalOption' -bi2 -r2 -matchMode2 -file2title 'fileTitle' '_maybe_quote_filename(localFilename)' 'originalFileWID.p_buf_id;
      diff(cmdLine);
...
//         _message_box(get_message(FILE_NOT_FOUND_RC,localPath));
   ...
   int commitFile(_str localFilename,_str comment=null,int options=0) {
      // call deferredInit() in commitFiles()
      _str localFilenames[];
