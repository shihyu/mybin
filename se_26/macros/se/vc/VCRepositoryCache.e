...
class VCRepositoryCache {
   ...
   */
   ...
   public int updateVersionCache(bool asyncUpdate)
   {
      ...
      dsay('updateVersionCache '__LINE__'  command='command);
      shellOptions := "PQ";
      ...
      dsay('updateVersionCache '__LINE__'  shellOptions='shellOptions' command='command);
      // do not run database
//      command :+= ' 1';
      // now run it and wait
      pid := 0;
      ...
      dsay('updateVersionCache '__LINE__'  status='status);
