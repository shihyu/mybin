...
class Perforce : IVersionControl {
   ...
   Perforce() {
      ...
      pVCUtil := getVCUtil();
      pVCUtil->setCaptionForCommand(SVC_COMMAND_COMMIT, "&Submit");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_EDIT, "Check &Out");
...
//      }
   ...
   int updateFiles(_str (&localFilenames)[],int options=0) {
      ...
      pVCUtil := getVCUtil();
      if ( !pVCUtil ) return(VSRC_SVC_COULD_NOT_GET_VC_INTERFACE_RC);
      commandCaption := pVCUtil->getCaptionForCommand(SVC_COMMAND_UPDATE,false,false);
      origPath := getcwd();
