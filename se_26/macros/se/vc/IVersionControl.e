...
interface IVersionControl {
   ...
   */
   ...
   int getAnnotatedBuffer(_str localFilename,_str version,_str options,int &annotatedBufWID);
   ...
   /**
    * For systems that have version numbers that are not meaningful 
    * to humans, we us the revision caption returned in the history 
    * information, which is generally the first line of the 
    * comment.  If there is an acceptable short length of the 
    * version number (most Git commands will accept 7 characters), 
    * we will use <B>minimumVersionLength()</B> characters of the 
    * version number, then a colon, then the caption.  This is so 
    * that users looking for a version number do not have to scroll
    * through the captions looking for the version number in the 
    * information on the bottom pane. 
    * 
    * @return int Mininum length to show/use of a system's version 
    *         number.  -1 if there is not one.
    */
