...
#require "se/vc/VersionControlUtil.e"
#import "se/vc/QueuedVCCommand.e"
#import "se/vc/QueuedVCCommandManager.e"
#import "se/vc/SubversionBuildFile.e"
...
class Git : IVersionControl {
   ...
   Git() {
      pVCUtil := getVCUtil();
      if ( !pVCUtil ) return;
      pVCUtil->setCaptionForCommand(SVC_COMMAND_COMMIT, "&Commit");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_EDIT, "&Lock");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_DIFF, "&Diff");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_HISTORY, "&History");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_MERGE, "&Merge");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_REVERT, "&Revert");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_UPDATE, "&Update");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_ADD, "&Add");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_REMOVE, "Remove");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_CHECKOUT, "Clone");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_BROWSE_REPOSITORY, "Browse repository");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_PUSH_TO_REPOSITORY, "Push to repository");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_PULL_FROM_REPOSITORY, "Pull from repository");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_HISTORY_DIFF, "History diff");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_SYMBOL_QUERY, "Find symbol changes");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_ANNOTATE, "&Blame");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_STASH, "Stash");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_STASH_POP, "Stash &Pop");
      pVCUtil->setCaptionForCommand(SVC_COMMAND_SWITCH, "Checkou&t");
   }
   ...
   int diffLocalFile(_str localFilename,_str version="",int options=0, bool modal=false) {
      ...
      if ( remoteFilename=="" ) {
         ...
         getCurLocalRevision(localFilename,auto curRevision="");

         ver1Str := "";
         ver2Str := "";
         if ( version=="" ) {
            // still local the same version
            version = curRevision;
         }
         if ( version != "" ) {
            ver1Str = " (Local - ":+substr(curRevision,1,GIT_SHORT_HASH_LENGTH):+')';
            ver2Str = " (Remote - ":+substr(version,1,GIT_SHORT_HASH_LENGTH):+')';
         }

         // Have to be careful here.  If the title is just a filename and we put
         // a space at the end of it thinking there may be a version string, and
         // there is not, it can cause trouble later
         if ( ver1Str=="" && ver2Str=="" ) {
            diff(modalOption' -viewid2 -r2 -internalCloseBuffer2 -file1title "'localFilename'" -file2title "'remoteFilename'"' _maybe_quote_filename(localFilename)' 'originalFileWID);
         } else if ( ver1Str!="" && ver2Str=="" ) {
            diff(modalOption' -viewid2 -r2 -internalCloseBuffer2 -file1title "'localFilename' 'ver1Str'" -file2title "'remoteFilename'"' _maybe_quote_filename(localFilename)' 'originalFileWID);
         } else if ( ver1Str=="" && ver2Str!="" ) {
            diff(modalOption' -viewid2 -r2 -internalCloseBuffer2 -file1title "'localFilename'" -file2title "'remoteFilename:+ver2Str'"' _maybe_quote_filename(localFilename)' 'originalFileWID);
         } else {
            // We have ver1Str and ver1Str
            diff(modalOption' -viewid2 -r2 -internalCloseBuffer2 -file1title "'localFilename:+ver1Str'" -file2title "'remoteFilename:+ver2Str'"' _maybe_quote_filename(localFilename)' 'originalFileWID);
         }
      }
   ...
   int getFile(_str localFilename,_str version,int &fileWID,bool getIndexVersion=false,bool setEditorLanguage=true) {
      fileWID = 0;
      status := getExeStr(auto exeStr);
      if ( status ) {
         _message_box(get_message(status,exeStr));
...
//         _message_box(get_message(status,localFilename,stdErrData));
      ...
      // Give this a document name so that if it gets leaked, we can 
      // tell by running "list-buffers -h"
      fileWID.p_DocumentName = "remote/hidden:":+gitFilename;
   ...
   int getFileStatus(_str localFilename,SVCFileStatus &fileStatus,int options=0,bool checkForUpdates=true) {
      ...
      origPath := getcwd();
      status = chdir(_file_path(localFilename),1);
      rootPath := localRootPath();
      chdir(_file_path(rootPath),1);
      if (rootPath=="") {
   ...
   int updateFile(_str localFilename,int options=0) {
      // Call deferredInit() in updateFiles()
   ...
   int revertFile(_str localFilename,int options=0) {
      // Call deferredInit() in revertFiles()
      STRARRAY tempFilenames;
      tempFilenames[0] = localFilename;
...
#if 0 //17:05pm 11/27/2021
         relCurFilename := relative(curFilename);
         if ( relCurFilename=="" ) {
            // This has to be the whole directory.  It had to be added to get
            // this far
            continue;
         }
#endif
   ...
   SVCCommandsAvailable commandsAvailable() {
      return SVC_COMMAND_AVAILABLE_COMMIT|
         ...
         SVC_COMMAND_AVAILABLE_GET_ANNOTATIONS;
   }

   int getAnnotatedBuffer(_str filename,_str version,_str options,int &annotatedBufWID) {
      status := getExeStr(auto exeStr);
      if ( status ) {
         _message_box(get_message(status,exeStr));
         return status;
      ...
      logOutputWID := 0;
      cmdLine := _maybe_quote_filename(exeStr)" blame "_maybe_quote_filename(filename);

      origPath := getcwd();
      chdir(_file_path(filename),1);
      status = gitRunCommand(cmdLine,logOutputWID,auto stdErrData);
      chdir(origPath,1);
      maybeOutputWIDToLog(logOutputWID,"getRepositoryInformation stdout");
      maybeOutputStringToLog(stdErrData,"getRepositoryInformation stderr");
      if ( status || (length(stdErrData)>1 && logOutputWID.p_Noflines==0) ) {
         _message_box(get_message(VSRC_SVC_COULD_NOT_GET_ANNOTATION_INFO,filename,stdErrData));
         SVCWriteToOutputWindow(stdErrData.get());
         return VSRC_SVC_COULD_NOT_GET_HISTORY_INFO;
      }
      annotatedBufWID = logOutputWID;
   ...
   int editFile(_str localFilename,int options=0) {
      // Call deferredInit() in editFiles
      STRARRAY tempFilenames;
      tempFilenames[0] = localFilename;
   ...
   int removeFile(_str localFilename,_str comment=null,int options=0) {
      status := 0;
      // Call deferredInit() in removeFiles()
      STRARRAY tempFilenames;
      tempFilenames[0] = localFilename;
...
//         _message_box(get_message(FILE_NOT_FOUND_RC,localPath));
   ...
   bool hotkeyUsed(_str hotkeyLetter,bool onMenu=true) {
      return getVCUtil()->hotkeyUsed(hotkeyLetter,onMenu);
   }

   _str getCaptionForCommand(SVCCommands command,bool withHotkey=true,bool mixedCaseCaption=true) {
      return getVCUtil()->getCaptionForCommand(command,withHotkey,mixedCaseCaption);
   }
   ...
   int stash(_str path="",SVCStashFlags flags=0,STRARRAY &listOfStashes=null) {
     ...
     listStashes := flags&SVC_STASH_LIST;
      ...
      p_window_id = origWID;
      if ( fileList!="" ) {
         maybePopCaption := pop ? "pop":"";
         _message_box(nls("Git stash %s caused the following conflicts:\n\n%s",maybePopCaption, fileList));
      }
      SVCWriteToOutputWindow(stdErrData.get());
      _delete_temp_view(logOutputWID);
      return 0;
   ...
   int getMinimumVersionNumberLength() {
      return GIT_SHORT_HASH_LENGTH;
   }
};
