 ...
 */
_command int windbg_write_dumpfile(_str cmdline="") name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
   ...
   while ( cmdline != "" ) {
      parse cmdline with auto option cmdline;
      if ( lowcase(option) == "-small" || lowcase(option) == '-s' ) {
         small_without_heap = true;
      } else if ( lowcase(option) == "-full" || lowcase(option) == '-f' ) {
         full_with_heap = true;
      } else if (get_extension(option) == "dmp") {
         filename = option;
      } else {
   ...
   if (session_id > 0 && dbg_get_callback_name(session_id) == "windbg") {
      if (filename == "") {
         status := textBoxDialog("Write Dump File",
                                 0,
                                 0,
                                 "",
                                 "Write &Mini Dump (no heap),Write &Full Dump (with heap),Cancel:_cancel\tSpecify the file to write mini dump to.",
                                 "",
                                 "-bnf Dumpfile (*.dmp):");

         if (status < 0) return status;
         if (status == 1) small_without_heap=true;
         if (status == 2) full_with_heap=true;
         filename = _param1;
         if (filename == "") {
      ...
      int status = dbg_windbg_write_dumpfile(filename, full_with_heap, small_without_heap);
      if (status) {
...
{
   return _OnUpdate_vcproj_visual_studio_debug(cmdui, target_wid, command);
}
