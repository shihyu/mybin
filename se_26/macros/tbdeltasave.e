...
int ctlsave_as.lbutton_up()
{
   ...
   int curVersionViewId = getVersionInWID(filename, curVersion, status, readFromDisk);
...
{
   filename := actualFileName(_mdi.p_child.p_buf_name, _mdi.p_child.p_DocumentName);
   version := ctltree1._TreeGetCaption(ctltree1._TreeCurIndex(), 0);
   history_diff_machine(filename, version);
}
...
void ctltree1.C_A()
{
   // Don't do anyhting here.  Once we select 3 items, we deselect the first so
   // we can never select more than two items
}
...
static int removeMenuCaption(int menuHandle, _str caption)
{
   index := _menu_find_loaded_menu_caption(menuHandle,caption);
   if ( index >= 0 ) {
      status := _menu_delete(menuHandle,index);
      return status;
   }  else {
      return index;
   }
...
static void showContextMenu(_str menuName, int x=-1, int y=-1)
{
   ...
   twoFilesSelected := _TreeGetNumSelectedItems() == 2;
   if ( twoFilesSelected ) {
      removeMenuCaption(mh,"Run Diff on Selected Backup");
      removeMenuCaption(mh,"Save Selected Backup as...");
      removeMenuCaption(mh,"Open Selected Backup");
      removeMenuCaption(mh,"Revert to Selected Backup");
      removeMenuCaption(mh,"Add comments to Selected Backup");
   } else {
      removeMenuCaption(mh,"Run Diff on Selected Backups");
   }
...
void ctltree1.rbutton_up,context(int x=-1, int y=-1) {
...
_command int TBD_diffSelectedVersions() name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveBackupHistory()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_OR_STANDARD_EDITION, "Backup History");
      return VSRC_FEATURE_REQUIRES_PRO_OR_STANDARD_EDITION;
...
static int diffWithCurrent()
{
   ...
   if ( numItems>1 ) {
      ...
      _HistoryDiffBackupHistoryFile(filename,(int)versionNum,true,false);
   }else{
      filename := "";
      filename = actualFileName(_mdi.p_child.p_buf_name, _mdi.p_child.p_DocumentName);
      typeless currentVersionNumber = actualFileVersion(_mdi.p_child.p_buf_name, _mdi.p_child.p_DocumentName);
      ...
      do {
         ...
         int versionBViewId = getVersionInWID(filename, versionB, status);
         if (!versionBViewId) {
            break;
         }
         ...
         if (currentVersionNumber == "-1") {
            ...
            int versionAViewId = getVersionInWID(filename, currentVersionNumber, status);
            if (!versionAViewId) {
               break;
            }
...
static int diffAB()
{
   ...
   int versionAWID = getVersionInWID(filename, versionA, status);
   if (!versionAWID) {
      return(status);
   }
   int versionBWID = getVersionInWID(filename, versionB, status);
   if (!versionBWID) {
      return(status);
   }
   ...
   if ( (int)versionA < (int)versionB ) {
      _DiffModal('-r1 -r2 -viewid1 -viewid2 -file1title "'filename' (Version 'versionB')" -file2title "'filename' (Version 'versionA')" 'versionBWID' 'versionAWID);
   } else {
      _DiffModal('-r1 -r2 -viewid1 -viewid2 -file1title "'filename' (Version 'versionA')" -file2title "'filename' (Version 'versionB')" 'versionAWID' 'versionBWID);
   }
   p_window_id = beforeDiffWID;
   _SetDialogInfoHt("inDiffWithCurrent", 0, ctltree1);

   _delete_temp_view(versionAWID);
   _delete_temp_view(versionBWID);
...
static int getVersionInWID(_str filename, int version, int &status, bool forceReadFromDisk = false)
{
   newViewId := 0;
