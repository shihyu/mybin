...
bool def_auto_set_new_filename=true;
...
/**
 * Scope for processing visible windows.
 *  
 ...
 */
_metadata enum VISIBLE_EDITOR_WINDOWS_SCOPE {
   VT_CURRENT_WINDOW,
   VT_VISIBLE_WINDOWS,
   VT_ALL_WINDOWS,

   // for compatibilty (old symbol coloring code)
   SC_CURRENT_WINDOW = VT_CURRENT_WINDOW,
   SC_VISIBLE_WINDOWS = VT_VISIBLE_WINDOWS,
   SC_ALL_WINDOWS = VT_ALL_WINDOWS,
};
      ...
      if ( substr(word_nq,1,1)=='+' || substr(word_nq,1,1)=='-' ) {
         ...
         if ( letter=='T' || letter=='V' || letter=='B' ) {
            ...
            if (letter=='T') {
               ...
               if (param=='' && def_auto_set_new_filename) {
                  _CreateTempBufName();
               }
         ...
         } else if (upcase(substr(word_nq,2))=='FN') {
            options :+= " "word_nq;
         ...
         if ( !do_smartopen &&
            ...
            if (do_smartopen) {
               ...
               if ((def_edit_flags &EDITFLAG_WORKSPACEFILES) && _workspace_filename!='') {
                  foundFileList :+= ' '_WorkspaceFindFile(word_nq, _workspace_filename, false, false, true, true,def_filematch_case_insensitive);
               }
      ...
      */
      ...
      if ( status ) {
         ...
         */
         ...
         if (temp_buf_name!='' && file_exists(temp_buf_name)) {
            result:=_message_box(nls("Do you want to delete '%s'?",temp_buf_name),'',MB_YESNO);
            if (result==IDYES) {
               _menu_remove_filehist(temp_buf_name);
               delete_file(temp_buf_name);
               p_buf_flags&= ~VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME;
            }
         }
      }
#else
...
_command int save(_str cmdline="",int flags= -1) name_info(FILENEW_ARG','VSARG2_ICON|VSARG2_NOEXIT_SCROLL|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   /*
      IF user wants prompting on save AND
         ...
         buffer has been saved to disk AND file on disk is newer than buffer
   */
   if (preplace &&
       !(p_buf_flags&VSBUFFLAG_PROMPT_REPLACE) &&
       (_file_eq(strip(afilename,'','"'),p_buf_name) || _file_eq(strip(absolute(afilename,null,true),'','"'),p_buf_name_no_symlinks)) &&
       file_exists(filename) &&
       ((p_file_date!='' && p_file_date!=0) && p_file_date<_file_date(filename,'B') )) {
...
_command int name(_str newName="",bool doAbsolute=true, bool prompt_for_newName=true) name_info(FILENEW_ARG','VSARG2_READ_ONLY|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   orig_name:=_strip_filename(p_buf_name,'P');
   ...
   */
   ...
   if (p_LangId=='fundamental' && 
         ((orig_ext==get_extension(p_buf_name,true)) ||
          // Name part didn't change
          (_strip_filename(p_buf_name,'P')==orig_name))
       && orig_LangId!='fundamental') {
      _SetEditorLanguage(orig_LangId);
   } else if (file_eq(orig_ext,'.sh') && orig_ext==get_extension(p_buf_name,true) && p_LangId=='e' && orig_LangId=='bourneshell') {
      _SetEditorLanguage(orig_LangId);
   }
   return(0);
   ...
   if (!multi) {
      ...
      if ((qfilename=='e' || qfilename=='edit')) {
         ...
         if (qfilename=="-#") {
            ...
            if (!_default_option(VSOPTION_OPEN_AS_FILE_NOT_WORKSPACE) && _is_workspace_filename(filename)) {
...
void _actapp_files(_str gettingFocus="")
{
   ...
   if (def_save_config_flags & SAVE_CONFIG_IMMEDIATELY_SHARE_CONFIG) {
      path:=_ConfigPath();
      ...
      if(_default_option(VSOPTION_LOAD_PLUGINS)) {
         filename_no_quotes=_plugin_get_user_plugins_path();
         // IF the user plugin directory date changed
         if (_config_file_changed(filename_no_quotes)) {
            // install/uninstall all the plugins
            plugin_update();
         }
      }
   }
...
void _save_all_filepos()
{
   ...
   for (;;) {
      ...
      if (!p_modify && (RestoreSelDisp|| RestoreLineModify) && _need_to_save2()) {
         if (!(_IsTempBufName() && p_DocumentName!='')) {
                                  ...
                                  int reloadThreshold=0)
{
   ...
   if( bufIdList ) {
      ...
      for( ;; ) {
         noption := 'N';
         if( (!(p_buf_flags & VSBUFFLAG_HIDDEN) || p_AllowSave) && p_buf_id!=skipBufferId ) {
            bufIds[bufIds._length()]=p_buf_id;
         }
         _next_buffer(noption'RH');
         if( p_buf_id==first_buf_id ) {
 ...
 */
void _RestoreMarkersInFile(MarkerSaveInfo &markerSaves, int adjustLinesBy=0)
{
   if (!_isEditorCtl()) return;
...
static _str gassociated_extensions_for_language:[] = {
   ...
   "c"            => "m mm cpp cppm cp cc cxx c++ inl ixx h hpp hp hh hxx h++ resx xaml qth mpp mxx",
   "m"            => "m mm cpp cppm cp cc cxx c++ inl ixx h hpp hp hh hxx h++ resx xaml qth mpp mxx",
   "ansic"        => "h c i",
   "ada"          => "ada adb ads",
   "d"            => "d di",
   "e"            => "e sh",
...
const VSFILE_TYPES_SEPCHAR=",";
#if 0
static _str _file_types_convert_strlist(_str strlist) {
   if (strlist=='' || !pos("\1",strlist)) {
   ...
   for (;;) {
      ...
      parse strlist with auto filter ',' strlist;
      if (filter=='') continue;
      if (newlist=='') {
         newlist=filter;
      } else {
         strappend(auto dest,VSFILE_TYPES_SEPCHAR:+filter);
      }
   }
}
#endif
void _file_types_get_filters(_str strlist, _str (&filter_array)[]) {
   filter_array._makeempty();
   for (;;) {
      if (strlist=='') {
         return;
      }
      parse strlist with auto filter (VSFILE_TYPES_SEPCHAR) strlist;
      if (filter=='') continue;
      filter_array:+=filter;
   }
}
void _file_types_get_names_and_wildcards(_str strlist, _str (&filter_names)[],_str (&filter_wildcards)[]) {
   //strlist=_file_types_convert_strlist(strlist);
   filter_names._makeempty();
   filter_wildcards._makeempty();
   for (;;) {
      if (strlist=='') {
         return;
      }
      parse strlist with auto filter (VSFILE_TYPES_SEPCHAR) strlist;
      i:=lastpos('(',filter);
      _str filter_name,filter_value;
      if (i>1 && (substr(filter,i-1,1):==' ' || substr(filter,i+1,1):=='*')) {
         filter_name=strip(substr(filter,1,i-1));
         filter_value=substr(filter,i+1);
         if (last_char(filter_value):==')') {
            filter_value=substr(filter_value,1,length(filter_value)-1);
         }
      } else {
         filter_name='';
         filter_value=filter;
      }
      if (filter_name=='' && filter_value=='') continue;
      filter_names:+=filter_name;
      filter_wildcards:+=filter_value;
   }
}
_str _file_types_get_wildcards_by_name(_str strlist,_str name,_str langid_default_wildcards='')  {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      if (strieq(name,filter_names[i])) {
         return filter_wildcards[i];
      }
   }
   if (langid_default_wildcards!='') {
      return _GetWildcardsForLanguage(langid_default_wildcards);
   }
   return '';
}
_str _file_types_get_filter_by_name(_str strlist,_str name)  {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      if (strieq(name,filter_names[i])) {
         return _file_types_make_filter(filter_names[i],filter_wildcards[i]);
      }
   }
   return '';
}

_str _file_types_get_filter_by_wildcards(_str strlist,_str wildcards)  {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_wildcards._length();++i) {
      if (file_eq(wildcards,filter_wildcards[i])) {
         return _file_types_make_filter(filter_names[i],filter_wildcards[i]);
      }
   }
   return '';
...
_str _file_types_get_wildcards_by_wildcards(_str strlist,_str wildcards,bool do_contains_match=false)  {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_wildcards._length();++i) {
      if (do_contains_match) {
         if (pos(';'wildcards';',';'filter_wildcards[i]';',1,_fpos_case)) {
            return filter_wildcards[i];
         }
      } else if (file_eq(wildcards,filter_wildcards[i])) {
         return filter_wildcards[i];
      }
   }
   return '';
}
_str _file_types_make_filter(_str filter_name,_str filter_wildcards) {
   if (filter_name=='') {
      return filter_wildcards;
   }
   // name and filter_wildcards can't have comma's
   filter_name=stranslate(filter_name,' ',',');
   filter_wildcards=stranslate(filter_wildcards,'',',');

   // Filter name can't have ' ('
   filter_name=stranslate(filter_name,' ',' (');

   return filter_name:+" (":+filter_wildcards")";
}
_str _file_types_prepend_filters(_str strlist, _str filters)  {
   if (strlist=='') {
      return filters;
   }
   return filters:+VSFILE_TYPES_SEPCHAR:+strlist;
}
_str _file_types_append_filters(_str strlist, _str filters)  {
   if (strlist=='') {
      return filters;
   }
   return strlist:+VSFILE_TYPES_SEPCHAR:+filters;
}
_str _file_types_prepend_name_and_wildcards(_str strlist, _str filter_name, _str filter_wildcards) {
   _str filter=_file_types_make_filter(filter_name,filter_wildcards);
   strlist=strlist;
   if (strlist=='') {
      return filter;
   }
   return filter:+VSFILE_TYPES_SEPCHAR:+strlist;
}
_str _file_types_append_name_and_wildcards(_str strlist, _str filter_name, _str filter_wildcards) {
   _str filter=_file_types_make_filter(filter_name,filter_wildcards);
   if (strlist=='') {
      return filter;
   }
   return strlist:+VSFILE_TYPES_SEPCHAR:+filter;
}

_str _file_types_from_names_and_wildcards(_str (&filter_names)[],_str (&filter_wildcards)[]){
   strlist:='';
   for (i:=0;i<filter_names._length();++i) {
      if (strlist!='') {
         strappend(strlist,VSFILE_TYPES_SEPCHAR:+_file_types_make_filter(filter_names[i],filter_wildcards[i]));
      } else {
         strlist=_file_types_make_filter(filter_names[i],filter_wildcards[i]);
      }
   }
   return strlist;
}
_str _file_types_from_filters(_str (&filter_array)[]){
   strlist:='';
   for (i:=0;i<filter_array._length();++i) {
      if (strlist!='') {
         strappend(strlist,VSFILE_TYPES_SEPCHAR:+filter_array[i]);
      } else {
         strlist=filter_array[i];
      }
   }
   return strlist;
}
_str _file_types_rename(_str strlist,_str oldLabel,_str newLabel) {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      if (strieq(oldLabel,filter_names[i])) {
         filter_names[i]=newLabel;
         break;
      }
   }
   return _file_types_from_names_and_wildcards(filter_names,filter_wildcards);
}
_str _file_types_change_wildcards_by_name_and_wildcards(_str strlist,_str name,_str old_wildcards, _str new_wildcards) {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      if (strieq(name,filter_names[i]) && file_eq(filter_wildcards[i],old_wildcards)) {
         filter_wildcards[i]=new_wildcards;
         break;
      }
   }
   return _file_types_from_names_and_wildcards(filter_names,filter_wildcards);
...
_str _file_types_remove_by_filter(_str strlist,_str filter) {
   _file_types_get_filters(strlist, auto filter_array);
   for (i:=0;i<filter_array._length();++i) {
      if (file_eq(filter,filter_array[i])) {
         filter_array._deleteel(i);
         break;
      }
   }
   return _file_types_from_filters(filter_array);
}
_str _file_types_remove_by_name(_str strlist,_str name) {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      if (strieq(name,filter_names[i])) {
         filter_names._deleteel(i);
         filter_wildcards._deleteel(i);
         break;
      }
   }
   return _file_types_from_names_and_wildcards(filter_names,filter_wildcards);
...
_str _file_types_remove_by_wildcards(_str strlist,_str wildcards) {
   _file_types_get_names_and_wildcards(strlist, auto filter_names, auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      if (strieq(wildcards,filter_wildcards[i])) {
         filter_names._deleteel(i);
         filter_wildcards._deleteel(i);
         break;
      }
...
void _file_types_add_wildcards_to_combo(_str filetypes = def_file_types, bool add_allfiles=true,bool add_no_duplicates=false)
{
   _str filter_names[];
   _str filter_wildcards[];
   _file_types_get_names_and_wildcards(filetypes,filter_names,filter_wildcards);

   for (i:=0;i<filter_names._length();++i) {
      wildcards:=filter_wildcards[i];
      if (wildcards != ALLFILES_RE || add_allfiles) {
         if (add_no_duplicates) {
            _lbadd_item_no_dupe(wildcards, _fpos_case, LBADD_BOTTOM);
         } else {
            _lbadd_item(wildcards);
         }
      }
   }
	_lbtop();
}
void _file_types_add_filters_to_combo(_str filetypes = def_file_types, bool add_allfiles=true, bool add_no_duplicates=false) {
   _str filter_names[];
   _str filter_wildcards[];
   //say('###'filetypes);
   _file_types_get_names_and_wildcards(filetypes,filter_names,filter_wildcards);
   /*for (ii:=0;ii<filter_wildcards._length();++ii) {
      say('i='ii' n='filter_names[ii]);
      //say('    i='i' w='filter_values[i]);
   } */
   ...
   for (i:=0;i<filter_names._length();++i) {
      ...
      if (wildcards != ALLFILES_RE || add_allfiles) {
         if (add_no_duplicates) {
            _lbadd_item_no_dupe(_file_types_make_filter(filter_names[i],wildcards), _fpos_case, LBADD_BOTTOM);
         } else {
            //say(_file_types_make_filter(filter_names[i],wildcards));
            _lbadd_item(_file_types_make_filter(filter_names[i],wildcards));
         }
      }
   }
	_lbtop();
...
/**
 * Reset our lists of visible windows (document/file tabs) and 
 * other windows to handle. 
 *  
 * @param vt      (reference) visible tabs cache info 
 */
void _ResetVisibleFileTabs(VisibleTabsCache &vt)
{
   vt.other_tab_iter = 0;
   vt.visible_tab_iter = 0;
   vt.tab_minimal_work = true;
   vt.gvisible_tabs._makeempty();
   vt.gother_tabs._makeempty();
}

/**
 * Build an array of editor windows to handle.  Can be restricted to just 
 * the current editor window, visible editor windows, or all editor windows. 
 * 
 * @param vt                (reference) visible tabs cache info 
 * @param visible_tabs      (output) list of all visible editor window IDs
 * @param other_tabs        (output) list of other editor window IDs
 * @param getCurrentOnly    only get the current active editor winndow 
 *                          (other_tabs will be left empty)
 * @param getVisibleOnly    only the the current active editor window and 
 *                          other visible editor windows
 *                          (other_tabs will be left empty)
 */
void _GetVisibleFileTabs(VisibleTabsCache &vt,
                         int (&visible_tabs)[], 
                         int (&other_tabs)[],
                         bool getCurrentOnly,
                         bool getVisibleOnly)
{
   ...
   if (vt.gvisible_tabs._length() > 0) {
      have_invalid_editorctl_wid := false;
      foreach (auto editor_wid in vt.gvisible_tabs) {
      ...
      if (!have_invalid_editorctl_wid) {
         visible_tabs = vt.gvisible_tabs;
         other_tabs   = vt.gother_tabs;
         return;
      }
   }
...
/**
 * Remove the given editor window from the cache of visible document windows.
 *  
 * @param vt          (reference) visible tabs cache info 
 * @param wid         editor window ID
 * @param tab_index   index of item in list of visible tabs
 */
void _RemoveVisibleFileTab(VisibleTabsCache &vt, int wid, int tab_index)
{
   // remove from list of visible file tabs
   if (vt.gvisible_tabs._length() > 1) {
      if (tab_index < vt.gvisible_tabs._length() && vt.gvisible_tabs[tab_index] == wid) {
         vt.gvisible_tabs._deleteel(tab_index);
         if (tab_index < vt.visible_tab_iter) {
            --vt.visible_tab_iter;
         }
      } else if (vt.visible_tab_iter < vt.gvisible_tabs._length() && vt.gvisible_tabs[vt.visible_tab_iter] == wid) {
         vt.gvisible_tabs._deleteel(vt.visible_tab_iter);
      }
   }
   // remove from list of other file tabs
   if (tab_index < vt.gother_tabs._length() && vt.gother_tabs[tab_index] == wid) {
      vt.gother_tabs._deleteel(vt.other_tab_iter);
      if (tab_index < vt.other_tab_iter) {
         --vt.other_tab_iter;
      }
   } else if (vt.other_tab_iter < vt.gother_tabs._length() && vt.gother_tabs[vt.other_tab_iter] == wid) {
      vt.gother_tabs._deleteel(vt.other_tab_iter);
   }
}

/**
 * Run the given callback for all visible file tabs.
 * 
 * @param vt                        (reference) visible file tabs cache
 * @param pfnUpdateForOneWindow     pointer to funtion to call
 * @param force                     force function to be called, even if idle time is not past
 * @param doCurrentWindowOnly       do current editor window only?
 * @param idle_time_delay           amount of time of inactivaty to wait for
 * @param timeout_in_ms             timeout after this amount of work           
 */
void _DoCallbackForAllVisibleTabs(VisibleTabsCache &vt,
                                  int (*pfnUpdateForOneWindow)(bool force, bool doMinimalWork, bool okToRefresh, int tab_index),
                                  bool force=false, 
                                  bool doCurrentWindowOnly=false,
                                  bool doVisibleOnly=false,
                                  int idle_time_delay=500, int timeout_in_ms=100)
{
   // no callback function!
   if (pfnUpdateForOneWindow == null) {
      return;
   }

   // update symbol coloring only when the editor has been idle for a while
   idle := _idle_time_elapsed();
   if (!force && idle < idle_time_delay) {
      return;
   }
   ...
   if (doCurrentWindowOnly && !_no_child_windows() && _isEditorCtl()) {
      if (p_IsMinimap) {
         visible_tabs[0] = p_minimap_to_editor_wid;
         visible_tabs[1] = p_window_id;
      } else if (p_minimap_wid) {
         visible_tabs[0] = p_window_id;
         visible_tabs[1] = p_minimap_wid;
      } else {
         visible_tabs[0] = p_window_id;
      }
   } else {
      _GetVisibleFileTabs(vt, visible_tabs, other_tabs, doCurrentWindowOnly, doVisibleOnly);
   }

   // set a timeout
   sc.lang.ScopedTimeoutGuard timeout(timeout_in_ms);
   ...
   if (visible_tabs._varformat() == VF_ARRAY && visible_tabs._length() >= 1) {
      ...
      status := (*pfnUpdateForOneWindow)(force, doMinimalWork:true, okToRefresh:true, tab_index:0);
      ...
      status = (*pfnUpdateForOneWindow)(force, doMinimalWork:false, okToRefresh:false, tab_index:0);
      if (status || _CheckTimeout()) {
         // go back to our original window
         p_window_id = orig_wid;
         return;
      ...
      for (i:=1; i<visible_tabs._length(); i++) {
         ...
         status = (*pfnUpdateForOneWindow)(force, doMinimalWork:true, okToRefresh:true, i);
         if (status || _CheckTimeout()) {
            // go back to our original window
            p_window_id = orig_wid;
            return;
      ...
      while (!doCurrentWindowOnly && vt.visible_tab_iter < visible_tabs._length()) {
         // non-zero status means we broke out before completing this tab
         p_window_id = visible_tabs[vt.visible_tab_iter];
         status = (*pfnUpdateForOneWindow)(force, doMinimalWork:false, okToRefresh:false, vt.visible_tab_iter);
         if (status || _CheckTimeout()) {
            // go back to our original window
            p_window_id = orig_wid;
            return;
   ...
   if (!doCurrentWindowOnly && other_tabs._varformat() == VF_ARRAY && other_tabs._length() >= 1) {
      while (vt.other_tab_iter < other_tabs._length()) {
         wid := other_tabs[vt.other_tab_iter];
         if (!_iswindow_valid(wid) || !wid._isEditorCtl()) continue;
         p_window_id = wid;
         status := (*pfnUpdateForOneWindow)(force, vt.tab_minimal_work, okToRefresh:false, vt.other_tab_iter);
         if (!status) {
            // hey, we finished this tab!
            vt.other_tab_iter++;
            if (vt.other_tab_iter >= other_tabs._length() && vt.tab_minimal_work) {
               vt.tab_minimal_work = false;
               vt.other_tab_iter = 0;
            }
         } else {
            // non-zero status means we broke out before completing this tab
...
void _load_all_standard_pic_bitmaps()
{
   ...
   _pic_lbuser=load_picture(-1,"_f_user.svg");
   ...
   _pic_lbvs_star=load_picture(-1,"_f_vs_pro_star.svg");
   ...
   _pic_dim_bulb = load_picture(-1, "_ed_dimbulb.svg");
   ...
   _pic_file_vc_date_overlay = load_picture(-1, "_f_overlay_updateable.svg");
   if (_pic_file_vc_date_overlay>=0) {
      set_name_info(_pic_file_vc_date_overlay,"This file is older than the version in the repository");
   }
   ...
   _pic_file_bin_diff_overlay  = load_picture(-1, "_f_overlay_binary.svg");
   if (_pic_file_bin_diff_overlay>=0) {
      set_name_info(_pic_file_bin_diff_overlay,"This file is a binary mode file. A binary compare was performed they do not match");
   }
   _pic_file_bin_svc_overlay  = load_picture(-1, "_f_overlay_svc_binary.svg");
   if (_pic_file_bin_svc_overlay>=0) {
      set_name_info(_pic_file_bin_svc_overlay,"This file is a binary mode file.  It was reported different by your version control system.");
   }
   ...
   _pic_file_unknown_overlay = load_picture(-1, "_f_overlay_unknown.svg");
   if (_pic_file_unknown_overlay>=0) {
      set_name_info(_pic_file_unknown_overlay,"This file is unkown");
   }
   ...
   _pic_file_mfdiff_add_overlay = load_picture(-1, "_f_overlay_add.svg@xA");
   if (_pic_file_mfdiff_add_overlay>=0) {
      set_name_info(_pic_file_mfdiff_add_overlay,"This file only exists in this path");
   }
   _pic_file_conflict_overlay  = load_picture(-1, "_f_overlay_conflict.svg");
   if (_pic_file_conflict_overlay>=0) {
   ...
   _pic_file_edited_overlay=load_picture(-1,"_f_overlay_edited.svg");
   if (_pic_file_edited_overlay>=0) {
      set_name_info(_pic_file_edited_overlay,"This file has been edited locally.");
   }
   ...
   _pic_otb_overlay_readonly = load_picture(-1, "_f_overlay_readonly.svg");
   if (_pic_otb_overlay_readonly) {
      set_name_info(_pic_otb_overlay_readonly,"This file is read only");
   }
   _pic_otb_overlay_writable = load_picture(-1, "_f_overlay_writable.svg");
   if (_pic_otb_overlay_writable) {
      set_name_info(_pic_otb_overlay_writable,"This file is writable (not read-only)");
   }
