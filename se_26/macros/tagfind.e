...
void ctl_search_for.on_create()
{
   ...
   ctl_options_button.resizeToolButtonToLabel(ctl_options_label.p_window_id);
...
void ctl_search_for.up()
{
   ctl_symbols.call_event(ctl_symbols,UP);
}

void ctl_search_for.down()
{
   ctl_symbols.call_event(ctl_symbols,DOWN);
}

void ctl_search_for.pgup()
{
   ctl_symbols.call_event(ctl_symbols,PGUP);
}

void ctl_search_for.pgdn()
{
   ctl_symbols.call_event(ctl_symbols,PGDN);
}
...
void ctl_symbols.rbutton_up,context()
{
   // Get handle to menu:
...
void ctl_goto_symbol.lbutton_up()
{
   ...
   if (!status) {
      ...
      if (editorctl_wid && editorctl_wid._isEditorCtl(false) && editorctl_wid.p_mdi_child) {
         persistent_wid=editorctl_wid._get_persistent_wid();
      }
      ...
      if (!status && orig_buf_id != 0) {
         ...
         if (def_search_result_push_bookmark) {
            status = editorctl_wid.push_bookmark(mark,persistent_wid: persistent_wid);
         }
      }
   }
}
