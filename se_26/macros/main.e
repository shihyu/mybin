...
bool def_show_build_vslickerrorpath=false;
bool def_show_build_cd=false;

int _last_temp_num=0;
int _lastdir_temp_num=0;
/*
...
int def_javadoc_parammax=24;
int def_javadoc_exceptionmin=6;
int def_javadoc_exceptionmax=24;
...
/**
 * space delimited list of import option versions
 */
_str def_config_imports_from;

/**
 * Version of SlickEdit that def_config_imports_from corresponds 
 * to. If this doesn't match the SlickEdit version, ignore this 
 * list. 
 */
_str def_config_imports_ver;
bool _allow_clearSlickCErrorMessages=true;
...
int def_exit_flags=0;
SaveConfigFlags def_save_config_flags=SAVE_CONFIG_IMMEDIATELY_SHARE_CONFIG/*|EXIT_FILES_PROMPT*/;
...
/**
 * @return 
 * Returns {@code true} if we are currently building the state file.
 */
bool _BuildingStateFile()
{
   return buildingStateFile;
}
...
{
   // Save config?
   if((def_save_config_flags & (SAVE_CONFIG_IMMEDIATELY|SAVE_CONFIG_IMMEDIATELY_SHARE_CONFIG)) && 
...
static void _CheckForReadOnlyConfig()
{
   // back door for stubborn users
   if (!def_warn_about_vslickconfig) {
      return;
   ...
   // make sure the versioned configuration directory is there and writable
   config_dir := _ConfigPath();
   if (!file_exists(config_dir)) {
      status := _make_path(config_dir);
      if ( status ) {
         _message_box(nls('Unable to create configuration directory "%s"',config_dir)'.  'get_message(status));
         fexit();
      }
   }

   // make sure the instances directory is there and writable also
   instances_dir := config_dir;
   _maybe_append_filesep(instances_dir);
   instances_dir :+= "instances";
   if ( !file_exists(instances_dir) ) {
      status := _make_path(instances_dir);
      if ( status ) {
         _message_box(nls('Unable to create configuration directory "%s"',instances_dir)'.  'get_message(status));
         fexit();
      }
   }
}
 ...
 * set {@code new_app_theme} to a valid theme name, or '' 
...
_str def_open_workspace_exts=PRJ_FILE_EXT' ':+
   WORKSPACE_FILE_EXT:+
   VCPP_PROJECT_WORKSPACE_EXT' ':+
   TORNADO_WORKSPACE_EXT' ':+
   VCPP_EMBEDDED_PROJECT_WORKSPACE_EXT' ':+
   XCODE_PROJECT_EXT' ':+
   XCODE_PROJECT_LONG_BUNDLE_EXT' ':+
   XCODE_PROJECT_SHORT_BUNDLE_EXT' ':+
   XCODE_WKSPACE_BUNDLE_EXT' ':+
   VISUAL_STUDIO_SOLUTION_EXT' ':+
   JBUILDER_PROJECT_EXT' ':+
   MACROMEDIA_FLASH_PROJECT_EXT:+
   ' Cargo.toml';
bool _is_workspace_filename(_str filename) {
   ...
   if(_file_eq(workspace_file_ext,PRJ_FILE_EXT) ||
      ...
      if (pos(' 'workspace_file_ext' ',' 'def_open_workspace_exts' ',1,_fpos_case)
          /* maybe we should hardwire .vpw and .sln since init.cpp has them hardwired
             to restore the MDI window coordinates.*/
          //|| file_eq(workspace_file_ext,WORKSPACE_FILE_EXT)
   ...
   name:=_strip_filename(filename,'P');
   if (_file_eq(name,'Cargo.toml')) {
      if (pos(' 'name' ',' 'def_open_workspace_exts' ',1,_fpos_case)) {
         return true;
      }
   }
   return false;
...
defmain()
{
  ...
  workspaceToOpen := '';
  open_as_file_not_workspace:=_default_option(VSOPTION_OPEN_AS_FILE_NOT_WORKSPACE);
  for (;;) {
     ...
     if ( substr(cmdline,1,1)!='-' ) {
        if ( cmdline=='' && macro_program=='') {
           ...
           if (!open_as_file_not_workspace && (_is_workspace_filename(filename) || _lazy_workspace_spec(filename))) {
  ...
  if ( error_code ) {
    ...
    if ( error_code==FILE_NOT_FOUND_RC ) {
      ...
      if ( error_code==1  ) {  /* rc from Slick Translator? */
        ...
        if ( !quiet ) {
           _message_box(nls("Error compiling or loading:")" ":+module:+".  "get_message((int)error_code));
        }
      }
      if ( index ) { 
         rc=2;
...
_str _default_c_wildcards()
{
   if (_isUnix()) {
      if (_isMac()) {
         return("*.c;*.cxx;*.cpp;*.cppm;*.c++;*.m;*.mm;*.h;*.hpp;*.hxx;*.h++;*.inl;*.ixx;*.mpp;*.mxx");
      }
      return("*.c;*.cxx;*.cpp;*.cppm;*.c++;*.h;*.hpp;*.hxx;*.h++;*.inl;*.ixx;*.mpp;*.mxx");
   }
   return("*.c;*.cxx;*.cpp;*.cppm;*.c++;*.h;*.hpp;*.hxx;*.h++;*.inl;*.ixx;*.mpp;*.mxx");
}
