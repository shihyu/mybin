...
static void list_editor_sort()
{
   if (_GetDialogInfo(LIST_EDITOR_IS_SORTED, ctl_tree)) {
      ...
      if (origIndex!=index) {
         ctl_tree._TreeSetCurIndex(origIndex);
      }

      ctl_tree._TreeSortCaption(TREE_ROOT_INDEX);

      if (wasNewEntryNode) {
         int newIndex = ctl_tree._TreeAddListItem(newEntryCaption);
...
void list_editor_delete()
{
   ...
   if (index > 0) {
      ...
      if (delete_callback!=null && _isfunptr(delete_callback)) {
         result:=(*delete_callback)(caption);
         if (result._varformat()==VF_LSTR && result=='')  {
            return;
         }
      }
