...
static int maybeGetVCFromWorkspace(bool &handledInWkspace,_str &vcs="" ,bool getAutoDetectedSystem=true)
{
   ...
   if ( gWorkspaceHandle >=0 ) {
      ...
      if ( vcsproject=="" || vcsproject==VC_SELECTED_NONE_STRING || vcsproject==VC_SELECTED_NONE_STRING':' ) {
         handledInWkspace = false;
         return 0;
      }
...
static init_vc_form_project(typeless use_ini_vcs)
{
   ...
   _file_types_get_names_and_wildcards(filetypes,auto filter_names,auto filter_values);
   if (wildcards=='') {
      if (wildcards=='' && filter_values._length()) {
         wildcards=filter_values[0];
      }
      if (wildcards=='') {
         wildcards=ALLFILES_RE;
      }
   }
   _file_types_add_filters_to_combo(filetypes);
...
void ctldefault.on_change(int reason)
{
   if (reason==CHANGE_CLINE) {
      lastSystem := _GetDialogInfoHt(p_name".lastSystem");
      if (substr(p_text,1,2)=="--") {
         p_text = lastSystem;
      } else {
         _SetDialogInfoHt(p_name".lastSystem",p_text);
      }
   }
}

static bool isInSCCSList(_str systemName)
{
   SCCSystemList := _GetDialogInfoHt("SCCSystemList");
   foreach ( auto curSystem in SCCSystemList ) {
      if (systemName == curSystem) return true;
   }
   return false;
}

void ctloptions_btn.lbutton_up()
{
   system := p_prev.p_text;
   _SetDialogInfoHt("lastActiveComboBoxWID",p_prev);
   if ( system == VC_SELECTED_NONE_STRING || system == "" ) {
      return;
   }
   ...
   if ( _isWindows() ) {
      // No SCC systems on other platforms
      show("-modal _scc_open_project_form", p_prev.p_text);
   }
}
...
static bool systemIsModified(_str systemName1, _str systemName2)
{
   if ( (systemName1=="" || systemName1==VC_SELECTED_NONE_STRING) && systemName2==VC_SELECTED_NONE_STRING ) {
      return false;
   }
...
bool _vc_select_form_is_modified()
{
   ...
   if ( systemIsModified(systemsForAllScopes:[VC_SCOPE_DEFAULT_STRING], ctldefault.p_text) ||
        systemIsModified(systemsForAllScopes:[VC_SCOPE_WORKSPACE_STRING], ctlworkspace.p_text) ||
        systemIsModified(systemsForAllScopes:[VC_SCOPE_PROJECT_STRING], ctlproject.p_text)
        ) {
      return true;
   }
   return false;
...
static int whichSystemIsSCC()
{
   if (isInSCCSList(ctldefault.p_text)) {
      return ctldefault;
   }
   if (isInSCCSList(ctlworkspace.p_text)) {
      return ctlworkspace;
   }
   if (isInSCCSList(ctlproject.p_text)) {
      return ctlproject;
   }
   return 0;
}

static _str getScopeValueForHTOrFile(_str value)
{
   if ( value==VC_SELECTED_NONE_STRING ) {
      value = "";
   }
   return value;
}
...
/**
 * We never want to store VC_SELECTED_NONE_STRING, convert it to 
 * "" 
 * 
 * @param systemsForAllScopes hashtable with value for all 
 *                            scopes
 * @param scope scope to set value for
 * @param value value to set
 */
static void setScopeValueForHT(STRHASHTAB &systemsForAllScopes, _str scope, _str value)
{
   systemsForAllScopes:[scope] = getScopeValueForHTOrFile(value);
}

/**
 * For comboboxes we have to convert "" to 
 * VC_SELECTED_NONE_STRING
 *  
 * @param systemsForAllScopes hashtable with value for all 
 *                            scopes
 * @param scope to get
 * 
 * @return _str Displayable verision of <B>scope</B>
 */
static _str getScopeValueForCombo(STRHASHTAB &systemsForAllScopes, _str scope)
{
   value := systemsForAllScopes:[scope];
   if ( value=="" ) {
      value = VC_SELECTED_NONE_STRING;
   }
   return value;
}
...
void _vc_select_form_apply()
{
   ...
   defaultSystem := ctldefault.p_text;
   workspaceSystem := ctlworkspace.p_text;
   projectSystem := ctlproject.p_text;
   SCCSystemList := _GetDialogInfoHt("SCCSystemList");

   sccNameList := isInSCCSList(defaultSystem) ? defaultSystem:+"\n" : "";

   sccNameList = isInSCCSList(projectSystem) ? sccNameList:+"\n":+projectSystem:+"\n" : sccNameList;

   sccNameList = isInSCCSList(workspaceSystem) ? sccNameList:+"\n":+workspaceSystem:+"\n" : sccNameList;

   if ( last_char(sccNameList) == "\n" ) {
      sccNameList = substr(sccNameList,1,length(sccNameList)-1);
   }

   systemsForAllScopes := _GetDialogInfoHt("systemsForAllScopes");
 
   if ( _isWindows() ) {
      origSCCProjName := _SccGetCurProjectInfo(VSSCC_PROJECT_NAME);
      validSCCSystem := false;
      if ( sccNameList!="" || origSCCProjName != _SccGetCurProjectInfo(VSSCC_PROJECT_NAME) ) {
         sccProvName := _SccGetCurProjectInfo(VSSCC_PROVIDER_NAME);
         sccProjName := _SccGetCurProjectInfo(VSSCC_PROJECT_NAME);
         msg := "";
         if ( sccProvName == "" ) {
            msg = nls("The following systems will not be saved because they were not initialized:\n\n%s", sccNameList);
         } else if ( sccProjName == "" ) {
            msg = nls("The following systems will not be saved because no SCC project was opened:\n\n%s", sccNameList);
         } else {
            if ( whichSystemIsSCC() != ctlproject ) {
               msg = nls("SCC systems may only be used with the project and not in combination with other systems.\n\n%s will be saved as the version control system for the project", sccNameList);
            }
            validSCCSystem = true;
         }
         if ( msg!="" ) {
         ...
         if ( validSCCSystem ) {
            setScopeValueForHT(systemsForAllScopes,VC_SCOPE_DEFAULT_STRING,"");
            setScopeValueForHT(systemsForAllScopes,VC_SCOPE_WORKSPACE_STRING,"");
            setScopeValueForHT(systemsForAllScopes,VC_SCOPE_PROJECT_STRING,sccProvName);
            SetVCSLocalPath(_ProjectHandle(),sccProvName,_SccGetCurProjectInfo(VSSCC_LOCAL_PATH));
            SetVCSAuxPath(_ProjectHandle(),sccProvName,_SccGetCurProjectInfo(VSSCC_AUX_PATH_INFO));
            _ProjectSet_VCSProject(_ProjectHandle(),SCC_PREFIX:+sccProvName':'_SccGetCurProjectInfo(VSSCC_PROJECT_NAME));
            return;
         }
      } else {
         sccProvName := _SccGetCurProjectInfo(VSSCC_PROVIDER_NAME);
...
//      ctlscope_combo.call_event(CHANGE_SELECTED,ctlscope_combo,ON_CHANGE,'W');
   ...
   if ( getScopeValueForCombo(systemsForAllScopes,VC_SCOPE_DEFAULT_STRING)!=ctldefault.p_text && !pos(ctldefault.p_text,sccNameList) ) {
      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_DEFAULT_STRING,ctldefault.p_text);
      _SetVCSystemName(systemsForAllScopes:[VC_SCOPE_DEFAULT_STRING]);
   }

   // If the vcs for the workspace changed, set that in .vpw file.  Don't 
   // have to worry  about _config_modify_flags
   if ( _workspace_filename!="" &&
        getScopeValueForCombo(systemsForAllScopes,VC_SCOPE_WORKSPACE_STRING)!=ctlworkspace.p_text && !pos(ctlworkspace.p_text,sccNameList) ) {
      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_WORKSPACE_STRING,ctlworkspace.p_text);
   ...
   // have to worry  about _config_modify_flags
   if ( machine()=='WINDOWS' && _project_name!="" &&
        (getScopeValueForCombo(systemsForAllScopes,VC_SCOPE_PROJECT_STRING)!=ctlproject.p_text && !pos(ctlproject.p_text,sccNameList)) ) {
      if (  isInSCCSList(ctlproject.p_text) ) {
         SetVCSLocalPath(_ProjectHandle(),ctlproject.p_text,_SccGetCurProjectInfo(VSSCC_LOCAL_PATH));
         SetVCSAuxPath(_ProjectHandle(),ctlproject.p_text,_SccGetCurProjectInfo(VSSCC_AUX_PATH_INFO));
         _ProjectSet_VCSProject(_ProjectHandle(),SCC_PREFIX:+ctlproject.p_text':'_SccGetCurProjectInfo(VSSCC_PROJECT_NAME));
      } else {
         _SccCloseProject();
         _SccUninit();
         SetVCSLocalPath(_ProjectHandle(),"","");
         SetVCSAuxPath(_ProjectHandle(),"","");
         _ProjectSet_VCSProject(_ProjectHandle(),getScopeValueForHTOrFile(ctlproject.p_text)':');
      }
      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_PROJECT_STRING,ctlproject.p_text);
      _ProjectSave(_ProjectHandle());
      _WorkspacePutProjectDate();
   }/*else{
...
static const SVC_SYSTEM_CAPTION = "Integrated systems";
static const SCC_SYSTEM_CAPTION = "SCC systems";
static const LEGACY_SYSTEM_CAPTION = "Legacy systems";
static const SVC_SYSTEM_COMBO_CAPTION = "-- Integrated systems --";
static const SCC_SYSTEM_COMBO_CAPTION = "-- SCC systems --";
static const LEGACY_SYSTEM_COMBO_CAPTION = "-- Legacy systems-- ";
static const NUM_TYPES = 3;

static const SYSTEM_TYPE_DEPTH = 1;
static const SYSTEM_DEPTH = 2;

static bool SVCSystemIsSelected()
{
   SVCSystemList := _GetDialogInfoHt("SVCSystemList");
   len := SVCSystemList._length();
   for (i:=0;i<len;++i) {
...
#if 0 //18:54pm 6/22/2021
static void enableCLButtons(bool visible, bool enabled)
{
   ctladd.p_visible = visible;
   ctlcopy.p_visible = visible;
   ctldelete.p_visible = visible;
   ctldelete.p_visible = visible;
   ctlrename.p_visible = visible;

   ctladd.p_enabled = enabled;
   ctlcopy.p_enabled = enabled;
   ctldelete.p_enabled = enabled;
   ctldelete.p_enabled = enabled;
   ctlrename.p_enabled = enabled;
}
#endif

/**
 * Use this function if you need to know if the name listed (not
 * decorated with "SCC:" etc) is an SCC system
 * 
 * @return bool true if <B>systemName</B> is an SCC system
 * 
 * @param systemName
 * 
 */
static bool nameIsSCC(_str systemName)
{
   SCCTable := _GetDialogInfoHt("SCCTable");
   if (SCCTable==null) {
      _SccGetProvidersList(auto providers);
      len := providers._length();
      for (i:=0;i<len;++i) {
         SCCTable:[providers[i]] = "";
      }
      _SetDialogInfoHt("SCCTable",SCCTable);
      SCCTable = _GetDialogInfoHt("SCCTable");
   }
   return SCCTable._indexin(systemName);
}

#if 0 //17:58pm 6/22/2021
void ctlsystem_list.on_change(int reason,int index)
{
   if ( index < 0 ) {
      return;
   }
   lastComboWID := _GetDialogInfoHt("lastComboWID");
   switch ( reason ) {
   case CHANGE_SELECTED:
      enableCLButtons(true,true);
      break;
   case CHANGE_LEAF_ENTER:
      if ( lastComboWID != null) {
         lastComboWID.p_text = _TreeGetCaption(_TreeCurIndex());
      }
      p_visible = false;
      break;
   }
...
const VCSYSTEM_DEFAULT_FLAG   = 0x1;
const VCSYSTEM_WORKSPACE_FLAG = 0x2;
const VCSYSTEM_PROJECT_FLAG   = 0x4;
...
void _vc_select_form.on_resize()
{
   ...
   ctlminihtml1.p_x = ctldefault.p_x;
   ctlminihtml1.p_width = ctldefault.p_width;

   ybuffer := ctldefault.p_y;
   ctlminihtml1.p_y = ctlproject.p_y_extent + ybuffer;
}
...
static void initAllSystemScopes(STRHASHTAB &systemsForAllScopes)
{
   setScopeValueForHT(systemsForAllScopes,VC_SCOPE_DEFAULT_STRING,"");
   setScopeValueForHT(systemsForAllScopes,VC_SCOPE_WORKSPACE_STRING,"");
   setScopeValueForHT(systemsForAllScopes,VC_SCOPE_PROJECT_STRING,"");
}
...
static _str getCurrentVCForAllScopes(_str &scope)
{
   ...
   setScopeValueForHT(systemsForAllScopes,VC_SCOPE_DEFAULT_STRING,vcs);
   ...
   if ( vcswkspace !="" ) {
      ...
      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_WORKSPACE_STRING,vcs);
   }
   vcsproject := _ProjectGet_VCSProject(_ProjectHandle());
   GetVCSAndProjectName(vcsproject,vcs,auto projectname,auto isscc);
   if ( vcsproject !="" ) {
      // See comment above
      _maybe_strip(vcsproject,':');
//      vcs = MaybeStripLeadingVCSName(vcsproject,auto localpathvcs);
      scope = VC_SCOPE_PROJECT_STRING;
      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_PROJECT_STRING,vcs);
   }
   if ( _isscc(systemsForAllScopes:[VC_SCOPE_PROJECT_STRING]) && (vcsproject!="" || vcswkspace!="") ) {
...
void ctldefault.on_got_focus()
{
   _SetDialogInfoHt("lastActiveComboBoxWID",p_window_id);
}

static void setComboBoxes(STRHASHTAB systemsForAllScopes,_str scope,_str overrideValue="")
{
   if ( systemsForAllScopes._indexin(scope) ) {
      defaultVC := getScopeValueForCombo(systemsForAllScopes,scope);
      if ( _isscc(defaultVC) ) {
         defaultVC = _SccGetCurProjectInfo(VSSCC_PROVIDER_NAME);
      }
      status := _lbsearch(defaultVC);
      if (overrideValue!="") {
         p_text = overrideValue;
      } else if ( !status ) {
         p_text = _lbget_text();
      } else {
         p_text = VC_SELECTED_NONE_STRING;
      }
      _SetDialogInfoHt(p_name".lastSystem",p_text);
   }
}

static bool noSCCSystems(STRHASHTAB &systemsForAllScopes)
{
   return !isInSCCSList(systemsForAllScopes:[VC_SCOPE_DEFAULT_STRING]) &&
          !isInSCCSList(systemsForAllScopes:[VC_SCOPE_WORKSPACE_STRING]) &&
          !isInSCCSList(systemsForAllScopes:[VC_SCOPE_PROJECT_STRING]);
}

static _str getSCCSystem(STRHASHTAB &systemsForAllScopes)
{
   if ( _isscc(systemsForAllScopes:[VC_SCOPE_DEFAULT_STRING]) ) {
      return systemsForAllScopes:[VC_SCOPE_DEFAULT_STRING];
   } else if ( _isscc(systemsForAllScopes:[VC_SCOPE_WORKSPACE_STRING]) ) {
      return systemsForAllScopes:[VC_SCOPE_WORKSPACE_STRING];
   } else if ( _isscc(systemsForAllScopes:[VC_SCOPE_PROJECT_STRING]) ) {
      return systemsForAllScopes:[VC_SCOPE_PROJECT_STRING];
   } 
   return "";
}
...
static void initVCSetup(_str type='',_str system='',_str localpath='',_str project='')
{
   ...
   haveUnusedSCCSystems := false;
   if (machine()=='WINDOWS' && _haveVersionControl()) {
      wid=p_window_id;
      status = _SccGetProvidersList(auto providerList=null);
   ...
   VersionControlSettings.getCommandLineProviderList(auto clList);
   STRARRAY SVCSystemList = null;
   STRARRAY legacySystemList = null;
   INTARRAY comboBoxWIDList;
   _nocheck _control ctldefault;
   _nocheck _control ctlworkspace;
   _nocheck _control ctlproject;

   comboBoxWIDList :+= ctldefault;
   comboBoxWIDList :+= ctlworkspace;
   comboBoxWIDList :+= ctlproject;
   comboBoxListLen := comboBoxWIDList._length();
   foreach (auto provName => auto systemProv in clList) {
      if ( _VCIsSpecializedSystem(provName) ) {
         SVCSystemList :+= provName;
      } else {
         legacySystemList :+= provName;
      }
   }

   foreach ( auto curComboBoxWID in comboBoxWIDList ) {
      curComboBoxWID._lbclear();
      curComboBoxWID._lbadd_item2(VC_SELECTED_NONE_STRING,0);
      curComboBoxWID._lbadd_item2(SVC_SYSTEM_COMBO_CAPTION,0);
      foreach ( auto curItem in SVCSystemList ) {
         curComboBoxWID._lbadd_item2(curItem,_pic_lbvs_star);
      }
      if (machine()=='WINDOWS' && _haveVersionControl() && !SIMULATE_NON_NT) {
         _SccGetProvidersList(auto SCCSystemList);
         curComboBoxWID._lbadd_item2(SCC_SYSTEM_COMBO_CAPTION,0);
         foreach ( auto curSCCProvider in SCCSystemList ) {
            curComboBoxWID._lbadd_item2(curSCCProvider, _pic_cvs_file);
         }
         _SetDialogInfoHt("SCCSystemList",SCCSystemList);
      }
      curComboBoxWID._lbadd_item2(LEGACY_SYSTEM_COMBO_CAPTION,0);
      foreach ( curItem in legacySystemList ) {
         curComboBoxWID._lbadd_item2(curItem,_pic_lbvs);
      }
   }
   _SetDialogInfoHt("SVCSystemList",SVCSystemList);
   _SetDialogInfoHt("legacySystemList",legacySystemList);

   if ( noSCCSystems(systemsForAllScopes) ) {
      ctldefault.setComboBoxes(systemsForAllScopes, VC_SCOPE_DEFAULT_STRING);
      ctlworkspace.setComboBoxes(systemsForAllScopes, VC_SCOPE_WORKSPACE_STRING);
      ctlproject.setComboBoxes(systemsForAllScopes, VC_SCOPE_PROJECT_STRING);
   } else {
      sccSystemName := getSCCSystem(systemsForAllScopes);
      ctldefault.setComboBoxes(systemsForAllScopes, VC_SCOPE_DEFAULT_STRING,VC_SELECTED_NONE_STRING);
      ctlworkspace.setComboBoxes(systemsForAllScopes, VC_SCOPE_WORKSPACE_STRING,VC_SELECTED_NONE_STRING);
      ctlproject.setComboBoxes(systemsForAllScopes, VC_SCOPE_PROJECT_STRING);

      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_DEFAULT_STRING,VC_SELECTED_NONE_STRING);
      setScopeValueForHT(systemsForAllScopes,VC_SCOPE_WORKSPACE_STRING,VC_SELECTED_NONE_STRING);
      _SetDialogInfoHt("systemsForAllScopes", systemsForAllScopes);
   }

#if 0 //18:04pm 6/22/2021
...
//      ctlscc_systems._lbselect_line();
   ...
   if (machine()=='WINDOWS') {
//      ctlsystem_list.call_event(CLIndex,CHANGE_SELECTED,ctlsystem_list,ON_CHANGE,'W');
   }
   if (machine()!='WINDOWS'||SIMULATE_NON_NT) {
#if 0 //20:43pm 3/30/2021
      int diff=ctlok.p_y - ctlscc_frame.p_y;//_auto_checkout.p_y-frame1.p_y;
...
{
#if 0 //17:37pm 6/22/2021
   bool clList:[];
   origWID := p_window_id;
   p_window_id = ctlsystem_list;
   CLFolderIndex := _TreeSearch(TREE_ROOT_INDEX,SVC_SYSTEM_CAPTION);
   LegacyFolderIndex := _TreeSearch(TREE_ROOT_INDEX,LEGACY_SYSTEM_CAPTION);
   if (CLFolderIndex<0 || LegacyFolderIndex<0) return;
   _TreeDelete(CLFolderIndex,'C');
   _TreeDelete(LegacyFolderIndex,'C');
   VersionControlSettings.getCommandLineProviderList(clList);
   foreach (auto provName => auto systemProv in clList) {
      bmIndex := 0;
      if ( _VCIsSpecializedSystem(provName) ) {
         _TreeAddItem(CLFolderIndex,provName,TREE_ADD_AS_FIRST_CHILD,0,_pic_lbvs_star,TREE_NODE_LEAF);
      } else {
         if ( systemProv ) {
            bmIndex = _pic_lbvs;
         } else {
         ...
         _TreeAddItem(LegacyFolderIndex,provName,TREE_ADD_AS_CHILD,0,bmIndex,TREE_NODE_LEAF);
      }
   }

   if (selection != '') {
      index := ctlsystem_list._TreeSearch(TREE_ROOT_INDEX,selection,'I');
   }
   p_window_id = origWID;
#endif
}
...
{
   // get the provider id
   system := ctlsystem_list._lbget_text();
...
void ctlrename.lbutton_up()
{
   ...
   _str newvcs=_param1;
   vcs := ctlsystem_list._lbget_text();
...
{
   index := ctlsystem_list._TreeCurIndex();
   if ( index<0 || index==TREE_ROOT_INDEX ) {
      _message_box("No system selected");
      return;
   }
   source := ctlsystem_list._TreeGetCaption(index);
   typeless result=show('-modal _textbox_form',
...
_command int vchistory(_str filename='') name_info(','READ_ONLY_ARG2|EDITORCTL_ARG2|VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   vcs := svc_get_vc_system(filename);
   if (machine()=='WINDOWS' && _isscc(vcs)) {
      STRARRAY filenameList;
      filenameList[0] = filename;
      status=_SccHistory(filenameList);
                    ...
                    bool enableSaveAs=false)
{
   ...
   if ( _isdiffed(p_buf_id) ) {
      _message_box(get_message(VSDIFF_FILE_IS_BEING_DIFFED,p_buf_name));
      return VSDIFF_FILE_IS_BEING_DIFFED;
   }
...
/**
 * @return STRARRAY List of the SVC systems in the order we want 
 *         to display them in a list
 */
static STRARRAY GetSVCList()
{
   STRARRAY list;
   list :+= "Git";
   list :+= "Subversion";
...
void _init_menu_vc(int menu_handle,int no_child_windows,bool isPopUpMenu=false)
{
   if ( !_haveVersionControl() ) {
      // Delete Version Control menu if there is no version control
      index := _menu_find_loaded_menu_caption(menu_handle,"Version Control");
      if ( index > 0 ) {
         _menu_delete(menu_handle,index);
         index = _menu_find_loaded_menu_caption(menu_handle,"Shelves");
         if ( index > 0 ) {
            _menu_delete(menu_handle,index);
         }
         return;
   ...
   index := _menu_find_loaded_menu_caption(menu_handle,"Version Control",vcMenuSubhandle);
   if ( index < 0 ) return;
   if (vcs=="") {
      return;
   }
   ...
   for (;;) {
      status=_menu_delete(vcMenuSubhandle, 0);
      if ( status ) break;
   }
   numberOfItemsOnMenu := 0;
   if ( _VCIsSpecializedSystem(vcs) ) {
      SetupMenuForSpecializedSystem(vcMenuSubhandle,vcs,filename,numberOfItemsOnMenu,isPopUpMenu);
   }else{
      SetupMenuForNonSpecializedSystem(vcMenuSubhandle,numberOfItemsOnMenu);
   }
...
static void SetupMenuForSpecializedSystem(int vc_menu_handle,_str system_name,_str cur_bufname,
                                          int &numberOfItemsOnMenu,bool isPopUpMenu)
{

   SVCMenuSetup(cur_bufname,vc_menu_handle,auto count,system_name,isPopUpMenu);
...
static void SetupMenuForNonSpecializedSystem(int vc_menu_handle,int &numberOfItemsOnMenu)
{
   ...
   vc_index    := _menu_find_caption(menu_handle,"Version Control");
   if (vc_index) {
      int child_index;
...
defeventtab _scc_open_project_form;

const SCC_PROVIDER_LABEL_CAPTION       = "SCC Provider Name:";
const SCC_OPEN_PROJECT_BUTTON_CAPTION  = "&Open SCC Project";
const SCC_CLOSE_PROJECT_BUTTON_CAPTION = "&Close SCC Project";

void ctlclose.on_create(_str SCCSystemName="")
{
   p_active_form.p_caption = SCCSystemName:+" Setup";
   ctlscc_provider_name.p_caption = SCC_PROVIDER_LABEL_CAPTION:+' ':+SCCSystemName;
   ctlscc_projname.p_caption = VC_SELECTED_NONE_STRING;
   ctlscc_init.p_enabled = _SccGetCurProjectInfo(VSSCC_PROVIDER_NAME) != SCCSystemName;
   ctlscc_open_project.p_enabled = _SccGetCurProjectInfo(VSSCC_PROVIDER_NAME) != "";
   ctlscc_open_project.p_caption = SCC_OPEN_PROJECT_BUTTON_CAPTION;

   sccProjName := _SccGetCurProjectInfo(VSSCC_PROJECT_NAME);
   if ( sccProjName != "" ) {
      ctlscc_projname.p_caption = sccProjName;
   }
}
...
static _str getSCCSystemNameFromDialog()
{
   parse ctlscc_provider_name.p_caption with (SCC_PROVIDER_LABEL_CAPTION) auto curSystemName;
   return strip(curSystemName);
}
...
int ctlscc_init.lbutton_up()
{
   ...
   if (machine()=='WINDOWS' && _haveVersionControl()) {
      ...
      //are running Intel Windows
      parse ctlscc_provider_name.p_caption with (SCC_PROVIDER_LABEL_CAPTION) auto curSystemName;
      curSystemName = strip(curSystemName);
      curProvider := _SccGetCurProjectInfo(VSSCC_PROVIDER_NAME);
      if ( curProvider != curSystemName ) {
         mou_hour_glass(true);
         status=_SccInit(curSystemName);
         mou_hour_glass(false);
      } else if ( curProvider != "" ) {
         ctlscc_advanced.p_enabled = false;
      }
      if (status) {
         _message_box(nls("Could not initialize provider %s",curSystemName));
         return(1);
      }
      ctlscc_projname.p_caption=MaybeNone(_SccGetCurProjectInfo(VSSCC_PROJECT_NAME));
      enableSCCButtons(true,true,_SccGetCommandOptions(VSSCC_PROVIDER_NAME)!="");
   }
   return(0);
}
...
void ctlscc_open_project.lbutton_up()
{
   ...
   if (machine()=='WINDOWS' && _haveVersionControl()) {
      //This is strictly a precaution.  We should never get here unless we
      //are running Intel Windows
      curSystem := getSCCSystemNameFromDialog();
      if ( p_caption == SCC_OPEN_PROJECT_BUTTON_CAPTION ) {
         promptForPath := true;

         int providerCapabilityFlags = _SccGetProviderCapabilities();
         promptForPath = ! (providerCapabilityFlags&SCC_CAP_GETPROJPATH);
         
         if (promptForPath && lowcase(curSystem)==lowcase(PERFORCE_NAME) ) {
            ...
            mou_hour_glass(false);
         }else if (lowcase(curSystem)=='clearcase') {
            status=show('-modal _textbox_form',
         ...
         }else if ((lowcase(curSystem)=='mks source integrity scc extension') ||
                   (lowcase(curSystem)=='mks scc integration')) {
            status=show('-modal _textbox_form',
            ...
            p_window_id = origwid;
            mou_hour_glass(false);
         }else if (lowcase(curSystem)=='starbase starteam') {
            status=show('-modal _textbox_form',
            ...
            p_window_id = origwid;
            mou_hour_glass(false);
         }else if (lowcase(curSystem)=='reliable software code co-op') {
               status=show('-modal _textbox_form',
            ...
            newproject := false;
            if (IsHarvestSCC('SCC:'curSystem) || IsPVCSSCC('SCC:'curSystem) ) {
            ...
            LocalPath := "";
            AllowChangePath := 1;
            if (newproject && IsHarvestSCC('SCC:'curSystem)) {
               AllowChangePath=0;
               if (_project_name=='') {
                  LocalPath=getcwd();
               }else{
         ...
         if (!status || status==VSSCC_E_PROJECTALREADYOPEN) {
            _ProjectSet_VCSProject(_ProjectHandle(),SCC_PREFIX:+curSystem':'_SccGetCurProjectInfo(VSSCC_PROJECT_NAME));
            ctlscc_projname.p_caption=curSystem;
            SetVCSLocalPath(_ProjectHandle(),curSystem,SCC_PREFIX:+curSystem':'_SccGetCurProjectInfo(VSSCC_LOCAL_PATH));
            SetVCSAuxPath(_ProjectHandle(),curSystem,_SccGetCurProjectInfo(VSSCC_AUX_PATH_INFO));
            status=_ProjectSave(_ProjectHandle());
            if (status) {
               _message_box(nls("Could not write version control information to project file %s1.\n\n%s2\n\nThis version control project will probably not be properly linked with the current SlickEdit project.",_project_name,get_message(status)));
            }
         ...
         if (_SccGetCurProjectInfo(VSSCC_PROJECT_NAME)!='') {
            p_caption=SCC_CLOSE_PROJECT_BUTTON_CAPTION;
         }
      }else if (p_caption==SCC_CLOSE_PROJECT_BUTTON_CAPTION) {
         ...
         if (_SccGetCurProjectInfo(VSSCC_PROJECT_NAME)=='') {
            p_caption=SCC_OPEN_PROJECT_BUTTON_CAPTION;
         }
      }
      ctlscc_projname.p_caption=MaybeNone(_SccGetCurProjectInfo(VSSCC_PROJECT_NAME));
      orig_view_id := 0;
      ...
      _SetVCSystemName(SCC_PREFIX:+curSystem,false);
      toolbarUpdateFilterList(_project_name);
...
static void enableSCCButtons(bool visible, bool enabled, bool systemInitialized)
{
   ctlscc_init.p_visible = visible;
   ctlscc_open_project.p_visible = visible;
   ctlscc_advanced.p_visible = visible;
   ctlsccproject_label.p_visible = visible;

   ctlscc_init.p_enabled = !systemInitialized;
   ctlscc_open_project.p_enabled = systemInitialized;
   ctlscc_advanced.p_enabled = systemInitialized;
   ctlscc_projname.p_enabled = systemInitialized;
}
