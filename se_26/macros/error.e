...
struct COLLECTION_ITEM {
   ...
   _str macro;
   _str sev;
   _str name;   // Name from the config, ex: "Pyline,pylint_warn".
};
...
static COLLECTION_ITEM gExclusionExpressions[]; // holds expressions that are "false positive", meaning
                                     ...
                                     // error line.
// Last item that matched an error.  Set by the error parsing function.
COLLECTION_ITEM gLastMatchItem;
...
static int common_find_error_file(bool &found_it,_str absoluteFilename, _str &filename,_str VslickErrorPath, _str add_include_path='',bool allow_prompting=true) {
   // The "julia" compiler encodings ~ in the path on unix.
   if (def_unix_expansion && pos('~',filename)) {
      filename2 := _unix_expansion(filename);
      if (file_exists(filename2)) {
         absoluteFilename=filename2;
      }
   }
   found_filename := "";
   found_it = false;
   ...
   for (;;) {
      if(file_exists(absoluteFilename:+try_ext)) {
         ...
         if (_isUnix() && !_isRelative(filename)) {
            // Some tools either truncate paths or run
            // in a chroot or container. Either case  produces absolute paths
            // that will not exist when we look for them.  Our best bet is
            // to search projects for the best match for the path as if it
            // were a relative path.  So remove the leading filesep so the
            // project code treats it as relative.
            name_to_check = substr(filename, 2);
         }

         // if not found, search for file in the current project and workspace
         if (!found_it) {
            if (allow_prompting) {
               found_filename=_ProjectWorkspaceFindFile(name_to_check:+try_ext);
            } else {
               found_filename=_ProjectWorkspaceFindFile(name_to_check:+try_ext, true, false, true);
            }
                        ...
                        _str lookingBackwards="") name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI|VSARG2_AUTO_DESELECT)
{
   ...
   if (_mfXMLOutputIsActive && _LanguageInheritsFrom("xml") || p_name == "ctloutput") {
      ...
      return 0;
   } else if (_mfMessageListIsActive || p_name == "_message_tree") {
...
{   
   ...
   for (hashIndex._makeempty(); status < 1;) {
      ...
      if (status) {
         ...
         gLastMatchItem = gLangExpressions[hashIndex];
      }
   }
   ...
   if (expressionThatMatched == "" && status < 1) {
      for (hashIndex._makeempty();;) {
         ...
         if (status) {
            ...
            gLastMatchItem = gAllExpressions[hashIndex];
            break;
         }
      }
   }
...
static void _errorre_add_property(int handle,int profile_node,_str folder_name,ERRORRE_INFO &info,int &last_position,int (&hash_position):[]) {
   ...
   */
   ...
   if (info.m_severity==null || strieq(info.m_severity,'<Auto>')) {
...
/**
 * Pops up a dialog box with information about which error 
 * expression matched the last matched error.  Useful for 
 * troubleshooting unwanted error matches, or error matches that 
 * are too broad. 
 */
_command void last_error_match_info() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_MDI)
{
   if (gLastMatchItem.name == '') {
      message("No previous error message matched.");
      return;
   }

   parse gLastMatchItem.name with auto folder ',' auto name;
   _message_box("Expression name "folder" -> "name:+
                "\nseverity: "gLastMatchItem.sev:+
                "\n\nregex: "gLastMatchItem.regex);
}
