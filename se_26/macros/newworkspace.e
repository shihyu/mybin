...
static const WORKSPACE_NEW_FORM_NAME= '_workspace_new_form';
...
static void _workspace_new_form_initial_alignment()
{
   ...
   sizeBrowseButtonToTextBox(ctlsourcedir.p_window_id, ctlBrowseSourceDir.p_window_id);
   ...
   // workspace tab
   sizeBrowseButtonToTextBox(ctlnew_workspace_dir.p_window_id, ctlBrowseWorkspace.p_window_id, 0,
                             ctlnew_workspace_name.p_x_extent);

}

static void resizeFileNew(int tabWidth, int tabHeight, int borderX, int borderY)
{
   ctlDocModes.p_height = tabHeight - ctlDocModes.p_y - 3*borderY;

   ctladd_to_project_name.p_width = tabWidth - ctladd_to_project_name.p_x - 2*borderX; 
   ctlfilename.p_width  = tabWidth  - ctlfilename.p_x - 2*borderX;
   ctlBrowsedir.p_x = tabWidth - 2*borderX - ctlBrowsedir.p_width;
   ctldirectory.p_width = tabWidth  - ctldirectory.p_x - ctlBrowsedir.p_width - 2*borderX;
   ctlencoding.p_width  = tabWidth  - ctlencoding.p_x - 2*borderX;
   ctllineformat.p_width  = tabWidth  - ctllineformat.p_x - 2*borderX;
}

static void resizeProjectNew(int tabWidth, int tabHeight, int borderX, int borderY)
{
   below_proj_tree_height := 0;
   if (ctlcustomize.p_visible || ctl_new_proj_wizard.p_visible) {
      below_proj_tree_height += borderY;
      below_proj_tree_height += ctlcustomize.p_height;
      below_proj_tree_height += borderY;
   }
   if (ctl_workspace_scope_frame.p_visible) {
      below_proj_tree_height += borderY;
      below_proj_tree_height += ctl_workspace_scope_frame.p_height;
      below_proj_tree_height += borderY;
   }

   if (ctlProjTree.p_visible) {
      ctlProjTree.p_height = tabHeight - ctlProjTree.p_y - below_proj_tree_height - borderY;
      ctlcustomize.p_y = ctlProjTree.p_y_extent + borderY;
      ctl_new_proj_wizard.p_y = ctlProjTree.p_y_extent + borderY;
      ctl_workspace_scope_frame.p_y = tabHeight - ctl_workspace_scope_frame.p_height - 3*borderY;
   } else {
      ctl_workspace_scope_frame.p_y = ctl_source_files.p_y;
   }

   _new_prjname.p_width = tabWidth - _new_prjname.p_x - 2*borderX; 
   ctlBrowseCD.p_x = tabWidth - 2*borderX - ctlBrowseCD.p_width;
   ctlProjectNewDir.p_width = tabWidth - ctlProjectNewDir.p_x - ctlBrowseCD.p_width - 2*borderX;
   ctlROLocation.p_width = tabWidth - ctlROLocation.p_x - 2*borderX; 
   ctlExecutableName.p_width = tabWidth - ctlExecutableName.p_x - 2*borderX; 
   ctl_source_files.p_width = tabWidth - ctl_source_files.p_x - 2*borderX;
   
   sourceWidth := ctl_source_files.p_width; 
   ctlBrowseSourceDir.p_x = sourceWidth - 2*borderX - ctlBrowseSourceDir.p_width;
   ctlsourcedir.p_width   = sourceWidth - ctlsourcedir.p_x - ctlBrowseSourceDir.p_width - 2*borderX;
   ctlinclude_filespecs.p_width = sourceWidth - ctlinclude_filespecs.p_x - 2*borderX; 
   ctlexclude_filespecs.p_width = sourceWidth - ctlexclude_filespecs.p_x - 2*borderX; 

}

static void resizeWorkspaceNew(int tabWidth, int tabHeight, int borderX, int borderY)
{
   ctlworkspace_type_list.p_height = tabHeight - ctlworkspace_type_list.p_y - 3*borderY;

   ctlnew_workspace_name.p_width  = tabWidth  - ctlnew_workspace_name.p_x - 2*borderX;
   ctlBrowseWorkspace.p_x = tabWidth - 2*borderX - ctlBrowseWorkspace.p_width;
   ctlnew_workspace_dir.p_width = tabWidth  - ctlnew_workspace_dir.p_x - ctlBrowseCD.p_width - 2*borderX;
}

void _workspace_new_form.on_resize()
{
   clientWidth  := _dx2lx(SM_TWIP,p_client_width);
   clientHeight := _dy2ly(SM_TWIP,p_client_height);
   borderX := ctl_gn_tab.p_x;
   borderY := ctl_gn_tab.p_y;
   buttonH := ctlok.p_height;
   bottomH := buttonH + 2*borderY;

   // enforce minimum form with and height
   minWidth  := 8*borderX + 2*ctlDocModes.p_width;
   minHeight := 5*borderY + ctl_source_files.p_y_extent + bottomH;
   p_active_form._set_minimum_size(minWidth, minHeight);

   ctl_gn_tab.p_width  = clientWidth - 2*borderX;
   ctl_gn_tab.p_height = clientHeight - 2*borderY - bottomH;

   buttonY := ctl_gn_tab.p_y_extent + borderY;
   ctlok.p_y = buttonY;
   ctlCancel.p_y = buttonY;
   ctlhelp.p_y = buttonY;
   ctldontshow.p_y = ctlok.p_y_extent - ctldontshow.p_height;

   tabWidth  := _dx2lx(SM_TWIP,ctl_gn_tab.p_client_width);
   tabHeight := _dy2ly(SM_TWIP,ctl_gn_tab.p_client_height);

   resizeFileNew(tabWidth, tabHeight, borderX, borderY);
   resizeProjectNew(tabWidth, tabHeight, borderX, borderY);
   resizeWorkspaceNew(tabWidth, tabHeight, borderX, borderY);
}
...
ctlProjTree.on_change(int reason)
{
   if (reason == CHANGE_SELECTED) {
      ...
      UpdateSourceDirLocation();

      // if they switch project types, attempt to restore the last wildcard
      // settings for that project type.
      escapedPackageName := stranslate(packageName, '_', ' ');
      escapedPackageName = stranslate(escapedPackageName, '_', ':');
      includeSpecs := _retrieve_value(WORKSPACE_NEW_FORM_NAME'.'ctlinclude_filespecs.p_name'-'escapedPackageName);
      excludeSpecs := _retrieve_value(WORKSPACE_NEW_FORM_NAME'.'ctlexclude_filespecs.p_name'-'escapedPackageName);
      doRecursive  := _retrieve_value(WORKSPACE_NEW_FORM_NAME'.'ctlrecursive.p_name'-'escapedPackageName);
      doWildcards  := _retrieve_value(WORKSPACE_NEW_FORM_NAME'.'ctlwildcard.p_name'-'escapedPackageName);

      // if there is no append/retrieve info for this project package name, try default settings
      if (includeSpecs == null || includeSpecs == "") includeSpecs = p_active_form._GetDialogInfoHt(ctlinclude_filespecs.p_name);
      if (excludeSpecs == null || excludeSpecs == "") excludeSpecs = p_active_form._GetDialogInfoHt(ctlexclude_filespecs.p_name);
      if (doRecursive  == null || doRecursive  == "") doRecursive  = p_active_form._GetDialogInfoHt(ctlrecursive.p_name);
      if (doWildcards  == null || doWildcards  == "") doWildcards  = p_active_form._GetDialogInfoHt(ctlwildcard.p_name);

      // set the items that we can set
      if (includeSpecs != null && includeSpecs != "") ctlinclude_filespecs.p_text = includeSpecs;
      if (excludeSpecs != null && excludeSpecs != "") ctlexclude_filespecs.p_text = excludeSpecs;
      if (doRecursive !=  null && isinteger(doRecursive)) ctlrecursive.p_value = doRecursive;
      if (doWildcards !=  null && isinteger(doWildcards)) ctlwildcard.p_value  = doWildcards;
   }
}
...
void ctlcustomize.lbutton_up()
{
   ...
   if(_find_formobj(WORKSPACE_NEW_FORM_NAME,'n')){
...
static void UpdateSourceDirLocation()
{
   // calculate the new source/working directory
   workingDir := ctlProjectNewDir.p_text;
   _maybe_append_filesep(workingDir);
   if (ctlCreateProjDir.p_enabled && ctlCreateProjDir.p_value) {
      workingDir :+= _new_prjname.p_text;
      _maybe_append_filesep(workingDir);
   }

   // update the source directory
   if (ctlsourcedir.p_enabled) {
      ctlsourcedir.p_text = workingDir;
   }
}
...
void ctlok.lbutton_up()
{
   ...
   creatingFromInvocationDirectory := (!ctlsourcedir.p_enabled && ctl_source_files.p_value);
   addingSourceFileDirectoryTree   := (ctl_source_files.p_enabled && ctl_source_files.p_value);
   ...
   if (ctl_gn_tab.p_ActiveTab==NEW_FILE_TAB) {
      ...
      _append_retrieve(ctladd_to_project, ctladd_to_project.p_value, WORKSPACE_NEW_FORM_NAME:+'.ctladd_to_project');
      _nocheck _control ctlshowproperties;
      _append_retrieve(ctlshowproperties, ctlshowproperties.p_value, WORKSPACE_NEW_FORM_NAME:+'.ctlshowproperties');
      _nocheck _control ctllineformat;
      _append_retrieve(ctllineformat, ctllineformat.p_text, WORKSPACE_NEW_FORM_NAME:+'.ctllineformat');
      ...
      packageName := OTHER_PROJ_PACK_NAME;
      initMacroBefore := "";
      ...
      if (_new_prjname.p_enabled && _new_prjname.p_text == '') {
      ...
      SourceFileWorkingDirectory:="";
      projectName := "";
      STRARRAY filespecs;
      STRARRAY excludeFilespecs;
      if ( addingSourceFileDirectoryTree ) {
         SourceFileWorkingDirectory = ctlsourcedir.p_text;
      ...
      showprojectprops:=ctlshowproperties.p_value!=0;
      ...
      escapedPackageName := stranslate(packageName, '_', ' ');
      escapedPackageName = stranslate(escapedPackageName, '_', ':');
      _append_retrieve(ctlinclude_filespecs, ctlinclude_filespecs.p_text, WORKSPACE_NEW_FORM_NAME'.'ctlinclude_filespecs.p_name'-'escapedPackageName);
      _append_retrieve(ctlexclude_filespecs, ctlexclude_filespecs.p_text, WORKSPACE_NEW_FORM_NAME'.'ctlexclude_filespecs.p_name'-'escapedPackageName);
      _append_retrieve(ctlrecursive,         ctlrecursive.p_value,        WORKSPACE_NEW_FORM_NAME'.'ctlrecursive.p_name'-'escapedPackageName);
      _append_retrieve(ctlwildcard,          ctlwildcard.p_value,         WORKSPACE_NEW_FORM_NAME'.'ctlwildcard.p_name'-'escapedPackageName);

      ctlinclude_filespecs._append_retrieve(ctlinclude_filespecs, ctlinclude_filespecs.p_text);
      ctlexclude_filespecs._append_retrieve(ctlexclude_filespecs, ctlexclude_filespecs.p_text);
      _append_retrieve(ctlrecursive,       ctlrecursive.p_value,       WORKSPACE_NEW_FORM_NAME:+'.ctlrecursive'      );
      _append_retrieve(ctlwildcard,        ctlwildcard.p_value,        WORKSPACE_NEW_FORM_NAME:+'.ctlwildcard'       );
      _append_retrieve(ctlsymlinks,        ctlsymlinks.p_value,        WORKSPACE_NEW_FORM_NAME:+'.ctlsymlinks'       );
      _append_retrieve(ctlcustomfolders,   ctlcustomfolders.p_value,   WORKSPACE_NEW_FORM_NAME:+'.ctlcustomfolders'  );
      _append_retrieve(ctldirectoryfolder, ctldirectoryfolder.p_value, WORKSPACE_NEW_FORM_NAME:+'.ctldirectoryfolder');
      _append_retrieve(ctlCreateProjDir,   ctlCreateProjDir.p_value,   WORKSPACE_NEW_FORM_NAME:+'.ctlCreateProjDir'  );
      _append_retrieve(ctl_source_files,   ctl_source_files.p_value,   WORKSPACE_NEW_FORM_NAME:+'.ctl_source_files'  );
      _append_retrieve(0,                  packageName,                WORKSPACE_NEW_FORM_NAME:+'.ctlProjTree'       );
      ...
      if (addingSourceFileDirectoryTree && (!creatingFromInvocationDirectory || showprojectprops)) {
         project_handle := _ProjectHandle(projectName);
         if (createsubfolders || directoryfolder) {
            AutoFolders:=_ProjectGet_AutoFolders(project_handle);
            if (!_ProjectGet_FilesPresent(project_handle) && (AutoFolders=='' || strieq(AutoFolders,VPJ_AUTOFOLDERS_CUSTOMVIEW))) {
               /* There are no files in this project and it's in custom view.
                  Clear out the filter filters.
                */
               files_node:=_ProjectGet_FilesNode(project_handle);
               if (files_node > 0) {
                  _xmlcfg_delete(project_handle,files_node,true);
               }
         ...
         addSourceFilesToProject(projectName, project_handle, 
                                 SourceFileWorkingDirectory,
                                 filespecs, excludeFilespecs,
                                 recursive, symlinks, add_as_wildcard,
                                 createsubfolders, directoryfolder);
      }

      if (!addingSourceFileDirectoryTree || showprojectprops) {
         show_project_properties_files_tab();
      }
...
static void addSourceFilesToProject(_str projectName, 
                                    int project_handle, 
                                    _str SourceFileWorkingDirectory, 
                                    _str (&includeList)[], 
                                    _str (&excludeList)[], 
                                    bool recursive,
                                    bool follow_symlinks, 
                                    bool add_as_wildcard, 
                                    bool create_subfolders,
                                    bool directory_folder)
{
   path := SourceFileWorkingDirectory;
   folderName := path;
   if (length(path)>1 && !(_isWindows() && length(path)==3 && substr(path,2,1)==':')) {
      folderName= _strip_filename(substr(path,1,length(path)-1),'P');
   }

   OrigProjectFileList := 0;
   _project_before_modify_files(_project_name,project_handle,OrigProjectFileList);

   is_custom_view := strieq(_ProjectGet_AutoFolders(project_handle),VPJ_AUTOFOLDERS_CUSTOMVIEW);
   if (add_as_wildcard) {
      if (directory_folder) {
         _projecttbAddDirectoryToProject(onlyUpdateProjectFile:true, 
                                         projectName, project_handle, 
                                         FolderNode: -1, 
                                         path, includeList, excludeList, 
                                         recursive, follow_symlinks, create_subfolders);
      } else {
         _projecttbAddWildcardsToProject(onlyUpdateProjectFile:true, 
                                         projectName, project_handle,
                                         FolderNode: -1, 
                                         path, includeList, excludeList, 
                                         recursive, follow_symlinks, create_subfolders);
      ...
      _projecttbAddTreeToProject(onlyUpdateProjectFile:true,
                                 FolderNode: -1,
                                 projectName, project_handle, 
                                 path, includeList, excludeList, 
                                 recursive, follow_symlinks, 
                                 (is_custom_view)?create_subfolders:false);
      mou_hour_glass(false);
   }

   project_dir := _strip_filename(projectName, 'n');
   if (SourceFileWorkingDirectory != "") {
      if (file_eq(SourceFileWorkingDirectory, project_dir)) {
         _ProjectSet_WorkingDir(project_handle,".");
      } else {
         _ProjectSet_WorkingDir(project_handle, relative(SourceFileWorkingDirectory, project_dir));
      }
   }
   ...
   _clearProjectFileListCache(_workspace_filename, projectName, true);
...
void ctlok.on_create(_str option='',_str (&Files)[]=null,_str workspaceFilename="")
{
   ...
   if ( workspaceFilename=="" ) {
      ...
      //ctlshowproperties.p_visible=false;
      //ctlcustomfolders.p_visible=false;
   } else {
      ...
      ctlsourcedir.p_enabled=false;
      ctlBrowseSourceDir.p_enabled=false;
      //if (!ctlProjTree.p_visible) {
      ...
      //      ctlshowproperties.p_y    -= adjust_y; 
      //   } 
      //}
   }
   if (option=='p') {
      ...
      // restore whether or not to create a project directory
      retrieveValue := ctlCreateProjDir._retrieve_value();
      if (ctlCreateProjDir.p_enabled && retrieveValue != null && isinteger(retrieveValue)) {
         ctlrecursive.p_value = retrieveValue;
      }
      ...
      retrieveValue = ctl_source_files._retrieve_value();
      if (ctl_source_files.p_enabled && retrieveValue != null && isinteger(retrieveValue)) {
         ctl_source_files.p_value = retrieveValue;
      }
      ...
      ctlinclude_filespecs._retrieve_value();
      ctlexclude_filespecs._retrieve_value();
      orig_ctlinclude_filespecs := ctlinclude_filespecs.p_text;
      orig_ctlexclude_filespecs := ctlexclude_filespecs.p_text;
      ctlinclude_filespecs.add_filetypes_to_combo();
      includeSpecs := _default_option(VSOPTIONZ_DIR_PROJECT_INCLUDES);
      excludeSpecs := _default_option(VSOPTIONZ_DIR_PROJECT_EXCLUDES);
      if (includeSpecs != "") {
         ctlinclude_filespecs._cbset_text(_default_option(VSOPTIONZ_DIR_PROJECT_INCLUDES));
      }
      if (excludeSpecs != "") {
         ctlexclude_filespecs._cbset_text(_default_option(VSOPTIONZ_DIR_PROJECT_EXCLUDES));
      }
      ...
      // if not creating a project form an invocation directory,
      // retrieve the last retrieved values for the source file options.
      if ( workspaceFilename=="" ) {
         if (orig_ctlinclude_filespecs != "") {
            ctlinclude_filespecs._cbset_text(orig_ctlinclude_filespecs);
         }
         if (orig_ctlexclude_filespecs != "") {
            ctlexclude_filespecs._cbset_text(orig_ctlexclude_filespecs);
         }
         retrieveValue = ctlrecursive._retrieve_value();
         if (retrieveValue != null && isinteger(retrieveValue)) {
            ctlrecursive.p_value = retrieveValue;
         }
         retrieveValue = ctlwildcard._retrieve_value();
         if (retrieveValue != null && isinteger(retrieveValue)) {
            ctlwildcard.p_value = retrieveValue;
         }
         retrieveValue = ctlsymlinks._retrieve_value();
         if (retrieveValue != null && isinteger(retrieveValue)) {
            ctlsymlinks.p_value = retrieveValue;
         }
         retrieveValue = ctlcustomfolders._retrieve_value();
         if (retrieveValue != null && isinteger(retrieveValue)) {
            ctlcustomfolders.p_value = retrieveValue;
         }
         retrieveValue = ctldirectoryfolder._retrieve_value();
         if (retrieveValue != null && isinteger(retrieveValue)) {
            ctldirectoryfolder.p_value = retrieveValue;
         }
      }

      p_active_form._SetDialogInfoHt(ctlinclude_filespecs.p_name, ctlinclude_filespecs.p_text);
      p_active_form._SetDialogInfoHt(ctlexclude_filespecs.p_name, ctlexclude_filespecs.p_text);
      p_active_form._SetDialogInfoHt(ctlwildcard.p_name,          ctlwildcard.p_value);
      p_active_form._SetDialogInfoHt(ctlrecursive.p_name,         ctlrecursive.p_value);
      //_SetDialogInfoHt(ctlcustomfolders.p_name,     ctlcustomfolders.p_value);
      //_SetDialogInfoHt(ctldirectoryfolder.p_name,   ctldirectoryfolder.p_value);

      project_type := _retrieve_value(WORKSPACE_NEW_FORM_NAME'.'ctlProjTree.p_name);
...
{
   button := _find_object(WORKSPACE_NEW_FORM_NAME:+'.ctl_new_proj_wizard');
   if (button < 0) return;
...
static _str GetWorkspaceRoot(bool trimLastDirectoryPath=true)
{
   if (_workspace_filename=='') {
      return('');
   }
   NewWorkspaceDir := _GetWorkspaceDir();
   if (!trimLastDirectoryPath) {
...
static void initialize_project_tab(_str Files[]=null,bool forcedDirectoryLocation=false)
{
   ...
   if (_haveBuild()) {
      ...
      // restore the last value of show project properties option
      retrieveValue := ctlshowproperties._retrieve_value();
      if (retrieveValue != null && isinteger(retrieveValue)) {
         ctlshowproperties.p_value = retrieveValue;
      } else {
         ctlshowproperties.p_value = 0;
      }
      ...
      label1.p_x = _new_prjname.p_x = ctlCreateProjDir.p_x = ctlLocationLabel.p_x =
         ctlProjectNewDir.p_x = ctlROLocation.p_x = ctllabel4.p_x;
      ctlBrowseCD.p_x -= xDiff;

      ctl_source_files.p_y = ctlExecutableLabel.p_y;
      ctl_workspace_scope_frame.p_y = ctl_source_files.p_y;
   ...
   if (ctlcreate_new_workspace.p_value) {
      ctlProjectNewDir.p_text=GetWorkspaceRoot(ctlCreateProjDir.p_value != 0);
      UpdateSourceDirLocation();
   }
...
void ctlcreate_new_workspace.lbutton_up()
{
   ...
   NewWorkspace := (ctladd_to_workspace.p_value==0);
