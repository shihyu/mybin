...
static typeless PUSER_IN_ON_CHANGE(...) {
   if (arg()) ctlremove.p_user=arg(1);
   return ctlremove.p_user;
}
...
void _create_plugin_form.on_resize() {
   ...
   ctladd_profiles.p_y=ctlmove_down.p_y=ctlremove.p_y=ctladd_files.p_y_extent+100;
}
...
void ctladd_files.lbutton_up() {
   ...
   for (;;) {
      ...
      NewIndex:=ctlfiles._TreeSearch(RelationIndex,name,_fpos_case);
      if (NewIndex < 0) {
