 ...
 * </pre>
 * The {@code callback_name} argument specifies the address of
 ...
 * callback is {@code null}, the normal processing for that
 ...
 *
 * @return {@code true} if the event is short-circuited
 *         or the dialog is closed.
 *         {@code false} for the normal case.
 */
                      ...
                      _str message_text=null)
{
      ...
      if (n > 0) {
         if (flags & SL_COLWIDTH) {
            ...
            for (i = 0; i < n; i++) {
               ...
               if (autosize_width < header_width) {
                  cwidths[i] = header_width;
               } else {
         ...
         for (i = 0; i < n; i++) {
            ...
            if (cflags[i] & TREE_BUTTON_IS_DATETIME) {
               se.datetime.DateTime dateTime;
               item_index := ctl_tree._TreeGetFirstChildIndex(TREE_ROOT_INDEX);
               while (item_index > 0) {
                  date_column := ctl_tree._TreeGetCaption(item_index, i);
                  if (date_column != "" && isuinteger(date_column) && date_column >= 10000000000000) {
                     dateTime.fromTimeB(date_column);
                     ...
                     ctl_tree._TreeSetDateTime(item_index,i,
                                               year,month,day,
                                               hour,minute,second);
                  }
                  item_index = ctl_tree._TreeGetNextSiblingIndex(item_index);
               }
            }
         }
      }
   ...
   if (flags & SL_COLWIDTH) {
      ...
      left_width := ctl_tree._TreeGetLeftPadding(0);
