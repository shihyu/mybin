...
defload()
{
   ...
   if (_pic_symbol<=0) {
      _pic_symbol=_update_picture(-1,'_f_symbol.svg');
      if (_pic_symbol>0) {
   ...
   if ( _pic_symbold_overlay<=0 ) {
      _pic_symbold_overlay  = load_picture(-1, "_f_overlay_modified.svg@xA");
      if (_pic_symbold_overlay>0) {
         set_name_info(_pic_symbold_overlay,"This symbol is modified");
      }
   }
   if (_pic_symbolm_overlay<=0) {
      _pic_symbolm_overlay=_update_picture(-1,'_f_overlay_symbol_delete.svg');
      if (_pic_symbolm_overlay>0) {
         set_name_info(_pic_symbolm_overlay,"This symbol does not exist");
      }
   }
   if (_pic_symbolp_overlay<=0) {
      _pic_symbolp_overlay=_update_picture(-1,'_f_overlay_symbol_add.svg');
      if (_pic_symbolp_overlay>0) {
         set_name_info(_pic_symbolp_overlay,"This symbol only exists in this path");
      }
   }
   if (_pic_symbol_viewed_overlay<=0) {
      _pic_symbol_viewed_overlay=_update_picture(-1,'_f_overlay_viewed.svg');
      if (_pic_symbol_viewed_overlay>0) {
         set_name_info(_pic_symbol_viewed_overlay,"This symbol has been viewed");
      }
   }
   if (_pic_fldopenp<=0) {
   ...
   if (_pic_symbolmoved_overlay<=0) {
      _pic_symbolmoved_overlay=_update_picture(-1,'_f_overlay_symbol_moved.svg');
      if (_pic_symbolmoved_overlay>0) {
         set_name_info(_pic_symbolmoved_overlay,"These symbols match, but have moved");
      }
   }
   rc=0;
}
...
bool _DiffTreatAsBinaryFile(_str filename)
{
   return _IsBinaryModeFile(filename) || (_file_size(filename) >= (def_diff_force_bin_compare_limit*1024));
}
...
//      say('fillInThreadedMFDiff table._length()='table._length());
   ...
   foreach (curItem in table) {
      ...
      bmindex1 := 0;
      bmindex2 := 0;
      bmindex1 = bmindex2 = _pic_sm_file;
      INTARRAY overlays1;
      INTARRAY overlays2;
      nodeFlags := 0;
      ...
      case MFDIFF_STATUS_FILES_DIFFERENT:
         overlays1 :+= _pic_file_mod_overlay;
         overlays2 :+= _pic_file_mod_overlay;
         if ( _DiffTreatAsBinaryFile(curItem.filename1) || _DiffTreatAsBinaryFile(curItem.filename2) ) {
            overlays1 :+= _pic_file_bin_diff_overlay;
            overlays2 :+= _pic_file_bin_diff_overlay;
         }
      ...
      case MFDIFF_STATUS_FILE_ONLY_IN_PATH1:
         {
            overlays1 :+= _pic_file_mfdiff_add_overlay;
            name := name_name(_pic_file_mfdiff_add_overlay);
            overlays2 :+= _pic_file_deleted_overlay;
            if (!(GMFDiffViewOptions&DIFF_VIEW_MISSING_FILES2)) {
               nodeFlags = TREENODE_HIDDEN;
            } else {
               folderNodeFlags = 0;
      ...
      case MFDIFF_STATUS_FILE_ONLY_IN_PATH2:
         {
            overlays1 :+= _pic_file_deleted_overlay;
            overlays2 :+= _pic_file_mfdiff_add_overlay;
            name := name_name(_pic_file_mfdiff_add_overlay);
            if (!(GMFDiffViewOptions&DIFF_VIEW_MISSING_FILES1)) {
               nodeFlags = TREENODE_HIDDEN;
            } else {
      ...
      if ( def_mfdiff_functions==1 &&
           !_TreeHaveOverlay(overlays1, _pic_file_mfdiff_add_overlay) &&
           !_TreeHaveOverlay(overlays1, _pic_file_deleted_overlay) &&
      ...
      index1 := tree1._TreeAddItem(parent1,
                                   ...
                                   nodeFlags,
                                   date1:+ASCII1);
      tree1._TreeSetOverlayBitmaps(index1, overlays1);
      ...
      index2:= tree2._TreeAddItem(parent2,
                                  ...
                                  nodeFlags,
                                  date2:+ASCII1);
      tree2._TreeSetOverlayBitmaps(index2, overlays2);
      ++numFiles;
...
bool _IsFunctionIndex(int index)
{
   ...
   // Only have to look for _pic_symbol, others are overlays now
   if ( bm1==_pic_symbol ) {
      return(true);
   }
...
bool _MFDiffHaveThisOverlay(INTARRAY &overlays,int overlayIndex)
{
   len := overlays._length();
   for ( i:=0; i<len; ++i ) {
      if ( overlays[i]==overlayIndex ) {
         return true;
      }
   }
   return false;
...
static void EnableButtons()
{
   ...
   tree2_bmindex2 := 0;
   flags := 0;
   tree1._TreeGetInfo(tree1index,junk,tree1_bmindex1,tree1_bmindex2,flags,auto ln1, -1, auto overlays1);
   tree2._TreeGetInfo(tree2index,junk,tree2_bmindex1,tree2_bmindex2,flags,auto ln2, -1, auto overlays2);
   ...
   if (tree1._IsFunctionIndex(tree1index)) {
      ...
      if ( _MFDiffHaveThisOverlay(overlays1, _pic_symbold_overlay) ) {
         DiffFunctionSetup(true);
      } else if ( _MFDiffHaveThisOverlay(overlays1, _pic_symbolm_overlay) ) {
         MissingFunctionSetup(true);
      }else if ( _MFDiffHaveThisOverlay(overlays1, _pic_symbolp_overlay) ) {
         OnlyFunctionSetup(true);
      }else if ( _MFDiffHaveThisOverlay(overlays1, _pic_symbolmoved_overlay) ) {
   ...
   haveMissing := false;
   haveOnly := false;
   len := overlays1._length();
   matching := len==0;
   if ( !matching ) {
      for ( i:=0; i<len; ++i ) {
         overlayIndex1 := overlays1[i];
         if ( overlayIndex1 == _pic_file_mod_overlay ) {
            haveModified = true;
         } else if ( overlayIndex1 == _pic_file_deleted_overlay ) {
            haveMissing = true;
         } else if ( overlayIndex1 == _pic_file_mfdiff_add_overlay ) {
            haveOnly = true;
         }
      }
   }
...
void tree1.on_change(int reason, int index=0)
{
   ...
   case CHANGE_SELECTED:
      ...
      INTARRAY overlays2;
      if (p_window_id==tree1) {
         _TreeGetOverlayBitmaps(cur_index, overlays1);
         tree2._TreeCurLineNumber(_TreeCurLineNumber());
         ...
         index2 := tree2._TreeCurLineNumber();
         tree2._TreeGetOverlayBitmaps(index2, overlays2);
         tree1._TreeCurLineNumber();
         if (NumSelected1 && NumSelected2) {
...
int _UpdateFileBitmaps(_str filename1,int &index1,int Tree1WID,
                       ...
                       int filesAlreadyMatch=0,bool DoEnableButtons=true,
                       DIFF_SHOWING_INFO *pMiscInfo=null)
{
   pindex := 0;
   filesMatch := 0;
   if (!index1) {
   ...
   Tree1WID._TreeGetInfo(index1,auto stateUnused,auto bmIndex);
   isSymbol := bmIndex==_pic_symbol;
   ...
   if (!filesAlreadyMatch) {
      ...
      if (def_diff_flags&DIFF_LEADING_SKIP_COMMENTS) {
         NCSStatus1=_GetNonCommentSeek(viewid1,seek1);
         NCSStatus2=_GetNonCommentSeek(viewid2,seek2);
      }

      lineRange1 := "";
      lineRange2 := "";
      if ( isSymbol ) {
         userInfo1 := Tree1WID._TreeGetUserInfo(index1);
         parse userInfo1 with auto firstLineNum1 ',' auto lastLineNum1;
         lineRange1 = firstLineNum1:+'-':+lastLineNum1;
         
         userInfo2 := Tree2WID._TreeGetUserInfo(index2);
         parse userInfo2 with auto firstLineNum2 ',' auto lastLineNum2;
         lineRange2 = firstLineNum2:+'-':+lastLineNum2;
      }
      ...
      if (NCSStatus1==STRING_NOT_FOUND_RC &&
         ...
         if ( !(def_diff_flags&FORCE_PROCESS_OPTIONS) &&
            ...
            info.options = def_diff_flags|DIFF_OUTPUT_BOOLEAN|DIFF_NO_BUFFER_SETUP|DIFF_DONT_STORE_RESULTS;
            info.numDiffOutputs = 0;
            info.isSourceDiff = false;
            info.loadOptions = def_load_options;
            info.gaugeWID = 0;
            info.maxFastFileSize = def_max_fast_diff_size;
            info.lineRange1 = lineRange1;
            info.lineRange2 = lineRange2;
            ...
            DiffFreeAllColorInfo(info.viewID1.p_buf_id);
            DiffFreeAllColorInfo(info.viewID2.p_buf_id);
         }
      }
      if (inmem1) {
   ...
   state2 := 0;
   bmindex := 0;
   Tree1WID._TreeGetInfo(index1,state1,auto curBMIndex,auto curBMIndexUnused,auto moreFlags,auto lineNumber,-1,auto overlays1);
   Tree2WID._TreeGetInfo(index2,state2,curBMIndex,curBMIndexUnused,moreFlags,lineNumber,-1,auto overlays2);
   if (!filesMatch) {
      _TreeAddOverlay(overlays1,_pic_symbold_overlay);
      _TreeAddOverlay(overlays2,_pic_symbold_overlay);
      if (isSymbol) {
         bmindex=_pic_symbol;
      } else {
         bmindex=_pic_filed2;
      ...
      overlays1._makeempty();
      overlays2._makeempty();
      if (isSymbol) {
         bmindex=_pic_symbol;
      } else {
         bmindex=_pic_file_match;
   ...
   _TreeAddOverlay(overlays1,_pic_symbol_viewed_overlay);
   _TreeAddOverlay(overlays2,_pic_symbol_viewed_overlay);
   flags := 0;
   ...
   Tree1WID._TreeSetInfo(index1,state1,bmindex,bmindex,flags,1,-1,overlays1);
   Tree2WID._TreeSetInfo(index2,state2,bmindex,bmindex,flags,1,-1,overlays2);
...
static void getDisplayNumberAndUnit(long size,long &displayNumber,_str &unitLabel)
{  
   static _str TB;
   static _str GB;
   static _str MB;
   static _str KB;
   if ( TB=="" ) eval_exp(TB,"2**40",10); 
   if ( GB=="" ) eval_exp(GB,"2**30",10); 
   if ( MB=="" ) eval_exp(MB,"2**20",10); 
   if ( KB=="" ) eval_exp(KB,"2**10",10); 
   if ( size > TB ) {
      displayNumber = size >> 40;
      unitLabel = "TB";
   } else if ( size > GB ) {
      displayNumber = size >> 30;
      unitLabel = "GB";
   } else if ( size > MB ) {
      displayNumber = size >> 20;
      unitLabel = "MB";
   } else if ( size > KB ) {
      displayNumber = size >> 10;
      unitLabel = "KB";
   }
}
   ...
   case COPY_SYMBOL_RIGHT_CAPTION:
      {
         ...
         sourceTreeWID._TreeGetInfo(curindex,state,bm1,0,auto moreFlags,auto lineNumber,-1,auto overlays1);
         mou_hour_glass(true);
         if ( _MFDiffHaveThisOverlay(overlays1,_pic_symbold_overlay) ) {
   ...
   case DIFF_CAPTION2:
      {
         ...
         if (!tree1._IsFunctionIndex(curindex1)) {
            ...
            size1 := _file_size(filename1);
            size2 := _file_size(filename2);
            if ( size1 > (def_diff_force_bin_compare_limit*1024) ||
                 size2 > (def_diff_force_bin_compare_limit*1024) ) {
               size := max(size1,size2);
               getDisplayNumberAndUnit(size,auto displayNum,auto unitLabel);
               result = _message_box(nls("You are about to diff a file greater than %s %s\n\nContinue?",displayNum,unitLabel),"",MB_YESNO);
               if ( result != IDYES ) return;
            }
            fid=p_active_form;
            ...
            _mdi.p_child.diff(iniRestoreOption' -RegisterAsMFDChild 'fid' -nomapping  -RefreshTagsClose 'curindex1' 'curindex2' 'AutoCloseOption' 'SourceDiffOption' '_maybe_quote_filename(filename1)' '_maybe_quote_filename(filename2));
            break;
         }else{
            fid=p_active_form;
            ...
            _str AutoCloseOption=(def_diff_edit_flags&DIFFEDIT_AUTO_CLOSE)?' -autoclose ':'';
            _mdi.p_child.diff(iniRestoreOption' -range1:'range1' -range2:'range2' -RefreshTagsClose 'pindex1' 'pindex2' -RegisterAsMFDChild 'fid' -nomapping 'AutoCloseOption' '_maybe_quote_filename(Path1:+File1)' '_maybe_quote_filename(Path2:+File2));
            break;
         }
      }
   case VIEW_CAPTION:
...
void tree1.rbutton_up,context()
{
   rclickmenu();
}

void tree2.rbutton_up,context()
{
   rclickmenu();
}
      ...
      if (_last_char(cap1)==FILESEP) {
         ...
         if (old_file_date1!=file_date1 ||
            ...
            if (!(def_diff_flags&FORCE_PROCESS_OPTIONS)&&
               ...
               DIFF_INFO info;
               info.viewID1 = File1ViewId;
               info.viewID2 = File2ViewId;
               info.options = flags|DIFF_NO_BUFFER_SETUP;
               info.numDiffOutputs = 0;
               info.isSourceDiff = false;
               info.loadOptions = def_load_options;
               info.gaugeWID = 0;
...
static void ReplaceFiles(int index,int bmOrOverlayIndex)
{
   UnhidParents := 0;
   state := 0;
   curbmindex := 0;
   ...
   found := false;
   for (;index>=0;) {
      _TreeGetInfo(index,state,curbmindex,curbmindex,flags,auto lineNumber,-1,auto overlays);
      ...
      if ((flags&TREENODE_HIDDEN) &&
          (curbmindex==bmOrOverlayIndex || (bmOrOverlayIndex<0 && manuallyhidden==1)) ) {
         found = true;
      } else {
         len := overlays._length();
         for ( i := 0; i<len ;++i ) {
            if ( overlays[i]==bmOrOverlayIndex ) {
               found = true;
               break;
            }
         }
      ...
      if (cindex>=0 &&
         ...
         ReplaceFiles(cindex,bmOrOverlayIndex);
      }
      index=_TreeGetNextSiblingIndex(index);
   }
}

static void ReplaceFunctions(int index,int bmOrOverlayIndex)
{
   UnhidParents := 0;
   state := 0;
   curbmindex := 0;
   ...
   for (;index>=0;) {
      ...
      if ( bmOrOverlayIndex==null ) {
         ...
         if (_IsFunctionIndex(index) &&
             (flags&TREENODE_HIDDEN) &&
             _TreeHaveOverlay(overlays,bmOrOverlayIndex)) {
            showIndex = true;
         }
      }
      if ( showIndex ) {
      ...
      cindex=_TreeGetFirstChildIndex(index);
      if (cindex>=0) ReplaceFunctions(cindex,bmOrOverlayIndex);
      index=_TreeGetNextSiblingIndex(index);
   }
}

static void HideFiles(int index,int bmOrOverlayIndex)
{
   state := 0;
   curbmindex := 0;
   flags := 0;
   ...
   for (;index>=0;) {
      _TreeGetInfo(index,state,curbmindex,curbmindex,flags,auto lineNumber,-1,auto overlays);
      if (!(flags&TREENODE_HIDDEN) && curbmindex==bmOrOverlayIndex) {
         found = true;
      } else {
         len := overlays._length();
         for ( i := 0; i<len ;++i ) {
      ...
      if ( found ) {
         _TreeSetInfo(index,state,curbmindex,curbmindex,flags|TREENODE_HIDDEN);
         cindex=_TreeGetFirstChildIndex(index);
         if (cindex>=0) HideFiles(cindex,bmOrOverlayIndex);
      }
      found = false;
      cindex=_TreeGetFirstChildIndex(index);
      if (cindex>=0) HideFiles(cindex,bmOrOverlayIndex);
...
static void HideFunctions(int index,int bmOrOverlayIndex)
{
   for (;index>=0;) {
      _TreeGetInfo(index,auto state,auto bm1,auto bm2,auto flags,auto lineNumber,-1,auto overlays);

      bool hideIndex = false;
      if (_IsFunctionIndex(index) && !(flags&TREENODE_HIDDEN) ) {
         if ( bmOrOverlayIndex==null ) {
            if ( overlays._length() ==0  ) {
               hideIndex = true;
            }
            ...
            len := overlays._length();
            for ( i:=0;i<len;++i ) {
               if ( _TreeHaveOverlay(overlays,bmOrOverlayIndex) ) {
                  hideIndex = true;
               }
            }
            if ( bm1 == bmOrOverlayIndex ) {
               hideIndex = true;
            }
         }
         if ( hideIndex ) {
      ...
      cindex := _TreeGetFirstChildIndex(index);
      if ( cindex>=0 ) HideFunctions(cindex, bmOrOverlayIndex);
      index=_TreeGetNextSiblingIndex(index);
   }
}
...
_command void MFDiffCommand(_str cmdName='') name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   case 'hdifferentfunctions':
      ...
      if (GMFDiffViewOptions&DIFF_VIEW_DIFFERENT_SYMBOLS) {
         ...
         tree1.HideFunctions(TREE_ROOT_INDEX,_pic_symbold_overlay);
         tree2.HideFunctions(TREE_ROOT_INDEX,_pic_symbold_overlay);
      }else{
         GMFDiffViewOptions|=DIFF_VIEW_DIFFERENT_SYMBOLS;
         tree1.ReplaceFunctions(TREE_ROOT_INDEX,_pic_symbold_overlay);
         tree2.ReplaceFunctions(TREE_ROOT_INDEX,_pic_symbold_overlay);
      }
   ...
   case 'hmatchingfunctions':
      ...
      if (GMFDiffViewOptions&DIFF_VIEW_MATCHING_SYMBOLS) {
         ...
         tree1.HideFunctions(TREE_ROOT_INDEX,null);
         tree2.HideFunctions(TREE_ROOT_INDEX,null);
      }else{
         GMFDiffViewOptions|=DIFF_VIEW_MATCHING_SYMBOLS;
         tree1.ReplaceFunctions(TREE_ROOT_INDEX,null);
         tree2.ReplaceFunctions(TREE_ROOT_INDEX,null);
      }
   ...
   case 'hmissing1':
      ...
      if (GMFDiffViewOptions&DIFF_VIEW_MISSING_FILES1) {
         ...
         tree2.HideFiles(TREE_ROOT_INDEX,_pic_file_mfdiff_add_overlay);
         tree1.HideFiles(TREE_ROOT_INDEX,_pic_file_deleted_overlay);
         tree2.HideFiles(TREE_ROOT_INDEX,_pic_fldopenp);
         tree1.HideFiles(TREE_ROOT_INDEX,_pic_fldopenm);
         ...
         GMFDiffViewOptions|=DIFF_VIEW_MISSING_FILES1;
         tree2.ReplaceFiles(TREE_ROOT_INDEX,_pic_file_mfdiff_add_overlay);
         tree1.ReplaceFiles(TREE_ROOT_INDEX,_pic_file_deleted_overlay);
   ...
   case 'hmissingfunctions1':
      ...
      if (GMFDiffViewOptions&DIFF_VIEW_MISSING_SYMBOLS1) {
         ...
         tree2.HideFunctions(TREE_ROOT_INDEX,_pic_symbolp_overlay);
         tree1.HideFunctions(TREE_ROOT_INDEX,_pic_symbolm_overlay);
      }else{
         GMFDiffViewOptions|=DIFF_VIEW_MISSING_SYMBOLS1;
         tree2.ReplaceFunctions(TREE_ROOT_INDEX,_pic_symbolp_overlay);
         tree1.ReplaceFunctions(TREE_ROOT_INDEX,_pic_symbolm_overlay);
      }
   ...
   case 'hmissing2':
      ...
      if (GMFDiffViewOptions&DIFF_VIEW_MISSING_FILES2) {
         ...
         tree1.HideFiles(TREE_ROOT_INDEX,_pic_file_mfdiff_add_overlay);
         tree2.HideFiles(TREE_ROOT_INDEX,_pic_file_deleted_overlay);
         tree1.HideFiles(TREE_ROOT_INDEX,_pic_fldopenp);
         tree2.HideFiles(TREE_ROOT_INDEX,_pic_fldopenm);
         ...
         GMFDiffViewOptions|=DIFF_VIEW_MISSING_FILES2;
         tree1.ReplaceFiles(TREE_ROOT_INDEX,_pic_file_mfdiff_add_overlay);
         tree2.ReplaceFiles(TREE_ROOT_INDEX,_pic_file_deleted_overlay);
   ...
   case 'hmissingfunctions2':
      ...
      if (GMFDiffViewOptions&DIFF_VIEW_MISSING_SYMBOLS2) {
         ...
         tree1.HideFunctions(TREE_ROOT_INDEX,_pic_symbolp_overlay);
         tree2.HideFunctions(TREE_ROOT_INDEX,_pic_symbolm_overlay);
      }else{
         GMFDiffViewOptions|=DIFF_VIEW_MISSING_SYMBOLS2;
         tree1.ReplaceFunctions(TREE_ROOT_INDEX,_pic_symbolp_overlay);
         tree2.ReplaceFunctions(TREE_ROOT_INDEX,_pic_symbolm_overlay);
      }
   ...
   case 'hmovedfunctions':
      {
         mou_hour_glass(true);
         if (GMFDiffViewOptions&DIFF_VIEW_MOVED_SYMBOLS) {
1            GMFDiffViewOptions&=~DIFF_VIEW_MOVED_SYMBOLS;
