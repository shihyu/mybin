 ...
 * 
 */
_command gui_open(typeless flags="",_str initial_directory='') name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_MDI)
{
   if (isEclipsePlugin()) {
      _eclipse_file_open();
      return(0);
   ...
   case OPEN_BROWSE_FOR_FILES:
      return browse_open(a2_flags,initial_directory: initial_directory);
      break;
   case OPEN_SMART_OPEN:
   default:
      return smart_open(a2_flags,initial_directory);
      break;
   }
}

_command void smart_open(int editFlags = EDIT_DEFAULT_FLAGS,_str initial_directory='') name_info(',')
{
   ...
   restore_group := false;
   activate_open(restore_group,initial_directory);
}

_command browse_open(int editFlags = EDIT_DEFAULT_FLAGS, typeless flags = OFN_READONLY|OFN_ALLOWMULTISELECT, int parentWid=0, _str initial_directory='') name_info(',')
{
