...
const VSCFGPROFILE_PROCESS_COMPLETION= "process_completion";
const VSCFGPROFILE_FPROCESS_COMPLETION_VERSION= 1;
const VSCFGPROFILE_SEARCH_ENGINES= "search_engines";
const VSCFGPROFILE_SEARCH_ENGINES_VERSION= 1;
...
_str vsCfgPackage_for_LangAPIHelpProfiles(_str LangId) {
   if (LangId=='') {
      return (VSCFGPACKAGE_MISC:+VSXMLCFG_FILESEP:+'apihelp_profiles');
   }
   return (vsCfgPackage_for_Lang(LangId):+VSXMLCFG_FILESEP:+'apihelp_profiles');
}
const VSCFGPROFILE_HELP_VERSION= '1';
...
const VSCFGPROFILE_COLOR_VERSION= 6;
const VSCFGPACKAGE_COLOR_PROFILES= 'color_profiles';

const VSCFGPROFILE_HIGHLIGHT_VERSION= 1;
const VSCFGPACKAGE_HIGHLIGHT_PROFILES= 'highlight_profiles';
...
int _plugin_prompt_add_profile(_str package,_str &profileName,_str copyFrom='') {
   ...
   for (;;) {
      ...
      profileName = _param1;
      if (copyFrom!='' && strieq(profileName,copyFrom)) {
         _message_box("Can't overwrite the profile you are copying from");
         continue;
      }
   ...
   if (!stopped_on_error) {
      ...
      foreach (k => index in plugin_file_not_used_anymore) {
         if (name_type(index)&MODULE_TYPE) {
            ...
            //say('remove pic key='k' ix='index' 'name_name(index));
            // Remove this picture
            delete_name(index);
         }
      }
...
int _plugin_uninstall(_str pluginName,bool display_error=true,bool updatePlugins=true) {
   ...
   _str temp=pluginName;
   if (last_char(temp):==FILESEP) {
      temp=substr(temp,1,length(temp)-1);
   }
   temp=_strip_filename(pluginName,'P');
   {
      i:=pos('.ver.',temp,1,_fpos_case);
      if (i>0) {
         temp=substr(temp,1,i-1);
      }
   }
   temp=translate(temp,'_','.');
   temp=stranslate(temp,'_','[^a-zA-Z0-9_$]','ry');
   strappend(temp,"_before_uninstall");
   index:=find_index(temp,COMMAND_TYPE|PROC_TYPE);
   if (index_callable(index)) {
      bu_status:=call_index(index);
      if (isinteger(bu_status) && bu_status) {
         return bu_status;
      }
   }
