 ...
 *
 */
_command push_bookmark(_str markid="", bool isReferences=false,int persistent_wid=0) name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   if (persistent_wid<=0 && markid=='' && _isEditorCtl(false) && p_mdi_child) {
      persistent_wid=_get_persistent_wid();
   }
   ...
   _BookmarkAdd(bm_id,(int)markid,bm_flags,-1,0,0,'','','',persistent_wid);
   if ( isReferences && !(def_references_options & VSREF_NO_AUTO_PUSH) ) {
...
_command push_tag,f(_str proc_name="", VSCodeHelpFlags extra_codehelp_flags=VSCODEHELPFLAG_NULL) name_info(TAG_ARG','VSARG2_EDITORCTL|VSARG2_REQUIRES_MDI|VSARG2_READ_ONLY)
{
   ...
   if (_isEditorCtl(false) && p_mdi_child) {
      persistent_wid=_get_persistent_wid();
   }
   ...
   if (orig_buf_id != 0) {
      ...
      status=push_bookmark(mark,false,persistent_wid);
   }
...
/** 
 * The <b>push_tag_subword</b> command pushes a bookmark at the cursor 
 * position and places the cursor on the source code for
 * <i>tag_name</i>.  Use the <b>pop_bookmark</b> command (Ctrl+,)
 * to return to the previous bookmark.  If <i>tag_name</i> is not
 * specified, the word at the cursor is used.   The <i>tag_name</i> 
 * may be a subword pattern matching expression. 
 *  
 * @param proc_name              encoded tag name to find.  See {@link tag_tree_compose_tag}. 
 *  
 * @see push_tag 
 ...
 * @categories Search_Functions
 */
_command push_tag_subword,fsw(_str proc_name="") name_info(SUBWORDTAG_ARG','VSARG2_EDITORCTL|VSARG2_REQUIRES_MDI|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   return push_tag(proc_name,VSCODEHELPFLAG_SUBWORD_MATCHING_WORKSPACE_ONLY|
                   VSCODEHELPFLAG_SUBWORD_MATCHING_INC_AUTO_UPDATED|
                   VSCODEHELPFLAG_SUBWORD_MATCHING_INC_COMPILER);
}

_str swtag_match(_str name, int find_first)
{
   return tag_match(name, find_first, "", SE_TAG_CONTEXT_MATCH_STSK_SUBWORD);
}
...
void ctlTagList.rbutton_up,context()
{
   // Get handle to menu:
