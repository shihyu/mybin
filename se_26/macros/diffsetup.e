...
void ctlok.on_create(_str filename1='',bool OptionsOnly=false,
                     ...
                     bool isModal=false)
{
   _SetDialogInfoHt("isModal",isModal);
   origChangeDialog := setChangingDialog(true);
   gTimerHandle=-1;
   ...
   if ( info!=null ) {
      ...
      if ( info.viewID1.p_buf_name=="" ) {
         ctlpath1.p_text = "Untitled<"info.viewID1.p_buf_id'>';
      } else {
         ctlpath1.p_text = info.viewID1.p_buf_name;
      }
      if ( info.viewID2.p_buf_name=="" ) {
         ctlpath2.p_text = "Untitled<"info.viewID2.p_buf_id'>';
      } else {
         ctlpath2.p_text = info.viewID2.p_buf_name;
      }
...
static void oncreateOptions(bool removeDialogOptions)
{
   ...
   setLinkButtonPicture();
   setChangingDialog(origChangeDialog);
   ctlintraline_length_limit.p_text = def_diff_max_intraline_len;
   ctldiff_length_limit.p_text = def_diff_force_bin_compare_limit;
...
int ctlok.lbutton_up()
{
   ...
   do {
      ...
      if ( !haveUntitledBuffer ) {
         bool changed=true;
         DiffSessionCollection *pdiffSessionData = _GetDialogInfoHtPtr("diffSessions");
         GetCompareOptionsFromDialog(gDiffSetupData.compareOptions,auto diffMaxIntralineLen,auto diffMaxDiffLen,auto diffTimeout);
   ...
   if (!status) {
      if ( !gDiffSetupData.Modal ) {
         isModal := (val==null?0:val);
         gDiffSetupData.Modal = isModal;
      }
      return _DiffFromModelessSetupDialog();
   }
...
void ctlsave_session.lbutton_up()
{
   ...
   do {
      ...
      GetCompareOptionsFromDialog(auto compareOptions=0,auto diffMaxIntralineLen=0,auto diffMaxDiffLen=0,auto diffTimeout=0);
...
static int okOptions()
{
   ...
   diffTimeout := 0;
   diffMaxDiffLen := 0;
   GetCompareOptionsFromDialog(compareOptions,diffMaxIntralineLen,diffMaxDiffLen,diffTimeout);
   ...
   do {
      ...
      if ( def_diff_force_bin_compare_limit!=diffMaxDiffLen ) {
         def_diff_force_bin_compare_limit = diffMaxDiffLen;
         _config_modify_flags(CFGMODIFY_DEFVAR);
      }
...
/**
 * @param options             output: options set on dialog
 * @param diffMaxIntralineLen output: max length for intraline 
 *                            diff
 * @param diffMaxDiffLen      output: max length to use diff, 
 *                            use byte by byte compare otherwise
 * @param diffTimeout         output: amount of time before diff 
 *                            times out
 */
static void GetCompareOptionsFromDialog(int &options, int &diffMaxIntralineLen, int &diffMaxDiffLen, int &diffTimeout)
{
   options=0;
   ...
   if ( isinteger(ctlintraline_length_limit.p_text) ) {
      diffMaxIntralineLen = (int)ctlintraline_length_limit.p_text;
   }
   if ( isinteger(ctldiff_length_limit.p_text) ) {
      diffMaxDiffLen = (int)ctldiff_length_limit.p_text;
   }
...
static void GetDataForTwoFiles()
{
   ...
   if ( _diff_is_http_filename(ctlpath1.p_text) ) {
      ...
      gDiffSetupData.file1.bufID = (int)bufId1Str;
   }else{
   ...
   if (_diff_is_http_filename(ctlpath2.p_text)) {
      ...
      gDiffSetupData.file2.bufID = (int)bufId2Str;
   }else{
   ...
   if ( _file_eq(gDiffSetupData.file1.fileName,gDiffSetupData.file2.fileName) && gDiffSetupData.file2.firstLine==0 ) {
