...
{
   return (pos(' 'ext' ',' '_default_option(VSOPTIONZ_ZIP_EXT_LIST)' ',1,_fpos_case) || 
...
{
   _bgm_filter_project_files(p_window_id,wildcards,file_exclude);
#if 0
   ...
   if (bgm_NofFiles>0) {
      ...
      if (bgm_mfflags & MFFIND_FIND_FILES) {
         ...
         if (bgm_mfflags & MFFIND_FILESONLY) {
            if (orig_bgm_mfflags & MFFIND_FILESONLY) {
               text='Matching files: 'bgm_NofFileMatches'     Total files searched: 'bgm_NofFiles;
            } else {
               text='Total found(truncated): 'bgm_NofMatches'     Matching files: 'bgm_NofFileMatches'     Total files searched: 'bgm_NofFiles;
            }
         } else {
         ...
         case 'filelist':
            {
               ...
               if (last_char(cur_file)==FILESEP) {
                  bgm_results.insertMessage('Directory 'cur_file);
               } else { 
                  bgm_results.insertMessage('File 'cur_file);
               }
            }
            break;
         case 'buf_full':
...
static _str find_files_summary_line(_str files, _str wildcards, _str file_exclude, _str file_stats, int mfflags)
{
   _str subdir_opts;
   ...
   if (mfflags & MFFIND_FIND_FILES_MATCH_DIRECTORIES) {
      if (mfflags & MFFIND_FIND_FILES_EXCLUDE_FILES) {
         strappend(summary,FINDFILE_MATCH_DIRS', ');
      } else {                               
         strappend(summary,FINDFILE_MATCH_FILES_AND_DIRS', ');
      }
   } else {
      strappend(summary,FINDFILE_MATCH_FILES', ');
   }
   if (disp_files :!= '') {
...
void start_bgsearch(_str search_string,_str options,_str files,int mfflags,bool searchProjectFiles,bool searchWorkspaceFiles,_str wildcards,_str file_exclude,bool files_delimited_with_semicolon,int grep_id,int before_lines=0,int after_lines=0,_str file_stats='')
{
   ...
   if (mfflags & MFFIND_FIND_FILES) {
      topline = find_files_summary_line(files, wildcards, file_exclude, file_stats, mfflags);
   ...
   _maybe_start_main_timer();

   orig_bgm_mfflags=bgm_mfflags=mfflags;
   bgm_NofMatches=0;
   ...
   if ((color_flags && false) || list_context) {
      ...
      vs_program:=_get_editor_launcher();
      bgm_proc_handle=_PipeProcess(_maybe_quote_filename(vs_program):+' +newi ':+lock_option:+symlink_option' -sc '_maybe_quote_filename(get_env('SLICKEDITCONFIG'))' -q -st 0 -mdihide -slocalsta -sloadplugins -sautobuildtagfiles -nogui -r bgsearch',bgm_hin,bgm_hout,bgm_herr,'');
   } else {
...
static void bgs_stop_and_exit(bgs_state& search_state)
{
   ...
   /*
     On Windows, the process can die BEFORE
     the 'done' command gets seen. Wait .1 seconds
     so that SlickEdit has time to read the 'done' command. 
   */
   delay(10);
   exit();
}
...
static int bgs_process_file(bgs_state& search_state,_str cur_file,long arg_offset= -1)
{
   sent_cur_file := true;
...
_command void bgsearch()
{
   ...
   if (search_state.sort_filenames || (search_state.mfflags& (MFFIND_FIND_FILES|MFFIND_FIND_FILES_MATCH_DIRECTORIES|MFFIND_FIND_FILES_EXCLUDE_FILES))) {
   ...
   if (search_state.sort_filenames != 0) {
      ...
      if (search_state.mfflags& (MFFIND_FIND_FILES|MFFIND_FIND_FILES_MATCH_DIRECTORIES|MFFIND_FIND_FILES_EXCLUDE_FILES)) {
         top();up();
         while (!down()) {
            get_line(fname);
            bgs_process_file(search_state, fname);
   ...
   if (!(search_state.mfflags& (MFFIND_FIND_FILES|MFFIND_FIND_FILES_MATCH_DIRECTORIES|MFFIND_FIND_FILES_EXCLUDE_FILES))) {
      _mffind_file_stats_init('', search_state.file_stats);
      NofFilesSearched:=0;

      VSGREP_ARGS grep_args;
      grep_args.mfflags=search_state.mfflags;
      grep_args.search_string=search_state.search_string;
      grep_args.options=search_state.options;
      grep_args.callback=bgs_process_file;
      grep_args.callback_args=search_state;
      //grep_args.cancel_callback=maybe_cancel_callback;
      grep_args.file_stats=file_stats;
      grep_args.buffer_list=buffers;
      grep_args.wildcards=search_state.wildcards;
      grep_args.file_exclude=search_state.file_exclude;
      grep_args.attr_flags=def_find_file_attr_options;
      
      int status;
      if (search_state.sort_filenames) {
         status=_grep_file_list(search_state.sortfile_view_id,0, def_search_threads, grep_args, NofFilesSearched);
      } else {
         status=_grep_file_list(file_list_view_id,VSGREPFLAG_EXPAND_WILDCARDS, def_search_threads, grep_args, NofFilesSearched);
      }
      search_state=grep_args.callback_args;
      search_state.num_searched_files=NofFilesSearched;
   }
