 ...
 * <p>
 *
 * {@code hints} are 1 or more of
 * {@code AutoRestoreHint}. 
 *
 * @param options 
...
_command restore(_str options='', int alternate_view_id=0, _str relativeToDir=null, int hints=0)
{
   ...
   */
   ...
   if (!_no_child_windows()) {
       //say("restore:  RESTORE FOCUS TO MDI");
       //_mdi.p_child._set_focus();
       _post_call(activate_editor);
   }
...
static int restore2(_str restore_options, _str relativeToDir, bool restoreFiles, bool restoreLayout, bool restoringFromProject,int restoreOnlyFloatingLayout=-1)
{
   ...
   for (;;) {
      ...
      } else if ( rtype == 'TOOLBARS5:' ) {
         gHitOldToolbarsSection = true;
      } else if (rtype=='BOOKMARK2:') {
         rtype='BOOKMARK3:';
      }
...
static _str process_window(_str line,_str bufferline,_str viewline,_str relativeToDir,int (&modified_temp_name2buf_id):[],bool (&unable_to_restore_file):[],bool inAutoSave)
{
   ...
   typeless persistent_wid='';
   parse line with x y width height icon_x icon_y state 'WF=' wf 'WT=' wt '"' font_info '"' 'DID='dup_id' MI="'minimap_info'"' 'PI='persistent_wid .;
   if (font_info=='') {
   ...
   p_window_flags=wf;
   p_tile_id=wt;
   if (!isinteger(persistent_wid)) {
      persistent_wid=0;
   }
   p_persistent_wid=persistent_wid;
   if(isinteger(dup_id)) {
...
static _str process_buffer(_str buf_info,_str window_options,_str bufferProperties,_str relativeToDir,int &load_file_status,int (&modified_temp_name2buf_id):[],bool (&unable_to_restore_file):[],bool setModifiedTempHash=false,bool inAutoSave=false)
{
   ...
   CLscheme := SCscheme := "";
   typeless SCenabled="", SCerrors="", SCoverride="";
   ...
   typeless buf_font_info="";
   typeless map_font_info="";
   typeless delete_file_on_close_or_rename;
   if ( bufferProperties!='' ) {

      // data looks like this:
      // 
      // MA=1 74 1  TABS= WWS=9 IWT= ST=8 IN=2 BW=0 US=32000 RO=0 SE=1 SN=0 BIN=0 MN=	HM=0 MF=100663392 TL=0 MLL=0 ASE=0 LNL=1 LCF=0 CAPS=0 E=70 ESBU2=-1 CL="Charcoal," SC="" SCE= SCU= ALM=0 FWRM=0 HMRE=0 HMRBW=0 SPCWT= SWF=2 SM= STAT= BFONT="" MFONT=",4"
      // MA=1 74 1  TABS= WWS=9 IWT= ST=8 IN=2 BW=0 US=32000 RO=0 SE=1 SN=0 BIN=0 MN=	HM=0 MF=100663904 TL=0 MLL=0 ASE=0 LNL=1 LCF=0 CAPS=0 E=70 ESBU2=-1 CL="Charcoal," SC="" SCE= SCU= ALM=0 FWRM=0 HMRE=0 HMRBW=0 SPCWT= SWF=2 SM= STAT= BFONT=",14,0,1," MFONT=""
      // MA=1 74 1  TABS= WWS=9 IWT= ST=8 IN=2 BW=0 US=32000 RO=0 SE=1 SN=0 BIN=0 MN=	HM=0 MF=120783456 TL=0 MLL=0 ASE=0 LNL=1 LCF=0 CAPS=0 E=70 ESBU2=-1 CL="Charcoal," SC="" SCE= SCU= ALM=0 FWRM=0 HMRE=0 HMRBW=0 SPCWT= SWF=2 SM= STAT= BFONT="" MFONT=",4"
      //                                                                              ^
      //                                                                             TAB
      parse bufferProperties with "MA="      margins 
                                  " TABS="   tabs 
                                  " WWS="    word_wrap_style 
                                  ...
                                  " TL="     TruncateLength
                                  " MLL="    MaxLineLength
                                  " ASE="    AutoSelectLanguage
                                  " LNL="    line_numbers_len
                                  " LCF="    LCFlags
                                  " CAPS="   caps
                                  " E="      encoding
                                  " ESBU2="  encoding_set_by_user
                                  " CL=\""   CLscheme
                                  "\" SC=\"" SCscheme
                                  "\" SCE="  SCenabled
                                  " SCU="    SCerrors
                                  " ALM="    AutoLeftMargin
                                  " FWRM="   FixedWidthRightMargin
                                  " HMRE="   hex_mode_reload_encoding
                                  " HMRBW="  hex_mode_reload_buf_width
                                  " SPCWT="  spell_check_while_typing
                                  " SWF="    soft_wrap_flags
                                  " SM="     show_minimap
                                  " STAT="   show_statements 
                                  " BFONT=\""   buf_font_info
                                  "\" MFONT=\"" map_font_info
                                  "\"" " DF=" delete_file_on_close_or_rename
                                  " SCO="    SCoverride
                                  . ;
   }
   BUFscheme := MAPscheme := "";
   //say("process_buffer H"__LINE__": CLScheme="CLscheme" bn="bn);
   parse CLscheme with BUFscheme ',' MAPscheme;
   if ( bn =='' ) {
   ...
   if ( status ) {
      ...
      if (bufferProperties!='') {
         ...
         if (isinteger(delete_file_on_close_or_rename)) {
         ...
         if (SCscheme != "") {
            // if a buffer has a symbol coloring scheme which is out-dated,
            // try to update it to the more modern version.
            color_profile := BUFscheme;
            if (color_profile == "") {
               color_profile = def_color_scheme;
            }
            // the color scheme needs to have symbol colors defined
            if (beginsWith(SCscheme, "All symbols -")) {
               if (ColorScheme.hasSymbolColoringRulesDefined(color_profile)) {
                  SCscheme == "All symbols";
               }
            } else if (beginsWith(SCscheme, "Classes and Types -")) {
               if (ColorScheme.hasSymbolColoringRulesDefined(color_profile)) {
                  SCscheme == "Classes and Types";
               }
            }
         ...
         if (isinteger(SCoverride))  _SetSymbolColoringOverrides(SCoverride);
         ...
         if (buf_font_info != "") {
            p_BufferFontInfo = buf_font_info;
         }
         if (map_font_info != "") {
            p_MinimapFontInfo = map_font_info;
         }
         if (BUFscheme != '') {
            p_BufferColorProfile = BUFscheme;
         }
         if (MAPscheme != '') {
            p_MinimapColorProfile = MAPscheme;
         }
      }
   }
   return(0);
 ...
 * <p>
 *
 * Optionally save to an {@code alternate_view_id} instead
 * of "vrestore.slk". 
 *
 * <p>
 *
 ...
 * <p>
 *
 * Optionally set {@code relativeToDir} to store file name
...
static void write_window_data(int buf,bool specifyFont,bool forceMaximized)
{
   ...
   out=out:+x' 'y' 'width:+' 'height' 'icon_x' 'icon_y' 'windowState' ':+
       ' WF='p_window_flags:+' WT='p_tile_id' 'font_info:+' DID='p_mdi_child_duplicate_id' MI="'minimap_info'" PI='p_persistent_wid;
   activate_window(buf);
   insert_line(out);
   activate_window(this_buf);
}
...
static void write_buffer_data(_str relativeToDir,int buf,bool insertBufferInfo,bool restore_named_as_modified)
{
   ...
   _str SCoverride = _QSymbolColoringOverrides();
   if (SCenabled  == _QSymbolColoringEnabled(true))   SCenabled = "";
   if (SCerrors   == _QSymbolColoringErrors(true))    SCerrors  = "";
   if (SCoverride == _QSymbolColoringOverrides(true)) SCoverride = "";

   // color scheme options
   BUFscheme := p_BufferColorProfile;
   MAPscheme := p_MinimapColorProfile;
   if (BUFscheme == _GetDefaultColorProfileForEditor())  BUFscheme = "";
   if (MAPscheme == _GetDefaultColorProfileForMinimap()) MAPscheme = "";
   ...
   // get buffer font information
   BUFfont := p_BufferFontInfo;
   MAPfont := p_MinimapFontInfo;
   if (BUFfont == _get_default_editor_font_info())  BUFfont = "";
   if (MAPfont == _get_default_minimap_font_info()) MAPfont = "";
  ...
  out2 := "BI: MA="p_margins:+
             ...
             " CL="_dquote(BUFscheme','MAPscheme):+
             " SC="_dquote(SCscheme):+
             " SCE="SCenabled:+
             " SCU="SCerrors:+
             ...
             " SM="show_minimap:+
             " STAT="DEFStatements:+
             " BFONT="_dquote(BUFfont):+
             " MFONT="_dquote(MAPfont):+
             " DF="(p_buf_flags & VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME):+
             " SCO="SCoverride:+
             "";
   activate_window(buf);
   insert_line(out);
