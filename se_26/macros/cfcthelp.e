                         ...
                         int depth=0)
{
   ...
   if (_LanguageInheritsFrom("d")) {
      ...
      not_function_words=JULIA_NOT_FUNCTION_WORDS;
   }
   gInJavadoc_flag=VSAUTOCODEINFO_NULL;
   exclude_colors := "xcs";
   flags=0;
   ...
   word := "";
   {
      ...
      for (;;) {
         ...
         ch=get_text_safe();
         if (_chdebug) {
            isay(depth, "_c_fcthelp_get_start H"__LINE__": LOOPING ch="ch" seek="_nrseek());
         }
         if (ch=="(") {
            save_pos(auto p);
            if( p_col==1 ){
                   ...
                   VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   } else if (_LanguageInheritsFrom('js', lang)) {
      javascript=true;
      common='[,{};()[]|'common'|'JAVA_MORE_END_OF_STATEMENT_RE;
   } else if (_LanguageInheritsFrom("cfscript", lang)) {
      javascript=true;
      case_sensitive=false;
      common='[,{};()[]|'common'|'JAVA_MORE_END_OF_STATEMENT_RE;
   } else if (_LanguageInheritsFrom("rul", lang)) {
      isrul=true;
      common='[,;()[]|'common'|'RUL_MORE_END_OF_STATEMENT_RE;
   } else if (_LanguageInheritsFrom("phpscript", lang)) {
      isphp=true;
      common='[,;()[]|'common'|'PHP_MORE_END_OF_STATEMENT_RE;
   } else if (_LanguageInheritsFrom("sas", lang)) {
      isother=true;
      case_sensitive=false;
   ...
   } else if (_LanguageInheritsFrom("d", lang)) {
      isdlang=true;
      common='[,{};()]|'common'|'D_MORE_END_OF_STATEMENT_RE;
   } else if (_LanguageInheritsFrom("dart", lang)) {
      isdart=true;
   } else if (_LanguageInheritsFrom("rs", lang)) {
      isrust=true;
      common='[,;()[]|'common;
   } else if (_LanguageInheritsFrom("m", lang)) {
      isobjc=true;
      save_pos(auto op);
   ...
   } else if (_LanguageInheritsFrom("julia", lang)) {
      case_sensitive=p_EmbeddedCaseSensitive;
      isother=true;
      common='[,;()[]|'JULIA_COMMON_END_OF_STATEMENT_RE;
      is_generic_lang = true;

   } else {
      ispython = _LanguageInheritsFrom("py", lang);
      isrlang  = _LanguageInheritsFrom("r",  lang);
      case_sensitive=p_EmbeddedCaseSensitive;
      isother=true;
      common='[,;()[]|'common;
      if ( _LanguageInheritsFrom("pl",     lang) ||
           _LanguageInheritsFrom("erlang", lang)  ) {
         is_generic_lang = true;
      }
   }
   ...
   had_c_function_template_arglist := false;
   exclude_colors := "xcs";
   ...
   for (;;) {
      ...
      if (ch=="(") {
         ...
         } else if (get_text_left() == '>' && get_text_left(2) != "->") {
            had_c_function_template_arglist=true;
         }
         ++stack_top;
         initFnHelpCtx(ctx_stack[stack_top], (int)point('s'));
         /*if (get_text(2)=="(*") {
      ...
      if (_chdebug) {
         isay(depth, "_c_fcthelp_get H"__LINE__": ch="ch);
         isay(depth, "_c_fcthelp_get H"__LINE__": ch before="get_text_safe(1,match_length('s')-1));
         isay(depth, "_c_fcthelp_get H"__LINE__": ch after word="get_text_safe(1,match_length('s')+match_length()));
         isay(depth, "_c_fcthelp_get H"__LINE__": word_chars="word_chars);
      }
      if (ch=="#" || ch=="{" || (pos("[~"word_chars"]",get_text_safe(1,match_length('s')-1),1,"r") &&
         ...
         if (stack_top>=1 && ispython && (ch=="f" || ch=="i" || ch=="w")) {
            word=cur_identifier(junk);
            if (word=="for" || word=="if" || word=="while") {
               status=repeat_search();
               continue;
            }
         }
         ...
         if (!(has_bracket_expressions && ctx_stack[stack_top].brace_nesting)) {
            restore_pos(p);
            if (_chdebug) {
               isay(depth, "_c_fcthelp_get: H5 NOT IN ARGUMENT LIST, seek="_QROffset()" ch="ch);
            }
            return(VSCODEHELPRC_NOT_IN_ARGUMENT_LIST);
         }
      }
   ...
   for (;;--stack_top) {
      ...
      } else if (had_c_function_template_arglist) {
         goto_point(ctx_stack[stack_top].lparen_offset);
         if (get_text_safe()=="(") left();
         _clex_skip_blanks("-");
         if (get_text_safe() == ">") {
      ...
      if ((flags & VSAUTOCODEINFO_IN_TEMPLATE_ARGLIST) || had_d_template_arglist) {
         ...
         if (iscsharp || is_generic_lang) {
            status = _Embeddedget_expression_info(true,lang,idexp_info,visited,depth+1);
         } else {
    ...
    if (_chdebug) {
      ...
      if (!status) {
         ...
         if (_LanguageInheritsFrom("julia", lang) && pos('@',idexp_info.prefixexp)) {
            idexp_info.prefixexp=stranslate(idexp_info.prefixexp,'','@');
            //idexp_info.info_flags|=VSAUTOCODEINFO_HAS_REF_OPERATOR;
            match_flags |= SE_TAG_FILTER_DEFINE;
         }
         if (_LanguageInheritsFrom("julia", lang) && pos('@', match_symbol)) {
            match_symbol = stranslate(match_symbol,'','@');
            match_flags |= SE_TAG_FILTER_DEFINE;
         }
         tag_return_type_init(auto rt);
         if (!isjava && !iscsharp && !javascript && !isrul && !isphp && !isdlang && !isdart && !isother) {
            match_flags |= SE_TAG_FILTER_DEFINE;
         ...
         if (_LanguageInheritsFrom("sas", lang)) {
            match_flags |= SE_TAG_FILTER_DEFINE;
         }
         ...
         if (num_matches == 0) {
            ...
            if ((isjava || iscsharp || isphp || isdlang || isdart) && pos("new ",idexp_info.prefixexp) == 1) {
               // 08/24/2000 --
            ...
            if (!iscsharp && (isjava || isdlang || isdart) && idexp_info.lastid=="super") {
            ...
            if (idexp_info.lastid!="" || match_symbol!="") {
               ...
               if (num_matches == 0 && idexp_info.prefixexp=="" &&
                   (_LanguageInheritsFrom("c", lang) || _LanguageInheritsFrom("cs", lang)) && 
                   !pos(VS_TAGSEPARATOR_package, rt.return_type)) {
            ...
            if (num_matches==0 && !(idexp_info.info_flags & VSAUTOCODEINFO_IN_TEMPLATE_ARGLIST)) {
               ...
               if (num_matches==0 && !status && rt.return_type!="" && _LanguageInheritsFrom("julia")) {
                  // construct a "fake" constructor for classes that do not have one
                  tag_list_symbols_in_context("", rt.return_type, 0, 0, tag_files, "",
                                              num_matches, def_tag_max_function_help_protos,
                                              SE_TAG_FILTER_MEMBER_VARIABLE, context_flags,
                                              exact_match:false, case_sensitive:true, 
                                              visited, depth+1, rt.template_args);
                  if (num_matches > 0) {
                     tag_get_match_browse_info(1, auto constr_info);
                     _str sortable_arguments[];
                     constr_info.arguments = "";
                     constr_info.return_type = "";
                     constr_info.member_name = idexp_info.lastid;
                     tag_remove_duplicate_symbol_matches();
                     num_matches = tag_get_num_of_matches();
                     for (i:=1; i<=num_matches; i++) {
                        tag_get_match_browse_info(i, auto match_info);
                        a := _dec2hex(match_info.line_no, 16, 8);
                        a :+= " ";
                        a :+= _dec2hex(match_info.seekpos, 16, 8);
                        a :+= "\t";
                        a :+= match_info.member_name;
                        a :+= ":: ";
                        a :+= match_info.return_type;
                        sortable_arguments :+= a;
                     }
                     sortable_arguments._sort();
                     foreach (auto a in sortable_arguments) {
                        _maybe_append(constr_info.arguments, ', ');
                        parse a with "\t" a;
                        constr_info.arguments :+= a;
                     }
                     tag_clear_matches();
                     if (_chdebug) {
                        isay(depth, "_c_fcthelp_get H"__LINE__": FAKE CONSTRUCTOR, signature="constr_info.arguments);
                     }
                     tag_clear_matches();
                     tag_insert_match_browse_info(constr_info);
                     num_matches = 1;
                  }
               }
            }
            ...
            if ((slickc || javascript || isphp || isrlang || ispython) && num_matches == 0) {
         ...
         for (i:=0; i<num_unique; i++) {
            ...
            if (cm.class_name != "") {
               if (javascript || isjava || iscsharp || isdlang || slickc || isphp || isdart) {
            ...
            // use the template argument signature if we are in the template argument list
            if (symbol_to_match != null) cm = symbol_to_match;
            argument_list := cm.arguments;
            if (idexp_info.info_flags & VSAUTOCODEINFO_IN_TEMPLATE_ARGLIST) {
               if ((cm.flags & SE_TAG_FLAG_TEMPLATE) && cm.template_args != null && cm.template_args != "") {
                  argument_list = '<'cm.template_args'>';
               }
            }

            if (symbol_to_match != null) {
               match_list :+= symbol_to_match.member_name "\t" symbol_to_match.type_name "\t" argument_list "\t" symbol_to_match.return_type "\t" j "\t" duplicate_indexes[i];
               break;
            } else {
               match_list :+= list_proc_name "\t" cm.type_name "\t" argument_list "\t" cm.return_type"\t"j"\t"duplicate_indexes[i];
         ...
         if (match_list._length()>0) {
            ...
            for (i=0; i<match_list._length(); i++) {
               ...
               if (substr(signature, 1, 1) == "<" && !slickc && !javascript && !isphp) {
               ...
               while (argument != "") {
                  /*
                  Python sample 3.8 function definition:
                     def foo(a, /, b, *, c):
                  */
                  if ((argument=='/' || argument=='*') && ispython) {
                     arg_start = tag_get_next_argument(signature, arg_pos, argument);
                     continue;
                  }
                  ...
                  if (j == ParamFoundAt || (!foundNamedArg && j >= ParamNameNum && ParamName!="")) {
                     ...
                     if (pos("^["word_chars"]*([=]?*|)$",argument,1,'r')) {
                        ...
                        foundParamName = argument;
                                ...
                                pos("^(typename|class|struct|interface):b["word_chars"]*([=]?*|)$",argument,1,'r')) {
                        parse argument with . argument "=" auto init_to;
                        functionHelpArgK.ParamName=argument;
                        functionHelpArgK.ParamType=argument;
                        foundParamName = argument;
                             ...
                             VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   status = _c_fcthelp_get(errorArgs,auto functionHelpList,
                           listChanged,auto cursor_x,
                           auto helpWord,startOffset,
                           VSAUTOCODEINFO_NULL,
