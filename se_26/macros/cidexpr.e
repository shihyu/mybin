...
AutoCodeInfoFlags gInJavadoc_flag = VSAUTOCODEINFO_NULL;
...
const JULIA_COMMON_END_OF_STATEMENT_RE = "begin|break|catch|continue|do|else|elseif|end|export|finally|for|function|global|if|import|let|macro|module|return|struct|try|using|while|where";
...
const CS_NOT_FUNCTION_WORDS = " int long double float bool short char decimal byte ubyte string var in is checked unchecked ushort ulong nint nuint uint catch do for foreach if return sizeof typeid switch using throw while lock with fixed yield ";
...
const JULIA_NOT_FUNCTION_WORDS = " baremodule begin break catch const continue do else elseif end export false finally for function global if import let local macro module quote return struct true try using while abstract mutable primitive where in isa ";
                           ...
                           VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   if (gtk :== '@' && isjulia) {
      idexp_info.prefixexp=gtk:+idexp_info.prefixexp;
      idexp_info.prefixexpstart_offset=(int)point('s')+1;
      gtk=c_prev_sym_same_line();
   }
   ...
   for (;;) {
      ...
      case ".":
         ...
         if (gtk!=TK_ID) {
            ...
            // Python shouldn't back up across blank lines
            if (_LanguageInheritsFrom('py') && at_first_non_blank()) {
               return 0;
            }
            gtk=c_prev_sym2();
         }
