...
_str tags_filename(_str lang="",bool includeSlickC=true)
{
   ...
   if (lang!="" && (langTagFileList != "" || _istagging_supported(lang))) {
      if ( ( 
             ...
             !_LanguageInheritsFrom("dart", lang) && 
 ...
 * @param allowGlobalTagFile (default false) try using global tagfile
 *
 ...
 */
   ...
   if (!context_found) {
      ...
      // set up context flags if we are doing subword matching
      subword_context_flags := SE_TAG_CONTEXT_NULL;
      if (extra_codehelp_flags & VSCODEHELPFLAG_SUBWORD_MATCHING_WORKSPACE_ONLY) {
         subword_context_flags = SE_TAG_CONTEXT_MATCH_STSK_SUBWORD;
         subword_context_flags |= SE_TAG_CONTEXT_ONLY_WORKSPACE;
      }
      if (extra_codehelp_flags & VSCODEHELPFLAG_SUBWORD_MATCHING_INC_AUTO_UPDATED) {
         subword_context_flags |= SE_TAG_CONTEXT_INCLUDE_AUTO_UPDATED;
      }
      if (extra_codehelp_flags & VSCODEHELPFLAG_SUBWORD_MATCHING_INC_COMPILER) {
         subword_context_flags |= SE_TAG_CONTEXT_INCLUDE_COMPILER;
      }
      if (extra_codehelp_flags & VSCODEHELPFLAG_SUBWORD_MATCHING_RELAX_ORDER) {
         subword_context_flags |= SE_TAG_CONTEXT_MATCH_RELAX_ORDER;
      }
      if (extra_codehelp_flags & VSCODEHELPFLAG_SUBWORD_MATCHING_GLOBALS_FIRST_CHAR) {
         subword_context_flags |= SE_TAG_CONTEXT_MATCH_FIRST_CHAR;
      }
      if (!(extra_codehelp_flags & VSCODEHELPFLAG_COMPLETION_NO_FUZZY_MATCHES)) {
         subword_context_flags |= SE_TAG_CONTEXT_MATCH_CORRECTIONS;
      }

      status=find_tag_matches(tag_files, proc_name, context_flags: subword_context_flags);
      if (_chdebug) {
...
int _OnUpdate_push_ref(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   if (command=="cb-find" || command=="cf") {
      ...
      if (isPoundInclude) {
         msg = get_message(VSRC_GO_TO_INCLUDE, word);
      } else if (word=="") {
         if (goto_def) {
            msg = get_message(VSRC_GO_TO_DEF_NO_WORD);
         } else if (showSymbolInfo) {
            msg = get_message(VSRC_SHOW_SYMBOL_INFO_NO_WORD);
         } else if (!goto_def) {
            msg = get_message(VSRC_GO_TO_DECL_NO_WORD);
         }
      } else {
         if (goto_def) {
            msg = get_message(VSRC_GO_TO_DEF, word);
         } else if (showSymbolInfo) {
            msg = get_message(VSRC_SHOW_SYMBOL_INFO, word);
         } else if (!goto_def) {
            msg = get_message(VSRC_GO_TO_DECL, word);
         }
      }
   }
...
_command int push_ref,r(_str params="") name_info(TAG_ARG','VSARG2_EDITORCTL|VSARG2_REQUIRES_MDI|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   if (_mdi.p_child._isEditorCtl(false) && _mdi.p_child.p_mdi_child) {
      persistent_wid=_mdi.p_child._get_persistent_wid();
   }
   //tag_refs_clear_pics();
   wid := p_window_id;
   ...
   ret := _mdi.p_child.push_bookmark(mark, isReferences:true,persistent_wid);
...
/**
 * Find tags in a documentation comment.  This function handles @see tags 
 * and @link tags and other related documentation comment tagging scenarios. 
 * 
 * @param errorArgs          List of argument for codehelp error messages
 * @param prefixexp          prefix expression, see {@link _c_get_expression_info}
 * @param lastid             identifier under cursor
                     ...
                     SETagContextFlags context_flags=SE_TAG_CONTEXT_NULL)
{
   ...
   status=tag_list_duplicate_matches(proc_name, tag_files, max_matches, filter_flags, context_flags);
   //say("find_tag2: status="status" proc_name="proc_name);
   if (status && proc_name != orig_proc_name) {
      status=tag_list_duplicate_matches(orig_proc_name, tag_files, max_matches, filter_flags, context_flags);
   }
   if (!status) {
      clear_message();
   }
                               ...
                               int &func_start_line)
{
   ...
   nearest_id := tag_nearest_context(p_RLine, SE_TAG_FILTER_ANYTHING, find_tag_below:true);
...
_str _ShrinkFilenameToScreenWidth(_str file_name)
{
   ...
   return _ShrinkFilename(file_name, clamp(vwidth, 800, vwidth intdiv 4));
}
...
{
   //say("adjustTagfilePaths H"__LINE__": IN, tagfile="tagfile);
   //say("adjustTagfilePaths H"__LINE__": origPath="origPath);
   //say("adjustTagfilePaths H"__LINE__": newPath="newPath);
   if (!_haveContextTagging()) {
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   }
   ...
   while (!down()) {
      ...
      rename_status := tag_rename_file(origFilename, newFilename);
      //say("adjustTagfilePaths H"__LINE__": rename_status="rename_status);
...
{
   //say("check_autoupdated_tagfiles H"__LINE__": HERE");
   //_StackDump();
   if (!_haveContextTagging()) {
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   }
   ...
   //say("check_autoupdated_tagfiles H"__LINE__": workspaceName="workspaceName);
   if (workspaceName == "") {
      return 0;
   }

   handle := -1;
   if (workspaceName == _workspace_filename || gWorkspaceHandle < 0) {
      handle = gWorkspaceHandle;
   } else {
      handle = _xmlcfg_open(workspaceName, status);
      if (handle < 0) return status;
   }
   //say("check_autoupdated_tagfiles H"__LINE__": handle="handle);

   // Make sure the workspace tag file directory exists
   tagFileDir := VSEWorkspaceTagFileDir(workspaceName);
   //say("check_autoupdated_tagfiles H"__LINE__": tagFileDir="tagFileDir);
...
{
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": localFilename="localFilename);
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": removeFilename="remoteFilename);
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": workspacename="workspaceName);
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": local exists="file_exists(localFilename));
   ...
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": absolute localFilename="localFilename);
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": absolute remoteFilename="remoteFilename);
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": absolute local exists="file_exists(localFilename));

   // get the date of the remote file
   remoteDate := _file_date(remoteFilename, "B");
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": remoteDate="remoteDate);
   ...
   //say("isAutoUpdatedTagfileOutdated H"__LINE__": localDate="localDate);
...
{
   //say("_updateAutoUpdatedTagfile H"__LINE__": IN, localFilename="localFilename);
   //say("_updateAutoUpdatedTagfile H"__LINE__": remoteFileName="remoteFilename);
   //say("_updateAutoUpdatedTagfile H"__LINE__": workspaceName="workspaceName);
   if (!_haveContextTagging()) {
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   }
   // make sure both files are absoluted to the workspace
...
int _c_buildAutoTagFile(AUTOTAG_BUILD_INFO &autotagInfo, bool backgroundThread = true)
{
   ...
   if (autotagInfo.directoryPath == "" && _haveBuild()) {
      ...
      if(status==VSRC_VSREFACTOR_CONFIGURATION_NOT_FOUND_1A || refactor_config_count() <= 0 ) {
         generate_default_configs(force:true);
         status=0;
      }
      if (status < 0) {
         return status;
