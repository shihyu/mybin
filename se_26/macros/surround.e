...
/** 
 * @return 
 * Return {@code true} if the cursor is at or past the end of the current line.
 */
bool at_end_of_line()
{
   column := p_col;
...
/** 
 * @return 
 * Return {@code true} if the cursor is at or before the first non-blank
 * character of the current line.
 */
bool at_first_non_blank()
{
   column := p_col;
   _first_non_blank();
   at_first := (column<=p_col);
   p_col=column;
   return(at_first);
}
                             ...
                             bool doingSurroundInComment)
{
   ...
   for (i=0; i<num_lines; ++i) {
      ...
      _RestoreMarkersInFile(markerSaves, end_lines);
   }
   _updateTextChange();
 ...
 * {@code def_surround_mode_flags} are used to determine 
...
/**
 * When on, if you copy the first line of a simple block statement 
 * without an active selection, you will be prompted if you want to 
 * copy the entire block, or just delete the outermost statement.
 *
 * @default true
 * @categories Configuration_Variables
 */
bool def_auto_copy_code_block=true;
/**
 * Prompt user for confirmation before copying code blocks using 
 * {@link copy_to_clipboard}.
 * 
 * @default true
 * @categories Configuration_Variables
 */
bool def_prompt_for_copy_code_block=true;
...
_command void unsurround() name_info(',' VSARG2_REQUIRES_EDITORCTL | VSARG2_MARK)
{
   ...
   while (p_RLine > first_line) {
      ...
      if (maybe_unsurround_block(createClipboard:false, '', orig_line, force:false, hideDeleteLine:true)) {
...
/**
 * Check if the current line starts a documentation comment. 
 * This looks specifically for documentation comments.
 * 
 ...
 */
static bool check_for_doc_comment(int &first_line, int &last_line, 
                                  ...
                                  bool force)
{
   ...
   do {
      ...
      if ( _clex_find(0,'D') != CFG_DOCUMENTATION ) break;
                           ...
                           bool ignoreContinuedStatements=false) 
{
   ...
   // Check if we are on a documentation comment
   if ( !found_block ) {
      found_block = check_for_doc_comment(first_line, last_line, num_first_lines, num_last_lines, allowUnsurround, force);
      //say("get_code_block_lines["__LINE__"]: found_block="found_block);
 ...
 * @param force              force it to prompt, even if not to prompt is off 
 * @param hideCopyLine       hide the option to just delete the current line 
 * 
...
/**
 * Workhorse function for auto copy surround block. 
 * This calls the language-specific callback and verifies that we are on the 
 * start of a statement, then it either copies the entire block or copies the 
 * outermost surround statement, depending on what the user selected.
 * 
 * @param cbname             clipboard name to use (from cut-line)
 * @param invoked_from_line  line that must be part of unsurround block 
 * @param force              force it to prompt, even if not to prompt is off 
 * @param hideCopyLine       hide the option to just copy the current line 
 * @param caller_fallthru_copy_line 
 * 
 * @return 'true' if it did something, 'false' if this was not
 *         a block statement, or if they selected "delete line".
 */
bool maybe_copy_block(_str cbname='', 
                      int  invoked_from_line=0,
                      bool force=false,
                      bool hideCopyLine=false,
                      bool caller_fallthru_copy_line=false)
{
   if (caller_fallthru_copy_line && !def_auto_copy_code_block && !force) {
      int pm = _lineflags()&(PLUSBITMAP_LF|MINUSBITMAP_LF);
      if (pm==PLUSBITMAP_LF) {
         // expand these lines because they can't be expanded with a plus
         plusminus();
   ...
   // Is this option even enabled?
   if (!def_auto_copy_code_block && !force) {
      return false;
   }

   // if they passed in a clipboard name, then this
   // was definately not a simple copy-to-clipboard
   if (cbname != '') {
      return false;
   }

   // Avoid being called recursively
   static bool in_copy_surround;
   if (in_copy_surround) {
      return false;
   }
   ...
   found_block := get_code_block_lines(auto first_line, auto num_first_lines, 
                                       auto last_line, auto num_last_lines, auto indent_change, 
                                       auto isComment, auto allowCopySurround, force);
   if (!found_block) {
      return false;
   }

   // no code in the block to copy surround for, then get out of here
   if (first_line+num_first_lines > last_line-num_last_lines+1) {
      return false;
   }

   // check if there are no lines to copy unsurround
   if (first_line+num_first_lines == last_line-num_last_lines+1) {
      allowCopySurround = false;
   }

   // copy surround line not within the block
   if (invoked_from_line > 0 && (invoked_from_line < first_line || invoked_from_line > last_line)) {
      return false;
   }

   // no leading or trailing lines, then do not allow copy surround
   if ( num_first_lines <= 0 && num_last_lines <= 0 ) {
      allowCopySurround = false;
   }
   ...
   select_line();
   p_RLine=first_line;
   /* So that pressing undo after copy code block works and
      so that the correct selection is displayed when
      "Extend selection as cursor moves" is unchecked.
      Note that the built-in _select_line is used below selections
      which the user never sees.  To bad there isn't a way to temporarily suspend
   ...
   */
   ...
   answer := 'U';
   _use_timers=0;
   if (invoked_from_line > 0 && allowCopySurround) {
      // coming from the "unsurround" command, so just copy the outer statement
      if (allowCopySurround) {
         answer = 'U';
      } else {
         answer=ESC;
      }
      ...
      answer = 'D';
   } else if ( !allowCopySurround && first_line == last_line ) {
      answer = 'D';
   } else {
      // use the auto copy block form
      if (!hideCopyLine || def_prompt_for_delete_code_block) {
         int orig_color_flags = p_color_flags;
         p_color_flags |= CLINE_COLOR_FLAG;
         answer = show("-modal _auto_copy_block_form", 
                       allowCopySurround, 
                       hideCopyLine, force);
         p_color_flags=orig_color_flags;
      } else {
   ...
   case 'D':
      // delete the current line, drop through
      // NOTE: never get here for unsurround() command case
      in_copy_surround=false;
      if ( force ) _delete_line();
      return false;

   case 'B':
      // copy block, first make a selection of the whole block
      _deselect();
      p_RLine=last_line;
      _select_line();
      p_RLine=first_line;
      ...
      // now copy the entire code block to the clipboard
      copy_to_clipboard();
      break;

   case 'U':
      // copy the outer (surround) block statement.
      // copy the leading part of the block statement
      if (num_first_lines > 0) {
         _deselect();
         p_RLine=first_line;
         _select_line();
         p_RLine=first_line+num_first_lines-1;
         _select_line();
         copy_to_clipboard(cbname);
      }

      // then copy the trailing part of the statement
      if (num_last_lines > 0) {
         _deselect();
         p_RLine=last_line-num_last_lines+1;
         _select_line();
         p_RLine=last_line;
         _select_line();
         append_to_clipboard(cbname);
...
///////////////////////////////////////////////////////////////////////////////
defeventtab _auto_copy_block_form;
void ctl_copy_line.on_create(bool allowCopySurround=false,
                             bool hideCopyLine=false,
                             bool hideCheckBox=false)
{
   ctl_always_copy_line.p_value = (int) !def_auto_copy_code_block;
   ctl_always_copy_block.p_value =(int) !def_prompt_for_copy_code_block;
   ctl_copy_surround.p_enabled = allowCopySurround;
   ctl_copy_label.p_visible = true;
   ctl_always_copy_line.p_visible = !hideCheckBox;
   ctl_always_copy_line.p_enabled = !hideCheckBox;

   if (hideCopyLine) {
      ctl_copy_line.p_visible=false;
      ctl_always_copy_line.p_visible=false;
      ctl_always_copy_line.p_enabled=false;

      delta_y := (ctl_copy_surround.p_y - ctl_copy_block.p_y);
      ctl_copy_block.p_y        -= delta_y;
      ctl_always_copy_block.p_y -= delta_y;
      ctl_copy_surround.p_y     -= delta_y;
      ctl_copy_label.p_y        -= delta_y;
      ctl_cancel.p_y            -= delta_y;
      p_active_form.p_height    -= delta_y;
   } else {
      ctl_always_copy_block.p_visible = false;
      ctl_always_copy_block.p_enabled = false;
   }

   int keys[];
   copy_key_bindings_to_form("copy-to-clipboard", ctl_copy_line.p_window_id, LBUTTON_UP, keys);
}

void _auto_copy_block_form.on_load()
{
   if ( !ctl_always_copy_line.p_visible ) {
      ctl_copy_line.p_default = false;
      ctl_copy_block.p_default = true;
      ctl_copy_block._set_focus();
   }
}
void ctl_always_copy_line.lbutton_up()
{
   ctl_copy_surround.p_enabled = (p_value==0);
   ctl_copy_block.p_enabled    = (p_value==0);

   if ( ctl_always_copy_line.p_value != 0 ) {
      ccb_cmd := "copy-code-block";
      ccb_key := _mdi.p_child._where_is(ccb_cmd);
      if (ccb_key!='') {
         _message_box("To copy entire code blocks on demand:\ninvoke the "ccb_cmd" command directly by pressing: "ccb_key".");
      } else {
         reply := _message_box("To copy entire code blocks on demand:\ncreate a key binding for the '"ccb_cmd"' command.\n\nWould you like to create a key binding now?", "SlickEdit", MB_YESNO);
         if (reply == IDYES) {
            gui_bind_command("copy_code_block");
         }
      }
   }
}
void ctl_always_copy_block.lbutton_up()
{
   ctl_copy_surround.p_enabled = (p_value==0);
   ctl_copy_line.p_enabled     = (p_value==0);

   if ( ctl_always_copy_block.p_value != 0 ) {
      ccb_cmd := "copy-surround_block";
      ccb_key := _mdi.p_child._where_is(ccb_cmd);
      if (ccb_key!='') {
         _message_box("To copy surround statements from code blocks on demand:\ninvoke the "ccb_cmd" command directly by pressing: "ccb_key".");
      } else {
         reply := _message_box("To copy surround statements from code blocks on demand:\ncreate a key binding for the '"ccb_cmd"' command.\n\nWould you like to create a key binding now?", "SlickEdit", MB_YESNO);
         if (reply == IDYES) {
            gui_bind_command("copy_surround_block");
         }
      }
   }
}
void ctl_copy_line.lbutton_up()
{
   if (ctl_always_copy_line.p_enabled && 
       ctl_always_copy_line.p_value == def_auto_copy_code_block) {
      def_auto_copy_code_block = (ctl_always_copy_line.p_value == 0);
      _config_modify_flags(CFGMODIFY_DEFVAR);
   }
   p_active_form._delete_window('D');
}
void ctl_copy_surround.lbutton_up()
{
   p_active_form._delete_window('U');
}
void ctl_copy_block.lbutton_up()
{
   if (ctl_always_copy_block.p_enabled && 
       ctl_always_copy_block.p_value == def_prompt_for_copy_code_block) {
      def_prompt_for_copy_code_block = (ctl_always_copy_block.p_value == 0);
      _config_modify_flags(CFGMODIFY_DEFVAR);
   }
   p_active_form._delete_window('B');
}
