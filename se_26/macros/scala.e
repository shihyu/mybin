...
static int find_java_tools_jar(_str &javaExe, _str &toolsJar)
{
   ...
   // Call the projconv code to compare the version, as it already knows how to 
   // deal with the version strings.
   if (pos('(\d+(?:\.\d+)*)', javaConfig, 1, 'L') > 0) {
      version := substr(javaConfig, pos('s1'), pos('1'));
      major := 0; minor := 0; sub1 := 0; sub2 := 0;
      CheckLatestJDK(version, major, minor, sub1, sub2);
      if ((major == 1 && minor >= 9) || major > 1) {
...
static int check_debug_prereqs(_str& javaExe, _str& toolsJar)
{
   ...
   if (rc) {
      _message_box("Problem checking debugger prereqs: "get_message(rc));
      return INVALID_ARGUMENT_RC;
   }
