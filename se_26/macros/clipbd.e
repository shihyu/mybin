...
bool def_copy_system_clipboards=true;
...
int cut2(bool push=true,bool doCopy=false,_str name='', bool quiet=false,int MarkFlags=-1)
{
   ...
   if ( push ) {
      if (def_copy_system_clipboards && _clipboard_format(VSCF_TEXT,true)) {
         cb_format:=_clipboard_format(VSCF_VSTEXTINFO,true);
         _str pid,cbtype,cbrest;
         parse cb_format with 'pid='pid cbtype cbrest;
         if (cbtype=='') cbtype='CHAR';
         if ((_clipboard_format(VSCF_VSTEXTINFO,true)==0 || (isinteger(pid) && getpid()!=pid)) && !_clipboard_empty(true)) {
            _cvtsysclipboard2(false,cbtype,cbrest,true,true,lexername,-1);
         }
      }
...
_command int copy_to_clipboard(_str name="",int MarkFlags=-1) name_info(','VSARG2_LASTKEY|VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_READ_ONLY|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   // if just one cursor, and no selection, and on the first line of a block, 
   // prompt for copy-code-block or copy-surround-block
   if (!_MultiCursorAlreadyLooping() && !select_active(name) && def_auto_copy_code_block) {
      if (maybe_copy_block('', 0, force:false, hideCopyLine:false, caller_fallthru_copy_line:true)) {
         return 0;
      }
   }
...
_command cut_line(_str cbname='') name_info(','VSARG2_MULTI_CURSOR|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   if ( command_state() ) {
      ...
      if (!_MultiCursorAlreadyLooping() &&
          ...
          maybe_unsurround_block(createClipboard:true, cbname, caller_fallthru_deletes_line:true)) {
         return(0);
      }
...
_command void cut_code_block() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   ...
   if (!maybe_unsurround_block(createClipboard:true, '', unsurround_line:0, force:true, hideDeleteLine:true)) {
...
{
   if (!maybe_unsurround_block(createClipboard:false, '', unsurround_line:0, force:true, hideDeleteLine:true)) {
      message("Can not delete code block starting at this line");
   }
}

/**
 * Attempts to identify the block of code starting at the current line, 
 * and prompts to copy the entire block of code to the clipboard.
 * You may retrieve the line(s) with the command <b>paste</b>.
 *
 * @return  Returns 0 if successful.  Common return codes are 1 (no block at cursor)
 * and TOO_MANY_SELECTIONS_RC.  On error, message is displayed.
 *
 * @see copy_to_clipboard
 * @see copy_surround_block
 * @see select_code_block 
 * @see unsurround
 *
 * @categories Combo_Box_Methods, Edit_Window_Methods, Editor_Control_Methods, Text_Box_Methods
 */
_command void copy_code_block() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_MARK)
{
   if (select_active()) {
      copy_to_clipboard();
      return;
   }
   if (!maybe_copy_block('', 0, force:true, hideCopyLine:true)) {
      message("Can not copy code block starting at this line");
   }
}

/**
 * Attempts to identify the block of code starting at the current line, 
 * and prompts to copy the outermost surrounding statement of the block of code to the clipboard.
 * You may retrieve the line(s) with the command <b>paste</b>.
 *
 * @return  Returns 0 if successful.  Common return codes are 1 (no block at cursor)
 * and TOO_MANY_SELECTIONS_RC.  On error, message is displayed.
 ...
 * @categories Combo_Box_Methods, Edit_Window_Methods, Editor_Control_Methods, Text_Box_Methods
 */
_command void copy_surround_block() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   if (!maybe_copy_block('', p_RLine, force:true, hideCopyLine:true)) {
      message("Can not copy code block starting at this line");
   }
}
...
static int _cvtsysclipboard2(bool make_temp_view,_str cbtype,_str cbrest,bool make_temp_utf8=true,bool isClipboard=true,_str lexername='',int MarkFlags=-1)
{
   ...
   if (make_temp_view) {
      ...
      activate_window(_clipboards_view_id);
      old_append_next_clipboard:=_append_next_clipboard;
      _append_next_clipboard=false;
      push_clipboard_itype(cbtype,'',0,p_UTF8,p_lexer_name,_MultiCursorCount());
      _append_next_clipboard=old_append_next_clipboard;
   }
