...
_str ALL_BEAUT_LANGUAGES[] = {'c', 'm', 'java', 'scala', 'cs', 'js', 'xml', 'xhtml', 'phpscript', 'html', 'docbook', 'bas', 'vbs', 'cfml', 'vpj', 'xsd', 'ant',
                              'android', 'tld', 'py', 'systemverilog','verilog', 'groovy', 'json', 'typescript'};
...
const VSCFGP_BEAUTIFIER_BRACELOC_REQUIRES= "braceloc_requires";
...
_command void beautify_with_profile(_str profile=BEAUTIFIER_DEFAULT_PROFILE) name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   ...
   if (rc < 0) {
      ...
      // For a full beautify, we ignored adaptive formatting/editorconfig, so 
      // force it to re-evaluate the adaptive settings.
      if (!has_selection) {
         update_profile_from_aff(ibeautifier);
      }
   }
...
void _c_update_brace_settings(int bes, _str lang, int ibeautifier) {
   ...
   _beautifier_set_property(ibeautifier,VSCFGP_BEAUTIFIER_BRACELOC_REQUIRES,bstyle);
}
...
static _str gKeywordToConfigKey:[] = {
   ...
   'requires' => VSCFGP_BEAUTIFIER_BRACELOC_REQUIRES,
...
int beaut_initial_anonfn_indent() {
   lang := p_LangId;
   if (lang == 'm' || lang == 'js' || lang == 'typescript') {
      ibeautifier := _beautifier_cache_get(lang,p_buf_name);
...
int beaut_anon_fn_indent_relative() {
   lang := p_LangId;
   if (has_beautifier_profiles(lang) && (lang == 'js' || lang == 'typescript')) {
      ibeautifier := _beautifier_cache_get(lang,p_buf_name);
