...
_command int '/',find,l(_str string='',_str options=null,VSSEARCH_BOUNDS vssearch_bounds=null) name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   ...
   if (status && status != INVALID_REGULAR_EXPRESSION_RC && status != COMMAND_CANCELLED_RC && confirm_wrap(new_flags)) {
      save_pos(auto p);
                ...
                typeless doSearchPrompt='',int editorctl_wid=0 )
{
   ...
   if ( key:==BACKSPACE ) {  /* Incremental search must be on. */
      ...
      if (!notfound || re_search) {
        ...
        save_search(auto junk,old_search_flags,old_word_re,old_search_reserved,old_search_flags2);
      }
...
static void _init_markers()
{
   ...
   _MarkerTypeSetPriority(gIncSearchMarker, VSMARKER_TYPE_PRIORITY_INCREMENTAL_SEARCH);
}
