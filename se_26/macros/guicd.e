 ...
 * {@code file_file} is specified, or the {@code flags} 
 * are not {@code CDN_PATH_MUST_EXIST}.  Any other options
                ...
                bool allow_custom_folders = false)
{
   ...
   */
   ...
   if (!show_wildcard_option) {
      ...
      int yDiff;
      if (_isUnix()) {
         yDiff = (ctlwildcard.p_y_extent - ctlcustomfolders.p_y);
      } else {
         yDiff = (!allow_custom_folders) ? (ctlwildcard.p_y_extent - ctlrecursive.p_y) : (ctlwildcard.p_y_extent - ctlcustomfolders.p_y);
      }
      ctlok.p_y -= yDiff;
      ctlcancel.p_y = ctlhelp.p_y = ctlok.p_y;
      p_active_form.p_height -= yDiff;
...
{
   _file_types_add_wildcards_to_combo(filetypes,add_allfiles,true);
}
