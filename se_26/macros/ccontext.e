                         ...
                         VS_TAG_RETURN_TYPE &prefix_rt=null)
{
   ...
   if (tag_tree_type_is_class(cur_type_name) && !endsWith(cur_context, cur_tag_name)) {
   ...
   if (cur_context != "") {
      ...
      if (!_c_parse_return_type(errorArgs, tag_files,
         ...
         if (_chdebug) {
            isay(depth, "_c_find_context_tags H"__LINE__": cur_context="cur_context);
         }
         if (_chdebug) {
   ...
   if ((context_flags & SE_TAG_CONTEXT_ONLY_LOCALS) && prefixexp == "") {
      ...
      if (!(context_flags & SE_TAG_CONTEXT_ONLY_LOCALS)) {
         ...
         status = tag_list_in_class(lastid, rt.return_type,
                                    ...
                                    visited, depth+1);
         // if looking for a destructor, also include the class name
         if (_first_char(lastid) == '~') {
            tag_split_class_name(rt.return_type, auto inner_class_name, auto outer_class_scope);
            if (_chdebug) {
               isay(depth, "_c_find_context_tags: LOOK FOR DESTRUCTOR CLASS, inner_class_name="inner_class_name" outer_class_scope="outer_class_scope);
            }
            if (exact_match && lastid == '~':+inner_class_name) {
               if (outer_class_scope == "") {
                  ...
                  tag_list_in_class(inner_class_name, outer_class_scope,
                                    0, 0, tag_files,
                                    num_matches, max_matches,
                                    SE_TAG_FILTER_ANY_STRUCT, context_flags,
                                    exact_match, case_sensitive,
                                    rt.template_args, friend_list, 
                                    visited, depth+1);
               }
...
void _c_check_context_for_private_scope(VS_TAG_RETURN_TYPE &rt, _str cur_class_name, int depth=0)
{
   ...
   if (cur_context_id == 0) {
      ...
      if (class_pos>0 && class_pos+length(cur_class_name)==length(rt.return_type)+1) {
         ...
         // if the type is an inner class, we have private access to it from
         // the scope of the outer class (Java and C#, not C++)
         if (_LanguageInheritsFrom("java") || _LanguageInheritsFrom("cs")) {
            if (pos(cur_class_name:+VS_TAGSEPARATOR_class, rt.return_type) == 1) {
               rt.return_flags |= VSCODEHELP_RETURN_TYPE_PRIVATE_ACCESS;
               if (_chdebug) {
                  isay(depth, "_c_check_context_for_private_scope: OUTER CLASS");
               }
            }
         }
      }
