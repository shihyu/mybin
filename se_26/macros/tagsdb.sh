...
const VS_TAG_USER_VERSION_64BITFLAGS= 26000;
/**
 ...
 */
const VS_TAG_LATEST_VERSION=          26000;
...
enum_flags SETagFlags {
    ...
    * Opaque is like a black-box, meaning the symbols inside 
    * the enum or namespace have to be qualified when referenced. 
    */
    ...
    * from declarations to definitions (and vice-versa), is never required. 
   ...
   /**
    * C++ constexpr - a compile-time constant, can be used for both 
    * variables and functions.  All constexpr variables are const.
    */
   ...
   /**
    * C++20 consteval - A consteval function is guaranteed to be evaluated at compile time.
    */
   SE_TAG_FLAG_CONSTEVAL         = 0x0000020000000000,
   /**
    * C++20 constinit - Since C++20, asserts that a variable has static 
    * initialization (zero initialization or constant initialization) 
    */
   ...
   /**
    * Indicates that this symbol, despite being a statement type, should be 
    * included in the current context, and optionally displayed in the outline.
    */
   SE_TAG_FLAG_IN_CONTEXT        = 0x0000100000000000,
   /**
    * Indicates that this symbol is a local struct, object, function, etc, 
    * nested inside another declaration body.  This is a symbol which 
    * should be displayed in the Defs tool window, be seen by next-tag/prev-tag, 
    * and be recognized as the curent function under the cursor, but since it 
    * is locally declared, it should be ignored by context tagging searches.
    */
   SE_TAG_FLAG_NESTED_LOCAL      = 0x0000200000000000,

   /**
    * Asynchronous method or statement (runs in thread)
    */
   SE_TAG_FLAG_ASYNCHRONOUS      = 0x0000400000000000,
   /**
    * Factory method for creating class instances.
    */
   SE_TAG_FLAG_FACTORY           = 0x0000800000000000,
   /**
    * Late / lazy variable initialization
    */
   SE_TAG_FLAG_LATE              = 0x0001000000000000,
};
...
enum_flags SETagContextFlags {
   ...
   /**
    * Attempt to find symbol matches after correcting a transposed character, 
    * missing character, repeated character, or mistyped character. 
    */
   SE_TAG_CONTEXT_MATCH_CORRECTIONS   = 0x0000020000000000,

   /**
    * Only look for this symbol in the current search class.  Do not 
    * look in parent classes or the global scope. 
    */
   SE_TAG_CONTEXT_ONLY_SEARCH_CLASS   = 0x0000040000000000,
   ...
   SE_TAG_CONTEXT_RESTRICTIVE_FLAGS   = ( SE_TAG_CONTEXT_ONLY_VOLATILE        |
                                          ...
                                          SE_TAG_CONTEXT_ONLY_SEARCH_CLASS    |
...
enum_flags AutoCodeInfoFlags {
   ...
   VSAUTOCODEINFO_DO_ACTION_MASK = (VSAUTOCODEINFO_DO_AUTO_LIST_PARAMS|
                                    VSAUTOCODEINFO_DO_FUNCTION_HELP|
                                    VSAUTOCODEINFO_DO_LIST_MEMBERS|
                                    VSAUTOCODEINFO_OPERATOR_TYPED),

   // mask for auto-code flags that indicate a certain kind of context
   VSAUTOCODEINFO_CONTEXT_MASK = (VSAUTOCODEINFO_IN_INITIALIZER_LIST    |
                                  VSAUTOCODEINFO_IN_GOTO_STATEMENT      |
                                  VSAUTOCODEINFO_IN_THROW_STATEMENT     |
                                  VSAUTOCODEINFO_IN_PREPROCESSING       |
                                  VSAUTOCODEINFO_IN_JAVADOC_COMMENT     |
                                  VSAUTOCODEINFO_IN_STRING_OR_NUMBER    |
                                  VSAUTOCODEINFO_IN_IMPORT_STATEMENT    |
                                  VSAUTOCODEINFO_HAS_CLASS_SPECIFIER    |
                                  VSAUTOCODEINFO_IN_PREPROCESSING_ARGS  |
                                  VSAUTOCODEINFO_OBJECTIVEC_CONTEXT     |
                                  VSAUTOCODEINFO_HAS_FUNCTION_SPECIFIER ),

   // mask for auto-code flags that indicate a certain kind of following context
   VSAUTOCODEINFO_FOLLOW_MASK = (VSAUTOCODEINFO_LASTID_FOLLOWED_BY_PAREN      |
                                 VSAUTOCODEINFO_LASTID_FOLLOWED_BY_BRACKET    |
                                 VSAUTOCODEINFO_LASTID_FOLLOWED_BY_ASSIGNMENT |
                                 VSAUTOCODEINFO_LASTID_FOLLOWED_BY_BRACES),
...
struct VS_TAG_IDEXP_INFO {
   ...
   AutoCodeInfoFlags info_flags;    // bitset of VSAUTOCODEINFO_*
...
/**
 * Information needed to call find context tags callbacks.
 */
struct VS_TAG_FIND_TAGS_INFO {
   ...
   /**
    * Current language mode (ID)
    */
   _str langId;

   /**
    * Search for parents of classes
    */
   bool find_parents;

   /**
    * Search for exact matches (whole word matches)
    */
   bool exact_match;
   ...
   /**
    * Bitset of symbol filtering flags (SE_TAG_FILTER_*)
    */
   SETagFilterFlags  filter_flags;

   /**
    * Bitset of context tagging flags (SE_TAG_CONTEXT_*)
    */
...
/**
 * Returns next tag name which is a prefix match of {@code name}.
 * {@code find_first} must be 1 to initialize matching.
 ...
 * @param context_flags (optional) bitset of SE_TAG_CONTEXT_*
 * 
 ...
 *
 * @param yes_or_no  {@code true} to configure databse to use memory mapped files
 *
 * @return 0 on success, &lt;0 on error. 
 *  
 * @since 20.0
 ...
 *
 * @param yes_or_no  {@code true} to configure databse to use independent cacahing
 *
 * @return 0 on success, &lt;0 on error. 
 *  
 * @note 
...
/**
 * @return Returns {@code true} if the current database is open for writing.
 *
 * @categories Tagging_Functions
 */
 ...
 * This is an exclusive lock which keeps all other threads out of the database. 
 *  
 * @return Returns {@code true} if we were able to get the lock. 
 *
 * @categories Tagging_Functions
 */
 ...
 * @param included_by
 *                  name of file that {@code file_name} was included by
 ...
 * This corresponds to the {@link p_LangId} property of {@code file_name}, 
 ...
 *     SE_TAG_TYPE_SWITCH           91/switch      Switch statement
 * </pre>
 ...
 *                   <dt>SE_TAG_FLAG_ASYNCHRONOUS<dd style="margin-left:135pt"> Asynchronous method or statement (runs in thread)
 *                   <dt>SE_TAG_FLAG_FACTORY<dd style="margin-left:135pt">      Factory method or constructor
 *                   <dt>SE_TAG_FLAG_LATE<dd style="margin-left:135pt">         Late / lazy variable initialization
 *                   </dl>
 * 
...
/**
 * @return 
 * Return {@code true} if a symbol with the given tag type and flags can be 
...
/** 
 * @return 
 * Return {@code true} if a symbol with the given tag type, flags, 
 ...
 * @param include_class       if {@code false}, does not include class name
 * @param include_args        if {@code false}, does not include function or template signature
 * @param include_tab         append class name after signature if {@code true},
 *                            prepend class name with :: if {@code false}.
 *
 ...
 * @param tagList       [output[ array of JavaDoc tags (keys of {@code hashtab})
 ...
 * 
 * @return {@code true} if the variable is in scope, false otherwise
 *
 * @categories Tagging_Functions 
 * @since 21.0
 */
...
/**
 * Kill all locals after {@code local_id}, not including {@code local_id}
 * <p> 
 * For synchronization, macros should perform a tag_lock_context(true)
 * prior to invoking this function.
 *
 ...
 * {@code exact}, with the exact tag name.  Use case-sensitive match if
 * case_sensitive != 0.
 ...
 * @param filter_all    if {@code true}, allow all the matches to be filtered out
 *
 * @categories Tagging_Functions
 */
 ...
 * @param longest_case_prefix          (output) set to longest prefix with matches {@code lastid} case-sensitive
...
/** 
 * Walk through all the matches in the current set of symbol pattern matches 
...
/** 
 * Walk through all the matches in the current set of fuzzy symbol matches 
 * and compute the longest common prefix match, and also determine if it is 
 * unique, and if for all prefix matches of the same length, if they have 
 * the same casing. 
 *  
 ...
 * 
 * @return Returns {@code true} if the symbol matches the given pattern.
 *
 * @categories Tagging_Functions
 */
 ...
 * 
 * @return Return {@code true} if the symbol matches the given identifier.
 *
 * @categories Tagging_Functions
 */
 ...
 * @param depth            (optional) depth of recursive search
 *
 * @return 1 if {@code child_class} derives from {@code parent_class}, otherwise 0.
 *
 * @see tag_find_class
 * @see tag_get_inheritance
 *
...
/**
 * Lookup {@code symbol} and see if it could be an enumerated type.
 ...
 * @param depth            (optional) depth of recursive search
 *
 * @return 1 if {@code symbol} could be an enum, 0 otherwise, &lt;0 on error.
 *
 * @categories Tagging_Functions
 */
...
/**
 * Lookup {@code symbol} and see if it could be an import.
 * If so, return 1, otherwise return 0.
 * <p> 
 * For synchronization, threads should perform a tag_lock_context(false) 
 * prior to invoking this function.
 ...
 * @param depth            (optional) depth of recursive search
 *
 * @return 1 if {@code symbol} could be an import, 0 otherwise, &lt;0 on error.
 *
 * @categories Tagging_Functions
 */
 ...
 * However, if the {@code arglist} variable is passed in, then it will return more complex 
 ...
 * @param line_no       If found in current context, define must be before {@code line_no}
...
/**
 * Look up <i>class_name</i> and see if it is a package, namespace, module or unit.
 * <p> 
 * If not, strip off the last part of the class name until we find a class 
 * name which is a package name. 
 * <p> 
 * For synchronization, threads should perform a tag_lock_context(false) 
 * prior to invoking this function.
 *
 * @param class_name       Class name to find package prefix of
 * @param tag_files        (reference to _str[]) list of tag files to search
 * @param case_sensitive   Specifies case sensitive comparison
 * @param visited          (optional) hash table of prior results
 * @param depth            (optional) depth of recursive search
 *
 * @return Returns package part of class name.  Returns "" if the class name 
 ...
 * @categories Tagging_Functions
 */
extern _str tag_get_package_from_class_name(_str class_name, _str (&tag_files)[],
                                            bool case_sensitive,
                                            typeless &visited=null, int depth=0);
...
/**
 * List the files containing {@code tag_name}, matching by prefix or
...
/**
 * Find the next occurrence of the symbol {@code tag_name} in the current file, 
...
/**
 * For each item in {@code class_parents}, normalize the class and place it in
 * {@code normal_parents}, along with the tag type, placed in {@code normal_types}.
 *
 ...
 * 
 * @return Returns &lt;0 if the {@code c1} is less than {@code c2}, 0 if they match,
 *         and &gt;0 if {@code c1} is greater than {@code c2}, much like strcmp().
 */
 ...
 * class {@code search_class} is found, othewise returns 0, indicating
 ...
 * @param depth           (optional) depth of recursive search
 *
 * @return qualified name if successful, {@code search_name} on error.
 *
 * @categories Tagging_Functions
 */
 ...
 * thus disabling any filtering, unless {@code strict} is true.
 * The current object must be an editor control or current buffer.
 * <p> 
 * For synchronization, macros should perform a tag_lock_matches(true) 
 * prior to invoking this function. 
 ...
 * @param case_sensitive  Case sensitive match or case-insensitive?
 *
 * @return 0 on success, 1 if list was truncated,  &lt;0 on error. 
 *  
 * @see tag_match
 * @since 5.0a
 *
 ...
 * @param filter_flags    Tag filter flags, only insert tags passing this filter.
 *                        See {@link tag_filter_type} for more details.
 * @param context_flags  Tag context flags, bitset of SE_TAG_CONTEXT_* 
 *
 ...
 * the given position {@code i} (first is 0).
 * <p> 
 * For synchronization, threads should perform a tag_lock_context(true) 
 * prior to invoking this function.
 *
 ...
 * prior to invoking this function.
 *  
 * @param countUnparsedOnly   If {@code true} only return the number of embedded 
...
/**
 * Disect the next argument (starting at {@code arg_pos}) from the
 * given parameter list (params).  Place the result in {@code argument}.
 ...
 * <p>
 * If you do not know the number of bytes to allocate to {@code argument},
 * call this with {@code argument} as NULL, and calculate ({@code arg_pos} - {@code orig_pos} + 1).
 * 
 ...
 * 
 * @return Returns the position where the argument begins in {@code params}.
 *         Note, this may differ from the original value of {@code arg_pos}
 *         if there are leading spaces.  Returns STRING_NOT_FOUND_RC 
 *         and sets {@code argument} to the empty string if there are no more
 ...
 * <p>
 * If {@code ext} is not specified, the current object
 * must be an editor control.
 *
 * @param parent     extension to compare to
 * @param ext        current language (default=p_LangId)
 * 
 * @return {@code true} if the extension matches, {@code false} otherwise.
 *
 * @categories Tagging_Functions
 ...
 * @param deferSlowerJobs  (input) if {@code true}, defer slower jobs that 
...
/**
 * @return 
 * Return {@code true} if local variable tagging is supported for the given language. 
 * 
 * @param langId     language mode identifier
 * 
 * @categories Tagging_Functions
...
/**
 * @return 
 * Return {@code true} if statement-level tagging is supported for the given language.
 * 
 * @param langId     language mode identifier
 * 
 * @categories Tagging_Functions
...
/**
 * @return 
 * Return {@code true} if tokenization is supported for the given language. 
 * 
 * @param langId     language mode identifier
 * 
...
/**
 * @return 
 * Return {@code true} if the given language's parser creates positional keywords tokens.
 * 
 * @param langId     language mode identifier
 * 
 * @see tag_lang_has_list_tags()
...
/**
 * @return 
 * Return {@code true} if the given language's parser supports incremental tagging.
 * 
 * @param langId     language mode identifier
 * 
 * @see tag_lang_has_list_tags()
 ...
 * @param isRunning        set to {@code true} if the build is still running
