...
_command void activate_context(_str noDropDown="") name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   // no "Current Context" form.
   // What if they had added the current context control to another toolbar?
   formwid  := _find_formobj('_tbcontext_form', 'n');
   if (!formwid) {
      foreach (auto tb in def_toolbartab) {
         wid := _tbGetWid(tb.FormName);
         if ( wid > 0 ) {
            ctlwid := wid._find_control('_tbcontext_combo_etab');
            if ( ctlwid > 0 ) {
               ctlwid._set_focus();
               if (noDropDown==1) return;
               ctlwid.call_event(ctlwid,F4,'w');
               return;
            }
         }
      }
...
_command activate_open,activate_project_open(bool restore_group=true,_str initial_directory='')  name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY)
{
   //activate_project_tab('Open','_openfile_list');
   status:=activate_tool_window('_tbopen_form', true, '_file_name_filter', restore_group);
   if (status>0 && initial_directory!='') {
      _nocheck _control _file_name_filter;
      status._file_name_filter.p_text=initial_directory;
      p:=length(initial_directory)+2;
      status._file_name_filter._set_sel(p,p);
   }
   return status;
}
