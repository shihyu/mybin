...
bool def_mfreplace_quick_save=true;
...
/**
 * Performs multi-file search and replace
 * 
 * @param search_string  String to search for
 * @param replace_string Replace search string with this string
 * @param options        Search options
 * @param files          Space delimited list of file paths
 * @param wildcards      Wildcard specifies for file paths 
 *                       specified.
 * @param file_exclude   Exclude wildcards
 * @param mfflags        Multi-file find flags
 * @param grep_id        Id of grep buffer
 * @param show_diff      Indicates whether to show diff
 * @param quick_save     Purpose is to speed up saves during multi-file replace. 
 *      When 1, disable backup history and special save options for files not already loaded and retag files later.
 *      When 0, does slow save.
 *      When -1, retrieve this setting from def_mfreplace_quick_save.
 * 
 * @return _str 
 */
_str _mfreplace2(_str search_string, _str replace_string, _str options,
                _str files, _str wildcards=ALLFILES_RE, _str file_exclude = '',
                 int mfflags = 0, int grep_id = 0, bool show_diff = false,int quick_save=-1)
{
   return(_mfreplace(search_string, replace_string, options, files, '', false, false, wildcards, file_exclude, true, mfflags, grep_id, show_diff,'',quick_save));
}
/**
 ...
 *
 * @param notused_atbuflist  Not used
 *
...
_str _mfreplace(_str search_string, _str replace_string, _str options,
                ...
                int mfflags = 0, int grep_id = 0, bool show_diff = false, _str file_stats = '',int quick_save=-1)
{
   _macro('m', 0);
   int window_list[];                             // List of Window ID's
   int i,count;
...
static int _mffind2util(_str search_string,_str options,_str files,int mfflags,bool searchProjectFiles,bool searchWorkspaceFiles,_str wildcards='',_str file_exclude='',bool files_delimited_with_semicolon=false,int grep_id=0,int before_lines=0,int after_lines=0,_str file_stats='')
{
   ...
   grep_flags:=VSGREPFLAG_DISPLAY_STATUS|VSGREPFLAG_LIST_BUFFERS;
   if (!def_search_result_sorted_filenames) {
      grep_flags|=VSGREPFLAG_EXPAND_WILDCARDS;
   }

   int status=_mfinit(list_view_id,files,wildcards,file_exclude,files_delimited_with_semicolon,searchProjectFiles,searchWorkspaceFiles,mfflags & MFFIND_LOOKINZIPFILES,def_search_result_sorted_filenames!=0);
   if (status) {
      mou_hour_glass(false);
      clear_message();
   ...
   orig_mfflags:=mfflags;

   _mffind_file_stats_init('' /*file_stats*/, args.file_stats);
   //status=_grep_file_list(list_view_id,args.TotalFilesSearched,search_string,options,_mffindProcessFile,args,maybe_cancel_callback,mfflags,
   //                       def_search_threads,VSGREPFLAG_DISPLAY_STATUS|VSGREPFLAG_LIST_BUFFERS);
   VSGREP_ARGS grep_args;
   grep_args.mfflags=mfflags;
   grep_args.search_string=search_string;
   grep_args.options=options;
   grep_args.callback=_mffindProcessFile;
   grep_args.callback_args=args;
   grep_args.file_stats=file_stats;
   //_str buffer_list[];
   grep_args.wildcards=wildcards;
   grep_args.file_exclude=file_exclude;
   grep_args.attr_flags=def_find_file_attr_options;

   status=_grep_file_list(list_view_id,grep_flags,def_search_threads,grep_args, auto TotalFilesSearched);
   args=grep_args.callback_args;
   args.TotalFilesSearched=TotalFilesSearched;
   activate_window(list_view_id);
   ...
   if (mfflags & MFFIND_SINGLE) {
      ...
      if (mfflags & MFFIND_FILESONLY) {
         if (orig_mfflags & MFFIND_FILESONLY) {
            results_text='Matching files: 'args.NofFileMatches'     Total files searched: 'args.TotalFilesSearched;
         } else {
            results_text='Total found (truncated): 'args.NofMatches'     Matching files: 'args.NofFileMatches'     Total files searched: 'args.TotalFilesSearched;
         }
      } else {
...
static int _mfreplaceProcessFile(MFREPLACE_ARGS &args,_str path,long offset=-1) {
   ...
   quick_save:= !needs_edit && (args.quick_save>0 || (args.quick_save<0 && def_mfreplace_quick_save)) && !(args.mfflags&MFFIND_LIST_CURRENT_CONTEXT);
   /* For better save performance on Windows, load
      the entire file if the file is 1MB or less.
   */
   load_files_status = _open_temp_view(path, load_files_view_id, auto junk, quick_save?('-L:1000 'load_options):load_options, file_already_loaded, false, !quick_save || args.color_flags!=0, 0, false, false);
   if (file_already_loaded) {
      quick_save=false;
   }
   if (quick_save) {
   ...
   if (edit_called) {
      ...
      if (!args.doDiff && (_Nofchanges > 0) && p_modify) {
         if (!quick_save) {
            ...
            typeless t=(typeless)_time('b');
            messageAge := t-args.last_message_time;
            if (!args.last_message_time || messageAge>=600) {
               args.last_message_time=t;
               //say(nls('Saving %s',p_buf_name));
               message(nls('Saving %s',p_buf_name));
            }
            status=_save_file('+o '_maybe_quote_filename(p_buf_name));
            if (status) {
               _message_box(nls("Unable to save '%s'",p_buf_name'. 'get_message(status)));
            }
            if (!status) {
               args.did_quick_save=true;
               p_modify=false;
            }
         }
         if (status) {
...
static _str _mfreplace2util(_str search_string, _str replace_string, _str options, _str files,
                            ...
                            _str wildcards = '', _str file_exclude = '', bool files_delimited_with_semicolon = false,
                            int mfflags = 0, int grep_id = 0, _str file_stats = '',int quick_save=-1)
{
   ...
   grep_flags:=VSGREPFLAG_DISPLAY_STATUS|(doDiff?0:VSGREPFLAG_LIST_BUFFERS);
   if (!def_search_result_sorted_filenames) {
      grep_flags|=VSGREPFLAG_EXPAND_WILDCARDS;
   }
   ...
   args.results=results;
   args.Nofchanges=0;
   args.color_flags=color_flags;
   args.quick_save=quick_save;
   args.did_quick_save=false;
   args.last_message_time=0;
   ...
   _mffind_file_stats_init('', args.file_stats);
   //status=_grep_file_list(list_view_id,unused,search_string,options,_mfreplaceProcessFile,args,maybe_cancel_callback,mfflags,
   //                       def_search_threads,VSGREPFLAG_DISPLAY_STATUS|(doDiff?0:VSGREPFLAG_LIST_BUFFERS));

   VSGREP_ARGS grep_args;
   grep_args.mfflags=mfflags;
   grep_args.search_string=search_string;
   grep_args.options=options;
   grep_args.callback=_mfreplaceProcessFile;
   grep_args.callback_args=args;
   grep_args.file_stats=file_stats;
   //_str buffer_list[];
   grep_args.wildcards=wildcards;
   ...
   filewatcherSetSuspendState(1);
   status=_grep_file_list(list_view_id,grep_flags,
      def_search_threads, grep_args, auto TotalFilesSearched);
   filewatcherSetSuspendState(0);

   args=grep_args.callback_args;
   if (args.did_quick_save) {
      _actapp_makefile(_AppHasFocus()?'':'0');
   }
   args.TotalFilesSearched=TotalFilesSearched;
   if (doMFUndo) {
      _MFUndoEnd();
...
int _mfinit(int &temp_view_id,_str files,_str wildcards='',_str file_exclude='',bool files_delimited_with_semicolon=true,
             bool searchProjectFiles=false,bool searchWorkspaceFiles=false,int look_in_zipfiles=0,bool expandWildcards=true
            )
{
   orig_view_id := 0;
   get_window_id(orig_view_id);
   int status=bgm_gen_file_list(temp_view_id,files,wildcards,file_exclude,files_delimited_with_semicolon,searchProjectFiles,searchWorkspaceFiles,expandWildcards,false,null,look_in_zipfiles);
   if (status) {
      if (status<0) {
