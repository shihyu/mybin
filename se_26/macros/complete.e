...
_str gcompletion_extensions_re;
...
void complete(_str completion_info=null)
{
   ...
   if ( col>1 && _Substr(line,col,1):==' ' && _Substr(line,col-1):!=' ' &&
      ...
      bool more_args_may_follow=false;
      ...
      if ( arg_number<=1 && completion_info==null ) {
         ...
         if ( completion_info!=null ) {
            status=_find_match_fun(line,arg_number,fun_name_prefix,false,completion_info,one_argument_text_box,null, more_args_may_follow);
         } else {
            status=_find_match_fun(line,arg_number,fun_name_prefix,false,completion_info,one_argument_text_box,null, more_args_may_follow);
         }
         if ( status || word=='' ) {
      ...
      expand_word(line,word,start_word_col,fun_name_prefix,command_case,one_argument_text_box,more_args_may_follow);
      return;
  }
...
/**
 * Replace the {@code word}, starting at {@code start_word_col}
 * with {@code new_word}.
 * 
                              ...
                              bool semicolon_separator=false)
{
   ...
   if (gcompletion_extensions_re!='') {
      for (;name:!='';) {
         if ((last_char(name)==FILESEP && (!endsWith(name,FILESEP"."FILESEP) ||!endsWith(name,FILESEP".."FILESEP)))
             || pos(gcompletion_extensions_re,name,1,'r'_fpos_case)) {
            break;
         }
         name=file_match(file_match_options'"'name,0);
      }
   }
...
_str def_binary_ext='.vtg .sx .ex .vsb .obj .exe .lib .dll .pdb .qfy .bmp .jpg .gif .ico .winmd .o .a .so .sl .svgz';
...
/**
 * Match only the word specified by {@code name}.
 * 
 * @param name          name to match
 * @param find_first    'true' to find first, 'false' to find next
 * 
 * @return {@code name} if {@code find_first} is true, '' otherwise.
 * 
 * @categories Completion_Functions
 */
 ...
 * {@code find_first} must be non-zero to initialize matching.
      ...
      /*if (p_Noflines==1 && name==result) {
         ...
         while(!down()) {
            ...
            if (doAddToArgumentCompletion) {
               ...
               tag_auto_complete_add_result(AUTO_COMPLETE_FILES_PRIORITY,
                                            displayText, "",
                        ...
                        bool more_args_may_follow)
{
   ...
   */ 
   ...
   if (dquote_ch:!='' || 
       (add_dquote && !(is_file_match_fun && last_char(strip_word)==FILESEP && NofSpaceMatches==0))) {
   ...
   if (more_args_may_follow) {
      match_flags|= MAYBE_APPEND_SPACE_MATCH;
   }
...
/**
 * @return 
 * Escape the characters in {@code cmdline} to prevent
