 ...
 */
_str convert_rgb_to_html_color(_str val, bool use_name=false)
{
 ...
 * This macro converts it to that format.
 */
int convert_html_color_to_rgb(_str val)
{
   if (isinteger(val)) {
      return (int)val;
   }
      ...
      if (_select_type() == 'CHAR') {
         ...
         if (end_tag !='') {
            ...
            if (textAfterCursor!="") {
               ...
               insert_line(indent_string(indent_col-1):+textAfterCursor);
               if (eline != sline) {
                  // Occurence where same line character selection w/ text after
                  up();
                  eline=p_line;
               }
            }
...
static _str get_html_color_attribute_value(_str text=null)
{
   ...
   color := _form_parent().convert_rgb_to_html_color(p_backcolor, true);
   return color;
}
