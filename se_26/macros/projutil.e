...
bool _IsVisualStudioWildcardProject(int handle)
{
   result := false;
   if (handle >= 0) {
      node := _xmlcfg_find_simple(handle, "/Project");
      if (node > 0) {
         if (_xmlcfg_get_attribute(handle,node,'Sdk')!='') {
            return true;
         }
         node = _xmlcfg_find_simple(handle, "/Project/Sdk");
         if (node > 0) {
            if (_xmlcfg_get_attribute(handle,node,'Name')!='') {
               return true;
            }
         }
      }
...
_command edit_file_in_workspace,ew(_str filenames='') name_info(WORKSPACE_FILE_ARG'*,'VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   ...
   foreach (auto f in filenames) {
      ...
      // qualify the file name by search for it in the current project
      foundFileList := _WorkspaceFindFile(f, _workspace_filename, false, false, true,false,def_filematch_case_insensitive);
      if (foundFileList == '') {
         foundFileList = _maybe_quote_filename(localFindFile(f));
         if (foundFileList == '') {
            _message_box("File not found: "f);
 ...
 *                               When true, do case insensitive
 *                               file matching even when on case
 *                               sensitive platform. Ignored on
...
_str _WorkspaceFindFile(_str filename, 
                        ...
                        bool force_case_insensitive_matching=false)
{
   _str projectList[];
   ...
   for (i := 0; i < projectList._length(); i++) {
      ...
      _str foundFileList = _projectFindFile(workspaceName, projectName, filename, (int)checkPath, 1, 1,force_case_insensitive_matching);
