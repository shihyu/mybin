...
static _str gSetActiveConfigTo;
 ...
 static bool gDoNotShowProjectProperties;
...
static _str gSpecialToolMessage:[]=
{
   ...
   "dartoptions"=>"Displays dialog for setting options for Dart tools"
};
...
{
   if (gDoNotShowProjectProperties) {
      return;
   }
   ProjectFilename := "";
                           ...
                           bool runInitMacros=true)
{
   ...
   gDoNotShowProjectProperties=false;
   if (!ShowPropertiesDialog) {
      gDoNotShowProjectProperties=true;
      initmacro = translate(initmacro,'_','-');
      if (name_eq(initmacro, "show_project_properties_files_tab") || name_eq(initmacro, "project_edit")) {
         initmacro = "";
      }
   }
...
void ctlproperties.lbutton_up()
{
   ...
   f.Excludes=_project_encode_special_excludes(f.Excludes);
   ...
   if (f.Excludes!='') {
      f.Excludes=_project_xlat_special_excludes(f.Excludes);
...
{
   return(!_IsWorkspaceAssociated(_workspace_filename));
}
...
void _add.lbutton_up()
{
   ...
   result := p_window_id._OpenDialog("-modal",
                         ...
                         _file_types_append_filters(def_file_types,EXTRA_FILE_FILTERS),
...
void _addWildcardsToProject(_str basePath, _str (&includeList)[], _str (&excludesList)[], bool recursive, bool followSymlinks, _str projectName=_project_name, int projectHandle=gProjectHandle, bool showFolders=true, bool directoryFolder=false)
{
   ...
   wfa.Excludes = _project_xlat_special_excludes(join(excludesList, ';'));
   ...
   if (modified) {
      if (gIsExtensionProject || gIsProjectTemplate) {
         ...
         if (doUpdateProjectToolbar) {
            _clearProjectFileListCache(_workspace_filename, gProjectName, true);
            toolbarUpdateFilterList(gProjectName);
         }

         p_window_id.call_list("_prjupdate_",gProjectName);
         if (gUpdateTags) {
            _TagCallList(TAGFILE_ADD_REMOVE_CALLBACK_PREFIX,'','');
            _TagCallList(TAGFILE_REFRESH_CALLBACK_PREFIX);
         }
...
static void FillInSrcFileList(_str ConfigName)
{
   ...
   if (!gOrigProjectFileList) {
      _project_before_modify_files(gProjectName,gProjectHandle,gOrigProjectFileList);
...
static int _workspace_open_directory(typeless dirs,bool &dialog_displayed,bool &workspace_created)  {
   ...
   vpw_file := projectPath:+projectName:+WORKSPACE_FILE_EXT;
...
_str _project_xlat_special_excludes(_str excludes) {
   int i;
   i = pos('(^|;)<Default Excludes>($|;)',excludes,1,'ri');
   if (i>0) {
       _str default_excludes=_default_option(VSOPTIONZ_DEFAULT_EXCLUDES);
       excludes=stranslate(excludes,_escape_re_chars(default_excludes),'(#<=(^|;))<Default Excludes>(#=($|;))',"ri");
   }

   i = pos('(^|;)<Project Excludes>($|;)',excludes,1,'ri');
   if (i>0) {
       _str project_excludes=_default_option(VSOPTIONZ_DIR_PROJECT_EXCLUDES);
       excludes=stranslate(excludes,_escape_re_chars(project_excludes),'(#<=(^|;))<Project Excludes>(#=($|;))',"ri");
   }
   ...
   if (i>0) {
      ...
      excludes=stranslate(excludes,_escape_re_chars(ext_list),'(#<=(^|;))<Binary Files>(#=($|;))',"ri");
   }
   return excludes;
}
static _str _project_encode_special_exclude(_str excludes,_str expanded_excludes,_str special_exclude_name) {
   if (last_char(expanded_excludes)==';') {
      expanded_excludes=substr(expanded_excludes,1,length(expanded_excludes)-1);
   }
   int i;
   i = pos('(^|;)'_escape_re_chars(expanded_excludes)'',excludes,1,'ri');
   if (i>0) {
       excludes=stranslate(excludes,special_exclude_name,'(#<=(^|;))'_escape_re_chars(expanded_excludes)'(#=($|;))',"ri");
   }
   return excludes;
}
_str _project_encode_special_excludes(_str excludes) {
   ext_list := _LangGetExtensions("binary");
   ext_list=stranslate(ext_list,';*.',' ');
   if (length(ext_list)) {
      ext_list='*.'ext_list;
   }
   excludes=_project_encode_special_exclude(excludes,ext_list,'<Binary Files>');
   excludes=_project_encode_special_exclude(excludes,_default_option(VSOPTIONZ_DEFAULT_EXCLUDES),'<Default Excludes>');
   excludes=_project_encode_special_exclude(excludes,_default_option(VSOPTIONZ_DIR_PROJECT_EXCLUDES),'<Project Excludes>');
   return excludes;
}
...
_command int project_add_directory_folder(typeless dirs='') name_info(DIR_ARG',')
{
   ...
   if (_workspace_filename=='') {
      ...
      projectName := _getDirTitledWorkspaceName(_workspace_path());
      status := workspace_new_project(inDialog:false,
                                      _default_option(VSOPTIONZ_DIR_PROJECT_TYPE),
                                      projectName,
                                      _workspace_path(),
      ...
      if (add_as_wildcard) {
         if (directory_folder) {
            _projecttbAddDirectoryToProject(onlyUpdateProjectFile:true,
                                            _project_name, project_handle, 
                                            FolderNode: -1, 
                                            path, includeList, excludeList, 
                                            recursive, follow_symlinks, create_subfolders,
                                            warnDuplicateFiles: (workspace_already_open || !workspace_created));
         } else {
...
_command int project_add_file(_str newfilename="",bool quiet=false,_str ProjectName=_project_name, _str& msg=null)
{
   ...
   if (_FileExistsInCurrentProject(newfilename,ProjectName)) {
      bool matched_wildcard=_VPJFileMatchesWildcard(_ProjectHandle(ProjectName),newfilename);
      if (matched_wildcard) {
