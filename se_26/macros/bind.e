...
/** 
 * Finds the key(s) that are bound to {@code commandName}.
 *  
 * @param commandName   the command to look for 
 * @param quiet         specifies how results are returned (see below) 
 * @param separatorChar specifies what character to separate multiple keys with 
 *  
 * @return There return value depends on the option given for {@code quiet}: 
 * <dl compact> 
 * <dt>{@literal ""}</dt>
 * <dd>By default, this command displays the key(s) that are bound to
 * {@code commandName} on the message line and returns "".</dd>
 * <dt>{@literal 1}</dt>
 * <dd>The message that would be displayed on the message line is returned.</dd>
 * <dt>{@literal 2}</dt>
 * <dd>Returns the key(s) that are bound to {@code commandName}.</dd>
 * </dl>
 *  
...
{
  arg1  := prompt(commandName,'Where is command');
...
/**
 * @return Returns the key(s) that are bound to {@code commandName}.
 *  
 * @param commandName   the command to look for 
 * @param separatorChar specifies what character to separate multiple keys with 
 *  
