...
void _evaluate_compilers(available_compilers& compilers, _str (&compiler_names)[])
{
   ...
   for (index=0;index<compiler_names._length();++index) {
      ...
      case COMPILER_NAME_VS2019:
         compilers.hasDotNet2019=true;
         DotNETYear=2019;
         compilers.latestMS=COMPILER_NAME_VS2019;
         break;
      case COMPILER_NAME_VS2022:
         compilers.hasDotNet2022=true;
         DotNETYear=2022;
         compilers.latestMS=COMPILER_NAME_VS2022;
         break;
...
void _ProjectSet_TargetAdvCmd(int handle,int Node,_str (&cmds)[])
{
   ...
   while (cmd_index>=0) {
      ...
      if (strieq(cmd_type,'run_tool')) {
         ...
         parse options with 'tool=','i' tool ' config=','i';
         parse options with 'config=','i' config ' tool=','i';
         tool=strip(tool);
         config=strip(config);
...
void _cbsave_project()
{
   ...
   if (_file_eq(_get_extension(p_buf_name,true),PRJ_FILE_EXT) && _workspace_filename!='') {
      ...
      for (i:=0;i<ProjectFiles._length();++i) {
         ...
         if (file_eq(vpj_project_name,p_buf_name)) {
            ...
            */
            ...
            call_list('_prjupdate_',p_buf_name);
            _clearProjectFileListCache(_workspace_filename,p_buf_name,true);
...
bool _ProjectGet_FilesPresent(int handle)
{
   XMLVARIATIONS xmlv;
   _ProjectGet_XMLVariations(handle,xmlv);

   result:=_xmlcfg_find_simple(handle,xmlv.vpjx_files"//"xmlv.vpjtag_f);
   return result>=0;
}
 ...
 * @return  true if version was set as the latest version
 */
bool CheckLatestJDK(_str version,int &major,int &minor,int &sub, int& subsub){
   cmajor := 0;
   cminor := 0;
   csub := 0;
   csubsub := 0;
...
_str _GetLatestCompiler(bool checkAll=false, bool generateConfigs=true)
{
   ...
   if ((checkAll)||(gLatestCompiler=='not found'||(gLatestCompiler=='')||(gLatestCompiler==LATEST_COMPILER_NOT_FOUND))) {
      _str version;
      major := 0;
      minor := 0;
      sub := 0;
...
_str _GetLatestJDK(bool checkAll=false, bool generateConfigs=true)
{
   ...
   if ((checkAll)||(gLatestJDK=='not found')||(gLatestJDK=='')||(gLatestJDK==LATEST_COMPILER_NOT_FOUND)) {
      gLatestJDK='';
...
{
   _file_types_get_names_and_wildcards(def_file_types,auto filter_names,auto filter_wildcards);
   for (i:=0;i<filter_names._length();++i) {
      PropertySheetItem psi;
      psi.Caption = strip(filter_names[i]);
      psi.Value = strip(filter_wildcards[i]);
...
_str _filter_form_import_summary(PropertySheetItem (&summary)[])
{
   ...
   foreach (psi in summary) {
      filter_names:+=psi.Caption;
      filter_wildcards:+=psi.Value;
   }

   // remove the last comma
   def_file_types = _file_types_from_names_and_wildcards(filter_names,filter_wildcards);
   _config_modify_flags(CFGMODIFY_DEFVAR);
...
void initialize_file_filters(_str filters, bool use_folders = false)
{
   ...
   _file_types_get_names_and_wildcards(filters,filter_names,filter_values);
...
void ctlFilterNew.lbutton_up()
{
   ...
   if (_param2=='') _param2=ALLFILES_RE;

   index := 0;
   filter_names._insertel(_param1,index);
   filter_values._insertel(_param2,index);
   ...
   ctlFilterList._TreeAddItem(TREE_ROOT_INDEX,name,TREE_ADD_AS_FIRST_CHILD,0,0,-1);
   ctlFilterList._TreeTop();
   ctlFilterList.enableDisableButtons();
   //update_filters();
}
...
_str compile_filter_string()
{
   ...
   result := _file_types_from_names_and_wildcards(filter_names,filter_values);
