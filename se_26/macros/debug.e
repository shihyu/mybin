 ...
 * <p>
 * The {@code attach_info} depends on the target debugger.
 * <ul>
...
void debug_message(_str header, int status=0, bool modal=true)
{
   ...
   if (status < 0) {
      ...
      } else if (status == DEBUG_GDB_MISSING_GDB_RC && machine() == 'MACOSX-M1') {
         // For the v26 release, GDB isn't buildable on M1, so not shipped as a 
         // 3rd party addition.  Add a reminder for the user, in case a version 
         // has come out in the meantime.
         msg :+= (modal)? "\n\n" : "<p>";
         msg :+= 'Go to "Tools -> Options -> Debugging -> Configurations" to ' :+
                 'add the location of your GDB executable.';
