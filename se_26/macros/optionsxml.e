...
bool _isLangAPIHelpExcluded(_str langId)
{
...
_command int config(_str nodeName = '', _str options = 'N', _str arguments = '', bool reOpen = false) name_info(',')
{
   ...
   if (!isOpen) {
      ...
      case 'S':
      case 'V':
         wid = show(openStr, OP_CONFIG, '', /*restoreLast*/true);
         break;
      case 'N':
 ...
 */
void _ctl_tree.rbutton_up,context(int x = -1, int y = -1)
{
   // find the menu
...
void _ctl_tree.on_create(int purpose = OP_CONFIG, _str nodeOrGroup = '', bool restoreLast = true, bool protect = false, _str arguments = '')
{
   ...
   _ctl_show_categories.resizeToolButtonToLabel(ctllabel1.p_window_id);
