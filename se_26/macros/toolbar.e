...
void _UpdateMacToolbars()
{
   foreach (auto i => auto tb in def_toolbartab) {
      if (tb.FormName == "_tbunified_form") {
         def_toolbartab._deleteel(i);
         _config_modify_flags(CFGMODIFY_DEFVAR);
         _tbNewVersion();
         break;
...
static void _tbAppendToolbarsToMenu(int menu_handle, bool list_debug_toolbars)
{
   ...
   for ( i = 0; i < def_toolbartab._length(); ++i ) {
      ...
      // skip the obsolete Mac unified toolbar
      if (ptb->FormName == "_tbunified_form") {
         continue;
      }
...
void _tbNewVersion()
{
   ...
   for( i=0;i<init_toolbartab._length();++i ) {
      ...
      if( 0==FormName_hashtab._indexin(init_toolbartab[i].FormName) ) {
         def_toolbartab :+= init_toolbartab[i];
      } else {
