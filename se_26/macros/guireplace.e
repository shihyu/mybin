...
int gui_replace2(_str search_string,_str replace_string,_str options, bool quiet = false, bool multifile = false, SearchResults* results = null, bool tempFile = false, bool close_mini_find_dialog=true)
{
   ...
   if (go) {
      ...
      if (!searchrc || (search_flags & VSSEARCHFLAG_WRAP)) {
         ...
         searchrc = _qreplace_all(search_string, replace_string, wrap_option'@':+options, add_Nofchanges, results);
         Nofchanges += add_Nofchanges;
         if (searchrc!=COMMAND_CANCELLED_RC && !searched_in_wrap_mark && _select_type(wrap_mark) != '' &&
      ...
      return(qreplace_done2(searchrc,mark,old_mark,wrap_mark,search_mark,searched_in_wrap_mark,Nofchanges,edit_wid,quiet));
   }
...
void _qrgo.lbutton_up()
{
   ...
   if (searchrc!=COMMAND_CANCELLED_RC && 0 == qreplace_wrap(orig_wid)) {
