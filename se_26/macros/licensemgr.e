...
static int elevatedWriteLicenseFile(bool &elevationSupport,_str srcLicenseFilename,_str dest_filename) {
   ...
   } else if (_isUnix() /*&& !_isMac()*/) {
      
      askpass_path := editor_name('P'):+"vs-ssh-askpass";
      old_sudo_askpass := get_env("SUDO_ASKPASS");
      set_env("SUDO_ASKPASS",askpass_path);   

      terminal_cmd := "sudo";
      terminal_cmd :+= " >/dev/null 2>&1";
      terminal_cmd :+= " --prompt '[sudo] Password for %p to Copy License File'";
      terminal_cmd :+=" -k --askpass";
      terminal_cmd :+= " -s eval '";
      terminal_cmd :+= "cp "_maybe_quote_filename(srcLicenseFilename)" "_maybe_quote_filename(dest_filename);
      terminal_cmd :+= ";";
      terminal_cmd :+= "chmod +r "_maybe_quote_filename(dest_filename);
      terminal_cmd :+= "'";

      //_message_box(terminal_cmd);
      status = dos("-s ":+terminal_cmd);
      //_message_box("status="status);
      set_env("SUDO_ASKPASS",old_sudo_askpass);
      if (!status) {
         elevationSupport=true;
      }
   }
...
static int lmw_error_shown()
{
   ...
   if(ginstall_license_file_probably_need_root_access || _isUnix()) {
      ...
      ctls3_error.p_text= "<br><b>File copy failed</b>. Start a terminal with root permissions (<b>su</b> or <b>sudo</b>) and execute the file copy command below:<br><br>":+
         ...
         "You may also need to set read permissions after copying the license file by executing the command below:<br><br>":+
         "chmod +r "_maybe_quote_filename(ginstall_license_file_dest)"<br><a href='chmod'>copy command to clipboard</a><br><br>":+
         more_msg:+
...
void ctls2_browse.lbutton_up()
{
   _str result=_OpenDialog('-modal',
                           ...
                           _file_types_append_filters(def_file_types,"License Files ("LICENSE_FILE_TYPE")"),       // File Type List
...
void ctls3_error.on_change(int reason,_str hrefText)
{
   if (reason==CHANGE_CLICKED_ON_HTML_LINK) {
      ...
      } else if (hrefText=='chmod') {
         text='chmod +r '_maybe_quote_filename(ginstall_license_file_dest);
