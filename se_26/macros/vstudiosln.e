...
static void InsertSolutionItemTree(int treeViewId, int treeNode, _str parentGuid, VS2005SolutionItems (&solutionItems)[])
{
   ...
   for (; ffix < foldersAtThisLevel._length(); ffix++) {
      ...
      if(isnumber(indexVal)) {
         ...
         if(solutionItemsNode > 0)
            _TreeDelete(solutionItemsNode);
         solutionItemsNode = _TreeAddItem(treeNode ,folderCaption,TREE_ADD_AS_FIRST_CHILD,_pic_tfldclos,_pic_tfldopen,0,0,folderGuid);
         // Create the new solution item virtual folder node
...
static void ReparentProjects2005(_str WorkspaceName)
{
   ...
   /* top level solution folders are supposed to be above.
      top level solution projects. No sort needed.
   */
   ...
   if(copiedProjNode <= 0) {
      ...
      /*
          Visual Studio sorts like this:
              ...
              ATextFile.txt
              ZTextFile.txt
          Before getting here we see this:
              AFolder
              ZFolder
              ATextFile.txt
              ZTextFile.txt
          We want to make sure we add this project BEFORE the text files.
      */
      child_index:=treeViewId._TreeGetFirstChildIndex(solutionFolderIndex);
      if (child_index<0) {
         // There are no other children in this folder
         // Add this project to bottom
         copiedProjNode = _TreeCopy2(treeViewId, projectIndex, solutionFolderIndex);
      } else {
         while (treeViewId._TreeGetNextSiblingIndex(child_index)>=0) {
            child_index=treeViewId._TreeGetNextSiblingIndex(child_index);
         }
         // We are setting on the last child.
         AddFlags:=TREE_ADD_AS_CHILD;
         for (;;) {
            if (child_index<0) {
               child_index=solutionFolderIndex;
               AddFlags=TREE_ADD_AS_FIRST_CHILD;
               break;
            }
            if (treeViewId._projecttbIsProjectNode(child_index) || treeViewId._projecttbIsFolderNode(child_index)) {
               AddFlags=TREE_ADD_AFTER;
               break;
            }
            child_index=treeViewId._TreeGetPrevSiblingIndex(child_index);
         }
         copiedProjNode = _TreeCopy2(treeViewId, projectIndex, child_index,AddFlags);
      }
      // delete the original
