...
static _str _projecttbTreeGetCurFolderPath(int index = -1,bool return_default_add_source_path=false)
{
   ...
   if (strieq(autoFolders, VPJ_AUTOFOLDERS_DIRECTORYVIEW)) {
      ...
      _str config;
      // Could do better here because _project_name might not be the project for this file.
      // This is a special case for Java anyway.
      ...
      if (_ProjectGet_Type(handle, config) :== 'java') {
         ...
         if (node >= 0) {
            ...
            if (child >= 0) {
               ...
               return _strip_filename(absfile, 'N');
            }
         }
      }
   ...
   */
   ...
   if (_IsWorkspaceAssociated(_workspace_filename)) {
      ...
      if (_IsMSBuildProj(_workspace_filename, AssociatedFileType)) {
         ...
         is_assoc_wildcard_project=_IsVisualStudioWildcardProject(csproj_handle);
   ...
   if (option=='f2') {
      // Nothing to do. File added to project if necessary.
      return;
   }
   if (option :== 'f3') {
      if (is_assoc_wildcard_project) {
...
int _OnUpdate_projecttbAddFiles(CMDUI &cmdui,int target_wid,_str command,bool doNewFile=false)
{
   ...
   if (target_wid && target_wid.p_object==OI_TREE_VIEW) {
      ...
      if (_IsWorkspaceAssociated(_workspace_filename)) {
         if (_IsVisualStudioWildcardProject(_ProjectGet_AssociatedHandle(handle))) {
            if (command:=='projecttbAddNewFile' || command:=='projecttbAddFiles') {
               return(MF_ENABLED);
            }
            return(MF_GRAYED);
         }
...
static void _projecttbAddFiles2(_str newfilename=null,_str ProjectName="",_str FolderName=null)
{
   ...
   _AddAndRemoveFilesFromVC(NewFilesList,null,ProjectName);
   _WorkspaceCache_Update();
   // Could have added a file matching a wildcard.
...
// _param5 - add as wildcard
   ...
   if (handle < 0) {
      _message_box(nls("Invalid project handle."));
      return;
   }
   FolderNode := _projecttbTreeFolderNode(handle);
   if (FolderNode < 0) {
      if (_IsWorkspaceAssociated(_workspace_filename)) {
         _message_box(nls("You can't add a file tree to an associated project."));
         return;
      } else {
         FolderNode=_ProjectGet_FilesNode(handle);
      }
...
// _param8 - create parent directory folder
   if (_param5) {
      if (_param8) {
...
void _projecttbAddDirectoryToProject(bool onlyUpdateProjectFile, 
                                     _str ProjectName, int handle, 
                                     int FolderNode, 
                                     _str basePath, 
                                     _str (&includeList)[], 
                                     _str (&excludesList)[], 
                                     ...
                                     bool warnDuplicateFiles=true)
{
   ...
   excludes := _project_xlat_special_excludes(join(excludesList, ';'));
   ...
   foreach (auto f in includeList) {
      ...
      if (warnDuplicateFiles && _ProjectGet_FileNode(handle, RelFilename) >= 0) {
...
_command void projecttbWildcardFolderProperties() name_info(',')
{
   ...
   if (!result) {
      ...
      _clearProjectFileListCache(_workspace_filename, project_name, true);
      _WorkspaceCache_Update();
      toolbarUpdateWorkspaceList();
   }
}
...
_command void projecttbRemove() name_info("," VSARG2_EXECUTE_FROM_MENU_ONLY)
{
   ...
   */
   ...
   probably_deleted_non_wildcard:=false;
   bool assoc_projects:[];
   for (ff:=1;;ff=0) {
      activate_window(treeWid); // just in case
      index=_TreeGetNextSelectedIndex(ff,info);
      ...
      ProjectName := _projecttbTreeGetCurProjectName(index);
      if (ProjectName!='' && assocWorkspace) {
         assoc_projects:[ProjectName]=true;
      }
      workspace_ext := _get_extension(_workspace_filename,true);
      if (_projecttbIsProjectFileNode(index) && !_ParentNodeSelected(index)) {
         if (_projecttbIsWorkspaceItemNode(index)) {
            ...
            probably_deleted_non_wildcard=true;
         } else {
            int handle=_ProjectHandle(ProjectName);
            if (_ProjectIs_vcxproj(handle)) {
               ...
               if (_CanWriteFileSection(ProjectName)) {
                  ...
                  if (assocWorkspace && _file_eq(workspace_ext,VISUAL_STUDIO_SOLUTION_EXT) && _IsVisualStudioWildcardProject(_ProjectGet_AssociatedHandle(handle))) {
   ...
   foreach (auto ProjectName => auto item in assoc_projects) {
      _ProjectCache_Update(ProjectName);
      _clearProjectFileListCache(_workspace_filename,ProjectName,true);
      call_list("_prjupdate_",ProjectName);
   }
   ...
   // Seem to need this for wildcard delete too (try deleting parent wildcard folder).
   toolbarUpdateWorkspaceList();
   /*if (probably_deleted_non_wildcard) {
      toolbarUpdateWorkspaceList();
   } */
}

// Performs a refresh on the project with the given name.
void tbrefresh_project_by_name(_str pname)
{
   if (pname!='') {
      int handle=_ProjectHandle(pname);
      _str AutoFolders=_ProjectGet_AutoFolders(handle);
      if (strieq(AutoFolders,VPJ_AUTOFOLDERS_PACKAGEVIEW) ||
          strieq(AutoFolders,VPJ_AUTOFOLDERS_DIRECTORYVIEW)
         ) {
...
_command void projecttbRefresh(_str force=0) name_info("," VSARG2_EXECUTE_FROM_MENU_ONLY)
{
   ...
   ProjectName := _projecttbTreeGetCurProjectName();
   tbrefresh_project_by_name(ProjectName);
...
//3:30pm 7/20/2000 Tested after move from project.e
void _proj_tooltab_tree.rbutton_up,context()
{
   if (_workspace_filename=='') {
      return;
   }
...
void _projecttbGetProjects(int parentIndex, int (&projects):[])
{
   ...
   while (index >= 0) {
      if (_projecttbIsProjectNode(index)) {
         ...
         projects:[_file_case(VSEProjectFilename(_AbsoluteToWorkspace(fullPath)))] = index;
      } else {
