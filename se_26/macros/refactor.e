...
/**
 * If {@code true}, the menu items for C++ refactoring will be removed. 
 *  
 * @default true
...
/**
 * If {@code true}, we have already tried to generate default compiler 
 * configurations for {@code compilers.xml} and failed to find any installed 
 * compilers.  Use this to avoid trying again and again. 
 */
static bool gTriedAutogenCompilersAndFailed=false;
...
definit()
{
   ...
   gTriedAutogenCompilersAndFailed=false;
}
                                    ...
                                    bool quiet=false, bool useThread=false)
{
   ...
   make_tag_args := "";
   if (config_type == "cpp") {
      wildcards:=_file_types_get_wildcards_by_name(def_file_types,"C/C++ Files");
      if (wildcards=='') {
...
void generate_default_configs(bool force=false)
{
   if (!_haveBuild()) {
      return;
   }

   if (!force && gTriedAutogenCompilersAndFailed) {
      return;
   }
...
//      say("config = " i " name = " config_names[i] " header = " header_names[i] " includes = " config_includes[i] );
   ...
   if (!gTriedAutogenCompilersAndFailed && (config_names._length() + java_config_names._length()) <= 0) {
      gTriedAutogenCompilersAndFailed = true;
   }
...
_str _get_vs_sys_includes(_str config_name)
{
   ...
   if (_isWindows()) {
      ...
      case COMPILER_NAME_VS2022:
         major_version := 15;
         target_version := "15.0";
         if (config_name == COMPILER_NAME_VS2022) {
            major_version = 17;
            target_version = "17.0";
         } else if (config_name == COMPILER_NAME_VS2019) {
            major_version = 16;
            target_version = "16.0";
         }
         ...
         if (SDKINSTALLDIR != "") {
            ...
            strappend(include_path, SDKINSTALLDIR:+"include":+FILESEP:+extradir:+"winrt":+FILESEP);
            strappend(include_path, ";");
            strappend(include_path, SDKINSTALLDIR:+"include":+FILESEP:+extradir:+"cppwinrt":+FILESEP);
         }
         break;
