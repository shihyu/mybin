...
static SCROLL_BAR_HEIGHT(int screen_height) {
   ...
   static AutoCodeInfoFlags gFunctionHelp_flags;
                       ...
                       _str streamMarkerMessage=null)
{
   ...
   status := _Embeddedfcthelp_get_start(errorArgs,
                                        ...
                                        flags, depth:1);
   if (_chdebug) {
      say("_do_function_help H"__LINE__": status="status);
   }
   ...
   if (!DisplayImmediate && (def_codehelp_idle || gFunctionHelp_MouseOver)) {
      ...
      gFunctionHelp_InsertedParam=false;
      if (_chdebug) {
         say("_do_function_help: HERE"__LINE__" operator="gFunctionHelp_OperatorTyped" infunction="ginFunctionHelp" pending="gFunctionHelp_pending" onname="cursorIsOnFunctionName" inlist="cursorInsideArgumentList);
      }
 ...
 *
 * @return {@code true} if the function help dialog is active,
...
{
   if (ginFunctionHelp) {
      really_had_function_help_active := false;
      if (_iswindow_valid(gFunctionHelp_form_wid)) {
         really_had_function_help_active = true;
         if (gFunctionHelp_InsertedParam && 
      ...
      if (alsoTerminateAutoComplete) {
         if (really_had_function_help_active || !gFunctionHelp_pending) {
 ...
 */
_str _CodeHelpCurWord(bool &allHelpWorkDone,int &FunctionHelp_FunctionNameOffset=0)
{
   ...
   FunctionHelp_FunctionNameOffset=gFunctionHelp_FunctionNameOffset;
...
_command void function_argument_help() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   _UpdateContextAndTokens(AlwaysUpdate:true, ForceUpdate:true);
...
int _inString(_str &delim,bool allow_raw_strings=true)
{
   ...
   delim = get_text();
   if (delim != '"' && !(delim == "'" && !_LanguageInheritsFrom('terraform') && !_LanguageInheritsFrom('json'))) {
      restore_pos(p);
      return 0;
   }
...
static int do_complete_add_history(_str lastid, 
                                    _str lang,
                                    _str longest_prefix,
                                    ...
                                    int depth=1)
{
   // check if we have an exact match in the matches, if not, we were just
   // completing a longest prefix match
   VS_TAG_BROWSE_INFO longest_cm;
   tag_browse_info_init(longest_cm);
   num_matches := tag_get_num_of_matches();
   for (match_id := 1; match_id <= num_matches; match_id++) {
      tag_get_detail2(VS_TAGDETAIL_match_name, match_id, auto match_name);
      if (match_name != longest_prefix) continue;
      tag_get_match_info(match_id, longest_cm);
   }
   if (longest_cm.member_name == "") {
      return BT_RECORD_NOT_FOUND_RC;
   }

   // have to reconstruct the expression info
   VS_TAG_IDEXP_INFO idexp_info;
   tag_idexp_info_init(idexp_info);
   status := _Embeddedget_expression_info(false, lang, idexp_info, visited, 1);
   if (status < 0) {
      return status;
   }

   // and the prefix expression return type
   _str errorArgs[];
   ...
   // now we can add the item to auto-complete history
   AUTO_COMPLETE_INFO word;
   word.priorityLevel = AUTO_COMPLETE_SYMBOL_PRIORITY;
   word.displayWord   = longest_cm.member_name;
   word.symbol        = longest_cm;
   word.caseSensitive = longest_prefix_case_is_same;
   AutoCompleteAddHistory(lastid, lang, idexp_info, prefixexp_rt, context_rt, word);
   return 0;
}
...
void _do_complete(bool isFirstAttempt=true)
{
   ...
   // try to re-use past completions with this same identifier prefix
   status := AutoCompleteFindHistorySymbols(lang, lastid, caseSensitive, visited, depth:1);
   if (status > 0) {
      num_matches = tag_get_num_of_matches();
   }
   ...
   // if there were no matches, look for pattern matching matches
   // and transposed character matches
   ...
   if (!timedOut && 
      ...
      // too many matches, then we are going to bring up list-symbols
      if (num_pattern_matches >= def_tag_max_find_context_tags) {
      ...
      if (!timedOut && num_pattern_matches < def_tag_max_find_context_tags) {
         ...
         // look for matches with transposed characters
         if (!(codehelpFlags & VSCODEHELPFLAG_COMPLETION_NO_SUBWORD_MATCHES)) {
            //say("_do_complete H"__LINE__": check for pattern");
         ...
         // look for matches with transposed characters
         if (longest_prefix == "" && !(codehelpFlags & VSCODEHELPFLAG_COMPLETION_NO_FUZZY_MATCHES)) {
            //say("_do_complete H"__LINE__": check for fuzzy");
   ...
   if (_chdebug) {
      say("_do_complete H"__LINE__": num_matches="num_matches);
      say("_do_complete H"__LINE__": longest_caption="longest_caption);
      say("_do_complete H"__LINE__": longest_prefix="longest_prefix);
   }
   ...
   if (num_matches <= 0) {
      if (_chdebug) {
         say("_do_complete H"__LINE__": NO SYMBOLS FOUND");
      }
      status = VSCODEHELPRC_NO_SYMBOLS_FOUND;
      if (num_matches < 0) status = num_matches;
   ...
   if (length(longest_prefix)+1 >= lastid_len && longest_prefix != lastid && longest_prefix_case_is_same) {
      do_complete_replace_word(lastid, lastid_len, longest_prefix, longest_case_prefix);
      do_complete_add_history(lastid, lang, longest_prefix, longest_prefix_case_is_same, visited);
   } else if (num_matches <= 1 || length(longest_prefix) == lastid_len) {
...
void _CodeHelp(bool AlwaysUpdate=false)
{
   ...
   // check if there was a quick symbol popup displayed and the cursor has moved
   orig_wid := p_window_id;
   focus_wid := _get_focus();
   if (focus_wid && _iswindow_valid(focus_wid)) {
      focus_wid._MaybeCloseCursorXYBBWid();
   }
   if (_iswindow_valid(orig_wid)) {
 ...
 *
 * @return {@code true} if the list members dialog is active,
 *         {@code false} otherwise
 *  
...
{
   return AutoCompleteActive(itemIsSelected);
}
                                       ...
                                       bool insert_keyword=false)
{
   ...
   if (i>=0 && i<n) {
      ...
      if (_chdebug) {
         say("maybe_insert_current_param: HERE"__LINE__" param_name="param_name);
      }
                                 ...
                                 VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   if (!_haveContextTagging()) {
      ...
      if (fast_get_index != 0 && get_index != 0) {
         ...
         new_time := (long)_time('b');
         gnew_total_time += (new_time - orig_time);
         isay(depth, "_Embeddedget_expression_info: (using "name_name(fast_get_index)")");
         isay(depth, "   p_line="p_line" p_col="p_col" offset="_QROffset()" status="status);
         get_line(auto line);
         isay(depth, "   LINE="line);
