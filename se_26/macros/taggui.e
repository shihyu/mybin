...
void ctl_tag_tree_view.rbutton_up,context()
{
   // Get handle to menu:
...
int tagProgressCallback(int percentage, bool breakIfEditorVisible = false, typeless userData = "")
{
   ...
   if(wid) {
      ...
      def_actapp=0;
      _use_timers=0;
      process_events(cancelTagProgressFormGlobal, 'E'/*, 250*/);
      _use_timers=orig_use_timers;
 ...
 *  
 * This command is invoked by the {@code vsmktags} utility. 
 * <pre> 
   ...
   for (i = 0; i < workspaceList._length(); i++) {
      ...
      if (useThread) {
         ...
         while (tag_get_num_async_tagging_jobs('U') > 0 || numAsyncBuildsRunning > 0) {
            ...
            status = _ReportAsyncTaggingResults(true, wid, timeout:250);
            if (status < 0) {
               wasCancelled = true;
               break;
            }
...
bool cancel_form_cancelled(int checkFrequency=250)
{
   ...
   if (!gbuild_cancel) {
      process_events(gbuild_cancel, ''/*, timeout*/);
   }

   // process events and look for escape
   if (!gbuild_cancel) {
      process_events(gbuild_cancel, 'E'/*, 250*/);
   }
