...
_metadata enum_flags OpenTBFileOrigins {
   ...
   OTBFO_ISDOTDOTDIR          = 0x80,
   OTBFO_MODIFIED             = 0x100,
   OTBFO_READONLY_ON_DISK     = 0x200,
   OTBFO_WRITABLE_ON_DISK     = 0x400,
};
...
static void refreshFileTree(_str calledFrom=''){
   ...
   */
   ...
   // include readable/writable if they are set
   viewsToShow |= (def_opentb_view_flags & OTBFO_READONLY_ON_DISK);
   viewsToShow |= (def_opentb_view_flags & OTBFO_WRITABLE_ON_DISK);
...
void _file_tree.on_create()
{
   ...
   _expand_dir_panel_button.resizeToolButtonToLabel(_opencd.p_window_id);
...
void _file_tree.rbutton_up,context()
{
   showRightClickMenu();
}
...
static void showRightClickMenu()
{
   ...
   // check/uncheck read-only/writable
   if (_open_tb_visible_views() & OTBFO_READONLY_ON_DISK) {
      _menu_set_state(menuHandle, "tbopen-menu-command showreadonly", MF_CHECKED, 'M');
   }
   if (_open_tb_visible_views() & OTBFO_WRITABLE_ON_DISK) {
      _menu_set_state(menuHandle, "tbopen-menu-command showwritable", MF_CHECKED, 'M');
   }

   // check/uncheck view options
...
static void addFilesToProject(int treeWid)
{
   ...
   call_list('_prjupdate_',_project_name);
...
_command void tbopen_menu_command(_str cmdline="") name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY)
{
   ...
   case 'SHOWREADONLY':
      // change this option to the opposite of whatever it is now
      toggleOpenTBView(OTBFO_READONLY_ON_DISK);
      break;
   case 'SHOWWRITABLE':
      // change this option to the opposite of whatever it is now
      toggleOpenTBView(OTBFO_WRITABLE_ON_DISK);
      break;
