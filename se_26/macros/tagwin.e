...
void _tbtagwin_form.on_change(int reason)
{
   if( reason==CHANGE_AUTO_SHOW ) {
      ...
      _UpdateOneTagWindow(form_wid, 
                          gPreviewFormList:[form_wid], 
                          elapsed: 0,
                          form_wid._MDIGetActiveMDIChild(),
                          AlwaysUpdate:true);
   }
}
...
static _str mycur_word(VS_TAG_IDEXP_INFO &idexp_info)
{
   ...
   if(get_index != 0) {
      ...
      info_flags := VSAUTOCODEINFO_NULL;
      typeless junk;
                                ...
                                struct VS_TAG_BROWSE_INFO (&cmlist)[]=null)
{
   ...
   // make sure the form that was passed in is valid and active (do not update hidden tag windows)
   ...
   */
   ...
   // DJB 10-6-2021
   // If list-symbols is active and has an item selected, do not let
   // anything come along and change what is displayed.
   if (cm == null && cmlist == null && AutoCompleteActive(itemIsSelected:true) && formInfo.m_tagwin_saved_info != null) {
      return;
   }
