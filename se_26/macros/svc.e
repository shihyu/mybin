...
#import "se/vc/GitVersionedFile.e"
#import "se/vc/Hg.e"
#import "se/vc/HgVersionedFile.e"
#import "se/vc/Perforce.e"
#import "se/vc/PerforceVersionedFile.e"
#import "se/vc/SVN.e"
#import "se/vc/SVNVersionedFile.e"
#import "se/vc/CVSClass.e"
#import "se/vc/SVN.e"
...
using se.vc.VersionControlUtil;
using se.vc.CVS;
using se.vc.Git;
using se.vc.Hg;
...
_command void svc_gui_mfupdate(_str path='',_str options="") name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   // All that should be left in path is the actual path, we don't
   // want to quote that
   path = strip(path,'B','"');
                          ...
                          bool searchUserInfoForVersion=false) name_info(FILE_ARG','VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   if ( isProjectToolWindow() ) {
      ...
      // Is this an annotated buffer?  If so, we want to bring up the history for
      // the file that was annotated
      annotationCommandName := pInterface->getCaptionForCommand(SVC_COMMAND_ANNOTATE);
      if ( annotationCommandName != "" && p_DocumentName!="" && substr(p_DocumentName,1,length(annotationCommandName:+' ')) == annotationCommandName' ' ) {
         filename = substr(p_DocumentName,length(annotationCommandName)+length(ANNOTATE_INFO_FOR_LITERAL)+1);
      } else {
         filename=p_buf_name;
      }
   }
   ...
   if ( _last_char(filename)==FILESEP ) {
      _message_box(nls("Cannot get history for directory %s",filename));
      return;
   }
   dsay('svc_history '__LINE__' option='option' branchOption='branchOption);
   ...
   do {
      ...
      dsay('svc_history '__LINE__' curLocalRevision='curLocalRevision);
      if ( curLocalRevision=="" ) {
         dsay('svc_history '__LINE__' calling getCurLocalRevision');
         status = pInterface->getCurLocalRevision(filename,curLocalRevision);
         dsay('svc_history '__LINE__' status='status' filename='filename' curLocalRevision='curLocalRevision);
         if ( status ) break;
      }
      ...
      dsay('svc_history '__LINE__'  branchName='branchName);
      if ( branchName=="" ) {
         status = pInterface->getLocalFileBranch(filename,branchName);
         dsay('svc_history '__LINE__'  getLocalFileBranch status='status);
         if ( status ) break;
         dialogInfo.branchName = branchName;
         dsay('svc_history '__LINE__'  dialogInfo.branchName='dialogInfo.branchName);
      }
      dsay('svc_history '__LINE__' isURL='isURL);
      if ( !isURL ) {
         dsay('svc_history '__LINE__' calling getFileStatus');
         status = pInterface->getFileStatus(filename,fileStatus);
         dsay('svc_history '__LINE__' getFileStatus status='status);
      }
      if ( status ) break;
   } while (false);
   mou_hour_glass(false);
   dialogInfo.fileStatus = fileStatus;
   dsay('svc_history '__LINE__'  fileStatus='fileStatus' historyInfo._length()='historyInfo._length());
...
_command void svc_diff_current_symbol_with_tip() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   // Symbol names are too long and don't help much
...
const ANNOTATE_INFO_FOR_LITERAL = " info for ";

_command void svc_get_annotated_buffer(_str filename='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return;
   ...
   annotationCommandName := pInterface->getCaptionForCommand(SVC_COMMAND_ANNOTATE);
   annotationPrefix := annotationCommandName:+ANNOTATE_INFO_FOR_LITERAL;
   ...
   if ( _isEditorCtl() && annotationCommandName != "" && substr(p_DocumentName,1,length(annotationPrefix)) == annotationPrefix ) {
      return;
   }
   if ( !(pInterface->commandsAvailable()&SVC_COMMAND_AVAILABLE_GET_ANNOTATIONS) ) {
      _message_box(get_message(VSRC_SVC_COMMAND_NOT_AVAILABLE,"blame"));
      return;
   }
   STRARRAY fileList;
   cap := lowcase(pInterface->getCaptionForCommand(SVC_COMMAND_ANNOTATE,false,false));
   if ( isProjectToolWindow() ) {
      getListFromProjectTree(fileList);
   } else if ( _no_child_windows() && filename=='' ) {
      _str result=_OpenDialog('-modal',
   ...
   status := pInterface->getAnnotatedBuffer(filename,"","",auto annotatedBufWID);

   int child_wid=_MDICurrentChild(0);
   if ( !status && child_wid && !_no_child_windows()) {
      origLineNum := 0;
      if ( !isProjectToolWindow() ) {
         origLineNum = p_line;
      }
      ...
      _delete_temp_view(annotatedBufWID);

      p_DocumentName = nls("%s%s%s",annotationCommandName,ANNOTATE_INFO_FOR_LITERAL,filename);
      lang := _Filename2LangId(filename);
      _SetEditorLanguage(lang);
      p_modify=false;
      p_ReadOnly = true;

      p_adjust_color_coding_start=true; 
      top();up();
      while ( !down() ) {
         _begin_line();
         long startOffset=_QROffset();

         // Have to search for first ) in line because the length of the 
         // annotation can change based on the longest name, which can be
         // "Not Committed Yet", which is longer than any of our names
         status = search(')','@');
         if (!status) {
            long endOffset=_QROffset();
            markerLength := endOffset-startOffset+1;
            _StreamMarkerAdd(0,startOffset,markerLength+1,
                             true,  // Better not to use fake seek positions when possible.
                             0,_InteractiveOutputMarkerType(),null);
         }
      }

      if ( !isProjectToolWindow() ) {
         // Try to put on same line number
         p_line = origLineNum;
         center_line();
      }
   }
...
void SVCMenuSetup(_str cur_bufname,int vc_menu_handle,int &count,_str system_name,bool is_popup_menu)
{
   if ( !_haveVersionControl() ) {
      return;
   }
   ...
   if ( pInterface->commandsAvailable()&SVC_COMMAND_AVAILABLE_DIFF ) {
      ...
      _menu_insert(vc_menu_handle,-1,MF_ENABLED,menuCap,"svc_diff_current_symbol_with_tip","","help version control");
   }
   if ( pInterface->commandsAvailable()&SVC_COMMAND_AVAILABLE_HISTORY ) {
   ...
   if ( pInterface->commandsAvailable()&SVC_COMMAND_AVAILABLE_GET_ANNOTATIONS ) {
      curCap := pInterface->getCaptionForCommand(SVC_COMMAND_ANNOTATE);
      menuCap := "Get "pInterface->getCaptionForCommand(SVC_COMMAND_ANNOTATE)" info for "just_name'...';
      if ( cur_bufname=="" ) {
         menuCap = "Get "pInterface->getCaptionForCommand(SVC_COMMAND_ANNOTATE)' for from 'vcSystemCaption'...';
      }
      menuPos := _menu_insert(vc_menu_handle,-1,MF_ENABLED,menuCap,"svc_get_annotated_buffer\t"_maybe_quote_filename(cur_bufname),'','help version control');
      _mdi.p_child._menu_set_binding(vc_menu_handle, menuPos);
      ++count;
   }
   if ( pInterface->commandsAvailable()&SVC_COMMAND_AVAILABLE_HISTORY_DIFF ) {
   ...
   if ( !is_popup_menu) {
      shelveMenuHandle := _menu_insert(vc_menu_handle,-1,MF_ENABLED|MF_SUBMENU,"Shelves","","","help version control");
      ++count;
      if ( shelveMenuHandle ) {
         _menu_insert(shelveMenuHandle, -1, MF_ENABLED, "List Shelves...",'svc-list-shelves','','help version control');
         ++count;
         _menu_insert(shelveMenuHandle,-1, MF_ENABLED, "Open Shelf...",'svc-open-shelf','','help version control');
         ++count;
      }
   }
