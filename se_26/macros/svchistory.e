...
static void fillInTreeRecursive(SVCHistoryInfo *pNode,SVCHistoryInfo(&historyInfo)[],SVCHistoryFileInfo &dialogInfo,
                                int treeviewIndex,INTHASHTAB& captionTable=null,_str lastURL="",bool &hadRevisionCaption=false)
{
   ...
   for ( parentIndex:=true;;parentIndex=false ) {
      ...
      if (pVCI->getSystemSpecificFlags() & SVC_HISTORY_DIALOG_ALLOWS_COLUMNS) {
         ...
         minimumVersionNumberLength := pVCI->getMinimumVersionNumberLength();
         if ( minimumVersionNumberLength > 0 ) {
            caption :+= substr(pNode->revision,1,minimumVersionNumberLength);
         } else {
            caption :+= pNode->revision;
         }
         if ( pNode->revisionCaption!="" ) {
            hadRevisionCaption = true;
            caption :+= "\t":+stranslate(pNode->revisionCaption,' ',"\t");
         }
...
static void fillInDialog(SVCHistoryInfo (&historyInfo)[],SVCHistoryFileInfo &dialogInfo,bool searchUserInfoForVersion, bool &hadRevisionCaption=false)
{
   ...
   if ( pRoot->firstChildIndex>=0 ) {
      ...
      if ( pFirstChild!=null ) {
         ...
         ctltree1.fillInTreeRecursive(pFirstChild,historyInfo,dialogInfo,TREE_ROOT_INDEX,null,hadRevisionCaption:hadRevisionCaption);
...
{                                                     
   ...
   hadRevisionCaption := false;
   if ( historyInfo!=null ) {
      fillInDialog(historyInfo,dialogInfo,searchUserInfoForVersion,hadRevisionCaption);
   }
   origWID := p_window_id;
   p_window_id = ctltree1;
   if (pVCI->getSystemSpecificFlags() & SVC_HISTORY_DIALOG_ALLOWS_COLUMNS) {
      _TreeSetColButtonInfo(0, 2000, TREE_BUTTON_SORT_DATE | TREE_BUTTON_SORT_TIME | TREE_BUTTON_PUSHBUTTON, -1, 'Date');
      _TreeSetColButtonInfo(1, 2000, TREE_BUTTON_SORT, -1, 'Author');
      _TreeSetColButtonInfo(2, -1, TREE_BUTTON_SORT_NONE/*|TREE_BUTTON_AL_RIGHT*/, -1, 'Version');
      _TreeSizeColumnToContents(2);
      if ( hadRevisionCaption ) {
         _TreeSetColButtonInfo(3, -1, TREE_BUTTON_SORT_NONE, -1, 'Comment');
      }
      _TreeSortCol(0);
...
void _svc_history_form.on_resize()
{
   ...
   ctltree1._TreeSizeLastColumnToTreeWidth();
}
...
static void diffLocalFileWithPastVersion()
{
   ...
   for (i:=0;i<len;++i) {
      curLine := info.lineArray[i];
      parse curLine with "<B>Revision:</B>&nbsp;" selectedVersionNumber "<br>";
      if ( selectedVersionNumber!="" ) break;
   }
   ...
   if ( selectedVersionNumber == "" ) {
      versionTable := _GetDialogInfoHt("versionTable");
      selectedVersionNumber = versionTable:[currentVersionCaption];
   }
   ...
   if ( substr(localFilename,1,7)=="http://" ) {
     _message_box(nls("For URLs, you must diff past versions (use Diff drop down menu)"));
      ...
      pVCI->diffLocalFile(localFilename,selectedVersionNumber);
   }
   origFormID.ctltree1._set_focus();
}
...
static int svcHistoryDiffPast(_str filename, bool noNewerOrOlder=false)
{
   ...
   wid := show('_svc_get_past_version_form',"Choose other version",'"'ver1'"',"",versionTable);
   ...
   if ( fileURL1!="" && fileURL2!="" ) {
      ...
      svcDiffPastVersions(localFilename,ver1,ver2,noNewerOrOlder);
   }
   chdir(origDir,1);
   tree1._set_focus();
   return(0);
...
//   if ( status ) return;
   ...
   status := pVCI->getRemoteFilename(localFilename,auto remoteFilename);
   if ( status ) return;
   svcHistoryDiffPast(remoteFilename, true);
}
...
static int svcDiffPastVersions(_str remoteFilename,_str ver1,_str ver2, bool noNewerOrOlder=false)
{
   IVersionControl *pVCI = svcGetInterface(svc_get_vc_system());
   if ( pVCI==null ) return 1;
   ...
   if ( lowcase(pVCI->getSystemNameCaption())=="git" ) {
      ver1 = substr(ver1,1,GIT_SHORT_HASH_LENGTH);
      ver2 = substr(ver2,1,GIT_SHORT_HASH_LENGTH);
   }
   ver1Str := " (Newer - ":+ver1:+')';
   ver2Str := " (Older - ":+ver2:+')';
   if ( noNewerOrOlder ) {
   ...
   status = diff('-modal -r1 -r2 -bi1 -bi2 -file1title "'remoteFilename' 'ver1Str'" -file2title "'remoteFilename' 'ver2Str'"' fileWID1.p_buf_id' 'fileWID2.p_buf_id);
