...
/**
 * This setting controls whether Project &gt; Workspace Open... should prompt 
 * with a list of kinds of workspaces to open, or just go directly to the 
 * file browser to locate an existing SlickEdit workspace to open. 
 *
 * @default true
 * @categories Configuration_Variables
 */
bool def_workspace_smart_open = true;
...
static _str gNextProjectCacheUpdateTime;
_str gNextProjectCacheUpdateCount;
static int _cache_idle = 0; // only runs once during an idle period
static const PROJECT_CACHE_INITIAL_MIN_IDLE=2000;
int gProjectCacheUpdateMinIdle = PROJECT_CACHE_INITIAL_MIN_IDLE;

definit()
{
   s_defaultProjectHandle = -1;
   gNextProjectCacheUpdateCount=0;
   gNextProjectCacheUpdateTime=0;_cache_idle=0;
   gProjectCacheUpdateMinIdle= PROJECT_CACHE_INITIAL_MIN_IDLE;
}
...
void setEnvironmentFromProjectTargetNode(int projectHandle, int targetNode, bool set_in_build = false)
{
...
static int _rust_create_cargo_project(_str &WorkspaceFilename) {
    ...
    // Note: don't retag workspace files here, to avoid double calls 
    // to _MaybeRetagWorkspace().  This is already instigated in a 
    // init macro.
...
_str _get_editor_launcher() {
   vs_program:=editor_name("E");
   if (endsWith(vs_program,"_exe") && file_exists(substr(vs_program,1,length(vs_program)-4))) {
      vs_program=substr(vs_program,1,length(vs_program)-4);
   }
   return vs_program;
}
/**
...
{
   //say("workspace_open H"__LINE__": IN, workspace="WorkspaceFilename);
   if (def_workspace_smart_open) {
      if (WorkspaceFilename=="" && restoring_from_invocation=="") {
         response := show("-modal -mdi -new _quick_start_setup_project_form", "open");
         if (response == COMMAND_CANCELLED_RC || response == "") {
            return COMMAND_CANCELLED_RC;
         }
         if (_param1 == 1) {
            def_workspace_smart_open = false;
            _config_modify_flags(CFGMODIFY_DEFVAR);
         }
         if (response != "" && response != "ctl_workspace_open") {
            parse response with "ctl_" response;
            execute(response);
            return rc;
         }
      }
   ...
   //say("workspace_open H"__LINE__": BEFORE auto-update");
...
_command int workspace_open_visualcpp(_str workspaceFilename = "")   name_info(FILE_ARG'*,')
{
   ...
   return workspace_open_other(workspaceFilename, "Open Visual C++ Workspace",
                               "Visual C++ Workspace Files (*"VISUAL_STUDIO_SOLUTION_EXT";*"VCPP_PROJECT_WORKSPACE_EXT"),All Files ("ALLFILES_RE")");
}
                              ...
                              bool RetagWorkspaceFiles=true) name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   ...
   for (;;) {
      ...
      projects:[filename]=true;
      // validate the project file
      int handle=_ProjectHandle(filename,status);
      if (status) {
   ...
   foreach (auto ProjectName => auto item in projects) {
      call_list('_prjupdate_',ProjectName);
   }
   toolbarUpdateWorkspaceList();
   return(0);
}
...
_command int workspace_remove(_str Filename='',_str NewActiveProject='',bool doUpdateToolbarWorkspaceList=true) name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   ...
   _TagCallList(TAGFILE_REFRESH_CALLBACK_PREFIX);
   call_list('_prjupdate_',Filename);
...
_command int workspace_close() name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   ...
   // Write WildcardCache section
   _ProjectWriteCacheState(_workspace_filename,VSEWorkspaceStateFilename());
...
int _srg_workspace(_str option='',_str info='')
{
   ...
   if (option=='N' || option=='R') {
      ...
      ignore_restoreworkspace:=(def_save_config_flags & SAVE_CONFIG_IMMEDIATELY_SHARE_CONFIG) && 
                         ...
                         )
{
   ...
   if (add_to_workspace) {
      ...
      if (addExistingProject) {
         ...
         status=workspace_new_project2(newProjectName,
                                       ...
                                       null,
                                       createdDirectory,
                                       RemoveWorkspaceFileIfCancelled:false, 
                                       ShowProperties || runInitMacros);
      }
   ...
   status=workspace_new_project2(ProjectFilename,
                                 ...
                                 RetagWorkspaceFiles:false,
                                 null,
                                 createdDirectory,
                                 RemoveWorkspaceFileIfCancelled:true,
                                 runInitMacros
                                 );
   toolbarUpdateWorkspaceList();
   return(status);
                          ...
                          bool alreadyUpdatedWorkspaceList=false)
{
      ...
      for (i=0;i<ProjectFiles._length();++i) {
         ...
         if (VSEDate!=_file_date(DisplayName,'B')) {
            ...
            if (_IsWorkspaceAssociated(_workspace_filename)) {
               ...
               status = _GetAssociatedProjectInfo(vpj_project_name,vendorProjectName,vendorProjectType);
               if (!status) {
                  _ProjectCache_Update(vendorProjectName);
               }
            }

            _clearProjectFileListCache(_workspace_filename,vpj_project_name,true);
   ...
   if (!alreadyUpdatedWorkspaceList) {
      ...
      for (i:=0;i<ProjectFiles._length();++i) {
         _str vpj_project_name=_AbsoluteToWorkspace(ProjectFiles[i]);
         call_list('_prjupdate_',vpj_project_name);
      }
      toolbarUpdateWorkspaceList();
   }
   /*
...
int _WorkspaceOpenAndUpdate(_str WorkspaceFilename)
{
   ...
   // Never re-read the WildcardCache. Caller probably
   // just cleared the wildcard cache for this workspace.
   if (!file_eq(old_workspace_filename,_workspace_filename)) {
      _ProjectReadCacheState(_workspace_filename,VSEWorkspaceStateFilename());
   }
   return(0);
}
...
_str _macros_path() {
   if (def_use_workspace_slickc_macros && _workspace_filename != "") {
...
/**
 * Macro used for alias expansion to prefix a path with a leading
 * quote if the path contains a space.  For example, this could be 
 * used in an alias as follows. 
 * <pre>
 *    %\M_maybe_prefix_quote_func _macros_path%
 * </pre>
 */
_str _maybe_prefix_quote_func(_str macro_func_returning_string)
{
   if (macro_func_returning_string == "") {
      return "";
   }
   parse macro_func_returning_string with macro_func_returning_string auto args;
   index := find_index(macro_func_returning_string);
   if (index <= 0) {
      return "";
   }
   value := call_index(args, index);
   if (_maybe_quote_filename(value) == value) {
      return value;
   }
   return '"' :+ value;
}
...
/**
 * Macro used for alias expansion to prefix the value of an environment 
 * variable with a leading quote if the path contains a space.
 * For example, this could be used in an alias as follows. 
 * <pre>
 *    %\M_maybe_prefix_quote_env SLICKEDITCONFIG%
 * </pre>
 */
_str _maybe_prefix_quote_env(_str EnvVarName)
{
   if (EnvVarName == "") {
      return "";
   }
   value := get_env(EnvVarName);
   if (_maybe_quote_filename(value) == value) {
      return value;
   }
   return '"' :+ value;
...
void toolbarMaybeRefreshProjects(int form_wid, _str (&projects)[])
{
   ...
   foreach(auto projname in projects) {
      if (!projIndex._indexin(_file_case(projname))) continue;

      index := projIndex:[_file_case(projname)];
      state := 0;
      _TreeGetInfo(index, state);
...
void _UpdateWorkspaceCache(bool forceUpdate = false)
{
   ...
   /*
      Exclude insignificant (non-dragging mouse moves).
   */
   elapsed := _idle_time_elapsed(true);
   /*
      Idle time starts out at PROJECT_CACHE_INITIAL_MIN_IDLE (2) seconds. When
      _ClearWorkspaceFileListCache or scClearProjectFileListCache
      is called, the idle time is set to 1 second.
      Idle time is set back to PROJECT_CACHE_INITIAL_MIN_IDLE (2) seconds.
   */
   // Update every 1 or more seconds
   if (forceUpdate || (elapsed > gProjectCacheUpdateMinIdle)) {
      if (!forceUpdate) {
         if (gNextProjectCacheUpdateTime) {
            //say('gNext='gNextProjectCacheUpdateTime' '_time('F'));
            if (gNextProjectCacheUpdateTime && gNextProjectCacheUpdateTime > _time('F')) {
               return;
            }
         }
      } else {
         gNextProjectCacheUpdateCount=1;
      }
      ...
      if (status <= 0) {
         ...
         //say('_MaybeRetagWorkspace idle='gProjectCacheUpdateMinIdle);
         _MaybeRetagWorkspace(false, false, false, true);
      ...
      switch (gNextProjectCacheUpdateCount) {
      case 0:
         d = d.add((long)1, DT_SECOND);
         break;
      case 1:
      case 2:
         d = d.add((long)2, DT_SECOND);
         break;
      case 3:
      case 4:
         d = d.add((long)3, DT_SECOND);
         break;
      default:
         d = d.add((long)5, DT_SECOND);
      }
      ++gNextProjectCacheUpdateCount;
      if (gNextProjectCacheUpdateCount>20) {
         gNextProjectCacheUpdateCount=20;
      }
      gNextProjectCacheUpdateTime = d.toTimeF();
      _cache_idle = 1; 
   } else {
      if (_cache_idle) {
         _cache_idle = 0;gNextProjectCacheUpdateTime=0;gNextProjectCacheUpdateCount=0;
      }
   }
}
...
/**
 * Open one of the sample workspaces provided with SlickEdit.
 * 
 * @param sampleName   name of sample workspace to open
 *  
 * @see workspace_open 
 *
 * @categories Project_Functions
 */
_command int workspace_open_sample(_str sampleName="") name_info(FILE_ARG'*,')
{
   // load the sample projects if we do not already have them
   if (def_workspace_info._length() == 0) {
      maybeAddSampleProjectsToTree(def_workspace_info);
   }

   _str sampleWorkspaceFiles[];
   _str sampleWorkspaces[];
   foreach (auto ws_info in def_workspace_info) {
      if (ws_info.isFolder && ws_info.caption == WORKSPACE_FOLDER_SAMPLES) {
         foreach (auto sample_info in ws_info.u.list) {
            if (sample_info.isFolder) continue;
            ws_name := sample_info.caption;
            if (ws_name == null || ws_name == "") {
               ws_name = sample_info.u.description;
            }
            ws_file := _replace_envvars(sample_info.filename);
            if (ws_name == null || ws_name == "") {
               ws_name = _strip_filename(sample_info.filename, 'p');
            }
            sampleWorkspaces :+= ws_name :+ "\t" :+ ws_file;
            sampleWorkspaceFiles :+= ws_file;
            if (sampleName != "") {
               if (sampleName == ws_name || file_eq(_strip_filename(sampleName, 'pe'), _strip_filename(sample_info.filename, 'pe'))) {
                  return workspace_open(ws_file);
               }
            }
         }
         break;
   ...
   ws_file := select_tree(sampleWorkspaces, sampleWorkspaceFiles, caption: "Select Sample Workspace to Open", SL_COLWIDTH, "Name,Path", help_item: "tutorials");
   if (ws_file == "" || ws_file == COMMAND_CANCELLED_RC) {
      return COMMAND_CANCELLED_RC;
   }
   return workspace_open(ws_file);
}
