 ...
 * @return int
 */
int getPropertyGroupBackground(int nodeIndex, int col=1)
{
   // get the base color of the first row
   typeless bg = 0;
   _TreeGetColor(nodeIndex, col, auto fg, bg, auto ff);
   ...
   for (i := 0; i < psPtr -> getNumItems(); i++) {
      ...
      if (type == TMT_GROUP) {
         ...
         if (pg != null) {
            ...
            if (pgColor == 0) pgColor = _ctl_property_sheet.getPropertyGroupBackground(rowID);
...
static bool getColor(Property * p, _str &value)
{
   ...
   if (new_color == '' || new_color == color) {
      ...
      value = "0x":+_dec2hex(new_color,16,6);
   }
