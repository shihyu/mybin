...
int def_diff_force_wrap_len      =  10000;
...
bool _isdiff_update_form()
{
   return p_active_form.p_name=="_svc_mfupdate_form";
}
...
bool _isdiff_editor_window(int buf_id=0)
{
   if ((p_name=="_ctlfile1" || p_name=="_ctlfile2") &&
      ...
      if (!buf_id || buf_id==p_buf_id) {
         ...
         // Special case, don't use _DiffGetMiscInfo here. Because there are too
         // many window IDs it doesn't abstract much
         DIFF_SHOWING_INFO misc=p_parent._GetDialogInfoHt(DIFFEDIT_CONST_MISC_INFO:+_ctlfile1.p_buf_name,p_parent._ctlfile1);
         if (misc != null) {
            if (!buf_id) buf_id=p_buf_id;
            if (misc.buffer1.WholeFileBufId==buf_id ||
                misc.buffer2.WholeFileBufId==buf_id) {
               return(true);
            }
         }
      }
...
bool _isdiffed(int buf_id)
{
   ...
   if (buf_id > 0) {
      ...
      if (!status) {
         isdiffed := _GetBufferInfoHt(DIFFEDIT_CONST_BUFFER_IS_DIFFED:+buf_id);
         p_window_id = origWID;
...
bool _DiffFileBufferOrFileInViewExists(_str filename,bool doAbsolute=true,
                                     ...
                                     int &flags)
{
   ...
   if (EOL1!=EOL2) {
      ...
      if ( _DiffDialogIsSVCUpdate() ) {
         _ctlfile1.p_enabled = _ctlfile2.p_enabled = false;
         status := _message_box(nls("EOL Characters do not match.\n\nCompare EOL chars?"),"",MB_YESNO);
         _ctlfile1.p_enabled = _ctlfile2.p_enabled = true;
      } else {
...
void InitMiscDiffInfo(DIFF_SHOWING_INFO &misc,_str Type)
{
    misc.DiffParentWID = -1;

    misc.buffer1.BufStartTime = "";
    misc.buffer2.BufStartTime = "";
    misc.IntraLineIsOff = 0;
    misc.DontDeleteMergeOutput = false;
    misc.Bookmarks = null;
    misc.PreserveInfo = "";
    misc.AutoClose = false;
    misc.buffer1.WholeFileBufId = -1;
    misc.buffer1.origReadOnly = false;
    misc.buffer1.MarkId = -1;
    misc.buffer2.WholeFileBufId = -1;
    misc.buffer2.origReadOnly = false;
    misc.buffer2.MarkId = -1;
    misc.buffer1.origEncoding = -1;
    misc.buffer2.origEncoding = -1;
    misc.RefreshTagsOnClose = false;
    misc.buffer1.TagParentIndex = -1;
    misc.buffer2.TagParentIndex = -1;
    misc.buffer1.SymbolViewId = 0;
    misc.buffer2.SymbolViewId = 0;
    misc.buffer1.fileDate = "";
    misc.buffer1.fileDate = "";
    misc.Comment = "";
    misc.OkPtr = null;
    misc.ShowNoEditorOptions;
    misc.buffer1.softWrap = false;
    misc.buffer2.softWrap = false;
    misc.closeBuffer2 = false;
    misc.buffer1.doNotCopyBuffer = false;
    misc.buffer2.doNotCopyBuffer = false;
...
static void markRegion(int ViewId, int FirstLine,int LastLine,int &markid,
                       ...
                       bool OrigReadOnly,int dialogInfoWID,_str forceEOL="")
{
   ...
   // If the user has not changed the VSOPTION_FORCE_WRAP_LINE_LEN, set it to 
   // 10k for the temp diff buffer.  Otherwise, they have modified it, so we'll
   // assume they set it where they want it
   if ( _default_option(VSOPTION_FORCE_WRAP_LINE_LEN) <= 4000 ) {
      p_ForceWrapLineLen = def_diff_force_wrap_len;
   }
...
/**
 * If the current buffer has wrapped lines, and it is in memory,
 * save it, open it, temporarily change the wrap limit, open it,
 * set the property when things have not already wrapped
 * 
 * @param inmem  true if file was already in memory
 * 
 * @return int 
 */
static int getUnwrappedBuffer(int wrapLen,_str &tempFilename,_str options, bool origModify,_str langID)
{
   tempFilename = mktemp();
   ...
   do {
      longestLN := 0;
      longestLineLen := 0;
      INTHASHTAB wrappedLines;
      if ( wrapLen<0 ) {

         top();up();
         numWrapped := 0;
         while ( !down() ) {
            curLineLen := _line_length(true);
            curLN := p_RLine;

            curLineIsWrapped := (_lineflags()&VSLF_EOL_MISSING)!=0;
            if ( curLineIsWrapped ) {
               numWrapped = 1;
               // If the line is wrapped, there won't be an EOL char
               lastLen := 0;
               while ( !down() ) {
                  curLineLen += _line_length(true);
                  ++numWrapped;
                  if ( !(_lineflags()&VSLF_EOL_MISSING) ) {
                     lastLen = _line_length(true);
                     break;
                  }
               }
            }
            ...
            if ( curLineIsWrapped ) {
               wrappedLines:[curLN]= numWrapped;
            }
            numWrapped = 0;
         }
         wrapLen = longestLineLen;
      }

      origGlobalWrapLength := _default_option(VSOPTION_FORCE_WRAP_LINE_LEN);
      _default_option(VSOPTION_FORCE_WRAP_LINE_LEN,wrapLen);
      numWrappedLines := p_Noflines - p_NofNoSave - p_RNoflines;

      // Be sure we get the one off of disk and not the one in memory again
      status = load_files(options' +d '_maybe_quote_filename(tempFilename));
      p_buf_flags |= VSBUFFLAG_HIDDEN;
      p_ForceWrapLineLen = wrapLen;
      _default_option(VSOPTION_FORCE_WRAP_LINE_LEN,origGlobalWrapLength);                                                                    

      _DiffSetEditorLanguage(langID);
      _updateTextChange();
...
int loadFileAndGetRegionForFuncOrRange(DIFF_SETUP_FILE_DATA &file,
                                       DIFF_SHOWING_INFO &diffShowingInfo,
                                       int oneOrTwo,
                                       bool &InMem,
                                       _str SymbolName,
                                       ...
                                       _str forceEOL="",
                                       _str attachedToBufferName="",
                                       bool lookForFilesepOnBuffer=false
                                       )
{
   tempUnwrappedFilename := "";
   DIFF_BUFFER_INFO *pBufInfo = oneOrTwo==1 ? &diffShowingInfo.buffer1 : &diffShowingInfo.buffer2;
   pBufInfo->SymbolViewId =0;
   ...
   origBufID := -1;
   usedFilename := false;
   do_SetEditorLanguage := false;
   if (SourceBufId>-1) {
      status=load_files(Options' +q +bi 'SourceBufId);
      origBufID = SourceBufId;
      file.isBuffer = status?false:true;
   }else if (ViewId) {
      status=load_files(Options' +q +bi 'ViewId.p_buf_id);
      origBufID = ViewId.p_buf_id;
      file.isBuffer = status?false:true;
   }else if (IsDisk) {
      _str opts=build_load_options(filename);
      status=load_files(Options' +q 'opts' +d '_maybe_quote_filename(filename));
      ...
      if ( lookForFilesepOnBuffer ) {
         tempFilename:+=FILESEP;
      }
      status=load_files(Options' +q +b 'tempFilename);
      file.isBuffer = status?false:true;
      if (status) {
         InMem=false;
         status=load_files(Options' '_maybe_quote_filename(tempFilename));
         do_SetEditorLanguage=true;
   ...
   if ( file.isBuffer && oneOrTwo==2 && p_readonly_mode==1 && p_buf_id == diffShowingInfo.buffer1.WholeFileBufId && 
        file.firstLine!=0 ) {
      // If this is the second file, and it was in memory, and the buffer was 
      // already readonly, and this buffer ID matches the whole buffer ID for 
      // buffer 1 and we're comparing a line range within the file
      p_readonly_mode = diffShowingInfo.buffer1.origReadOnly;
   }
   ...
   do {
      ...
      unwrappedBuffer := false;

      // Should not have no save lines at this point, but it won't matter
      numWrappedLines := p_Noflines - p_NofNoSave - p_RNoflines;
      if ( numWrappedLines>0 ) {
         // The buffer we need unwrapped is the current one we just loaded. Re-load
         // the buffer temporarily, save if necessary
         status = getUnwrappedBuffer(def_diff_force_wrap_len,tempUnwrappedFilename,Options,p_modify,p_LangId);
         if ( status ) {
            if ( status==-1 ) {
               // What we have is no good, the lines are still being wrapped
               result := _message_box(nls("These files will not diff correctly because there are lines longer than %s bytes, which will be wrapped.  Would you like to temporarily set the limit for this buffer so that those files will not wrap?\n\nThis may impact performance depending on the number of long lines.",def_diff_force_wrap_len),"",MB_YESNO);
               if ( result==IDYES ) {
                  _delete_buffer();
                  delete_file(tempUnwrappedFilename);

                  tempUnwrappedFilename = "";
                  status = getUnwrappedBuffer(-1,tempUnwrappedFilename,Options,p_modify,p_LangId);
                  if ( !status ) {
                     unwrappedBuffer = true;
                  }
                  ...
                  break;
               }// If they clicked "No", we told them it wouldn't diff right...
            }
         } else {
            unwrappedBuffer = true;
         }
      ...
      */
      ...
      pBufInfo->WholeFileBufId = unwrappedBuffer?origBufID:p_buf_id;
      bufLoadedInfo.bufIDLoaded = p_buf_id;
      bufLoadedInfo.bufNameLoaded = p_buf_name;
      pBufInfo->origReadOnly = p_readonly_mode;
      if ( p_readonly_mode ) {
         file.readOnly = DIFF_READONLY_SET_BY_USER;
      }
      ...
      pBufInfo->fileDate=_file_date(p_buf_name,'B');
      if ( !InMem ) p_buf_flags|=VSBUFFLAG_HIDDEN;
      if ( LastLine < 0 ) LastLine=p_Noflines;

      if ( InMem ) {
         if ( unwrappedBuffer ) {
            // If we unwrapped the buffer, be sure we set the right buffer (the
            // one that was already open, not the unwrapped one, read only
            withBufID(origBufID, bufSetReadOnlyCallback, 2);
         } else {
            typeless t=2;
            OldReadOnly=p_readonly_mode;
            p_readonly_mode=t;
      ...
      if ( unwrappedBuffer ) {
         file.bufID=tempWID.p_buf_id;
         file.isCopiedBuffer=true;

         // Have to load the original file temporarily to be sure the right 
         // file gets marked up
         withTempWID(tempWID,loadBufID,origBufID);

         pBufInfo->SymbolViewId=tempWID;
         markRegion(pBufInfo->SymbolViewId,FirstLine,LastLine,pBufInfo->MarkId,OldReadOnly,dialogInfoWID,forceEOL);

         // Reload the original file into the view
         withTempWID(pBufInfo->SymbolViewId,loadBufID,file.bufID);

         _DiffAddToDeleteList(tempWID.p_buf_id,false,"",pDelItemList);
      } else if ( (!alwaysCopyBuffer && (!InMem || closeBufferWhenDone) && FirstLine==0 && LastLine==0) ||
           file.doNotCopyBuffer) {
         if (!closeBufferWhenDone) {
            _DiffAddToDeleteList(p_buf_id,false,attachedToBufferName,pDelItemList);
            _DiffSetEditorLanguage();
         }
         file.bufID = p_buf_id;
         ...
         _str NewLine=p_newline;
         lastBufIDInTempWID := p_buf_id;

         p_window_id = origWID;
         pBufInfo->SymbolViewId=_GetViewWithRegion(tempWID,FirstLine,LastLine,pBufInfo->MarkId,NewLine,OldReadOnly,dialogInfoWID,forceEOL);
         p_window_id=pBufInfo->SymbolViewId;
         file.bufID=p_buf_id;
         p_window_id=origWID;
         file.isCopiedBuffer=true;
      }
      if (closeBufferWhenDone) {
         if (ViewId) {
            _DiffAddToDeleteList(ViewId,true,attachedToBufferName,pDelItemList);
         }
      }
      if (!unwrappedBuffer) _delete_temp_view(tempWID,false);

#if 1
      // Have to get rid of the blank buffer that was originally in the temp view
      withBufID(origTempBufID,deleteTempBufferCallback);
#else
      p_window_id = HIDDEN_WINDOW_ID;
      _safe_hidden_window();
...
#endif
      if ( InMem ) {
#if 1 //15:34pm 11/30/2021
         withBufID(bufIDOnDisk,deleteTempBufferCallback);
#else
                         ...
                         bool ReturnIfEncoded=false)
{
   ...
   if (utf81!=utf82) {
      ...
      if (status=='') {
         ...
         p_window_id=ViewId1;
         status=MaybeReloadFile(option,misc.buffer1.origEncoding);
         p_window_id=orig_view_id;
         if (status) return(status);

         p_window_id=ViewId2;
         status=MaybeReloadFile(option,misc.buffer2.origEncoding);
         p_window_id=orig_view_id;
         if (status) return(status);
...
_str _DiffGetForceDiskLoadString(bool tryDisk,
                                       bool isBuffer,
                                       int bufferIndex,
                                       int firstLine,
                                       int lastLine)
...
static int DiffSetupDialog(_str File1Name,bool RestoreFromINI,DIFF_INFO info=null,bool isModal=false)
{
   ...
   sessionName := "";
   int wid=show('-mdi -hidden -wh -xy _diffsetup_form',File1Name,false,RestoreFromINI,info,false,isModal);
   if ( !wid ) {
      return(1);
   }
   if ( DialogX!=MAXINT ) {
...
void _DiffSetEditorLanguage(_str lang="")
{
   if ( !(lang=="" && p_LangId!="") ) {
...
int _DiffFromModelessSetupDialog()
{
...
//   gDiffSetupData.isvsdiff = false;
   ...
   if (_DiffIsDirectory(gDiffSetupData.file1.fileName) ||
      ...
      return(MFDiff2(gDiffSetupData,false,false));
   }
   bufwidth=gDiffSetupData.RecordFileWidth;
   file1trydisk=!gDiffSetupData.file1.isBuffer;
   file2trydisk=!gDiffSetupData.file2.isBuffer && gDiffSetupData.file2.firstLine==0;
   ...
   if (!gDiffSetupData.file1.isBuffer && gDiffSetupData.file1.bufID<0 && !gDiffSetupData.file1.isViewID) {
      gDiffSetupData.file1.fileName=_diff_absolute(gDiffSetupData.file1.fileName);
   }
   if (!gDiffSetupData.file2.isBuffer && gDiffSetupData.file2.bufID<0 && !gDiffSetupData.file2.isViewID) {
   ...
   /*if ( gDiffSetupData.Interleaved ) {
      ...
      _DiffSetupDocumentNames(gDiffSetupData.file1.bufID,gDiffSetupData.file1.viewID,
      ...
      _DiffSetupDocumentNames(gDiffSetupData.file2.bufID,gDiffSetupData.file2.viewID,
      ...
      // Don't have to mess with these, they will only be for a single dialog
      ...
      if ( gDiffSetupData.balanceBuffersFirst ) {
         ...
         if ( status==COMMAND_CANCELLED_RC ) {
            _DiffSetDeleteList(delItemList,diff_form_wid);
      ...
      status = loadRegions(gDiffSetupData,misc,usedRegion,
                           ...
                           gDiffSetupData.matchMode2?gDiffSetupData.file1.fileName:"");

      // Have to make copy here
      file1 := gDiffSetupData.file1;
      _SetDialogInfoHt("fileInfo:"file1.bufID,file1,diff_form_wid._ctlfile1);

      file2 := gDiffSetupData.file2;
      _SetDialogInfoHt("fileInfo:"file2.bufID,file2,diff_form_wid._ctlfile1);
      ...
      if ( misc.buffer1.SymbolViewId!=0 ) {
         diff_form_wid._DiffAddToDeleteList(misc.buffer1.SymbolViewId,true,misc.buffer1.SymbolViewId.p_DocumentName,&delItemList);
      }
      if ( misc.buffer2.SymbolViewId!=0 ) {
         diff_form_wid._DiffAddToDeleteList(misc.buffer2.SymbolViewId,true,"",&delItemList);
      }
      if ( misc.deleteBufferList!="" ) {
         for (;;) {
            ...
            diff_form_wid._DiffAddToDeleteList((int)cur,isWindow,misc.buffer1.SymbolViewId.p_DocumentName,&delItemList);
         }
      }
      ...
      status = Wid1.load_files('+bi 'gDiffSetupData.file1.bufID);
      ...
      if ( ( lang!='' && !file1InMem && gDiffSetupData.file1.bufID<0 && !gDiffSetupData.file1.viewID && !gDiffSetupData.file1.isBuffer) ) {
         Wid1._DiffSetEditorLanguage(lang);
      }

      if (status) {
         _DiffSetDeleteList(delItemList,diff_form_wid._ctlfile1);
      ...
      if ( !file1InMem ) {
         documentName := misc.buffer1.SymbolViewId?misc.buffer1.SymbolViewId.p_DocumentName : "";
         diff_form_wid._DiffAddToDeleteList(misc.buffer1.WholeFileBufId,false,documentName,&delItemList);
      }
      ...
      origBufID2 := 0;
      // Load buffer into diff dialog's window
      status = Wid2.load_files('+bi 'gDiffSetupData.file2.bufID);
      if ( !file2InMem ) {
         documentName := misc.buffer2.SymbolViewId?misc.buffer2.SymbolViewId.p_DocumentName : "";
         diff_form_wid._DiffAddToDeleteList(misc.buffer2.WholeFileBufId,false,documentName,&delItemList);
      }
      if ( gDiffSetupData.matchMode2 ){
      ...
      _DiffSetDiffShowingInfo(misc,diff_form_wid._ctlfile1);
      ...
      if ( _haveProDiff() ) {
         ...
         if ( gDiffSetupData.balanceBuffersFirst ) {
            ...
            Wid2._DiffSetEditorLanguage(Wid1.p_LangId);
         }
      }
      if ( !setEditorLanguage ) {
         lang = _DiffFilename2LangId(gDiffSetupData.file2.fileName,gDiffSetupData.file2.bufID);
         if ( lang!='' && !file2InMem && misc.buffer2.WholeFileBufId<0 ) {
            Wid2._DiffSetEditorLanguage(lang);
         }
      }
      _DiffSetDiffShowingInfo(misc,diff_form_wid._ctlfile1);
      ...
      misc=_DiffGetDiffShowingInfo(diff_form_wid._ctlfile1);
      if (status) {
      ...
      */
      ...
      if ( !file2InMem ) {
         documentName := misc.buffer2.SymbolViewId?misc.buffer2.SymbolViewId.p_DocumentName : "";
         diff_form_wid._DiffAddToDeleteList(diff_form_wid._ctlfile2.p_buf_id,false,documentName,&delItemList);
      }
      ...
      misc.buffer1.BufStartTime=_file_date(Wid1.p_buf_name,'B');
      misc.buffer2.BufStartTime=_file_date(Wid2.p_buf_name,'B');
      _DiffSetDiffShowingInfo(misc,diff_form_wid._ctlfile1);
   }
   mou_hour_glass(true);
   gaugeWID=0;
   ...
   diffHandle := -1;
      ...
      if (!AllCommentOverride) {
         if (!(gDiffSetupData.file1.bufID >- 1&&misc.buffer1.WholeFileBufId>=0)) {
            LineInfo1=StartLine1;
         }
         if (!(gDiffSetupData.file1.bufID >-1&&misc.buffer2.WholeFileBufId>=0)){
         ...
         info.viewID1 = ViewId1;
         info.viewID2 = ViewId2;
         info.options = flags;
         info.numDiffOutputs = 0;
         info.isSourceDiff = calledBalanceFiles;
         ...
         _SetDialogInfoHt("lastDiffOptions",info.options,diff_form_wid._ctlfile1);
         if ( diffHandle<0 ) {
         ...
         _control _ctlcopy_right;
         misc.buffer1.softWrap=SoftWrap1;
         misc.buffer2.softWrap=SoftWrap2;
      ...
      if (gDiffSetupData.Interleaved) {
         ...
         if (result==IDNO) {
            ...
            if ( misc.buffer2.WholeFileBufId>0 ) {
               ...
               if ( !status ) {
                  p_readonly_mode = misc.buffer2.origReadOnly;
   ...
   if ( diff_form_wid ) {
      ...
      if ( !gDiffSetupData.Interleaved ) {
         ...
         if ( usedRegion || gDiffSetupData.noSourceDiff ) {
            _nocheck _control ctltypetoggle;
            diff_form_wid.ctltypetoggle.p_visible = diff_form_wid.ctltypetoggle.p_enabled =false;
         }
         diff_form_wid.setFirstLines(gDiffSetupData.file1.firstLine,gDiffSetupData.file2.firstLine);
         if ( gDiffSetupData.matchMode2 ) {
            orig_def_prompt_extless_select_mode := def_prompt_extless_select_mode;
            def_prompt_extless_select_mode = false;
            diff_form_wid._ctlfile2.select_mode(diff_form_wid._ctlfile1.p_mode_name);
            def_prompt_extless_select_mode = orig_def_prompt_extless_select_mode;
         }
...
static void setupOKButton(_str Comment,_str CommentButtonCaption,typeless OkPtr)
{
   ...
   if (gDiffSetupData.Comment!='') {
      ...
      _ctlok.p_width=max(_ctlok._text_width(' '_ctlok.p_caption' '),_ctlok.p_prev.p_width);
...
{
   _nocheck _control ctltypetoggle,_ctlundo,_ctlhelp;
   if ( !_haveProDiff() ) {
...
void _DiffAddToDeleteList(int itemID,bool isView,_str attachedToBufferName,DIFF_DELETE_ITEM (*pItemList)[])
{
   DIFF_DELETE_ITEM curItem;
   curItem.isSuspended = false;
   curItem.isView = isView;
   curItem.item = itemID;
   curItem.attachedToBufferName = attachedToBufferName;

   len := pItemList->_length();
   for (i:=0;i<len;++i) {
      if ( (pItemList->[i].isView && pItemList->[i].item.p_buf_id==itemID) ||
           (!pItemList->[i].isView && pItemList->[i].item==itemID) ) {
         return;
      }
   }
...
void _DiffSetupProgressForFile(int gauge_form_wid,int &gaugeWID)
{
   ...
   if ( gauge_form_wid == p_active_form ) {
      p_active_form.p_caption='Diff Progress';
   }
   _nocheck _control gauge1,label1,label2,label3;
   gaugeWID=gauge1;
   ...
   // This is visible in the resource and hidden below.
   // If we get here and this is already been set to false
   // this dialog was already up.  It is probably on the _svc_update_form
   isPeristentDialog := label2.p_visible == false;
   ...
   if ( !isPeristentDialog ) {
      gauge_form_wid.p_height -= (label2.p_height+label3.p_height+(2*labelSpace));
   }

   if ( p_active_form.p_name == gauge_form_wid.p_name ) {
      // gauge_form_wid could actually be a frame on another form
      // where it is persistent, so we can't delete these.
...
{
   diffCodeInsertedMarkerType := _GetDialogInfoHt("diffCodeInsertedMarkerType", WID2);
   if ( diffCodeInsertedMarkerType==null ) {
   ...
   diffCodeModifiedMarkerType := _SetDialogInfoHt("diffCodeModifiedMarkerType", WID2);
   if ( diffCodeModifiedMarkerType==null ) {
   ...
   diffCodeDeletedMarkerType := _SetDialogInfoHt("diffCodeDeletedMarkerType", WID2);
   if ( diffCodeDeletedMarkerType==null ) {
...
void initDiffInfo(DIFF_INFO &info)
{
   info.viewID1 = -1;
   info.viewID2 = -1;
   info.options = -1;
   info.numDiffOutputs = -1;
...
void _DiffSetDeleteList(DIFF_DELETE_ITEM (&delItemList)[],int dataWID)
{
   _SetDialogInfoHt("DeleteList",delItemList,dataWID);
}

void _DiffGetDeleteList(DIFF_DELETE_ITEM (&delItemList)[],int dataWID)
{
   delItemList = _GetDialogInfoHt("DeleteList",dataWID);
}

static void setVisibleProperty(INTARRAY WIDList,bool visibleState)
{
   foreach ( auto curWID in WIDList ) {
      curWID.p_visible = visibleState;
   }
}

static void showSVCDialogGauge(int diff_form_wid,int gaugeFormWID)
{
   XCenterOfParent := diff_form_wid.p_width intdiv 2;
   XCenterOfGaugeForm := gaugeFormWID.p_width intdiv 2;
   YCenterOfParent := diff_form_wid.p_height intdiv 2;
   YCenterOfGaugeForm := gaugeFormWID.p_height intdiv 2;

   gaugeFormWID.p_x = XCenterOfParent - XCenterOfGaugeForm;
   gaugeFormWID.p_y = YCenterOfParent - YCenterOfGaugeForm;
   hideSVCDiffWindows(diff_form_wid,gaugeFormWID);

   gaugeFormWID.p_visible = true;
}

INTARRAY getSVCDiffWindowIDList()
{
   INTARRAY WIDList;
   _control ctlcontextCombo1;
   _control ctlcontextCombo2;
   _control _ctlfile1;
   _control _ctlfile2;
   _control vscroll1;
   _control _ctlfile1_readonly;
   _control _ctlfile2_readonly;
   _control _ctlfile1line;
   _control _ctlfile2line;
   WIDList :+= ctlcontextCombo1;
   WIDList :+= ctlcontextCombo2;
   WIDList :+= _ctlfile1;
   WIDList :+= _ctlfile2;
   WIDList :+= vscroll1;
   WIDList :+= hscroll1;
   WIDList :+= _ctlfile1_readonly;
   WIDList :+= _ctlfile2_readonly;
   WIDList :+= _ctlfile1line;
   WIDList :+= _ctlfile2line;
   return WIDList;
}

static void sizeAndPositionSVCGauge(int diffFormWID, int gaugeFormWID)
{
   _nocheck _control ctlpicture1;
   xDiff := ctlpicture1.p_width - gaugeFormWID.p_width;
   gaugeFormWID.p_x =  xDiff intdiv 2;
}

static void hideSVCDiffWindows(int diff_form_wid,int gaugeFormWID)
{
   WIDList := getSVCDiffWindowIDList();
   setVisibleProperty(WIDList,false);

   _nocheck _control ctlpicture1;
   sizeAndPositionSVCGauge(ctlpicture1,gaugeFormWID);

   gaugeFormWID.p_visible = true;
}

static void showSVCDiffWindows(int diff_form_wid,int gaugeFormWID)
{
   WIDList := getSVCDiffWindowIDList();
   setVisibleProperty(WIDList,true);

   gaugeFormWID.p_visible = false;
}
 ...
 * <dt><b>-matchmode2</b>
 * <dd>Set file 2's mode to match that of file 1.  Also will
 * save the filename so taht when buffers are deleted it will
 * know that this buffer is associated with that file when
 * things are cleaned up in certain cases.
 ...
 * <dd>Reserved for internal SLickEdit use.  Closes buffer 2
 * when the dialog closes regardless of if it was in memory to
 * start with or not.  This is usually for version control diffs
 * where the archive version of the file is in a temp buffer.
 * <dd>
 * </dl>
 * <p>
 ...
 */
_command int diff(_str cmdline='',bool showNoEditorOptions=false,DIFF_INFO &info=null,int diff_form_wid=0,int gaugeFormWID=0) name_info(FILE_ARG'*,'VSARG2_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveDiff()) {
   ...
   diff_form_wid_passed_in := diff_form_wid != 0;
   gDiffSetupData = null;
   _str LineInfo1="1",LineInfo2="1";
   if (p_window_id==_mdi) {
      p_window_id=_mdi.p_child;
   ...
   name := "";
   typeless status=0;
   bool okToDestoyGaugeWID = gaugeFormWID==0;
   ...
   if ( NeedToPromptForArgs(cmdline,gDiffSetupData.Modal,restorefromini) /*arg(1)=='' || lowcase(arg(1))=='-modal'*/ ) {
      ...
      // gDiffSetupData has not been filled in yet
      modalSetup := pos(' -vsdiff ',' 'cmdline' ')!=0;
      status=DiffSetupDialog(gDiffSetupData.file1.fileName,restorefromini,info,modalSetup);
      // DiffSetupDialog will call _DiffFromModelessSetupDialog, nothing else to
      // do here
      return status;
   } else {
      mou_hour_glass(true);
      ...
      if ( gDiffSetupData.file1.getBufferIndex ) {
         gDiffSetupData.file1.bufID=(int)gDiffSetupData.file1.fileName;
      }
      if ( gDiffSetupData.file2.getBufferIndex ) {
         gDiffSetupData.file2.bufID=(int)gDiffSetupData.file2.fileName;
      }
      ...
      gDiffSetupData.file1.doNotCopyBuffer = misc.buffer1.doNotCopyBuffer;
      gDiffSetupData.file2.doNotCopyBuffer = misc.buffer2.doNotCopyBuffer;
      if (gDiffSetupData.FileSpec!='' || gDiffSetupData.Recursive || gDiffSetupData.DiffTags) {
         ...
         if (!_DiffFileBufferOrFileInViewExists(gDiffSetupData.file1.fileName,!SomeBuf1,gDiffSetupData.file1.bufID,DocName1,gDiffSetupData.file1.viewID!=0) && !gDiffSetupData.Recursive) {
            _message_box(nls("File '%s' does not exist.",gDiffSetupData.file1.fileName));
            return(FILE_NOT_FOUND_RC);
         }
         if (!_DiffFileBufferOrFileInViewExists(gDiffSetupData.file2.fileName,!SomeBuf2,gDiffSetupData.file2.bufID,DocName2,gDiffSetupData.file2.viewID!=0) && !gDiffSetupData.Recursive) {
   ...
   if ( gDiffSetupData.Interleaved ) {
      if (gDiffSetupData.file1.fileName=="") {
         gDiffSetupData.file1.fileName = gDiffSetupData.file1.bufID;
         file1disk = '+bi';
         file1InMem = true;
      }
      status=GetFileHidden(gDiffSetupData.file1.fileName,ViewId1,orig_view_id,file1InMem,file1disk);
      ...
      if (gDiffSetupData.file2.fileName=="") {
         gDiffSetupData.file2.fileName = gDiffSetupData.file2.bufID;
         file2disk = '+bi';
         file2InMem = true;
      }
      status=GetFileHidden(gDiffSetupData.file2.fileName,ViewId2,orig_view_id,file2InMem,file2disk,File2Size);
      ...
      _DiffSetupDocumentNames(gDiffSetupData.file2.bufID,gDiffSetupData.file2.viewID,
                             name2,DocName2,otherbufinfo2,file2disk,gDiffSetupData.file2.fileTitle,
                             gDiffSetupData.file2.fileName,gDiffSetupData.file2.tryDisk);
      isSVCUpdateForm := diff_form_wid_passed_in && diff_form_wid.p_active_form.p_name=="_svc_mfupdate_form";
      if ( !diff_form_wid ) {
         diff_form_wid = show('-desktop -xy -span -hidden -new _diff_form',name1,name2,'diff');
      } else if (isSVCUpdateForm) {
         // svc update.  Would have to set captions 
         _ctlfile1label.p_caption = name1;
         _ctlfile2label.p_caption = name2;
      }
      if ( diff_form_wid ) {
         _SetDialogInfo(DIFFEDIT_READONLY1_VALUE,gDiffSetupData.file1.readOnly,diff_form_wid._ctlfile1);
         _SetDialogInfo(DIFFEDIT_READONLY2_VALUE,gDiffSetupData.file2.readOnly,diff_form_wid._ctlfile1);
      ...
      modifiedMarkerType := _GetDialogInfoHt("modifiedMarkerType",diff_form_wid._ctlfile1);
      if ( modifiedMarkerType==null ) {
         modifiedMarkerType = _MarkerTypeAlloc();
         _MarkerTypeSetFlags(modifiedMarkerType,VSMARKERTYPEFLAG_AUTO_REMOVE);
         _SetDialogInfoHt("modifiedMarkerType",modifiedMarkerType,diff_form_wid._ctlfile1);
      }

      insertedMarkerType := _GetDialogInfoHt("insertedMarkerType",diff_form_wid._ctlfile1);
      if ( insertedMarkerType==null ) {
         insertedMarkerType = _MarkerTypeAlloc();
         _MarkerTypeSetFlags(insertedMarkerType,VSMARKERTYPEFLAG_AUTO_REMOVE);
         _SetDialogInfoHt("insertedMarkerType",insertedMarkerType,diff_form_wid._ctlfile1);
      }
      ...
      if ( gDiffSetupData.balanceBuffersFirst ) {
         ...
         if ( status==COMMAND_CANCELLED_RC ) {
            ...
            diff_form_wid._ctlfile1.p_buf_name = new_buf_name;
            diff_form_wid._delete_window(0);
            if (gaugeFormWID && okToDestoyGaugeWID ) {
               gaugeFormWID._delete_window();
            } else {
               showSVCDiffWindows(diff_form_wid,gaugeFormWID);
            }
            return(COMMAND_CANCELLED_RC);
         }
      }
      ...
      if ( !diff_form_wid_passed_in ) {
         diff_form_wid.p_caption=strip(gDiffSetupData.DialogTitle,'B','"');
      }

      if ( misc.buffer1.SymbolViewId!=0 ) {
         diff_form_wid._DiffAddToDeleteList(misc.buffer1.SymbolViewId,true,gDiffSetupData.file1.fileTitle,&delItemList);
      }
      if ( misc.buffer2.SymbolViewId!=0 ) {
         diff_form_wid._DiffAddToDeleteList(misc.buffer2.SymbolViewId,true,gDiffSetupData.file1.fileTitle,&delItemList);
      }
      if ( misc.deleteBufferList!="" ) {
         for (;;) {
            parse misc.deleteBufferList with auto cur ',' misc.deleteBufferList;
            ...
            diff_form_wid._DiffAddToDeleteList((int)cur,isWindow,gDiffSetupData.file1.fileTitle,&delItemList);
         }
      }

      misc.PreserveInfo=gDiffSetupData.file1.preserve' 'gDiffSetupData.file2.preserve;

      disabledDiffBufID1 := _GetDialogInfoHt("disabledDiffBufID1");
      if ( !diff_form_wid._isdiff_update_form() &&
           !diff_form_wid._DiffIsDeleteSuspended(Wid1.p_buf_id) &&
           (disabledDiffBufID1==null || Wid1.p_buf_id!=disabledDiffBufID1)
           ) {
         Wid1._delete_buffer();
      }
      status=1;
      ...
      diff_form_wid.DiffSetupWindows(Wid1,diff_form_wid._ctlfile1_readonly,file1InMem,
                                     gDiffSetupData.file1.readOnly);
      if ( !file1InMem ) {
         diff_form_wid._DiffAddToDeleteList(misc.buffer1.WholeFileBufId,false,gDiffSetupData.file1.fileTitle,&delItemList);
      }
      if ( !_haveProDiff() || gDiffSetupData.ViewOnly ) {
         diff_form_wid.DiffSetupDialogForViewing();
      }
      ...
      ViewId1=Wid1.p_window_id;
      disabledDiffBufID2 := _GetDialogInfoHt("disabledDiffBufID2");
      if ( !diff_form_wid._isdiff_update_form() &&
           !diff_form_wid._DiffIsDeleteSuspended(Wid2.p_buf_id) &&
           (disabledDiffBufID2==null || Wid2.p_buf_id!=disabledDiffBufID2)
           ) {
         Wid2._delete_buffer();
      }
      ...
      status = Wid2.load_files('+bi 'gDiffSetupData.file2.bufID);
      if ( !file2InMem ) {
         diff_form_wid._DiffAddToDeleteList(misc.buffer2.WholeFileBufId,false,gDiffSetupData.file1.fileTitle,&delItemList);
      }
      if ( gDiffSetupData.matchMode2 ){
         diff_form_wid._ctlfile2.p_mode_name = diff_form_wid._ctlfile1.p_mode_name;
      }
      ...
      */
      ...
      diff_form_wid.DiffSetupWindows(Wid2,diff_form_wid._ctlfile2_readonly,file2InMem,
                                     gDiffSetupData.file2.readOnly);
      if ( !file2InMem ) {
         diff_form_wid._DiffAddToDeleteList(diff_form_wid._ctlfile2.p_buf_id,false,gDiffSetupData.file1.fileTitle,&delItemList);
      }
   ...
   if ( def_diff_edit_flags&DIFFEDIT_SHOW_GAUGE ) {
      if ( !gaugeFormWID ) {
         ...
         showSVCDialogGauge(diff_form_wid,gaugeFormWID);
      }
      _DiffSetupProgressForFile(gaugeFormWID,gaugeWID);
   }
   AllCommentOverride := false;
   ...
   if (gDiffSetupData.Interleaved) {
      ...
      info.viewID1 = ViewId1;
      info.viewID2 = ViewId2;
      info.options = flags|DIFF_OUTPUT_INTERLEAVED;
      info.numDiffOutputs = NumOutputBuffers;
      info.isSourceDiff = false;
      info.loadOptions = def_load_options;
      info.gaugeWID = gaugeWID;
      info.maxFastFileSize = def_max_fast_diff_size;
      ...
      if (!AllCommentOverride) {
         if ( gDiffSetupData.matchMode2 ) {
            orig_def_prompt_extless_select_mode := def_prompt_extless_select_mode;
            def_prompt_extless_select_mode = false;
            file1Ext := _get_extension(diff_form_wid._ctlfile1.p_buf_name,true);
            diff_form_wid._ctlfile2.p_buf_name :+= file1Ext;

            diff_form_wid._ctlfile2.select_mode(diff_form_wid._ctlfile1.p_mode_name);

            diff_form_wid._ctlfile2.p_hex_mode = HM_HEX_OFF;
            def_prompt_extless_select_mode = orig_def_prompt_extless_select_mode;
         }
         if (!(gDiffSetupData.file1.bufID >- 1&&misc.buffer1.WholeFileBufId>=0)) {
            LineInfo1=StartLine1;
...
//   _mdi.p_child._set_focus();
   ...
   if ( SEDiffFilesMatch(diffHandle) || AllCommentOverride ) {
      ...
      if (gDiffSetupData.Interleaved) {
         ...
         diff_form_wid._DiffSetFilesMatch('Files match');
         result := gShowNoChangesMessage;
         if (!gDiffSetupData.Quiet && !gShowNoChangesMessage && !diff_form_wid._DiffDialogIsSVCUpdate()) {
            if (StartLine1!=1||StartLine2!=1) {
               result=_message_box(nls("Files match (except for leading comments), view them anyway?"),'',MB_YESNOCANCEL|MB_ICONQUESTION);
            }else{
               msg := nls("Files match, view them anyway?");
...
static void setFirstLines(int firstLine1,int firstLine2)
{
   firstLine1 = firstLine1 != null ? firstLine1 : 1;
   firstLine2 = firstLine2 != null ? firstLine2 : 1;
   _SetDialogInfoHt("diffFirstLines",firstLine1' 'firstLine2,_ctlfile1);
}

static int parseOptions(_str &cmdline,DIFF_SETUP_DATA &diffSetupData,DIFF_SHOWING_INFO &misc,int &flags,bool &done)
{
   typeless x1='';
   typeless x2='';
   file_list_info := "";//If not blank, this information is for the "-listonly" option
   ...
   for (;;) {
      ...
      if (option=='r1') {
         ...
         misc.buffer1.TagParentIndex=(typeless)parse_file(cmdline);
         misc.buffer2.TagParentIndex=(typeless)parse_file(cmdline);
         if (!isinteger(misc.buffer1.TagParentIndex) ||
             !isinteger(misc.buffer2.TagParentIndex)) {
            message('Cannot refresh tags after diff, invalid parent index');
            misc.RefreshTagsOnClose=false;
            misc.buffer1.TagParentIndex=0;
            misc.buffer2.TagParentIndex=0;
         }
...
//         file2trydisk=1;
      ...
      }else if (option=='donotcopybuffer1') {
         misc.buffer1.doNotCopyBuffer = true;
      }else if (option=='donotcopybuffer2') {
         misc.buffer2.doNotCopyBuffer = true;
...
static void _DiffShowInfo(int bufID,_str label="")
{
   if ( label=="" ) {
      label="_DiffShowInfo ":+__LINE__;
   }
   origWID := _create_temp_view(auto tempWID);
   origBufID := p_buf_id;

   status := load_files('+bi 'bufID);
   say('_DiffShowInfo ':+label' status='status' bufID='bufID' p_buf_id='p_buf_id);
   say('    p_readonly_mode='p_readonly_mode);
...
static int bufSetReadOnlyCallback(typeless callbackData)
{
   p_readonly_mode = callbackData;
   return 0;
}

static int deleteTempBufferCallback(typeless callbackData)
{
   _delete_buffer();
   return 0;
}

static int withBufID(int bufID, typeless *pfnCallback, typeless callbackData=null)
{
   origWID := _create_temp_view(auto tempWID);
   origBufID := p_buf_id;
   ...
   calbackStatus := (*pfnCallback)(callbackData);
...
static int loadBufID(typeless callbackData)
{
   return load_files('+bi 'callbackData);
}

static int withTempWID(int tempWID, typeless *pfnCallback, typeless callbackData=null)
{
   origWID := p_window_id;
   p_window_id = tempWID;
                       ...
                       )
{
   ...
   status := loadFileAndGetRegionForFuncOrRange(diffSetupData.file1,
                                                ...
                                                false,
                                                false,
                                                attachedToBufferName:attachedToBufferName);
   if ( status ) {
      mou_hour_glass(false);
      return(status);
   }
   ...
   */
   ...
   lookForFilesepOnBuffer := _file_eq(diffSetupData.file1.fileName,diffSetupData.file2.fileName);

   origFile2ReadOnly := diffSetupData.file2.readOnly;
   status = loadFileAndGetRegionForFuncOrRange(diffSetupData.file2,
                                               ...
                                               eolChars1,
                                               attachedToBufferName,
                                               lookForFilesepOnBuffer);

   if ( misc.buffer1.WholeFileBufId == misc.buffer2.WholeFileBufId ) {
      // The parent buffer is the same
      diffSetupData.file2.readOnly = origFile2ReadOnly;
   }
   ...
   if ( misc.buffer1.SymbolViewId!= 0 && misc.buffer1.WholeFileBufId!=0 ) {
      _DiffSetCopyBufIDToOriginaBufID(misc.buffer1.SymbolViewId.p_buf_id, misc.buffer1.WholeFileBufId, dialogInfoWID);
   }
   if ( misc.buffer2.SymbolViewId!=0 && misc.buffer2.WholeFileBufId!=0 ) {
      _DiffSetCopyBufIDToOriginaBufID(misc.buffer2.SymbolViewId.p_buf_id, misc.buffer2.WholeFileBufId, dialogInfoWID);
   }
...
static int setupForInterleaved(bool file1inmem,_str file1disk,bool file2inmem,_str file2disk,int ViewId1,int ViewId2,int OutputBufId,int gaugeFormWID)
{
   ...
   if (lang1!='') {
      _DiffSetEditorLanguage(lang1);
   }else{
      if (lang2!='') {
         _DiffSetEditorLanguage(lang2);
      }
   }
   //read_only_mode();
   p_color_flags|=MODIFY_COLOR_FLAG;
                           ...
                           int diff_form_wid,bool unicodeSourceFiles,bool AllCommentOverride)
{
   ...
   // both windows should have the same color scheme
   Wid2.p_WindowColorProfile = Wid1.p_WindowColorProfile;
}
...
void _DiffSetupDocumentNames(int BufferIndex,int ViewId,_str &name,
                             _str DocName,
                             _str otherbufinfo,_str filediskstr,
                             _str Title,_str Filename,int fileOnDisk)
{
   origTitle := Title;
   parse Title with Title '(' auto inParens ')' auto rest;
   haveParenInfo := inParens!="" && lowcase(inParens)!="buffer" && lowcase(inParens)!="file";
   ...
   if (BufferIndex>-1 || ViewId) {
      ...
      name=Filename;
      name=strip(name,'B','"');
      nameAndTitleMatchedOriginally := _file_eq(name, origTitle);
      ...
      if ( haveParenInfo ) {
         name :+= ' ('inParens')';
      } else if (!nameAndTitleMatchedOriginally && Title!="") {
         name=Title;
      }
   }
}
...
//         say('FastCompareFiles lang1='lang1' lang2='lang2);
         ...
         // we have to be sure we call _DiffSetEditorLanguage the same as if
...
//            say('FastCompareFiles setting lang');
            ...
            File1ViewId._DiffSetEditorLanguage(lang1);
            File2ViewId._DiffSetEditorLanguage(lang2);
         }
                        ...
                        _str ExcludeFilespecs='')
{
   ...
   if (status) {
      ...
      p_buf_name=NewName;
      say('AddToLogfile '__LINE__' p_buf_name='p_buf_name);
   }
   bottom();
...
{
   // need to declare controls because we are not in the diff form
   _nocheck _control _ctlfile1,_ctlfile2;

   // get the language-specific diff font options
   lang_font_name := "";
   typeless lang_font_size=10;
   typeless lang_font_flags=0;
   lang_font_info := LanguageSettings.getDiffEditorFontInfo(_ctlfile1.p_LangId);
   parse lang_font_info with lang_font_name','lang_font_size','lang_font_flags','.;

   // get the default font options
   font_name := "";
   typeless font_size=10;
   typeless font_flags=0;
   def_font_info := _default_font(fontIndex);
   parse def_font_info with font_name','font_size','font_flags','.;

   // merge the language-specific settings with the default settings
   if (lang_font_name != "") font_name = lang_font_name;
   if (lang_font_size != "" && lang_font_size > 0) font_size = lang_font_size;
   if (lang_font_flags != "" && lang_font_flags > 0) font_flags = lang_font_flags;
