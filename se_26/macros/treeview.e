...
static void addItemsToTable(STRHASHTAB &allOverlays,INTARRAY &newOverlays)
{
   foreach ( auto curOverlay in newOverlays ) {
      allOverlays:[curOverlay] = "";
   }
}

/**
 * Merge items from <B>newOverlays</B> into existing 
 * <B>overlays</B> without duplicates. 
 *  
 * These arrays will be short. 
 *  
 * As of 10/7/2021 this does not handle the order of overlays, 
 * but we don't have any that would matter. 
 * 
 * @param overlays    Array of overlays to merge 
 *                    <B>newOverlays</B> into
 * @param newOverlays Array of new overlays.  There might or 
 *                    might not be duplicates of items in
 *                    <B>overlays</B>.
 */
void _TreeMergeOverlays(INTARRAY &overlays,INTARRAY &newOverlays)
{
   STRHASHTAB allOverlays;
   addItemsToTable(allOverlays,overlays);
   addItemsToTable(allOverlays,newOverlays);
   overlays._makeempty();
   foreach ( auto key => auto value in allOverlays ) {
      overlays :+= key;
   }
}

/**
 * Add item to <B>overlays</B> without duplicates. 
 *  
 * These arrays will be short 
 *  
 * @param overlays    Array of overlays to append
 * @param newOverlays new overlay
 */
void _TreeAddOverlay(INTARRAY &overlays,int overlay)
{
...
/**
 * Remove item to <B>overlay</B> without duplicates.
 *  
 * @param overlays    Array of overlays to append
 * @param newOverlays New overlay to add
 */
void _TreeRemoveOverlay(INTARRAY &overlays,int overlay)
{
   len := overlays._length();
   INTARRAY indexesToRemove;
   for ( i:=0; i<len; ++i ) {
      if ( overlay == overlays[i] ) {
         indexesToRemove :+= i;
      }
   ...
   len = indexesToRemove._length();
   for ( i=len-1; i>=0; --i) {
      overlays._deleteel(indexesToRemove[i]);
   }
}

/**
 * @param overlays    Array of overlays to append
 * @param overlay index of overlay to look for 
 * @return true if <b>overlay</b> is in <B>overlays</B>
 */
bool _TreeHaveOverlay(INTARRAY &overlays,int overlay)
{
   len := overlays._length();
   for ( i:=0; i<len; ++i ) {
      if ( overlay == overlays[i] ) {
 ...
 */
int _TreeCopy2(int TreeWID, int SrcIndexParent, int DestIndexParent,int DestAddFlags=TREE_ADD_AS_CHILD)
{
   TREE_COPY_INFO info;
   //GetAllInfo(SrcIndex,info,SrcWID);
   if (TreeWID < 0) {
   ...
   int src_child_index;
   int next_dest_index = TreeWID._TreeAddItem(DestIndexParent,"",DestAddFlags);
...
static void callCloseEditor(int callEvent,int saveData,int closeEditor=1,int setParentFocus=0)
{
   ...
   // make sure the parent heirarch has tree control
   if (p_parent && p_parent.p_parent && p_parent.p_parent.p_object != OI_TREE_VIEW) {
      return;
   }
   if (!p_parent.p_parent._TreeIndexIsValid(index)) {
      return;
   }
...
void _list_pass_thru_ctrl_az(int select_all_wid=0) {
   switch (name_on_key(last_event())) {
   case 'select-all':
      if(select_all_wid) {
         select_all_wid.call_event(select_all_wid,C_A);
         return;
      }
      break;
   case 'cursor-up':
