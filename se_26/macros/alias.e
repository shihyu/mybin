...
/**
 * Format current time according to the {@code format} 
 * specification and return result. 
 *
 * <p>
 *
 * Uses the same conversion-specifiers as the operating system 
 * implementation of {@code strftime}. The '#' is used as a 
                             ...
                             bool isCommandState = false)
{
   ...
   // skip expanding <returns> if there is no return type
   if (pos("<returns>", alias_line, 1, 'i') && pos("</returns>", alias_line, 1, 'i')) {
      if (!local_param_names.returnNum()) {
         parse alias_line with auto before_return "<returns>",'i' auto retval "</returns>",'i' auto after_return;
         if (retval == '' || upcase(retval) == '%\C' || upcase(retval) == '%\L' || upcase(retval) == '%\S') {
            if (effectively_empty(after_return) && (before_return == "///" || before_return == "//!" || before_return == "*")) {
               cancel_expand_alias_line(expand_first, originalStartCol, originalLine, stop_info, multi_cursor_info);
               return 0;
            }
         }
         alias_line = before_return:+after_return;
      }
   }

   // calculate the exception width
   exception_width := (substr(alias_line,1, 2)=='//')? 0 : def_javadoc_exceptionmin;
   foreach (auto ex in local_param_names.m_exception) {
      if (length(ex) > exception_width) {
         exception_width = min(length(ex), def_javadoc_exceptionmax);
      }
   }

   // calculate the parameter name width
   param_width := (substr(alias_line,1, 2)=='//')? 0 : def_javadoc_parammin;
   foreach (auto pa in local_param_names.m_param) {
      if (length(pa) > param_width) {
         param_width = min(length(pa), def_javadoc_parammax);
      }
   }

   // now loop through to expand all the escape sequences
   typeless p='';
   i := k := l := 0;
   code := "";
   before := "";
   ...
   for (;;) {
      ...
      if ( ! expand_first && adjust_spaces ) {
         if ( comment_prefix!="" ) {
            comment_prefix_str := indent_string(comment_prefix_col-1):+comment_prefix;
      ...
      switch (code) {
         ...
         case '%\P':
            ...
            if (param_index >= 0 && param_index < local_param_names.m_param._length()) {
               ...
               param_after := "";
               if (length(param_name) < param_width) {
                  param_after = substr("", 1, (param_width - length(param_name)));
               }
               if (_first_char(after) == '>') {
                  after = param_name :+ _first_char(after) :+ param_after :+ substr(after,2);
               } else if (substr(after,1,2) == '">') {
                  after = param_name :+ substr(after,1,2) :+ param_after :+ substr(after,3);
               } else {
                  after = param_name :+ param_after :+ after;
               }
            }
            break;
         case '%\Q':
            ...
            param_index = local_param_names.paramNum() - paramExpansionCounter;
            if (param_index >= 0 && param_index < local_param_names.m_ptype._length()) {
   ...
   while (filename != '') {
      ...
      foreach (auto match_name => auto alias_text in aliases) {
         ...
         if (substr(match_name,1,9) != "=surround") {
            tag_auto_complete_add_result(AUTO_COMPLETE_ALIAS_PRIORITY, 
