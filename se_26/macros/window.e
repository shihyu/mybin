...
_metadata enum_flags ExitFlags {
   EXIT_CONFIRM         = 0x40,
};
                 ...
                 )
{
  ...
  if (justSaveBuffer) {
     if (saveBufferPos && changes_written && !(_IsTempBufName() && p_DocumentName!='')) _add_filepos_info(p_buf_name);
     return(0);
  }
   ...
   for (i=1;i<=last;++i) {
      if (_iswindow_valid(i) && i._isEditorCtl(false) && !i.p_IsTempEditor) {
         ...
         if (thiscfg!=MAXINT) {
            ...
            lang_font_info := "";
            if (i.p_IsMinimap) {
               lang_font_info = se.lang.api.LanguageSettings.getMinimapFontInfo(lang);
            } else {
               lang_font_info = se.lang.api.LanguageSettings.getEditorFontInfo(lang);
            }
            default_font_info := _default_font(thiscfg);
            parse default_font_info with font_name','auto str_font_size','auto str_font_flags','auto str_charset',';
            ...
            i.p_BufferFontInfo   = se.lang.api.LanguageSettings.getEditorFontInfo(lang);
            i.p_MinimapFontInfo  = se.lang.api.LanguageSettings.getMinimapFontInfo(lang);
            i.p_redraw=true;
         }
...
static int _safe_exit(_str doNotExit='', _str endingSession=false) {
   ...
   if (def_save_config_flags&EXIT_CONFIG_PROMPT) {
...
int _OnUpdate_menuminimapcmd(CMDUI cmdui,int target_wid,_str command)
{
   ...
   if (cmd=='symbol-coloring') {
      if (!_haveContextTagging()) {
         return(MF_GRAYED|MF_DELETED);
      }
...
_command void menuminimapcmd(_str cmd='') name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY) 
{
   ...
   case 'font-size':
      {
         ...
         int array[];
         if (set_both) {
            array :+= CFG_SBCS_DBCS_MINIMAP_WINDOW;
            array :+= CFG_UNICODE_MINIMAP_WINDOW;
         } else {
   ...
   case 'vertical-lines':
      ...
      return;
   case 'symbol-coloring':
      _default_option(VSOPTION_MINIMAP_SHOW_SYMBOL_COLORING,_default_option(VSOPTION_MINIMAP_SHOW_SYMBOL_COLORING)?0:1);
      return;
   case 'modified-lines':
      if (_default_option(VSOPTION_MINIMAP_LEFT_MARGIN) && _default_option(VSOPTION_MINIMAP_SHOW_MODIFIED_LINES)) {
         _default_option(VSOPTION_MINIMAP_SHOW_MODIFIED_LINES,0);
   ...
   case 'delayed':
      ...
      return;
   case 'color':
      show('-modal -xy -wh _window_color_form', '', p_MinimapColorProfile, p_IsMinimap? p_window_id:p_minimap_wid);
      return;
   default:
      if (isinteger(cmd)) {
