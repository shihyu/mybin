...
static void _resize_and_position_options() {
   ...
   ctl_regex_menu.resizeToolButtonToLabel(ctl_regex.p_window_id);
   ...
   ctl_matchcolor_menu.p_y = y;
   ctl_matchcolor_menu.resizeToolButtonToLabel(ctl_matchcolor.p_window_id);
...
static void _ds_init_markers()
{
   if (gDSMarkerType < 0) {
      ...
      _MarkerTypeSetPriority(gDSMarkerType, VSMARKER_TYPE_PRIORITY_DOC_SEARCH_MATCH);
   }
   if (gDSMarkerCurrentType < 0) {
      ...
      _MarkerTypeSetPriority(gDSMarkerCurrentType, VSMARKER_TYPE_PRIORITY_DOC_SEARCH_CURRENT);
   }
...
static int _ds_next(_str search_string, _str search_options, int search_range, bool doPrev, bool& wrap, bool doReplaceNext)
{
   ...
   if (status && status != INVALID_REGULAR_EXPRESSION_RC && status!=COMMAND_CANCELLED_RC) {
      if (p_mdi_child) {
