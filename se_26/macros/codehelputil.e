                                ...
                                )
{
   ...
   if (!_CheckUpdateContextSizeLimits(VS_UPDATEFLAG_context|VS_UPDATEFLAG_tokens, quiet:true)) {
      fast_get_index = 0;
   }
...
int getLocalFunctionParams(MI_localFunctionParams_t& localParms)
{
   ...
   if (tag_tree_type_is_func(cm.type_name) || 
      ...
      if (tag_tree_type_is_class(cm.type_name) || 
          ...
          cm.type_name == SE_TAG_TYPE_CONSTRUCTOR ||
          cm.type_name == SE_TAG_TYPE_DESTRUCTOR) {
         localParms.m_needsReturnType=false;
      }
   }
...
static int _override_method(bool quiet,VS_TAG_BROWSE_INFO &cm,bool doEdit)
{
   ...
   if (cm!=null && _LanguageInheritsFrom("c")) {
      if (filename=="") {
         ...
         wildcards := _file_types_get_wildcards_by_name(def_file_types, "C/C++ Files","c");
