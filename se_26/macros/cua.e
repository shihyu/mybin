   ...
   */
   ...
   if (just_call_key) {
      ...
      */
      ...
      _macro_append('p=point();orig_col=p_col;');
      line_case := _select_type()=='LINE';
      _begin_select();
      p=point();orig_col:=p_col;
      if( _select_type()=='LINE' ) {
         // Guarantee that the cursor does not end up in the middle of
         // virtual space after call to delete_selection().
         _macro_append('if (_select_type()=="LINE") {');
      ...
      delete_selection();
      last_index(0);
      _macro_append('if( line_case || orig_col==1) {');
      _macro_append('   if (p!=point()) {');
      _macro_append('       _end_line();');
      _macro_append('   }');
      _macro_append('}');
      if( line_case || 
          /* Char selection case where deleting all lines after the cursors
