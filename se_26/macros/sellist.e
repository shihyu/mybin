...
enum_flags {
   ...
   /**
    * Only works when combined with BUFLIST_FLAG_FORCE_OLD_BUFLIST
    */
   BUFLIST_FLAG_SHOW_BUFID=0x20,
};
      ...
      if ( !skipBuf && (p_buf_id!=temp_buf_id) ) {
         ...
         if (!list_modified ||
            ...
            if ( ! is_hidden || allow_system_buffers) {
               ...
               bufferCaption := substr(modify,1,BUFFER_COL-1):+ strip(line,'L');
               if (def_buflist&BUFLIST_FLAG_SHOW_BUFID) {
                  bufferCaption :+= ' bufid:'buf_id;
               }
               insert_line(bufferCaption);
               p_buf_id=buf_id;
