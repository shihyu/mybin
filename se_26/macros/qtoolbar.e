...
void autorestore_qtoolbars(_str option, int& noflines)
{
   ...
   if (option=='r' || option=='n') {
      ...
      had_unified_toolbar := false;
      had_current_context := false;
      _str toolbarFormNames[];
      int loaded[];
      typeless movable[];
      ...
      while (noftoolbars-- > 0) {
         ...
         // the obsolete Mac unified toolbar included the current context form
         // the standard toolbar does not, so to migrate from unified to
         // current context, we need to add it
         if (formName == "_tbunified_form") {
            toolbarFormNames._insertel("_tbstandard_form", 0);
            loaded._insertel(0, 0);
            movable._insertel(locked, 0);
            if (_haveCurrentContextToolBar()) {
               toolbarFormNames._insertel("_tbcontext_form", 1);
               loaded._insertel(0, 1);
               movable._insertel(locked, 1);
            }
            had_unified_toolbar = true;
            tbstate = "";
         } else if (formName == "_tbcontext_form") {
            if (had_unified_toolbar && _haveCurrentContextToolBar()) {
               toolbarFormNames._deleteel(1);
               loaded._deleteel(1);
               movable._deleteel(1);
            }
            toolbarFormNames :+= formName;
            loaded  :+= 0;
            movable :+= locked;
         } else {
            toolbarFormNames :+= formName;
      ...
      foreach (i => formName in toolbarFormNames) {
...
void _qtoolbar_etab2.rbutton_up,context()
{
   if (!isEclipsePlugin()) {
      list_toolbars := ( arg(1)=="" );
