...
int _restore_bookmark(_str filename, int line_no,bool allowBufferListToBeReordered,_str markid,int persistent_wid)
{
   _maybe_restore_bookmark_wid(markid,persistent_wid,filename);
                          ...
                          bool relocatable=true)
{
   ...
   for ( i:=0; i < _BookmarkQCount(); ++i ) {
      ...
      _BookmarkGetInfo(i, bmInfo.BookmarkName, 0, bmInfo.vsbmflags, buf_id, 1,
                       ...
                       bmInfo.Filename, bmInfo.DocumentName,bmInfo.persistent_wid);
...
void _RestoreBookmarksInFile(BookmarkSaveInfo (&bmSaves)[], int adjustLinesBy=0)
{
   ...
   for (j := 0; j < bmSaves._length(); ++j) {
      ...
      _BookmarkAdd(bmSaves[j].BookmarkName, 
                   ...
                   bmSaves[j].Filename, bmSaves[j].DocumentName,bmSaves[j].persistent_wid);
   }
...
_command old_goto_bookmark(_str bookmarkName="") name_info(BOOKMARK_ARG','VSARG2_READ_ONLY)
{
   ...
   if ( bm_data != "" ) {  /* a valid bookmark was selected */
      ...
      parse bm_data with bm_id markid .;
      status=_BookmarkGetInfo(_BookmarkFind(bm_id),
                              bm_id,markid,vsbmflags,buf_id,
                              0,RealLineNumber,col,BeginLineROffset,
                              LineData,filename,DocumentName,persistent_wid
                             );
      if (!status || status==TEXT_NOT_SELECTED_RC) {
         status=_restore_bookmark(filename,RealLineNumber,true,markid,persistent_wid);
      }
      if (status) {
         return (status);
      }
...
static int gotoBookmarkId(int bm_id, _str &bookmarkName,bool allowBufferListToBeReordered=true)
{
   ...
   if (!status || status==TEXT_NOT_SELECTED_RC) {
      status=_restore_bookmark(filename,RealLineNumber,allowBufferListToBeReordered,markid,persistent_wid);
      if (status) {
         return (status);
      }
   }
 ...
 *               </ul>
 *               and {@code bookmark_name} is the name of the bookmark
 *               to create.
 * 
 * @see push_bookmark
 * @see pop_bookmark
...
_command set_bookmark,sb(_str arg1="", bool quiet = false) name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   **************************************************************************/
   ...
   if ( bm_id != "" && ! (mark_id != "" && cmd_option :== '-M') ) {
      ...
      _BookmarkAdd(bm_id,mark_id, vsbmflags,-1,0,0,'','','',_get_persistent_wid());
      if (!quiet) {

         // find the correspond key binding for activate_bookmarks
         // or goto-bookmark and display it on the message bar.
         gtkeys := _where_is("activate_bookmarks");
         if (gtkeys == "") {
            gtkeys = _where_is("goto_bookmark");
         }
         msgkey := "";
         if (gtkeys!="") {
...
_command void alt_bookmark() name_info(','VSARG2_LASTKEY|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   gtkeys := _where_is("alt_gtbookmark");
   while (gtkeys!="") {
      msgkey := "";
      parse gtkeys with msgkey ',' gtkeys;
...
void relocateBookmarks ()
{
   ...
   for (i = 0; i < bmInfos._length(); ++i) {
      ...
      _BookmarkAdd(bmInfos[i].BookmarkName, 
                   ...
                   bmInfos[i].Filename, bmInfos[i].DocumentName,bmInfos[i].persistent_wid);
   }
 ...
 * @return _str
 */
_str _sr_bookmark3(_str option="",_str info="")
{
   ...
   if ( option=='R' || option=='N' ) {
      ...
      for (i=0;i<_BookmarkQCount();++i) {
         _BookmarkGetInfo(i,BookmarkName,markid,vsbmflags,
                          bufid,
                          0,RealLineNumber,col,BeginLineROffset,LineData,
                          Filename,DocumentName,persistent_wid);
         if (!(vsbmflags & VSBMFLAG_STANDARD)) {
            continue;
         }
         ...
         RELOC_MARKER lm;
         bool has_relocatable_marker=true;
         if (!getRelocMarker(Filename, BookmarkName, lm)) {
            has_relocatable_marker=false;
         }
         ...
         if (has_relocatable_marker) {
            insert_line(BookmarkName \1 vsbmflags \1 RealLineNumber \1 col \1 BeginLineROffset \1 LineData \1 InsertFilename \1 DocumentName \1 lm.aboveCount \1 lm.belowCount \1 persistent_wid);
            int j;
            int k;
            _str RMLine;
            ...
            insert_line(BookmarkName \1 vsbmflags \1 RealLineNumber \1 col \1 BeginLineROffset \1 LineData \1 InsertFilename \1 DocumentName \1 0 \1 0 \1 persistent_wid);
         }
         ++Nofbookmarks;
      }
      orig_line2:=p_line;
...
void read_BOOKMARK3 (int Nofbookmarks)
{
   ...
   for (i=1; i<=Nofbookmarks ; ++i) {
      ...
      aboveCount := "";
      belowCount := "";
      parse line with BookmarkName \1 vsbmflags \1 RealLineNumber \1 col \1 BeginLineROffset \1 LineData \1 Filename \1 DocumentName \1 aboveCount \1 belowCount \1 persistent_wid;
      if (vsbmflags == "") {
         break;
      }
      if (def_bm_show_picture) {
      ...
      if (!isinteger(aboveCount)) {
         aboveCount = 0;
      }
      if (!isinteger(belowCount)) {
         belowCount = 0;
      }
      ...
      if (status) {
         _BookmarkAdd(BookmarkName,markid,vsbmflags,RealLineNumber,col,
                      BeginLineROffset,LineData,Filename,DocumentName,persistent_wid);
      } else {
         ...
         p_col=col;
         _select_char(markid);
         _BookmarkAdd(BookmarkName,markid,vsbmflags,-1,0,0,'','','',persistent_wid);
         _delete_temp_view(temp_view_id);
 ...
 */
void ctl_bookmarks_tree.rbutton_up,context()
{
   // get the menu form
