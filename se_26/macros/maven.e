...
// Returns the project name for the external dependencies project.
_str extern_dependency_project_name()
{
   wsdir := _strip_filename(_workspace_filename, 'N');
   _maybe_append_filesep(wsdir);
   return wsdir :+ EXTERN_DEP_PROJ_NAME :+ PRJ_FILE_EXT;
}
...
void get_jar_dependencies(int handle, _str (&files)[])
{
   ...
   if (dephand >= 0) {
      _xmlcfg_find_simple_array(dephand, "/Project/Files//F[strieq(@Type,'"JAR_DEPENDENCY_FILE_TYPE"')]", deps);
...
// Adds a set of out of tree directories to the external dependencies project, 
// with the given (implied recursive) wildcard.  For now, only supports a single
// wildcard.
void add_dependency_directories(_str (&files)[], _str wildcard)
{
   dephand := extern_dependency_project_handle();

   if (dephand >= 0) {
      depfold := _xmlcfg_find_simple(dephand, "/Project/Files");
      if (depfold < 0) {
         message('Could not find /Project/Files');
         return;
      }
      ...
      foreach (auto file in files) {
         ...
         _xmlcfg_add_attribute(dephand, fn, 'N', file);
         _xmlcfg_add_attribute(dephand, fn, 'Type', JAR_DEPENDENCY_FILE_TYPE);
         _xmlcfg_add_attribute(dephand, fn, 'Recurse', '1');
         _xmlcfg_add_attribute(dephand, fn, 'D', '1');
      }
...
// Generic selection form for tagging of external dependencies.  Gradle and
// Maven use some hard to re-use wizards, so this is a generic version
// usable via the user_select_external_dependencies() function.
defeventtab _external_dependency_selection;
static SelectableDep gexdeps[];

void _external_dependency_selection.on_resize()
{
   hmargin := _ctldeps.p_x;
   vmargin := _ctl_instructions.p_y;

   _ctldeps.p_width = p_width - hmargin*2;
   _ctldeps.p_height = p_height - 2*vmargin - vmargin/2 - _ctl_instructions.p_height - _ctlok.p_height;
   _ctldeps.p_y = vmargin + vmargin/2 + _ctl_instructions.p_height;

   buttony := _ctldeps.p_y + _ctldeps.p_height + vmargin/2;
   _ctlok.p_y = buttony;
   _ctlcancel.p_y = buttony;
   _ctldeselectall.p_y = buttony;
   _ctlselectall.p_y = buttony;

   _ctlok.p_x = _ctldeps.p_x + _ctldeps.p_width - _ctlok.p_width;
   _ctlcancel.p_x = _ctlok.p_x - hmargin/2 - _ctlcancel.p_width;
   _ctldeselectall.p_x = _ctlcancel.p_x - hmargin/2 - _ctldeselectall.p_width;
   _ctlselectall.p_x = _ctldeselectall.p_x - hmargin/2 - _ctlselectall.p_width;
}

void _ctldeps.on_create()
{
   _str curdeps[];
   bool dephash:[];

   get_jar_dependencies(0, curdeps);
   foreach (auto ed in curdeps) {
      if (pos('*', ed) > 0) {
         // Strip off any wildcards from the end of the path.
         ed = _strip_filename(ed, 'N');
         ed = _strip_filename(ed, '/');
      }
      dephash:[ed] = true;
   }

   _ctldeps._TreeSetColButtonInfo(0,2000,TREE_BUTTON_PUSHBUTTON|TREE_BUTTON_SORT,0,"Dependency");
   _ctldeps._TreeSetColButtonInfo(1,2000,TREE_BUTTON_PUSHBUTTON|TREE_BUTTON_SORT,0,"Version");
   _ctldeps._TreeSetColButtonInfo(2,-1,TREE_BUTTON_PUSHBUTTON|TREE_BUTTON_SORT,0,"Summary");
   _ctldeps._TreeAdjustLastColButtonWidth();

   _ctldeps._TreeBeginUpdate(TREE_ROOT_INDEX);
   for (i := 0; i < gexdeps._length(); i++) {
      d := gexdeps[i];
      leaf := _ctldeps._TreeAddItem(TREE_ROOT_INDEX, d.dep_name"\t"d.ver"\t"d.desc, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
      _ctldeps._TreeSetUserInfo(leaf, i);
      _ctldeps._TreeSetCheckable(leaf, 1, 0, dephash._indexin(d.full_name) ? TCB_CHECKED : TCB_UNCHECKED);
   }
   _ctldeps._TreeEndUpdate(TREE_ROOT_INDEX);
   _ctldeps._TreeAdjustColumnWidths(-1);
}

void _ctlok.lbutton_up()
{
   // Update selected state in item array.
   int info;
   tindex := _ctldeps._TreeGetNextCheckedIndex(1, info);

   for (i := 0; i < gexdeps._length(); i++) {
      gexdeps[i].selected = false;
   }

   while (tindex >= 0) {
      dindex := _ctldeps._TreeGetUserInfo(tindex);
      gexdeps[dindex].selected = true;
      tindex = _ctldeps._TreeGetNextCheckedIndex(0, info);
   }
...
void _ctlselectall.lbutton_up()
{
   int i;

   for (i = 1; i <= _ctldeps._TreeGetNumChildren(TREE_ROOT_INDEX); i++) {
      _ctldeps._TreeSetCheckState(i, TCB_CHECKED);
   }
}

void _ctldeselectall.lbutton_up()
{
   int i;

   for (i = 1; i <= _ctldeps._TreeGetNumChildren(TREE_ROOT_INDEX); i++) {
      _ctldeps._TreeSetCheckState(i, TCB_UNCHECKED);
   }
}

// Displays the given dependencies, allowing the user to select
// what dependencies to tag.  Previous selections are remembered.
// If the user hit's ok, returns 0, and the deps[n].selected flag 
// will be true for any dependencies selected.  Returns < 0 on cancel.
int user_select_external_dependencies(SelectableDep (&deps)[])
{
   gexdeps = deps;
   rc := show('-modal _external_dependency_selection');
   if (rc == 0) {
      deps = gexdeps;
   }
   return rc;
