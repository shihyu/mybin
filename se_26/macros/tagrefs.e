...
void _tbtagrefs_form.on_resize()
{
   ...
   ctl_options_button.resizeToolButtonToLabel(ctllookinlabel.p_window_id);
   ...
   if (ctl_options_button.p_value) {
      ...
      ctl_filter_button.p_y = ctlshowall.p_y;
...
static int next_references_marker_type()
{
   ...
   if (pic_type <= 0) {
      if (gref_pic_types._length() < REFS_MAX_PIC_TYPES+1) {
         ...
         _MarkerTypeSetPriority(pic_type, VSMARKER_TYPE_PRIORITY_REFERENCES);
      } else {
...
void ctlrefname.on_create()
{
   ...
   ctl_options_button.resizeToolButtonToLabel(ctlsymlabel.p_window_id);
...
// Try to pass thru some keys to the list based on users emulation.
void ctlrefname.c_a-c_z()
{
   _list_pass_thru_ctrl_az();
}

// Have some keys operate on the list instead of the combo box
void ctlrefname.up()
{
   ctlreferences.call_event(ctlreferences,UP);
}

void ctlrefname.down()
{
   ctlreferences.call_event(ctlreferences,DOWN);
}

void ctlrefname.pgup()
{
   ctlreferences.call_event(ctlreferences,PGUP);
}

void ctlrefname.pgdn()
{
   ctlreferences.call_event(ctlreferences,PGDN);
}
...
void ctlreferences.rbutton_up,context()
{
...
void _RestoreReferencesInFile(ReferenceSaveInfo (&referenceSaves)[], int adjustLinesBy=0)
{
   ...
   foreach (auto r in referenceSaves) {
      ...
      nearest_col := 0;
      status := search(_escape_re_chars(r.ref_name):+"|$", 'rh@');
      while (!status) {
