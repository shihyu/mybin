...
int RetagCurrentFile(bool useThread=false, bool useFileOnDisk=false, _str tagDatabase=null)
{
   ...
   */
   ...
   // check for compressed .tar.XX
   if (fext == 'z' || fext == 'gz' || fext == 'xz' || fext == 'bz2') {
      tarext := lowcase(_get_extension(_strip_filename(p_buf_name, 'pe')));
      if (tarext == 'tar') {
         switch (fext) {
         case 'z':   fext = 'taz'; break;
         case 'gz':  fext = 'tgz'; break;
         case 'xz':  fext = 'txz'; break;
         case 'bz2': fext = 'tbz'; break;
         }
      }
   }

   // look up the corresponding load-tags function
