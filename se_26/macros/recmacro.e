...
static int check_macroid(_str name)
{
   ...
   index:=find_index(name,OBJECT_TYPE|EVENTTAB_TYPE|PROC_TYPE|COMMAND_TYPE);
...
_command gui_save_macro() name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   if (!result && name!='' && do_bind_to_key!='') {
      // There may be a relink pending for vusrmacs.e
      // Make sure it gets reloaded before listing commands.
      _post_call(gui_bind_to_key,name);
