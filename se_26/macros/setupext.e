...
static const USING_DEFAULT_OPTION=     "Default: ";
   ...
   while (extOptions != '') {
      ...
      if (_first_char(word)=='-') {
         ...
         case '6':    
            ...
            break;
         case 'autoclose':    
            tabNumber = 'Auto-Close'; 
            break;
         case 'autosurround':    
            tabNumber = 'Auto-Surround'; 
            break;
         case 'tagging':         
         case '7':         
         ...
         case 'fileoptions':         
            ...
            break;
         case 'projects':         
            tabNumber = 'Single File Projects';
            break;
         case 'interactiveprofiles':         
            tabNumber = 'Interactive Profiles';
            break;
         case 'errors':         
            tabNumber = 'Live Error Profiles';
            break;
         default:
            tabNumber = 'General';
            break; 
...
int _OnUpdate_setupext(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   extOptions := "";
   parse command with command extOptions;
   if (_first_char(extOptions) == '-') {
      extOptions = substr(extOptions, 2);
      if (!isinteger(extOptions)) {
         extOptions = ' ' :+ _Capitalize(extOptions);
      }
   }
   ...
   int status = _menu_set_state(cmdui.menu_handle,
                                ...
                                MF_ENABLED,
                                "p",
                                modeName :+ extOptions :+ " &Options...\t"keys);
...
static void get_all_mode_names_mark_installed()
{
   ...
   for (i := 0; i < langs._length(); i++) {
     ...
     if (_IsInstalledLanguage(name)) {
        ...
        _lbadd_item(_LangGetModeName(name), 100, _pic_lbuser);
     }
   }
}
 ...
 * {@link def_codehelp_flags} as the default.
 * 
 ...
 * for a language, then use {@link def_codehelp_flags} as the default.
 * 
...
void _build_update_table_from_list(_str updateList, typeless (&updateTable):[])
{
   ...
   for (;;) {
      ...
      case FIXED_WIDTH_RIGHT_MARGIN_SHORT_KEY:
         ...
         break;
      case COLOR_PROFILE_SHORT_KEY:
         key = COLOR_PROFILE_UPDATE_KEY;
         break;
      case ALT_COLOR_PROFILE_SHORT_KEY:
         key = ALT_COLOR_PROFILE_UPDATE_KEY;
         break;
      case MINIMAP_COLOR_PROFILE_SHORT_KEY:
         key = MINIMAP_COLOR_PROFILE_UPDATE_KEY;
         break;
      case ALT_MINIMAP_COLOR_PROFILE_SHORT_KEY:
         key = ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY;
         break;
      case ALT_COLOR_FILESPEC_SHORT_KEY:
         key = ALT_COLOR_FILESPEC_UPDATE_KEY;
         break;
      case UNICODE_FONT_INFO_SHORT_KEY:
         key = UNICODE_FONT_INFO_UPDATE_KEY;
         break;
      case UNICODE_DIFF_FONT_INFO_SHORT_KEY:
         key = UNICODE_DIFF_FONT_INFO_UPDATE_KEY;
         break;
      case UNICODE_MINIMAP_FONT_INFO_SHORT_KEY:
         key = UNICODE_MINIMAP_FONT_INFO_UPDATE_KEY;
         break;
      }
...
void _get_update_table_for_all_buffer_settings(_str langId,_str (&updateTable):[]) {
   ...
   updateTable:[COLOR_PROFILE_UPDATE_KEY]=LanguageSettings.getColorProfile(langId);
   updateTable:[ALT_COLOR_PROFILE_UPDATE_KEY]=LanguageSettings.getAltColorProfile(langId);
   updateTable:[MINIMAP_COLOR_PROFILE_UPDATE_KEY]=LanguageSettings.getMinimapColorProfile(langId);
   updateTable:[ALT_COLOR_FILESPEC_UPDATE_KEY]=LanguageSettings.getAltColorFileSpec(langId);
   updateTable:[ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY]=LanguageSettings.getAltMinimapColorProfile(langId);
   updateTable:[UNICODE_FONT_INFO_UPDATE_KEY]=LanguageSettings.getFontInfo(langId,CFG_UNICODE_SOURCE_WINDOW);
   updateTable:[UNICODE_DIFF_FONT_INFO_UPDATE_KEY]=LanguageSettings.getFontInfo(langId,CFG_UNICODE_DIFF_EDITOR_WINDOW);
   updateTable:[UNICODE_MINIMAP_FONT_INFO_UPDATE_KEY]=LanguageSettings.getFontInfo(langId,CFG_UNICODE_MINIMAP_WINDOW);
}
...
static void _update_cur_buffer(_str lang,typeless (&arg_updateTable):[],bool update_ad_form_now, bool update_ad_form_flags,int &progressWid,int &displayProgressCount,bool allow_ad_progress_dialog=true,bool apply_overrides=false) {
   ...
   font_key := UNICODE_FONT_INFO_UPDATE_KEY;
   if (p_IsMinimap) {
      cfg = CFG_UNICODE_MINIMAP_WINDOW;
      font_key = UNICODE_MINIMAP_FONT_INFO_UPDATE_KEY;
   }
   if (updateTable._indexin(font_key)) {
      font_info := updateTable:[font_key];
      font_name := "";
      typeless font_size='';
      typeless font_flags='';
      typeless charset='';
      ...
      if (charset=='') charset=VSCHARSET_DEFAULT;
      p_font_charset=charset;
      if (font_flags=='') font_flags=0;
      _font_flags2props(font_flags);
      p_redraw=true;
   }

   if (updateTable._indexin(COLOR_PROFILE_UPDATE_KEY) || 
       updateTable._indexin(ALT_COLOR_PROFILE_UPDATE_KEY) || 
       updateTable._indexin(MINIMAP_COLOR_PROFILE_UPDATE_KEY) || 
       updateTable._indexin(ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY) || 
       updateTable._indexin(ALT_COLOR_FILESPEC_UPDATE_KEY)) {
      lang_color_profile := "";
      minimap_color_profile := "";
      if (updateTable._indexin(COLOR_PROFILE_UPDATE_KEY)) {
         lang_color_profile = updateTable:[COLOR_PROFILE_UPDATE_KEY];
      }
      if (updateTable._indexin(MINIMAP_COLOR_PROFILE_UPDATE_KEY)) {
         minimap_color_profile = updateTable:[MINIMAP_COLOR_PROFILE_UPDATE_KEY];
      }
      if (updateTable._indexin(ALT_COLOR_FILESPEC_UPDATE_KEY)) {
         alt_filespec := updateTable:[ALT_COLOR_FILESPEC_UPDATE_KEY];
         if (alt_filespec != "" && _IsFileMatchedExtension(_strip_filename(p_buf_name, 'p'), alt_filespec)) {
            if (updateTable._indexin(ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY)) {
               alt_color_profile := updateTable:[ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY];
               if (alt_color_profile != "") minimap_color_profile = alt_color_profile;
            }
            if (updateTable._indexin(ALT_COLOR_PROFILE_UPDATE_KEY)) {
               alt_color_profile := updateTable:[ALT_COLOR_PROFILE_UPDATE_KEY];
               if (alt_color_profile != "") lang_color_profile = alt_color_profile;
            }
         }
      }
      if (lang_color_profile != "") {
...
void _update_buffer_from_new_setting(_str (updateTable):[],_str langId='') {
   ...
   for (;;) {
      ...
      if (p_LangId!='' && _LangIsDefined(lang) && (langId=='' || lang==langId)) {
...
void _update_buffers_from_table(_str lang, typeless (&updateTable):[] = null, bool resetAll=false, bool getSettingsTable=false, typeless (&settingsTable):[]:[]=null)
{
   ...
   for (;;) {
      ...
      if (p_LangId!='' && found_match && !(resetAll && getSettingsTable && settingsTable._indexin(p_LangId))) {
         if (resetAll) {
            updateTable._makeempty();
...
static _str get_update_key(_str key)
{
   ...
   case HEX_MODE_SHORT_KEY:
      ...
      break;
   case COLOR_PROFILE_SHORT_KEY:
      unifiedKey = COLOR_PROFILE_UPDATE_KEY;
      break;
   case ALT_COLOR_PROFILE_SHORT_KEY:
      unifiedKey = ALT_COLOR_PROFILE_UPDATE_KEY;
      break;
   case MINIMAP_COLOR_PROFILE_SHORT_KEY:
      unifiedKey = MINIMAP_COLOR_PROFILE_UPDATE_KEY;
      break;
   case ALT_MINIMAP_COLOR_PROFILE_SHORT_KEY:
      unifiedKey = ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY;
      break;
   case ALT_COLOR_FILESPEC_SHORT_KEY:
      unifiedKey = ALT_COLOR_FILESPEC_UPDATE_KEY;
      break;
   case UNICODE_FONT_INFO_SHORT_KEY:
      unifiedKey = UNICODE_FONT_INFO_UPDATE_KEY;
      break;
   case UNICODE_DIFF_FONT_INFO_SHORT_KEY:
      unifiedKey = UNICODE_DIFF_FONT_INFO_UPDATE_KEY;
      break;
   case UNICODE_MINIMAP_FONT_INFO_SHORT_KEY:
      unifiedKey = UNICODE_MINIMAP_FONT_INFO_UPDATE_KEY;
      break;
   }
...
static typeless validate_update_data(_str key, typeless value)
{
   ...
   //case SHOW_TABS_UPDATE_KEY: /* same as special chars key */ 
...
bool _copy_language_view_settings(_str srcLang, _str destLang)
{
   ...
   if (def_symbol_color_profile != "") {
      ...
      SymbolColorAnalyzer.initAllSymbolAnalyzers(true);
   }
...
static bool indent_defines_blocks(_str langID) {
   return langID == 'py';
}

static bool _is_tab_aligns_multiple_cursor(_str langID) {
   // check if Tab key is assigned to 'smarttab' command
   ...
   if (keytab != '') {
      ...
      if (index > 0) {
         ...
         index = _FindLanguageCallbackIndex("_%s_is_tab_aligns_multiple_cursors_supported", langID);
         if (!index || !index_callable(index)) {
            return false;
         }
         return call_index(index);
   ...
   // not using the smarttab command for Tab
   return false;
}
...
void _language_editing_form_init_for_options(_str langID)
{
   ...
   if (langID == ALL_LANGUAGES_ID) {
      ...
      ctllabel16.p_visible=false;
      ctltabstyle.p_visible=false;
      ctllabel18.p_visible=false;
      ctlIndentStyle.p_visible=false;

      y_adjust := (ctltabcycle.p_next.p_y - ctllabel18.p_y);
      ctltabmulticursor.p_y -= y_adjust;
      ...
      if (!ctltabcycle.p_visible) {
         y_adjust := (ctltabcycle.p_next.p_y - ctltabcycle.p_y);
         ctltabmulticursor.p_y -= y_adjust;
         ctlInsertRealIndent.p_y -= y_adjust;
         ctlBackspaceUnindent.p_y -= y_adjust;
         _smartp.p_y -= y_adjust;
   ...
   if (!ctlBoundsFrame.p_visible) {
      ...
      ctlBeautifyFrame.p_y = ctlBoundsFrame.p_y;
   }
...
_str _language_editing_form_get_value(_str controlName, _str langId)
{
   ...
   case 'ctltabcycle':
      ...
      break;
   case 'ctltabmulticursor':
      value = LanguageSettings.getTabAlignsCursors(langId) ? '1' : '0';
      break;
   case 'ctlspellwt':
      value= _LangGetPropertyInt32(langId, VSLANGPROPNAME_SPELL_CHECK_WHILE_TYPING, 0);
      if (value) {
...
bool _language_editing_form_is_lang_included(_str controlName, _str langId, bool allLangsExclusion)
{
   ...
   case 'ctltabmulticursor':
      included = (langId == ALL_LANGUAGES_ID || _is_tab_aligns_multiple_cursor(langId));
      break;
...
_str _language_editing_form_apply_control(_str controlName, _str langId, _str value)
{
   ...
   case 'ctltabmulticursor':
      updateKey = '';
      updateValue = '';
      if (_is_tab_aligns_multiple_cursor(langId) || langId==ALL_LANGUAGES_ID) {
         LanguageSettings.setTabAlignsCursors(langId, value != '0');
      }
      break;
...
void ctltabreindent.on_change() 
{
   if (p_text==TAB_REINDENT_IN_LEADING_BLANKS ||
      ...
      ctltabmulticursor.p_enabled = true;
      _ctl_beautify_line.p_enabled = true;
   } else if (p_text==TAB_REINDENT_ALWAYS) {
      _ctl_beautify_line.p_enabled = true;
      ctltabmulticursor.p_enabled = false;
      ctltabmulticursor.p_value = 0;
   } else {
      ctltabmulticursor.p_enabled = true;
      _ctl_beautify_line.p_enabled = false;
      _ctl_beautify_line.p_value = 0;
   ...
   // multicursor needs to be disabled if we are not using "smarttab" command
   langId := _get_language_form_lang_id();
   if (langId != ALL_LANGUAGES_ID && !_is_tab_aligns_multiple_cursor(langId)) {
      ctltabmulticursor.p_enabled = false;
      ctltabmulticursor.p_value = 0;
   }
}
...
void _language_auto_complete_form_init_for_options(_str langID)
{
   ...
   sizeBrowseButtonToTextBox(ctl_auto_complete_history.p_window_id, ctl_edit_completion_history.p_window_id);
...
_str _language_auto_complete_form_get_value(_str controlName, _str langId)
{
   ...
   case 'ctl_auto_complete_expand_alias':
      ...
      break;
   case 'ctl_auto_complete_history':
      value = (flags & AUTO_COMPLETE_HISTORY) ? 1 : 0;
      break;
   ...
   case 'ctl_auto_complete_show_categories':
      ...
      break;
   case 'ctl_auto_complete_show_history':
      value = (flags & AUTO_COMPLETE_SHOW_HISTORY) ? 1 : 0;
      break;
...
void _language_auto_complete_form_apply_control(_str controlName, _str langId, _str value)
{
   ...
   case 'ctl_auto_complete_show_categories':
      ...
      break;
   case 'ctl_auto_complete_show_history':
      flag = AUTO_COMPLETE_SHOW_HISTORY;
      break;
   case 'ctl_auto_complete_show_prototypes':
...
void ctl_auto_complete_frame.lbutton_up()
{
   ...
   ctl_auto_complete_history.p_enabled=enabled;
   ...
   ctl_auto_complete_show_history.p_enabled=enabled && (ctl_auto_complete_show_list.p_value? true:false);
   ...
   if (ctl_auto_complete_show_categories.p_value) {
      ctl_auto_complete_show_history.p_value = 1;
      ctl_auto_complete_show_history.p_enabled = false;
   }
...
void ctl_auto_complete_show_list.lbutton_up() {
   ...
   ctl_auto_complete_show_history.p_enabled = p_value? true:false;
...
void ctl_auto_complete_show_categories.lbutton_up() {
   ...
   if (ctl_auto_complete_show_categories.p_value) {
      ...
      ctl_auto_complete_show_history.p_enabled = true;
   }
}
/**
 * Check consistency, then toggle picture in auto complete illustration.
...
void ctl_edit_completion_history.lbutton_up() {
   langId := _get_language_form_lang_id();
   AutoCompleteEditHistory(langId);
}
...
void _language_view_form_init_for_options(_str langID)
{
   ...
   if (langID == ALL_LANGUAGES_ID) {
      _modified_lines_link.p_visible = false;
      _current_line_link.p_visible = false;
      _ctl_config_special_chars.p_visible = false;
...
#region Options Dialog Helper Functions (Language > Appearance)

defeventtab _language_appearance_form;

static _str APPEARANCE_OPTIONS_UNICODE_EDITOR_FONT_MODIFIED(...) 
{
   if (arg()) ctl_unicode_editor_label.p_user=arg(1);
   return ctl_unicode_editor_label.p_user;
}
static _str APPEARANCE_OPTIONS_UNICODE_MINIMAP_FONT_MODIFIED(...) 
{
   if (arg()) ctl_unicode_minimap_label.p_user=arg(1);
   return ctl_unicode_minimap_label.p_user;
}
static _str APPEARANCE_OPTIONS_UNICODE_DIFF_FONT_MODIFIED(...) 
{
   if (arg()) ctl_unicode_diff_label.p_user=arg(1);
   return ctl_unicode_diff_label.p_user;
}
static _str APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(...) 
{
   if (arg()) ctl_scheme_name.p_user=arg(1);
   return ctl_scheme_name.p_user;
}

static _str APPEARANCE_OPTIONS_SYMBOL_COLORING_MODIFIED(...) 
{
   if (arg()) ctl_symbol_coloring.p_user=arg(1);
   return ctl_symbol_coloring.p_user;
}

static bool APPEARANCE_OPTIONS_IGNORE_FONT_CHANGE(...) 
{
   if (arg()) ctl_fonts.p_user=arg(1);
   return ctl_fonts.p_user;
}

static void langUpdateFontNameList(int cfg, int altcfg, bool fixed_fonts_only=true)
{
   opt := fixed_fonts_only? 'F' : 'S';

   orig_font_name := p_text;
   _lbclear();
   _lbadd_item("");
   _insert_font_list(opt);

   default_font_info := _default_font(cfg);
   parse default_font_info with auto default_font_name ',' .;
   if (altcfg != CFG_NULL && default_font_name == "") {
      alt_font_info := _default_font(altcfg);
      parse alt_font_info with default_font_name ',' .;
   }

   if (fontHasLimitedSizes(default_font_name)) {
      real_font_name := p_window_id._real_font_name(default_font_name);
      default_font_name = default_font_name ' (' real_font_name ')';
   }

   p_style = PSCBO_EDIT;
   p_user = USING_DEFAULT_OPTION:+default_font_name;
   _ComboBoxSetPlaceHolderText(p_window_id, USING_DEFAULT_OPTION:+default_font_name);

   _cbset_text(orig_font_name);
}

static void langUpdateFontSizeList(int cfg, int altcfg, _str font_name="")
{
   orig_font_size := p_text;
   _lbclear();
   _lbadd_item("");

   if (cfg == CFG_UNICODE_MINIMAP_WINDOW || cfg == CFG_SBCS_DBCS_MINIMAP_WINDOW) {
   ...
   _cbset_text(orig_font_size);

   default_font_info := _default_font(cfg);
   parse default_font_info with . ',' auto default_font_size ',' .;
   if (altcfg != CFG_NULL && default_font_size == "") {
      alt_font_info := _default_font(altcfg);
      parse alt_font_info with default_font_size ',' .;
   }

   p_style = PSCBO_EDIT;
   p_user = USING_DEFAULT_OPTION:+default_font_size;
   _ComboBoxSetPlaceHolderText(p_window_id, USING_DEFAULT_OPTION:+default_font_size);
}

void langUpdateColorProfileList(_str default_profile_name, bool includeAutomatic=true)
{
   se.color.DefaultColorsConfig dcc;
   dcc.loadFromDefaultColors();

   _lbclear();
   _lbaddColorProfileNames(dcc, includeAutomatic);
   _lbadd_item("");
   _lbsort('i');

   p_style = PSCBO_EDIT;
   p_user = USING_DEFAULT_OPTION:+default_profile_name;
   _ComboBoxSetPlaceHolderText(p_window_id, USING_DEFAULT_OPTION:+default_profile_name);
}

void _language_appearance_form_init_for_options(_str langID)
{
   // make all the edit/noedit combo boxes have the same height
   noedit_combo_height := ctl_strict_symbols.p_height;
   ctl_unicode_editor_font_name_list.p_auto_size = false;
   ctl_unicode_minimap_font_name_list.p_auto_size = false;
   ctl_unicode_diff_font_name_list.p_auto_size = false;
   ctl_unicode_editor_font_size_list.p_auto_size = false;
   ctl_unicode_minimap_font_size_list.p_auto_size = false;
   ctl_unicode_diff_font_size_list.p_auto_size = false;
   ctl_unicode_editor_font_name_list.p_height = noedit_combo_height;
   ctl_unicode_minimap_font_name_list.p_height = noedit_combo_height;
   ctl_unicode_diff_font_name_list.p_height = noedit_combo_height;
   ctl_unicode_editor_font_size_list.p_height = noedit_combo_height;
   ctl_unicode_minimap_font_size_list.p_height = noedit_combo_height;
   ctl_unicode_diff_font_size_list.p_height = noedit_combo_height;
   ctl_scheme_name.p_auto_size = false;
   ctl_alt_scheme_name.p_auto_size = false;
   ctl_minimap_scheme_name.p_auto_size = false;
   ctl_alt_minimap_scheme_name.p_auto_size = false;
   ctl_scheme_name.p_height = noedit_combo_height;
   ctl_alt_scheme_name.p_height = noedit_combo_height;
   ctl_minimap_scheme_name.p_height = noedit_combo_height;
   ctl_alt_minimap_scheme_name.p_height = noedit_combo_height;

   // adjust position of "Fixed Fonts only button"
   ctl_fixed_fonts.p_x = ctl_font_name_label.p_x_extent + ctl_font_size_label.p_width;
   ctl_font_size_label.p_x = max(ctl_font_size_label.p_x, ctl_fixed_fonts.p_x_extent + ctl_font_size_label.p_width);

  // size font picker buttons to fit
   rightMargin := ctl_alt_files.p_x_extent;
   sizeBrowseButtonToTextBox(ctl_unicode_editor_font_size_list.p_window_id, ctl_unicode_editor_font_picker.p_window_id, 0, rightMargin);
   sizeBrowseButtonToTextBox(ctl_unicode_minimap_font_size_list.p_window_id, ctl_unicode_minimap_font_picker.p_window_id, 0, rightMargin); 
   sizeBrowseButtonToTextBox(ctl_unicode_diff_font_size_list.p_window_id, ctl_unicode_diff_font_picker.p_window_id, 0, rightMargin); 

   // size color profile picker buttons to fit
   sizeBrowseButtonToTextBox(ctl_scheme_name.p_window_id, ctl_scheme_picker.p_window_id, 0, ctl_minimap_scheme_name.p_x-60);
   sizeBrowseButtonToTextBox(ctl_alt_scheme_name.p_window_id, ctl_alt_scheme_picker.p_window_id, 0, ctl_minimap_scheme_name.p_x-60); 
   sizeBrowseButtonToTextBox(ctl_minimap_scheme_name.p_window_id, ctl_minimap_scheme_picker.p_window_id, 0, rightMargin); 
   sizeBrowseButtonToTextBox(ctl_alt_minimap_scheme_name.p_window_id, ctl_alt_minimap_scheme_picker.p_window_id, 0, rightMargin); 

   // load all font names and sizes
   APPEARANCE_OPTIONS_IGNORE_FONT_CHANGE(true);
   ctl_unicode_editor_font_name_list.  langUpdateFontNameList(CFG_UNICODE_SOURCE_WINDOW,      CFG_NULL);
   ctl_unicode_minimap_font_name_list. langUpdateFontNameList(CFG_UNICODE_MINIMAP_WINDOW,     CFG_UNICODE_SOURCE_WINDOW);
   ctl_unicode_diff_font_name_list.    langUpdateFontNameList(CFG_UNICODE_DIFF_EDITOR_WINDOW, CFG_UNICODE_SOURCE_WINDOW);

   ctl_unicode_editor_font_size_list.  langUpdateFontSizeList(CFG_UNICODE_SOURCE_WINDOW,      CFG_NULL);
   ctl_unicode_minimap_font_size_list. langUpdateFontSizeList(CFG_UNICODE_MINIMAP_WINDOW,     CFG_UNICODE_SOURCE_WINDOW);
   ctl_unicode_diff_font_size_list.    langUpdateFontSizeList(CFG_UNICODE_DIFF_EDITOR_WINDOW, CFG_UNICODE_SOURCE_WINDOW);

   // load all the profile names into the combo boxes, 
   // putting user defined profiles at the top of the list.
   minimap_color_scheme := def_minimap_color_scheme;
   if (minimap_color_scheme == "") {
      minimap_color_scheme = def_color_scheme;
   }
   ctl_scheme_name.langUpdateColorProfileList(def_color_scheme);
   ctl_alt_scheme_name.langUpdateColorProfileList(def_color_scheme);
   ctl_minimap_scheme_name.langUpdateColorProfileList(minimap_color_scheme);
   ctl_alt_minimap_scheme_name.langUpdateColorProfileList(minimap_color_scheme);

   // set up the symbol coloring options
   if (_haveContextTagging()) {
   ...
   _language_form_init_for_options(langID, _language_appearance_form_get_value, _language_appearance_form_is_lang_included);
   ...
   APPEARANCE_OPTIONS_IGNORE_FONT_CHANGE(false);
}


bool _language_appearance_form_is_lang_included(_str controlName, _str langId, bool allLangsExclusion)
{
   included := true;

   switch (controlName) {
   case 'ctl_symbol_errors':
   case 'ctl_symbol_override':
   ...
   case 'ctl_positional_keywords':
      included = (langId==ALL_LANGUAGES_ID || _are_positional_keywords_supported(langId));
      break;
   }
...
static bool langValidatedFontSize(_str font_size)
{
   if (font_size == "") return true;
   if (font_size == LANGS_DONT_MATCH_CB) return true;
   return isuinteger(font_size) && font_size != 0 && ((int)font_size <= 128);
}

bool _language_appearance_form_validate(int action)
{
   langID := _get_language_form_lang_id();

   if (langID == ALL_LANGUAGES_ID || action == OPTIONS_APPLYING) {
      if (!langValidatedFontSize(ctl_unicode_editor_font_size_list.p_text)) {
         ctl_unicode_editor_font_size_list._text_box_error("Font size must be a positive integer <= 128.");
         return false;
      }
      if (!langValidatedFontSize(ctl_unicode_minimap_font_size_list.p_text)) {
         ctl_unicode_minimap_font_size_list._text_box_error("Font size must be a positive integer <= 128.");
         return false;
      }
      if (!langValidatedFontSize(ctl_unicode_diff_font_size_list.p_text)) {
         ctl_unicode_diff_font_size_list._text_box_error("Font size must be a positive integer <= 128.");
         return false;
      }
   }
...
void _language_appearance_form_restore_state()
{
   _language_form_restore_state(_language_appearance_form_get_value, 
                                _language_appearance_form_is_lang_included);
}

/**
 * Changes the color scheme of all MDI edit windows.
 * 
 * @param langId                 language ID to update editor controls for
 * @param scheme_name            Color scheme name
 * @param minimap_scheme_name    Color scheme name for minimap controls (use scheme_name if empty)
 * @param alt_scheme_name        Alternate color scheme for files matching file spec (use scheme_name if empty)
 * @param alt_filespec           File specification for file extensions to use alternate scheme name for
 * @param update_editor_colors   Update editor windows
 * @param update_minimap_colors  Update minimap windows
 */
static void _change_all_window_colors_for_language(_str langId,
                                                   _str profile_name, 
                                                   _str minimap_profile_name, 
                                                   _str alt_profile_name, 
                                                   _str alt_minimap_profile_name, 
                                                   _str alt_filespec,
                                                   bool update_editor_colors, 
                                                   bool update_minimap_colors)
{
   if (profile_name == "") {
      profile_name = def_color_scheme;
   }
   if (alt_profile_name == "") {
      alt_profile_name = def_color_scheme;
   }
   if (minimap_profile_name == "") {
      minimap_profile_name = def_minimap_color_scheme;
   }
   if (alt_minimap_profile_name == "") {
      alt_minimap_profile_name = def_minimap_color_scheme;
   }
   ...
   for (i:=1; i<=last; ++i) {
      if (_iswindow_valid(i) && i._isEditorCtl(false)) {
         ...
         if (i.p_LangId != langId && langId != ALL_LANGUAGES_ID) {
            continue;
         }
         buf_profile_name := profile_name;
         map_profile_name := minimap_profile_name;
         if (alt_filespec != "" && (alt_profile_name != "" || alt_minimap_profile_name != "")) {
            if (_IsFileMatchedExtension(_strip_filename(i.p_buf_name, 'p'), alt_filespec)) {
               buf_profile_name = alt_profile_name;
               map_profile_name = minimap_profile_name;
            }
         }
         if (i.p_IsMinimap) {
            //say("_change_all_window_colors_for_language H"__LINE__": minmap, i.p_buf_name="i.p_buf_name);
            i.p_redraw=false;
            i.p_MinimapColorProfile = map_profile_name;
            i.p_redraw=true;
         } else {
            //say("_change_all_window_colors_for_language H"__LINE__": editor, i.p_buf_name="i.p_buf_name);
            i.p_redraw=false;
            i.p_BufferColorProfile  = buf_profile_name;
            i.p_redraw=true;
         }
      }
   }
...
void _reset_language_fonts_and_colors()
{
...
static void _change_all_window_fonts_for_language(_str langId, int cfg, int altcfg=CFG_NULL)
{
   default_font_info := _default_font(cfg);
   parse default_font_info with auto default_font_name ',' auto default_font_size ',' auto font_flags ',' auto font_charset ',' .;
   if (altcfg != CFG_NULL && default_font_name == "") {
      alt_font_info := _default_font(altcfg);
      parse alt_font_info with default_font_name ',' auto alt_font_size ',' auto alt_font_flags ',' auto alt_font_charset ',' .;
      if (default_font_size == "") default_font_size = alt_font_size;
      if (font_flags == "") font_flags = alt_font_flags;
      if (font_charset == "") font_charset = alt_font_charset;
   }

   font_info := LanguageSettings.getFontInfo(langId, cfg);
   parse font_info with auto font_name ',' auto font_size ',' auto lang_font_flags ',' auto lang_font_charset.;
   ...
   if (isinteger(lang_font_flags) && lang_font_flags != 0) font_flags = lang_font_flags;
   if (isinteger(lang_font_charset) && lang_font_charset != VSCHARSET_DEFAULT) font_charset = lang_font_charset;

   if (font_flags == "") font_flags = 0;
   if (font_charset == "") font_charset = VSCHARSET_DEFAULT;

   // try to override hard-coded font size limits
   if (fontHasLimitedSizes(font_name) && !_mdi._is_valid_font_size(font_name, font_size)) {
      font_name = _mdi._real_font_name(font_name);
   }

   _change_all_wfonts(font_name, font_size, (int)font_flags, (int)font_charset, cfg, langId);
}

bool _language_appearance_form_apply()
{
   langId := _get_language_form_lang_id();

   APPEARANCE_OPTIONS_UNICODE_EDITOR_FONT_MODIFIED(false);
   APPEARANCE_OPTIONS_UNICODE_MINIMAP_FONT_MODIFIED(false);
   APPEARANCE_OPTIONS_UNICODE_DIFF_FONT_MODIFIED(false);
   APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(false);
   APPEARANCE_OPTIONS_SYMBOL_COLORING_MODIFIED(false);

   _language_form_apply(_language_appearance_form_apply_control);

   if (APPEARANCE_OPTIONS_UNICODE_EDITOR_FONT_MODIFIED()) {
      _change_all_window_fonts_for_language(langId, CFG_UNICODE_SOURCE_WINDOW);
      _change_all_window_fonts_for_language(langId, CFG_SBCS_DBCS_SOURCE_WINDOW);
   }
   if (APPEARANCE_OPTIONS_UNICODE_MINIMAP_FONT_MODIFIED()) {
      _change_all_window_fonts_for_language(langId, CFG_UNICODE_MINIMAP_WINDOW, CFG_UNICODE_SOURCE_WINDOW);
      _change_all_window_fonts_for_language(langId, CFG_SBCS_DBCS_MINIMAP_WINDOW, CFG_SBCS_DBCS_SOURCE_WINDOW);
   }
   if (APPEARANCE_OPTIONS_UNICODE_DIFF_FONT_MODIFIED()) {
      _change_all_window_fonts_for_language(langId, CFG_UNICODE_DIFF_EDITOR_WINDOW, CFG_UNICODE_SOURCE_WINDOW);
      _change_all_window_fonts_for_language(langId, CFG_DIFF_EDITOR_WINDOW, CFG_SBCS_DBCS_SOURCE_WINDOW);
   }

   if (APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED()) {
      _change_all_window_colors_for_language(langId,
                                             se.color.ColorScheme.removeProfilePrefix(ctl_scheme_name.p_text),
                                             se.color.ColorScheme.removeProfilePrefix(ctl_minimap_scheme_name.p_text),
                                             se.color.ColorScheme.removeProfilePrefix(ctl_alt_scheme_name.p_text),
                                             se.color.ColorScheme.removeProfilePrefix(ctl_alt_minimap_scheme_name.p_text),
                                             ctl_alt_files.p_text,
                                             update_editor_colors:true, 
                                             update_minimap_colors:true);
      APPEARANCE_OPTIONS_SYMBOL_COLORING_MODIFIED(true);
   }

   if (APPEARANCE_OPTIONS_SYMBOL_COLORING_MODIFIED()) {
      // Reinitialize symbol analyzers
      SymbolColorAnalyzer.initAllSymbolAnalyzers(true);
   }
...
_str _language_appearance_form_apply_control(_str controlName, _str langId, _str value)
{
   updateKey := '';
   updateValue := '';
   ...
   switch (controlName) {
   // Color Profile
   case "ctl_scheme_name":
      updateKey = COLOR_PROFILE_UPDATE_KEY;
      oldValueString = LanguageSettings.getColorProfile(langId);
      newValueString = se.color.ColorScheme.removeProfilePrefix(value);
      if (newValueString != oldValueString) {
         APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(true);
         LanguageSettings.setColorProfile(langId, newValueString);
         updateValue = newValueString;
      }
      break;
   case "ctl_minimap_scheme_name":
      updateKey = MINIMAP_COLOR_PROFILE_UPDATE_KEY;
      oldValueString = LanguageSettings.getMinimapColorProfile(langId);
      newValueString = se.color.ColorScheme.removeProfilePrefix(value);
      if (oldValueString == "") oldValueString = def_minimap_color_scheme;
      if (newValueString != oldValueString) {
         APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(true);
         LanguageSettings.setMinimapColorProfile(langId, newValueString);
         updateValue = newValueString;
      }
      break;
   case "ctl_alt_scheme_name":
      updateKey = ALT_COLOR_PROFILE_UPDATE_KEY;
      oldValueString = LanguageSettings.getAltColorProfile(langId);
      newValueString = se.color.ColorScheme.removeProfilePrefix(value);
      if (newValueString != oldValueString) {
         APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(true);
         LanguageSettings.setAltColorProfile(langId, newValueString);
         updateValue = newValueString;
      }
      break;
   case "ctl_alt_minimap_scheme_name":
      updateKey = ALT_MINIMAP_COLOR_PROFILE_UPDATE_KEY;
      oldValueString = LanguageSettings.getAltMinimapColorProfile(langId);
      newValueString = se.color.ColorScheme.removeProfilePrefix(value);
      if (oldValueString == "") oldValueString = def_minimap_color_scheme;
      if (newValueString != oldValueString) {
         APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(true);
         LanguageSettings.setAltMinimapColorProfile(langId, newValueString);
         updateValue = newValueString;
      }
      break;
   case "ctl_alt_files":
      updateKey = ALT_COLOR_FILESPEC_UPDATE_KEY;
      oldValueString = LanguageSettings.getAltColorFileSpec(langId);
      if (value != oldValueString) {
         APPEARANCE_OPTIONS_COLOR_PROFILE_MODIFIED(true);
         LanguageSettings.setAltColorFileSpec(langId, value);
         updateValue = value;
      }
      break;
   // Symbol Coloring
   ...
   case 'ctl_symbol_override':
      ...
      if ((int)value) newValue = oldValue & ~SYMBOL_COLOR_NO_OVERRIDE_LIBRARY_SYMBOLS;
      else newValue = oldValue | SYMBOL_COLOR_NO_OVERRIDE_LIBRARY_SYMBOLS;
   ...
   // Fonts
   default:
      // fonts are a bit more complicated, we need to get the default so that
      // we can so the right font name, even if we are not overriding the default.
      cfg := langGetCFGConstantsForFontControl(controlName, auto altcfg, updateKey);
      if (cfg != CFG_NULL) {
         default_font_info := _default_font(cfg);
         parse default_font_info with auto default_font_name ',' auto default_font_size ',' auto default_font_opts;
         if (altcfg != CFG_NULL && (default_font_name == "" || default_font_size == "")) {
            alt_font_info := _default_font(altcfg);
            parse alt_font_info with auto alt_font_name ',' auto alt_font_size ',' auto alt_font_opts;
            if (default_font_name == "") default_font_name = alt_font_name;
            if (default_font_size == "") default_font_size = alt_font_size;
            if (default_font_opts == "") default_font_opts = alt_font_opts;
         }
         have_font_change := false;
         lang_font_info := LanguageSettings.getFontInfo(langId, cfg);
         parse lang_font_info with auto lang_font_name ',' auto lang_font_size ',' auto lang_font_opts;

         lang_font_flags   := 0;
         lang_font_charset := VSCHARSET_DEFAULT;
         if (lang_font_opts != "") {
            parse lang_font_opts with auto str_flags ',' auto str_charset;
            if (isinteger(str_flags) && str_flags != 0) lang_font_flags = (int)str_flags;
            if (isinteger(str_charset) && str_charset != VSCHARSET_DEFAULT) lang_font_charset = (int)str_charset;
         }

         // if they use the font picker there could be more information tucked away
         picker_font_opts := "";
         font_flags   := 0;
         font_charset := VSCHARSET_DEFAULT;
         switch (controlName) {
         // Fonts
         case "ctl_unicode_editor_font_picker":    
         case "ctl_unicode_editor_font_name_list":    
         case "ctl_unicode_editor_font_size_list":
            picker_font_opts = ctl_unicode_editor_font_picker.p_user;
            break;                                    
         case "ctl_unicode_minimap_font_picker":   
         case "ctl_unicode_minimap_font_name_list":   
         case "ctl_unicode_minimap_font_size_list": 
            picker_font_opts = ctl_unicode_minimap_font_picker.p_user;
            break;                                    
         case "ctl_unicode_diff_font_picker":      
         case "ctl_unicode_diff_font_name_list":      
         case "ctl_unicode_diff_font_size_list":    
            picker_font_opts = ctl_unicode_diff_font_picker.p_user;
            break;                                  
         }
         if (picker_font_opts != null && picker_font_opts != "") {
            parse picker_font_opts with auto str_flags ',' auto str_charset;
            if (isinteger(str_flags) && str_flags != 0) font_flags = (int)str_flags;
            if (isinteger(str_charset) && str_charset != VSCHARSET_DEFAULT) font_charset = (int)str_charset;
         }

         if (endsWith(controlName, "picker")) {
            if (value == default_font_opts) value = "";
            if (value != lang_font_opts) {
               have_font_change = true;
               LanguageSettings.setFontInfo(langId, cfg, lang_font_name, lang_font_size, font_flags, font_charset);
               updateValue=lang_font_name','lang_font_size','picker_font_opts;
            }
         } else if (endsWith(controlName, "name_list")) {
            if (value == default_font_name) value = "";
            if (endsWith(value, ")")) parse value with value " (";
            if (value == default_font_name) value = "";
            if (value != lang_font_name) {
               have_font_change = true;
               LanguageSettings.setFontInfo(langId, cfg, value, lang_font_size, lang_font_flags, lang_font_charset);
               updateValue=value','lang_font_size','lang_font_opts;
            }
         } else {
            if (value == default_font_size) value = "";
            if (value != lang_font_size) {
               have_font_change = true;
               LanguageSettings.setFontInfo(langId, cfg, lang_font_name, value, lang_font_flags, lang_font_charset);
               updateValue=lang_font_name','value','lang_font_opts;
            }
         }
         if (have_font_change) {
            switch (controlName) {
            ...
            case "ctl_unicode_editor_font_name_list":    
            case "ctl_unicode_editor_font_size_list":
               APPEARANCE_OPTIONS_UNICODE_EDITOR_FONT_MODIFIED(true);
               break;                                    
            case "ctl_unicode_minimap_font_picker":   
            case "ctl_unicode_minimap_font_name_list":   
            case "ctl_unicode_minimap_font_size_list": 
               APPEARANCE_OPTIONS_UNICODE_MINIMAP_FONT_MODIFIED(true);
               break;                                    
            case "ctl_unicode_diff_font_picker":      
            case "ctl_unicode_diff_font_name_list":      
            case "ctl_unicode_diff_font_size_list":    
               APPEARANCE_OPTIONS_UNICODE_DIFF_FONT_MODIFIED(true);
               break;                                  
            }
         }
      }
...
_str _language_appearance_form_get_value(_str controlName, _str langId)
{
   _str value = null;
   ...
   // Color Profile
   case "ctl_scheme_name":
      value = LanguageSettings.getColorProfile(langId);
      if (value != "" && value != def_color_scheme) {
         value = se.color.ColorScheme.addProfilePrefix(value);
      } else {
         value = "";
      }
      break;
   case "ctl_alt_scheme_name":
      value = LanguageSettings.getAltColorProfile(langId);
      if (value != "" && value != def_color_scheme) {
         value = se.color.ColorScheme.addProfilePrefix(value);
      } else {
         value = "";
      ...
      break;
   case "ctl_minimap_scheme_name":
      value = LanguageSettings.getMinimapColorProfile(langId);
      if (value != "" && value != def_minimap_color_scheme) {
         value = se.color.ColorScheme.addProfilePrefix(value);
      } else {
         value = "";
      }
      break;
   case "ctl_alt_minimap_scheme_name":
      value = LanguageSettings.getAltMinimapColorProfile(langId);
      if (value != "" && value != def_minimap_color_scheme) {
         value = se.color.ColorScheme.addProfilePrefix(value);
      } else {
         value = "";
      ...
      break;
   case "ctl_alt_files":
      value = LanguageSettings.getAltColorFileSpec(langId);
      break;
   // Symbol Coloring
   case 'ctl_symbol_coloring':
   ...
   case 'ctl_symbol_errors':
      ...
      break;
   case 'ctl_symbol_override':
      override := LanguageSettings.getSymbolColoringOptions(langId);
      value = (override & SYMBOL_COLOR_NO_OVERRIDE_LIBRARY_SYMBOLS) ? 0 : 1;
      break;
   case 'ctl_strict_symbols':
   ...
   default:
      // fonts are a bit more complicated, we need to get the default so that
      // we can so the right font name, even if we are not overriding the default.
      //say('controlName='controlName);
      cfg := langGetCFGConstantsForFontControl(controlName, auto altcfg);
      if (cfg != CFG_NULL) {
         font_info := LanguageSettings.getFontInfo(langId, cfg);
         parse font_info with auto font_name ',' auto font_size ',' auto font_opts;
         if (endsWith(controlName, "picker")) {
            if (font_opts == "") return "";
         } else if (endsWith(controlName, "name_list")) {
            if (font_name == "") return "";
         } else {
            if (font_size == "" || font_size == 0) return "";
         }

         // tuck away any extra font information
         if (endsWith(controlName, "picker")) {
            switch (controlName) {
            case "ctl_unicode_editor_font_picker":    
               ctl_unicode_editor_font_picker.p_user = font_opts;
               break;                                    
            case "ctl_unicode_minimap_font_picker":   
               ctl_unicode_minimap_font_picker.p_user = font_opts;
               break;                                    
            case "ctl_unicode_diff_font_picker":      
               ctl_unicode_diff_font_picker.p_user = font_opts;
               break;                                  
            }
         }
         ...
         if (endsWith(controlName, "picker")) {
            if (font_opts != default_font_opts) return font_opts;
         } else if (endsWith(controlName, "name_list")) {
            if (font_name != default_font_name) return font_name;
         } else {
            if (font_size != default_font_size) return font_size;
         }
         return "";
      }
   }
...
ctl_fixed_fonts.lbutton_up()
{
   fixed_fonts_only := (p_value != 0);
   ctl_unicode_editor_font_name_list.  langUpdateFontNameList(CFG_UNICODE_SOURCE_WINDOW,      CFG_NULL,                    fixed_fonts_only);
   ctl_unicode_minimap_font_name_list. langUpdateFontNameList(CFG_UNICODE_MINIMAP_WINDOW,     CFG_UNICODE_SOURCE_WINDOW,   fixed_fonts_only);
   ctl_unicode_diff_font_name_list.    langUpdateFontNameList(CFG_UNICODE_DIFF_EDITOR_WINDOW, CFG_UNICODE_SOURCE_WINDOW,   fixed_fonts_only);
}

void ctl_unicode_editor_font_name_list.on_got_focus() 
{
   if (p_style != PSCBO_NOEDIT) {
      p_style = PSCBO_NOEDIT;
   }
}
void ctl_unicode_editor_font_name_list.on_lost_focus() 
{
...
void ctl_unicode_editor_font_name_list.on_drop_down(int reason) 
{
...
static int langGetCFGConstantsForFontControl(_str controlName, int &altcfg=null, _str &updateKey=null)
{
   cfg := 0;
   altcfg = 0;
   updateKey = "";
   ...
   case "ctl_unicode_editor_font_size_list":
      cfg = CFG_UNICODE_SOURCE_WINDOW;
      updateKey = UNICODE_FONT_INFO_UPDATE_KEY;
      break;                                    
   case "ctl_unicode_minimap_font_picker":   
   case "ctl_unicode_minimap_font_name_list":   
   case "ctl_unicode_minimap_font_size_list": 
      cfg = CFG_UNICODE_MINIMAP_WINDOW;
      altcfg = CFG_UNICODE_SOURCE_WINDOW;
      updateKey = UNICODE_MINIMAP_FONT_INFO_UPDATE_KEY;
      break;                                    
   case "ctl_unicode_diff_font_picker":      
   case "ctl_unicode_diff_font_name_list":      
   case "ctl_unicode_diff_font_size_list":    
      cfg = CFG_UNICODE_DIFF_EDITOR_WINDOW;
      altcfg = CFG_UNICODE_SOURCE_WINDOW;
      updateKey = UNICODE_DIFF_FONT_INFO_UPDATE_KEY;
      break;                                  
   }
   return cfg;
}

void ctl_unicode_editor_font_name_list.on_change(int reason, int index = 0)
{
   if (APPEARANCE_OPTIONS_IGNORE_FONT_CHANGE()) {
      return;
   }
   switch (reason) {
   case CHANGE_OTHER:
   ...
   case CHANGE_CLINE_NOTVIS:
      if (p_next.p_object == OI_COMBO_BOX && endsWith(p_name, 'font_name_list')) {
         font_name := get_font_name_from_list();
         if (font_name == "" && p_user != null) {
            parse p_user with . ':' font_name;
            font_name = strip(font_name);
         }
         controlName := p_next.p_name;
         cfg := langGetCFGConstantsForFontControl(controlName, auto altcfg);
         p_next.langUpdateFontSizeList(cfg, altcfg, font_name);
         p_next.p_next.p_user = null;
      } else 
      if (endsWith(p_name, 'font_size_list')) {
         p_next.p_user = null;
      }
      break;
   default:
...
void ctl_unicode_editor_font_name_list.DEL()
{
   _cbset_text("");

   // reset the advanced font flags / charset settings
   if (p_next.p_object == OI_COMBO_BOX && endsWith(p_name, 'font_name_list')) {
      p_next.p_next.p_user = null;
   } else 
   if (endsWith(p_name, 'font_size_list')) {
...
void ctl_unicode_editor_font_picker.lbutton_up() 
{
   font_name := p_prev.p_prev.get_font_name_from_list();
   font_size := p_prev.p_text;

   if (font_name == "" && p_prev.p_prev.p_user != null) {
      parse p_prev.p_prev.p_user with . ':' font_name;
   }
   if (font_size == "" && p_prev.p_user != null) {
      font_size = p_prev.p_user;
   }
   ...
   if (pos(':',font_size)) {
      parse font_size with ':' font_size;
      font_size=strip(font_size);
   }
   font_info := font_name ',' font_size ',' p_user;

   fixed_fonts_only := (ctl_fixed_fonts.p_value != 0)? 'f' : 's';
   result := show('-modal _wfont_form', 'C':+fixed_fonts_only, font_info);
   if (result == "") {
      return;
   }
   parse result with font_name ',' font_size ',' font_info;

   // try to get the font, either by full match or by prefix match
   if (p_prev.p_prev.p_text == "" && p_prev.p_prev.p_user != null && 
       (p_prev.p_prev.p_user == USING_DEFAULT_OPTION:+font_name || 
        beginsWith(p_prev.p_prev.p_user, USING_DEFAULT_OPTION:+font_name:+' ('))) {
      // leave empty font name alone (it is the same)
   } else if (p_prev.p_prev._lbfind_item(font_name) > 0) {
      p_prev.p_prev._cbset_text(font_name);
   } else if (!p_prev.p_prev._lbsearch('^':+_escape_re_chars(font_name), 'r')) {
      p_prev.p_prev._lbselect_line();
      p_prev.p_prev.p_text = p_prev.p_prev._lbget_text();
   } else {
      p_prev.p_prev._cbset_text(font_name);
   }

   if (p_prev.p_text == "" && p_prev.p_user != null && p_prev.p_user == USING_DEFAULT_OPTION:+font_size) {
      // leave empty font size alone (it is the same)
   } else {
      p_prev._cbset_text(font_size);
   }

   // stow away just the font flags and charset
   p_user = font_info;
}

void ctl_scheme_picker.lbutton_up() 
{
   langId := _get_language_form_lang_id();
   profile_name := se.color.ColorScheme.removeProfilePrefix(p_prev.p_text);
   result := show('-modal -wh _window_color_form', 'C', profile_name, 0, 1, langId);
   if (result == "") {
      return;
   }
   p_prev._cbset_text(result);
}

void _language_appearance_form.on_destroy()
{
   _language_form_on_destroy();
}

ctl_configure_fonts.lbutton_up()
{  
   config('Fonts');
}
ctl_configure_color_schemes.lbutton_up()
{  
   config('Colors');
}
...
ctl_symbol_coloring.lbutton_up()
{
   ...
   if (langId != ALL_LANGUAGES_ID) {
      ...
      ctl_symbol_errors.p_enabled = enabled &&
         _language_appearance_form_is_lang_included('ctl_symbol_errors', langId, false);

      ctl_symbol_override.p_enabled = ctl_symbol_errors.p_enabled;
   }
}

#endregion Options Dialog Helper Functions (Language > Appearance)
