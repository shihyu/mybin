...
static void _show_results_options(int mode)
{
   ...
   if (_result_opt_button.p_value == 0) {
      ...
      ctrl_ids[7] = _mfmatch.p_window_id;
...
#if 0
   /*
     Don't want the "Incremental searching is disabled" warning if the user is not 
     incremental searching. The Incremental searching check box is disabled. In any
     case, this is a poor place to display this message.
   */
...
static void _init_findfilematch()
{
   _mfmatch._lbadd_item(FINDFILE_MATCH_FILES);
   _mfmatch._lbadd_item(FINDFILE_MATCH_FILES_AND_DIRS);
   _mfmatch._lbadd_item(FINDFILE_MATCH_DIRS);
}
...
static void _tbfind_form_initial_alignment()
{
   ...
   _search_opt_button.resizeToolButtonToLabel(_lookin_label.p_window_id);
   _result_opt_button.resizeToolButtonToLabel(_lookin_label.p_window_id);
...
static int _search(_str search_text, _str search_options, int search_range)
{
   ...
   if (status==COMMAND_CANCELLED_RC) {
   } else if (do_next && !_search_continue()) {
...
static void _search_files()
{
   ...
   if (files != '') {
      if (_mfmatch.p_text==FINDFILE_MATCH_DIRS) {
         mfflags |= MFFIND_FIND_FILES_MATCH_DIRECTORIES|MFFIND_FIND_FILES_EXCLUDE_FILES;
      } else if (_mfmatch.p_text==FINDFILE_MATCH_FILES_AND_DIRS) {
         mfflags |= MFFIND_FIND_FILES_MATCH_DIRECTORIES;
      }
   ...
   if (search_range == VSSEARCHRANGE_ALL_BUFFERS) {
      ...
      for (;;) {
         if (!_isGrepBuffer(p_buf_name)) {
            status =_find_all(search_text, new_search_options, show_bookmarks, list_all, &results);  ++num_files;
            if (status==COMMAND_CANCELLED_RC) {
               break;
            }
            num_matches += status;
...
static void _incsearch_init_markers()
{
   if (gIncMarkerType < 0) {
      ...
      _MarkerTypeSetPriority(gIncMarkerType, VSMARKER_TYPE_PRIORITY_INCREMENTAL_SEARCH);
   }
...
static int _replace_in_project(_str search_text, _str search_options, _str replace_text, bool search_workspace, bool show_diff, SearchResults* results, int& num_replaced, bool close_mini_find_dialog=true)
{
   ...
   _save_pos2(auto p);
   /*
      In case search wrapping is off, start at the top of the file. Doing this,
      could cause the replace to be done too many times especially if the user already
      did some replaces using the mini replace dialog and then presses Alt+A
      to do the rest. User really needs to press Alt+A to start with.

      If wrap search is on, don't really need to start and the top but you will get
...
static void _init_findfiletypes(bool forceRefresh = false)
{
   ...
   if (PUSER_FILETYPELIST_INIT_DONE() == '') {
      ...
      _file_types_add_filters_to_combo(def_find_file_types);
...
static void _init_findfiletypes_ext()
{
   ...
   result:=_file_types_get_filter_by_wildcards(def_find_file_types,wildcard);
   if (result!='') {
      wildcard=result;
   }
...
{
   def_find_file_types=_file_types_remove_by_filter(def_find_file_types,value);
   _config_modify_flags(CFGMODIFY_DEFVAR);
}
...
static _str _get_search_results_label()
{
   ...
   if (_mfmatch.p_visible) {
      strappend(line, _mfmatch.p_text", ");
   }
   if (_findtabs.p_ActiveTab == VSSEARCHMODE_FINDINFILES) {
      if (_mfforegroundsearch.p_value) {
...
{
   if (pos(' (',text)) {
      parse text with auto name ' (' auto wildcards ')';
      def_find_file_types=_file_types_remove_by_name(def_find_file_types,name);
      def_find_file_types=_file_types_remove_by_wildcards(def_find_file_types,wildcards);
   } else {
      def_find_file_types=_file_types_remove_by_wildcards(def_find_file_types,text);
   }
   def_find_file_types=_file_types_prepend_name_and_wildcards(def_find_file_types,'',text);
   _config_modify_flags(CFGMODIFY_DEFVAR);
}
