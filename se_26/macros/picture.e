 ...
 * for {@code newValue} to set value.
 * 
 * @param newValue 
 * 
 * @return typeless
...
/**
 * Ensure that a list of controls all have the same width and height. 
 *  
 * @param size_to_control  this control has the "right" size 
 * @param controls         variadic arguments with window IDs of controls to resize 
 */
void resizeControlsIdentically(int size_to_control, ...)
{
   w := size_to_control.p_width;
   h := size_to_control.p_height;
   numControls := arg()-1;
   for (i := 0; i < numControls; i++) {
      wid := arg(i+2);
      if (!wid) {
...
/**
 * Scale down this button so that it's size is in line with the 
 * font height for a label on the same form. 
 *  
 * The current object is expected to be a tool window button. 
 * 
 * @param label_font_height   label height to base size on
 */
void resizeToolButtonToLabel(int label_wid)
{
   label_font_height := label_wid.p_height;
   label_font_height = (label_font_height*4 intdiv 5);
   resizeToolButton(label_font_height, allowTinySizes:true);
}
