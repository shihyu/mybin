...
int def_touch_max_jump=3;
int def_touch_jump_count=2;
int def_touch_min_swing_dist=300;
int def_touch_timer=5;
...
static bool gUseCursorXY = false;
...
bool _touch_debug=false;
bool _touch_debug2=false;
//_str _touch_debug2_list[];
                  ...
                  int option /* 1 from cursor, 2- end prev word, 10 return '' */)
{
   ...
   if (p_col>_text_colc() ||
       !( pos('[\od'_extra_word_chars:+_get_embedded_word_chars()']',get_text(1),1,'r') ||
          in_selection
        )
      ) {
      result="";
...
extern bool _IsTouchEmulationEvent();
struct TOUCHINFO {
   int x;
   int y;
   int start_x;
...
struct TOUCHINFOTIME {
   TOUCHINFO m_info;
   typeless m_time;
};
struct MOU_MORE_ARG_INFO {
   _str mstyle;
   int mx;
   int my;
   int wx;
   int wy;
   bool lastkey_not_rbutton;
   typeless orig_pos;
   _str old_scroll_style;
   _str copytext_option;
};
static bool mou_click_place_cursor(
   int &status,int in_touch,
   bool extend_mark_option,bool select_words,_str &mark_name,
   int &first_word_start_offset,int &first_word_end_offset,
   MOU_MORE_ARG_INFO more_args,
   bool was_minimap,int add_cursor,int &mark_locked,
   int in_left_margin,bool menu_option,bool &init_mark) {
   bool after_event_loop=false;
   if (in_touch==2) {
      after_event_loop=true;
      in_touch=0;
   }
   ...
   if ( extend_mark_option ) {
      ...
      if (select_words) {
         ...
         select_more_words(first_word_start_offset,first_word_end_offset,more_args.mstyle,more_args.mx,more_args.my);
      } else {
         p_cursor_y=more_args.my;
         p_cursor_x=more_args.mx;
         select_it(mark_name,'','CN');
      }
      ...
      if (!was_minimap) {
         if (add_cursor==1) {
            ...
            /* Would like to call _deselect() here but this will mess up the mou_in_selection() 
             ...
             Ctrl+Click can be used to copy selected text or add a cursor.
            */
   ...
   if (!in_touch && !add_cursor && _MultiCursor()) {
      _MultiCursorClearAll();
   }
   ...
   if (!in_touch && mark_name=='BLOCK' && p_hex_mode) {
      status=0;
      return true;
   }
   ...
   if (!in_touch && !after_event_loop 
       && !select_words && def_dragdrop && !in_left_margin && p_hex_mode!=HM_HEX_ON && !extend_mark_option && !menu_option &&
      ...
      status=_mou_click_handle_drag_drop(more_args.mx,more_args.my,more_args.wx,more_args.wy,mark_name,more_args.mstyle,select_words,menu_option,mark_locked,first_word_start_offset,first_word_end_offset,more_args.lastkey_not_rbutton,more_args.orig_pos,more_args.old_scroll_style,more_args.copytext_option);
   ...
   if (!in_touch && !select_words /*&& def_dragdrop */&&
       !extend_mark_option &&
       menu_option
       /*&& mou_in_selection(mou_last_x(),mou_last_y()) */
      ) {
      if (machine()=='WINDOWS' && _win32s()!=1) {
         if (!_isDragDrop(more_args.mx,more_args.my)) {
            if (mark_locked) {
               _select_type('','S','C');
            }
            ...
            _scroll_style(more_args.old_scroll_style);
            status=0;
            return true;
         }
      }
   ...
   if ( !in_touch && ! extend_mark_option ) {
      // IF this is a mark start with shift-arrow keys or mouse and */
      // marks are not persistant  OR  menu_option given
      if ( (_cua_select && def_persistent_select!='Y') || menu_option
         // OR the mark is a locked persistant mark
   ...
   if (init_mark && select_words) {
      select_whole_word(more_args.mstyle);
      init_mark=false;
   ...
   if (init_mark && mark_name == "LINE") {
      select_it(mark_name,'',more_args.mstyle);
   }
...
static bool _touch_set_speed(_str event,TOUCHINFOTIME (&touch_list)[],_str touch_direction,_str &scroll_arg,bool &touch_in_swing,int &touch_scroll_count,bool &touch_just_places_cursor,bool &in_slow_down,int &init_delay,int first_delay,int &slow_down_after_count,int &slow_down_inc,int &slow_down_count,int &count) {
   if (touch_list._length()>=2) {
      // scan entries
      //typeless curtime=_time('b');
      typeless curtime=touch_list[touch_list._length()-1].m_info.m_time;
      //bool same_dir=true;
      bool dir_set=false;
      typeless last_dxy='';
      int dir_dxy=0;
      if (_touch_debug) say('******************************');
      for (i:=touch_list._length()-1;i>=0;--i) {
         //time_diff:=curtime-touch_list[i].m_time;
         time_diff:=curtime-touch_list[i].m_info.m_time;
         if (time_diff>100) {
            if (_touch_debug) say('break');
            break;
         }
         int dxy;
         if (touch_direction=='UD') {
            dxy=touch_list[i].m_info.y- touch_list[i].m_info.start_y;
            if (_touch_debug) say('a1 time_diff='time_diff' dxy='dxy);
         } else {
            dxy=touch_list[i].m_info.x- touch_list[i].m_info.start_x;
            if (_touch_debug) say('a2 time_diff='time_diff' dxy='dxy);
         }
         if (!dir_set) {
            dir_dxy= dxy;
            dir_set=true;
      ...
      /*if (last_dxy==dir_dxy) {
         i=touch_list._length()-1;
         if (touch_direction=='UD') {
            dir_dxy=touch_list[i].m_info.y;
            last_dxy=touch_list[i].m_info.start_y;
         } else {
            dir_dxy=touch_list[i].m_info.x;
            last_dxy=touch_list[i].m_info.start_x;
         }
         say('SPECIAL CASE');
      } */
      _str new_scroll_arg;
      ...
      if (_touch_debug) say('dist='dist);
      int temp=0;
      if (touch_direction=='UD') {
         _dxy2lxy(SM_TWIP,temp,dist);
      } else {
         _dxy2lxy(SM_TWIP,dist,temp);
      }
      if (_touch_debug) say('h2 dist='dist);

      if (touch_in_swing) {
         if (dist<def_touch_min_swing_dist) {
            if (_touch_debug) say("BREAK");
            return true;
         }
      } else {
      ...
      mult:=((dist-def_touch_min_swing_dist) intdiv def_touch_min_swing_dist)+1;

      touch_list._makeempty();
      touch_scroll_count=0;
      touch_just_places_cursor=true;

      scroll_arg=new_scroll_arg;
      in_slow_down=false;
      init_delay=first_delay;
      slow_down_after_count=5*mult;
      slow_down_inc=7;
      slow_down_count=5;
      if (slow_down_after_count<10) {
         slow_down_inc=3;
         slow_down_count=5;
      }
      //event=swing_scroll(scroll_arg,init_delay,slow_down_after_count,slow_down_inc,slow_down_count);
      //if (event:=='') {
      //   say('DONE');
      //break;
      //}
      _set_timer(init_delay);count=0;in_slow_down=false;
      touch_in_swing=true;
      if (_touch_debug) say('CCCCCCCCCCCCCCCCCCCCCCCCCCC');
      return false;
   } else {
      if (_touch_debug) say('ELSE BREAK e='event2name(event)' jpc='touch_just_places_cursor);
      return true;
   }
}
static void _touch_scroll_loop(bool &touch_just_places_cursor) {
   typeless touch_time=_time('b');
   TOUCHINFOTIME touch_list[];
   int touch_scroll_count=0;   // Lines or char width
   _str touch_direction='';    // UD, LR
   touch_just_places_cursor=true;
   typeless event='';
   bool touch_in_swing=false;
   int count=0;
   bool in_slow_down=false;
   int init_delay=0;
   int slow_down_after_count=0;
   int slow_down_inc=0;
   int slow_down_count=0;
   int first_delay=def_touch_timer;
   int jmp_Noflines=1;
   int jmp_count=0;
   _str scroll_arg='';
   int wid_list[];
   if (p_active_form.p_name=='_diff_form' || 
       p_active_form.p_name=='_history_diff_form' || 
       p_active_form.p_name=='_svc_mfupdate_form'
       ) {
      wid_list[wid_list._length()]=_find_control('_ctlfile1');
      wid_list[wid_list._length()]=_find_control('_ctlfile2');
   } else if (p_active_form.p_name=='_merge_form') {
      wid_list[wid_list._length()]=_find_control('ctlbase');
      wid_list[wid_list._length()]=_find_control('ctlrev1');
      wid_list[wid_list._length()]=_find_control('ctlrev2');
      wid_list[wid_list._length()]=_find_control('ctloutput');
   } else {
      wid_list[wid_list._length()]=p_window_id;
   }
   /* 
       For some reason, Qt 5 (didn't test Qt 4) doesn't always return LBUTTON_UP immediately
       when the touch press is released. Generating timer events seems to fix the problem.
   */ 
   _set_timer(100);
   for (;;) {
       if (event:=='') {
       ...
       t:=_time('b');
       //say('ename='event2name(event)' jpc='touch_just_places_cursor' t='t' d='((typeless)t-touch_time));
       if (_touch_debug2 && event:==LBUTTON_UP) say('LBUTTON_UP');
       if (touch_just_places_cursor) {
          if (((typeless)t-touch_time)>500) {
             touch_just_places_cursor=false;
             if (_touch_debug2) {
                say('h2 ename='event2name(event)' jpc='touch_just_places_cursor);
             }
          }
       }
       if ( event:==MOUSE_MOVE ) {
          ...
          if (touch_direction=='') {
             // Choose whether scrolling up/down or left/right
             touch_dx:=_touchinfo[0].x- _touchinfo[0].start_x;
             touch_dy:=_touchinfo[0].y- _touchinfo[0].start_y;
             _dxy2lxy(SM_TWIP,touch_dx,touch_dy);
             if (abs(touch_dy)>abs(touch_dx)) {
                touch_direction='UD';
             } else {
                touch_direction='LR';
             }
          ...
          TOUCHINFOTIME info;
          info.m_info=_touchinfo[0];
          info.m_time=_time('b');
          touch_list[touch_list._length()]=info;
          if (touch_direction=='UD') {
             //typeless touch_diff_time=touch_time-_time('b');
             //if (touch_diff_time) {
             //}
             touch_dy:=_touchinfo[0].y- _touchinfo[0].start_y;
             touch_Noflines:= touch_dy intdiv p_font_height;
             touch_remainder:=touch_dy % p_font_height;
             if (touch_dy<0) {
                if (-touch_remainder>=(p_font_height intdiv 2)) {
                   --touch_Noflines;
                }
             } else {
                if (touch_remainder>=(p_font_height intdiv 2)) {
             ...
             int touch_diff=touch_Noflines-touch_scroll_count;
             if (touch_in_swing) {
                if (touch_diff) {
                   if (touch_diff>0) {
                      if (scroll_arg:!='U') {
                         if (_touch_debug) say("h1 CHANGE DIR");
                         if (touch_list._length()>=2) {
                            if (_touch_set_speed(
                               event,touch_list,touch_direction,
                               scroll_arg,touch_in_swing,touch_scroll_count,
                               touch_just_places_cursor,in_slow_down,init_delay,
                               first_delay,slow_down_after_count,slow_down_inc,slow_down_count,count) ) {
                               break;
                            } else {
                               jmp_Noflines=1;jmp_count=0;
                            }
                         }
                         event='';
                         continue;
                      ...
                      if (scroll_arg:!='D') {
                         if (_touch_debug) say("h2 CHANGE DIR="scroll_arg);
                         if (touch_list._length()>=2) {
                            if (_touch_set_speed(
                               event,touch_list,touch_direction,
                               scroll_arg,touch_in_swing,touch_scroll_count,
                   ...
                   if (in_slow_down) {
                      in_slow_down=false;
                      count=slow_down_after_count-20;
                      init_delay=first_delay;
                      _set_timer(init_delay);
                   } else {
                      count-=10;
                ...
                for (x:=0;x<wid_list._length();++x) {
                   w:=wid_list[x];
                   if (touch_diff>0) {
                      w._scroll_page('U',touch_diff);
                   } else if (touch_diff<0) {
                      w._scroll_page('D',-touch_diff);
                   }
                }
             }
             touch_scroll_count+=touch_diff;
          } else {
             touch_dx:=_touchinfo[0].x- _touchinfo[0].start_x;
             touch_Noflines:= touch_dx intdiv p_font_width;
             touch_remainder:=touch_dx % p_font_width;
             if (touch_dx<0) {
                if (-touch_remainder>=(p_font_width intdiv 2)) {
                   --touch_Noflines;
                }
             } else {
                if (touch_remainder>=(p_font_width intdiv 2)) {
             ...
             // ABCDEFGHIJKLMNOPQRST ABCDEFGHIJKLMNOPQRST  ABCDEFGHIJKLMNOPQRST  ABCDEFGHIJKLMNOPQRST ABCDEFGHIJKLMNOPQRST ABCDEFGHIJKLMNOPQRST  ABCDEFGHIJKLMNOPQRST  ABCDEFGHIJKLMNOPQRST
             int touch_diff=touch_Noflines-touch_scroll_count;
             if (touch_in_swing) {
                if (touch_diff) {
                   if (touch_diff>0) {
                ...
                for (x:=0;x<wid_list._length();++x) {
                   w:=wid_list[x];
                   if (touch_diff>0) {
                      w._sb_line_left(touch_diff);
                   } else if (touch_diff<0) {
                      w._sb_line_right(-touch_diff);
                   }
                }
             }
             touch_scroll_count+=touch_diff;
       ...
       //   event='';
          ...
          if (touch_in_swing) {
             for (x:=0;x<wid_list._length();++x) {
                w:=wid_list[x];
                if (scroll_arg:=='L') {
                   w._sb_line_left(jmp_Noflines);
                } else if (scroll_arg:=='R') {
                   w._sb_line_right(jmp_Noflines);
                } else {
                   w._scroll_page(scroll_arg,jmp_Noflines);
                }
             }
             if (in_slow_down) {
                ++count;
                if (count>=slow_down_count) {
                   if (_touch_debug) say('DONE WITH COUNTS');
                   break;
                }
                init_delay+=slow_down_inc;
                _set_timer(init_delay);
             } else {
                ++count;
                if (count>=slow_down_after_count) {
                   in_slow_down=true;
                   init_delay+=slow_down_inc;
                   count=0;
                   _set_timer(init_delay);
          ...
          if (_touch_debug) say('done e='event2name(event)' jpc='touch_just_places_cursor);
          if (touch_just_places_cursor) {
             return;
          }
          bool was_touch_in_swing=touch_in_swing;
          _str old_scroll_arg=scroll_arg;
          if (_touch_set_speed(
             event,touch_list,touch_direction,
             scroll_arg,touch_in_swing,touch_scroll_count,
             touch_just_places_cursor,in_slow_down,init_delay,
          ...
          event='';
          if (was_touch_in_swing && old_scroll_arg==scroll_arg) {
             ++jmp_count;
             if (jmp_count>=def_touch_jump_count) {
                ++jmp_Noflines;
                jmp_count=0;
             }
             if (jmp_Noflines>def_touch_max_jump) {
                jmp_Noflines=def_touch_max_jump;
             }
             //say('jmp_Noflines='jmp_Noflines' sa='scroll_arg' 'old_scroll_arg);
             if (jmp_Noflines>=3) {
                slow_down_after_count=slow_down_after_count*3;
             } else {
                slow_down_after_count=slow_down_after_count*jmp_Noflines;
             }
             touch_just_places_cursor=false;
          } else {
             //say('touch_just_places_cursor=true');
             jmp_Noflines=1;jmp_count=0;
             //say('RESET jmp_Noflines='jmp_Noflines' sa='scroll_arg' 'old_scroll_arg);
          }
          if (jmp_Noflines>5) jmp_Noflines=5;
       }
   }
   if (_touch_debug) say('##############################################');
   _kill_timer();
                       ...
                       int arg_add_cursor=0) name_info(','VSARG2_MARK|VSARG2_TEXT_BOX|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_ICON|VSARG2_NOEXIT_SCROLL)
{
   ...
   bool was_minimap=p_IsMinimap;
   if (_IsTouchEmulationEvent() && p_IsMinimap) {
      p_window_id=p_minimap_to_editor_wid;
   }
   ...
   add_cursor := ((arg_add_cursor==1 && !was_minimap)? 1:0);
   ...
   extend_mark_option := upcase(option)=='E' && !was_minimap;
   bool in_touch=_IsTouchEmulationEvent() && !extend_mark_option && _touchinfo._length();
   ...
   if (!was_minimap && !extend_mark_option && !mou_in_window()) {
      if (_isEditorCtl()) {
         //say('mx='mx);
   ...
   if (!was_minimap && p_LCHasCursor) {
      p_LCHasCursor=false;
   }

   if (!was_minimap && !extend_mark_option && def_seldisp_single && (!mou_in_window())) {
      if (_isEditorCtl()) {
         if (p_scroll_left_edge>=0) {
            _scroll_page('r');
         }
   ...
   int status;
   MOU_MORE_ARG_INFO more_args;
   more_args.mstyle=mstyle;
   more_args.mx=mx;
   more_args.my=my;
   more_args.wx=wx;
   more_args.wy=wy;
   more_args.lastkey_not_rbutton=lastkey_not_rbutton;
   more_args.orig_pos=orig_pos;
   more_args.old_scroll_style=old_scroll_style;
   more_args.copytext_option=copytext_option;
   
   if (!in_touch) {
      if(mou_click_place_cursor(
         status,0,extend_mark_option,select_words,
         mark_name,first_word_start_offset,first_word_end_offset,
         more_args,was_minimap,add_cursor,mark_locked,in_left_margin,menu_option,init_mark)) {
         return status;
      }
   }
   ...
   _MultiCursorClearAllIfOtherBuffer();
   if (in_touch) {
      bool touch_just_place_cursor;
      _touch_scroll_loop(touch_just_place_cursor);
      //say('after loop');
      if (touch_just_place_cursor) {
         //say('TOUCH_JUST_PLACES_CURSOR TRUE');
         if(mou_click_place_cursor(
            status,2,false/*extend_mark_option*/,false /*select_words*/,
            mark_name,first_word_start_offset,first_word_end_offset,
            more_args,was_minimap,add_cursor,mark_locked,in_left_margin,menu_option,init_mark)) {
            return status;
         }
   ...
   if (!do_sel_blk || !select_active()) {
      ...
      if (arg_add_cursor==2) {
         ...
         if (wid!=0) {
            cfg := _ColorFormGetColorIndexUnderCursor(p_window_id);
            wid._ColorFormSelectColor(cfg);
         }
      }
      if (!select_words && key:==LBUTTON_DOUBLE_CLICK) {
         call_event(p_window_id,LBUTTON_DOUBLE_CLICK);
         //call_root_key(key,true);
      }
   ...
   if (select_words && key:==LBUTTON_TRIPLE_CLICK) {
      call_event(p_window_id,LBUTTON_DOUBLE_CLICK);
      //call_root_key(key,true);
   }
   return(0);
...
void _sb_line_left(int count=1)
{
   if (p_scroll_left_edge<0) {
   ...
   int new_value;
   if (p_object!=OI_LIST_BOX && p_fixed_font) {
      new_value=p_scroll_left_edge-count;
   } else {
      new_value=p_scroll_left_edge-p_font_width*count;
   }
   if (new_value<0) {
      new_value=0;
   }
...
void _sb_line_right(int count=1)
{
   ...
   if (p_object!=OI_LIST_BOX && p_fixed_font) {
      p_scroll_left_edge+=count;
   } else {
      p_scroll_left_edge += p_font_width*count;
   }
}
void _sb_page_left()
{
...
void _ECTimerCallback(ECTimerCallbackArgs args)
{
   ...
   mx := 0;
   my := 0;
   gUseCursorXY = args.useCursorXY;
   if (args.useCursorXY && args.editorctl_wid && args.editorctl_wid._isEditorCtl()) {
      mx = args.editorctl_wid.p_cursor_x;
      my = args.editorctl_wid.p_cursor_y; 
      _map_xy(args.editorctl_wid,0,mx,my);
      gmx=mx;
      gmy=my;
   } else {
      mou_get_xy(mx, my);
   }
   in_rect := ( mx>=args.x && mx<args.x+args.width &&
...
void _MaybeCloseCursorXYBBWid() 
{
   if (gUseCursorXY && gBBWid && _iswindow_valid(gBBWid) && _isEditorCtl()) {
      in_form_rect := true;
      mx := p_cursor_x;
      my := p_cursor_y; 
      _map_xy(p_window_id,0,mx,my);
      if (mx != gmx || my != gmy) {
         _KillMouseOverBBWin();
      }
   }
...
void _lostfocus_mouse_over_bbwid()
{
   _KillMouseOverBBWin();
}
...
void _MouseMoveTimerCallback(_str args)
{
   ...
   do {
      ...
      if (_iswindow_valid(editorctl_wid) && editorctl_wid._isEditorCtl() && !editorctl_wid.p_IsTempEditor) {
...
static void handle_mouse_move(bool forceUpdate=false)
{
   ...
   if (_isEditorCtl(false) && 
      ...
      if (p_IsMinimap) {
         ...
         do_symbol_info_help(mx, my, useCursorXY:false);
      }
   }
   if (_chdebug) {
...
static void do_symbol_info_help(int mx, int my, bool useCursorXY, 
                                 VSCodeHelpFlags extra_codehelp_flags=VSCODEHELPFLAG_NULL)
{
   if (!_isEditorCtl()) {
      return;
   }
   if (_chdebug) {
      say("do_quick_symbol_help H"__LINE__": IN EDITOR WINDOW");
   }
   ...
   debug_expr := "";
   save_pos(auto p);
   header := debug_get_mouse_expr(cursor_x,width,
                                  &streamMessage,msg,debug_expr,
                                  tagList,
                                  useCursorXY,
                                  extra_codehelp_flags);
   if (_chdebug) {
      say("do_quick_symbol_help H"__LINE__": header="header);
      say("do_quick_symbol_help H"__LINE__": debug_expr="debug_expr);
      say("do_quick_symbol_help H"__LINE__": msg="msg);
      _dump_var(tagList, "do_quick_symbol_help");
   }
   if (msg == "" && smWidth>0) {
      width=smWidth;
      cursor_x = smCursorX;
   ...
   if (header != "" || msg != "" || tagList._length() > 0) {
      ...
      last_msg = curr_msg;
      if (_chdebug) {
         say("do_quick_symbol_help H"__LINE__": msg="msg" width="width" cursor_x="cursor_x);
      }
      _ConsiderBBWin(header,msg,OrigLineNum,OrigCol,below:true,cursor_x,width,tagList,useCursorXY);
   }
   restore_pos(p);
}

_command void symbol_info_help() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   if (_chdebug) {
      say("handle_mouse_move H"__LINE__": IN EDITOR WINDOW");
   }
   ...
   my := p_cursor_y; 
   _map_xy(p_window_id,0,mx,my);
   do_symbol_info_help(mx, my, useCursorXY:true, VSCODEHELPFLAG_MOUSE_OVER_INFO|VSCODEHELPFLAG_DISPLAY_MEMBER_COMMENTS|VSCODEHELPFLAG_DISPLAY_RETURN_TYPE);
}
int _OnUpdate_symbol_info_help(CMDUI &cmdui,int target_wid,_str command)
{
   return(_OnUpdate_push_ref(cmdui,target_wid,command));
}

_command void codehelp_trace_symbol_info_help() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveContextTagging()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Parameter help");
      return;
...
void _mouse_move()
{
   ...
   if (_IsTouchEmulationEvent() || !_isEditorCtl(false) || !_AppActive() || (p_hex_mode == HM_HEX_ON) || _tbInDragDropCtlMode()) {
      _KillMouseOverBBWin();
      return;
   }
