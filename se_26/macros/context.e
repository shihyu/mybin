 ...
 * @param onlyCheckModifyFlags   skip elapsed time checks and retry of _UpdateContext()
 * 
 * @return 
 ...
 */
bool _ContextIsUpToDate(long elapsed=0, int modifyFlags=MODIFYFLAG_CONTEXT_UPDATED, bool onlyCheckModifyFlags=false)
{
   ...
   // just do the most simple check of modify flags
   if (onlyCheckModifyFlags) {
      return false;
   }
...
static void handleUpdateContextFailure(int nUpdateFlags, _str warning, _str adjust_var)
{
   ...
   cm.member_name = warning :+ "  " :+
                    ...
                    "or go to Tools > Options > Editing > Context Tagging":+
                    VSREGISTEREDTM_TITLEBAR:+
                    " > Update Context performance tuning":+
                    " > Maximum size of file ... or Maximum number of tags ...";
...
{
   // check if the context is already up-to-date
   modifyFlags := MODIFYFLAG_CONTEXT_UPDATED;
   if (nUpdateFlags & VS_UPDATEFLAG_tokens) {
      modifyFlags |= MODIFYFLAG_TOKENLIST_UPDATED;
   }
   if (nUpdateFlags & VS_UPDATEFLAG_statement) {
      modifyFlags |= MODIFYFLAG_STATEMENTS_UPDATED;
   }

   if (_ContextIsUpToDate(elapsed: 0, modifyFlags, onlyCheckModifyFlags:true)) {

      if (p_ModifyFlags & MODIFYFLAG_CONTEXT_OVERSIZED) {
         if (_DidUpdateContextExceedLimits()) {
            return false;
         }
      } else {
         return true;
   ...
   if (buf_ksize > def_update_context_max_ksize) {
      ...
      if (!tag_check_cached_context(nUpdateFlags) || !(p_ModifyFlags & MODIFYFLAG_CONTEXT_UPDATED)) {
         ...
         p_ModifyFlags |= MODIFYFLAG_CONTEXT_UPDATED;
         p_ModifyFlags |= MODIFYFLAG_CONTEXT_OVERSIZED;
      }
      return false;
   }
   if (!LTindex && PSindex) {
...
int _ReportAsyncTaggingResults(bool AlwaysUpdate=false, int progressWid=0, int timeout=0)
{
   ...
   sc.lang.ScopedTimeoutGuard timeoutGuard;
   if (!AlwaysUpdate) {
   ...
   loop {
      ...
      haveNextTaggingJob := (status >= 0);
      if (!haveNextTaggingJob) {
         totalJobs := tag_get_num_async_tagging_jobs('U');
      ...
      if ((!AlwaysUpdate || timeout) && (!def_background_tagging_timeout || _CheckTimeout())) {
         break;
      }

      // no additional tagging jobs the last time we checked then stop now
      if (!haveNextTaggingJob) {
         break;
      }
...
bool _MaybeRetryTaggingWhenFinished(bool quiet=false, _str caption="", bool doRefreshTagFiles=true)
{
   ...
   while (tag_get_num_async_tagging_jobs('U') > 0) {
      ...
      if (_ReportAsyncTaggingResults(true, progressWid, timeout:250) < 0) {
         wasCancelled = true;
         break;
      }
      ...
      if (!quiet && !progressWid) {
         ...
         // give other threads a chance to play
         delay(250);
      }
   }
...
void _UpdateLocals(bool AlwaysUpdate=false, bool list_all=true)
{
   ...
   //}
   //say("_UpdateLocals: elapsed time ="((long)_time('b')-orig_time)"ms");
 ...
 *
 * @param idexp_info    prefix expression information structure to initialize
 *
 * @categories Tagging_Functions
 */
...
/**
 * Initialize a find tags information structure.
 *
 * @param find_info  find tags information structure to initialize
 *
 * @categories Tagging_Functions
 */
void tag_find_tags_info_init(struct VS_TAG_FIND_TAGS_INFO &find_info, _str lang=null)
{
   // maybe we can figure out the current language ID
   if (lang == null && _isEditorCtl()) {
      lang = p_LangId;
   }

   if (lang != null && lang != "") {
      find_info.langId = lang;
      find_info.tag_files = tags_filenamea(lang);
   } else {
      find_info.langId = "";
      find_info.tag_files = null;
   }

   find_info.find_parents = false;
   find_info.exact_match  = false;
   find_info.case_sensitive = true;
   find_info.max_matches = def_tag_max_list_matches_symbols;

   find_info.filter_flags = SE_TAG_FILTER_ANYTHING;
   find_info.context_flags = SE_TAG_CONTEXT_ANYTHING;
}
...
void tag_idexp_info_dump(struct VS_TAG_IDEXP_INFO &idexp_info, _str where="", int depth=0)
{
   ...
   if (idexp_info.cursor_col==null) idexp_info.cursor_col=0;
   isay(depth, "idexp_info.cursor_col="idexp_info.cursor_col);
...
_str tag_dump_tag_flags(SETagFlags tag_flags)
{
   ...
   if (tag_flags & SE_TAG_FLAG_IN_CONTEXT)   flags :+= "in_context |";
   if (tag_flags & SE_TAG_FLAG_NESTED_LOCAL) flags :+= "nested |";
   if (tag_flags & SE_TAG_FLAG_ASYNCHRONOUS) flags :+= "async |";
   if (tag_flags & SE_TAG_FLAG_FACTORY)      flags :+= "factory |";
   if (tag_flags & SE_TAG_FLAG_LATE)         flags :+= "late |";
...
void tag_dump_context_flags(SETagContextFlags context_flags, _str where = "", int level=0)
{
   ...
   case SE_TAG_CONTEXT_MATCH_CORRECTIONS:  flags :+= "correction |"; break;
   }
...
/**
 * Display statistics collected for each active tag file. 
 * This is a debugging tool to help identify tag files that can 
 * cause certain operations to be more time-consuming while editing. 
 * 
 * @param cmd     One of the following: 
 *                <ul>
 *                <li><b>summary</b> -- print summary information (default)</li>
 *                <li><b>all</b> -- show information for all tag files, not just currently active ones</li>
 *                <li><b>dump -- dump all statistics to the vsdebug window</b>
 * </ul> 
 *  
 * @see tags_filenamea
 ...
 * @since 26.0
 */
_command void codehelp_tag_file_stats(_str cmd="") name_info(COMMAND_ARG',')
{
   editorctl_wid := 0;
   if (_isEditorCtl()) {
   ...
   if (tag_files._length() <= 0) {
      _message_box("No tag files for this mode");
      return;
   }
   ...
   foreach (auto i => auto tf in tag_files) {
      ...
      sizeK := (_file_size(tf) intdiv 1024);
      sizeM := (sizeK / 1024);
      round(sizeM, 1);
      tagFileSize := (sizeK > 1000)? sizeM:+"MB" : sizeK:+"KB";
      file_size :+= tagFileSize;

      // open the tag file
      handle := tag_read_db(tf);
      if (handle < 0) {
         open_status :+= get_message(handle,tf);
         lang_modes  :+= "-";
         num_files   :+= "=";
         num_classes :+= "-";
         num_tags    :+= "-";
      ...
      // good open status, nothing to report
      open_status :+= "";

      // get the language modes in this tag file
      nModes := "";
      status := tag_find_language(auto lang);
      while (status >= 0 && lang != "") {
         _maybe_append(nModes, ',');
         nModes :+= _LangId2Modename(lang);
         status = tag_next_language(lang);
      }
      lang_modes :+= nModes;

      // get the number of files
      tag_get_detail(VS_TAGDETAIL_num_files, auto nFiles);
      num_files :+= nFiles;

      // get the number of classes
      tag_get_detail(VS_TAGDETAIL_num_classes, auto nClasses);
      num_classes :+= nClasses;

      // get the number of tags
      tag_get_detail(VS_TAGDETAIL_num_tags, auto nTags);
      num_tags :+= nTags;

      // does the database have references?
   ...
   if (lowcase(cmd) == "dump") {
      foreach (i => tf in tag_files) {
         say("-----------------------");
         say("codehelp_tag_file_stats: ":+tf);
         say("           open_status : ":+open_status[i]);
         say("           lang_modes  : ":+lang_modes[i]);
         say("           file_size   : ":+file_size[i]);
         say("           num_files   : ":+num_files[i]);
         say("           num_classes : ":+num_classes[i]);
         say("           num_tags    : ":+num_tags[i]);
         say("           has_refs    : ":+has_refs[i]);
         say("-----------------------");
      }
      return;
   }

   pic_file_refs       := _update_picture(-1,'_f_references.svg');
   pic_file_tags       := _update_picture(-1,'_f_symbols.svg');
   pic_file_tags_error := _update_picture(-1,'_f_symbols_error.svg');
   ...
   foreach (i => tf in tag_files) {
      ...
      case "No":
         ...
         break;
      default:
         icons :+= pic_file_tags_error;
         break;
      }

      caption := tf;
      caption :+= "\t";
      caption :+= open_status[i];
      caption :+= "\t";
      caption :+= file_size[i];
      caption :+= "\t";
      caption :+= lang_modes[i];
      caption :+= "\t";
      caption :+= num_files[i];
      caption :+= "\t";
      caption :+= num_classes[i];
      caption :+= "\t";
      caption :+= num_tags[i];
      caption :+= "\t";
      caption :+= has_refs[i];
   ...
   select_tree(cap_array:     lines,
               picture_array: icons,
               //callback:      codehelp_update_context_stats_cb,
               caption:       "Context Tagging "VSREGISTEREDTM" Tag Files Summary",
               sl_flags:      SL_CLOSEBUTTON|SL_COLWIDTH|SL_SIZABLE|SL_RESTORE_XY|SL_RESTORE_HEIGHT|SL_DEFAULTCALLBACK,
               col_names:     "Tag File,Status,Size,Language(s),Files,Classes,Symbols,References",
               col_flags:     (TREE_BUTTON_SORT_COLUMN_ONLY|TREE_BUTTON_SORT_FILENAME|TREE_BUTTON_PUSHBUTTON|TREE_BUTTON_IS_FILENAME)",":+
