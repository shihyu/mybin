 ...
 * 
 */
_command int html_preview(_str filename="")  name_info(FILE_ARG','VSARG2_MARK|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
...
static bool isStandAloneTag(_str tag, bool includeListItems=true)
{
   tag=upcase(tag);
   foreach (auto li in listItemTagList) {
      if (tag == li) {
         return includeListItems;
      }
   }
   foreach (auto name in standAloneTagList) {
      if (tag == name) {
         return true;
      }
   }
...
static int matchTag(int cursorAtEndTag, _str &direction, int pmatch_max_diff_ksize=MAXINT, int pmatch_max_level=MAXINT, bool canTimeout=true)
{
   ...
   if ( !_LanguageInheritsFrom("xml") && isStandAloneTag(tag, includeListItems:false) ) {
      return( -2 );
   }
   typeless status=0;
   if ( startTag ) {
...
// If the cursor is between an empty open/close tag pair when
// return is hit, and it's not a nosplit enter, re-expand the tags
// to be on separate lines, with the cursor indented on a line 
// in between them.  This allows us to expand tags that 
// don't _require_ inner newlines on one line.  If the user wants
// the tags to be more like a block, they can hit return. Returns
// true if the conditions were met and the tags were moved.
bool _html_expanded_empty_tag_pair()
{
   bool rv;
   curc := get_text_safe(2);
   if (name_on_key(ENTER) == "nosplit-insert-line"
      ...
      cursor := _QROffset();
      right(); right();
      endtag := cur_identifier(auto startcol);
      _GoToROffset(cursor);
      starttag := _html_goto_previous_tag();
      if (starttag != '') {
         if (starttag == endtag) {
            start_col := p_col;
            _GoToROffset(cursor);
            call_root_key(ENTER);
            get_line(auto cline);
            replace_line(indent_string(start_col-1):+strip(cline));
            _GoToROffset(cursor);
            call_root_key(ENTER);
            p_col = start_col + p_SyntaxIndent;
            rv = true;
         } else {
            _GoToROffset(cursor);
            rv = false;
...
{  
   if (_html_expanded_empty_tag_pair()) {
      return false;
   }
   if (XW_doEnter()) {
      return false;
...
// A weaker test than the "standalone" attribute.  Returns true 
// if the inner newlines != 0 and the setting is not disabled.
bool tag_requires_inner_newlines(_str tag)
{
   tag=_beautifier_resolve_tag(gtagcache_info,tag);
   inner := _beautifier_get_tag_lb_within(gtagcache_info, tag, auto inner_apply);

   return inner_apply && inner != '0';
}
...
static void insert_html_close_tag( _str endtag = "")
{
   ...
   if (tag_requires_inner_newlines(endtag)) {
 ...
 * 
 * @return {@code msg} with href tags stripped out
 */
