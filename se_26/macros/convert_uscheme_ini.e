...
class OldColorScheme : IColorCollection /*, sc.lang.IEquals*/ {
   ...
   void updateColorScheme(int old_version) {
      ...
      if (old_version < COLOR_SCHEME_VERSION_SYMBOL_COLORING_PALETTE) {
         //Set default color to orange
         m_colors[CFG_NAVHINT].m_background = m_colors[CFG_WINDOW_TEXT].m_background;
         m_colors[CFG_NAVHINT].m_fontFlags |= F_INHERIT_BG_COLOR;
         m_embedded[CFG_NAVHINT].m_foreground = m_colors[CFG_NAVHINT].m_foreground;
         m_embedded[CFG_NAVHINT].m_background = m_embedded[CFG_WINDOW_TEXT].m_background;
         m_embedded[CFG_NAVHINT].m_fontFlags |= F_INHERIT_BG_COLOR;
      }
         ...
         if (handle>=0) {
            ...
            if (property_node>=0) {
               ...
               if (attrs_node>=0) {
                  ...
                  if (bg!='' && fg!='' && flags!='') {
                     bg    = ColorInfo.parseColorValue(bg);
                     fg    = ColorInfo.parseColorValue(fg);
            ...
            if (property_node>=0) {
               ...
               if (attrs_node>=0) {
                  ...
                  if (embg!='' && flags!='') {
                     ...
                     embg = ColorInfo.parseColorValue(embg);
   ...
   int loadScheme(_str fileName,  _str schemeName) {
       ...
       foreach (auto colorInfo in colors) {
          ...
          case "bg":
             c->m_background = se.color.ColorInfo.parseColorValue(value);
             break;
          case "fg":
             c->m_foreground = se.color.ColorInfo.parseColorValue(value);
             break;
          case "ff":
          ...
          case "em":
             ...
             ec->m_background = se.color.ColorInfo.parseColorValue(value);
             break;
          }
       }
