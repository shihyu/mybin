...
int _alltimer_handle = -1; //Main timer. Global timer handle
int _alltimer_handle2 = -1; // Timer for restarting main timer
...
definit()
{
   ...
   if (_alltimer_handle2>=0) {
      _kill_timer(_alltimer_handle2);
      _alltimer_handle2=-1;
   }
   _alltimer_handle2=_set_timer(90000 /* every 90 seconds.*/,_maybe_start_main_timer);
...
void _UpdateWordHighlighting(bool AlwaysUpdate=false);
...
static void registerStandardAutoSaveTimerCallbacks()
{
   ...
   /////////////////////////////////////////////////////////////////////////////
   {
      ...
      as_running_timer_functions :+= _UpdateWordHighlighting;
...
static _str _untitledfiles_directory()
{
   path := get_env(_SLICKEDITCONFIG);
   _maybe_append_filesep(path);
   return(path:+'UntitledFiles':+FILESEP);
}
...
bool _IsTempBufName() {
   temp_name:=_untitledfiles_directory();
   _maybe_append_filesep(temp_name);
   strappend(temp_name,'Untitled ');
   if (beginsWith(p_buf_name,temp_name,false,_fpos_case) /*&& (p_buf_flags & VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME)*/) {
      return true;
   }
   return false;
}
void _CreateTempBufName() {
   _str temp_name;
   temp_name=_untitledfiles_directory();
   _maybe_append_filesep(temp_name);
   make_path(temp_name,'N');
   strappend(temp_name,'Untitled '_date('i'));
   if (_FindModifiedTempName(temp_name)<0) {
      p_buf_name=temp_name;
      p_buf_flags|=VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME;
      return;
   }
   //Not unique? Now append the hour to make this more unique
   t:=_time('m');
   strappend(temp_name,' 'substr(t,1,2));
   if (_FindModifiedTempName(temp_name)<0) {
      p_buf_name=temp_name;
      p_buf_flags|=VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME;
      return;
   }
   //Not unique? Now append the minutes
   strappend(temp_name,substr(t,4,2));
   if (_FindModifiedTempName(temp_name)<0) {
      p_buf_name=temp_name;
      p_buf_flags|=VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME;
      return;
   }
   //Still not unique? Append count
   for (i:=2;;++i) {
      if (_FindModifiedTempName(temp_name:+'-':+i)<0) {
         p_buf_name=temp_name:+'-':+i;
         p_buf_flags|=VSBUFFLAG_DELETE_FILE_ON_CLOSE_OR_RENAME;
         return;
      }
   }
...
void _maybe_start_main_timer() {
   if (!def_use_timers || !_use_timers) {
      // Don't want the main timer
      return;
   }
   if (!isinteger(_alltimer_handle) || !_timer_is_valid(_alltimer_handle)) {
      return;
   }
   // IF this timer was stopped due to a Slick-C stack.
   if (_timer_is_stopped(_alltimer_handle)) {
      //say('restarting main timer');
      _kill_timer(_alltimer_handle);_alltimer_handle=-1;
      turn_on_timer();
   }
}
bool _is_main_timer(int timer_handle) {
   if (timer_handle!=_alltimer_handle) {
      return false;
   }
   if (!def_use_timers || !_use_timers) {
      // Don't want the main timer
