...
const LANGS_DONT_MATCH_TB='';
const LANGS_DONT_MATCH_CB='Languages Differ';
      ...
      if (wid.p_visible || wid.p_object == OI_SSTAB_CONTAINER) {
         ...
         if (canSetControl(wid)) {
            ...
            if (langId == ALL_LANGUAGES_ID) {               
               ...
               // if the control is an empty combo box or text box with placeholder text
               // treat the value as empty rather than null.
               if (value == null && isCombo && wid.p_text == "") {
                  placeholder := wid._ComboBoxGetPlaceHolderText(wid);
...
bool setControlValue(int wid, _str value)
{
   ...
   if (wid.p_visible) {
      ...
      case OI_IMAGE:
         if (wid.p_name=='ctl_unicode_editor_font_picker' || 
             wid.p_name=='ctl_unicode_minimap_font_picker' || 
             wid.p_name=='ctl_unicode_diff_font_picker'
             ) {
            wid.p_user=value;
            set=true;
         } else {
