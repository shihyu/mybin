...
class RubberBand {
   ...
   /**
    * Destroy rubber-band window. Use {@link create} to
    * recreate the window.
    */
   public void destroy() {
      if( m_wid > 0 ) {
    ...
    * window is valid, then use {@link isValid} instead. 
    * 
    ...
    * expensive than {@link isCreated} since it tests if the 
