...
void ctl_filter.home,c_home/*,"c-u"*/()
{
   if (last_event():==HOME) {
...
{
   _list_pass_thru_ctrl_az(nextTreeControlWid());
}
...
static int FileListAddBuffer(int tree_wid, bool isCurrent)
{
   ...
   if (tbfilelist_is_modified()) {
      flags = TREENODE_FORCECOLOR;
      star='*';
   } else if (p_ReadOnly) {
      flags |= TREENODE_ITALIC;
   }
         ...
         if ( !forceUpdate && (_get_focus() == file_list) ) {
            ...
            if ( index>=0 ) {
               ...
               if ( editorctl_wid.tbfilelist_is_modified() ) {            
                  nodeFlags|=TREENODE_FORCECOLOR;
                  state=1;
               } else if (editorctl_wid.p_ReadOnly) {
                  nodeFlags|=TREENODE_ITALIC;
                  state=0;
               } else {
