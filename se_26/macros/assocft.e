...
void fillExtensionTree()
{
   ...
   if (_ctl_view_combo.p_text == VIEW_BY_LANG) {
      ...
      // clear column headers, sort of
      _ctl_ext_tree._TreeSetColButtonInfo(0, _ctl_ext_tree.p_width, TREE_BUTTON_SORT, -1, 'Language Mode > File Extension');
      _ctl_ext_tree._TreeSetColButtonInfo(1, _ctl_ext_tree.p_width, TREE_BUTTON_AUTOSIZE, -1, '');
      ...
      for (i := 0; i < langs._length(); i++) {
         ...
         if (list != '') {
            ...
            extensions._sort();
            addedOne := false;
            foreach ( auto ext in extensions ) {
               // not all extensions are associable
               if (_allowable_for_association(ext)) {
                  extNode := _ctl_ext_tree._TreeAddItem(langNode, ext, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);

                  checked := isExtensionAssociated(ext);
      ...
      // set column headers
      _ctl_ext_tree._TreeSetColButtonInfo(0, _ctl_ext_tree.p_width intdiv 2, TREE_BUTTON_SORT, -1, 'File Extension');
      _ctl_ext_tree._TreeSetColButtonInfo(1, _ctl_ext_tree.p_width intdiv 2, TREE_BUTTON_SORT, -1, 'Language Mode');
      ...
      foreach (auto ext in extensions) {
        if (_allowable_for_association(ext)) {
           ...
           if (length(lang) <= 0 && _ExtensionGetIgnoreSuffix(ext)) {
              continue;
           }
           modename := _LangGetModeName(lang);
           extNode := _ctl_ext_tree._TreeAddItem(TREE_ROOT_INDEX, ext:+"\t":+modename, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
...
void _ctl_ext_tree.on_change(int reason, int index)
{
   if (reason == CHANGE_CHECK_TOGGLED) {
      ...
      if (child > 0) {
         while (child > 0) {
            extList :+= _TreeGetCaption(child, 0);
            child = _TreeGetNextSiblingIndex(child);
         }
      } else {
         extList[0] = _TreeGetCaption(index, 0);
      }
