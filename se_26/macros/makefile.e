...
static void GetVCXBuildTaskExtensions(_str projectExt, _str (&ExtToTask):[])
{
   ...
   ExtToTask:['mpp']    = 'ClCompile';
   ExtToTask:['mxx']    = 'ClCompile';
...
static int AddFileToVisualStudioStandardProject(_str FileList,_str VisualStudioProjectName,
                                                _str ext,bool FileExistsOnDisk=true,bool &only_added_files_matching_wildcards=false)
{
   only_added_files_matching_wildcards=true;
   ...
   int temp_handle=-1;
   int wildcardNodes[];
   assoc_handle := _xmlcfg_open(VisualStudioProjectName, auto xcstatus, VSXMLCFG_OPEN_REFCOUNT|VSXMLCFG_OPEN_ADD_PCDATA);
   if (assoc_handle > 0) {
      ...
      if (is_wildcard_project) {
         ...
         AssociatedFile=VisualStudioProjectName;
   ...
   for (;;) {
      cur=parse_file(list);
      if (cur=='') break;
      rel_filename:=relative(cur,_strip_filename(VisualStudioProjectName,'N'));
      if (wildcardNodes._length()) {
         if(_ProjectFileMatchesWildcards(temp_handle,wildcardNodes,rel_filename)) {
            continue;
         }
      }
      insert_line(' 'rel_filename);
      only_added_files_matching_wildcards=false;
   }
   p_window_id=orig_view_id;
   if (temp_handle>=0) {
   ...
   int status;
   if (temp_view_id.p_Noflines>0) {
      status=WriteVisualStudioStandardProjectFile2(VisualStudioProjectName,
                                                   AppName,
                                                    ...
                                                    bool &removed_a_file=false)
{
   ...
   bool AutoAdd=_IsVisualStudioWildcardProject(handle);
...
int _AddFileToVCPPMakefile(_str filelist,_str VCPPProjectName='',
                           bool Quiet=false,bool &only_added_files_matching_wildcards=false)
{
   if (VCPPProjectName=='') {
      //Find the associated makefile(.dsp file).
   ...
   if ( _file_eq(ext,VISUAL_STUDIO_VCPP_PROJECT_EXT) ||
      ...
      return(_AddFileToVisualStudioProject(filelist,VCPPProjectName,!Quiet,only_added_files_matching_wildcards));
   }
...
int _AddFileToVisualStudioProject(_str FileList,_str ProjectFilename,bool FileExistsOnDisk=true,bool &only_added_files_matching_wildcards=false)
{
   _str ext=_get_extension(ProjectFilename,true);
   if (_file_eq(ext,VISUAL_STUDIO_INTEL_CPP_PROJECT_EXT)) {
      ProjectFilename = absolute(getICProjAssociatedProjectFile(ProjectFilename),_strip_filename(ProjectFilename,'N'));
   ...
   return(AddFileToVisualStudioStandardProject(FileList,ProjectFilename,ext,FileExistsOnDisk,only_added_files_matching_wildcards));
}
 ...
 */
void _prjupdate_project_cache(_str ProjectName) 
{
                                      ...
                                      int iProjectIndex)
{
   ...
   if (_file_eq(ext,VISUAL_STUDIO_VCPP_PROJECT_EXT) ||
      ...
      hasWildcards := (_VPJHasWildcards(temp_handle) != 0);
      if (hasWildcards && !_isProjectInfoCached(_workspace_filename, AssociatedFile)) {
 ...
 * @param subStr2  String to use in place of {@code subStr1}
 */
 ...
 *                  referenced by {@code makefilename}
 * @param targetList
 * @param runTargetName
