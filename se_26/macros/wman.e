...
bool _NewInstanceSupported(bool updateConfigImmediately=true) {
   ...
   if (!(def_save_config_flags & SAVE_CONFIG_IMMEDIATELY_SHARE_CONFIG)) {
      ...
      def_save_config_flags=SAVE_CONFIG_IMMEDIATELY_SHARE_CONFIG;
      _config_modify_flags(CFGMODIFY_DEFVAR);
      _maybe_save_config();
...
void _AddWorkspaceTreeToMenu()
{
   ...
   // try to remove duplicate entries (only once, or if history changes)
...
static bool shortcut_not_supported() {
   if (_isMac() || _isWindows() || !_isUnix()) {
      _message_box('This shortcut command is not supported on this OS');
      return true;
   }
   return false;
}
_command void edit_desktop_shortcut(_str filename='',_str option='') name_info(',')
{
   if (shortcut_not_supported()) {
      return;
   }
   option=upcase(option);
   if (filename=='') {
      filename=_X11Shortcut_DesktopFilename();
   }
   if (file_exists(filename)) {
      edit(_maybe_quote_filename(filename));
      message("Default shortcut text copied to clipboard");
      _copy_text_to_clipboard((option=='D')? _X11Shortcut_DiffGetText(): _X11Shortcut_GetText());
      return;
   }
   edit(_maybe_quote_filename(filename));
   _insert_text((option=='D')? _X11Shortcut_DiffGetText(): _X11Shortcut_GetText());
}
_command void edit_app_shortcut() name_info(',')
{
   edit_desktop_shortcut(_X11Shortcut_AppFilename());
}
_command void edit_diff_app_shortcut() name_info(',')
{
   edit_desktop_shortcut(_X11Shortcut_DiffAppFilename(),'D');
}
