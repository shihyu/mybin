...
enum_flags CLEX_MATCH_NUMBER {
    ...
    CLEXMN_F_EXPONENT       =0x40000,  // 1.2F4 (Julia)
};
...
void ctlcolor_string_identifiers.lbutton_up()
{
   if (p_active_form.p_visible && !IN_LEXER_LIST_ON_CHANGE()) {
      ...
      bool orig_enabled=pcomment->enabled;
      _find_color_string_identifiers(pclexdef->comments,'color-string-identifiers',p_value?true:false);
      if (pcomment->enabled!=orig_enabled) {
         ctltoken_enabled.p_value=ctltoken_enabled.p_value?0:1;
      }
...
void ctldigit_float.lbutton_up() {
   if (p_active_form.p_visible && !IN_LEXER_LIST_ON_CHANGE()) {
      ctldote_float.p_enabled=ctld_exponent.p_enabled=ctlf_exponent.p_enabled=(p_value!=0);
      AddLexerToModList();
   }
}
...
void ctltoken_enabled.lbutton_up() {
   if (p_active_form.p_visible && !IN_LEXER_LIST_ON_CHANGE()) {
      COMMENT_TYPE *pcomment=get_item_pointer();
      pcomment->enabled=p_value?true:false;
      AddLexerToModList();
   }
}
void ctlrepeat_after.lbutton_up() {
   if (p_active_form.p_visible && !IN_LEXER_LIST_ON_CHANGE()) {
      COMMENT_TYPE *pcomment=get_item_pointer();
      pcomment->flags&=~ CLEXMF_REPEAT_AFTER;
      if (ctlrepeat_after.p_value) {
         pcomment->flags|=CLEXMF_REPEAT_AFTER;
      }
      AddLexerToModList();
   }
...
void ctlstart_substring_colors.lbutton_up() {
   if (p_active_form.p_visible && !IN_LEXER_LIST_ON_CHANGE()) {
      COMMENT_TYPE *pcomment=get_item_pointer();
      result:=show('-modal _substring_colors_form',pcomment->start_substring_colors,(pcomment->flags&(CLEXMF_REGEX|CLEXMF_PERLRE))?true:false);
      if (result=='') {
         return;
      }
      pcomment->start_substring_colors=_param1;
      AddLexerToModList();
   }
}
void ctlend_substring_colors.lbutton_up() {
   if (p_active_form.p_visible && !IN_LEXER_LIST_ON_CHANGE()) {
      COMMENT_TYPE *pcomment=get_item_pointer();
      result:=show('-modal _substring_colors_form',pcomment->end_substring_colors,(pcomment->flags&(CLEXMF_END_REGEX|CLEXMF_END_PERLRE))?true:false);
      if (result=='') {
         return;
      }
      pcomment->end_substring_colors=_param1;
      AddLexerToModList();
   }
}
...
static void LoadComments(int handle,int profile_node,COMMENT_TYPE (&comments)[]) {
   ...
   for (i:=0;i<array._length();++i) {
      ...
      if (!_xmlcfg_get_attribute(handle,attrs_node,'enabled')) {
         continue;
      }
...
static int gclexmf_table:[]={
   ...
   'repeat_after'=>CLEXMF_REPEAT_AFTER,
};
static int gclexmn_table:[]={
   ...
   "f_exponent"=>CLEXMN_F_EXPONENT,
};
static void _clex_add_xml_attrs(int handle,int attrs_node,COMMENT_TYPE &info) {
   ...
   if (!info.enabled) {
      _xmlcfg_set_attribute(handle,attrs_node,'enabled',info.enabled);
   } else {
      _xmlcfg_delete_attribute(handle,attrs_node,'enabled');
   }

   if (info.features!='') {
      _xmlcfg_set_attribute(handle,attrs_node,'features',info.features);
   } else {
      _xmlcfg_delete_attribute(handle,attrs_node,'features');
   }
   if (info.start_color!='') _xmlcfg_set_attribute(handle,attrs_node,'start_color',info.start_color);
   if (info.start_substring_colors!='') _xmlcfg_set_attribute(handle,attrs_node,'start_substring_colors',info.start_substring_colors);
   if (info.end_color!='') _xmlcfg_set_attribute(handle,attrs_node,'end_color',info.end_color);
   if (info.end_substring_colors!='') _xmlcfg_set_attribute(handle,attrs_node,'end_substring_colors',info.end_substring_colors);
...
static void _clex_add_property_item(int handle,int profileNode,int (&property_to_node):[],COMMENT_TYPE &info) {
   ...
   */
   if (info.delim2!='' || info.startcol>0 || info.endcol>0 || 
       ...
       || !info.enabled
       || info.features!=''
       || info.start_substring_colors!=''
       || info.end_substring_colors!=''
...
static void _clex_add_item2(int handle,int attrs_node,COMMENT_TYPE (&comments)[],COMMENT_TYPE comment) {
   ...
   temp=_xmlcfg_get_attribute(handle,attrs_node,'enabled');
   if (temp:==0 || temp:==1) comment.enabled= temp!=0;

   comment.features=_xmlcfg_get_attribute(handle,attrs_node,'features');
   comment.start_color=_xmlcfg_get_attribute(handle,attrs_node,'start_color');
   comment.start_substring_colors=_xmlcfg_get_attribute(handle,attrs_node,'start_substring_colors');
   comment.end_color=_xmlcfg_get_attribute(handle,attrs_node,'end_color');
   comment.end_substring_colors=_xmlcfg_get_attribute(handle,attrs_node,'end_substring_colors');
...
static void append_styles(_str &line) {
   ...
   for (;;) {
      if ((child.p_object==OI_CHECK_BOX || child.p_object==OI_RADIO_BUTTON) && child.p_value && child.p_name!='ctluseschema' && child.p_name!='ctlcolor_string_identifiers') {
...
static void cc_prepare_numbers_tab(_str lexername) {
   ...
   ctldote_float.p_enabled=ctld_exponent.p_enabled=ctlf_exponent.p_enabled=(ctldigit_float.p_value!=0);
...
static bool _find_color_string_identifiers(COMMENT_TYPE (&comments)[],_str feature,bool enabled, bool set_option_enabled=false) {
   len:=comments._length();
   for (i:=0;i<len;++i) {
      if (pos(feature,comments[i].features)) {
         if (set_option_enabled) {
            ctlcolor_string_identifiers.p_enabled=true;
            ctlcolor_string_identifiers.p_value= comments[i].enabled?1:0;
            return true;
         }
         comments[i].enabled=enabled;
      }
      if(_find_color_string_identifiers(comments[i].comments,feature,enabled,set_option_enabled)) return true;
   }
   return false;
}
static void cc_prepare_language_tab(_str lexername) {
   ...
   CLEXDEF *pclexdef= &gnew_keyword_table:[_ctllexer_list.p_text];
   //COMMENT_TYPE *pcomment=get_item_pointer();
   ctlcolor_string_identifiers.p_enabled=false;

   _find_color_string_identifiers(pclexdef->comments,'color-string-identifiers',true,true);
...
static void cc_set_token_enable() {
   ...
   ctlstart_substring_colors.p_enabled=enable;
   ctlend_color.p_enabled=ctlend_color_label.p_enabled=enable;
   ctlend_substring_colors.p_enabled=enable;
...
static void cc_enabled_multiline(COMMENT_TYPE *pcomment) {
   ...
   ctlend_substring_colors.p_enabled=has_end;
...
static void cc_prepare_token() {
   ...
   ctltoken_enabled.p_value=pcomment->enabled?1:0;
   ...
   ctlrepeat_after.p_value=(pcomment->flags&CLEXMF_REPEAT_AFTER);
...
void _init_comment(COMMENT_TYPE &comment) {
   ...
   comment.features='';
   comment.start_color='';
   comment.start_substring_colors='';
   comment.end_color='';
   comment.end_substring_colors='';
...
int terraform_proc_search(_str &proc_name,bool find_first)
{
   _str re_map:[];
   re_map:["NAME"] = '[^#/{}\[\]=<]#';
   return _generic_regex_proc_search('^<<<NAME>>>[ \t]@\{', proc_name, find_first!=0, "label", re_map);
...
defeventtab _substring_colors_form;
...
void ctlok.on_create(_str substring_colors='',bool allow_regex=true) {
   colSlice := ctltree.p_width intdiv 2;

   ctltree._TreeSetColButtonInfo(0, colSlice, TREE_BUTTON_PUSHBUTTON, -1, 'Color');
   ctltree._TreeSetColButtonInfo(1, colSlice, TREE_BUTTON_PUSHBUTTON, -1, 'Start,Length or Tagged Expresssion');
   if (!allow_regex) {
      ctladdtaggedexpression.p_enabled=false;
   }
   for (;;) {
      options:=parse_file(substring_colors,false);
      if (options=='') {
         break;
      }
      parse options with auto color ',' auto rest;
      color=_cc_convert_to_dlg_color_name(color);
      ctltree._TreeAddItem(TREE_ROOT_INDEX, color"\t"rest, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   }
   enableDisableButtons();
}
void ctladdstart.lbutton_up() {
   result:=show('-modal _substring_start_length_form','Keyword',0,1,false);
   if (result=='') return;
   ctltree._TreeAddItem(TREE_ROOT_INDEX, _param1"\t"_param2, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
}

void ctladdtaggedexpression.lbutton_up() {
   result:=show('-modal _substring_start_length_form','Keyword',0,0,true);
   if (result=='') return;
   ctltree._TreeAddItem(TREE_ROOT_INDEX, _param1"\t"_param2, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
}
void ctldelete.lbutton_up()
{
   treeWid := _control ctltree;
...
static void resizeTreeColumns()
{
   ...
   ctltree._TreeSetColButtonInfo(0, colSlice);
   ctltree._TreeSetColButtonInfo(1, colSlice);
}
void _substring_colors_form.on_resize()
{
   ...
   // determine padding
   padding_x := ctltree.p_x;
   padding_y := ctltree.p_y;

   widthDiff := width - (ctltree.p_x_extent + padding_x);
   heightDiff := height - (ctlcancel.p_y_extent + padding_y);
   ...
   ctlok.p_y+=heightDiff;ctladdstart.p_y+=heightDiff;ctldelete.p_y+=heightDiff;
   ctlcancel.p_y+=heightDiff;ctladdtaggedexpression.p_y+=heightDiff;
}
static void getTreeLineData(int treeWid, int index, _str &dlg_color, _str &rest)
{
   // get the caption, then parse out what is inside
   caption := treeWid._TreeGetCaption(index);
   parse caption with dlg_color "\t" rest;
}
void ctlok.lbutton_up() {
   result:='';
   index:=ctltree._TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   while (index > 0) {
      getTreeLineData(ctltree, index, auto dlg_color, auto rest);

      if (result=='') {
         result=_cc_convert_from_dlg_color_name(dlg_color)','rest;
      } else {
         strappend(result,' '_cc_convert_from_dlg_color_name(dlg_color)','rest);
      }
      index = ctltree._TreeGetNextSiblingIndex(index);
   }
   _param1=result;
   p_active_form._delete_window(1);
}

defeventtab _substring_start_length_form;
void ctlok.on_create(_str dlg_color='Comment',int start_offset=0,int len=0,bool is_tagged_expression=true) {
   ctlcolor.cc_fill_color_combo(false);
   ctlcolor.p_text=dlg_color;
   if (is_tagged_expression) {
      ctlstartlabel.p_caption='Tagged Expression:';
      ctlstart.p_text=start_offset;
      ctllengthlabel.p_visible=false;
      ctllength.p_visible=false;
   } else {
      ctlstart.p_text=start_offset;
      ctllength.p_text=len;
   }
}
void ctlok.lbutton_up() {
   if (!isinteger(ctlstart.p_text)) {
      _message_box('expecting integer');
      ctlstart._set_focus();
      return;
   }
   if (ctllength.p_visible) {
      if (!isinteger(ctllength.p_text)) {
         _message_box('expecting integer');
         ctllength._set_focus();
         return;
      }
      _param1=ctlcolor.p_text;
      _param2=ctlstart.p_text','ctllength.p_text;
      p_active_form._delete_window(1);
      return;
   }
   _param1=ctlcolor.p_text;
   _param2=ctlstart.p_text;
   p_active_form._delete_window(1);
}
