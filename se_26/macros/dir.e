...
_command dir,ls(_str path='') name_info(FILENOAUTODIR_ARG'*,'VSARG2_REQUIRES_MDI)
{
  ...
  // Since we set p_DocumentName, it's best to clear the buffer name
  p_buf_name='';
...
void _process_cd(_str param,_str idname=null,bool allow_cd_in_terminals=true)
{
   ...
   param=strip(param,"B","\"");
   _str hide='';
   if ((idname==null || idname=='') && !def_show_build_cd) {
      hide=_chr(2);
   }
   if ( isdrive(substr(param,1,2)) ) {
      if (_win32s()) {
         ...
         concur_command(hide:+'cd 'param,false,true,false,true,idname);
      } else {
         ...
         concur_command(hide:+drive' & cd 'param,false,true,false,true,idname);
      }
   } else {
      param=_maybe_quote_filename(param);
      if (pos('&',param,1) && substr(param,1,1)!='"') {
