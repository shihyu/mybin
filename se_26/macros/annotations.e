...
static void showFields (int (&noteIndices)[])
{
   ...
   nOfCols := _annotation_tree._TreeGetNumColButtons();
   for (i := 0; i < nOfCols; ++i) {
...
//                                        TREE_BUTTON_SORT, 0, 'Status');
   ...
   for (i = 0; i < noteIndices._length(); ++i) {
      ...
      if (activeAnnotations._indexin(j)) {
         ...
         for (fieldsCounter := 0; fieldsCounter < nOfFields; ++fieldsCounter) {
            ...
            fieldEntry := "";
            fieldInfo := annotations[j].fields[k];
            switch (fieldInfo.fieldType) {
            case TEXT_CONTROL_TYPE:
               if (!fieldInfo.text._isempty()) {
                  fieldEntry = fieldInfo.text;
               }
               break;
            case DATE_CONTROL_TYPE:
               if (!fieldInfo.defaultDate._isempty()) {
                  fieldEntry = fieldInfo.defaultDate;
               }
               break;
            case MULTILINE_TEXT_CONTROL_TYPE:
               if (!fieldInfo.editor._isempty()) {
                  fieldEntry = "";
                  fieldEntry = fieldInfo.editor[0];
               }
               break;
            case DROPDOWN_CONTROL_TYPE:
               if (fieldInfo.defaultIndices._length() > 0) {
                  index := fieldInfo.defaultIndices[0];
                  fieldEntry = fieldInfo.dropdown[index];
               }
               break;
            case LIST_CONTROL_TYPE:
               for (l := 0; l < fieldInfo.defaultIndices._length(); ++l) {
                  index := fieldInfo.defaultIndices[l];
                  fieldEntry =
                  fieldEntry:+fieldInfo.list[index]", ";
               }
               fieldEntry = substr(fieldEntry, 1, (length(fieldEntry) - 2));
               break;
            case CHECKBOX_CONTROL_TYPE:
               if (fieldInfo.checkbox == "true") {
                  fieldEntry = "true";
               } else {
                  fieldEntry = "false";
               }
   ...
   sortCol := _GetDialogInfoHt(ANNOTATION_TREE_SORT_COLUMN);
   if ( sortCol!=null ) {
   ...
   if (noteIDX >= 0 && noteIDX < annotations._length()) {
      ...
      _note_preview.p_text = "<em>No annotation selected.</em>";
   }
...
static void buildCustomFields (_str fieldName)
{
   ...
   noteCount := types:[lowcase(type)]._length();
   for (i := 0; i < noteCount; ++i) {
      ...
      for (k := 0; k < fieldCount; ++k) {
         ...
         if (fieldInfo.name == fieldName) {
            switch (fieldInfo.fieldType) {
            case TEXT_CONTROL_TYPE:
               content = fieldInfo.text;
               break;
            case DATE_CONTROL_TYPE:
               content = fieldInfo.defaultDate;
               break;
            case MULTILINE_TEXT_CONTROL_TYPE:
               content = fieldInfo.editor[0];
               break;
            case DROPDOWN_CONTROL_TYPE:
               content = fieldInfo.dropdown[0];
               break;
            case LIST_CONTROL_TYPE:
               content = fieldInfo.list[0];
               break;
            case CHECKBOX_CONTROL_TYPE:
               content = fieldInfo.checkbox;
               break;
            }
...
void filterAnnotations (int (&filteredAnnotations)[])
{
   ...
   for (i := 0; i < noteCount; ++i) {
      ...
      for (k := 0; k < fieldCount; ++k) {
         ...
         if (fieldInfo.name == fieldName) {
            _str yyyy, mm, dd;
            parse fieldInfo.defaultDate with yyyy "-" mm "-" dd;
...
void _annotation_tree.rbutton_up,context()
{
   menuName := "_annotation_tree_menu";
   idx := find_index(menuName, oi2type(OI_MENU));
   if (!idx) {
      return;
   }
   ...
   x := VSDEFAULT_INITIAL_MENU_OFFSET_X;
   y := VSDEFAULT_INITIAL_MENU_OFFSET_Y;
   x = mou_last_x('M') - x;
   y = mou_last_y('M') - y;
