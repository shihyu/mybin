...
struct PROCESS_COMPLETION_LIST_TYPE {
   _str m_list_type; //fileextlist,dirs,files
   _str m_fileextlist;
   bool m_repeat;
};
struct PROCESS_COMPLETION_OPTIONS {
   bool m_firstarg;
   bool m_nospace;
   _str m_match;
   _str m_match_options;  // r=regex l=perl_regex i=ignore_case
   PROCESS_COMPLETION_LIST_TYPE m_list[];
};
static void _process_arg_init(PROCESS_COMPLETION_OPTIONS &info) {
   info.m_firstarg=false;
   info.m_nospace=false;
   info.m_match='';
   info.m_match_options='';
   info.m_list._makeempty();
}
static void _parse_process_command_completion_info(int profile_handle,int node,PROCESS_COMPLETION_OPTIONS (&arg_info)[]) {
   arg_info._makeempty();
   while (node>=0) {
      PROCESS_COMPLETION_OPTIONS info;
      _process_arg_init(info);
      _str flags,match,match_flags;
      flags=_xmlcfg_get_attribute(profile_handle,node,'flags');
      if (pos(' firstarg ',' 'flags' ')) info.m_firstarg=true;
      if (pos(' nospace ',' 'flags' ')) info.m_nospace=true;
      info.m_match=_xmlcfg_get_attribute(profile_handle,node,'match');
      match_flags=_xmlcfg_get_attribute(profile_handle,node,'match_flags');
      if (pos(' regex ',' 'match_flags' ')) strappend(info.m_match_options,'r');
      if (pos(' ignore_case ',' 'match_flags' ')) strappend(info.m_match_options,'i');
      if (pos(' perl_regex ',' 'match_flags' ')) strappend(info.m_match_options,'l');
      child:=_xmlcfg_get_first_child(profile_handle,node,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END);
      if (child>=0) {
         while (child>=0) {
            name:=_xmlcfg_get_name(profile_handle,child);
            if (name=='fileextlist' || name=='dirs' || name=='files') {
               PROCESS_COMPLETION_LIST_TYPE lt_info;
               lt_info.m_list_type=name;
               lt_info.m_fileextlist='';
               lt_info.m_repeat=false;
               if (_xmlcfg_get_attribute(profile_handle,child,'repeat',0)) {
                  lt_info.m_repeat=true;
               }
               if (name=='fileextlist') {
                  lt_info.m_fileextlist=_xmlcfg_get_attribute(profile_handle,child,'exts');
               }
               info.m_list[info.m_list._length()]=lt_info;
            }
            child=_xmlcfg_get_next_sibling(profile_handle,child,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END);
         }
         if (info.m_list._length()) {
            arg_info[arg_info._length()]=info;
         }
      }
      node=_xmlcfg_get_next_sibling(profile_handle,node,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END);
   }
}
static void _get_process_command_completion_info(_str cur,PROCESS_COMPLETION_OPTIONS (&arg_info)[]) {
   int profile_handle=_plugin_get_profile(VSCFGPACKAGE_MISC,VSCFGPROFILE_PROCESS_COMPLETION);
   if (profile_handle<0) {
      return;
   }
   doc_element:=_xmlcfg_get_document_element(profile_handle);
   if (doc_element<0) {
      return;
   }
   node:=_xmlcfg_find_property(profile_handle,doc_element,cur,(_fpos_case=='')?1:0);

   if (node>=0) {
      node=_xmlcfg_get_first_child(profile_handle,node,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END);
      if (node>=0) {
         _parse_process_command_completion_info(profile_handle,node,arg_info);
      }
   }
   _xmlcfg_close(profile_handle);
}
static bool _match_process_completion_info(_str cmd,int cursor_col,PROCESS_COMPLETION_OPTIONS (&arg_info)[],PROCESS_COMPLETION_LIST_TYPE &info) {
   int j=1;
   int i=0;
   int argnum=0;
   PROCESS_COMPLETION_LIST_TYPE completion_for_next_argument[];
   for (;;) {
      // skip leading blanks
      while (j<=length(cmd) && substr(cmd,j,1)==' ') ++j;
      _str word;
      if (substr(cmd,j,1)=='"' || substr(cmd,j,1)=="'") {
         endch:=substr(cmd,j,1);
         i=pos(endch,cmd,j+1);
         if (!i) {
            i=length(cmd)+1;
            word=substr(cmd,j+1,i-j-1);
         } else {
            word=substr(cmd,j+1,i-j-1);
            ++i;
         }
         ...
         word=substr(cmd,j,i-j);
      }
      //say('word='word'> i='i);
      j=i;
      //say('cursor_col='cursor_col' i='i);
      if (argnum>0) {
         if (completion_for_next_argument._length()) {
            if (cursor_col<=i || completion_for_next_argument[0].m_repeat) {
               info=completion_for_next_argument[0];
               return true;
            }
            completion_for_next_argument._deleteel(0);
         }
         if (completion_for_next_argument._length()==0) {
            for (x:=0;x<arg_info._length();++x) {
               PROCESS_COMPLETION_OPTIONS ainfo=arg_info[x];
               info=ainfo.m_list[0];
               if ((!ainfo.m_firstarg || argnum==1) && 
                   (ainfo.m_match=='' || pos(ainfo.m_match,word,1,ainfo.m_match_options)>0) ) {
                   //say('matched one');
                   if (ainfo.m_nospace) {
                      if (cursor_col<=i) {
                         //say('h1 list_type='info.m_list_type' ext='info.m_fileextlist);
                         return true;
                      }
                      if (info.m_repeat) {
                         //say('h2 list_type='info.m_list_type' ext='info.m_fileextlist);
                         return true;
                      }
                      completion_for_next_argument=ainfo.m_list;
                      completion_for_next_argument._deleteel(0);
                   } else {
                      if (cursor_col+1<=i) {
                         //say('h3 list_type='info.m_list_type' ext='info.m_fileextlist);
                         return true;
                      }
                      if (info.m_repeat) {
                         //say('h4 list_type='info.m_list_type' ext='info.m_fileextlist);
                         return true;
                      }
                      completion_for_next_argument=ainfo.m_list;
                   }
...
_command void process_tab(_str autoCompleteAlreadyRunning="", typeless &autoCompleteWords=null) name_info(','VSARG2_LASTKEY|VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   ...
   if ( !command_state() && p_window_state:!='I') {
      ...
      _str cmd=_expand_tabsc(col,-1);
      cursor_col:=p_col-col+1;
      if (cursor_col<1) cursor_col=1;
      _str cur;
      parse cmd with cur .;
      PROCESS_COMPLETION_OPTIONS arg_info[];
      _get_process_command_completion_info(cur,arg_info);
      PROCESS_COMPLETION_LIST_TYPE info;
      gcompletion_extensions_re='';
      bool do_files=true;
      //say('len='arg_info._length()' cur='cur);
      if (arg_info._length() && _match_process_completion_info(cmd,cursor_col,arg_info,info)) {
         //say('info.m_list_type='info.m_list_type' exts='info.m_fileextlist);
         if (info.m_list_type=='fileextlist' && info.m_fileextlist!='') {
            _str exts=info.m_fileextlist;
            for (;;) {
               if (exts=='') break;
               ...
               if (ext!='') {
                  if (gcompletion_extensions_re=='') {
                     gcompletion_extensions_re=_escape_re_chars(ext,'r');
                  } else {
                     strappend(gcompletion_extensions_re,'|':+_escape_re_chars(ext,'r'));
                  }
               }
            }
            if (gcompletion_extensions_re!='') {
               gcompletion_extensions_re='.('gcompletion_extensions_re')';
            }
            strappend(gcompletion_extensions_re,'$');
         } else if (info.m_list_type=='dirs') {
            do_files=false;
            //say('do_files='do_files);
         }
      }
...
_command void interactive_load_selection(_str option='') name_info(','VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK) {
   ...
   save_pos(auto p);
   if (option=='F' && profileNames._length()) {
...
defeventtab _process_completion_form;

static bool PROCESS_COMPLETION_MODIFIED(...) {
   if (arg()) ctlcommands_label.p_user=arg(1);
   return ctlcommands_label.p_user;
}

void _process_completion_form_save_settings()
{
   PROCESS_COMPLETION_MODIFIED(false);
}

bool _process_completion_form_is_modified()
{
   return PROCESS_COMPLETION_MODIFIED();
}

bool _process_completion_form_apply()
{
   profile_handle:=_xmlcfg_create_profile(auto profile_node,VSCFGPACKAGE_MISC,VSCFGPROFILE_PROCESS_COMPLETION,VSCFGPROFILE_FPROCESS_COMPLETION_VERSION);
   child := ctltree._TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   while (child>=0) {
      getTreeLineData(ctltree,child,auto name,auto value);
      //_parse_process_command_completion_info(value,auto do_files,auto exts,auto repeat);
      //value=_make_process_command_options(value,auto do_files,auto exts,auto repeat);
      node:=_xmlcfg_add_property(profile_handle,profile_node,name,value);
      child=ctltree._TreeGetNextSiblingIndex(child);
   }
   _plugin_set_profile(profile_handle);
   _xmlcfg_close(profile_handle);
   return true;
}

_str _process_completion_form_export_settings(_str &file)
{
   return _plugin_export_profile(file,VSCFGPACKAGE_MISC,VSCFGPROFILE_PROCESS_COMPLETION);
}

_str _process_completion_form_import_settings(_str &file)
{
   error:=_plugin_import_profile(file,VSCFGPACKAGE_MISC,VSCFGPROFILE_PROCESS_COMPLETION);

   return error;
}


void ctltree.on_create()
{
   PROCESS_COMPLETION_MODIFIED(false);
   // set up the trees!
   resizeTreeColumns();
   /*colSlice := ctltree.p_width intdiv 8;

   ctltree._TreeSetColButtonInfo(0, colSlice*2, TREE_BUTTON_PUSHBUTTON, -1, 'Command');
   ctltree._TreeSetColButtonInfo(1, colSlice, TREE_BUTTON_PUSHBUTTON, -1, 'File/Dir');
   ctltree._TreeSetColButtonInfo(2, colSlice*5, TREE_BUTTON_PUSHBUTTON, -1, 'File Extensions');*/

   load_completion_into_tree();

   enableDisableButtons();
}
static void load_completion_into_tree()
{
   int profile_handle=_plugin_get_profile(VSCFGPACKAGE_MISC,VSCFGPROFILE_PROCESS_COMPLETION);
   if (profile_handle<0) {
      return;
   }
   doc_handle:=_xmlcfg_get_document_element(profile_handle);
   if (doc_handle<0) {
      return;
   }
   node:=_xmlcfg_get_first_child(profile_handle,doc_handle,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END);
   while (node>=0) {
      caption:='';
      name:=_xmlcfg_get_attribute(profile_handle,node,VSXMLCFG_PROPERTY_NAME);
      value:=_xmlcfg_get_attribute(profile_handle,node,VSXMLCFG_PROPERTY_VALUE);
      //arse_process_command_options(value,auto do_files,auto exts,auto repeat);

      caption=name"\t"value;

      ctltree._TreeAddItem(TREE_ROOT_INDEX, caption, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
      node=_xmlcfg_get_next_sibling(profile_handle,node,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END);
   }
}
...
{
   int selected[];
   ctltree._TreeGetSelectionIndices(selected);
...
void _process_completion_form.on_resize()
{
   ...
   // determine padding
   padding := ctlcommands_label.p_x;

   widthDiff := width - (ctledit.p_x_extent + padding);
   heightDiff := height - (ctltree.p_y_extent + padding);
   ...
   if (widthDiff) {
      ...
      // widen the trees
      ctltree.p_width += widthDiff;

      resizeTreeColumns();
   }
   ctltree.p_height += heightDiff;
}

static void resizeTreeColumns()
{
   colSlice := ctltree.p_width intdiv 5;

   ctltree._TreeSetColButtonInfo(0, colSlice, TREE_BUTTON_PUSHBUTTON, -1, 'Command');
   ctltree._TreeSetColButtonInfo(1, colSlice*4, TREE_BUTTON_PUSHBUTTON, -1, 'Completion Definition');
}
void ctltree.on_change(int reason)
{
   if (reason == CHANGE_SELECTED) {
      enableDisableButtons();
   }
}
static void getTreeLineData(int treeWid, int index, _str &name, _str &value)
{
   // get the caption, then parse out what is inside
   caption := treeWid._TreeGetCaption(index);
   parse caption with name "\t" value;
}
void ctledit.lbutton_up()
{
   getTreeLineData(ctltree,ctltree._TreeCurIndex(),auto name,auto value);
   _parse_process_command_completion_info(value,auto do_files, auto exts, auto repeat);
   //editExtensionlessFileSetting(AFTMT_PATTERN);
   status:=textBoxDialog(nls('Completion properties for %s',name),       // Form caption
                        0,             // Flags
                        8000,             // Use default textbox width
                        "", // Help item
                        nls("OK,Cancel:_cancel"),
                        '',
                        'File Extensions:'exts,
                        '-checkbox List files:'do_files,
                         '-checkbox (List files only) Repeat argument:'repeat
                        );
   if (status<0) {
      return;
   }
   ...
   _str caption;
   value=_make_process_command_options(do_files,exts,repeat);
   caption=name"\t"value;
   ctltree._TreeSetCaption(ctltree._TreeCurIndex(),caption);
   PROCESS_COMPLETION_MODIFIED(true);
}
void ctladd.lbutton_up()
{
   //editExtensionlessFileSetting(AFTMT_PATTERN);
   status:=textBoxDialog(nls('Add completion'),       // Form caption
                        0,             // Flags
                        8000,             // Use default textbox width
                        "", // Help item
                        nls("OK,Cancel:_cancel"),
                        '',
                         'Command:',
                        'File Extensions:',
                        '-checkbox List files:1',
                         '-checkbox (List files only) Repeat argument:0'
                        );
   if (status<0) {
      return;
   }
   ...
   if (exts!='') {
      strappend(caption,"("exts")");
   }
   ctltree._TreeAddItem(TREE_ROOT_INDEX, caption, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   enableDisableButtons();
   PROCESS_COMPLETION_MODIFIED(true);
}
static void deleteExtensionlessFileSetting(int treeWid)
{
...
void ctldelete.lbutton_up()
{
   deleteExtensionlessFileSetting(ctltree);
}
