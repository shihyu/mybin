...
{
   if (_file_types_get_wildcards_by_name(def_file_types,langLabel)=='') {
      def_file_types= _file_types_append_name_and_wildcards(def_file_types,langLabel,fileTypes);
...
static void updateDefFileTypesLabel(_str oldLabel, _str newLabel)
{
   ...
   _str text1, rest;
   def_file_types=_file_types_rename(def_file_types,oldLabel,newLabel);
   _config_modify_flags(CFGMODIFY_DEFVAR);
}
...
static void _c_update_settings() {
   ...
   if (_version_compare(UpdateVersion, old_getUpdateVersion('c')) > 0) {
      ...
      _str lt, rt;
      def_file_types=_file_types_change_wildcards_by_name_and_wildcards(
         def_file_types,"C/C++ Files","*.c;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.h;*.hh;*.hpp;*.hxx;h++;*.inl;*.xpm",
         "*.c;*.cc;*.cpp;*.cppm;*.cp;*.cxx;*.c++;*.h;*.hh;*.hpp;*.hxx;*.h++;*.inl;*.ixx;*.xpm;*.mpp;*.mxx");
      _config_modify_flags(CFGMODIFY_DEFVAR);
      def_find_file_types=_file_types_change_wildcards_by_name_and_wildcards(
         def_file_types,"C/C++ Files","*.c;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.h;*.hh;*.hpp;*.hxx;h++;*.inl;*.xpm",
         "*.c;*.cc;*.cpp;*.cppm;*.cp;*.cxx;*.c++;*.h;*.hh;*.hpp;*.hxx;*.h++;*.inl;*.ixx;*.xpm;*.mpp;*.mxx");
   }
 ...
 */
static void _UpgradeColorProfile()
{
   ...
   if (config_migrated_from_version != "") {
      ...
      */
      if (major<21) {
         ...
         _UpgradeColorProfile();
         _UpgradeSymbolColoringScheme(config_migrated_from_version);
         ...
         if (def_color_scheme != CONFIG_AUTOMATIC && !_plugin_has_profile(VSCFGPACKAGE_COLOR_PROFILES,def_color_scheme)) {
            def_color_scheme='Default';// Make sure the color profile exists
         }
         if (def_minimap_color_scheme != "" && 
             def_minimap_color_scheme != CONFIG_AUTOMATIC && 
             !_plugin_has_profile(VSCFGPACKAGE_COLOR_PROFILES,def_minimap_color_scheme)) {
            def_minimap_color_scheme="";// Make sure the color profile exists
         }
      }
   }

   if (major < 26) {
      // update Mac Standard toolbars
      _UpdateMacToolbars();
   }
