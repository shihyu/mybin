...
_str def_save_on_compile='1 0';//0=no save, 1=save current file, 2=save all files
                               //compile make
_str def_clear_build_immediate=false;
...
static bool using_dart(_str command)
{
   _str temp=_replace_envvars2(command);
   pgmname := _strip_filename(parse_file(temp,false),'PE');
   if (_file_eq(pgmname,'dart')) {
      return true;
   }
   return false;
}

static bool using_tool(_str command,_str name1, ...)
{
   _str temp=_replace_envvars2(command);
   pgmname := _strip_filename(parse_file(temp,false),'PE');
   if (_file_eq(pgmname,name1)) {
      return true;
   }
   for (i:=3;i<arg();++i) {
      if (_file_eq(pgmname,arg(i))) {
         return true;
      }
   }
   return false;
                     ...
                     _str debugWorkingDir="")
{
   ...
   //
   {
      ...
      if (type!='dart' && (using_dart(name) || (buildFirst && using_dart(buildFirstCommand)))) {
         if (set_dart_environment(buildFirst?buildFirstCommand:name)) {
            _message_box("Dart not found.\n\nSet the Dart location (\"Build\",\"Dart Options\")");
            return 1;
         }
      }
      if (using_tool(name,'julia') || (buildFirst && using_ant(buildFirstCommand))) {
         if (set_julia_environment(buildFirst?buildFirstCommand:name)) {
            _message_box("julia executable not found");
            return 1;
         }
      }
   ...
   if (CaptureOutputWith_ProcessBuffer &&
      ...
      if (def_clear_build_immediate) {
         _mdi.p_child.clear_pbuffer();
      } else {
         if (_isUnix()) {
            concur_command('echo -e "\001clear-pbuffer"');
         } else {
            concur_command("echo \1clear-pbuffer");
         }
      }
      //_mdi.p_child.clear_pbuffer();
   }
                                    ...
                                    int handle)
{
   ...
   if (!def_show_build_vslickerrorpath) {
      _maybe_append(options, ' ');
      options :+= "-hidevep";
   }
 ...
 * otherwise have mismatched parenthesis.</dd> 
 * <dt>%`shell command`</dt>
 * <dd>Run the given command and return result from standard output.  Example:
 *     %`"C:\Program Files (x86)\Microsoft Visual Studio\Installer\vswhere.exe" -version 16.0 | findstr productPath`</dd>
...
static void _project_convert_command(_str &command,_str buf_name='',_str field_name='',int handle=-1,_str config='')
{
   ...
   case 'cppclean':
      ...
      command=_maybe_quote_filename(get_env('VSLICKBIN1')'vsbuild')vsbuild_options' -t "%t" "%w" "%r" -c "%bn"';
      break;
   case 'antmake':
      command=_maybe_quote_filename(get_env('VSLICKBIN1')'vsbuild')vsbuild_options' -t "%t" "%w" "%r" -c "%bn"';
      ...
      break;
   case 'javamakejar':
      command=_maybe_quote_filename(get_env('VSLICKBIN1')'vsbuild')vsbuild_options' -t "%t" "%w" "%r" -c "%bn" -execmacro make_jad %r';
      break;
   case 'vstudiocompile':
      if (buf_name!='') {
      ...
      if (!alreadyUsing_vsbuild(command) && !strieq(_ProjectGet_BuildSystem(handle), "automakefile")) {
         if ((field_name=='build' || field_name=='rebuild')) {
            ...
            if (PreBuildCommandsNode>=0 || PostBuildCommandsNode>=0) {
               command=_maybe_quote_filename(get_env('VSLICKBIN1')'vsbuild')vsbuild_options' "%t" "%w" "%r" -c "%bn"';
